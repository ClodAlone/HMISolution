    ...
    public class TcpClientChannel : TcpChannel
    {
        ...
        {            
            ...
            try
            {
                ...
                BeginWriteMessage(new ArraySegment<byte>(buffer, 0, size), m_timeout, operation);
                buffer = null;
            }
            finally
            {
        ...
        private void SendOpenSecureChannelRequest(bool renew)
        {
            ...
            try
            {
                BeginWriteMessage(chunksToSend, m_timeout, m_handshakeOperation);
                chunksToSend = null;
            }
            finally
            {
            ...
            try
            {
                ...
                lock (DataLock)
                {
                    ...
                    if (token == CurrentToken)
                    {
                        ...
                        // begin the operation.
                        m_handshakeOperation = BeginOperation(m_timeout, m_HandshakeComplete, token);
                    ...
                    // create an operation.
                    m_handshakeOperation = BeginOperation(m_timeout, m_HandshakeComplete, null);
        ...
        private void OnHandshakeComplete(IAsyncResult result)
        {
            lock (DataLock)
            {
                try
                {
                    ...
                    m_handshakeOperation.End(m_timeout);
                    m_handshakeOperation = null;
                    m_reconnecting = false;
                }
                catch (Exception e)
