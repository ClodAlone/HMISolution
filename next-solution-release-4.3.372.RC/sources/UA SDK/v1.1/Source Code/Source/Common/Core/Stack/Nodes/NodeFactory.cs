...
namespace Opc.Ua
{
    ...
    public class NodeFactory
    {
        ...
        private static CreationRule GetCreationRule(NodeId modellingRule)
        {
            ...
            if (modellingRule == Objects.ModellingRule_CardinalityRestriction || modellingRule == Objects.ModellingRule_ExposesItsArray)
            {
            ...
            ushort                       namespaceIndex)
        {
            ...
            foreach (HierarchyBrowsePath browsePath in hierarchy.BrowsePaths.Values)
            {
                ...
                if (!updateReferences)
                {
                    ILocalNode parent = instance as ILocalNode;

                    while (parent != null)
                    {
                        ExpandedNodeId parentId = parent.References.FindTarget(ReferenceTypeIds.HasModelParent, false, false, null, 0);
                        ...
                        parent = m_nodes.Find(parentId) as ILocalNode;
                    }

                    // nothing more to do if not owned by the root.
                    if (parent == null)
                    {
                        continue;
                    }
                ...
                else
                {
                    ...
                    if (reference.ReferenceTypeId.IdType == IdType.Numeric && reference.ReferenceTypeId.NamespaceIndex == 0)
                    {
                        switch ((uint)reference.ReferenceTypeId.Identifier)
                        {
                            ...
                            case ReferenceTypes.HasModelParent:
                            {
                                singletonReferenceType = true;
                                break;
                            }
            ...
            InstanceDeclarationHierarchy hierarchy)
        {
            ...
            // check if following the model parent path.
            ExpandedNodeId modelParent = instance.References.FindTarget(ReferenceTypeIds.HasModelParent, false, false, null, 0);

            if (modelParent == parent.DeclarationId)
            {
                child.IsModelParent = true;

                // child is optional if parent is optional.
