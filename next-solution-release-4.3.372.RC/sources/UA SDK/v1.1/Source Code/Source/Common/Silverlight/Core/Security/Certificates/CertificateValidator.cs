    ...
    public class CertificateValidator
    {
        ...
        protected virtual void InternalValidate(X509Certificate2 certificate)
        {
            lock (m_lock)
            {
                ...
                // check for explicitly denied certificates.
                if (trustedCertificate != null)
                {
                    if ((trustedCertificate.ValidationOptions & CertificateValidationOptions.TreatAsInvalid) != 0)
                    {
                ...
                foreach (CertificateIdentifier issuer in issuers)
                {
                    ...
                    // check for explicitly denied certificates.
                    if ((issuer.ValidationOptions & CertificateValidationOptions.TreatAsInvalid) != 0)
                    {
                        throw ServiceResultException.Create(
                            StatusCodes.BadCertificateInvalid,
                            "Issuer certificate is not valid.\r\nSubjectName: {0}",
                            issuer.SubjectName);
                    }
