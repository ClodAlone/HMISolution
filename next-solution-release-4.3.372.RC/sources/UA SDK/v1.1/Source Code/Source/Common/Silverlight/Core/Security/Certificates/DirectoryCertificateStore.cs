    ...
    public class DirectoryCertificateStore : ICertificateStore
    {
        ...
        public void Open(string location)
        {
            ...
            {   
                ...
                m_directory = m_certificateSubdir = m_privateKeySubdir = new DirectoryInfo(location);
                
                // check for an OpenSSL directory structure.
                if (m_directory.Exists)
                {
                    ...
                    // create the private key directory if the public key directory exists.
                    ...
                    {
                        // create the public key directory if the private key directory exists.
                        if (m_privateKeySubdir.Exists)
                        {
                            m_certificateSubdir.Create();
                        }
                            
                        // use the root directory for both private and public keys.
                        else
                        {
        ...
        private IDictionary<string, Entry> Load(string thumbprint)
        {
            lock (m_lock)
            {
                ...
                if (!m_certificateSubdir.Exists || !m_privateKeySubdir.Exists)
                {
                    m_certificates.Clear();
                ...
                if (m_certificateSubdir.LastWriteTimeUtc < m_lastDirectoryCheck && this.m_privateKeySubdir.LastWriteTimeUtc < m_lastDirectoryCheck)
                {
                    return m_certificates;
                }
        ...
        private void WriteFile(byte[] data, string fileName, bool includePrivateKey)
        {
            ...
            // convert a flat directory to a certificate store on write.
            if (!m_certificateSubdir.FullName.EndsWith("\\certs"))
            {
                DirectoryInfo subdir = new DirectoryInfo(m_certificateSubdir.FullName + "\\certs");
                subdir.Create();
                ...
                m_privateKeySubdir = new DirectoryInfo(m_certificateSubdir.FullName + "\\private");
                m_privateKeySubdir.Create();

                m_certificateSubdir = subdir;
            }
