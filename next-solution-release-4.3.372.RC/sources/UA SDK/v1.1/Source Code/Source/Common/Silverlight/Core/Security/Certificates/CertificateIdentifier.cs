...
namespace Opc.Ua
{
    ...
    {    
        ...
        public X509Certificate2 Find(bool needPrivateKey)
        {
            ...
            else
            {
                ...
                using (ICertificateStore store = CertificateStoreIdentifier.CreateStore(StoreType))
                {
                    ...
                    certificate = Find(store.Enumerate(), needPrivateKey);
        ...
        private X509Certificate2 Find(X509Certificate2Collection collection, bool needPrivateKey)
        {
            #if !SILVERLIGHT
            // find by thumbprint.
            if (!String.IsNullOrEmpty(m_thumbprint))
            {
                collection = collection.Find(X509FindType.FindByThumbprint, m_thumbprint, false);
                ...
                foreach (X509Certificate2 certificate in collection)
                {
                    if (!needPrivateKey || certificate.HasPrivateKey)
                    {
                        ...
                        List<string> subjectName = Utils.ParseDistinguishedName(m_subjectName);

                        if (Utils.CompareDistinguishedName(certificate, subjectName))
                        {
                            return certificate;
                        }
                    }
            ...
            if (!String.IsNullOrEmpty(m_subjectName))
            {
                ...
                collection = collection.Find(X509FindType.FindBySubjectName, m_subjectName, false);
