...
namespace Opc.Ua
{
    ...
    public class CertificateValidator
    {
        ...
        public bool GetIssuers(X509Certificate2 certificate, List<CertificateIdentifier> issuers)
        {
            ...
            do
            {
                issuer = GetIssuer(certificate, m_trustedCertificateList, m_trustedCertificateStore, false);

                if (issuer == null)
                {
                    issuer = GetIssuer(certificate, m_issuerCertificateList, m_issuerCertificateStore, false);
                }
                else
                {
                    isTrusted = true;
