...
namespace SnmpSharpNet
{
	...
	public class AuthenticationSHA1 : IAuthenticationDigest
	{
        ...
        public bool authenticateIncomingMsg(byte[] userPassword, byte[] engineId, byte[] authenticationParameters, byte[] wholeMessage)
        {
            byte[] authKey = PasswordToKey(userPassword, engineId);
            HMACSHA1 sha = new HMACSHA1(authKey);
            byte[] hash = sha.ComputeHash(wholeMessage);
            byte[] myhash = new byte[12];
            Array.Copy(hash, myhash, 12);
            sha.Clear(); // release resources
        ...
        public bool authenticateIncomingMsg(byte[] authKey, byte[] authenticationParameters, byte[] wholeMessage)
        {
            HMACSHA1 sha = new HMACSHA1(authKey);
            byte[] hash = sha.ComputeHash(wholeMessage);
            byte[] myhash = new byte[12];
