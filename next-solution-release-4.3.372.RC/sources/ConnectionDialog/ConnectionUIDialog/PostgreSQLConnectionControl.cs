...
{
	public partial class PostgreSQLConnectionUIControl : UserControl, IDataConnectionUIControl
	{
		public PostgreSQLConnectionUIControl()
		{
			InitializeComponent();
		...
		public void Initialize(IDataConnectionProperties connectionProperties)
		{
			...
			if (!(connectionProperties is PostgreSQLConnectionProperties))
			{
				throw new ArgumentException(Strings.PostgrSQLConnectionUIControl_InvalidConnectionProperties);
			}
		...
		public void LoadProperties()
		{
			...
			if (Properties.Contains(ServerProperty) &&
				Properties[ServerProperty] is string)
			{
				serverTextBox.Text = Properties[ServerProperty] as string;
			}
			if (Properties.Contains(DatabaseProperty) &&
				Properties[DatabaseProperty] is string)
			{
				DatabaseTextBox.Text = Properties[DatabaseProperty] as string;
			}
			if (Properties.Contains(UserNameProperty) &&
				Properties[UserNameProperty] is string)
			{
				userNameTextBox.Text = Properties[UserNameProperty] as string;
			}
			if (Properties.Contains(PasswordProperty) &&
				Properties[PasswordProperty] is string)
			{
        ...
        private void SetDatabase(object sender, EventArgs e)
        {
            if (!_loading)
            {
                Properties[DatabaseProperty] = (DatabaseTextBox.Text.Trim().Length > 0) ? DatabaseTextBox.Text.Trim() : null;
            }
        }
