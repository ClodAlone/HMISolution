 ...
 descriptions,
        ...
        public static DataProvider MySQLDataProvider
        {
            get
            {
                ...
                {
                    Dictionary<string, string> descriptions = new Dictionary<string, string>();
                    descriptions.Add(DataSource.MySQLDataSource.Name, Strings.DataProvider_MySQL_DataSource_Description);

                    Dictionary<string, Type> uiControls = new Dictionary<string, Type>();
                    uiControls.Add(String.Empty, typeof(MySQLConnectionUIControl));

                    _mysqlDataProvider = new DataProvider(
                        "MySql.Data.MySqlClient",
                        Strings.DataProvider_MySQL,
                        Strings.DataProvider_MySQL_Short,
                        Strings.DataProvider_MySQL_Description,
                        typeof(MySql.Data.MySqlClient.MySqlConnection),
                        descriptions,
                        uiControls,
                        typeof(MySQLConnectionProperties));
                }
                return _mysqlDataProvider;
            }
        }
        private static DataProvider _mysqlDataProvider;
        ...
        public static DataProvider SQLiteDataProvider
        {
            get
            {
                ...
                {
                    Dictionary<string, string> descriptions = new Dictionary<string, string>();
                    descriptions.Add(DataSource.SQLiteDataSource.Name, Strings.DataProvider_SQLite_DataSource_Description);
                    ...
                    _sqliteDataProvider = new DataProvider(
                        "System.Data.SQLite",
                        Strings.DataProvider_SQLite,
                        Strings.DataProvider_SQLite_Short,
                        Strings.DataProvider_SQLite_Description,
		...
		public static DataProvider PostgreSQLDataProvider
		{
			get
			{
				if (_postgreSQLDataProvider == null)
				{
					Dictionary<string, string> descriptions = new Dictionary<string, string>();
					descriptions.Add(DataSource.PostgreSQLDataSource.Name, Strings.DataProvider_PostgreSQL_DataSource_Description);

					Dictionary<string, Type> uiControls = new Dictionary<string, Type>();
					uiControls.Add(String.Empty, typeof(PostgreSQLConnectionUIControl));

					_postgreSQLDataProvider = new DataProvider(
						"PostgreSQL.Data.PostgreSQLClient",
						Strings.DataProvider_PostgreSQL,
						Strings.DataProvider_PostgreSQL_Short,
						Strings.DataProvider_PostgreSQL_Description,
						typeof(Npgsql.NpgsqlConnection),
						descriptions,
						uiControls,
						typeof(PostgreSQLConnectionProperties));
				}
				return _postgreSQLDataProvider;
			}
		}
		private static DataProvider _postgreSQLDataProvider;
