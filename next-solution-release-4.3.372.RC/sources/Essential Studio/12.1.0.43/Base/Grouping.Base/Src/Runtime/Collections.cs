				...
				if (allowCache && cachedElement != null && version == _table.version)
				{
					...
					else  
					{
						...
						else if (/*_table.FilteredChildTable == null && */adjustedTableIndex == cachedAdjustedTableIndex+cachedElementCount)
						{
							...
							cachedElement = ElementHelper.GetNextElementStepIn(cachedElement, CounterKind.CountDisplayElements, null, this._stepInNestedTables, this._table);
							...
							if (false)
							{
								...
								if (testIndexOf == -1 || testIndexOf != index)
								{
									...
									cachedElement = ElementHelper.GetNextElementStepIn(elx, CounterKind.CountDisplayElements, null, this._stepInNestedTables, this._table);
								}
...
//				value = ElementHelper.GetNextElementStepIn(value, CounterKind.CountDisplayElements, null, _stepInNestedTables);
	...
	public class DisplayElementsInTableCollectionEnumerator : IEnumerator 
	{
		...
		public bool MoveNext()
		{
			...
			{
				_next = ElementHelper.GetNextElementStepIn(_next, CounterKind.CountDisplayElements, null, _coll._stepInNestedTables, _coll._table);
