    ...
    public static class PropertyDescriptorExtensions
    {
        ...
        internal static KeyValuePair<PropertyDescriptor, Object> GetPropertyDescriptor(this PropertyDescriptorCollection pdc, object record, string columnName)
        {
            ...
            if (pd != null)
            {
                ...
                {
                    IList array = pd.GetValue(tRecord) as IList;
                    //return array[index];
        ...
        public static PropertyDescriptor GetPropertyDescriptor(this PropertyDescriptorCollection pdc, string columnName)
        {
            ...
            if (ColName.Count() > 0)
            {
                var ActualProperty = ColName.ElementAt(0).Substring(ColName.ElementAt(0).IndexOf('[') + 1, ColName.ElementAt(0).IndexOf(']') - ColName.ElementAt(0).IndexOf('[') - 1);
                int temp;
                if (Int32.TryParse(ActualProperty, out temp))
                    IsDictionary = false;
                else
                    IsDictionary = true;
            }
