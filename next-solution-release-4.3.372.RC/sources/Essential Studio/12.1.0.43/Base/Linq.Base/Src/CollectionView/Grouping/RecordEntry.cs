...
namespace Syncfusion.Windows.Data
{
    ...
    public class RecordEntry : NodeEntry, IComparable<RecordEntry>
    {
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="Syncfusion.Windows.Data.RecordEntry"/> class.
        /// </summary>
        ...
        public object Data
        {
            ...
            set
            {
                ...
                if (value is DataRow)
                {
                    throw new InvalidOperationException("Cannot set DataRow as Data, Try setting DataRowView");
                }
#endif
                this.data = value;
            }
    ...
    public class ListIndexer<T> where T : RecordEntry, IComparable<T>, IDisposable
    {
        ...
        public int Find(object item)
        {
            ...
            if (internalList.Count > 0)
            {
                ...
                int mid = 0;
                int prevMid = 0;
                var compareHashCode = item.GetHashCode();
                while (top <= bot && c != 0)
                {
                    mid = (top + bot) / 2;
                    var data = internalList[mid].Data;
                    c = data.GetHashCode().CompareTo(compareHashCode);
                    if (c > 0)
                    {
                        prevMid = mid;
                        bot = mid - 1;
    ...
    public class RecordsEntryList : IRecordsEntryList
    {
        ...
        {
            this.internalList.Insert(index, (RecordEntry)item);
        }
        ...
        {
            this.internalList.Add((RecordEntry)item);
        }
