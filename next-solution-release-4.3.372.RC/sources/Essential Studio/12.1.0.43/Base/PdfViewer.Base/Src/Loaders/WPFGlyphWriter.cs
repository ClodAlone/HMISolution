...
namespace Syncfusion.Windows.PdfViewer
{
    class WPFGlyphWriter
    {
        ...
        private System.Windows.Media.PathFigure Path;
        List<System.Windows.Media.PathFigure> m_tempPaths = new List<System.Windows.Media.PathFigure>();
        private List<System.Windows.Media.PathFigure> m_subPaths = new List<System.Windows.Media.PathFigure>();
        private List<System.Windows.Media.PathFigure> m_tempSubPaths = new List<System.Windows.Media.PathFigure>();
        private System.Windows.Point CurrentLocation;
        PdfUnitConvertor m_convertor = new PdfUnitConvertor();
        public WPFGlyphWriter(Dictionary<string, byte[]> glyphCharSet, bool isC1)
        {
            glyphs = glyphCharSet;
            this.is1C = isC1;
        }
        ...
        internal System.Windows.Media.PathGeometry glyphParser(string glyphChar, int rawInt, float currentWidth)
        {
        ...
        private void EndChar(int rawInt)
        {
            ...
            //String bchar = StandardFonts.getUnicodeChar(StandardFonts.STD, (int)operands[currentOp + 2]);
            //String achar = StandardFonts.getUnicodeChar(StandardFonts.STD, (int)operands[currentOp + 3]);

            //x = 0;
            //y = 0;
            //glyphParser(bchar, rawInt, 0);
            //Path.CloseFigure();
            //BeginPath(new string[] { adx.ToString(), ady.ToString() });
            //x = adx;
            //y = ady;
            //glyphParser(achar, rawInt, 0);
        ...
        private int EndChar(int rawInt, int dicEnd)
        {
            ...
            if (operandReached == 4)
            {
                ...
                Path.IsClosed = true;//.CloseFigure();//factory.closePath();

            p = dicEnd;
            return p;
        }

        private System.Windows.Media.PathGeometry FillPath(string rule)
        {
            System.Windows.Media.PathGeometry geom = new System.Windows.Media.PathGeometry();
            if (m_tempPaths.Count > 0)
            {
                Path.IsClosed = true;
                System.Windows.Media.PathFigure fig = new System.Windows.Media.PathFigure();
                System.Windows.Media.PathFigureCollection col = new System.Windows.Media.PathFigureCollection();
                foreach (System.Windows.Media.PathFigure f in m_tempPaths)
                {
                    fig = new System.Windows.Media.PathFigure();
                    fig.StartPoint = f.StartPoint;
                    foreach (System.Windows.Media.PathSegment seg in f.Segments)
                    {
                        if (!fig.Segments.Contains(seg))
                            fig.Segments.Add(seg);
                    }
                    col.Add(fig);
                }
                fig = new System.Windows.Media.PathFigure();
                fig.StartPoint = Path.StartPoint;
                foreach (System.Windows.Media.PathSegment seg in Path.Segments)
                {
                    if (!fig.Segments.Contains(seg))
                        fig.Segments.Add(seg);
                }
                ...
                //foreach (PathFigure f in m_tempPaths)
                //{
                //    col.Add(f);
                //}
                ...
                if (rule == "EvenOdd")
                    ...
                    geom.FillRule = System.Windows.Media.FillRule.Nonzero;

                //PageGraphics.DrawGeometry(new System.Windows.Media.SolidColorBrush(System.Windows.Media.Color.FromArgb(255, 0, 0, 0)), null, geom);
            }
            else
            {
                if (Path != null)
                {
                    ...
                    {
                        foreach (System.Windows.Media.PathFigure path in m_subPaths)
                        {
                            try
                            {
                                foreach (System.Windows.Media.PathSegment segment in path.Segments)
                                {
                    ...
                    try
                    {
                        ...
                        //PageGraphics.DrawGeometry(new System.Windows.Media.SolidColorBrush(System.Windows.Media.Color.FromArgb(255, 0, 0, 0)), null, geom);
                        m_subPaths.Clear();
                        Path = new System.Windows.Media.PathFigure();
                    }
                    catch
                    {
                        Path = new System.Windows.Media.PathFigure();
        ...
        private void AddLine(string[] line)
        {
            Path.Segments.Add(
                new System.Windows.Media.LineSegment(
                    new System.Windows.Point(double.Parse(line[0]), -double.Parse(line[1]))
                    , true));

            CurrentLocation = new System.Windows.Point(float.Parse(line[0]), -float.Parse(line[1]));
        }

        private void AddBezierCurve(string[] curve)
        {
            System.Windows.Point point1 = new System.Windows.Point(CurrentLocation.X, CurrentLocation.Y);
            System.Windows.Point point2 = new System.Windows.Point(float.Parse(curve[0]), -float.Parse(curve[1]));
            System.Windows.Point point3 = new System.Windows.Point(float.Parse(curve[2]), -float.Parse(curve[3]));
            System.Windows.Point point4 = new System.Windows.Point(float.Parse(curve[4]), -float.Parse(curve[5]));


            Path.Segments.Add(
                new System.Windows.Media.BezierSegment(point2, point3, point4, true));

            CurrentLocation = new System.Windows.Point(float.Parse(curve[4]), -float.Parse(curve[5]));
        }

        private void AddBezierCurve2(string[] curve)
        {
            System.Windows.Point point1 = new System.Windows.Point(CurrentLocation.X, CurrentLocation.Y);
            System.Windows.Point point2 = new System.Windows.Point(CurrentLocation.X, CurrentLocation.Y);
            System.Windows.Point point3 = new System.Windows.Point(float.Parse(curve[0]), -float.Parse(curve[1]));
            System.Windows.Point point4 = new System.Windows.Point(float.Parse(curve[2]), -float.Parse(curve[3]));
        ...
        private void BeginPath(string[] point)
        {
            ...
            CurrentLocation = new System.Windows.Point(float.Parse(point[0]), -float.Parse(point[1]));
