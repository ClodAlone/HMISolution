...
namespace Syncfusion.Windows.Forms.Diagram
{
    ...
    internal class EmfParser : MetafileParser
    {
        ...
        internal EmfParser(EmfRenderer renderer)
            : base(renderer)
        { 
        }
        ...
        /// </summary>
        internal override event ParseEventHandler BeginParse;
        ...
        internal override void OnBeginParse(EventArgs e)
        {
            if (BeginParse != null)
            {
                BeginParse(this, e);
        ...
        {
            // TODO : if to be needed in future.
        }
        #endregion
        ...
        protected bool EnumerateMetafile(EmfPlusRecordType recordType, int flags, int dataSize,
            IntPtr ptrData, PlayRecordCallback callbackData)
        {
            try
            {
                switch (recordType)
        ...
        private void Header(byte[] data)
        {
            ...
            SizeF size = Renderer.Size;

            Matrix mtxGraphicsTransform = Renderer.Graphics.Transform;
            float scaleX = size.Width / bounds.Width;
            float scaleY = size.Height / bounds.Height;
            SizeF szOffset = new SizeF(mtxGraphicsTransform.OffsetX, mtxGraphicsTransform.OffsetY);

            Renderer.TranslateTransform(-szOffset.Width, -szOffset.Height, MatrixOrder.Append);
            Renderer.ScaleTransform(scaleX, scaleY, MatrixOrder.Append);
            Renderer.TranslateTransform(szOffset.Width, szOffset.Height, MatrixOrder.Append);

            PointF location = pixSize.Location;
            float x = Math.Abs(location.X);
            float y = Math.Abs(location.Y);

            if (location != PointF.Empty)
            {
                Renderer.TranslateTransform(-x, -y, MatrixOrder.Append);
            }
        }
        ...
        private void ModifyWorldTransform(IntPtr ptr)
        {
            ...
            bool result = GdiApi.ModifyWorldTransform(Objects.Handle, ref recordData.xform, (int)recordData.iMode);
            CheckResult(result);
        }
        ...
        private void ExtCreateFontIndirect(IntPtr ptr)
        {
            ...
            Font font = new Font(faceName, emHeight, fontStyle);
        ...
        private void ExtTextOut(IntPtr ptr, bool bIsUnicode)
        {
            ...
            {
                text = Encoding.ASCII.GetString(textData);
            }
        ...
        private Bitmap GetBitmap(int imageOffset, uint imgSize, IntPtr ptr, IntPtr bitmapInfoPtr, int iUsageSrc)
        {
        ...
        /// <param name="iUsageSrc">Usage of the pixels.</param>
        /// <returns>Bitmap image.</returns>
        private IntPtr GetHBitmap(int imageOffset, uint imgSize, IntPtr ptr, IntPtr bitmapInfoPtr, int iUsageSrc)
        {
            imageOffset -= (2 * IntSize);
            IntPtr imgPtr = new IntPtr(ptr.ToInt64() + imageOffset);
            byte[] imgData = new byte[imgSize];
            ...
            else if (font != null)
            {
                ...
                using (Brush brush = new SolidBrush(Objects.ForeColor))
                {
                    ...
                    else
                    {
                        ...
                        for (int i = 0, numChars = text.Length; i < numChars; ++i)
                        {
                            string t = text[i].ToString();

                            Renderer.DrawString(t, font, brush, pos, format);
                            pos.X += widths[i];
                        }
                    }
                }
