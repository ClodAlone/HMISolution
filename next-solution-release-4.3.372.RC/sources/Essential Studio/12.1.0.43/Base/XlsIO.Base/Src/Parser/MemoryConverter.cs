    ...
    public MemoryConverter( int iMemoryBlockSize )
    {
      ...
      if( iMemoryBlockSize > DEF_MAX_BLOCK_SIZE )
      {
        throw new ArgumentOutOfRangeException( "iMemoryBlock",
                                               "Value cannot be greater " + DEF_MAX_BLOCK_SIZE.ToString() );
      }
    ...
    public void EnsureMemoryBlockSize( int iDesiredSize )
    {
      ...
      if( iDesiredSize > DEF_MAX_BLOCK_SIZE )
      {
        throw new ArgumentOutOfRangeException( "iDesiredSize", 
                                               "Value cannot be greater than " + DEF_MAX_BLOCK_SIZE );
      }
...
//    public LabelSSTStructure CopyToLabelSST()
//    {
//      return ( LabelSSTStructure )Marshal.PtrToStructure(
//        m_memoryBlock, typeof( LabelSSTStructure ) );
//    }
    ...
    {
      // NOTE: maybe later for better performance here should be not method calls but 
...
//      int iSize = Marshal.SizeOf( source );
      int iSize = iCount;

      if( arrDestination.Length < iSize )
        throw new ArgumentException( "arrDestination", "Array too short" );

      EnsureMemoryBlockSize( iSize );
