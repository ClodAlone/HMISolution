...
namespace Syncfusion.XlsIO.Implementation
{
    ...
    where TKey : IComparable
  {
    ...
    public virtual TValue GetByName(string name)
    {
        ...
        string[] arr = new string[m_arrKeys.Length];
        m_arrKeys.CopyTo(arr, 0);
        int index = Array.IndexOf(arr, name);       


        if (index < 0 || index >= m_arrKeys.Length)
            throw new ArgumentOutOfRangeException("index");
    ...
    private class ValueList : IList<TValue>
    {
...
#if ( WINRT )
        Sort(keys, vals, m_list.m_comparer);
#else
      ...
      {
          SortedDictionary<TKey, TValue> sortedDic = new SortedDictionary<TKey, TValue>(comparer);
          for (int i = 0; i < keys.Length; i++)
          {
              sortedDic.Add(keys[i], values[i]);
          }
          int j = 0;
          foreach (KeyValuePair<TKey, TValue> keyValue in sortedDic)
          {
              keys[j] = keyValue.Key;
              values[j] = keyValue.Value;
              j++;
          }
      }
#endif
