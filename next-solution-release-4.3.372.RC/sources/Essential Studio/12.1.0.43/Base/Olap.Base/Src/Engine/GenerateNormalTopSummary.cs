...
{
    /// <summary>
    /// NormalTopSummary Layout - Pivot Engine Generation for IListSource and IEnumerable Source
    /// </summary>
    public class GenerateNormalTopSummary
    {
        /*
        *  ==================================================================================================
        * 
        ...
        */
        ...
        internal static PivotEngine ProcessRowMeasure(List<Syncfusion.Olap.Engine.Extension.GroupResult> rowData, List<Syncfusion.Olap.Engine.Extension.GroupResult> columnData, SummaryInfo[] summaryInfos, List<Syncfusion.Olap.Engine.Extension.GroupResult> summaryData, IQueryable queryableSource, string[] rowGroup, string[] columnGroup, List<Syncfusion.Olap.Engine.Extension.GroupResult> columnSummaryData, bool isLayoutChanged)
        {
            ...
            rowHeaderLoop = (g) =>
            {
                ...
                if (summaryInfos.Length == 1)
                {
                    if (g.ElementsCount == 0)
                    {
                        ...
                        cel.Range = GridRangeInfo.FromTlhw(cRow, cColumn, (g.ElementsCount), 1);
                }
                else
                {
                    if (g.ElementsCount > 0)
                ...
                else
                {
                    ...
                    if (g.SubGroups != null)
                    {
                        ...
                        if (!isLayoutChanged)
                        {
                            ...
                            summaryCell.CellValue = g.Key + " Total";
                            ...
                            summaryCell.Range = GridRangeInfo.FromTlhw(cRow, cColumn, 1, (rowLevels) - g.Level);
                        ...
                        else
                        {
                            ...
                            {
                                PivotCellDescriptor summaryCell = engineRow[cRow, cColumn];
                                summaryCell.CellValue = g.Key + " " + summaryInfos[i].Key;
            ...
            colHeaderLoop = (g) =>
            {
                ...
                if (g.SubGroups != null)
                {
                    ...
                    cellDesc.CellValue = g.Key + " Total";
        ...
        internal static PivotEngine ProcessColumnMeasure(List<Syncfusion.Olap.Engine.Extension.GroupResult> rowData, List<Syncfusion.Olap.Engine.Extension.GroupResult> columnData, SummaryInfo[] summaryInfos, List<Syncfusion.Olap.Engine.Extension.GroupResult> summaryData, IQueryable queryableSource, string[] rowGroup, string[] columnGroup, List<Syncfusion.Olap.Engine.Extension.GroupResult> columnSummaryData, bool isLayoutChanged)
        {
            ...
            rowHeaderLoopin = (g) =>
            {
                ...
                if (g.ElementsCount > 0)
                {
                    ...
                    PivotCellDescriptor summaryCell = engine[currentRow, currentColumn];
                    summaryCell.CellValue = g.Key + " Total";
                    summaryCell.Value = g.Key.ToString();
                    summaryCell.CellCaption = g.Key.ToString();
                    summaryCell.CellType = PivotCellDescriptorType.SummaryRow;
                    summaryCell.Range = GridRangeInfo.FromTlhw(currentRow, currentColumn, 1, (rowLevels - (g.Level)));

                    summaryCell.ParentCellValues = new List<string>();
                    summaryCell.ParentCellValues.Add(g.Key.ToString());

                    int tempcount = 0;
                    for (int i = currentColumn + 1; i < rowLevels; i++)
                    {
                        // Summary children cells
                        PivotCellDescriptor summarySpanCell = engine[currentRow, i];
            ...
            columnHeaderLoopin = (g) =>
            {
                ...
                if (!isLayoutChanged)
                {
                    if (g.ElementsCount > 0)
                    {
                        PivotCellDescriptor spanCell = engine[currentRow + 1, currentColumn];
                        spanCell.CellValue = "Total";
                        spanCell.CellCaption = g.Key.ToString();
                        spanCell.Value = g.Key.ToString();
                        if (summaryInfos.Length > 0)
                        {
                        ...
                        spanCell.Range = GridRangeInfo.FromTlhw(currentRow + 1, currentColumn, ((columnLevels - 1) - g.Level), 1);

                        for (int row = spanCell.Range.Width; row < spanCell.Range.Height; row++)
                        {
                            PivotCellDescriptor SpanCell = engine[currentRow + row + 1, currentColumn];
                ...
                if (g.SubGroups != null)
                {
                    if (isLayoutChanged)
                    {
                        for (int i = 0; i < summaryInfos.Length; i++)
                        {
                            ...
                            summaryCell.CellCaption = summaryInfos[i].Key;
                            summaryCell.Tag = summaryInfos[i];
                            summaryCell.Range = GridRangeInfo.FromTlhw(currentRow + 1, currentColumn, (columnLevels - 1) - g.Level, 1);
                ...
                for (int column = rowLevels; column < engine.TableColumns.Count; column++)
                {
                    ...
                    if (cell.UniqueName == string.Empty)
                    {
                        ...
                        if (groupResult == null)
                        {
                            ...
                            //double[] values = GetReportTableValues(engine, row, column);
                            //result = GetSummary(values, headerInfo.SummaryInfo).ToString();
                        }
                        if (groupResult != null)
                        {
        ...
        #region Levels Calculation
            ...
            //// preserving the datasource
            engineRow.ItemSource = listSource;
        ...
        internal static PivotEngine ProcessColumnMeasure(List<Syncfusion.Olap.Engine.Extension.GroupResult> rowData, List<Syncfusion.Olap.Engine.Extension.GroupResult> columnData, SummaryInfo[] summaryInfos, List<Syncfusion.Olap.Engine.Extension.GroupResult> summaryData, IListSource listSource, string[] rowGroup, string[] columnGroup, List<Syncfusion.Olap.Engine.Extension.GroupResult> columnSummaryData, bool isLayoutChanged,int summaryStringCount)
        {
            int count = 0,
              rowCount = 0,
              columnCount = 0,
