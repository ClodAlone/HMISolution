		...
		public static PropertyDescriptorCollection GetRelatedItemProperties(object parentItemList, PropertyDescriptor pd, out string name, out object arrayList)
		{
            ...
            if (parentItemList != null && pd.PropertyType != typeof(string) && typeof(IEnumerable).IsAssignableFrom(pd.PropertyType)) // Support for ArrayList or other types of collection
            {
                ...
                foreach (object item in list)
                {
                    ...
                    ///Fix: SD12590 when the If an item beyond 20 elements has entries, then the following condition 
                    ///doesn't allows to get the property descriptors as it terminates the loop
                    ///so the condition is removed (if (++n > 20){break;})
...
//				return true;
		...
		{
			if (pd != null && (pd.GetType().FullName == "System.Data.DataColumnPropertyDescriptor" || pd.GetType().FullName == "Syncfusion.Grouping.DataTableColumnPropertyDescriptor"))
			{
				if (mInfo3 == null)
				{
					Type t = pd.GetType();
		...
		public static PropertyDescriptorCollection GetItemProperties(object dataSource)
		{
			...
			else
			{
                ...
                if (dataSource.GetType().IsGenericType && !(dataSource is IBindingList))
                {
                    Type[] t = dataSource.GetType().GetGenericArguments();
                    if (t.GetLength(0) == 1)
                    {
                        PropertyDescriptorCollection propColl = TypeDescriptor.GetProperties(t[0]);
                        PropertyDescriptor[] propDesc = new PropertyDescriptor[propColl.Count];
                        propColl.CopyTo(propDesc, 0);
                        PropertyDescriptorCollection trgColl = new PropertyDescriptorCollection(propDesc);
                        if (propColl != null)
                        {
