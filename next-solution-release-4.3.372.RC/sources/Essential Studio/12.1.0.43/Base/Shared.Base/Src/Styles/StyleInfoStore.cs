...
namespace Syncfusion.Styles
{
    ...
    public class StaticData
    {
        ...
        {
#if DEBUG
            if (Switches.Styles.TraceVerbose)
                TraceUtil.TraceCurrentMethodInfo(type, styleInfoType, sortProperties);
#else
			;
#endif
            ...
            if (fi != null)
            {
                ...
                if (sds != null)
                {
                    ...
                    styleInfoProperties = (ArrayList)sds.styleInfoProperties.Clone();
...
#if DEBUG
            if (Switches.Styles.TraceVerbose)
                TraceUtil.TraceCurrentMethodInfo(maxValue);
#else
			;
#endif
        ...
        StyleInfoProperty _CreateStyleInfoProperty(Type type, string name, short maxValue, bool makeBitValue, Type componentType)
        {
#if DEBUG
            if (Switches.Styles.TraceVerbose)
                TraceUtil.TraceCurrentMethodInfo(type.Name, name, maxValue, "MakeBit = " + makeBitValue.ToString());
#else
			;
#endif
            ...
            for (int n = 0; n < styleInfoProperties.Count; n++)
            {
                ...
                {
                    TraceUtil.TraceCurrentMethodInfo(sip2.PropertyName + " re-registered.", sip2.ComponentType == componentType, sip2.ComponentType.Assembly == componentType.Assembly, sip2.ComponentType.Module.FullyQualifiedName, componentType.Module.FullyQualifiedName);
                    found = n;
                    break;
                }
            }
      ...
      IXmlSerializable
    {
        ...
        /// <summary>
        /// Initializes a new <see cref="StyleInfoStore"/> from a serialization stream.
        /// </summary>
        ...
        protected StyleInfoStore(SerializationInfo info, StreamingContext context)
        {
            ...
            include = new BitVector32[vectorCount];
            changed = new BitVector32[vectorCount];
            objects = sd.objectCount > 0 ? new StyleInfoObjectStore() : null;
            expandableObjects = sd.expandableObjectCount > 0 ? new StyleInfoObjectStore() : null;

            SerializationInfoEnumerator sie = info.GetEnumerator();
            while (sie.MoveNext())
            {
                StyleInfoProperty sip = FindStyleInfoProperty(sie.Name);
                if (sip != null)
                {
                    // TODO: what to do if there is an unknown property? (versioning issue ...)
                    // or if sie.Type != sip.PropertyType
                    if (sip.ObjectStoreKey != -1)
                    {
                        object value;
                        if (sie.Value is string && sip.PropertyType.IsPrimitive)
                        {
                            value = NullableHelper.ChangeType(sie.Value, sip.PropertyType);
                        }
                        else if (sip.PropertyType == typeof(Type))
                        {
                            object typeObj = sie.Value;
                            if (typeObj is string)
                                value = ValueConvert.GetType((string)typeObj);
                            else
                                value = (Type)typeObj;
                        }
                        else
                    ...
                    {
                        this.SetValue(sip, sie.Value);
                    }
                    else
                        this.SetValue(sip, Convert.ToInt16(sie.Value));
                }
                //				else
                //					throw new InvalidOperationException("StyleInfoProperty " + sie.Name + " not found.");
        ...
        void ISerializable.GetObjectData(SerializationInfo info, StreamingContext context)
        {
            ...
            foreach (StyleInfoProperty sip in StyleInfoProperties)
            {
                ...
                {
                    // if (sip.IsExpandable) ??
                    //Trace.WriteLine(sip.PropertyName);
                    if (sip.ObjectStoreKey != -1 || sip.ExpandableObjectStoreKey != -1)
                    {
                        if (sip.PropertyType == typeof(ImageList))
                        {
                            ImageList il = this.GetValue(sip) as ImageList;
                            if (il != null)
                            {
                                info.AddValue("ColorDepth", il.ColorDepth);
                                info.AddValue("ImageSize", il.ImageSize);
                                info.AddValue("ImageStream", il.ImageStream);
                                info.AddValue("TransparentColor", il.TransparentColor);
                            }
                        }
                        else if (sip.PropertyType == typeof(Type))
                        {
                            object obj = this.GetValue(sip);
                            string s = "";
                            if (obj is Type)
                                s = ValueConvert.GetTypeName((Type)obj);
                            info.AddValue(sip.PropertyName, s);
                        }
                        else
                            info.AddValue(sip.PropertyName, this.GetValue(sip), sip.PropertyType);
                    }
                    else if (sip.PropertyType.IsEnum)
                        info.AddValue(sip.PropertyName, Enum.ToObject(sip.PropertyType, this.GetShortValue(sip)));
                    else
                    {
                        //						TypeConverter tc = sip.Descriptor.Converter;
                        //						if (tc != null)
                        //							info.AddValue(tc.sip.PropertyName, this.GetShortValue(sip));
                        //						else
                        info.AddValue(sip.PropertyName, this.GetShortValue(sip));
                    }
                }
            }
        ...
        {
            return include != null && sip.BitVectorIndex < include.Length && include[sip.BitVectorIndex][sip.BitVectorMask];
        }
        /// <summary>
        ...
        /// </summary>
        /// <param name="sipSrc">A <see cref="StyleInfoProperty"/> that identifies the property to operate on.</param>
        public virtual object GetValue(StyleInfoProperty sipSrc)
        {
            StyleInfoProperty sip = ConversionSIP(sipSrc);
        ...
        {
            StyleInfoProperty res = null;

            // Checking sipSrc. If sipSrc is another store than find SIP from current store by PropertyName 
            if (null != sipSrc && StaticDataStore.StyleInfoType == sipSrc.ComponentType)
            {
                res = sipSrc;
            }
            else
            {
                res = FindStyleInfoProperty(sipSrc.PropertyName);
            }
        ...
        {
            // get source STORE
            StyleInfoStore storeSrc = istyle.Store;
            storeSrc.FixVectorCount();

            StaticData sdDes = this.StaticDataStore;
            StaticData sdSrc = storeSrc.StaticDataStore;

            // infill destination properties
            Hashtable hash = new Hashtable();
            for (int n = 0, len = sdDes.styleInfoProperties.Count; n < len; n++)
            {
                StyleInfoProperty sip = (StyleInfoProperty)sdDes.styleInfoProperties[n];
                hash[sip.PropertyName] = sip;
            }

            // start to inherit values from source
            for (int n = 0, len = sdSrc.styleInfoProperties.Count; n < len; n++)
            {
                StyleInfoProperty sip = (StyleInfoProperty)sdSrc.styleInfoProperties[n];
                if (sip.IsExpandable)
                    this._ModifyExpandableProperty(sip, storeSrc, mt);
                else
                    MultiModifyProperty(sip, storeSrc, mt);
            }
        }
        /// <summary>
        ...
        {
            StyleInfoProperty currentSip = null;
            if (null != sip)
            {
                currentSip = GetCurrentStorageSIP(sip);
            }
            ...
            if (sip.ObjectStoreKey != -1 || sip.ExpandableObjectStoreKey != -1)
            {
                ...
                if (null != currentSip)
                {
                    SetValue(currentSip, obj);
                }
            }
            else if (null != currentSip)
                SetValue(currentSip, style.GetShortValue(sip));
        }
        /// <summary>
        ...
        {
            StyleInfoProperty sip = GetCurrentStorageSIP(sipSrc);
