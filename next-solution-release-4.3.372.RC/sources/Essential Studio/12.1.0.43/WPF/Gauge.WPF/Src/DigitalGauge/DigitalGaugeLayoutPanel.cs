// <copyright file="DigitalGaugeLayoutPanel.cs" company="Syncfusion Software">
...
namespace Syncfusion.Windows.Gauge
{
    ...
    public class DigitalGaugeLayoutPanel : Panel
    {
        ...
        public static readonly DependencyProperty GaugeParentProperty =
            DependencyProperty.Register("GaugeParent", typeof(GaugeBase), typeof(DigitalGaugeLayoutPanel), new FrameworkPropertyMetadata(null, new PropertyChangedCallback(OnGaugeParentChanged)));

        /// <summary>
        /// Identifies the <see cref="PanelHeight"/> dependency property.
        /// </summary>
        public static readonly DependencyProperty PanelHeightProperty =
            DependencyProperty.Register("PanelHeight", typeof(double), typeof(DigitalGaugeLayoutPanel), new FrameworkPropertyMetadata(0d, new PropertyChangedCallback(OnPanelHeightChanged)));

        /// <summary>
        /// Identifies the <see cref="PanelWidth"/> dependency property.
        /// </summary>
        public static readonly DependencyProperty PanelWidthProperty =
            DependencyProperty.Register("PanelWidth", typeof(double), typeof(DigitalGaugeLayoutPanel), new FrameworkPropertyMetadata(0d, new PropertyChangedCallback(OnPanelWidthChanged)));
        ...
        protected override Size ArrangeOverride(Size finalSize)
        {
            ...
            if (PanelWidth > 0 && PanelHeight > 0)
            {
                ...
                foreach (FrameworkElement element in InternalChildren)
                {
                    ...
                    if (element is CharacterBase)
                    {
                        if ((this.GaugeParent as DigitalGauge).HorizontalContentAlignment == HorizontalAlignment.Left)
                        {
                            element.Arrange(new Rect(offset + (this.GaugeParent as DigitalGauge).CharacterSpacing, (PanelHeight - height) / 2, element.DesiredSize.Width, height));
                        }
                        else if ((this.GaugeParent as DigitalGauge).HorizontalContentAlignment == HorizontalAlignment.Right)
                        {
                            element.Arrange(new Rect(((PanelWidth - width)) + offset - (this.GaugeParent as DigitalGauge).CharacterSpacing - (this.GaugeParent as DigitalGauge).FirstFrameThickness.Right, (PanelHeight - height) / 2, element.DesiredSize.Width, height));
                    ...
                    {
                        element.Arrange(new Rect((offset - element.DesiredSize.Width), (PanelHeight - height) / 2, element.DesiredSize.Width, height));
                    }
                }
