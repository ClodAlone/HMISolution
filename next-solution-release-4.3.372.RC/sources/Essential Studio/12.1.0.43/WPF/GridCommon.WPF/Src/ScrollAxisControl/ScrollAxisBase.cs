...
namespace Syncfusion.Windows.Controls.Scroll
{
    ...
    public abstract class ScrollAxisBase : IDisposable
    {
        ...
        void ScrollLinesHost_DefaultLineSizeChanged(object sender, EventArgs e)
        {
...
#warning Need To resolve some quirks with nested grids.
        ...
        public int GetNextPage(int lineIndex)
        {
            ...
            int count = LineCount;
            while (extent < ScrollPageSize && lineIndex < count && lineIndex != -1)
            {
                //Debug.WriteLine("NP " + lineIndex);
        ...
        public int GetPreviousPage(int lineIndex)
        {
            double extent = 0;
            double pageExtent = ScrollPageSize - GetLineSize(lineIndex);
            while (extent < ScrollPageSize && lineIndex > 0)
            {
