	...
	public class DataPagerExt :Control
	{
		...
		public static readonly DependencyProperty PageCountProperty =
			DependencyProperty.Register(
				...
				typeof(int),
				typeof(DataPagerExt),
                new PropertyMetadata(OnPageCountPropertyChanged));
		...
		/// </summary>
		private WeakEventListener<DataPagerExt, object, PropertyChangedEventArgs> _weakEventListenerPropertyChanged;
		...
		public DataPagerExt()
		{
			this.DefaultStyleKey = typeof(DataPagerExt);
			...
			this.SetBinding(DataPagerExt.PrivateForegroundProperty, foregroundBinding);
            //this.Grid = new GridDataControl();
            this.OnDemandPaging(new GridDataOnDemandPageLoadingEventArgs(0, this.PageSize));
		...
		//Temp Added
		public IPagedCollectionView EventSource
		{
			get;
			set;
		}
		...
		{
            this.OnDemandPaging(new GridDataOnDemandPageLoadingEventArgs(_currentPageIndex, this.PageSize));//Temp Added.
			...
			if (this._numericButtonPanel != null)
			{
				...
				{
					throw new InvalidOperationException(GridDataResourceWrapper.InvalidButtonPanelContent);
				}
			...
			if (this._currentPageTextBox != null)
			{
                this._currentPageTextBox.Visibility = System.Windows.Visibility.Collapsed;
				this._currentPageTextBox.KeyDown += new System.Windows.Input.KeyEventHandler(this.OnCurrentPageTextBoxKeyDown);
				this._currentPageTextBox.LostFocus += new RoutedEventHandler(this.OnCurrentPageTextBoxLostFocus);
		...
		{
			DataPagerExt pager = d as DataPagerExt;
		...
		private static void OnDisplayModePropertyChanged(DependencyObject d, DependencyPropertyChangedEventArgs e)
		{
			...
			if (!pager.AreHandlersSuspended())
			{
				if (!Enum.IsDefined(typeof(PagerDisplayMode), e.NewValue))
				{
					...
					//        Resources.InvalidEnumArgumentException_InvalidEnumArgument,
					//        "value",
					//        e.NewValue.ToString(),
				...
				if ((int)e.NewValue < 0)
				{
					...
					throw new ArgumentOutOfRangeException(
						...
						string.Format(
							...
							GridDataResourceWrapper.ValueMustBeGreaterThanOrEqualTo,
							"NumericButtonCount",
							0));
				}
		...
		private static void OnPageIndexPropertyChanged(DependencyObject d, DependencyPropertyChangedEventArgs e)
		{
			...
			if (!pager.AreHandlersSuspended())
			{
                ...
                //SD16219 PageIndex updated for OndemandPagging
                if (pager.IsPagingOnDemand && newPageIndex > 0)
                {
                    pager._currentPageIndex = newPageIndex - 1;
                    int startidx = pager._currentPageIndex * pager.PageSize;                    
                    pager.OnDemandPaging(new GridDataOnDemandPageLoadingEventArgs(startidx, pager.PageSize));
                    pager.UpdateControl();
                }
                if (((pager.Source == null && !pager.IsPagingOnDemand) || pager.PageSize == 0) && newPageIndex != -1)
                {
					pager.SetValueNoCallback(e.Property, e.OldValue);
					throw new ArgumentOutOfRangeException(
						"value",
						GridDataResourceWrapper.PageIndexMustBeNegativeOne);
				}
				...
				else
				{
                    ...
                    pager.SetValueNoCallback(e.Property, newPageIndex);
				}
			}
		}

        //SD16219 PageCount updated for OndemandPagging
        private static void OnPageCountPropertyChanged(DependencyObject d,DependencyPropertyChangedEventArgs e)
        {
            ...
            if (pager.PageIndex != -1 && pager.IsPagingOnDemand &&pagecount>0)
            {
                pager.UpdateControl();
            }
        }
		...
		private static void OnReadOnlyPropertyChanged(DependencyObject d, DependencyPropertyChangedEventArgs e)
		{
			...
			if (pager != null && !pager.AreHandlersSuspended())
			{
				...
				throw new InvalidOperationException(
					string.Format(
						CultureInfo.InvariantCulture,
						GridDataResourceWrapper.UnderlyingPropertyIsReadOnly,
						e.Property.ToString()));
			}
		}
		...
		private static void OnSourcePropertyChanged(DependencyObject d, DependencyPropertyChangedEventArgs e)
		{
			...
			if (newPagedCollectionView != null)
			{
				...
				{
					pager._weakEventListenerPropertyChanged = new WeakEventListener<DataPagerExt, object, PropertyChangedEventArgs>(pager);
				...
				if (pager.PageIndex != newPagedCollectionView.PageIndex)
				{
					if (newPagedCollectionView.PageIndex == -1 && newPagedCollectionView.IsPageChanging)
					{
						...
						pager.SetValueNoCallback(DataPagerExt.PageIndexProperty, -1);
					}
					else
					{
        ...
        private int GetButtonStartIndex(bool isOndemandPaging)
        {
            // Because we have a starting PageIndex, we want to try and center the current pages
            // around this value. But if we are at the end of the collection, we display the last 
            // available buttons.
            return Math.Min(
                Math.Max((_currentPageIndex + 1) - (this.NumericButtonCount / 2), 1), /* center buttons around pageIndex */
                Math.Max(this.PageCount - this.NumericButtonCount + 1, 1));        /* lastPage - number of buttons */
        }
		...
		private void OnFirstPageButtonBaseClick(object sender, RoutedEventArgs e)
		{
            ...
            if (IsPagingOnDemand)
			{
				//var griddata = new GridDataControl();
                int startidx = 0 * this.PageSize;
                _currentPageIndex = 0;
                this.PageIndex = _currentPageIndex + 1;
                UpdateControl();
			}
			//Till This
		...
		private void OnLastPageButtonBaseClick(object sender, RoutedEventArgs e)
		{
            ...
            {
                //var griddata = new GridDataControl();
                int startidx =( this.PageCount - 1) * this.PageSize;
                this._currentPageIndex = this.PageCount - 1;
                this.PageIndex = this._currentPageIndex + 1;
                UpdateControl();
            }
            //Till This
			if (this.PagedSource != null)
        ...
        internal int _currentPageIndex
        {
            get;
            set;
        }
		...
		private void OnNextPageButtonBaseClick(object sender, RoutedEventArgs e)
		{
            ...
            //Temp
            if (IsPagingOnDemand && _currentPageIndex < this.PageCount - 1)
            {
                //var griddata = new GridDataControl();
                int startidx = (_currentPageIndex + 1) * this.PageSize;
                this._currentPageIndex = _currentPageIndex + 1;
                this.PageIndex = _currentPageIndex + 1;
        ...
        internal virtual void OnDemandPaging(GridDataOnDemandPageLoadingEventArgs args)
        {
            args.RoutedEvent =GridDataOnDemandPageLoadingEvent;
            args.Source = this;
            base.RaiseEvent(args);
            //this.ItemsSource = args.ItemSource;
        }

        public static readonly RoutedEvent GridDataOnDemandPageLoadingEvent = EventManager.RegisterRoutedEvent(
            "OnDemandDataSourceLoad",
            RoutingStrategy.Tunnel,
            typeof(GridDataOnDemandPageLoadingEventHandler),
        ...
        public event GridDataOnDemandPageLoadingEventHandler OnDemandDataSourceLoad
        {
            add
            {
                this.AddHandler(GridDataOnDemandPageLoadingEvent, value);
            }

            remove
            {
                this.RemoveHandler(GridDataOnDemandPageLoadingEvent, value);
            }
        }
        //public GridDataControl Grid
        //{
        //    get;
        //    set;
        //}
		...
		{
			//Temp
            if (IsPagingOnDemand && _currentPageIndex > 0)
			{
                int startidx = (_currentPageIndex - 1) * this.PageSize;
		...
		private void PageMoveHandler(int oldPageIndex, int newPageIndex, PageMoveOperationDelegate pageMoveOperation)
		{
			...
			if (cancelArgs.Cancel)
			{
				...
				this.SetValueNoCallback(DataPagerExt.PageIndexProperty, oldPageIndex);
			}
			else
			{
		...
		private void ToggleButton_Checked(object sender, RoutedEventArgs e)
		{
            ...
            if (IsPagingOnDemand)
			{
                ...
                int pageIndex = this.GetButtonStartIndex(IsPagingOnDemand) + uiIndex - 1;
               // this.PageMoveHandler(_currentPageIndex, pageIndex, null);
                this.PageIndex = pageIndex + 1;
                _currentPageIndex = pageIndex;
                int startidx = pageIndex * this.PageSize;
				//var datagrid = new GridDataControl();
                UpdateControl();
		...
		{
			if (!this._ignoreToggleButtonUncheckedNotification)//temp ! removed
			{
				try
				{
		...
		private void UpdateButtonCount()
		{
			...
			if (this._numericButtonPanel != null)
			{
                ...
                if (this.IsPagingOnDemand)
                    this.UpdateButtonDisplay(IsPagingOnDemand);
				else
		...
		//Temp
		private void UpdateButtonDisplay(bool isEventPaging)
		{
			if (this._numericButtonPanel != null)
			{
                ...
                int startIndex;
                if (this.IsPagingOnDemand)
                    startIndex = this.GetButtonStartIndex(IsPagingOnDemand);
                else
				    startIndex = this.GetButtonStartIndex();
				...
				foreach (UIElement ui in this._numericButtonPanel.Children)
				{
					...
					if (button != null)
					{
						...
						//    button.IsChecked = true;
						//}
						if(_currentPageIndex==index-1)//if (this.PagedSource != null && this.PagedSource.PageIndex == index - 1)
						{
							try
							{
								this._ignoreToggleButtonCheckedNotification = true;
						...
						{
							button.Content = GridDataResourceWrapper.AutoEllipsisString;
						}
						else
						{
		...
		/// </summary>
		public void UpdateControl() //Temp Changed
		{
			...
			bool needPage = this.Source != null && 
             ...
             //This is for ondemand paging
            if (IsPagingOnDemand)
            {
                this.CanMoveToFirstPage = _currentPageIndex > 0;
                this.CanMoveToPreviousPage = this.CanMoveToFirstPage;
                this.CanMoveToNextPage = _currentPageIndex < this.PageCount - 1;
                this.CanMoveToLastPage = this.CanMoveToNextPage;
                this.SetCanChangePage();
                this.UpdateCanPageFirstAndPrevious();
                this.UpdateCanPageNextAndLast();
                if (this._currentPageTextBox != null)
                    this._currentPageTextBox.Text = (_currentPageIndex + 1).ToString(CultureInfo.CurrentCulture);
            }
            //Till This
            else
            {
			...
			this.UpdateCurrentPagePrefixAndSuffix(needPage);//Temp
		...
		private void UpdateCurrentPagePrefixAndSuffix(bool needPage)
		{
            ...
            if (this._currentPageTextBox!=null && this._currentPageTextBox.Visibility == System.Windows.Visibility.Collapsed)
            {
                return;
            }
			...
			if (this._currentPagePrefixTextBlock != null)
			{
				if (this._currentPagePrefixTextBlock != null)
				{
					...
					{
						textBlockCaption = GridDataResourceWrapper.CurrentPagePrefix_TotalPageCountUnknown;
					}
					else
					{
						textBlockCaption = string.Format(
							CultureInfo.InvariantCulture,
							GridDataResourceWrapper.CurrentPagePrefix_TotalPageCountUnknown,
			...
			if (this._currentPageSuffixTextBlock != null)
			{
				if (needTotalPageCountUnknownState)
				{
					textBlockCaption = GridDataResourceWrapper.CurrentPageSuffix_TotalPageCountUnknown;
				}
				else
				{
					textBlockCaption = string.Format(
						CultureInfo.InvariantCulture,
						GridDataResourceWrapper.CurrentPageSuffix_TotalPageCountKnown,
						this.PageCount.ToString(CultureInfo.CurrentCulture));
				}
