...
namespace Syncfusion.Windows.Controls.Grid
{
    ...
    public class GridExcelLikeDragDropMouseController : IMouseController
    {
        ...
        public int HitTest(MouseControllerEventArgs e, IMouseController controller)
        {
            ...
            hitTestInfo = null;
            if ((e.Button == MouseButton.Left || e.ClickCount == 1) && this.grid.Model.Options.ExcelLikeSelectionFrame)
            {
                if (controller == null
        ...
        private bool StartDragDrop(int rowIndex, int colIndex)
        {
            ...
            if (grid.Model.Selections.GetSelectedRanges(out selList, true) && selList.AnyRangeContains(GridRangeInfo.Cell(rowIndex, colIndex)))
            {
                ...
                if ((this.grid.Model.Options.DragDropDropTargetFlags & GridDragDropFlags.Move) == GridDragDropFlags.Move)
                    dropEffect = DragDrop.DoDragDrop(this.grid, dataObject, DragDropEffects.Copy | DragDropEffects.Move);
                else
                    dropEffect = DragDrop.DoDragDrop(this.grid, dataObject, DragDropEffects.Copy);
