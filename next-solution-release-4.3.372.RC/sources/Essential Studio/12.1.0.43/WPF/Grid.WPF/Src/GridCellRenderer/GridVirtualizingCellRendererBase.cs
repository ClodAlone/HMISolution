...
namespace Syncfusion.Windows.Controls.Grid
{
    ...
    // B = Base Class 
    using B = Syncfusion.Windows.Controls.Grid.GridCellRendererBase;
    ...
    // S = Style
    using S = Syncfusion.Windows.Controls.Grid.GridRenderStyleInfo;
    ...
    {
        internal VirtualizingCellUIElementBin<T> recycleBin = new VirtualizingCellUIElementBin<T>();
        ...
        protected void  SetBounds(UIElement el, Rect rect, bool forceMeasure, bool forceArrange)
        {
            ...
            if (!rect.IsEmpty)
            {
                ...
                if (forceMeasure)
                {
                    ...
                    //    return;
                    //}       
                    //var grid = this.GridControl.FindParentElementOfType<GridControl>();
                    ////if (grid != null || el is GridDataCellNestedGridEditor)
                    //if (!(this.GridControl is GridDataControlBaseImpl) || el is GridDataCellNestedGridEditor)
                    {
            ...
            //var gridcontrol = this.GridControl.FindParentElementOfType<GridControl>();
            ////if (gridcontrol != null || el is GridDataCellNestedGridEditor)
            //if (!(this.GridControl is GridDataControlBaseImpl) || el is GridDataCellNestedGridEditor)
            {
                el.Arrange(rect);
                OnElementArranged(el, rect);
        ...
        public void InitializeContent(T uiElement, S cellInfo)
        {
            try
            {
                ...
                if (this.GridControl.CurrentCell.IsEditing
                 && !this.GridControl.CurrentCell.CellRowColumnIndex.IsEmpty
                 && this.GridControl.CurrentCell.CellRowColumnIndex == cellInfo.CellRowColumnIndex)
                    OnInitializeContent(uiElement, cellInfo);
                else
                    CreateRendererElement(uiElement, cellInfo);
            }
            finally
            {
        ...
        /// <summary>
        /// Called to initialize the content of the cell when in edit
        /// using the information from the cell style (value, text,
        ...
        /// <summary>
        /// Called to initialize the content of the cell when
        /// using the information from the cell style (value, text,
        /// behavior etc.). You must override this method in your
        /// derived class.
        /// </summary>
        /// <param name="uiElement">The UI element.</param>
        /// <param name="cellInfo">The cell style info.</param>
        public abstract void CreateRendererElement(T uiElement, S cellInfo);
        ...
        protected override void OnPrepareUIElements(ArrangeCellArgs aca, List<UIElement> uiElements, ScrollControlChildFrame canvas, S cellInfo)
        {
            ...
            if (!aca.CloneUIElement)
                WireUIElement(uiElement);// Wire events
        ...
        /// <summary>
        /// Wire when the UIElement enqueued in Recycle bin. When a cell is scrolled out of view
        /// it is moved into a recycle bin if we set AllowRecycle property and Tthe next time a new element is scrolled into
        /// view the element is recovered from the recycle bin and reinitialized with the
        /// new content of the cell. The default value is false.
        /// </summary>
        /// <param name="uiElement">The UI element.</param>
        ...
        private void UnwireUIElement(T uiElement)
        {
            //this event unhook in uiElement_Unloaded(object sender, RoutedEventArgs e) method
            //uiElement.Unloaded -= new RoutedEventHandler(uiElement_Unloaded);
            OnUnwireUIElement(uiElement);
        }
        ...
        void uiElement_Unloaded(object sender, RoutedEventArgs e)
        {
            ...
            uiElement.Unloaded -= new RoutedEventHandler(uiElement_Unloaded);
            VirtualizingCellsControl cellsControl = VirtualizingCellsControl.GetCellRendererParentControl(uiElement);
            if (cellsControl != null && cellsControl.ArrangedCellUIElements != null)
                if (!cellsControl.ArrangedCellUIElements.Contains(VirtualizingCellsControl.GetCellRowColumnIndex(uiElement)))
                    UnwireUIElement(uiElement);
        }
#endif
