...
using MarkerStyle = Syncfusion.Windows.Gauge.MarkerStyle;
...
{
    /// <summary>
    /// Interaction logic for ReportingGauge.xaml
    /// </summary>
    internal partial class ReportingGauge : UserControl
    {
        ...
        public ReportingGauge(IReportItemModeler pageContent)
        {
            this.GaugePanel = new Canvas();
            this.Content = this.GaugePanel;
#if !SILVERLIGHT
            this.Name = pageContent.Name;
#endif
            this.GaugeModel = pageContent as GaugeModel;
            this.GaugePanel.Height = this.Height = pageContent.Height;
            this.GaugePanel.Width = this.Width = pageContent.Width;

            ReportingBrushConverter brush = new ReportingBrushConverter();
#if !SILVERLIGHT
            CustomLabelsCollection labels = new CustomLabelsCollection();
            foreach (var gaugelabel in GaugeModel.GaugePanelProperties.GaugeLabels)
            {
                CircularCustomLabel radiallabel = new CircularCustomLabel();
                radiallabel.LabelValue = gaugelabel.Text;
                radiallabel.TextAngle = gaugelabel.Angle;
                radiallabel.FontSize = gaugelabel.LabelStyle.FontSize;
                radiallabel.FontFamily = new FontFamily(gaugelabel.LabelStyle.FontFamily);
                radiallabel.BackgroundBrush = new ReportingBrushConverter().ConvertFromInvariantString(gaugelabel.TextColor);

                labels.Add(radiallabel);
            }
#endif

            if (this.GaugeModel.GaugePanelProperties.RadialGauges != null)
            {
                ReportingCircularGauge gaugeControl = new ReportingCircularGauge(pageContent);
                Canvas.SetTop(gaugeControl, 0);
                Canvas.SetLeft(gaugeControl, 0);
                this.GaugePanel.Children.Add(gaugeControl);
            }
            else if (this.GaugeModel.GaugePanelProperties.Indicators != null)
            {
                ReportingIndicator gaugeControl = new ReportingIndicator(pageContent);
                Canvas.SetTop(gaugeControl, 0);
                Canvas.SetLeft(gaugeControl, 0);
                this.GaugePanel.Children.Add(gaugeControl);
            }
            else if (this.GaugeModel.GaugePanelProperties.LinearGauges != null)
            {
                ReportingLinearGauge gaugeControl = new ReportingLinearGauge(pageContent);
                Canvas.SetTop(gaugeControl, 0);
                Canvas.SetLeft(gaugeControl, 0);
                this.GaugePanel.Children.Add(gaugeControl);
            }
        ...
        void ControlEvents()
        {
            try
            {
                ...
                if (actionInfo != null)
                {
                    ...
                    {
                        this.MouseEnter += new MouseEventHandler(Gauge_MouseEnter);
                        this.MouseLeave += new MouseEventHandler(Gauge_MouseLeave);
#if SILVERLIGHT
                    this.MouseLeftButtonDown += new MouseButtonEventHandler(Gauge_PreviewMouseLeftButtonDown);
#else
                        this.PreviewMouseLeftButtonDown += new MouseButtonEventHandler(Gauge_PreviewMouseLeftButtonDown);
#endif
        ...
        void Gauge_PreviewMouseLeftButtonDown(object sender, MouseButtonEventArgs e)
        {
            try
            {
    ...
    internal class ReportingIndicator : UserControl
    {
        ...
        public ReportingIndicator(IReportItemModeler pageContent)
        {
            ...
            foreach (var indicat in gaugePanelProperties.Indicators)
            {
                ...
                {
#if !SILVERLIGHT
                    renderIndicator.Visibility = Visibility.Hidden;
#else
        ...
        Viewbox GetTemplate(GaugeStateIndicatorStyles indicatorStyle, string fillColor)
        {
            ...
            vBox.Child = XamlReader.Load(xr) as Grid;
            return vBox;
#endif
        }
    }

    internal class ReportingCircularGauge : CircularGauge
    {
        ReportingBrushConverter brush = new ReportingBrushConverter();
        public ReportingCircularGauge(IReportItemModeler pagecontent)
            : base()
        {
            GaugeModel gaugeModel = pagecontent as GaugeModel;
            this.Width = pagecontent.Width;
            ...
            foreach (var radialgauge in gaugeModel.GaugePanelProperties.RadialGauges)
            {
                if (radialgauge.GaugeFrame.BackFrameStyle.BackgroundGradientType == Syncfusion.RDL.DOM.BackgroundGradientType.None)//|| radialgauge.GaugeStyle.BackgroundGradientType == Syncfusion.RDL.DOM.BackgroundGradientType.Default)
                    this.Background = new ReportingBrushConverter().ConvertFromInvariantString(radialgauge.GaugeFrame.BackFrameStyle.BackgroundColor);
                else
                    this.Background = GradientColor(radialgauge.GaugeFrame.BackFrameStyle.BackgroundColor, radialgauge.GaugeFrame.BackFrameStyle.BackgroundGradientEndcolor, radialgauge.GaugeFrame.BackFrameStyle.BackgroundGradientType);
                ...
                if (!string.IsNullOrEmpty(radialgauge.GaugeTooltip))
                {
                    ToolTipService.SetToolTip(this, radialgauge.GaugeTooltip);
                }

#if !SILVERLIGHT
                //float pivotx = 50f;
                //float pivoty = 50f;
                this.FrameType = GaugeFrameType.FullCircle;
                this.Radius = this.Width < this.Height ? this.Width / 2 : this.Height / 2;

                if (radialgauge.GaugeFrame.FrameProperty.BackgroundColor != null)
                {
                    this.FirstFrameFillColor = new ReportingBrushConverter().ConvertFromInvariantString(radialgauge.GaugeFrame.FrameProperty.BackgroundColor);
                }

                this.FirstFrameThickness = new Thickness(radialgauge.GaugeFrame.FrameWidth);
                this.SecondFrameThickness = new Thickness(0);

#endif
                var cir = new ScaleCollection();
                foreach (var scales in radialgauge.GaugeScales)
                {

                    var Circlescale = new CircularScale();
                    double radius = (this.Width < this.Height) ? this.Width / 2 : this.Height / 2;

                    Circlescale.BorderThickness = new Thickness(scales.ScaleStyle.BorderWidth);
                    Circlescale.GapSweepAngle = scales.SweepAngle;
                    Circlescale.Maximum = scales.MaximumValue.Value;
                    Circlescale.Minimum = scales.MinimumValue.Value;
                    if (!double.IsNaN(scales.MajorTickMark.Interval))
                        Circlescale.MajorIntervalValue = scales.MajorTickMark.Interval;
                    else
                        Circlescale.MajorIntervalValue = (scales.MaximumValue.Value - scales.MinimumValue.Value) / 10;
                    if (!double.IsNaN(scales.MinorTickMark.Interval))
                        Circlescale.MinorIntervalValue = scales.MinorTickMark.Interval;
                    else
                        Circlescale.MinorIntervalValue = Circlescale.MajorIntervalValue / 4;
#if SILVERLIGHT
                    Circlescale.Background = new ReportingBrushConverter().ConvertFromInvariantString("transparent");
#else
                    Circlescale.BackgroundBrush = new ReportingBrushConverter().ConvertFromInvariantString("transparent");
#endif

                    Circlescale.BorderBrush = new ReportingBrushConverter().ConvertFromInvariantString(scales.ScaleStyle.BorderColor);
                    Circlescale.Radius = radius * ((scales.Radius * 2) / 100);
                    Circlescale.ScaleBarSize = scales.ScaleWidth;
                    Circlescale.ShadowOffset = scales.ScaleStyle.OffSet;
                    Circlescale.StartAngle = scales.StartAngle + 90;

                    if (!string.IsNullOrEmpty(scales.ToolTip))
                    {
                        ToolTipService.SetToolTip(this, scales.ToolTip);
                    }
                    if (scales.Hidden)
                    {
#if !SILVERLIGHT
                    ...
                    foreach (var radialpointer in scales.ScalePointer)
                    {
                        ...
                        Pointer.Value = radialpointer.Value.Value;
                        Pointer.BorderThickness = new Thickness(radialpointer.PointerStyle.BorderWidth);
                        Pointer.PointerNeedleType = (PointerNeedleType)radialpointer.RadialPointer.PointerType;

                        if (Pointer.PointerNeedleType == PointerNeedleType.Needle)
                        {
                            switch (radialpointer.RadialPointer.NeedleStyle)
                            {
                                case DOM.NeedleStyleGauge.Arrow:
                                    Pointer.NeedleStyle = NeedleStyle.Arrow;
                                    break;
                                case DOM.NeedleStyleGauge.Rectangular:
                                    Pointer.NeedleStyle = NeedleStyle.Rectangle;
                                    break;
                                case DOM.NeedleStyleGauge.Triangular:
                                    Pointer.NeedleStyle = NeedleStyle.Triangle;
                                    break;
                                default:
                                    Pointer.NeedleStyle = NeedleStyle.Triangle;
                                    break;
                        ...
                        {
                            switch (radialpointer.RadialPointer.MarkerStyle)
                            {
                                case DOM.MarkerStyle.Triangle:
                                    Pointer.MarkerStyle = MarkerStyle.Triangle;
                                    break;
                                case DOM.MarkerStyle.Rectangle:
                                    Pointer.MarkerStyle = MarkerStyle.Rectangle;
                                    break;
                                case DOM.MarkerStyle.Circle:
                                    Pointer.MarkerStyle = MarkerStyle.Ellipse;
                                    break;
                                case DOM.MarkerStyle.Diamond:
                                    Pointer.MarkerStyle = MarkerStyle.Diamond;
                                    break;
                                case DOM.MarkerStyle.Trapezoid:
                                    Pointer.MarkerStyle = MarkerStyle.Trapezoid;
                                    break;
                                case DOM.MarkerStyle.Star:
                                    Pointer.MarkerStyle = MarkerStyle.Pentagon;
                                    break;
                                default:
                                    Pointer.MarkerStyle = MarkerStyle.Triangle;
                                    break;
                        ...
                        Pointer.EnablePointerInteraction = false;
#if !SILVERLIGHT
                        if (radialpointer.PointerStyle.BackgroundGradientType == Syncfusion.RDL.DOM.BackgroundGradientType.None)
                            Pointer.BackgroundBrush = new ReportingBrushConverter().ConvertFromInvariantString(radialpointer.PointerStyle.BackgroundColor);
                        else
                            Pointer.BackgroundBrush = GradientColor(radialpointer.PointerStyle.BackgroundColor, radialpointer.PointerStyle.BackgroundGradientEndcolor, radialpointer.PointerStyle.BackgroundGradientType);
#else
                         if (radialpointer.PointerStyle.BackgroundGradientType == Syncfusion.RDL.DOM.BackgroundGradientType.None)
                            Pointer.Background = new ReportingBrushConverter().ConvertFromInvariantString(radialpointer.PointerStyle.BackgroundColor);
                        else
                            Pointer.Background = GradientColor(radialpointer.PointerStyle.BackgroundColor, radialpointer.PointerStyle.BackgroundGradientEndcolor, radialpointer.PointerStyle.BackgroundGradientType);

                        Pointer.BorderBrush = new ReportingBrushConverter().ConvertFromInvariantString(radialpointer.PointerStyle.BorderColor);
#endif

                        if (Pointer.PointerNeedleType == PointerNeedleType.Marker)
                            Pointer.PointerLength = radialpointer.MarkerLength;
                        else
                            Pointer.PointerLength = radius * ((scales.Radius * 2) / 100);
                        Pointer.PointerPlacement = ScalePlacement.Inside;
                        Pointer.PointerWidth = radialpointer.PointerWidth;

                        if (!string.IsNullOrEmpty(radialpointer.PointerTooltip))
                        {
                            ToolTipService.SetToolTip(this, radialpointer.PointerTooltip);
                        }
                        ...
                        Circlescale.Pointers.Add(Pointer);

                        Syncfusion.Windows.Gauge.PointerCap Cap = new Syncfusion.Windows.Gauge.PointerCap();
                        Cap.CapOnTop = radialpointer.RadialPointer.CapProperties.OnTop;
                        Cap.PointerCapType = (PointerCapType)radialpointer.RadialPointer.CapProperties.PointerCapStyle;
                        ...
                        Cap.Background = new ReportingBrushConverter().ConvertFromInvariantString(radialpointer.RadialPointer.CapProperties.CapStyle.BackgroundColor);
                    else
                        Cap.Background= GradientColor(radialpointer.RadialPointer.CapProperties.CapStyle.BackgroundColor, radialpointer.RadialPointer.CapProperties.CapStyle.BackgroundGradientEndcolor, radialpointer.RadialPointer.CapProperties.CapStyle.BackgroundGradientType);

                    Cap.Width=radialpointer.RadialPointer.CapProperties.PointerCapWidth;
                    Cap.BorderThickness = new Thickness(0);
#else
                        if (radialpointer.RadialPointer.CapProperties.CapStyle.BackgroundGradientType == Syncfusion.RDL.DOM.BackgroundGradientType.None)
                            Cap.BackgroundBrush = new ReportingBrushConverter().ConvertFromInvariantString(radialpointer.RadialPointer.CapProperties.CapStyle.BackgroundColor);
                        else
                            Cap.BackgroundBrush = GradientColor(radialpointer.RadialPointer.CapProperties.CapStyle.BackgroundColor, radialpointer.RadialPointer.CapProperties.CapStyle.BackgroundGradientEndcolor, radialpointer.RadialPointer.CapProperties.CapStyle.BackgroundGradientType);
                        Cap.Width = radialpointer.RadialPointer.CapProperties.PointerCapWidth / 2;
#endif
                        if (radialpointer.RadialPointer.CapProperties.Hidden)
                        {
#if !SILVERLIGHT
                            this.Visibility = Visibility.Hidden;
#else
                        ...
                        Circlescale.PointerCap = Cap;
                    }
#if !SILVERLIGHT
                    var tick1 = new CircularMarkTick();
                    tick1.BackgroundBrush = new ReportingBrushConverter().ConvertFromInvariantString("black");
                    tick1.TickShape = (TickShape)Enum.Parse(typeof(TickShape), scales.MinorTickMark.TickMarkShape);
#else
                    var tick1=new MarkTickSet();
                    tick1.Background = new ReportingBrushConverter().ConvertFromInvariantString("black");
#endif
                    tick1.TickHeight = scales.MinorTickMark.Length;
                    tick1.DistanceFromScale = scales.MinorTickMark.DistanceFromScale;
                    tick1.TickStyle = TickStyle.MinorTick;
                    tick1.TickWidth = scales.MinorTickMark.Width;
                    tick1.TickPlacement = (ScalePlacement)scales.MinorTickMark.TickMarkPlacement;

#if !SILVERLIGHT
                    var tick2 = new CircularMarkTick();
                    tick2.TickShape = (TickShape)Enum.Parse(typeof(TickShape), scales.MajorTickMark.TickMarkShape);
                    tick2.BackgroundBrush = new ReportingBrushConverter().ConvertFromInvariantString("black");
#else
                    var tick2=new MarkTickSet();
                    tick2.Background = new ReportingBrushConverter().ConvertFromInvariantString("black");
#endif
                    tick2.Angle = 0;
                    tick2.TickHeight = scales.MajorTickMark.Length;
                    tick2.TickStyle = TickStyle.MajorTick;
                    tick2.TickWidth = scales.MajorTickMark.Width;
                    tick2.DistanceFromScale = scales.MajorTickMark.DistanceFromScale;
                    tick2.TickPlacement = (ScalePlacement)scales.MajorTickMark.TickMarkPlacement;


#if !SILVERLIGHT
                    var labeltick = new CircularLabelTick();
                    labeltick.BackgroundBrush = new ReportingBrushConverter().ConvertFromInvariantString("black");
#else
                    var labeltick=new LabelTickSet();
                    labeltick.Background = new ReportingBrushConverter().ConvertFromInvariantString("black");
#endif
                    labeltick.DistanceFromScale = scales.ScaleLabel.ScaleDistance + 10;
                    labeltick.FontSize = scales.ScaleLabel.LabelStyle.FontSize;
                    labeltick.TickPlacement = (ScalePlacement)scales.ScaleLabel.ScalePlacment;
                    labeltick.TickStyle = TickStyle.MajorTick;


                    Circlescale.Ticks.Add(tick1);
                    Circlescale.Ticks.Add(tick2);
                    Circlescale.Ticks.Add(labeltick);
                    if (scales.ScaleRange != null)
                    {
                        foreach (var radialrange in scales.ScaleRange)
                        {
                            CircularRange Range = new CircularRange();
                            Range.BorderThickness = new Thickness(radialrange.RangeStyle.BorderWidth);
                            Range.DistanceFromScale = radialrange.ScaleDistance;
                            if (radialrange.EndValue.Value == 0 && radialrange.StartValue.Value == 0)
                            {
                                Range.StartValue = 70;
                                Range.EndValue = 100;
                            }
                            ...
                            {
                                Range.EndValue = radialrange.EndValue.Value;
                                Range.StartValue = radialrange.StartValue.Value;
                            }
                            Range.EndWidth = radialrange.EndWidth;
                            Range.RangePosition = (ScalePlacement)radialrange.RangePlacement;
                            Range.StartWidth = radialrange.StartWidth;
#if SILVERLIGHT
                            if (radialrange.RangeStyle.BorderWidth == 0)
                            {
                                Range.BorderThickness =new Thickness(1);
                            }
                            else
                            {
                                Range.BorderThickness =new Thickness(radialrange.RangeStyle.BorderWidth);
                            }
                            Range.BorderBrush = (Brush)new ReportingBrushConverter().ConvertFromInvariantString(radialrange.RangeStyle.BorderColor.ToString());

                            if (radialrange.RangeStyle.BackgroundGradientType == Syncfusion.RDL.DOM.BackgroundGradientType.None)
                                Range.Background = new ReportingBrushConverter().ConvertFromInvariantString(radialrange.RangeStyle.BackgroundColor);
                            else
                                Range.Background = GradientColor(radialrange.RangeStyle.BackgroundColor, radialrange.RangeStyle.BackgroundGradientEndcolor, radialrange.RangeStyle.BackgroundGradientType);

#else
                            Range.BorderWidth = 1;
                            Range.BorderBrush = (Brush)new ReportingBrushConverter().ConvertFromInvariantString(radialrange.RangeStyle.BorderColor.ToString());
                            if (radialrange.RangeStyle.BackgroundGradientType == Syncfusion.RDL.DOM.BackgroundGradientType.None)
                                Range.BackgroundBrush = new ReportingBrushConverter().ConvertFromInvariantString(radialrange.RangeStyle.BackgroundColor);
                            else
                                Range.BackgroundBrush = GradientColor(radialrange.RangeStyle.BackgroundColor, radialrange.RangeStyle.BackgroundGradientEndcolor, radialrange.RangeStyle.BackgroundGradientType);
#endif
                            ...
                            Circlescale.Ranges.Add(Range);
                        }
                    }
                    this.Background = (Brush)this.brush.ConvertFromInvariantString(radialgauge.GaugeFrame.BackFrameStyle.BackgroundColor);
                    this.Scales.Add(Circlescale);
                }
            }
            this.Background = (Brush)this.brush.ConvertFromInvariantString(gaugeModel.GaugePanelProperties.RadialGauges[0].GaugeFrame.BackFrameStyle.BackgroundColor);
        }
        LinearGradientBrush GradientColor(string color, string gradientEndColor, Syncfusion.RDL.DOM.BackgroundGradientType gradientType)
        {
            ...
            Color bgcolor;
            System.Windows.Controls.Canvas colorCanvas = new System.Windows.Controls.Canvas();
            ...
            if (gradientType == Syncfusion.RDL.DOM.BackgroundGradientType.DiagonalLeft)
            {
                ...
                lgb.StartPoint = new System.Windows.Point(0.5, 0);
            }
            else if (gradientType == Syncfusion.RDL.DOM.BackgroundGradientType.DiagonalRight)
            {
                ...
                lgb.GradientStops = gsc;
                lgb.EndPoint = new System.Windows.Point(0.981, 0.024);
                lgb.StartPoint = new System.Windows.Point(0.008, 0.996);
            }
            else if (gradientType == Syncfusion.RDL.DOM.BackgroundGradientType.HorizontalCenter)
            {
                ...
                lgb.GradientStops = gsc;
                lgb.EndPoint = new System.Windows.Point(0.389, 0.972);
                lgb.StartPoint = new System.Windows.Point(0.386, 0.058);
            }
            else if (gradientType == Syncfusion.RDL.DOM.BackgroundGradientType.VerticalCenter)
            {
                ...
                lgb.GradientStops = gsc;
                lgb.EndPoint = new System.Windows.Point(0.975, 0.617);
                lgb.StartPoint = new System.Windows.Point(0.014, 0.617);
            }
            ...
            else
            {
                ...
                lgb.GradientStops = gsc;
                lgb.EndPoint = new System.Windows.Point(0.981, 0.482);
                lgb.StartPoint = new System.Windows.Point(-0.028, 0.494);
            }
            return lgb;
        }
    }

    internal class ReportingLinearGauge : Syncfusion.Windows.Gauge.LinearGauge
    {
        public ReportingLinearGauge(IReportItemModeler pageContent)
            : base()
        {
            try
            {
                ...
                foreach (var linear in gaugeModel.GaugePanelProperties.LinearGauges)
                {
                    if (linear.GaugeFrame.BackFrameStyle.BackgroundGradientType == Syncfusion.RDL.DOM.BackgroundGradientType.None)
                        this.Background = new ReportingBrushConverter().ConvertFromInvariantString(linear.GaugeFrame.BackFrameStyle.BackgroundColor);
                    else
                        this.Background = GradientColor(linear.GaugeFrame.BackFrameStyle.BackgroundColor, linear.GaugeFrame.BackFrameStyle.BackgroundGradientEndcolor, linear.GaugeFrame.BackFrameStyle.BackgroundGradientType);
                    if (linear.Orientation == DOM.Orientation.Horizontal)
                        this.Orientation = GaugeOrientation.Horizontal;
                    else if (linear.Orientation == DOM.Orientation.Vertical)
                        this.Orientation = GaugeOrientation.Vertical;
                    else
                    {
                        if (this.Width > this.Height)
                            this.Orientation = GaugeOrientation.Horizontal;
                    ...
                    this.SecondFrameThickness = new Thickness(0);
                    this.FirstFrameFillColor = new ReportingBrushConverter().ConvertFromInvariantString(linear.GaugeFrame.FrameProperty.BackgroundColor);
#endif

                    this.BorderBrush = new ReportingBrushConverter().ConvertFromInvariantString(linear.GaugeFrame.FrameProperty.BackgroundColor);

                    foreach (var scales in linear.GaugeScales)
                    {
                        var Linearscale = new Syncfusion.Windows.Gauge.LinearScale();
#if SILVERLIGHT
                        Linearscale.FlowDirection = System.Windows.FlowDirection.LeftToRight;
#endif

                        Linearscale.ScaleBarLength = (this.Width > this.Height) ? (this.Width / 100) * 85 : (this.Height / 100) * 85;
                        Linearscale.ScaleBarSize = (scales.ScaleWidth != 0.0 && scales.ScaleWidth > 10 ? scales.ScaleWidth : 10.00f); ;
                        Linearscale.Minimum = scales.MinimumValue.Value;
                        Linearscale.Maximum = scales.MaximumValue.Value;


                        if (!double.IsNaN(scales.MajorTickMark.Interval))
                            Linearscale.MajorIntervalValue = scales.MajorTickMark.Interval;
                        else
                            Linearscale.MajorIntervalValue = (scales.MaximumValue.Value - scales.MinimumValue.Value) / 5;
                        if (!double.IsNaN(scales.MinorTickMark.Interval))
                            Linearscale.MinorIntervalValue = scales.MinorTickMark.Interval;
                        else
                            Linearscale.MinorIntervalValue = Linearscale.MajorIntervalValue / 4;
#if !SILVERLIGHT

                        if (this.Orientation == GaugeOrientation.Vertical)
                            Linearscale.ScaleDirection = ScaleDirection.CounterClockwise;
                        else
                            Linearscale.ScaleDirection = ScaleDirection.Clockwise;
                        Linearscale.BackgroundBrush = new ReportingBrushConverter().ConvertFromInvariantString("Transparent");
#else
                        Linearscale.Background = new ReportingBrushConverter().ConvertFromInvariantString("Transparent");
#endif
                        Linearscale.BorderThickness = new Thickness(scales.ScaleStyle.BorderWidth);
                        Linearscale.BorderBrush = new ReportingBrushConverter().ConvertFromInvariantString(scales.ScaleStyle.BorderColor);
                        if (!string.IsNullOrEmpty(scales.ToolTip))
                        {
                            ToolTipService.SetToolTip(this, scales.ToolTip);
                        }
...
#if !SILVERLIGHT
                        var tick1 = new LinearMarkTick();
                        tick1.TickShape = (TickShape)Enum.Parse(typeof(TickShape), scales.MajorTickMark.TickMarkShape);
                        tick1.BackgroundBrush = new ReportingBrushConverter().ConvertFromInvariantString(scales.MajorTickMark.FillColor);
#else
                        var tick1 = new MarkTickSet();
                        tick1.Background = new ReportingBrushConverter().ConvertFromInvariantString(scales.MajorTickMark.FillColor);
#endif
                        tick1.TickStyle = TickStyle.MajorTick;
                        tick1.TickHeight = scales.MajorTickMark.Length;
                        tick1.TickWidth = scales.MajorTickMark.Width;
                        tick1.TickPlacement = (ScalePlacement)scales.MajorTickMark.TickMarkPlacement;
                        tick1.DistanceFromScale = scales.MajorTickMark.DistanceFromScale;

#if !SILVERLIGHT
                        var tick2 = new LinearMarkTick();
                        tick2.TickShape = (TickShape)Enum.Parse(typeof(TickShape), scales.MinorTickMark.TickMarkShape);
                        tick2.BackgroundBrush = new ReportingBrushConverter().ConvertFromInvariantString(scales.MinorTickMark.FillColor);
#else
                        var tick2 = new MarkTickSet();
                        tick2.Background = new ReportingBrushConverter().ConvertFromInvariantString(scales.MinorTickMark.FillColor);
#endif
                        tick2.TickStyle = TickStyle.MinorTick;
                        tick2.TickHeight = scales.MinorTickMark.Length;
                        tick2.TickWidth = scales.MinorTickMark.Width;
                        tick2.TickPlacement = (ScalePlacement)scales.MinorTickMark.TickMarkPlacement;
                        tick2.DistanceFromScale = scales.MinorTickMark.DistanceFromScale;

#if !SILVERLIGHT
                        var ticklabel = new LinearLabelTick();
                        ticklabel.BackgroundBrush = new ReportingBrushConverter().ConvertFromInvariantString(scales.ScaleLabel.TextColor);
#else
                        var ticklabel=new LabelTickSet();
                        ticklabel.Background=new ReportingBrushConverter().ConvertFromInvariantString(scales.ScaleLabel.TextColor);
#endif
                        if (scales.LogrithmicScale)
                        {
                            ticklabel.LogBase = scales.LogBase;
                            ticklabel.IsLogarithmic = scales.LogrithmicScale;
                            System.Globalization.NumberFormatInfo labelFormatInfo = new System.Globalization.NumberFormatInfo();
                            labelFormatInfo.NumberDecimalDigits = 3;
#if !SILVERLIGHT
                            ticklabel.NumberFormatInfo = labelFormatInfo;
#else
                            ticklabel.LabelFormat = labelFormatInfo;
#endif
                        }
                        ticklabel.DistanceFromScale = scales.ScaleLabel.ScaleDistance + 10;
                        ticklabel.TickStyle = TickStyle.MajorTick;
                        ticklabel.FontSize = scales.ScaleLabel.LabelStyle.FontSize;
                        ticklabel.TickPlacement = (ScalePlacement)scales.ScaleLabel.ScalePlacment;

                        Linearscale.Ticks.Add(tick1);
                        Linearscale.Ticks.Add(tick2);
                        Linearscale.Ticks.Add(ticklabel);


                        foreach (var pointer in scales.ScalePointer)
                        {
                            if (pointer.LinearPointer.PointerType == DOM.LinearPointerType.Thermometer)
                            {
                                LinearBarPointer Thermometer = new LinearBarPointer();
                                Thermometer.PointerWidth = pointer.PointerWidth;
#if !SILVERLIGHT
                                Thermometer.BarStyle = BarStyle.Thermometer;
                                if (pointer.PointerStyle.BackgroundGradientType == Syncfusion.RDL.DOM.BackgroundGradientType.None)
                                    Thermometer.BackgroundBrush = new ReportingBrushConverter().ConvertFromInvariantString(pointer.PointerStyle.BackgroundColor);
                                else
                                    Thermometer.BackgroundBrush = GradientColor(pointer.PointerStyle.BackgroundColor, pointer.PointerStyle.BackgroundGradientEndcolor, pointer.PointerStyle.BackgroundGradientType);
#else
                                     if (pointer.PointerStyle.BackgroundGradientType == Syncfusion.RDL.DOM.BackgroundGradientType.None)
                                        Thermometer.Background = new ReportingBrushConverter().ConvertFromInvariantString(pointer.PointerStyle.BackgroundColor);
                                    else
                                        Thermometer.Background = GradientColor(pointer.PointerStyle.BackgroundColor, pointer.PointerStyle.BackgroundGradientEndcolor, pointer.PointerStyle.BackgroundGradientType);

#endif
                                Thermometer.Value = pointer.Value.Value;
                                Thermometer.EnablePointerInteraction = false;
                                if (pointer.ScaleDistance > 0)
                                {
                                    if (this.Orientation == GaugeOrientation.Vertical)
                                        Thermometer.Margin = new System.Windows.Thickness(pointer.ScaleDistance + 5, 0.0, 0.0, 0.0);
                                    else
                                        Thermometer.Margin = new System.Windows.Thickness(0.0, pointer.ScaleDistance + 5, 0.0, 0.0);

                                }
                                Linearscale.Pointers.Add(Thermometer);
                            }



                            else if (pointer.LinearPointer.PointerType == DOM.LinearPointerType.Marker)
                            {
                                LinearMarkerPointer Marker = new LinearMarkerPointer();
                                switch (pointer.MarkerStyle)
                                {
                                    case DOM.MarkerStyle.Triangle:
                                        Marker.MarkerStyle = MarkerStyle.Triangle;
                                        break;
                                    case DOM.MarkerStyle.Rectangle:
                                        Marker.MarkerStyle = MarkerStyle.Rectangle;
                                        break;
                                    case DOM.MarkerStyle.Circle:
                                        Marker.MarkerStyle = MarkerStyle.Ellipse;
                                        break;
                                    case DOM.MarkerStyle.Diamond:
                                        Marker.MarkerStyle = MarkerStyle.Diamond;
                                        break;
                                    case DOM.MarkerStyle.Trapezoid:
                                        Marker.MarkerStyle = MarkerStyle.Trapezoid;
                                        break;
                                    case DOM.MarkerStyle.Star:
                                        Marker.MarkerStyle = MarkerStyle.Pentagon;
                                        break;
                                    default:
                                        Marker.MarkerStyle = Syncfusion.Windows.Gauge.MarkerStyle.Triangle;
                                        break;
                                }
#if !SILVERLIGHT
                                if (pointer.PointerStyle.BackgroundGradientType == Syncfusion.RDL.DOM.BackgroundGradientType.None)
                                    Marker.BackgroundBrush = new ReportingBrushConverter().ConvertFromInvariantString(pointer.PointerStyle.BackgroundColor);
                                else
                                    Marker.BackgroundBrush = GradientColor(pointer.PointerStyle.BackgroundColor, pointer.PointerStyle.BackgroundGradientEndcolor, pointer.PointerStyle.BackgroundGradientType);
#else
                                     if (pointer.PointerStyle.BackgroundGradientType == Syncfusion.RDL.DOM.BackgroundGradientType.None)
                                        Marker.Background= new ReportingBrushConverter().ConvertFromInvariantString(pointer.PointerStyle.BackgroundColor);
                                    else
                                        Marker.Background = GradientColor(pointer.PointerStyle.BackgroundColor, pointer.PointerStyle.BackgroundGradientEndcolor, pointer.PointerStyle.BackgroundGradientType);

#endif
                                Marker.PointerPlacement = (ScalePlacement)pointer.Placement;
                                Marker.PointerLength = pointer.MarkerLength;
                                Marker.Value = pointer.Value.Value;
                                Marker.PointerWidth = pointer.PointerWidth;
                                Marker.EnablePointerInteraction = false;
                                Linearscale.Pointers.Add(Marker);
                            }
                            else if (pointer.LinearPointer.PointerType == DOM.LinearPointerType.Bar)
                            {
                                LinearBarPointer Barpointer = new LinearBarPointer();
                                Barpointer.PointerWidth = pointer.PointerWidth;
                                Barpointer.Width = pointer.LinearPointer.BulbSize;
                                Barpointer.Value = pointer.Value.Value;
                                if (pointer.ScaleDistance > 0)
                                {
                                    if (this.Orientation == GaugeOrientation.Vertical)
                                        Barpointer.Margin = new System.Windows.Thickness(pointer.ScaleDistance + 5, 0.0, 0.0, 0.0);
                                    else
                                        Barpointer.Margin = new System.Windows.Thickness(0.0, pointer.ScaleDistance + 5, 0.0, 0.0);

                                }
#if !SILVERLIGHT
                                if (pointer.PointerStyle.BackgroundGradientType == Syncfusion.RDL.DOM.BackgroundGradientType.None)
                                    Barpointer.BackgroundBrush = new ReportingBrushConverter().ConvertFromInvariantString(pointer.PointerStyle.BackgroundColor);
                                else
                                    Barpointer.BackgroundBrush = GradientColor(pointer.PointerStyle.BackgroundColor, pointer.PointerStyle.BackgroundGradientEndcolor, pointer.PointerStyle.BackgroundGradientType);
#else
                                      if (pointer.PointerStyle.BackgroundGradientType == Syncfusion.RDL.DOM.BackgroundGradientType.None)
                                         Barpointer.Background = new ReportingBrushConverter().ConvertFromInvariantString(pointer.PointerStyle.BackgroundColor);
                                     else
                                         Barpointer.Background = GradientColor(pointer.PointerStyle.BackgroundColor, pointer.PointerStyle.BackgroundGradientEndcolor, pointer.PointerStyle.BackgroundGradientType);

#endif
                                Barpointer.EnablePointerInteraction = false;
                                Linearscale.Pointers.Add(Barpointer);
                            }
                            if (!string.IsNullOrEmpty(pointer.PointerTooltip))
                            {
                                ToolTipService.SetToolTip(this, pointer.PointerTooltip);
                            }
                        ...
                        foreach (var scalerange in scales.ScaleRange)
                        {
                            ...
                            Range.BorderThickness = new Thickness(scalerange.RangeStyle.BorderWidth);
                            if (scalerange.EndValue.Value == 0 && scalerange.StartValue.Value == 0)
                            {
                                Range.EndValue = 100;
                                Range.StartValue = 70;
                            }
                            ...
                            {
                                Range.EndValue = scalerange.EndValue.Value;
                                Range.StartValue = scalerange.StartValue.Value;
                            }
                            Range.StartWidth = scalerange.StartWidth;
                            Range.EndWidth = scalerange.EndWidth;
                            Range.RangePosition = (ScalePlacement)scalerange.RangePlacement;
#if !SILVERLIGHT
                            if (scalerange.RangeStyle.BackgroundGradientType == Syncfusion.RDL.DOM.BackgroundGradientType.None)
                                Range.BackgroundBrush = new ReportingBrushConverter().ConvertFromInvariantString(scalerange.RangeStyle.BackgroundColor);
                            else
                                Range.BackgroundBrush = GradientColor(scalerange.RangeStyle.BackgroundColor, scalerange.RangeStyle.BackgroundGradientEndcolor, scalerange.RangeStyle.BackgroundGradientType);
#else
                                if (scalerange.RangeStyle.BackgroundGradientType == Syncfusion.RDL.DOM.BackgroundGradientType.None)
                                    Range.Background = new ReportingBrushConverter().ConvertFromInvariantString(scalerange.RangeStyle.BackgroundColor);
                                else
                                    Range.Background = GradientColor(scalerange.RangeStyle.BackgroundColor, scalerange.RangeStyle.BackgroundGradientEndcolor, scalerange.RangeStyle.BackgroundGradientType);

#endif
                            Range.DistanceFromScale = scalerange.ScaleDistance;
                            if (!string.IsNullOrEmpty(scalerange.ToolTip))
                            {
                                ToolTipService.SetToolTip(this, scalerange.ToolTip);
                            }

                            if (scalerange.Hidden)
                            {
#if !SILVERLIGHT
                                this.Visibility = Visibility.Hidden;
#else
                            ...
                            Linearscale.Ranges.Add(Range);
                        }
                        this.Scales.Add(Linearscale);
                    }
                }
            }
            catch (Exception)
