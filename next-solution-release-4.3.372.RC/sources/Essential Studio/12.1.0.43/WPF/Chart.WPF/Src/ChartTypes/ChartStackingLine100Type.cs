// <copyright file="ChartStackingLine100Type.cs" company="Syncfusion">
...
namespace Syncfusion.Windows.Chart
{
    ...
    /// </remarks>
    /// <seealso cref="ChartStackingLine100Type"/>
#if SyncfusionFramework4_0
    [System.ComponentModel.DesignTimeVisible(false)]
#endif
    public class ChartStackingLine100Type : ChartStackingLineType
    {
        ...
        public static readonly DependencyProperty ShowValueAsProbabilityProperty =
            DependencyProperty.RegisterAttached("ShowValueAsProbability", typeof(bool), typeof(ChartStackingLine100Type), new UIPropertyMetadata(false, new PropertyChangedCallback(OnShowValueAsProbabilityChanged)));
        ...
        public new static readonly DependencyProperty RequiresNegativeSeriesStackProperty =
              DependencyProperty.RegisterAttached("RequiresNegativeSeriesStack", typeof(bool), typeof(ChartStackingLine100Type), new UIPropertyMetadata(true, new PropertyChangedCallback(OnRequiresNegativeSeriesStackChanged)));
        #endregion
        ...
        protected override void CalculateSegments(ChartSeries series, ChartIndexedDataPoint[] points)
        {
            ...
            for (int i = 0; i < points.Length; i++)
            {
                ...
                if (ChartStackingLine100Type.GetRequiresNegativeSeriesStack(series.Area))
                {
            ...
            for (int i = 0; i < points.Length; i++)
            {
                ...
                if(areaPoints[i]!=null)
                    if (ChartStackingLine100Type.GetShowValueAsProbability(series.Area) && ChartStackingLine100Type.GetRequiresNegativeSeriesStack(series.Area))
                    {
                            areaPoints[2 * points.Length - i - 1] = new ChartPoint(areaPoints[i].X, areaPoints[i].Y + ((points[i].DataPoint.Y / Math.Abs(sum)) * 100) / 100);
                            sum = 0;
                    }
                    else if (ChartStackingLine100Type.GetShowValueAsProbability(series.Area))
                    {
                    ...
                    else if (ChartStackingLine100Type.GetRequiresNegativeSeriesStack(series.Area))
                    {
