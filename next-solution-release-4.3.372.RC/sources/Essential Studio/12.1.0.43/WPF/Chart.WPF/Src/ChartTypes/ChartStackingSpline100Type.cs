// <copyright file="ChartStackingSpline100Type.cs" company="Syncfusion">
...
namespace Syncfusion.Windows.Chart
{
    ...
    /// </remarks>
    /// <seealso cref="ChartStackingSpline100Type"/>
#if SyncfusionFramework4_0
    [System.ComponentModel.DesignTimeVisible(false)]
#endif
    public class ChartStackingSpline100Type : ChartStackingSplineType
    {
        ...
        public static readonly DependencyProperty ShowValueAsProbabilityProperty =
            DependencyProperty.RegisterAttached("ShowValueAsProbability", typeof(bool), typeof(ChartStackingSpline100Type), new UIPropertyMetadata(false, new PropertyChangedCallback(OnShowValueAsProbabilityChanged)));
        ...
        public new static readonly DependencyProperty RequiresNegativeSeriesStackProperty =
              DependencyProperty.RegisterAttached("RequiresNegativeSeriesStack", typeof(bool), typeof(ChartStackingSpline100Type), new UIPropertyMetadata(true, new PropertyChangedCallback(OnRequiresNegativeSeriesStackChanged)));
        #endregion
        ...
        /// <seealso cref="ChartStackingSplineType"/>
        ...
        protected override void CalculateSegments(ChartSeries series, ChartIndexedDataPoint[] points)
        {
            ...
            ChartIndexedDataPoint[] newpts = new ChartIndexedDataPoint[points.Length];
            double[] yCoef;
            if (points.Length >= 2)
            {
                for (int i = 0; i < points.Length; i++)
                {
                    ...
                    if (ChartStackingSpline100Type.GetRequiresNegativeSeriesStack(series.Area))
                    {
                ...
                for (int i = 0; i < points.Length; i++)
                {
                    ...
                    {
                        if (chartSeries.PointsCount > points[i].Index)
                        {
                    ...
                    if (areaPoints[i] != null)
                        if (ChartStackingSpline100Type.GetShowValueAsProbability(series.Area) && ChartStackingSpline100Type.GetRequiresNegativeSeriesStack(series.Area))
                        {
                            areaPoints[2 * points.Length - i - 1] = new ChartPoint(areaPoints[i].X, areaPoints[i].Y + ((points[i].DataPoint.Y / Math.Abs(sum)) * 100) / 100);
                            sum = 0;
                        }
                        else if (ChartStackingSpline100Type.GetShowValueAsProbability(series.Area))
                        {
                        ...
                        else if (ChartStackingSpline100Type.GetRequiresNegativeSeriesStack(series.Area))
                        {
                ...
                for (int i = areaPoints.Length - 1, j = 0; i >= points.Length && j < points.Length; i--, j++)
                {
                ...
                for (int i = areaPoints.Length - 1, j = 1; i > points.Length && j < points.Length; i--, j++)
                {
                    ...
                    ChartSplineType.GetBezierControlPoints(startPoint, endPoint, yCoef[j - 1], yCoef[j], out startControlPoint, out endControlPoint);
                    series.Segments.Add(new ChartSplineSegment(areaPoints[i], startControlPoint, endControlPoint, areaPoints[i - 1], points[i - points.Length], points[i - 1 - points.Length], series));
                }
                ...
                if (series.AdornmentsInfo.Visible)
                {
                    ...
                    for (int i = 0; i < points.Length; i++)
                    {
                        ...
                        else
                        {
                            ...
                            {
                                series.Adornments.Add(new ChartAdornment(new ChartPoint(areaPoints[i].X, areaPoints[i].Y + ((points[i].DataPoint.Y / Math.Abs(sum)) * 100)), points[i], series));
                                sum = 0;
                            }
                            else
                            {
