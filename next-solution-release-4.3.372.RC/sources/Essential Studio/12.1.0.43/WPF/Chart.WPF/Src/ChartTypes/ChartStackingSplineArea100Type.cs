// <copyright file="ChartStackingSplineArea100Type.cs" company="Syncfusion">
...
namespace Syncfusion.Windows.Chart
{
    ...
    /// </remarks>
    /// <seealso cref="ChartStackingSplineArea100Type"/>
#if SyncfusionFramework4_0
    [System.ComponentModel.DesignTimeVisible(false)]
#endif
    public class ChartStackingSplineArea100Type : ChartStackingSplineAreaType
    {
        ...
        public static readonly DependencyProperty ShowValueAsProbabilityProperty =
            DependencyProperty.RegisterAttached("ShowValueAsProbability", typeof(bool), typeof(ChartStackingSplineArea100Type), new UIPropertyMetadata(false, new PropertyChangedCallback(OnShowValueAsProbabilityChanged)));
        ...
        public new static readonly DependencyProperty RequiresNegativeSeriesStackProperty =
              DependencyProperty.RegisterAttached("RequiresNegativeSeriesStack", typeof(bool), typeof(ChartStackingSplineArea100Type), new UIPropertyMetadata(true, new PropertyChangedCallback(OnRequiresNegativeSeriesStackChanged)));
        #endregion
        ...
        protected override void CalculateSegments(ChartSeries series, ChartIndexedDataPoint[] points)
        {
            ...
            if (points.Length >= 2)
            {
                ...
                for (int i = 0; i < points.Length; i++)
                {
                    ...
                    if (ChartStackingSplineArea100Type.GetRequiresNegativeSeriesStack(series.Area))
                    {
                ...
                for (int i = 0; i < points.Length; i++)
                {
                    ...
                    if (areaPoints[i] != null)
                        if (ChartStackingSplineArea100Type.GetShowValueAsProbability(series.Area) && ChartStackingSplineArea100Type.GetRequiresNegativeSeriesStack(series.Area))
                        {
                            areaPoints[2 * points.Length - i - 1] = new ChartPoint(areaPoints[i].X, areaPoints[i].Y + ((points[i].DataPoint.Y / Math.Abs(sum)) * 100) / 100);
                            sum = 0;
                        }
                        else if (ChartStackingSplineArea100Type.GetShowValueAsProbability(series.Area))
                        {
                        ...
                        else if (ChartStackingSplineArea100Type.GetRequiresNegativeSeriesStack(series.Area))
                        {
