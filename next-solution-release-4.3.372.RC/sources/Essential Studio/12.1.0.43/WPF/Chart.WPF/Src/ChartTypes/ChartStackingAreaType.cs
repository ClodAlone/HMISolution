// <copyright file="ChartStackingAreaType.cs" company="Syncfusion">
...
#endif
    public class ChartStackingAreaType : ChartAreaType
    {
        #region Properties
        ...
        public static readonly DependencyProperty RequiresNegativeSeriesStackProperty =
              DependencyProperty.RegisterAttached("RequiresNegativeSeriesStack", typeof(bool), typeof(ChartStackingAreaType), new UIPropertyMetadata(true, new PropertyChangedCallback(OnRequiresNegativeSeriesStackChanged)));
        ...
        protected override void CalculateSegments(ChartSeries series, ChartIndexedDataPoint[] points)
        {
            ...
            for (int i = 0; i < points.Length; i++)
            {
                ...
                if (ChartStackingAreaType.GetRequiresNegativeSeriesStack(series.Area))
                {
            ...
            int firstStackedSeries = 0;
            
            for (int i = 0; i < series.Area.Series.Count; i++)
            {
                if (series.Area.Series[i].Type == ChartTypes.StackingArea )
                {
                    firstStackedSeries=i;
                    break;
                }

            }
            bool firstboolcheck = false;
            if(series.Area !=null)
            if (series.Area.Series.Count >= firstStackedSeries)
            {
                if (series == series.Area.Series[firstStackedSeries])
                {
                    firstboolcheck = true;
            ...
            for (int i = 0; i < points.Length; i++)
            {
                ...
                if(ChartStackingAreaType.GetRequiresNegativeSeriesStack(series.Area))
                    areaPoints[2 * points.Length - i - 1] = new ChartPoint(areaPoints[i].X, firstboolcheck ? y: areaPoints[i].Y + y );
                else
                    areaPoints[2 * points.Length - i - 1] = new ChartPoint(areaPoints[i].X, firstboolcheck ? y : areaPoints[i].Y + y );
            }
