...
namespace Syncfusion.UI.Xaml.Charts
{
    static partial class WriteableBitmapExtensions
    {
        ...
        /// <param name="y2">The y-coordinate of the end point.</param>
        /// <param name="color">The color for the line.</param>
        public static unsafe void DrawLineBresenham(this WriteableBitmap bmp, int x1, int y1, int x2, int y2, int color)
        {
        ...
        /// <param name="y2">The y-coordinate of the end point.</param>
        /// <param name="color">The color for the line.</param>
        public static unsafe void DrawLineDDA(this WriteableBitmap bmp, int x1, int y1, int x2, int y2, int color)
        {
            // Use refs for faster access (really important!) speeds up a lot!
            int w = bmp.PixelWidth;
            int h = bmp.PixelHeight;
        ...
        {
            DrawLine(bmp, bmp.PixelWidth, bmp.PixelHeight, x1, y1, x2, y2, color);
        }
        ...
        /// <param name="y2">The y-coordinate of the end point.</param>
        /// <param name="color">The color for the line.</param>
        public static unsafe void DrawLine(WriteableBitmap bmp, int pixelWidth, int pixelHeight, int x1, int y1, int x2, int y2, int color)
        {
            var pixels = bmp.GetPixels();
        ...
        {
            DrawLineAa(bmp, bmp.PixelWidth, bmp.PixelHeight, x1, y1, x2, y2, color);
        }
        ...
        /// <param name="color">The color for the line.</param>
        /// </summary> 
        public static void DrawLineAa(WriteableBitmap bmp, int pixelWidth, int pixelHeight, int x1, int y1, int x2, int y2, int color)
        {
            if (x1 == 0 && x2 == 0) return;
            ...
            if (x1 < 1 && x2 < 1) return;
            if (x1 > pixelWidth - 2 && x2 > pixelWidth - 2) return;
            ...
            do
            {
                AlphaBlendNormalOnPremultiplied(bmp, addr, (zeroDot75 - twovduMulInvD) >> PRECISION_SHIFT, srb, sg);
                AlphaBlendNormalOnPremultiplied(bmp, addr + vincr, (invD2Du + twovduMulInvD) >> PRECISION_SHIFT, srb, sg);
                AlphaBlendNormalOnPremultiplied(bmp, addr - vincr, (invD2Du - twovduMulInvD) >> PRECISION_SHIFT, srb, sg);
        ...
        private static unsafe void AlphaBlendNormalOnPremultiplied(WriteableBitmap bmp, int index, int sa, uint srb, uint sg)
        {
            var pixels = bmp.GetPixels();
        ...
        public static void DrawPolyline(this WriteableBitmap bmp, int[] points, int color)
        {
            ...
            for (var i = 2; i < points.Length; i += 2)
            {
                ...
                DrawLine(bmp, w, h, x1, y1, x2, y2, color);
                x1 = x2;
                y1 = y2;
            }
        }
        ...
        public static void DrawTriangle(this WriteableBitmap bmp, int x1, int y1, int x2, int y2, int x3, int y3, int color)
        {
            ...
            DrawLine(bmp, w, h, x1, y1, x2, y2, color);
            DrawLine(bmp, w, h, x2, y2, x3, y3, color);
            DrawLine(bmp, w, h, x3, y3, x1, y1, color);
        }
        ...
        /// <param name="y2">The y-coordinate of the bounding rectangle's bottom side.</param>
        /// <param name="color">The color.</param>
        public static unsafe void DrawRectangle(this WriteableBitmap bmp, int x1, int y1, int x2, int y2, int color)
        {
            // Use refs for faster access (really important!) speeds up a lot!
            var w = bmp.PixelWidth;
            var h = bmp.PixelHeight;
        ...
        /// <param name="y2">The y-coordinate of the bounding rectangle's bottom side.</param>
        /// <param name="color">The color.</param>
        public static unsafe void FillRectangle(this WriteableBitmap bmp, int x1, int y1, int x2, int y2, int color)
        {
            ...
            var endOffset = startY + x2;
            var len = (x2 - x1);
            var srcOffsetBytes = startYPlusX1;
            var offset2 = ((y2) * w + x1);
            var x = startYPlusX1;
            for (var y = srcOffsetBytes + w; y <= offset2; y += w)
            {
                for (x = startYPlusX1; x < endOffset; x++)
                {
                    pixels[x] = color;
                }
                startYPlusX1 = y;
                endOffset = startYPlusX1 + len;
            }
        }
        ...
        /// <param name="points">The points of the polygon in x and y pairs, therefore the array is interpreted as (x1, y1, x2, y2, ..., xn, yn).</param>
        /// <param name="color">The color for the line.</param>
        public static unsafe void FillPolygon(this WriteableBitmap bmp, int[] points, int color)
        {
            // Use refs for faster access (really important!) speeds up a lot!
            var w = bmp.PixelWidth;
            var h = bmp.PixelHeight;
        ...
        /// <param name="yr">The radius of the ellipse in y-direction.</param>
        /// <param name="color">The color for the line.</param>
        public static unsafe void DrawEllipseCentered(this WriteableBitmap bmp, int xc, int yc, int xr, int yr, int color)
        {
            var pixels = bmp.GetPixels();
            var w = bmp.PixelWidth;
            var h = bmp.PixelHeight;
        ...
        /// <param name="yr">The radius of the ellipse in y-direction.</param>
        /// <param name="color">The color for the line.</param>
        public static unsafe void FillEllipseCentered(this WriteableBitmap bmp, int xc, int yc, int xr, int yr, int color)
        {
            // Use refs for faster access (really important!) speeds up a lot!
