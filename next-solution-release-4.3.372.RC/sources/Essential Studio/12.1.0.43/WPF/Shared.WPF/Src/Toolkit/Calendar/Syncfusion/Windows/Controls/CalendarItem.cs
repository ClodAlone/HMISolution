    ...
    public sealed partial class CalendarItem : Control
    {
            ...
            else
            {
                ...
                switch (this.Owner.SelectionMode)
                {
                    ...
                    case CalendarSelectionMode.SingleDate:
                    {
                        ...
                        if (!ctrl)
                        {
                            if (selectedDate.Equals(this.Owner.SelectedDate))
                            {
                                if (this.Owner.TemplatedParent != null && this.Owner.TemplatedParent is DateTimeEdit)
        ...
        private void Month_MouseLeftButtonDown(object sender, MouseButtonEventArgs e)
        {
            ...
            if (b != null)
            {
                ...
                if (this.Owner != null)
                {
                    if ((this.Owner.TemplatedParent as DateTimeEdit) != null)
                    {
                        this.Owner.DisableDateSelection = (this.Owner.TemplatedParent as DateTimeEdit).DisableDateSelection;
                    }
                    if ((this.Owner.DisplayMode == CalendarMode.Year )&& this.Owner.DisableDateSelection)
                    {
        ...
        private void PopulateGrids()
        {
            if (_monthView != null)
            {
                if (_dayTitleTemplate != null)
                {
                    ...
                    border.SetValue(Grid.ColumnSpanProperty, COLS);
                    border.HorizontalAlignment = HorizontalAlignment.Stretch ;
                    border.SetValue(Border.MarginProperty ,new Thickness(0,0,0,2));
                    border.VerticalAlignment = VerticalAlignment.Bottom;
                    //this._monthView.Children.Add(border);
                }
