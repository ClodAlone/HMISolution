...
namespace Syncfusion.Windows.Controls.Grid
{
    ...
    public class GridCellNestedGridRenderer : GridVirtualizingCellRenderer<GridCellNestedGridEditor>
    {
        ...
        protected override bool ShouldGridTryToHandlePreviewKeyDown(KeyEventArgs e)
        {
            ...
            /* Unreachable code
                        if (false)//!CurrentCellUIElement.IsKeyboardFocusWithin)
                        {
                            if (e.Key == Key.Down)
                            {
    ...
    public class GridCellNestedGridEditor : GridControlBase
    {
        ...
        public void ArrangeNested(ScrollViewer sv, ArrangeCellArgs aca, GridRenderStyleInfo style)
        {
            ...
            if (style != null)//Remove the padding from the CellCliprect
                clipRectSize = new Size(aca.CellClipRect.Width - (style.Padding.Left + style.Padding.Right), aca.CellClipRect.Height - (style.Padding.Top + style.Padding.Bottom));
            else
                clipRectSize = new Size(aca.CellClipRect.Width, aca.CellClipRect.Height);
            ...
            {
                case GridNestedAxisLayout.Normal:
                    this.HScrollBar.Value = Model.CachedHScrollValue + HScrollBar.Minimum + aca.CellClipRect.Left - aca.OriginalCellRect.Left;
                    break;
        ...
        public override bool MoveCurrentCellWithArrowKey(KeyEventArgs e)
        {
            ...
            bool selectAll = e.Key == Key.A && Keyboard.Modifiers.Equals(System.Windows.Input.ModifierKeys.Control);
            if (!b && ParentGrid != null && !selectAll)
            {
                if (e.Key == Key.Tab || (e.Key == Key.Tab && Keyboard.Modifiers == ModifierKeys.Shift) || e.Key == Key.Down
