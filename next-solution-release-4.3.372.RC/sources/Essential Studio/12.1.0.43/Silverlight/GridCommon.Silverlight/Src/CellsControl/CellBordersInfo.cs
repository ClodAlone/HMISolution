...
#endif
{
    ...
    public class CellBordersInfoStore : StyleInfoStore
    {
        ...
        static CellBordersInfoStore()
        {
            ...
            TopProperty.Format += BordersProperty_Format;
            TopProperty.Parse += BordersProperty_Parse;
            LeftProperty.Format += BordersProperty_Format;
            LeftProperty.Parse += BordersProperty_Parse;
            RightProperty.Format += BordersProperty_Format;
            RightProperty.Parse += BordersProperty_Parse;
            BottomProperty.Format += BordersProperty_Format;
            BottomProperty.Parse += BordersProperty_Parse;
        }

        static void BordersProperty_Parse(object sender, StyleInfoPropertyConvertEventArgs e)
        {
            var parseValue = e.Value != null ? e.Value.ToString() : string.Empty;
            if (parseValue != string.Empty)
            {
                try
                {
                    var thicknessString = parseValue.Split(new char[] { ',' })[0];
                    var hexaColor = parseValue.Split(new char[] { ',' })[1];
                    ...
                    e.Value = new Pen(new SolidColorBrush(color), double.Parse(thicknessString));
                    e.Handled = true;
                }
                catch
                {
        ...
        static void BordersProperty_Format(object sender, StyleInfoPropertyConvertEventArgs e)
        {
            var pen = e.Value as Pen;
            var brush = pen.Brush;
            var isSolidColorBrush = brush is SolidColorBrush;
            if (isSolidColorBrush)
            {
                e.Value = string.Format("{0},{1}", pen.Thickness.ToString(), ((SolidColorBrush)pen.Brush).Color.ToString());
            }
		...
		private CellBordersInfoStore(SerializationInfo info, StreamingContext context)
			: base(info, context)
		{
#if DEBUG
