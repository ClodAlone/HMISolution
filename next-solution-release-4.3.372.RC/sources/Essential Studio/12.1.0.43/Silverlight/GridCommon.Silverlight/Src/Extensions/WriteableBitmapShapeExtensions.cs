...
namespace Syncfusion.Windows
{
    ...
    internal static partial class WriteableBitmapExtensions
    {
        ...
        public static void DrawLine(this WriteableBitmap bmp, int x1, int y1, int x2, int y2, Color color, int thickness)
        {
            // Add one to use mul and cheap bit shift for multiplicaltion
            var a = color.A + 1;
            var col = (color.A << 24)
                     ...
                     | ((byte)((color.G * a) >> 8) << 8)
                     | ((byte)((color.B * a) >> 8));
            bmp.DrawLine(x1, y1, x2, y2, col, thickness);
        }
        ...
        /// <param name="color">The color for the line.</param>
        /// <param name="thickness">The thickness for the line.</param>
        public static void DrawLine(this WriteableBitmap bmp, int x1, int y1, int x2, int y2, int color, int thickness)
        {
            DrawLine(bmp.Pixels, bmp.PixelWidth, bmp.PixelHeight, x1, y1, x2, y2, color, thickness);
        }


        /// <summary>
        /// Draws a colored line by connecting two points using an optimized DDA(Thickness 1).
        ...
        {
            DrawLine(pixels, pixelWidth, pixelHeight, x1, y1, x2, y2, color, 1);
        }
        ...
        /// <param name="color">The color for the line.</param>
        /// <param name="thickness">The thickness for the line.</param>
        public static void DrawLine(int[] pixels, int pixelWidth, int pixelHeight, int x1, int y1, int x2, int y2, int color, int thickness)
        {
            // Distance start and end point
            int dx = x2 - x1;
            int dy = y2 - y1;
                ...
                for (int i = 0; i < lenX; i++)
                {
                    ...
                    if (x1 >= 0 && x1 < pixelWidth && y1 >= 0 && y1 < pixelHeight)
                    {
                        ...
                        for (int t = 1; t <= thickness; t++)
                        {
                            ...
                            {
                                if ((y1 - j) >= 0)
                                    pixels[((y1 - j) * pixelWidth + x1)] = color;
                            }
                            else if (((y1 + j) * pixelWidth + x1) < pixels.Length)
                            {
                                pixels[((y1 + j) * pixelWidth + x1)] = color;
                                j++;
                            }
