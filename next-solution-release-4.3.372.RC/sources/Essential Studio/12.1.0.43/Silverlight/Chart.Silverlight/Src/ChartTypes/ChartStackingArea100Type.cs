...
{
    /// <summary>
    /// Class implementation for StackingArea100segment
    /// </summary>
    public class StackingArea100Segment : Segment
    {
        ...
        public static readonly DependencyProperty TemplateProperty =
            DependencyProperty.Register("Template", typeof(DataTemplate), typeof(StackingArea100Segment), new PropertyMetadata(null));

        /// <summary>
        /// Identifies the AreaPoints dependency property.
        /// </summary>
        public static readonly DependencyProperty AreaPointsProperty =
          DependencyProperty.Register("AreaPoints", typeof(Geometry), typeof(StackingArea100Segment), new PropertyMetadata(null));
        ...
        internal StackingArea100Segment(ChartPointsCollection areapoints, ChartPointsCollection correspondingPoint, ChartSeries series)
            : base(series, correspondingPoint)
        {
            this.Template = ResourceManager.GetSeriesTemplate(typeof(ChartStackingArea100Type), ChartTypes.StackingArea100);
    ...
    public class ChartStackingArea100Type : ChartType
    {
        ...
        public static readonly DependencyProperty ShowValueAsProbabilityProperty =
            DependencyProperty.RegisterAttached("ShowValueAsProbability", typeof(bool), typeof(ChartStackingArea100Type), new PropertyMetadata(false, new PropertyChangedCallback(OnShowValueAsProbabilityChanged)));
        #endregion
        ...
        protected override void CalculateSegments(ChartSeries series, ChartPointsCollection points)
        {
            ...
            {
                if (series1.Visibility == Visibility.Visible && series1.Type == ChartTypes.StackingArea100 && series1.Data!=null)
                {
            ...
            for (int i = 0; i < newPoints.Count; i++)
            {
                ...
                for (int j = 0; j < series.StackedData.Count; j++)
                {
                    ...
                    {
                        if(GetShowValueAsProbability(series.Area))
                        {
                            y1 = ((series.StackedData[j].Y/sum[j].Y)*100)/100 + (series.YAxis.VisibleRange.Start * (-1));
                        }
                        else
                        {
                            y1 = (series.StackedData[j].Y / sum[j].Y) * 100 + (series.YAxis.VisibleRange.Start * (-1));
                        }
                        break;
                    }
                }
                ...
                if (series.AdornmentsInfo != null && series.AdornmentsInfo.Visible)
                {
                    ...
                    if (!double.IsNaN(series.StackedData[i].Y))
                        segmentsdata.Add(new ChartAdornment(newPoints[i], new ChartPoint(series.StackedData[i].X, y1), newPoints, series, 0d));
                }
            }
            ...
            for (int i = 0; i < previouspoints.Count; i++)
            {
                ...
                if (GetShowValueAsProbability(series.Area))
                {
                    y = ((previouspoints[i].Y / sum[i].Y) * 100)/100 + (series.YAxis.VisibleRange.Start * (-1)) + spaceval;
                }
                else
                {
                    y = (previouspoints[i].Y / sum[i].Y) * 100 + (series.YAxis.VisibleRange.Start * (-1)) + spaceval;
                }
                if (series.Area.Host == Host.Chart)
                {
            ...
            series.Segments.Add(new StackingArea100Segment(polypoints, newPoints, series));
