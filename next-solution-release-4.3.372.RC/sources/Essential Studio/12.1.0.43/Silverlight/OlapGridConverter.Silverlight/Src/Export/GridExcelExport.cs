...
namespace Syncfusion.Silverlight.Grid.Olap.Converter
{
    ...
    public class GridExcelExport
    {
        ...
        /// <param name="layout">The layout</param>
        /// <param name="itemSource">if set to <c>true</c> [itemsource]</param>
        public GridExcelExport(PivotEngine drillResult, object gridStyleinfo, GridLayout layout, bool itemSource)
            : this(drillResult, (ExportingGridStyleInfo)gridStyleinfo, layout, itemSource)
        {
        ...
        public void Export(Stream stream)
        {
            for (int i = 1; i <= PivotData.RowsCount; i++)
            {
                for (int j = 1; j <= PivotData.TableColumns.Count; j++)
                {
                    ...
                    if (cellDescriptor.CellType == PivotCellDescriptorType.Value && cellDescriptor.Value != "")
                    {
                            sheet.Range[i, j].Value2 = cellDescriptor.DoubleValue;
                            var cell = cellDescriptor.Tag as Syncfusion.OlapSilverlight.Data.Cell;
                            var format = string.Empty;
                            if (cell != null)
                            {
                                if (cell.FormatString == "Currency")
                                {
                                ...
                                else if (cell.FormatString == "Percentage")
                                {
                        ...
                        this.sheet.Range[i, j].Text = cellDescriptor.CellValue;
                }                    
            }
