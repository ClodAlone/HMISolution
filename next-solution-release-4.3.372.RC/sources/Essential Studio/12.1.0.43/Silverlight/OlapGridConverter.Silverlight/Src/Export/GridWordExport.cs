...
namespace Syncfusion.Silverlight.Grid.Olap.Converter
{
    ...
    public class GridWordExport
    {
        ...
        private ExportingGridStyleInfo GridExportStyle
        {
            get
            {
                return gridExportStyle;
            }
            set
            {
                gridExportStyle = value;
            }
        ...
        public void Export(Stream stream, ExportingGridStyleInfo styleinfo)
        {
            ...
            ////option to Allow table break
            if (styleinfo != null)
                this.GridExportStyle = styleinfo;
            else
            {
                styleinfo = new Syncfusion.Silverlight.Grid.Olap.Common.ExportingGridStyleInfo();
                this.GridExportStyle = styleinfo;               
            }              
            ...
            for (col = 7; col < Engine.TableColumns.Count; col += 7)
            {
                ...
                for (int row = 0; row <= Engine.RowsCount - 1; row++)
                {
                    ...
                    for (int c = temp; c <= count; c++)
                    {
                        ...
                        if (descriptorvalue.CellType == PivotCellDescriptorType.Value || descriptorvalue.CellExTypes.Count > 0)
                        {
                            if (descriptorvalue.CellExTypes.Count > 0)
                            {
                                if (descriptorvalue.CellExTypes.Contains(PivotCellDescriptorType.SummaryColumn.ToString()))
                                {
                                    ...
                                    summary.CharacterFormat.TextColor = ToDrawingColor(styleinfo.SummaryColumnForegroundColor);
                                ...
                                {
                                    IWTextRange summary = docTable.Rows[row].Cells[cellIndex].AddParagraph().AppendText(Value);
                                    summary.CharacterFormat.TextColor = ToDrawingColor(styleinfo.SummaryRowForegroundColor);
                                    summary.CharacterFormat.FontName = styleinfo.SummaryFontName;
                                    summary.CharacterFormat.FontSize = (float)styleinfo.SummaryFontSize;
                                    docTable.Rows[row].Cells[cellIndex].CellFormat.BackColor = ToDrawingColor(styleinfo.SummaryRowBackgroundColor);
                                ...
                                else
                                {
                                    if (Layout == GridLayout.ExcelLikeLayout && descriptorvalue.CellType != PivotCellDescriptorType.RowHeader && descriptorvalue.CellType != PivotCellDescriptorType.SummaryRow)
                                    {
                                        ...
                                        text.CharacterFormat.TextColor = ToDrawingColor(styleinfo.ValueTextColor);
                                        docTable.Rows[row].Cells[cellIndex].CellFormat.BackColor = ToDrawingColor(styleinfo.SummaryColumnBackgroundColor);// Color(styleinfo.SummaryColumnBackgroundColor);
                                    }
                                }
                            }
                            else
                        ...
                        {
                            docTable.Rows[row].Cells[cellIndex].CellFormat.BackColor = ToDrawingColor(styleinfo.HeaderBackgroundColor);
                            IWTextRange text = docTable.Rows[row].Cells[cellIndex].AddParagraph().AppendText(Value);
                            text.CharacterFormat.TextColor = ToDrawingColor(styleinfo.HeaderForeGroundColor);
                        ...
                        {
                            docTable.Rows[row].Cells[cellIndex].CellFormat.BackColor = ToDrawingColor(styleinfo.HeaderRowBackgroundColor);
                            IWTextRange text = docTable.Rows[row].Cells[cellIndex].AddParagraph().AppendText(Value);
                            text.CharacterFormat.TextColor = ToDrawingColor(styleinfo.HeaderRowForegroundColor);
                            text.CharacterFormat.FontName = GridExportStyle.HeaderFontName != "" ? GridExportStyle.HeaderFontName : "Times New Roman";
                            text.CharacterFormat.FontSize = (float)GridExportStyle.HeaderFontSize;
                            if (descriptor.Range.Height > 1)
                            {
                        ...
                        {
                            docTable.Rows[row].Cells[cellIndex].CellFormat.BackColor = ToDrawingColor(styleinfo.SummaryColumnBackgroundColor);
                            IWTextRange text = docTable.Rows[row].Cells[cellIndex].AddParagraph().AppendText(Value);
                            text.CharacterFormat.TextColor = ToDrawingColor(styleinfo.SummaryColumnForegroundColor);
                            ...
                            if (descriptor.CellValue == m_Total)
                            {
                                ...
                                {
                                    docTable.Rows[i].Cells[cellIndex].CellFormat.BackColor = ToDrawingColor(styleinfo.SummaryColumnBackgroundColor);
                                }
                            }
                            if (descriptor.Range.Height > 1)
                            {
                        ...
                        if (descriptor.CellType == PivotCellDescriptorType.SummaryRow)
                        {
                            ...
                            IWTextRange text = docTable.Rows[row].Cells[cellIndex].AddParagraph().AppendText(Value);
                            text.CharacterFormat.TextColor = ToDrawingColor(styleinfo.SummaryRowForegroundColor);
                            text.CharacterFormat.FontName = GridExportStyle.SummaryFontName != "" ? GridExportStyle.SummaryFontName : "Times New Roman";
                            text.CharacterFormat.FontSize = (float)GridExportStyle.SummaryFontSize;
                            if (descriptor.CellValue == m_Total || descriptor.CellValue == m_GrandTotal)
                            {
                                if (this.Layout != GridLayout.ExcelLikeLayout)
                                {
                                    ...
                                    {
                                        docTable.Rows[row].Cells[i].CellFormat.BackColor = ToDrawingColor(styleinfo.SummaryRowBackgroundColor);
                                    }
                                }
                                else
                                {
            ...
            if ((col - 7) < Engine.TableColumns.Count)
            {
                ...
                for (int r = 0; r <= Engine.RowsCount - 1; r++)
                {
                    ...
                    for (int c = (col - 7); c < Engine.TableColumns.Count; c++)
                    {
                        ...
                        if (descriptor.CellType == PivotCellDescriptorType.Value || descriptor.CellExTypes.Count > 0)
                        {
                            if (descriptor.CellExTypes.Count > 0)
                            {
                                ...
                                else if (descriptor.CellExTypes.Contains(PivotCellDescriptorType.SummaryRow.ToString()))
                                {
                                    ...
                                    summary.CharacterFormat.FontName = styleinfo.SummaryFontName;
                                    summary.CharacterFormat.FontSize = (float)styleinfo.SummaryFontSize;
                                    docTable.Rows[r].Cells[cellIndex].CellFormat.BackColor = ToDrawingColor(styleinfo.SummaryRowBackgroundColor);
                                }
                                else
                                {
                        ...
                        if (descriptor.CellType == PivotCellDescriptorType.ColumnHeader)
                        {
                            docTable.Rows[r].Cells[cellIndex].CellFormat.BackColor = ToDrawingColor(styleinfo.HeaderBackgroundColor);
                            IWTextRange text = docTable.Rows[r].Cells[cellIndex].AddParagraph().AppendText(Value);
                            text.CharacterFormat.TextColor = ToDrawingColor(styleinfo.HeaderForeGroundColor);
                            text.CharacterFormat.FontName = GridExportStyle.HeaderFontName != "" ? GridExportStyle.HeaderFontName : "Times New Roman";
                            text.CharacterFormat.FontSize = (float)GridExportStyle.HeaderFontSize;
                        ...
                        if (descriptor.CellType == PivotCellDescriptorType.RowHeader)
                        {
                            docTable.Rows[r].Cells[c].CellFormat.BackColor = ToDrawingColor(styleinfo.HeaderRowBackgroundColor);
                            IWTextRange text = docTable.Rows[r].Cells[cellIndex].AddParagraph().AppendText(Value);
                            text.CharacterFormat.TextColor = ToDrawingColor(styleinfo.HeaderRowForegroundColor);
                            text.CharacterFormat.FontName = GridExportStyle.HeaderFontName != "" ? GridExportStyle.HeaderFontName : "Times New Roman";
                            text.CharacterFormat.FontSize = (float)GridExportStyle.HeaderFontSize;
                        ...
                        if (descriptor.CellType == PivotCellDescriptorType.SummaryColumn)
                        {
                            docTable.Rows[r].Cells[cellIndex].CellFormat.BackColor = ToDrawingColor(styleinfo.SummaryColumnBackgroundColor);
                        ...
                        if (descriptor.CellType == PivotCellDescriptorType.SummaryRow)
                        {
                            ...
                            if (descriptor.CellValue == m_Total)
                            {
                                ...
                                {
                                    docTable.Rows[r].Cells[i].CellFormat.BackColor = ToDrawingColor(styleinfo.SummaryRowBackgroundColor);
                                }
                            }
        ...
        /// </summary>
        /// <param name="stream">The stream</param>
        /// <param name="gridStyleInfo">The styleinfo</param>
        public void Export(Stream stream, object gridStyleInfo)
        {
            Export(stream, (ExportingGridStyleInfo)gridStyleInfo);
        }
        #endregion
        ...
        {
            System.Windows.Media.SolidColorBrush solidBrush = Convert(color);//,typeof(SolidColorBrush),null);
