//-------------------------------------------------------------------------------------------------
// <copyright file="GridVolatileData.cs" company="syncfusion">
...
namespace Syncfusion.Windows.Forms.Grid
{
    ...
    internal interface IGridVolatileDataContainer
    {
        GridBaseStylesMap BaseStylesMap { get; }
        int HeaderRowCount { get; }
        int HeaderColCount { get; }
        void ChangeCell(int rowIndex, int colIndex, GridStyleInfo style);
        void GetCellInfo(int rowIndex, int colIndex, GridStyleInfo style);
        GridCellModelBase LookupCellModel(string id);
    }

    /// <summary>
    /// Defines the interface for a volatile data cache to be used with the <see cref="GridModel.VolatileData"/>
    /// property.
    /// </summary>
    ...
    /// </remarks>
    public class GridVolatileData : IGridVolatileData, IDisposable
    {
        private Hashtable data;
        private IGridVolatileDataContainer grid;
        private int rowCount;
        private int colCount;
        private bool hasRowCount;
        private bool hasColCount;
        private System.Threading.Thread guiThread;
        //private bool inResetItem;
        private ArrayList resetCellsList = new ArrayList();
        ...
        internal GridVolatileData(IGridVolatileDataContainer grid)
        {
            this.grid = grid;
            this.data = new Hashtable();
            guiThread = System.Threading.Thread.CurrentThread;
        ...
        public void Dispose()
        {
            ...
            //// Data contains WeakReferences as values. So, none
            //// of the values can be disposable.
        ...
        {
            return grid.LookupCellModel(id);
        }
        ...
        {
            // Returns NULL if cell is not found.
            object obj = data[cell];
            if (obj is WeakReference)
            {
                return ((WeakReference)obj).Target as GridStyleInfo;
            }
        ...
        public void SetItem(GridCellPos cell, GridStyleInfo style)
        {
            ...
            catch (InvalidOperationException ex)
            {
                ...
                Trace.WriteLine("Detected too many items in volatile date hashtable. Reducing GridVolatileData.forceGcCollect to a value below " + data.Count.ToString());
                TraceUtil.TraceCurrentMethodInfo("Hashtable count: ", data.Count);
                forceGcCollect = Math.Max(1000, data.Count - 1000);
                GC.Collect();
        ...
        {
#if DEBUG
            if (Switches.VolatileData.TraceVerbose)
            {
                TraceUtil.TraceCurrentMethodInfo(cell);
            }
#else
            ;
#endif
            ...
            {
                // In GC Thread - prevent ProcessResetCellsList 
        ...
        void ProcessResetCellsList()
        {
            ...
            {
                object cell = list[n];
                WeakReference wr = (WeakReference)data[cell]; // Returns null if cell is not found.
                // double check that cell has not been recreated in the meantime.
        ...
        public GridStyleInfo[] GetBaseStyles(GridStyleInfo thisStyleInfo, int rowIndex, int colIndex)
        {
            ...
            int headerRowCount = grid.HeaderRowCount;
            int headerColCount = grid.HeaderColCount;
        ...
        /// </summary>
        public GridStyleInfo /*IGridData*/ this[int rowIndex, int colIndex]
        {
            get
            {
                GridCellPos cell = new GridCellPos(rowIndex, colIndex);

                GridStyleInfo style;
                if (inGetBaseStyles)
                {
                    style = new GridStyleInfo(new GridStyleInfoIdentity(this, rowIndex, colIndex, true));
                    grid.GetCellInfo(rowIndex, colIndex, style);
                }
                else
                {
                    style = GetItem(cell);
                    ...
                    {
#if DEBUG
                        Trace.WriteLineIf(Switches.VolatileData.TraceVerbose, String.Format("VolatileData[{0},{1} allocates style object", rowIndex, colIndex));
#endif
                        //// Allocate style object and initialize its identity.
                        style = new GridStyleInfo(new GridStyleInfoIdentity(this, rowIndex, colIndex));
                        //// Avoid OnStyleChanged methods.
                        style.BeginInit();
                        //// Load data.
                        grid.GetCellInfo(rowIndex, colIndex, style);
                        style.EndInit();
            ...
            set
            {
                grid.ChangeCell(rowIndex, colIndex, value);
                if (value != null)
                {
        ...
        {
            GridVolatileData dataParent;

            internal RowStylesIndexer(GridVolatileData data)
            {
                dataParent = data;
            }
        ...
        public class ColStylesIndexer
        {
            ...
            internal ColStylesIndexer(GridVolatileData data)
            {
                dataParent = data;
            }
