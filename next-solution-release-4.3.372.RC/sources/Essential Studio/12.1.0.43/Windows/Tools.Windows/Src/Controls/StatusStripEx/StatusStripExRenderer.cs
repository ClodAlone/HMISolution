...
{
    public class StatusStripExRenderer : Office12ToolStripRenderer
    {
        #region Constants
        ...
        public StatusStripExRenderer(Office12ColorTable colorTable)
            : base(colorTable)
        {
        ...
        {
            Color clBegin = m_colorTable.ToolStripGradientBegin;
            Color clEnd = m_colorTable.ToolStripGradientEnd;
        ...
        {
            Color clBegin = m_colorTable.SeparatorStatusControlsAreaBegin;
            Color clEnd = m_colorTable.SeparatorStatusControlsAreaEnd;
            ...
            if (statusStrip != null)
            {
                if (WindowsAPI.GetWindowRect(statusStrip.Handle, ref rect))
                {
                    if (rect.Width > 0 && rect.Height > 0)
                    {
                        ...
                        {
                            // Draw separators
                            using (Pen penDark = new Pen(m_colorTable.SeparatorBegin))
                            {
                                using (Pen penLight = new Pen(m_colorTable.SeparatoraEnd))
                                {
                        ...
                        {
                            using (Pen penBegin = new Pen(m_colorTable.SeparatorGradientBegin))
                            {
                                using (Pen penMiddle = new Pen(m_colorTable.SeparatorGradientMiddle))
                                {
                                    using (Pen penEnd = new Pen(m_colorTable.SeparatorGradientEnd))
                                    {
                                        if (statusStrip.RightToLeft == RightToLeft.Yes)
                                        {
                                            g.DrawLine(penEnd, statusStrip.SeparatorPosition, 0, statusStrip.SeparatorPosition, rectDark.Height);
                                            g.DrawLine(penMiddle, statusStrip.SeparatorPosition + 1, 0, statusStrip.SeparatorPosition + 1, rectDark.Height);
                                            g.DrawLine(penBegin, statusStrip.SeparatorPosition + 2, 0, statusStrip.SeparatorPosition + 2, rectDark.Height);
                                        }
                                        else
                                        {
                                            g.DrawLine(penBegin, statusStrip.SeparatorPosition, 0, statusStrip.SeparatorPosition, rectDark.Height);
                                            g.DrawLine(penMiddle, statusStrip.SeparatorPosition + 1, 0, statusStrip.SeparatorPosition + 1, rectDark.Height);
                                            g.DrawLine(penEnd, statusStrip.SeparatorPosition + 2, 0, statusStrip.SeparatorPosition + 2, rectDark.Height);
                                        }
                                    }
                                }
                            }
                        ...
                        {
                            using (Pen penBorder = new Pen(m_colorTable.ToolStripBorder))
                            {
        ...
        internal override Rectangle GetCheckRect(ToolStripItemImageRenderEventArgs e)
        {
            Rectangle rc = base.GetCheckRect(e);

            if (e.ToolStrip is StatusContextMenuStrip && e.ToolStrip.RightToLeft == RightToLeft.Yes)
            {
                rc.X = e.Item.Bounds.Right - rc.Width - 1;
            }
