...
namespace Syncfusion.Pdf
{
    internal sealed class PageResourceLoader
    {
                ...
                if (fonts != null)
                {
                    ...
                    if (fontsDictionary != null)
                    {
                        foreach (KeyValuePair<PdfName, IPdfPrimitive> item in fontsDictionary.Items)
                        {
                            ...
                            {
                                pageResources.Add(
                                    item.Key.Value, new FontStructure(item.Key.Value,(item.Value as PdfReferenceHolder).Object));
                            }
                            else
                                pageResources.Add(item.Key.Value, new FontStructure(item.Key.Value,item.Value));
                        }
                    }
                }
            }
        ...
        public string DecodeTest(PdfPageBase page, string fontName, string textToDecode)
        {
            ...
            byte[] bytesToDecode = System.Text.Encoding.Unicode.GetBytes(textToDecode);
