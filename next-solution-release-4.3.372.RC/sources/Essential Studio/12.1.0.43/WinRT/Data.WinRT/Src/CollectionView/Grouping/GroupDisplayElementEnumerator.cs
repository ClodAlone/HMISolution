...
namespace Syncfusion.Data
{
    ...
    internal class TraversalHelper
    {
        ...
        public NodeEntry GetNext(NodeEntry entry)
        {
            ...
            {
                var record = entry.Parent as RecordEntry;
                var group1 = entry.Parent.Parent as Group;
                var rcount = group1.GetRelationsCount();
                if (((NestedRecordEntry)entry).NestedLevel == rcount)
                {
                    var index = group1.Records.IndexOf((RecordEntry)entry.Parent);
                    var count = group1.Records.Count;
                    if (index == count - 1)
                    {
        ...
        public NodeEntry GetPrevious(NodeEntry entry)
        {
            ...
            else if (entry is RecordEntry)
            {
                ...
                else
                {
                    ...
                    if (previousrecord.IsExpanded)
                        nodeEntry = previousrecord.ChildViews.LastOrDefault().Value;
                    else
            ...
            {
                var recordparent = entry.Parent as Group;
                var groupentry = recordparent.Details as GroupRecordEntry;
                var index = groupentry.Summaries.IndexOf((SummaryRecordEntry) entry);
                if (index == 0)
                {
                    RecordEntry lastRecord = recordparent.Records[recordparent.Records.Count - 1];
                    if (lastRecord.IsExpanded)
                        nodeEntry = lastRecord.ChildViews.LastOrDefault().Value;
                    else
        ...
        {
            NodeEntry value = null;
            if (group is VirtualGroup)
            {
                var counter0=group.GetRelationsCount() + 1;
                bool isRecordIndex = (index % counter0) == 0;
                if (isRecordIndex)
                {
                    value = group.GetRecordAt(index);
                }
                else
                {
                    var relationIndex = index % counter0;
                    var baseRecordIndex = index - relationIndex;
                    var record = group.GetRecordAt(baseRecordIndex);
                    relationIndex--;
                    if (record.IsExpanded)
                    {
                        foreach (var kvp in record.ChildViews)
