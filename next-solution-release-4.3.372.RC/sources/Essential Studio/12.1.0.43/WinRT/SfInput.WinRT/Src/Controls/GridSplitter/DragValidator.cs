...
namespace Syncfusion.UI.Xaml.Controls.Input
{
    ...
    internal class DragValidator
    {
        ...
        public DragValidator(UIElement targetElement)
        {
            ...
            _targetElement.PointerReleased += _targetElement_PointerReleased;
            _targetElement.PointerMoved += _targetElement_PointerMoved;
        }

        /// <summary>
        /// Handle the Pointer Released event for the UIElement to update the drag
        /// operation.
        /// </summary>
        /// <param name="sender">Inherited code: Requires comment.</param>
        /// <param name="e">Inherited code: Requires comment 1.</param>
        void _targetElement_PointerReleased(object sender, PointerRoutedEventArgs e)
        {
            _targetElement.ReleasePointerCapture(e.Pointer);
            _draggingActive = false;
            OnDragCompleted(e, false);
        }

        /// <summary>
        /// Handle the Pointer Pressed event for the UIElement to update the drag
        /// operation.
        /// </summary>
        /// <param name="sender">Inherited code: Requires comment.</param>
        /// <param name="e">Inherited code: Requires comment 1.</param>
        void _targetElement_PointerPressed(object sender, PointerRoutedEventArgs e)
        {
            PointerPoint point = e.GetCurrentPoint(_targetElement);
            _start = new Point(point.Position.X, point.Position.Y);
            if (_targetElement.RenderTransform != null)
            {
                _start = _targetElement.RenderTransform.TransformPoint(_start);
            }

            _draggingActive = _targetElement.CapturePointer(e.Pointer);
        ...
        /// <summary>
        /// Handle the Pointer Move event for the UIElement to update the drag
        /// operation.
        /// </summary>
        /// <param name="sender">Inherited code: Requires comment.</param>
        /// <param name="e">Inherited code: Requires comment 1.</param>
        void _targetElement_PointerMoved(object sender, PointerRoutedEventArgs e)
        {
            if (_draggingActive)
            {
                OnDragDelta(e);
        ...
        private void OnDragDelta(PointerRoutedEventArgs e)
        {
            EventHandler<DragDeltaEventArgs> handler = this.DragDeltaEvent;
            if (handler != null)
            {
                PointerPoint point = e.GetCurrentPoint(_targetElement);
                Point p = new Point(point.Position.X, point.Position.Y);
                if (_targetElement.RenderTransform != null)
                {
                    p = _targetElement.RenderTransform.TransformPoint(p);
                }
        ...
        private void OnDragCompleted(PointerRoutedEventArgs e, bool canceled)
        {
