...
namespace HistoricalEventControl
{
        ...
        , IGridLayoutUser
    {
        ...
        public static readonly DependencyProperty ControlForegroundProperty = DependencyProperty.Register("ControlForeground", typeof(Brush), typeof(HistoricalEvents), new UIPropertyMetadata(Brushes.LightGray));
        ...
        private readonly EventMessageConverter eventMessageConverter = new EventMessageConverter();
        private readonly EventStateConverter eventStateConverter = new EventStateConverter();
        ...
        public static readonly DependencyProperty ColumnTextWrapProperty = DependencyProperty.Register("ColumnTextWrap", typeof(TextWrapping), typeof(HistoricalEvents), new UIPropertyMetadata(TextWrapping.NoWrap));
           ...
           // tableView.UseLightweightTemplates = UseLightweightTemplates.None;
            ...
            Loaded += (o, e) =>
            {
                if (!bLoaded && !bDispose)
                {
                    ...
                    {
                        gridControl.AllowCollectionView = false;
        ...
        private void StringManager_CultureChanged(object sender, EventArgs e)
        {
            Dispatcher.BeginInvokeInBackgroundIfRequired(() =>
            {
                ...
                cancelSmall.ToolTip = TranslationHelper.TranlslateText($"_{stringPlaceolder}_DeleteSettings", stringlist, Properties.Resources.DeleteSettings); startTime.Content = TranslationHelper.TranlslateText($"_{stringPlaceolder}_DateTimeStart", StringList, Properties.Resources.HistoricalEvents_DateTimeStart);
...
//                busyContent.Text = Properties.Resources.WaitText;
        ...
        {
            if (e.Column.FieldName == "EventMessage")
            {
                e.Column.ColumnFilterMode = ColumnFilterMode.DisplayText;
                var eventDetails = string.Empty;
                var currentRow = e.Row as UFUAAuditLogItem;

                if (currentRow != null)
                {
                    eventDetails = currentRow.EventDetails;
                }
                
                e.DisplayText = 
                    (string)eventMessageConverter.Convert(new object[] { e.DisplayText, StringList, eventDetails }, null, null, CurrentCulture);
            }

            if (e.Column.FieldName == "EventState")
            {
                e.Column.ColumnFilterMode = ColumnFilterMode.DisplayText;
                e.DisplayText = (string)eventStateConverter.Convert(new object[] { e.DisplayText, StringList }, null, null, CurrentCulture);
            }
        }
        #endregion
        ...
        private void configMemory_SelectionChanged(object sender, RoutedEventArgs e)
        {
            ...
            {
                if (oldusername == helper?.Username && !bDesignmode)
                    SaveGridConfiguration();
        ...
        {
            (sender as TableView).HorizontalScrollbarVisibility = ScrollBarVisibility.Auto;
        }
