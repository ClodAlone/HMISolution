onPrevLoadHandler = window.onload;
window.onload = init;
...
var continuousPollingTimeout;
var continuousPollingInterval = 2000;
var repostPollingInterval = 1000;
var reconnectingTimeInterval = 5000;
var redrawFrameInteraval = 100;
var bRefreshBrowserAfterReconnection = false;
...
$.connection.hub.stateChanged(function (state) {
    if (state.newState == 0) {
        ...
        clearTimeout(timerReconnecting);
        timerReconnecting = setTimeout(function () {
            reconnect();
        }, reconnectingTimeInterval);
    }
});
...
function disconnect() {
    ...
    if (typeof timerRedrawAll !== 'undefined') {
        clearInterval(timerRedrawAll);
        timerRedrawAll = undefined;
    }

    clearTimeout(timerCommands);
    clearTimeout(continuousPollingTimeout);
    clearTimeout(timerResize);
        
    $.connection.hub.stop();
}

var timerReconnecting;
function reconnect() {
    clearTimeout(timerReconnecting);
    ...
    if (sessionid == null || sessionid == "")
        ...
        $.connection.hub.qs = { "url": url, "sessionid": sessionid, "id": parentid };

    var deferredStart = $.connection.hub.start();
    deferredStart.done(function () {
        if (bRefreshBrowserAfterReconnection) {
            // Simulate a refresh command on browser for cleaning memory.
            var href = window.location.href;
            var index = href.toLowerCase().indexOf("?url=");
            if (index > -1) {
                href = href.substr(0, index) + "?url=" + url;
            }
            ...
            var bodyWidth = window.innerWidth - canvasContainer.offsetLeft;
            var bodyHeight = window.innerHeight - canvasContainer.offsetTop;

            screenHub.server.register(bodyWidth, bodyHeight, bIsMobile, new Date().getTimezoneOffset()).done(onSuccessRegistration).fail(onFailureRegistration);
        }
    });
...
function init() {
    ...
    if (/MSIE 7|MSIE 8/i.test(navigator.userAgent)) {
        leftMouseButton = 1;
    }

    LoadingPanel.Show();

    if (onPrevLoadHandler) {
        onPrevLoadHandler();
    }
...
function resizeend() {
    if (new Date() - rtime < delta) {
        clearTimeout(timerResize);
        timerResize = setTimeout(resizeend, delta);
    } else {
        timeout = false;
        // alert('Done resizing');
        if (resizable) {
            resizeCanvas();
            screenHub.server.onResize(canvasMain.width, canvasMain.height);
...
function resizeCanvas() {
    var bodyWidth = window.innerWidth - canvasContainer.offsetLeft;
    var bodyHeight = window.innerHeight - canvasContainer.offsetTop;
    //var bodyWidth = canvasContainer.clientWidth;
    //var bodyHeight = canvasContainer.clientHeight;
    if (resizable) {
...
function redrawAll() {
    ...
    else if (typeof timerRedrawAll === 'undefined') {
        timerRedrawAll = setInterval("redrawAll()", redrawFrameInteraval);
    }
    ...
    for (var i in hashElements.items) {
        if (!hashElements.items[i].hasImage || hashElements.items[i].imageLoaded && hashElements.items[i].image != null) {
...
function unload() {
    if (initialized) {
        initialized = false;
        screenHub.server.unregister(clientId); //.done(onSuccessUnregister).fail(onFailedUnregister);
...
function onSuccessRegistration(data) {
    ...
    if (continuousPolling) {
        continuousPollingTimeout = setTimeout(function () {
            SetPendingPollingCommands(continuousPollingInterval);
        }, continuousPollingInterval);
    }
...
var sessionTimeout;
var timerSessionTimeout;
function RenewSessionTimeout() {
    clearTimeout(timerSessionTimeout);
    if (sessionTimeout > 0) {
        timerSessionTimeout = setTimeout(function () {
            clearTimeout(timerSessionTimeout);
            console.log("Session expired for inactivity timeout (" + sessionTimeout / 1000 + " seconds).");
            tryingToReconnect = false;
            disconnect();
            window.location.href = "Account/Login.aspx?Action=Logout";
        }, sessionTimeout);
    }
}
...
function onSuccessGetElementData(dataList) {
    ...
    try {
        ...
        $.each(dataList, function () {
            ...
            elementData.hasImage = this.hasImage;

            hashElements.setItem(this.id, elementData);

            if (elementData.hasImage)
            {
                try {
...
var timerCommands;
function SetPendingPollingCommands(delay) {
    if (!initialized)
        return;

    //clearTimeout(timerCommands);
    clearTimeout(continuousPollingTimeout);
    timerCommands = setTimeout("refreshCommand()", delay);
}

var isCheckingCommand = false;
...
function oncanvasMainMouseDown(e) {
    if (!initialized || e.button != leftMouseButton)
        return;
...
function onSuccessMouseUp(elementInfo) {
    ...
    if (elementInfo !== null && typeof (elementInfo) !== 'undefined') {
        ...
        if (sendingCommandElement !== null && typeof (sendingCommandElement) !== 'undefined') {
            //if (sendingCommandElement.dataType !== -1)
            //    editingValueProvider = false;
...
function OpenScreenUrl(url, isFromTile) {
    if (url == null || url == "")
        return;
    ...
    var bodyWidth = window.innerWidth - canvasContainer.offsetLeft;
    var bodyHeight = window.innerHeight - canvasContainer.offsetTop;
    screenHub.server.openUri(url, bodyWidth, bodyHeight, new Date().getTimezoneOffset(), isFromTile).done(onSuccessOpenUri).fail(onFailureOpenUri);
}
...
function onSuccessPollingPendingCommands(elementChanged) {
    ...
    if (elementChanged != null) {
        ...
        $.each(elementChanged, function () {
            if (this.isError) {
            ...
            else if (this.closeCurrent) {
                closeReturnCurrentPopup();
            }
            else if (this.isSynchro && !bIsMobile) {
                currentPopupScreen = this.url;
                var commandurl = "PopupScreen.aspx?url=" + this.url + "&id=" + clientId + "&sessionid=true";
                ...
                if (typeof(url) == "undefined")
                    url = getParameterByName("url");
                if (url.replace(/\\/g, "/") != this.url.replace(/\\/g, "/")) {
                    isOpeningUri = true;
                    OpenScreenUrl(this.url, false);
                }
            }
        });

        if (repostPollingCommand || continuousPolling) {
            var delay = repostPollingCommand ? repostPollingInterval : continuousPollingInterval;
            if (!repostPollingCommand)
                isCheckingCommand = false;
            repostPollingCommand = false;
            if (!isOpeningUri) {
                SetPendingPollingCommands(delay);
            }
            else {
                clearTimeout(timerCommands);
            }
...
screenHub.client.showDemoMode = function () {
    ShowDemoMode();
}

function ShowDemoMode() {
    try {
        if (!demoModeAlert.IsVisible())
            demoModeAlert.Show();
    }
    catch (e) {
        var parentWindow = window.parent;
        if (parentWindow !== null)
            parentWindow.ShowDemoMode();
    }
}
...
function onSuccessOpenUri(size) {
    ...
    if (size === null || typeof (size) === 'undefined') {
        LoadingPanel.Hide();
        alert("An error occured opening the screen or screen not found !");
        OpenScreenUrl(previousUrl, false);
...
function onFailureOpenUri() {
    LoadingPanel.Hide();
    writetostatus("Error Opening Uri: no response from service!!");
    if (continuousPolling) {
        continuousPollingTimeout = setTimeout(function () {
            SetPendingPollingCommands(continuousPollingInterval);
        }, continuousPollingInterval);
...
function CloseCurrentPopupScreen() {
    if (bSharingSession) {
        ...
        if (initialized) {
            initialized = false;
            screenHub.server.closePopupUri();
