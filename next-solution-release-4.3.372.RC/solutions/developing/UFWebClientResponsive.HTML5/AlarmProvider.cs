...
public class AlarmProvider
    {
        ...
        public static bool IsSoundBuzzing()
        {
            if (Global.DisableAlarmBuzzing)
                return false;
        ...
        public static AlarmSink.AlarmSink CreateAlarmSink()
        {
            ...
            var soundReference = Global.UFUAEditorComponent.GetNodeIdEntityReference(Global.projectDocument,
            ...
            var ret = new AlarmSink.AlarmSink(server, String.Format("{0}-{1}", Guid.NewGuid(), Global.ClientSessionName), soundReference);
            ret.Recycling += (o, e) =>
                {
                    lock (recycledSession)
                    {
