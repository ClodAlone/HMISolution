...
using static WebNExTHMI.PlatformComponents.RecipeData;
...
namespace WebNExTHMI.Session
{
    public class Session : ViewModelBase, IEntityReference
    {
        ...
        readonly static SessionCounter SessionCounter;
        ...
        static List<string> addingSessions = new List<string>();
        static Object newSessionLock = new Object();
        ...
        {
            List<Session> disposable = new List<Session>();
            lock (mapSessions)
            {
                mapSessions.Values.ToList().ForEach((session) => {
                    disposable.Add(session);
                });
                mapSessions.Clear();
                addingSessions.Clear();
            }
            disposable.ForEach(session =>
            {
                if (session != null)
                {
        ...
        public static int AddSession(String clientId, IClientProxy c)
        {
            ...
            lock (mapSessions)
            {
                ...
                addingSessions.Add(clientId);
            }

            int sessionCount;
            bool bSessionAdded = false;
            lock(newSessionLock)
                session = new Session(c, clientId);

            lock (mapSessions)
            {
                if (addingSessions.Contains(clientId))
                {
                    addingSessions.Remove(clientId);
                    mapSessions.Add(clientId, session);
                    bSessionAdded = true;
                }
                sessionCount = mapSessions.Count;
            }
            if (bSessionAdded)
        ...
        internal int CurrentUserAccessLevel
        {
            get 
            {
            ...
            set 
            {
                if (_currentUserAccessLevel != value)
                {
                    _currentUserAccessLevel = value;
                    ConditionStateFilterInitialized = false;
                }
            }
        }

        int _currentUserReadAccessMask;
        internal int CurrentUserReadAccessMask
        {
            get
            {
                return _currentUserReadAccessMask;
            }
            set
            {
                if (_currentUserReadAccessMask != value)
                {
                    _currentUserReadAccessMask = value;
                    ConditionStateFilterInitialized = false;
                }
            }
        }
        ...
        internal bool ConditionStateFilterInitialized 
        { 
            get; set; 
        }
        ...
        public bool KeepAlwaysTagsAlive(String screenid)
        {
            lock (mapScreenData)
            {
                if (mapScreenData.ContainsKey(screenid))
                    return mapScreenData[screenid].KeepAlwaysTagsAlive();
            }
        ...
        public void SetValue(String screenId, int id, String value, bool bForceSync)
        {
            ...
            screenData.SetValue(id, value, bForceSync);
        }
        ...
        public List<ConditionData> GetConditionStateList(String screenId, int id, CancellationToken ct, bool bSortByTimeDescending = false, bool bNeedsRefresh = false, Dictionary<object, object> filters = null, bool bConnectChildAlarms = false)
        {
            if (ct.IsCancellationRequested)
            ...
            if (screenData == null)
                throw new ArgumentException(Properties.Resources.CannotFindScreenToSetValue);
            return screenData.GetConditionStateList(id, ct, bSortByTimeDescending, bNeedsRefresh, filters, bConnectChildAlarms);
        }

        public void AckReset(String screenId, int id, string[] list, bool isReset, bool bConnectChildAlarms = false)
        {
            ScreenData screenData = null;
            lock (mapScreenData)
            {
            ...
            if (screenData == null)
                throw new ArgumentException(Properties.Resources.CannotFindScreenToSetValue);
            screenData.AckReset(id, list, isReset, bConnectChildAlarms);
        }

        public string InitializeRecipeDocument(string recipePath, int readWriteTimeOut, bool bInit)
        {
            var recipeNames = InitializeRecipeDataSet(recipePath, readWriteTimeOut, bInit);
            return JsonConvert.SerializeObject(recipeNames);
        }

        public Dictionary<string, List<JToken>> InitializeRecipeDataSet(string recipePath, int readWriteTimeOut, bool bInit, bool bExecutingCommand = false)
        {
            ...
            if (state == false)
            {
                throw new Exception(Properties.Resources.NoRecipeLicense);
            }
#endif
            lock (recipeLock)
            {
                ...
                if (!bRefill)
                {
                    var rd = new RecipeData(caller, recipePath, readWriteTimeOut, bExecutingCommand);
                    mapRecipeData[recipePath] = rd;
                }
                recipeData = mapRecipeData[recipePath];
                if (!bInit)
                {
                    recipeData.ForceInvalidateCanExecute();
            ...
            ret.Add("mapWrongValues", new List<JToken>() { JsonConvert.SerializeObject(recipeData.mapWrongValues) });
            return ret;
        }

        public string LoadDataGridData(string thisGridID, string connectionString, int commandTimeout, string sessionString, string dataProvider, string connection, string select, string where, string groupBy, string sort, string tableName, uint maxTransactionsBeforeCommit, bool allowPrimaryKeyChanging, CancellationToken ct)
        {
            DataGridData gridData;
            lock (mapDataGridData)
            {
                if (!mapDataGridData.ContainsKey(thisGridID))
                    mapDataGridData[thisGridID] = new DataGridData();
                gridData = mapDataGridData[thisGridID];
            }
            return gridData.LoadDataGridData(connectionString, sessionString, commandTimeout, dataProvider, connection, select, where, groupBy, sort, tableName, maxTransactionsBeforeCommit, allowPrimaryKeyChanging, ct);
        }
        ...
        public AggregatedValues LoadHistoricalData(string screenId, string connString, int commandTimeout, long? startTimestamp, long? endTimestamp, bool usesourcetimestamp, int maxRecord, bool aggregate, int maxaggregationfactor, int maxDeadLockRetry, bool localize, string viewTimeFrame, bool minAggregation, bool maxAggregation, bool avgAggregation, string tagName, string columnTagName, Dictionary<string, Tuple<string, string>> parametersMap, string serieTitle, int SVGReferenceId, string nodeID, string historicalName, bool bDlrSource, string conditionalString, bool bNeedsMedian, bool bNeedsVariance, bool bNeedsStdDev, CancellationToken ct)
        {
            ...
            return PlatformComponents.PlatformComponents.LoadHistoricalData(connString, commandTimeout, startTimestamp, endTimestamp, usesourcetimestamp, maxRecord, aggregate, maxaggregationfactor, maxDeadLockRetry, localize, System.Xml.XmlConvert.ToTimeSpan(viewTimeFrame), minAggregation, maxAggregation, avgAggregation, tagName, columnTagName, parametersMap, serieTitle, nodeID, GetReferenceAppName(screenId, SVGReferenceId, nodeID), historicalName, bDlrSource, GetSessionSettings(screenId), conditionalString, bNeedsMedian, bNeedsVariance, bNeedsStdDev, ct);
        }

        string GetReferenceAppName(string screenId, int SVGReferenceId, string nodeID)
        {
            ScreenData screenData = null;
            lock (mapScreenData)
            {
            ...
            OPCUAEntityReference reference = screenData.GetReference(SVGReferenceId);

            if (String.IsNullOrEmpty(reference?.AppName))
                throw new ArgumentException(String.Format("{0}{1}", Properties.Resources.CannotFindPenVariable, nodeID));

            return reference.AppName;
        }
        
        public StatesChartData LoadStatesChartHistoricalData(string screenId, string connString, int commandTimeout, int maxRecord, TimeSpan viewTimeFrame, TimeSpan recordEvery, string nodeID, string historicalname, bool bDlrSource, string sessionString, string coluName, int SVGReferenceId, string controlID, int clientTimezoneOffset, long? startTimestamp, long? endTimestamp, DateSpan rangeType, CancellationToken ct)
        {
            ...
            if (!bContainsNodeId) {
                var scData = PlatformComponents.PlatformComponents.InitStatesChartHistoricalData(connString, historicalname, bDlrSource, sessionString, nodeID, GetReferenceAppName(screenId, SVGReferenceId, nodeID));
        ...
        public void RecipeCommandExecute(String recipeStringPath, int commandType, int syncTimeout, bool bSynchronous)
        {
            lock (recipeLock)
            {
                if (!mapRecipeData.ContainsKey(recipeStringPath))
                    InitializeRecipeDataSet(recipeStringPath, 0, false, true);
            }
        ...
        public void SubscribeCanAckResetAll(string screenId)
        {
            var alarmVariable = alarmServerVariable;
            ScreenData screenData = null;
            lock (mapScreenData)
            {
                if (mapScreenData.ContainsKey(screenId))
            ...
            screenData.SubscribeCanAckResetAll();
        }

        public void AckResetAll(bool isReset, bool isCommand, string screenId)
        {
            var alarmVariable = alarmServerVariable;
            if (isCommand)
            {
                ...
                screenData.CommandAckResetAll(isReset);
                return;
            }
            AckReset(alarmVariable, isReset);
        }

        internal void AckReset(OPCUAEntityReference alarmVariable, bool isReset)
        {
            if (alarmVariable?.MonitoredItemViewModel != null)
            {
                if (isReset)
                    alarmVariable.MonitoredItemViewModel.ConditionConfirmAllCommand.Execute(null);
                else
                    alarmVariable.MonitoredItemViewModel.ConditionAcknowledgeAllCommand.Execute(null);
            }
        }

        public string GetAlarmCommandsSessionName()
        {
            return String.Format("{0}-{1}", PlatformComponents.PlatformComponents.GetSessionString(), clientId);
        }
        ...
        public void SetInIdle(String screenid, bool newValue)
        {
            lock (mapScreenData)
            {
                if (mapScreenData.ContainsKey(screenid))
                    mapScreenData[screenid].SetInIdle(newValue);
            }
        }
        ...
        public void UnsubscribeChildrenAlarmServer(string screenId)
        {
            ScreenData screenData = null;
            lock (mapScreenData)
            {
            ...
            if (screenData != null)
                screenData.UnsubscribeChildrenAlarmServer();
        }

        public void SubscribeChildrenAlarmServer(string screenId)
        {
            ScreenData screenData = null;
            lock (mapScreenData)
            {
            ...
            if (screenData != null)
                screenData.SubscribeChildrenAlarmServer();
        }
