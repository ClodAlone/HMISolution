...
WebHMIApp.controller(MAINCONTROLLER, ['$scope', '$compile', 
    function ($scope, $compile) {
        ...
        $scope.activatePopup = function (viewtype, option, actions = null) {
            attachPopup($scope, viewtype, option, actions);
        };
...
function StartApp($scope, ret) {
    ...
    registerExternalAuthSubscriptionsIfRequired();
    ...
    function registerExternalAuthSubscriptionsIfRequired() {

        if ($scope.configurationSettings.isExternalAuthActive) {
            $scope.corehubconnection.on("externalIdpUserLoggedIn", (code, responseState) => {
                $scope.corehubconnection.invoke("OnExternalIdpUserLoggedIn", code, responseState);
            });

            $scope.corehubconnection.on("externalLoginSuccessful", (externalLoginData => {
                $scope.currentUser = {
                    name: externalLoginData.username,
                    level: externalLoginData.level,
                    mask: externalLoginData.mask,
                    role: externalLoginData.role,
                    signature: externalLoginData.signature
                };
            }));

            $scope.corehubconnection.on("externalLoginFailed", (errorMessage) => {
                window.alert(errorMessage);
            });

            $scope.corehubconnection.on("navigateToExternalLogout", (logoutUrl) => {
...
function attachView($scope, parent, viewType, option, activatingLast, isdocked) {
    if (!registerScreenDataUpdate)
        registerScreenDataUpdate = $scope.$on(onUpdateScreenSettings, function (e, screenData) {
            ...
            if (screenData.keepAlwaysInMemory && !isNotDestroyable(screenData.name, screenData.parameter))
                dotNotDestroyView.push({ name: screenData.name, parameter: screenData.parameter, keepTagsAlive: screenData.keepAlwaysTagsAlive });
            else
    ...
    if (!isdocked) {
        ...
        if (foundHidden) {
            ...
            var arrayparameters = JSON.parse(option);
            if (!isKeepTagsAlive(arrayparameters.name, arrayparameters.parameter)) {
                var screenLoadedId = view.find("svg").attr("id");
                $scope.corehubconnection.invoke("SetScreenInIdle", screenLoadedId, false);
            }
            ...
            if (activeView) {
                ...
                var arrayparameters = JSON.parse(lastOption);
                if (!isKeepTagsAlive(arrayparameters.name, arrayparameters.parameter)) {
                    var screenLoadedId = activeView.find("svg").attr("id");
                    $scope.corehubconnection.invoke("SetScreenInIdle", screenLoadedId, true);
                }
...
function isKeepTagsAlive(name, parameter) {

    for (var i = 0; i < dotNotDestroyView.length; ++i) {

        if (equalsEmpty(dotNotDestroyView[i].name, name) &&
            equalsEmpty(dotNotDestroyView[i].parameter, parameter))
            return dotNotDestroyView[i].keepTagsAlive;
    }
...
function attachPopupWithParameters($scope, viewType, page, parameters, popupKey, bOpenAtCoords, actions = null) {
    showPopup({
        $scope: $scope,
        id: viewType,
        content: page,
        ...
        onContentReadyFunc: function (popupContainer, jparent) {
            ...
            if (actions && actions.onContentReady)
                actions.onContentReady(popupContainer);
        },
...
function attachPopup($scope, viewType, option, actions = null) {
    ...
    try {
        ...
        attachPopupWithParameters($scope, viewType, page, parameters, popupKey, bOpenAtCoords, actions);
    }
    catch (e) {
...
var findThresholdDataBySeverity = function (severity, thresholds, severityColorMap) {
    if (severity in severityColorMap)
        return severityColorMap[severity];

    var finalThreshold = thresholds[0];
    $(thresholds).each(function (i, e) {
        if (parseInt(e.ThresholdValue, 10) > parseInt(severity, 10))
            return false;
        finalThreshold = e;
    });
    var colorsObj = {
        ...
        ThresholdIconPath: finalThreshold.ThresholdIconPath
    };
    severityColorMap[severity] = colorsObj;
    return colorsObj;
};

var updateThresholdElementColor = function (data, $element, thresholds, severityColorMap, $styledElements) {
    if (!$element)
        return;
    if (!$styledElements)
    ...
    var backcolor = "";
    var forecolor = "";
    var thresoldColors = findThresholdDataBySeverity(data.severity, thresholds, severityColorMap);
    ...
    fastdom.measure(function () {
        ...
        fastdom.mutate(function () {
            ...
            $styledElements.css('color', forecolor);
        });
    });
};
...
//pendingAnimationFrameRef: object reference to the latest animationframe: '{ id : <frameId> }' that can be canceled by the caller func
function invokeDelayedAction(action, pendingAnimationFrameRef, periodMS = 0) {
    var startMS;
    function checkExecuteAction(nowMS) {
        ...
        if (timeSinceStart < periodMS) {
            pendingAnimationFrameRef.id = requestAnimationFrame(checkExecuteAction);
        }
        else {
            action();
            pendingAnimationFrameRef.id = null;
            bInvoking = false;
            startMS = undefined;
        }
    }
...
function getEnumReferenceItemsList(reference) {
    var titems = [];
    try {
        var enumStrings = reference.enumStrings;
        var digitalTrueValue = reference.digitalTrueValue;
        var digitalFalseValue = reference.digitalFalseValue;
        var isDigital = digitalTrueValue && digitalFalseValue;
        if (enumStrings) {
            jQuery.each(reference.enumStrings, function (index, item) {
                titems.push(i18next.t1(item));
            });
        }
        else if (isDigital) {
            titems.push(i18next.t1(digitalFalseValue));
            titems.push(i18next.t1(digitalTrueValue));
        }
    }
    catch { }
    return titems;
...
function setObjectBorder(cssStyleObject, parameters, thisPage, mainContainer, SVGStrokeScreenId = false) {
    ...
    if (parameters.BorderThickness && (parameters.BorderBrush.Color || parameters.BorderBrush.Source)) {
        if (parameters.BorderThickness.Top > 0 || parameters.BorderThickness.Right > 0 || parameters.BorderThickness.Bottom > 0 || parameters.BorderThickness.Left > 0) {
            if (SVGStrokeScreenId)
                cssStyleObject["stroke-width"] = Math.min(parameters.BorderThickness.Top, parameters.BorderThickness.Bottom, parameters.BorderThickness.Left, parameters.BorderThickness.Right) + "px";
            else
            ...
            if (!SVGStrokeScreenId) {
                ...
                cssStyleObject["stroke"] = urlReferenceReplace(parameters.BorderBrush.Color, SVGStrokeScreenId);
        }
        else if (!SVGStrokeScreenId)
...
function parseGroupTransform($sourceNode, scope, webKitRenderCorrection) {
    var ret = { x: 0, y: 0 };
    var tempTransform = { x: 0, y: 0 };
    if (scope.biOS) {
        $.each($sourceNode.parents("g[transform], g[oldTransformX]"), function (i, gr) {
            if (webKitRenderCorrection) {
                var bHasTransform = gr.transform.baseVal.length > 0;
                if (bHasTransform) {
                    if (gr.transform.baseVal[0].type == 2) { // translation
                        tempTransform.x += gr.transform.baseVal[0].matrix.e;
                        tempTransform.y += gr.transform.baseVal[0].matrix.f;
                        $(gr).attr({
                            "oldTransformX": gr.transform.baseVal[0].matrix.e,
                            "oldTransformY": gr.transform.baseVal[0].matrix.f
                        }).removeAttr("transform");
                    }
                }
                else {
                    tempTransform.x += parseFloat(gr.getAttribute("oldTransformX"));
                    tempTransform.y += parseFloat(gr.getAttribute("oldTransformY"));
                }
            }
            else if (gr.transform.baseVal[0].type == 2) {
                ret.x += gr.transform.baseVal[0].matrix.e;
                ret.y += gr.transform.baseVal[0].matrix.f;
            }
        });
        if (webKitRenderCorrection && $sourceNode[0].transform.baseVal.length > 0) {
            if ($sourceNode[0].transform.baseVal[0].type == 4 && $sourceNode[0].transform.baseVal[0].matrix.e != 0 && $sourceNode[0].transform.baseVal[0].matrix.f != 0) { // fixing control rotation center making it relative to parent group's translation
                var splittedTransform = $sourceNode.attr("transform").split(/\s/);
                var angle = parseFloat(splittedTransform[0].replace(/[^0-9]/g, ''));
                var newCenterX = parseFloat(splittedTransform[1]) + tempTransform.x;
                var newCenterY = parseFloat(splittedTransform[2]) + tempTransform.y;
                $sourceNode.attr("transform", "rotate(" + angle + " " + newCenterX + " " + newCenterY + ")");
            }
        }
    }
    return ret;
...
function safelyRemoveFobj($sourceNode, $targetNode, scope, webKitRenderCorrection) {
    ...
    var ret = { x: 0, y: 0 };
    if (scope.biOS)
        ret = parseGroupTransform($sourceNode, scope, webKitRenderCorrection);
...
function setTextWrappingOptions(btnContentCss, btnTextCss, textWrapping, bHasNewLines, biOS) {
    switch (textWrapping) {
        case 0:
            btnContentCss["white-space"] = "break-spaces";
            btnTextCss["display"] = "inline-block";
            break;
        case 1:
...
function configureTextClipPath(clipPath, clipPathID, positionAndSize, textNode, fobjReplacerContainer, bUpdateWidth) {
    if (!clipPath) {
        clipPath = $(SVG("<clipPath id='" + clipPathID + "'><rect x='" + positionAndSize.x + "' y='" + positionAndSize.y + "' width='" + positionAndSize.width + "' height='" + positionAndSize.height + "' /></clipPath>").node);
        clipPath.appendTo(fobjReplacerContainer);
        textNode.attr("clip-path", "url(#" + clipPathID + ")");
    }
    else {
        let cpAttrs = { "height": positionAndSize.height };
        if (bUpdateWidth)
            cpAttrs.width = positionAndSize.width;
        clipPath.find("rect").attr(cpAttrs);
    }
    return clipPath;
}
...
function updateControlSecurity(oldContainer, newContainer) {
    if ($(oldContainer).hasClass(animationHiddenClass))
        $(newContainer).addClass(animationHiddenClass);
...
function checkControlSetVisible(controlToShow) {
    ...
    for (var screenId in window.currentEntitiesContext) {
        ...
        var entity = screenContext.entities.find(function (item) {
            ...
            if (control.length <= 0)
                return false;
            var node = controlToShow.node ? controlToShow.node : controlToShow[0];
            var $controlNode = $(control[0].node);
            return node == control[0].node || ($controlNode.hasClass("skip_me") && node == $controlNode.children()[0]);
        });
        if (entity) {
...
function isCssGradient(color) {
    if (!color)
        return false;
    var lowered = color.toLowerCase();
    return lowered.indexOf("linear-gradient") !== -1 || lowered.indexOf("radial-gradient") !== -1;
}

var topBarButtonsHelper = function (bTopBarVisible, buttonsContainerDiv, parameters, toolbarForegroundColor) {
    if (!toolbarForegroundColor)
        toolbarForegroundColor = parameters.ToolbarForeground.Color;
    if (!bTopBarVisible)
        buttonsContainerDiv.css("display", "none");
    else {
        var scrollOptions = {
        ...
        buttonsContainerDiv.dxScrollView(scrollOptions).dxScrollView("instance");
    }
    
    return {
        styleButtons: function () {
            var styledButtons = [];
            $.each(buttonsContainerDiv.find("div.topButton"), function (i, el) {
                var elem = $(el);
                elem.css({
                ...
                styledButtons.push(elem);
            });
            return styledButtons;
        },
        setHeight: function (referenceEl) {
            if (bTopBarVisible)
                buttonsContainerDiv.css("height", referenceEl.height() + parseFloat(buttonsContainerDiv.css("padding-top")) + parseFloat(buttonsContainerDiv.css("padding-bottom")) + parseFloat(referenceEl.css("margin-top")) + parseFloat(referenceEl.css("margin-bottom")) + 1.5);
        }
    };
}

var collapsibleLegendPanel = function (chart, legendStatus, legendTitle, legendContainer, dataGridContainer, trendcontainer, thisPage, thisPageHeight, parentHeight, openHeightOffset) {
    var startY;
    var mouseMoveAnimationFrameRef = { id: null };

    var onLegendTitleClick = function (e) { //mouseover
        if (e.type == "touchstart") {
            //e.stopPropagation(); 
            e.preventDefault();
        }
        legendStatus.bLegendExpanded = !legendStatus.bLegendExpanded;
        legendTitle.css({
            "color": legendStatus.bLegendExpanded ? "orange" : "unset",
            "opacity": legendStatus.bLegendExpanded ? 0.7 : 1,
        });
        legendContainer.stop(true).animate({ "margin-top": legendStatus.bLegendExpanded ? 0 : thisPageHeight }, 300, "swing", function () {
            if (legendStatus.bLegendExpanded)
                legendTitle.addClass("selectedViewerPanel");
            else
                legendTitle.removeClass("selectedViewerPanel");
            if (legendStatus.bLegendExpanded) {
                if (!legendStatus.openLegendHeight) {
                    legendContainer.trigger("resetChartHeight");
                    //openLegendHeight = latestLegendHeight ? latestLegendHeight : thisPage.parent().height() - trendcontainer.height() - 70;
                    legendStatus.openLegendHeight = legendStatus.latestLegendHeight ? legendStatus.latestLegendHeight : parentHeight - trendcontainer.height() - openHeightOffset;
                }
                else
                    legendContainer.trigger("updateChartHeight");
                if (undefinedOrNull(legendStatus.latestLegendHeight))
                    legendStatus.latestLegendHeight = legendStatus.openLegendHeight;
                openLegendAtHeight(legendStatus.latestLegendHeight);
            }
            else {
                legendContainer.trigger("resetChartHeight");
            }
    ...
    var openLegendAtHeight = function (height) {
        dataGridContainer.height(height);
        legendContainer.css("margin-top", 0);
        legendTitle.addClass("selectedViewerPanel");
    }
    var onLegendResize = function(e) {
        e.preventDefault();
        invokeDelayedAction(function () {
            var dy = e.pageY - startY;
            startY = e.pageY;
            var newHeight = dataGridContainer.height() - dy;
            if (newHeight > minLegendHeight && newHeight < parentHeight * 0.9) {
                legendContainer.css("margin-top", parseFloat(legendContainer.css("margin-top")) + dy);
                dataGridContainer.height(newHeight);
            }
        }, mouseMoveAnimationFrameRef, mouseMoveThrottlingMS);
    }
    var onLegendDragStart = function (e) {
        if (e.type == "touchstart") {
            //e.stopPropagation(); 
            e.preventDefault();
        }
        if (legendStatus.bLegendExpanded) {
            $(currentRoot).on("mouseup touchend", onLegendDragEnd);
            legendStatus.latestLegendHeight = dataGridContainer.height();
            legendContainer.stop(true);
            startY = e.pageY;
            $(currentRoot).on("mousemove touchmove", onLegendResize);
        }
    };
    var onLegendDragEnd = function (e) {
        if (e.type == "touchend") {
            //e.stopPropagation(); 
            e.preventDefault();
        }
        $(currentRoot).off("mousemove touchmove", onLegendResize);
        if (mouseMoveAnimationFrameRef.id !== null)
            cancelAnimationFrame(mouseMoveAnimationFrameRef.id);
        var newLegendHeight = dataGridContainer.height();
        chart.option("size.height", Math.max(0, parseFloat(chart.option("size.height")) - (newLegendHeight - legendStatus.latestLegendHeight)));
        legendContainer.css("margin-top", 0);
        legendStatus.latestLegendHeight = newLegendHeight;
        $(currentRoot).off("mouseup", onLegendDragEnd);
        legendContainer.trigger("legendDragEnd");
    }
    return {
        openLegendAtHeight: openLegendAtHeight,
        init: function () {
            legendTitle.on("mousedown touchstart", onLegendTitleClick);
            thisPage.on("mousedown touchstart", ".dx-datagrid-headers, #legendDragger", onLegendDragStart);
        },
        dispose: function () {
            if (mouseMoveAnimationFrameRef.id !== null)
                cancelAnimationFrame(mouseMoveAnimationFrameRef.id);
            legendTitle.off("mousedown touchstart", onLegendTitleClick);
            thisPage.off("mousedown touchstart", ".dx-datagrid-headers, #legendDragger", onLegendDragStart);
            $(currentRoot).off("mousemove touchmove", onLegendResize);
            $(currentRoot).off("mouseup", onLegendDragEnd);
        }
    }
...
(function (dxChartHelpers) {
    dxChartHelpers.ensureGeneralScaleVisibility = function (chart, series) {
        if (series.length < 2)
            return;

        var bScaleShown;
        var axes = chart.option("valueAxis");
        $.each(series, function (index, serie) {
            if (!bScaleShown && serie.visible !== false) {
                bScaleShown = true;
                axes[index].visible = axes[index].tick.visible = axes[index].label.visible = true;
            }
            else
                axes[index].visible = axes[index].tick.visible = axes[index].label.visible = false;
        });
    };
})(window.dxChartHelpers = window.dxChartHelpers || {});

function buildStyleID(thisPage, thisParentId, screenName) {
    return (thisPage.attr("id") + thisParentId + screenName).replace(/\W+/g, "_");
}

function getSolidColor(prop) {
    return prop.SolidColor ? prop.SolidColor : prop.Color;
}
...
function updateDatasource(instance, dataSource, filter) {
    instance.option("dataSource", dataSource);
    if (filter) {
        instance.option("dataSource",
            instance.option("dataSource").filter(function (itemData) {
                function looseFnParse(obj) {
                    return Function(`"use strict";return (${obj})`)()(itemData);
                }
                return looseFnParse("function(itemData){ return " + filter + "}");
            })
        );
    }
}

function findRealHeight($el) {
    if (!$el.is(":visible"))
        return 0;
...
function truncateSVGTextToWidth(textSVGNode, maxLineWidth) {
    var initialText = textSVGNode.textContent;
    if (textSVGNode.getBBox().width <= maxLineWidth)
        return;

    for (var x = initialText.length; x > 0; x -= 1) {
        if (textSVGNode.getSubStringLength(0, x) <= maxLineWidth) {
            textSVGNode.textContent = initialText.substring(0, x);
            return;
        }
    }
}

function populateItemsSource(itemsSource) {
    var titems = new Array();
    jQuery.each(itemsSource, function (index, item) {
        let translatedItems = i18next.t1(item).split("|");
        $.each(translatedItems, function (i, el) {
            titems.push(el);
        });
    });
    return titems;
...
function autoWrapSVGTextToWidth(textSVGNode, maxLineWidth, bJustify) {
    let bHasNewLineChars = textSVGNode.textContent.indexOf('\n') != -1;
    let bExceedsWidth = textSVGNode.getBBox().width > maxLineWidth;
    if (!bExceedsWidth && !bHasNewLineChars)
        return $(textSVGNode);

    var entireText = textSVGNode.textContent;
    var line = '';
    var $textSVGNode = $(textSVGNode);
    var lineHeight = textSVGNode.getBBox().height;
    var startX = $textSVGNode.attr("x");
    var startY = $textSVGNode.attr("y");

    var $finalTextNode = $textSVGNode.clone();
    var finalTextNode = $finalTextNode[0];
    ...
    if (bHasNewLineChars) {
        var lines = entireText.split('\n');
        for (var n = 0; n < lines.length; n++) {
            let lineText = lines[n];
            innerHTML += "<tspan x='" + startX + "' " + (n == 0 ? "y='" + startY + "'" : "dy='" + lineHeight + "'") + ">" + lineText + "</tspan>";
        }
        finalTextNode.innerHTML = innerHTML;
        $textSVGNode.replaceWith(finalTextNode);
        return $finalTextNode;
    }

    var letters = entireText.split('');
    var spaceWidth = 0;
    if (bJustify) {
        var firstSpaceIndex = entireText.indexOf(' ');
        if (firstSpaceIndex !== -1)
            spaceWidth = textSVGNode.getSubStringLength(firstSpaceIndex, 1);
    }

    let printedLetters = 0;
    for (var n = 0; n < letters.length; n++) {
        let spaceOnEnd = false;
        var testLine = line + letters[n];
        textSVGNode.textContent = testLine;
        //width overcome
        if (textSVGNode.getSubStringLength(0, testLine.length) > maxLineWidth && n > 0) {
            var lineTextLastSpace = testLine.lastIndexOf(' ');
            var lineText = line;
            if (lineTextLastSpace !== -1) { //wrapping at the latest space: next word prints in the new line
                lineText = testLine.substring(0, lineTextLastSpace);
                printedLetters++;
                n = printedLetters + lineTextLastSpace;
                spaceOnEnd = true;
            }
            if (bJustify && spaceWidth > 0) {
                var addingSpaceChars = Math.floor((maxLineWidth - textSVGNode.getSubStringLength(0, lineText.length)) / spaceWidth);
                $finalTextNode.css("white-space", "pre");
                var lineWords = lineText.split(' ');
                lineText = '';
                for (var i = 0; i < addingSpaceChars; i++) {
                    var wordIndex = i % (lineWords.length - 1);
                    lineWords[wordIndex] += ' ';
                }
                $(lineWords).each(function (i, e) {
                    lineText += lineWords[i] + ' ';
                });
            }
            innerHTML += "<tspan " + (spaceOnEnd ? "class='spaceOnEnd'" : "") + " x='" + startX + "' " + (lineIndex == 0 ? "y='" + startY + "'" : "dy='" + lineHeight + "'") + ">" + lineText + "</tspan>";
            printedLetters += lineText.length;
            line = letters[n];
            lineIndex++;
        }
        else {
    ...
    innerHTML += "<tspan x='" + startX + "' dy='" + lineHeight + "'>" + line + "</tspan>";
    finalTextNode.innerHTML = innerHTML;
...
function checkVAlignment(textNode, textBox, tspans, rectBox, vAlignPropertyValue, bHasNewLine, vMargin = 0, clipPath = null) {
    let vAlignedY = null;
    if (vAlignPropertyValue != 0 && vAlignPropertyValue != 3) {
        textBox = textNode[0].getBBox();
        if (vAlignPropertyValue == 2) //bottom
            vAlignedY = rectBox.y + rectBox.height - textBox.height - vMargin;
        else
            vAlignedY = rectBox.y + rectBox.height / 2 - textBox.height / 2;
    }
    if (tspans.length == 0) {
        if (vAlignedY != null)
            textNode.attr("y", vAlignedY);
    }
    else {
        if (!bHasNewLine) {
            if (vAlignedY != null) {
                textNode.attr("y", vAlignedY);
                tspans.first().attr("y", vAlignedY);
            }
        }
        else {
            if (vAlignedY != null)
    ...
    if (clipPath)
        clipPath.children().first().attr("y", vAlignedY);

    return textBox;
}

var MyDecimal = (function () {
    var decimal;
    function MyDecimal(val) {
        try {
