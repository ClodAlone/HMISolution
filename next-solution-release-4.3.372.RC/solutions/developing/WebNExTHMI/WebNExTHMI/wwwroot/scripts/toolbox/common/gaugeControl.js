function onGaugeControlLoaded(obj) {
    ...
    var gaugeSvgPoint;
    var gaugeElement;
    var defaultRangeOffset = -29; //= default gauge offset on desktop
    ...
    var containerCss = {};
    setObjectBorder(containerCss, parameters, thisPage, container);
    if (parameters.Background && parameters.Background.Color)
        containerCss["background"] = parameters.Background.Color;
    if (scope.biOS && parentOpacity)
        containerCss["opacity"] = parentOpacity;

    if (!$.isEmptyObject(containerCss)) {
        ...
        gaugeValueDisplay = display.dxTextBox({
            ...
            onContentReady: function (e) {
                ...
                var displayCssOpt = {
                    ...
                    "color": getSolidColor(parameters.ValueForeground)
                };
    ...
    var paramFontSize = 10;
    var bHasLabels = parameters.MajorIntervalCount > 1 || parameters.ShowFirstLabel;
    if (bHasLabels)
        paramFontSize = parseInt(parameters.LabelFontSettings.FontSize, 10);
    ...
    var gaugeConfig = {
        ...
        scale: {
            ...
            label: {
                ...
                font: {
                    ...
                    color: bHasLabels ? getSolidColor(parameters.LabelForeground) : "transparent"
                },
        ...
        onDrawn: function (e) {
            ...
            gaugeElement = e.component.element();

            if (!gaugeArcsCenter) {
                var needleHole = gaugeElement.find(".dxg-spindle-hole");
                gaugeArcsCenter = {
            ...
            if (bHasRangeBar && e.component.option("valueIndicator.type") !== "rangebar") {
                bFirstDrawn = false;
                e.component.option({
            ...
            if (spindleStyleProp && scope.Styles && scope.Styles[parameters.screenId])
                applySpindleStyle(gaugeElement);

            var rangeContainer = gaugeElement.find("g.dxg-range-container")[0];
            ...
            if (bHasRangeBar)
                gaugeElement.find(".dxg-value-indicator .dxg-space-bar").attr("fill", parameters.RangeBarBackground.Color);

            fixRangesLayout();

            if (!bHasSVGBackground) {
                var finalRangeContainerSize = gaugeElement.find("g.dxg-range-container")[0].getBoundingClientRect();
    ...
    function setValueLabelPosition(pageTop, scaleTopCorrectionFactor) {
        ...
        if (parameters.ShowEngeneeringUnit) {
            gaugeEU = $("<div class='gaugeEUDisplay' style='line-height: 1px; background: transparent; white-space: nowrap; border: none; margin-left:50%; text-align: center; position: " + (scope.biOS && (parameters.IsFastGaugeControl || parameters.currentPopupId) && !webKitRenderCorrection ? "absolute" : "fixed") + "; color: " + getSolidColor(parameters.EngeneeringUnitForeground) + "; font-family: " + parameters.EngeneeringUnitFontSettings.FontFamily + "; font-size: " + parameters.EngeneeringUnitFontSettings.FontSize + "px; font-weight:" + fontWeightConverter(parameters.EngeneeringUnitFontSettings.FontWeight) + "; z-index: 3;'>" + i18next.t1(parameters.EngeneeringUnit) + "</div>")
    ...
    function onGaugeDataChanged() {
        ...
        try {
            ...
            if (gauge.option("animation.enabled") == false) {
                gauge.option("animation.enabled", true);
                fixRangesLayout();
            }
    ...
    function setGaugeValue(newInvariantValue) {
        ...
        scope.corehubconnection.invoke("SetValue", parameters.SVGReferenceId, String(newInvariantValue), parameters.screenId, false)
            .then((ret) => {
    ...
    function onDragging(e) {
        ...
        if (cursorpt != null) {
            ...
            if (latestDraggedValue != null) {
                if (gauge.option("animation.enabled") == true) {
                    gauge.option("animation.enabled", false);
                    fixRangesLayout();
                }
    ...
    function notifyServerOnDragging() {
        ...
        scope.corehubconnection.invoke("SetValue", parameters.SVGReferenceId, String(latestDraggedValue), parameters.screenId, false).finally(() => {
    ...
    function fixRangesLayout() {
        for (var i = 0; i < gaugeConfig.rangeContainer.ranges.length; i++) {
            ...
            if (i === 0)
                gaugeElement.find("svg.dxg.dxg-circular-gauge").css("overflow", "visible");
            var rangeArc = gaugeElement.find(".dxg-range.dxg-range-" + i)[0];
            ...
            if (rangeArc) {
                ...
                var rangeInfoNormalizedOffset = rangeInfo.offset - defaultRangeOffset;
                var rangeOffset = rangeInfoNormalizedOffset * -36 / defaultRangeOffset * -1 /* * sizeFactor*/;
                ...
                if (!bHasSVGBackground && (gaugeType == "full"))
                    scale += -defaultRangeOffset / 100 / sizeFactor;
