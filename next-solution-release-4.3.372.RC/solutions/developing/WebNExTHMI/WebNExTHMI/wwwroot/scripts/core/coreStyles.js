function initStyles($scope) {
    $scope.applyControlStyle = function ($scope, thisParent, controlcontainer, parameters, webKitRenderCorrection = false, styleNodeAttributes = undefined, bPrepend = false) {
        ...
        return (function () {
            if (bStyleApplied) {
                    ...
                    if (webKitRenderCorrection) {
                        ...
                        if (nodeON.find("animateTransform").length > 0) {
                            ...
                            fastdom.mutate(function () {
                                ...
                                if (bPrepend)
                                    nodeON.detach().prependTo(fobjReplacerContainer);
                                else
                ...
                fastdom.mutate(function () {
                    if (styleNodeAttributes) {
                        if (controlON && controlON.node)
                            $(controlON.node).attr(styleNodeAttributes);
                        if (controlOFF && controlOFF.node)
                            $(controlOFF.node).attr(styleNodeAttributes);
                        if (controlNULL && controlNULL.node)
                            $(controlNULL.node).attr(styleNodeAttributes);
                    }
            ...
            return {
                ...
                "activateStyleON": function () {
                    ...
                    if (parameters.BackgroundOn && parameters.BackgroundOn.Color) {
                        ...
                        for (var i = 0; i < tagsBG_ON.length; i++) {
                            $(tagsBG_ON[i]).attr("fill", urlReferenceReplace(parameters.BackgroundOn.Color, parameters.screenId));
                        }
                    }
                    else if (parameters.SvgBackground && parameters.SvgBackground.Color) {
                        var tagsBG_ON = $(controlON.node).find("[tag='BG']");
                        for (var i = 0; i < tagsBG_ON.length; i++) {
                            $(tagsBG_ON[i]).attr("fill", urlReferenceReplace(parameters.SvgBackground.Color, parameters.screenId));
                        }
                    }
                ...
                "toggleButtonStyle": function (bOn) {
                    ...
                    if (bPrepend)
                        styleNode.prependTo(controlcontainer);
                    else
