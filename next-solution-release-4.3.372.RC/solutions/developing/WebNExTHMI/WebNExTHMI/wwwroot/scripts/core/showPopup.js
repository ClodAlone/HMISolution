...
function showPopup(arg/*
    ...
    var deregisterListenerPopup = arg.$scope.$on(onPopupSizeRequest + arg.parent, function (value, obj) {
        ...
        if (ip) {
            ...
            bWindowStyleNone = windowStyle === "none";
            if (bWindowStyleNone) {
            ...
            if (bFits && !bWindowStyleNone && overlayContentSize.h !== null) {
                var toolbar = overlayContent.find(".dx-toolbar");
                overlayContentSize.h += toolbar.length > 0 ? parseFloat(toolbar[0].clientHeight) : 0;
            }
