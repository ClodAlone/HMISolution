...
define(function () {
    return {
        ...
        OpenPadCommandExecute: async function (scope, command, control, screenId) {
            if (command.SVGMaxValueReferenceId == -1 || (command.SVGMinValueReferenceId == -1 && command.openpadtype != 1))
                return;

            var minValue = command.minValue;
            var maxValue = command.maxValue;
            //if (command.useEUnit && !notNumeric(scope.dataValues[parameters.screenId][parameters.SVGReferenceId].rangeLow) && !notNumeric(scope.dataValues[parameters.screenId][parameters.SVGReferenceId].rangeHigh)) {
            ...
            //}
            //else {
                if (command.SVGMinValueReferenceId && command.SVGMinValueReferenceId != -1 && scope.dataValues[screenId][command.SVGMinValueReferenceId].isGood && !isNaN(parseFloat(scope.dataValues[screenId][command.SVGMinValueReferenceId].invariantValue))) {
                    minValue = scope.dataValues[screenId][command.SVGMinValueReferenceId].invariantValue;
                }
                if (command.SVGMaxValueReferenceId && command.SVGMaxValueReferenceId != -1 && scope.dataValues[screenId][command.SVGMaxValueReferenceId].isGood && !isNaN(parseFloat(scope.dataValues[screenId][command.SVGMaxValueReferenceId].invariantValue))) {
...
function SetValue(scope, id, value, busy, screenId, command) {
    ...
    scope.corehubconnection.invoke("SetValue", id, String(value), screenId, true)
        .then((ret) => {
