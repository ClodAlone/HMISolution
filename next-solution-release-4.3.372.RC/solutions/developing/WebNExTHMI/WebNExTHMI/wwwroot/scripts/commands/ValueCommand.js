        ...
        ValueCommandExecute: async function (scope, command, control, screenId) {
            switch (command.type) {
                ...
                case /*'TransferValue'*/6:
                    ...
                    break;
                case /*'NumericPad'*/7:
                    if (command.SVGMaxValueReferenceId == -1 || command.SVGMinValueReferenceId == -1)
                        return;
                ...
                case /*'AlphaNumericPad'*/8:
                    if (command.SVGMaxValueReferenceId == -1)
                        return;
                ...
                case /*SwapSign*/ 13:
                    ...
                    break;
                case /*AppendString*/ 14:
                    var finalValue = i18next.t1(command._value);
                    try {
                        finalValue = scope.dataValues[screenId][command.SVGReferenceId].value + finalValue;
                    }
                    catch { }
                    SetValue(scope, command.SVGReferenceId, finalValue, false, screenId, command);
                    break;
            }
        },
