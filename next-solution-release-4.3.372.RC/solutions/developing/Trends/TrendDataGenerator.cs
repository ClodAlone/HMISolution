...
namespace Trends
{
    public class TrendDataGenerator : BaseDataGenerator
    {
        #region Declarations
        ...
        #region Constructors
        public TrendDataGenerator(string penId, DataGeneratorSettings settings, int commandTimeout)
        ...
        protected override void SetDataSources(System.Threading.CancellationToken ct)
        {
            ...
            var task1 = Task.Factory.StartNew(() =>
            {
                ...
                if (ret != null && ret.Count != 0)
                {
                    ...
                    var _sampnum = (from c in templ
                                    where DateTime.Compare(c.SourceTimestamp, endTime) >= 0
            ...
            task1.ContinueWith(ret =>
            {
                ...
                {
                    values = new SafeObservableCollection<MyDataValue>(ret.Result);
                    UpdateStat();
                    OnHistoryLoaded();
                }
            }, System.Threading.Tasks.TaskScheduler.FromCurrentSynchronizationContext());
