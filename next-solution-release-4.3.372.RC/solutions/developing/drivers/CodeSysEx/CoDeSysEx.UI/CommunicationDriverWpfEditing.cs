...
namespace CoDeSys.UI
{
    public class CommunicationDriverWpfEditing : ICommunicationDriverWpfEditing3, IDisposable
    {
        ...
        public bool CopyFile(string sourceconn, string targetconn)
        {
            ...
            bool result = DriverCodeBaseEx.CommunicationDriver.CopyFile<CoDeSysDriverSettings>(sourceconn, targetconn, drivername);
            result &= DriverCodeBaseEx.CommunicationDriver.CopyFile<CoDeSysChangeTag>(sourceconn, targetconn, drivername, DriverCodeBaseEx.Helpers.DriverInfo.GetChangeDynTagsExtension());
            return result;
        }
        ...
        {
            CoDeSysDynTagSettings dynTag = new CoDeSysDynTagSettings();
        ...
        {
            var genSet = generalSettings as CoDeSysDriverSettings;
            if (genSet == null)
                return null;

            if (channelEditor == null)
                channelEditor = new ChannelDetails();
            var ch = new CoDeSysChannelSettings(genSet.Session);
        ...
        public UserControl NewStationSettingsEditor(object generalSettings)
        {
            ...
            var st = new CoDeSysStationSettings(genSet.Session);
        ...
        public bool DisableDriver(string connectionString, bool bProtected, Guid protectionCode)
        {
            ...
            {
                string drivername = DriverCodeBaseEx.Helpers.DriverInfo.GetDriverName(this).Replace(".UI", "");
                configurationEditor.Init<CoDeSysDriverSettings>(connectionString, drivername, bProtected, protectionCode);
