 ...
 * <type>Int</type>
 * <description>
 *  Inactivity timeout in seconds. Only for BlkDrvTcp.
 *      - If no block is received or sent within this time, the socket of this TCP connection is closed.
 *      - If no block is received within 1/3 (a third) of this time
 *        on a newly opened socket (where the runtime is the server),
 *        the socket of this TCP connection is closed
 *        and requests from the client’s IP address are discarded for the next BLKDRVTCPKEY_INT_INACTIVITY_TO seconds.
 * </description>
 */
 ...
 * <type>String</type>
 * <description>
 *  The BlkDrvUsb is designed to support different USB drivers of the operating system layer, e. g. LibUsb. 
 *  For each of these OS drivers, the CODESYS runtime (or Gateway) needs a specific so called USB mini port 
 *  driver to adapt the OS driver interface to the generic BlkDrvUsb.
 *  This mandatory setting defines the component name of the USB mini port driver used for a set of devices.
 *  The index of the settings allows to use different USB mini port drivers for several devices. 
 *  Example: 0.Name=CmpUsbMpdUsbLib
 *  Please note, the USB mini port driver component must also be known by the component manager. Therefore
 *  the USB mini port driver component must additional be added to its component list. 
 *  Only for BlkDrvUsb.
 *  A typical configuration for one instance of the CmpBlkDrvUsb looks as follows:
 *  0.Name=CmpUsbMpdUsbLib
 *  0.Dev.0.Name=UsbLibDev0
 *  0.Dev.0.VendorId=0x16DE
 *  0.Dev.0.DeviceId=0x0030
 *  0.Dev.0.Config=1
 *  0.Dev.0.Interface=0
 *  0.Dev.0.EndpointIn=0x81
 *  0.Dev.0.EndpointOut=0x03
 * </description>
 */
 ...
 * <type>String</type>
 * <description>
 *  Prefix to identify an instance of the related USB mini port driver. The second number is an index of the 
 *  configuration for the device counted separately for each driver.
 *  Example: 0.Dev.0.Name=UsbLibDev0
 *  Only for BlkDrvUsb.
 * </description>
 */
 ...
 * <type>String</type>
 * <description>
 *  Name of the CmpBlkDrvUsb instance. This name is used for the registration at the router.
 *  Example: 0.Dev.0.Name=UsbLibDev0
 *  Only for BlkDrvUsb.
 * </description>
 */
 ...
 * <type>Int</type>
 * <description>
 *	VendorId of the device for which the driver is to be used.
 *  Each USB device has its own VendorId and DeviceId describing it.  
 *  Example: 0.Dev.0.VendorId=0x16DE
 *  Only for BlkDrvUsb.
 * </description>
 */
 ...
 * <type>Int</type>
 * <description>
 *	DeviceId of the device for which the driver is to be used.
 *  Each USB device has its own VendorId and DeviceId describing it.  
 *  Example: 0.Dev.0.DeviceId=0x0030
 *  Only for BlkDrvUsb.
 * </description>
 */
 ...
 * <type>Int</type>
 * <description>
 *	Setting to select the configuration of the USB device, which is to be used.
 *  USB devices can have several possible configurations with several interfaces.
 *  A configuration provides one or more interfaces, each with zero or more endpoint descriptors 
 *  describing a unique set of endpoints within the configuration. For detailed information see
 *  the official USB specification (9.6.3 Configuration).
 *  Example: 0.Dev.0.Config=1
 *  Only for BlkDrvUsb.
 * </description>
 */
 ...
 * <type>Int</type>
 * <description>
 *	Setting to configure the interface of the USB device, which is to be used.
 *  USB devices can have several possible interfaces with several endpoints.
 *  For detailed information see the official USB specification (9.6.5 Interface).
 *  Example: 0.Dev.0.Interface=0
 *  Only for BlkDrvUsb.
 * </description>
 */
 ...
 * <type>Int</type>
 * <description>
 *  Setting for specifying the EndpointIn of the USB device that is used to read from it.
 *  Make sure that the used EndpointIn is also provided in the interface that is used.
 *  For detailed information see the official USB specification (9.6.6 Endpoints).
 *  Example: 0.Dev.0.EndpointIn=0x81
 *  Only for BlkDrvUsb.
 * </description>
 */
 ...
 * <type>Int</type>
 * <description>
 *  Setting for specifying the EndpointOut of the USB device that is used to write to it.
 *  Make sure that the used EndpointOut is also provided in the interface that is used.
 *  For detailed information see the official USB specification (9.6.6 Endpoints).
 *  Example: 0.Dev.0.EndpointOut=0x03
 *  Only for BlkDrvUsb.
 * </description>
 */
 ...
 * <type>String</type>
 * <description>
 *	Logical local address of the USB serial interface. Used for AutoAdressing and DuplexAutoNegotiate
 *  features. Configured value is only default, effective value is managed and written by the BlkDrvUsb.
 *  Example: 0.Dev.0.LocalAddress=254
 *  Only for BlkDrvUSB.
 * </description>
 */
 ...
 *  peers use the value 2, they will both enable the auto addressing feature. Thus in general there is no
 *  need to change this setting.
 *  Example: 0.Dev.0.EnableAutoAddressing=2
 *  Only for BlkDrvUSB.
 * </description>
 */
 ...
 *  two peers will use auto addressing.
 *  Supports only point-to-point communication.
 *  Example: 0.Dev.0.HalfDuplexAutoNegotiate=2
 *  Only for BlkDrvUsb.
 * </description>
 */
