...
namespace Fatek.UI
{
    public class CommunicationDriverWpfEditing : ICommunicationDriverWpfEditing3
    {
        ...
        public bool CopyFile(string sourceconn, string targetconn)
        {
            ...
            bool result = DriverCodeBaseEx.CommunicationDriver.CopyFile<FatekDriverSettings>(sourceconn, targetconn, drivername);
            result &= DriverCodeBaseEx.CommunicationDriver.CopyFile<FatekChangeTag>(sourceconn, targetconn, drivername, DriverCodeBaseEx.Helpers.DriverInfo.GetChangeDynTagsExtension());
            return result;
        }
        ...
        {
            Fatek.FatekDynTagSettings dynTag = new Fatek.FatekDynTagSettings();
        ...
        {
            var genSet = generalSettings as FatekDriverSettings;
            if (genSet == null)
                return null;

            if (channelEditor == null)
                channelEditor = new ChannelDetails();
            var ch = new FatekChannelSettings(genSet.Session);
        ...
        public UserControl NewStationSettingsEditor(object generalSettings)
        {
            ...
            var st = new FatekStationSettings(genSet.Session);
        ...
        public bool DisableDriver(string connectionString, bool bProtected, Guid protectionCode)
        {
            ...
            {
                string drivername = DriverCodeBaseEx.Helpers.DriverInfo.GetDriverName(this).Replace(".UI", "");
                configurationEditor.Init<FatekDriverSettings>(connectionString, drivername, bProtected, protectionCode);
