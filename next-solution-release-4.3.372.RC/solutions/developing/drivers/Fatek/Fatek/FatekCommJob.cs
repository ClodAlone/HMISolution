...
namespace Fatek
{
    public class FatekCommJob : CommJob
    {
        ...
        public override JobAggregationType TestAggregateJob(CommJob candJob, out uint extraBytes)
        {
            ...
            uint start = (uint)(AreaAddress * 16 + ElementNumber);
            uint startTest = (uint)(testJob.AreaAddress * 16 + testJob.ElementNumber);
        ...
        public CommandTypes GetRequestType()
        {
            switch (Type)
            {
                case LinkType.Input:
                    return CommandTypes.ReadCmd;
                case LinkType.UnconditionalOutput:
                    return CommandTypes.WriteCmd;
