...
namespace S7TCP
{
    public enum S7ErrorCodes : int
    {
        ...
        ErrorTooFewData,
        ErrorExceptionParsingReply,
        ErrorConnectionToDevice = 1400,
        ErrorStatusNotZero = 1500,
        ErrorFromDevice = 2000,
        ErrorUnrecognizedReply = 2001,
        ErrorPrepareRequestFailed = 2002,
    }
    ...
    {
        public class ReadWriteListLimitateSize : ICloneable
        {
            public uint RequestLength { get; set; }    // request length
            public uint ResponseLength { get; set; } // responde length

            public ReadWriteListLimitateSize()
            {
                RequestLength = S7Protocol.INITIAL_REQUEST_LEN;
                ResponseLength = S7Protocol.INITIAL_RESPONSE_LEN;
            }

            public void AddPaddingByteIfOddAddressResponse()
            {
                //Checked fill byte
            ...
            public void AddPaddingByteIfOddAddressRequest()
            {
                if ((RequestLength & 1) != 0)
                    RequestLength++;
            }
        ...
        public const int RESP_PDU_SZL_ID = 29;

        //public static uint MIN_PLC_PDU_SIZE = 480; // minimum size of PLC PDU returned during Connect; under this value, use smaller frame size        
        //public static uint PLC_PDU_SIZE = 0; // minimum size of PLC PDU returned during Connect; under this value, use smaller frame size        
        //public static uint NEW_PLC_PDU_SIZE = PLC_PDU_SIZE; // use to manage frame data size change "event"
        //public static uint MAX_TEL_LENGTH = 0; // 240; --> moved into S7Protocol() CTor()
        //public static uint MAX_MPI_TLG_LEN = 0; // 240; --> moved into S7Protocol() CTor()
        //public static uint MAX_DATA_BYTES = 0; // 212; --> moved into S7Protocol() CTor()        
        //public static int MAX_TEL_LENGTH_BEGIN_DEVICE_READ = 0; // 259; --> moved into S7Protocol() CTor()
        //public static uint MAX_BUFFER_SIZE = 0; // 256

        public const uint DEFAULTSTRINGLENGTH_TIAPORTAL = 254;
        public const uint SINGLEFRAME_STRING_MAX_SIZE = 200;

        public const uint INITIAL_REQUEST_LEN = 12;
        public const uint INITIAL_RESPONSE_LEN = 14;
        ...
        public static byte[] RequestReadUserData = { 0x32,	// PROTO_ID
                                                   0x07,	// ROSCTR Remote Operating Services Control
                                                   0x00,	// RED_ID Redundancy Identification (HI)
                                                   0x00,	// RED_ID Redundancy Identification (LO) 
                                                   0x02,	// PDU_REF (HI)
                                                   0x00,	// PDU_REF (LO)
                                                   ...
                                                   0x00,	// DAT_LG Data Length (HI)
                                                   0x08};// Number of S7 pointers

        public static byte[] RequestReadSZL = { 0x00,	// Header
                                                0x01,	// Header
                                                0x12,	// Header
                                                0x04,	// length
                                                0x11,	// Method (Request/Response): Req (0x11)
                                                0x44/*,	// 0100 .... = Type: Request (4) .... 0100 = Function group: CPU functions (4)
                                                0x01,	// Subfunction: Read SZL (1)
        ...
        public const string TEST_COMM_DYNAMIC_SETTINGS = "S7TCP.Station={0}|LinkType=0|SA=MB0";
        #region methods override

        public static uint PrepareRequest(S7TCPChannel ch, List<CommJob> list, List<CommJob> listJobExec, ref byte[] buffer)
        {
            ...
            if (list[0].ReadRequest())
                nRes = PrepareReadRequest(ch, list, ref listJobExec, ref buffer);
            else
                nRes = PrepareWriteRequest(ch, list, ref listJobExec, ref buffer);
        ...
        static uint PrepareWriteRequest(S7TCPChannel ch, List<CommJob> ListJobPending, ref List<CommJob> ListJobExec, ref byte[] buffer)
        {
            uint idx = 0;
            ...
            try
            {
                ...
                byte[] dataBuf = new byte[ch.MAX_BUFFER_SIZE];
                ...
                {                                
                    ...
                    {
                        int LastAddedTag = j.GetTagListOnWritingCount();
                        ...
                        if (!j.LimitatedGetJobData(ref objectData, nTotalByteAnyPointerAndVarValue + dataOverheadSize, out S7TCPTag firstTag))
                        {
                            RequestEnd = true;
                        ...
                        if (j.isProtocolBool())
                        {
                            if (firstTag.TagNode.DataType == Opc.Ua.DataTypes.Boolean || j.ElementNumber > 0)
                            {
                                ...
                                List<Tag> tagListOnWriting = j.GetTagListOnWriting();
                                for (int tagIndex = LastAddedTag; tagIndex < tagListOnWriting.Count(); tagIndex++)
                                {
                                    if (tagListOnWriting[tagIndex].TagNode.ArrayDimension == 0)
                                        nTag += (tagListOnWriting[tagIndex].Size / sizeDataType);
                                    else
                                        nTag += ((tagListOnWriting[tagIndex].Size / tagListOnWriting[tagIndex].TagNode.ArrayDimension) / sizeDataType);
                                }
                            }
                            else
            ...
            {
                // cause of error is probably a switch to PLC with smaller frame data size
                idx = 0;
            }
        ...
        static uint PrepareReadRequest(S7TCPChannel ch, List<CommJob> ListJobPending, ref List<CommJob> ListJobExec, ref byte[] buffer)
        {
            uint idx = 0;
        ...
        public static uint PrepareReadRequestTestInformation(S7TCPChannel ch, List<CommJob> ListJobPending, ref List<CommJob> ListJobExec, ref byte[] buffer, bool TestConnectionReadFirmware)
        {
            uint idx = 0;

            S7Protocol.IpReadHeader.CopyTo(buffer, idx);
            idx += 3;
            S7Protocol.RequestReadUserData.CopyTo(buffer, idx);
            idx += 10;
            S7Protocol.RequestReadSZL.CopyTo(buffer, idx);
            idx += 6;

            buffer[idx++] = 0x01; // Subfunction: Read SZL (1)
            buffer[idx++] = 0x00; // Sequence number: 0

            buffer[idx++] = 0xff; //Return code: Success (0xff)
            buffer[idx++] = 0x09; //Transport size: OCTET STRING(0x09)
            buffer[idx++] = 0x00; //Length: 4
            buffer[idx++] = 0x04; //Length: 4
            ...
            buffer[idx++] = 0x01;
            buffer[idx++] = 0x11;
            if (!TestConnectionReadFirmware)
            {
                //SZL-Index: 0x0001 [Identification of the module]
                buffer[idx++] = 0x0;
                buffer[idx++] = 0x1;//Identificazione dell'unità
            }
            if (TestConnectionReadFirmware)
            {
                ////SZL-Index: 0x0001 [Identification of the module]
                buffer[idx++] = 0x0;
                buffer[idx++] = 0x7;//Identificazione del firmware di base
            }
        ...
        //public static void SetConnect_block2_PduSize(uint len = 480)
        //{
        //    byte[] pduLen = BitConverter.GetBytes(len);

        //    S7Protocol.connect_block2[19] = pduLen[1];
        //    S7Protocol.connect_block2[20] = pduLen[0];
        //}

        public static uint GetConnect_block2_PduSize(byte[] readBuffer)
        {
            if (readBuffer == null || readBuffer.Length < 23)
                return 0;

            byte[] pduLen = new byte[] { readBuffer[22], readBuffer[21] };

            uint size = (uint)BitConverter.ToUInt16(pduLen, 0);
            
            return size;
        }

        //public static void GetAddressOffsetAndSize(Tag tag, out int offset, out uint sizeByte)
        //{
        //    S7TCPTag startTag = (S7TCPTag)tag;
        //    S7TCPDynTagSettings startDyn = (S7TCPDynTagSettings)(tag).DynSettings;

        //    offset = startDyn.Offset;
        //    sizeByte = startTag.Size;

        //    if (startTag.TagNode.DataType.IdType == IdType.Numeric)
        //    {
        //        switch ((uint)startTag.TagNode.DataType.Identifier)
        //        {
        ...
        //                int nrBytes = Math.DivRem((int)startTag.Size, 8, out int restBit);
        //                if (restBit > 0)
        //                    nrBytes++;
        //                sizeByte = (uint)nrBytes;
        //                break;
        //        }
        //    }
        //}

        public static int GetJobStartAddress(CommJob job)
        {
            return GetTagStartAddress(job.TagsList[0]);
        }

        public static int GetTagStartAddress(Tag tag)
        {
            S7TCPTag startTag = (S7TCPTag)tag;

            int offset = ((S7TCPDynTagSettings)(tag).DynSettings).Offset;
            if (startTag.TagNode.DataType.IdType == IdType.Numeric && (uint)startTag.TagNode.DataType.Identifier == (uint)BuiltInType.String)
                offset -= 1;                

            return offset;
        }

        public static void GetJobAddressOffsetAndSize(CommJob commJob, out int offset, out uint sizeByte)
        {
            S7TCPDynTagSettings startDyn = (S7TCPDynTagSettings)(commJob.TagsList[0]).DynSettings;

            offset = startDyn.Offset;
            sizeByte = commJob.TotalJobSize;

            if (((S7TCPCommJob)commJob).isLenStringEnable)
            {
                offset -= 1;
                sizeByte++;
            }
