...
namespace BrPvi.UI
{
    ...
    public partial class BrPviPlcImportParser : BrPviImportBase, IDisposable
    {
        ...
        BrPviConnectionStatus connectionStatus = BrPviConnectionStatus.NotInitialized;
        ...
        bool CreatePlcPviObjects()
        {
            ...
            pviDevice = new BrPviPviObject();
            gcHandlePviDeviceObj = GCHandle.Alloc(pviDevice);
            // Ethernet
            if (brStationSettings.BrPviInterfaceType == InterfaceTypes.Ethernet)
            {
        ...
        bool CreatePviTaskObjects()
        {
            ...
            {
                BrPviPviObject taskObj = new BrPviPviObject();
            ...
            foreach (KeyValuePair<string, string[]> item in mapPviVariables)
            {
                foreach (string varDefinition in item.Value)
                {
                    ...
                    if (spaceIndex > 0)
                    {
                        ...
                        BrPviPviObject pviObj = new BrPviPviObject();
        ...
        {
            connectionStatus = BrPviConnectionStatus.NotInitialized;
            int sleepCycle = 10;
            ...
            while (doLoop)
            {
                ...
                switch (currentStatus)
                {
                    ...
                    case WorkingThreadStatus.WaitConnectionToPviManager:
                        if (connectionStatus == BrPviConnectionStatus.Arranged)
                        {
                    ...
                    switch (eventInfo.nType)
                    {
                        case PviComManager.POBJ_EVENT_PVI_CONNECT:
                            if (eventInfo.ErrCode == 0) // Connection OK
                            {
                                ...
                                if (connectionStatus != BrPviConnectionStatus.Arranged)
                                {
                                    connectionStatus = BrPviConnectionStatus.Connected;
                                }
                            }
                        ...
                        case PviComManager.POBJ_EVENT_PVI_ARRANGE:
                            ...
                            {
                                lastConnectionErrorCode = 0;
                                connectionStatus = BrPviConnectionStatus.Arranged;
                            }
                            else // Connection error
                            {
                                lastConnectionErrorCode = (int)eventInfo.ErrCode;
        ...
        void AddPlcVariable(BrPviPviObject pviObject, string typeDefinition)
        {
            ...
            if (!String.IsNullOrWhiteSpace(accessType))
            {
                if (accessType.IndexOf("r") >= 0)
                {
                    ...
                    {
                        jobType = DriverCodeBase.Enumerators.LinkType.Input;
                    }
                }
                else if (accessType.IndexOf("w") >= 0)
                {
                    jobType = DriverCodeBase.Enumerators.LinkType.ExceptionOutput;
                }
            }
