...
namespace PubNub
{
    public enum PubNubCommJobStatus : byte
    {
        Idle,
    ...
    public class PubNubCommJob : CommJob
    {
                #region Constructors
        public PubNubCommJob(Station station, PubNubCommJobSettings settings)
            : base(station, settings)
        {
            _TagName = settings.TagName;
            _RestoreInitialQuality = false;
            CheckJobValid();
        }

        public PubNubCommJob(Station station, PubNubTag defTag)
            : base(station, defTag)
        {
            _TagName = defTag.PubNubDynSettings.TagName;
            _Status = PubNubCommJobStatus.Idle;
            _RestoreInitialQuality = false;

            CheckJobValid();
        }

        public PubNubCommJob(Station station)
            : base(station)
        {
        ...
        {
            if (_RestoreInitialQuality)
            {
                _RestoreInitialQuality = false;
                if(Status != PubNubCommJobStatus.PublishRequestPending)
                {
                    PubNubTag tag = null;
                    lock (lockListObject)
                    {
                    ...
                    if( (tag != null) && (tag.Value != null))
                    {
                        ...
                        {
                            if ((tag.Value.StatusCode != StatusCodes.Good) && (tag.Value.StatusCode != StatusCodes.Uncertain) && (tag.Value.StatusCode != StatusCodes.UncertainLastUsableValue))
                            {
                                SetUncertainLastUsableValueQuality();
                            }
                        }
                        else
                        {
                            if ((tag.Value.StatusCode != StatusCodes.Good) && (tag.Value.StatusCode != StatusCodes.Uncertain) && (tag.Value.StatusCode != StatusCodes.UncertainLastUsableValue) && (tag.Value.StatusCode != StatusCodes.BadWaitingForInitialData))
                            {
                                SetUncertainLastUsableValueQuality();
                            }
                        }
        ...
        public bool MustPublish()
        {
            ...
            if ((Type != LinkType.Input) &&
                ((GetTagListOnWritingCount() > 0) || (Type == LinkType.UnconditionalOutput)))
            {
                returnValue = true;
            }
        ...
        public override void SetJobData(object jobData, ref List<Tag> changed)
        {
            ...
            lock (lockListObject)
            {
                ...
                {
                    PubNubTag pubnubTag = (PubNubTag)TagsList[TagIndex];
                    if (pubnubTag.TagNode.DataType == Opc.Ua.DataTypes.Boolean)
                    {
                        if (isProtocolBool())
                        {
                            if (pubnubTag.SetTagValue(ref rec, (int)pubnubTag.ByteOffset))
                                changed.Add(pubnubTag);
                        }
                        else
                        {
                            uint ArraySize = pubnubTag.TagNode.ArrayDimension;
                            if (ArraySize == 0)
                                ArraySize = 1;
                            byte[] tmpData = new byte[ArraySize];
                            pubnubTag.setMemRW(rec, (int)pubnubTag.ByteOffset, (int)(sizeProtocolData * ArraySize));
                            for (int ArrayIndex = 0; ArrayIndex < ArraySize; ArrayIndex++)
                            {
                                bool valBool = pubnubTag.getBoolValueFromMemRW((int)pubnubTag.ByteOffset + sizeProtocolData * ArrayIndex, ElementNumber);
                                tmpData[ArrayIndex] = (byte)(valBool ? 1 : 0);
                            }

                            if (pubnubTag.SetTagValue(ref tmpData, 0))
                                changed.Add(pubnubTag);
                        }
                    }
                    else
                    {
                        if (isProtocolBool())
                        {
                            if (pubnubTag.SetTagValue(ref rec, (int)pubnubTag.ByteOffset, (uint)(ElementNumber > 0 ? 1 : 0)))
                                changed.Add(pubnubTag);
                        }
                        else
                        {
                            if (!ProtocolDataSizeBig())
                            {
                                ...
                                if (pubnubTag.SetTagValue(ref rec, (int)pubnubTag.ByteOffset, elemsize))
                                    changed.Add(pubnubTag);
                            }
                            else
                            {
                                UInt16 sizeTmpData = (UInt16)GetDataTypeByteSize((uint)pubnubTag.TagNode.DataType.Identifier);
                                uint ArraySize = pubnubTag.TagNode.ArrayDimension;
                                if (ArraySize == 0)
                                    ArraySize = 1;
                                ...
                                for (int ArrayIndex = 0; ArrayIndex < ArraySize; ArrayIndex++)
                                {
                                    Array.Copy(rec, pubnubTag.ByteOffset + sizeProtocolData * ArrayIndex + ElementNumber * sizeTmpData, tmpData, indexTmpData, sizeTmpData);
        ...
        public void ManagePublishErrorMessage(PubnubClientError pubnubError)
        {
            ...
            lastPublishErrorMessage = pubnubError.Message;
            Status = PubNubCommJobStatus.PublishError;
            ExecutedJobArgs eJob = new ExecutedJobArgs();
            eJob.ErrorCode = (DriverCodeBaseEx.Enumerators.DriverErrorCodes)PubNubErrorCodes.ErrorCodeErrorPublish;
            ClearTagListOnWriting();
            eJob.Job = this;
            PubNubChannel pnChannel = (PubNubChannel)Station.GetChannel();
            if(pnChannel != null)
            {
        ...
        public override uint SamplingInterval
        {
            get
            {
                ...
                PubNubChannel pnChannel = null;
                if (Station != null)
                {
                    pnChannel = (PubNubChannel)Station.GetChannel();
                }
                ...
                if (pnChannel == null)
                {
                    if (!MustPublish())
                    {
                        // The job has no requests to send
                        newSamplingInterval = CommJob.JOB_NOT_SCHEDULABLE;
                    }
                }
                else
                {
                    // Keep the job active if the subscription to the channel has not already completed
                    if ((pnChannel.subscriptionDone == true) && !MustPublish())
                    {
                        // The job has no requests to send
                        newSamplingInterval = CommJob.JOB_NOT_SCHEDULABLE;
                    }
        ...
        /// Status
        /// </summary>
        private PubNubCommJobStatus _Status;
        public PubNubCommJobStatus Status
        {
            get
            {
                return _Status;
        ...
        /// <summary>
        /// Restore Initial Quality after a general error
        /// </summary>
        private bool _RestoreInitialQuality;
        public bool RestoreInitialQuality
        {
            get
            {
                return _RestoreInitialQuality;
            }

            set
            {
                _RestoreInitialQuality = value;
            }
        }
