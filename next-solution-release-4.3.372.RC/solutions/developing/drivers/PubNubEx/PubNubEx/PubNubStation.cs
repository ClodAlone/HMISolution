...
namespace PubNub
{
    class PubNubStation : Station
    {
            ...
            if (ListWholeJob.Count > 0)
            {
                ...
                foreach (var job in listJob)
                {
...
#if DEBUG
                    {
                        ...
                        System.Diagnostics.Debug.WriteLine(String.Format("PubNub DBG - {0} - ManageSubscriptionError setting _RestoreInitialQuality to true",
                                                           currentTime));
                    }
#endif
                    pJob.SetBadConnectionQuality();
                    pJob.RestoreInitialQuality = true;
                }
                ...
                OnJobExecuted(this, e);
            }
        }

        public void ManageSubscriptionError(string errorMessage, DriverErrorCodes errorCode)
        {
            if (ListWholeJob.Count > 0)
            {
                var listJob = new List<CommJob>();
        ...
        public void ManageConnectionRestored()
        {
            ...
            {
                string currentTime = DateTime.Now.ToString("HH:mm:ss.fff");
                System.Diagnostics.Debug.WriteLine(String.Format("PubNub DBG - {0} - ManageConnectionRestored called",
                                                   currentTime));
            }
#endif
            if (ListWholeJob.Count > 0)
