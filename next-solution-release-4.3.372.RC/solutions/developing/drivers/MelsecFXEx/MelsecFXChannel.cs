    ...
    public class MelsecFXChannel : SerialChannel
    {
        ...
        protected override void GetCustomChildJobs(CommJob exjob, out List<CommJob> resultJobs, out bool write)
        {
            ...
            else
            {
                ...
                resultWrite = (exjobFX.GetTagListOnWritingCount() > 0 || exjobFX.Type == LinkType.UnconditionalOutput);
                if (resultWrite)
                {
            ...
            else
            {
                ...
                if (exjobFX.IsBitArray())
                {
                    ...
                    if (write)
                    {
                        ...
                        {
                            foreach (Tag tag in exjobFX.GetTagListOnWriting())
                    ...
                    else
                    {
                        if (e.ErrorCode == DriverErrorCodes.ErrorNoError)
                        {
                            foreach (var j in exjobFX.ChildJobsRead.Values)
                                subArraySum = Tag.ArraySumTagValue(j.TagsList[0], j.TagsList[0].GetValue(), subArraySum);

                            byte[] answer = (byte[])subArraySum;

                            //e.Values = subArraySum;

                            List<object> ChangedTags = new List<object>();
                            MelsecFXCommJob FXJ = exjobFX as MelsecFXCommJob;
                            if (FXJ.ParseData(answer, ref ChangedTags))
                            {
                                Tag tag = exjob.TagsList[0];
                                foreach (var changedTag in ChangedTags)
                                {
                                    var cTag = changedTag as Tag;
                                    if (cTag != null)
