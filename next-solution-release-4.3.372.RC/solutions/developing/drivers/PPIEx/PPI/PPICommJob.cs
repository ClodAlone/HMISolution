...
namespace PPI
{
    public class PPICommJob : CommJob
    {
        #region Constructors
        public PPICommJob(Station station, PPICommJobSettings settings)
        ...
        public PPICommJob(Station station, PPITag defTag)
            : base(station, defTag)
        {

            _Area = defTag.PPIDynSettings.Area;
            _Format = defTag.PPIDynSettings.Format;
            _Trans = defTag.PPIDynSettings.Trans;
            _Offset = defTag.PPIDynSettings.Offset;
            _Length = defTag.PPIDynSettings.Length;
            _Bit = defTag.PPIDynSettings.Bit;
            _German = defTag.PPIDynSettings.German;
            if (TotalJobSize == 0)
                TotalJobSize = (uint)defTag.PPIDynSettings.Length;
            ExecuteTask = false;
        ...
        public PPICommJob(Station station)
            : base(station)
        {
            ExecuteTask = false;
            CheckJobValid();
        ...
        public int GetNumOfElements()
        {
            if (PartialElementEnd == PartialElementStart)
                ...
                return Length - PartialElementStart;
            else
                return PartialElementEnd - PartialElementStart;
        }
        ...
        public uint GetTagWriteData(byte[] bufferIn, ref byte[] buffer, int elementsize, int index = 0)
        {
            ...
            {
                uint length = (uint)(GetNumOfElements() * elementsize);
                if (Format == Step7Format.frmBit)
                {
                    ...
                    Array.Copy(bufferIn, PartialElementStart * elementsize, buffer, index, length);
                return length;
            }
        ...
        private void CheckJobValid()
        {
            ...
            PPIProtocol.SetS7JobLength(this);
            IsValid = true;
        ...
        {
            switch (PPIProtocol.DataType(Format))
            {
        ...
        {
            return PPIProtocol.MAX_DATA_BYTES;
        }
        public override JobAggregationType TestAggregateJob(CommJob candJob, out uint ExtraBytes)
        {
            ...
            PPICommJob testJob = candJob as PPICommJob;
            if (testJob == null)
                return JobAggregationType.JobAggregImpossible;
            ...
            else if ((Format != Step7Format.frmByte) &&
                            (Area != Step7Area.aT) &&
                            (Area != Step7Area.aC) &&
                ...
                ))
            {
                    ...
                    if ((Area == Step7Area.aTIEC && nNewJobSize > 88) || nNewJobSize > 148)
                        return JobAggregationType.JobAggregImpossible;
            }
	        else if(Area == Step7Area.aH)
            {
            ...
            if (Format == Step7Format.frmBit)
            {
                if (nNewJobSize > PPIProtocol.MAX_ELEMENT_BOOL)
                {
                    return JobAggregationType.JobAggregImpossible;
                }
            }
        ...
        public override bool AggregateJob(CommJob candJob, JobAggregationType AggType, uint ExtraBytes)
        {
             ...
             */
            PPICommJob mJ = candJob as PPICommJob;
            switch (AggType)
            {
                case JobAggregationType.JobAggregFits:
                    foreach (var tag in candJob.TagsList)
                    {
                        var t = new PPITag(tag.TagNode, tag.ByteOffset, 0);
                ...
                case JobAggregationType.JobAggregBackward:
                    ...
                    TotalJobSize += ExtraBytes;
                    Offset = ((PPICommJob)candJob).Offset;
                    break;
                default:
                    return false;
            }
        ...
        public override void GetJobData(ref object jobData)
        {
            ...
            do
            {
                ...
                if (Format == Step7Format.frmBit)
                {
                    uint candSize = cand.Size;
                    if (ElementNumber > 0 || ProtocolDataSizeBig())
                    {
                        if (cand.TagNode.ArrayDimension == 0)
                            candSize = GetProtocolDataByteSize();
                        else
                            candSize = GetProtocolDataByteSize() * cand.TagNode.ArrayDimension;
                    }
                    if (candSize + len >= PPIProtocol.MAX_ELEMENT_BOOL)
                    {
                        PartialElementEnd = (ushort)(PPIProtocol.MAX_ELEMENT_BOOL + PartialElementStart - len);
                        if (PartialElementEnd >= candSize)
                        {
                            PartialElementEnd = 0;
                        }
                    }
                ...
                lock (lockListObject)
                {
                    ...
                    {
                        byte[] tmpBuffer = new byte[TotalJobSize];
                        tsize = cand.GetTagBuffer(ref tmpBuffer, false, 0, (ElementNumber > 0 && !ProtocolDataSizeBig() ? GetProtocolDataByteSize() : 0));
                        GetTagWriteData(tmpBuffer, ref jobdata, (int)((GetProtocolDataBitSize() + 7) / 8), (int)len);
                    }
                }
                ...
                if (Format != Step7Format.frmBit)
                {
                    ...
                    PPIProtocol.PrepareDataWrite(jobdata, ref outData, Format, Trans, 0, tsize);
                    len += tsize;
                }
                else
                {
                    //byte[] bufTmp = new byte[((tsize + 7) / 8)];
                    //Array.Copy(jobdata, len, bufTmp, 0, bufTmp.Length);
                    //len += (uint)bufTmp.Length;
                    //outData.AddRange(bufTmp);
