...
namespace BrPvi
{
    public class BrPviCommJob : CommJob
    {
        public enum BrPviState : int
        {
            ...
            JobUnSubScribe,                     // job's have to be unsubscribe --> not active
        }
            ...
            : base(station, settings)
        {
            ...
            _BrPviUnSubScribeRequested = UnSubScribeState.None;
            _BrPviLastUnSubScribeExecutionTime = DateTime.MinValue;
            CheckJobValid();
        }
        ...
        public void DeletePviObjects()
        {
            ((BrPviStation)Station).DeleteJobPviObjects(this);
            pviObjectState = BrPviCommJob.PviObjectState.NotInitialized;
            _BrPviUnSubScribeRequested = BrPviCommJob.UnSubScribeState.Performed;
        }
        ...
        public override bool SetInUse(NodeId node, bool inUse, double samplinginterval = -1)
        {
            bool inUseBefore = this.InUse;

            bool ret = base.SetInUse(node, inUse, samplinginterval);
            // manage unsuscribe "status" only if requested
            if (((BrPviStation)this.Station).BrPviRemoveDisabledItemAfterSecs > 0)
            {
                // need to be unsubscribed ?
                if (inUseBefore && !this.InUse)
                {
                    if (pviObjectState == PviObjectState.Activated)                    
                        ((BrPviChannel)this.Station.GetChannel()).AddJobToUnSubScribe(this);                    
                }
            }
        ...
        public override uint SamplingInterval
        {
            get
            {
                if (BrPviUnSubScribeRequested == UnSubScribeState.Requested)
        ...
        private UnSubScribeState _BrPviUnSubScribeRequested = UnSubScribeState.None;
        public UnSubScribeState BrPviUnSubScribeRequested
        { 
            get { return _BrPviUnSubScribeRequested; }
            set { _BrPviUnSubScribeRequested = value; }
        }

        private DateTime _BrPviLastUnSubScribeExecutionTime = DateTime.MinValue;
        public DateTime BrPviLastUnSubScribeExecutionTime
        {
            get { return _BrPviLastUnSubScribeExecutionTime; }
            set { _BrPviLastUnSubScribeExecutionTime = value; }
        }
        #endregion
    }
}
