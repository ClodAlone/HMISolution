...
namespace MpiPcAdapter.UI
{
    ...
    {
        ImportDataModelMpiPcAdapter importDataModel;
        bool alreadyLoaded = false;        
        //List<UFUAFolder> folders;
        int m_lGlobalID = 0;        
        ...
        public ImportTagsEditorTree()
        {
            ...
            Loaded += (o, e) =>
            {
                ...
                baseImportTree.SetFileFilter("awl files|*.awl|sdf files|*.sdf");                
        ...
        public void ImportSelectedTags()
                    ...
                    if (single != null && (notToBeImported.IndexOf(single) < 0))
                     {
                         ...
                         if (single != null)
                         {
                             ...
                             MpiPcAdapterDynTagSettings sp = new MpiPcAdapterDynTagSettings();
                             ...
                             ImportTag tagtoimport = new ImportTag()
                             {
                                 ...
                                 DataType = ((ImportDataMpiPcAdapter)single).Type,
                 ...
                 if (elem.Children.Count != 0)
                 {
                     ...
                     foreach (var t in elem.Children)
                     {
                         ...
                         if (el != null)
                         {
                             ...
                             if (((ImportDataMpiPcAdapter)el).ImportType == ImportTypes.Struct)
                             {
                             ...
                             a.Name = el.Name;
                             a.DataType = ((ImportDataMpiPcAdapter)el).Type;
         ...
         {             
             importDataModel = new ImportDataModelMpiPcAdapter(readStationName, readAddDBnumber);
         ...
         private void ParseVariableAWL(ref System.IO.StreamReader readFile)
         {
             ...
             while ((line = readFile.ReadLine()) != null)
             {
                 ...
                 if (bIsStruct)
                 {
                     ...
                     {
                         bool endOfStruct = false;
                         AWLGetStructure(ref readFile, szVarName, szDescription, ref nCurrentAddress,
                             ref nDBNumber, ref nBitNumber, ref bIncrementAddressIfNotBit, ref nGlobalLevel, ref endOfStruct);
             ...
             int lParentID = -1, ImportData inRootItem = null)
         {
             ...
             if (GetMoviconTypeId(szVarType, ref nConversion, ref nVarSize, ref nType))
             {
                 ...
                 if (nDBNumber > 0)
                     ((ImportDataMpiPcAdapter)IVar).PreName = string.Format("DB{0}", nDBNumber);
                 IVar.Name = szNamePrefix + szVarName;
                ((ImportDataMpiPcAdapter)IVar).Type = nType;
                 IVar.szType = szVarType;
                 if (szVarType == "STRING")
                     nVarSize = nStringLen + 2;
                ((ImportDataMpiPcAdapter)IVar).Size = nVarSize;
                ((ImportDataMpiPcAdapter)IVar).ElemType = DataType.Boolean;
                 IVar.Address = szAddress;
                 IVar.Description = szDescription;
                 IVar.Id = m_lGlobalID++;
                 IVar.ArrayDimension = 0;
                ((ImportDataMpiPcAdapter)IVar).ImportType = ImportTypes.Standard;
        ...
        private void ParseDataTypeAWL(ref System.IO.StreamReader readFile)
             ...
             String szVarType, int parentId = -1, ImportData inRootItem = null)
         {
            ...
            ((ImportDataMpiPcAdapter)rootItem).PreName = string.Format("DB{0}", nDBNumber);
             rootItem.Name = szVarName;
             rootItem.szType = szVarType;
            ((ImportDataMpiPcAdapter)rootItem).Type = DataType.Boolean;
             rootItem.Description = description;
            ((ImportDataMpiPcAdapter)rootItem).Size = nFieldSize;
             ...
             foreach (string itemStructValue in m_mapUDT[szVarType])
             {
                 ...
                 switch (nFieldType)
                 {
                     ...
                     case ImportTypes.Standard:
                         if (nMovType != "String" && !endOfStruct)
                         {
                     ...
                     case ImportTypes.Array:
                         ...
                         if (elementType == ImportTypes.Standard && !endOfStruct)
                         {
                             AddStructRoot(inRootItem, rootItem, ref isFirst, szAddress, szVarType);
                             AddArray(nDBNumber, ref nCurrentAddress, ref nBitNumber, ref bIncrementAddressIfNotBit,
                     ...
                     case ImportTypes.Struct:
                         if (!endOfStruct)
                         {
                             ...
                             AddStruct(nDBNumber, ref nCurrentAddress, ref nBitNumber, ref bIncrementAddressIfNotBit,
                             szStructNamePrefix, szFieldName, elemDescription, szFieldType);
         ...
         private void AddStructRoot(ImportData inRootItem, ImportData rootItem, ref bool isFirst, string szAddress, string szVarType)
         {
             if (isFirst)
             {
                ...
                ((ImportDataMpiPcAdapter)rootItem).ImportType = ImportTypes.Struct;
                 rootItem.szType = szVarType;
         ...
         bool AWLGetStructure(ref System.IO.StreamReader awlFile, string structName, string description,
             ...
             ref uint nGlobalLevel, ref bool endOfStruct, string treeName = "")
         {
             ...
             uint nVarSize = 0;
             if (nDBNumber > 0)
                 ((ImportDataMpiPcAdapter)structure).PreName = string.Format("DB{0}", nDBNumber);//https://support/Products/default.asp?7797
             structure.Name = structName;
            ((ImportDataMpiPcAdapter)structure).Type = DataType.Boolean;

             structure.szType = "STRUCT";

            ((ImportDataMpiPcAdapter)structure).Size = nVarSize;
            ((ImportDataMpiPcAdapter)structure).ElemType = DataType.Boolean;
             structure.Address = string.Empty;
             structure.Description = description;
             structure.Id = m_lGlobalID++;
            ((ImportDataMpiPcAdapter)structure).ImportType = ImportTypes.Struct;
                     ...
                     if (szFileRecord.Length != 0)
                     {
                         ...
                         if (bIsStruct)
                         {
                             ...
                             else
                             {
                                 AWLGetStructure(ref awlFile, szVarName, szDescription, ref nCurrentAddress,
                                     ...
                                     ref endOfStruct, treeName == "" ? structName : treeName + "_" + structName);
                             }
                             continue;
                         }
                         if (szVarName.Length == 0 || szVarType.Length == 0)
                         ...
                         else if (!bIsArray)
                         {
                             ...
                             if (GetMoviconTypeId(szVarType, ref nConv, ref nSize, ref nType))
                             {
                                 var IVar = importDataModel.addImportData();
                                 if (nDBNumber > 0)
                                    ((ImportDataMpiPcAdapter)IVar).PreName = string.Format("DB{0}", nDBNumber);//https://support/Products/default.asp?7797
                                 IVar.Name = szVarName;
                                ((ImportDataMpiPcAdapter)IVar).Type = nType;
                                 ...
                                 if(endOfStruct)
                                 {
                                     IVar.Name = string.Empty;
                                     if (treeName == "")
                                         IVar.Name = structName + "_" + szVarName;
                                     else
                                         IVar.Name = treeName + "_" + structName + "_" + szVarName;
                                 }
                                 ((ImportDataMpiPcAdapter)IVar).Size = nVarSize;
                                ((ImportDataMpiPcAdapter)IVar).ElemType = DataType.Boolean;
                                 IVar.Address = szAddress;
                                ...
                                ((ImportDataMpiPcAdapter)IVar).ImportType = ImportTypes.Standard;
                                 ...
                                 if (firstelement && !endOfStruct)
                                 {
                                     firstelement = false;
                                     structure.Address = szAddress;
                                     AddTreeItem(structure);
                                 }
                                 AddTreeItem(IVar,endOfStruct? null : structure);
                             }
...
//**********************************************************************
             ...
             int lParentID = -1, ImportData inRootItem = null)
         {
             ...
             for (i = 0; i < nArrayDim; i++)
             {
                 ...
                 if (nDBNumber > 0)
                    ((ImportDataMpiPcAdapter)f).PreName = string.Format("DB{0}", nDBNumber);//https://support/Products/default.asp?7797
                 f.Name = szFieldLine;

                 if (szVarType == "STRING")
                     nVarSize = nStringLen + 2;

                ((ImportDataMpiPcAdapter)f).Size = nVarSize;
                 f.szType = szVarType;
                ((ImportDataMpiPcAdapter)f).Type = nElemType;
                ...
                ((ImportDataMpiPcAdapter)f).ImportType = ImportTypes.Standard;
             ...
             int lParentID = -1, ImportData inRootItem = null)
         {
            ...
            ((ImportDataMpiPcAdapter)IVar).ElemType = nType;

            // FOGBUGZ 11412 and 11413
            //IVar.Type = DataType.Boolean;
            ((ImportDataMpiPcAdapter)IVar).Type = nType;

            ((ImportDataMpiPcAdapter)IVar).Size = nArrayDim * nVarSize;
             ...
             IVar.parentId = lParentID;
            ((ImportDataMpiPcAdapter)IVar).ImportType = ImportTypes.Array;
             ...
             for (i = 0; i < nArrayDim; i++)
             {
                ...
                ((ImportDataMpiPcAdapter)f).Size = nVarSize;
                 f.szType = szVarType;
                ((ImportDataMpiPcAdapter)f).Type = ((ImportDataMpiPcAdapter)IVar).ElemType;
                 f.Address = GetAddress(nVarType, nDBNumber, ref nCurrentAddress, ref nBitNumber,
                     ref bIncrementAddressIfNotBit, nStringLen, ref szVarType);
         ...
         private void DisplayImportFileSdf(string file)
         {
             try
             {
                 ...
                 while ((line = readFile.ReadLine()) != null)
                 {
                     ...
                     int length = 0;
                     bool german = false;
                     if (!MpiPcAdapterDynTagSettings.StaticParseAddress(strAddr, ref area, ref dbnumber,
                         ref format, ref trans, ref offset, ref bit, ref length, ref german))
                         continue;
                     ...
                     if (dbnumber != -1)
                        ((ImportDataMpiPcAdapter)IVar).PreName = string.Format("DB{0}", dbnumber);//https://support/Products/default.asp?7797
                     IVar.Name = strSymb;
                    ((ImportDataMpiPcAdapter)IVar).Type = nType;
                    ((ImportDataMpiPcAdapter)IVar).Size = nVarSize;
                     IVar.szType = strType;
    ...
    public class ImportDataMpiPcAdapter : ImportData, IDisposable
    {
        public ImportDataMpiPcAdapter(ImportDataModel inDataModel)
            : base(inDataModel)
        {
            dataModelMpiPcAdapter = inDataModel as ImportDataModelMpiPcAdapter;
        }
        private ImportDataModelMpiPcAdapter dataModelMpiPcAdapter { get; set; }
        ...
        public override string Name
        {
            get 
            {
                ...
                if (Parent == null)
                {
                    if (dataModelMpiPcAdapter.getAddDBnumber() && (!string.IsNullOrEmpty(PreName)) && PreName != null)
                        outName = PreName + "_" + outName;
                    string StationName = dataModelMpiPcAdapter.getStationName();
                    if (StationName != "_")
        ...
        public void Dispose()
        {
            ...
            if (Children.Count() > 0)
            {
                ...
                {
                    var sls7 = el as ImportDataMpiPcAdapter;
                    if (sls7 != null)
                    {
                        sls7.Dispose();
                    }
    ...
    public class ImportDataModelMpiPcAdapter : ImportDataModel, IDisposable
    {
        public GetAddDBnumber getAddDBnumber { get; private set; }

        public ImportDataModelMpiPcAdapter(GetStationName inGetStationName, GetAddDBnumber inGetAddDBnumber) :
        ...
        {
            ImportDataMpiPcAdapter importData = new ImportDataMpiPcAdapter(this);
            return importData;
        }
        #region IDisposable Members
        ...
        public void Dispose()
        {
            foreach (ImportData el in Children)
            {
                ImportDataMpiPcAdapter els7 = el as ImportDataMpiPcAdapter;
                if (els7 != null)
                {
                    els7.Dispose();
                }
