...
namespace Simotion
{
    public class SimotionCommJob : CommJob
    {
        #region Constructors
        public SimotionCommJob(Station station, SimotionCommJobSettings settings)
        ...
        public SimotionCommJob(Station station, SimotionTag defTag)
            : base(station, defTag)
        {
            _StringLength = 0;
            _S7DataFormat = defTag.SimotionDynTagSettings.S7DataFormat;
            if (_S7DataFormat == S7DataFormats.String/* || _S7DataFormat == S7DataFormats.WString*/)
                SetStringTagDataFormat(_S7DataFormat);
            _StartAddress = defTag.SimotionDynTagSettings.StartAddress;
            //_Trans = defTag.SimotionDynTagSettings.Trans;
            _Trans = Step7WordTrans.wtT;
        ...
        public SimotionCommJob(Station station)
            : base(station)
        {
            CheckJobValid();
        ...
        public static S7DataFormats GetSimotionType(string Identifier)
        {
            switch (Identifier)
            {
                case "1":
        ...
        public override uint GetMaxJobSize()
        {
            ...
            return SimotionProtocol.MAX_DATA_BYTES;
        }

        public override uint getProtocolDataType()
        {
            switch (SimotionProtocol.DataType(S7DataFormat))
            {
        ...
        public override void GetJobData(ref object jobData)
        {
            ...
            {                               
                ...
                if (cand.TagNode.ArrayDimension == 0)
                    ...
                    nData = (UInt16)(cand.Size * cand.TagNode.ArrayDimension);
            }
            lock (lockListObject)
            {
                ...
                //{
                //    ((SimotionTag)cand).GetTagBufferWString(ref jobData);
                //    return;
                //}
                //else
                //{
        ...
        public override void SetJobData(object jobData, ref List<Tag> changed)
        {
            ...
            lock (lockListObject)
            {
                ...
                if (TagsList[0].TagNode.DataType == Opc.Ua.DataTypes.Boolean)
                {
                    if (isProtocolBool())
                    {
                        if (((SimotionTag)(TagsList[0])).SetTagValue(ref rec, (int)TagsList[0].ByteOffset, UpdateTimeStamp))
                    ...
                    else
                    {
                        ...
                        if (((SimotionTag)(TagsList[0])).SetTagValue(ref tmpData, 0, UpdateTimeStamp))
                            changed.Add(TagsList[0]);
                    }
                }
                else
                {
                    ...
                    {
                        if (((SimotionTag)(TagsList[0])).SetTagValue(ref rec, (int)TagsList[0].ByteOffset, UpdateTimeStamp, (uint)(ElementNumber > 0 ? 1 : 0)))
                            changed.Add(TagsList[0]);
                    }
                    else
                    {
                        if (!ProtocolDataSizeBig())
                        {
                            ...
                            //            offset = nCicli + 1;
                            //        }
                            //        if (((SimotionTag)(TagsList[nCicli])).SetTagValue(ref rec, offset, UpdateTimeStamp, elemsize))
                            //        {
                            ...
                            //}
                            //else 
                            if (((SimotionTag)(TagsList[0])).SetTagValue(ref rec, (int)TagsList[0].ByteOffset, UpdateTimeStamp, elemsize))
                            {
                                changed.Add(TagsList[0]);
                            }
                            //else
        ...
        //    result = AGL4.Simotion_CreateAccessByPath(pRootHandle, _StartAddress, ref m_stSymbolicRW.AccessHandle, ref error_pos);
        ...
        public bool LoadAccessHandleAndSize(IntPtr pRootHandle, string StationName, string addres, ref string szError)
        {
            ...
            result = AGL4.Simotion_CreateAccessByPath(pRootHandle, addres, ref m_stSymbolicRW.AccessHandle, ref error_pos);
            if (result != AGL4.AGL40_SUCCESS)
            {                
                szError = string.Format("{0} {1} {2}", StationName, _StartAddress, string.Format(Properties.Resources.ErrorFromDllAGLink, result));
                return (false);
            ...
            if (_S7DataFormat == S7DataFormats.String/* || _S7DataFormat == S7DataFormats.WString*/)
            {
                StringLength = (uint)m_stSymbolicRW.BufferLen;
                TagsList[0].Size = _StringLength;
                if (TagsList[0].Size == 0)
                    TagsList[0].Size = 80;
            }            
            return (true);
        }
        ...
        public void SetStringTagDataFormat(S7DataFormats s7StringDataFormat)
        {
            ...
            {
                ((SimotionTag)tag).S7StringDataFormat = s7StringDataFormat;
