...
namespace PPI
{
    ...
    class PPIChannel : SerialChannel, IDisposable
    {
        ...
        void SetDelays(int CommPortBaudRate)
        {
            ...
            MaxTsyn = (int)(1 + 1000 * (fTbit * 33)); // Tempo espresso in msec
            MaxTsdi = (int)(1 + 1000 * (fTbit * 33));
            MaxTsdr = (int)(1 + 1000 * (fTbit * 40));
            MaxTrdy = (int)(1 + 1000 * (fTbit * 20));
            MaxTqui = (int)(1 + 1000 * (fTbit * 5));
            MaxTset = (int)(1 + 1000 * (fTbit * 2));
            MaxTsm = (int)(1 + 1000 * (fTbit * 2) + 2 * MaxTset + MaxTqui);
	        MinTid1	= 1 + MaxTsyn + MaxTsm;
            MinTtd = (int)(1 + 1000 * (fTbit * 17));
            MaxTslot = (int)(1 + 1000 * (fTbit * 100));
        }
        ...
        bool PreparePDU(PPIStation s, PPICommJob j, out byte[] outBuf, ref ushort BytesToRequest, uint elementsize)
        {
            ...
            byte NumOfPointers;
            if (!j.IsRead && j.Format == Step7Format.frmBit)
                NumOfPointers = (byte)j.GetNumOfElements();
            else
                NumOfPointers = 0x01;
        ...
        protected override void WorkingThread(object data)
        {
            ...
            while (true)
            {
                PPICommJob nextjob = null;
                if (ListJobPending.Count == 0 || MultiPointProtocol)
                {
                    ScheduleListJob();
                    if (SynchroJob != null)
                        nextjob = SynchroJob as PPICommJob;
                    else
                        nextjob = GetNextPendingJob() as PPICommJob;
                    if (nextjob != null)
                        ListJobPending.Add(nextjob);
                }
                else
                {
                    PPICommJob lastJob = ListJobPending[0] as PPICommJob;
        ...
        public override void ExecuteJob(CommJob job)
        {
            ...
            if (myJob.Type == DriverCodeBase.Enumerators.LinkType.UnconditionalOutput)
            {
                lock (myJob.retLockList())
                {
                    if (myJob.TagsListToWrite.Count == 0)
                        myJob.TagsListToWrite.AddRange(myJob.TagsList);
                }
            }
            base.ExecuteJob(job);
            Layer2(s, myJob);
