...
namespace ModBus
{
    ...
    public class ModbusProtocol
    {
        ...
        public const string TEST_COMM_DYNAMIC_SETTINGS = "ModBus.Station={0}|LinkType=0|FC=2|SA=0|File=0|BCast=False";
        #region methods override
        public uint GetFrameLength(ModbusCommJob job)
        {
            if (job.ReadRequest())
        ...
        public uint PrepareRequest(ModbusCommJob job, ref byte[] buffer)
        {
            
            if (job == null || (job.Station as ModbusStation == null))
                return 0;
            uint WriteCh = 0;
            ...
            if (job.ReadRequest())
            {
                ...
                buffer[WriteCh++] = (byte)((ModbusStation)job.Station).StationID;
        ...
        public static bool ParseData(byte [] receivebuffer, ref ModbusCommJob job, ref List<object> items)
        {
             ...
             */
            if (receivebuffer[0] != (byte)((ModbusStation)job.Station).StationID)
            { 
                //error. wrong station ID!!!
                items.Add((int)ModbusErrorCodes.ErrorStationID);
                return false;
            }
            ...
            { 
                ...
                items.Add((int)ModbusErrorCodes.ErrorFunctionCode);
                return false;
            }
               ...
               (receivebuffer[2] != job.TotalJobSize))
            {
                 ...
                 * with job dimension.
                 */
                items.Add((int)ModbusErrorCodes.ErrorWrongSize);
                return false;
            }
            ...
            if ((receivebuffer[1] == 1 || receivebuffer[1] == 2))
            {
                ...
                if (receivebuffer[2] != size)
                {
                    items.Add((int)ModbusErrorCodes.ErrorWrongSize);
                    if (areArguments)
                        items[0] = (int)ModbusErrorCodes.ErrorWrongSize;
                    return false;
                }
            }
