    ...
    public partial class ImportTagsEditorTree : UserControl, IDisposable
    {
        ...
        public ImportTagsEditorTree()
        {
            ...
            Loaded += (o, e) =>
            {
                ...
                ChangeLimitMaxProtocol();
            };

        }
        /// <summary>
        /// The CmbStationChanged it is called up when you change stations in the comboboxn 
        /// </summary>
        ...
        /// <summary>
        /// The ChangeLimitMaxProtocol Change the protocol limits
        /// </summary>
        private void ChangeLimitMaxProtocol()
        {
            if (!baseImportTree.HasStation(baseImportTree.ReadStationName()))
                return;

            MelsecQEthStation station = new MelsecQEthStation(new MelsecQEthDriver(conn), (MelsecQEthStationSettings)baseImportTree.GetStationSettings(baseImportTree.ReadStationName()));
            if ((station.PlcType == MelsecQEthProtocol.PlcTypes.CPU_MODEL_L_Q) ||
                (station.PlcType == MelsecQEthProtocol.PlcTypes.CPU_MODEL_iQR))
                maxSize = MelsecQEthProtocol.MAXBYTE_SIZE_R_L_Q;
            else
                maxSize = MelsecQEthProtocol.MAXBYTE_SIZE;
        }
                            ...
                            {                     
                                ...
                                switch (((ImportDataMelsecQ)single).TagTypeInt)
                                {
                                    ...
                                    case MelsecQEthFileImportParser.ImportDataTypeStruct:
                                        ImportPrototype proto = addPrototype(protoMap, single, progressBar);
                                        if (proto == null)
                                            continue;                                
                                        break;
        ...
        private ImportPrototype addPrototype(Dictionary<string, ImportPrototype> protoMap, ImportData elem, ImportDataProgressBar progressBar)
        {
            progressBar.ThrowIfCancellationRequested();
            ...
            if (!protoMap.ContainsKey(protoname))
            {
                if (elem.Children.Count != 0)
                {
                    ...
                    foreach (var t in elem.Children)
                    {
                        ...
                        if (((ImportDataMelsecQ)t).TagTypeInt == MelsecQEthFileImportParser.ImportDataTypeStruct)
                        {
                            ImportPrototype protoElem = addPrototype(protoMap, t, progressBar);
                            if (protoElem == null)
                                continue;
