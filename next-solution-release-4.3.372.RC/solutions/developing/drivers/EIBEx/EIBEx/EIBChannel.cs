    ...
    class EIBChannel : Channel
    {
            ...
            SmartThreadPool.QueueWorkItem(() =>
            {
                ...
                catch (Exception ex)
                {
                    try
                    {
                        ...
                        executedJob.SetStateCommandVariableBit(false, (ushort)EIBConditionalVariableBits.BitForceRead);
                        executedJob.SetStateCommandVariableBit(true, (ushort)EIBConditionalVariableBits.BitReadError);
        ...
        {                        
            SmartThreadPool.QueueWorkItem(() =>
            {
                ...
                try
                {
                    executedJob.SetStateCommandVariableBit(false, (ushort)EIBConditionalVariableBits.BitForceWrite);
                    executedJob.SetStateCommandVariableBit(LastErrorCode != DriverErrorCodes.ErrorNoError, (ushort)EIBConditionalVariableBits.BitWriteError);
                }
                catch
                {
                }
            ...
            foreach (var eibJob in listJobs)
            {
                ...
                {                    
                    if (eibJob.ErrorCode == (EIB_ERROR_CODES)DriverErrorCodes.ErrorNoError)
                        eibJob.SetStateCommandVariableBit(true, (ushort)EIBConditionalVariableBits.BitNewData);
        ...
        public override bool Startup()
        {
            if (!bChannelStarted)
            {
                ...
                CyclicCheckStart((int)Properties.Settings.Default.ConnectionRepeatDelay, System.Threading.Timeout.Infinite);
            }
