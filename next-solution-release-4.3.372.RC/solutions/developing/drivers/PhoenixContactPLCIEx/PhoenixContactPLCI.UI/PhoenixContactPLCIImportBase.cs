...
namespace PhoenixContactPLCI.UI
{
    public class PhoenixContactPLCIImportBase : IDisposable
    {
        ...
        private void AddVariableStandard(MetaElement refItem, ref List<ImportedVariable> listImportVariables)
        {
            ...
            //Remove the sequence of characters "[{X}]" that are inserted in the path to distinguish that the path is an array type data.
            RemoveArrayMarker(ref pathname);
            ...
            bool bIsAddUdt = AddUDTVariable(var.VariableName, var);
            if (!bIsAddUdt)
            {
        ...
        private bool IsArrayOfStructure(MetaElement refItem)
        {
            return (PhoenixContactPLCIProtocol.GetVarType(refItem.GetArrayTypeName()) == PhoenixContactPLCIProtocol.VarType.UNKNOWN);
        }
        ...
        private void AddVariableArray(MetaElement refItem, ref List<ImportedVariable> listImportVariables)
        {
            ...
            else
            {
                ...
                listImportVariables[pr.OffsetOfList - 1].VarTypeOfStruct = refItem.GetArrayTypeName(); // refItem.GetTypeName();
        ...
        private void RemoveArrayMarker(ref string pathname)
        {
            int start = pathname.IndexOf("[{");
            while (start >= 0)
            {
                int stop = pathname.IndexOf("}]");
                if (stop > start)
                {
                    pathname = pathname.Substring(0, start) + pathname.Substring(stop + 2, pathname.Length - stop - 2);
                    start = pathname.IndexOf("[{");
                }
                else
                {
                    break;
        ...
        private bool AddUDTVariable(string name, ImportedVariable var)
        {
            bool bRet = false;
            ...
            if (MapStructWitRifLisVar.ContainsKey(strWithoutMember))
            {
                MapStructWitRifLisVar[strWithoutMember].NumberOfElement += 1;

                //I extract the name of the structure from the address without the name of the member
                string nameStruct = MapStructWitRifLisVar[strWithoutMember].structType;
