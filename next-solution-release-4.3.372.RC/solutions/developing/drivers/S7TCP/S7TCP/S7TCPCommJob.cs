    ...
    public class S7TCPCommJob : CommJob
    {
        ...
        public bool IsBigArray()
        {
            ...
            if (TagsList[0].TagNode.ArrayDimension != 0)
            {
                switch (val)
                {
                    case (uint)Opc.Ua.DataTypes.Boolean:
                        {
                            ...
                            if (byteSize > S7Protocol.MAX_DATA_BYTES)
                            {
                                ret = true;
                            }
                        }
                    ...
                    case (uint)Opc.Ua.DataTypes.String:
                        break;
                    default:
                        if (TagsList[0].Size > S7Protocol.MAX_DATA_BYTES)
                        {
                            ret = true;
                        }
                        break;
            ...
            else if (val == (uint)Opc.Ua.DataTypes.String)
            {
                ...
                if (sizeToBeChecked > S7Protocol.MAX_DATA_BYTES)
                {
                    ret = true;
                }
            }
        ...
        public override uint GetMaxJobSize()
        {
            ...
            return S7Protocol.MAX_DATA_BYTES;
        }
        ...
        public bool LimitatedGetJobData(ref object jobData, long RequestSize, out S7TCPTag firstTag)
        {
            ...
            do
            {
                ...
                {
                    if ((jobdata.Length - ExecutionOffsetWrite) > (S7Protocol.MAX_MPI_TLG_LEN - currentRequestSize))
                    {
                        byte[] subArray = new byte[(S7Protocol.MAX_MPI_TLG_LEN - currentRequestSize)];
                        Array.Copy(jobdata, ExecutionOffsetWrite, subArray, 0, (S7Protocol.MAX_MPI_TLG_LEN - currentRequestSize));
                ...
                if (jobdata.Length + currentRequestSize > S7Protocol.MAX_MPI_TLG_LEN)
                {
                    rt = false;
                    break;
                }
        ...
        public uint GetReadResponseLength()
        {
            ...
            if (IsBigArray())
            {
                ...
                if (remain > S7Protocol.MAX_DATA_BYTES)
                {
                    nBytes = S7Protocol.MAX_DATA_BYTES;
                }
                else
                {
                    nBytes = remain;
        ...
        public uint GetWriteRequestLength(ref List<Tag> listToWrite, ref List<Tag> listToWriteLater)
        {
            ...
            if (Area != Step7Area.aT && Trans != Step7WordTrans.wtT)
            {
                ...
                lock (lockListObject)
                {
                    actuallist.AddRange(TagsListToWrite);
                    TagsListToWrite.Clear();
                ...
                if (isBigArrayFlag)
                {
                    ...
                    if (remain > S7Protocol.MAX_DATA_BYTES)
                    {
                        size = S7Protocol.MAX_DATA_BYTES;
                    }
                    else
                    {
                        size = remain;
        ...
        public int GetDataLength()
        {
            ...
            if (IsBigArray())
            {
                ...
                if (remain > S7Protocol.MAX_DATA_BYTES)
                {
                    remain = S7Protocol.MAX_DATA_BYTES;
                }
                return (int)remain;
            }
            else
