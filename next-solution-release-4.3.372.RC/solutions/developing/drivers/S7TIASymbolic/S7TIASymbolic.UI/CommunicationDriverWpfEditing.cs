...
namespace S7TIASymbolic.UI
{
    public class CommunicationDriverWpfEditing : ICommunicationDriverWpfEditing2, IDisposable
    {
        ...
        private bool CopyFileInternal(string sourceconn, string targetconn, string drivername)
        {
            ...
            string DriverConfig = DriverCodeBase.CommunicationDriver.GetConnectionString(sourceconn, null, drivername, null);

            IDataLayer idl = DriverCodeBase.CommunicationDriver.GetSpecificDataLayer(DriverConfig, out Filebase, out InMemory, out TargetIsFile);
            using (ufw = new UnitOfWork(idl))
            {
                ...
                if ((configuration != null) && (configuration.StationSettings.Count > 0))
                {
                    ...
                    {                        
                        ...
                        DriverCodeBase.ChannelSettings channelDrv = configuration.ChannelSettings.FirstOrDefault(c => c.Name == settings.Channel);
                        ...
                        if (channelDrv != null)
                        {
                            ...
                            Result &= DriverCodeBase.CommunicationDriver.CopyFile<S7TIADBsNameaAndTables>(sourceconn, targetconn, channel.GetListNameDBsNameTablesFileNameOnly(station.Name), ".xml");
                            ...
                            Result &= DriverCodeBase.CommunicationDriver.CopyFile<S7TIAImportProjectTag>(sourceconn, targetconn, channel.GetImportProjectTagsFileNameOnly(station.Name), ".xml");
        ...
        public bool CopyFile(string sourceconn, string targetconn)
        {
            ...
            bool result = DriverCodeBase.CommunicationDriver.CopyFile<S7TIADriverSettings>(sourceconn, targetconn, drivername);
            result &= DriverCodeBase.CommunicationDriver.CopyFile<S7TIASymbolicChangeTag>(sourceconn, targetconn, drivername, DriverCodeBase.Helpers.DriverInfo.GetChangeDynTagsExtension());
