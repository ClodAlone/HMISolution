...
namespace NaisFp.UI
{
    public class CommunicationDriverWpfEditing : ICommunicationDriverWpfEditing3, IDisposable
    {
        ...
        public bool CopyFile(string sourceconn, string targetconn)
        {
            ...
            bool result = DriverCodeBaseEx.CommunicationDriver.CopyFile<NaisFpDriverSettings>(sourceconn, targetconn, drivername);
            result &= DriverCodeBaseEx.CommunicationDriver.CopyFile<NaisFpChangeTag>(sourceconn, targetconn, drivername, DriverCodeBaseEx.Helpers.DriverInfo.GetChangeDynTagsExtension());
            return result;
        }
        ...
        {
            NaisFpDynTagSettings dynTag = new NaisFpDynTagSettings();
        ...
        {
            var genSet = generalSettings as NaisFpDriverSettings;
            if (genSet == null)
                return null;

            if (channelEditor == null)
                channelEditor = new ChannelDetails();
            var ch = new NaisFpChannelSettings(genSet.Session);
        ...
        public UserControl NewStationSettingsEditor(object generalSettings)
        {
            ...
            var st = new NaisFpStationSettings(genSet.Session);
        ...
        public bool DisableDriver(string connectionString, bool bProtected, Guid protectionCode)
        {
            ...
            {
                string drivername = DriverCodeBaseEx.Helpers.DriverInfo.GetDriverName(this).Replace(".UI", "");
                configurationEditor.Init<NaisFpDriverSettings>(connectionString, drivername, bProtected, protectionCode);
