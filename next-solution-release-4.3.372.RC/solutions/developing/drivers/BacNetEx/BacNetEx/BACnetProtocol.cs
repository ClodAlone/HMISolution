...
namespace BACnet
{
    ...
    public class NPDU : IPackable, IExpandable
    {
        ...
        public void Expand(ref byte[] DataArray, ref int Offset)
        {
            ...
            if (_Apdu.PduType == BACnetEnums.BACnetPDU.COMPLEX_ACK)
            {
                if(((byte)_Apdu.ConfirmedServiceChoice == ((byte)BACnetEnums.ConfirmedService.READ_PROP_MULTIPLE)) &&
                    _Apdu.ServiceRequest.List.Count > 1 && _Apdu.ServiceRequest.List[1].isTagList )
                {
                    BACnetDriver.Addinfo("<----------------------------------->");
                    UInt32 DeviceIstanceNumber = BitConverter.ToUInt32((byte[])_Apdu.ServiceRequest.List[0].Values, 0) & 0x3fffff;
                    BACnetDriver.Addinfo(string.Format(Properties.Resources.DeviceIstanceNumber, DeviceIstanceNumber.ToString()));
                  
                    ParserRequestMultyProperty(_Apdu.ServiceRequest.List[1].TagList);
                }
                else
                    BACnetDriver.Addinfo(Properties.Resources.ErrorImpossibleToRetrieveDeviceInformation);
            }
        }

        public void ParserRequestMultyProperty(ServiceTagList TagList)
        {
            for(int index = 0; index < TagList.List.Count(); index += 2)
            {
                switch(TagList.List[index].UInt)
                {
                    case (uint)BACnetEnums.PropertyIdentifier.OBJECT_NAME:
                        {
                            if(TagList.List[index + 1].TagList.List.Count > 0)
                                BACnetDriver.Addinfo(string.Format(Properties.Resources.ObjectName, TagList.List[index + 1].TagList.List[0].CharacterString));
                            else
                                BACnetDriver.Addinfo(string.Format(Properties.Resources.ObjectName, "Error"));
                        }
                        break;
                    case (uint)BACnetEnums.PropertyIdentifier.VENDOR_NAME:
                        {
                            if (TagList.List[index + 1].TagList.List.Count > 0)
                                BACnetDriver.Addinfo(string.Format(Properties.Resources.VendorName, TagList.List[index + 1].TagList.List[0].CharacterString));
                            else
                                BACnetDriver.Addinfo(string.Format(Properties.Resources.VendorName, "Error"));
                        }
                        break;
                    case (uint)BACnetEnums.PropertyIdentifier.FIRMWARE_REVISION:
                        {
                            if (TagList.List[index + 1].TagList.List.Count > 0)
                                BACnetDriver.Addinfo(string.Format(Properties.Resources.FirmwareRevision, TagList.List[index + 1].TagList.List[0].CharacterString));
                            else
                                BACnetDriver.Addinfo(string.Format(Properties.Resources.FirmwareRevision, "Error"));
                        }
                        break;
                    case (uint)BACnetEnums.PropertyIdentifier.MAX_APDU_LENGTH_ACCEPTED:
                        {
                            if (TagList.List[index + 1].TagList.List.Count > 0)
                                BACnetDriver.Addinfo(string.Format(Properties.Resources.MaxAPDULengthAccepted, TagList.List[index + 1].TagList.List[0].UInt.ToString()));
                            else
                                BACnetDriver.Addinfo(string.Format(Properties.Resources.MaxAPDULengthAccepted, "Error"));
                        }
                        break;
                    case (uint)BACnetEnums.PropertyIdentifier.MAX_SEGMENTS_ACCEPTED:
                        {
                            if (TagList.List[index + 1].TagList.List.Count > 0)
                                BACnetDriver.Addinfo(string.Format(Properties.Resources.MaxSegmentsAccepted, TagList.List[index + 1].TagList.List[0].UInt.ToString()));
                            else
                                BACnetDriver.Addinfo(string.Format(Properties.Resources.MaxSegmentsAccepted, "Error"));
                        }
                        break;
                    case (uint)BACnetEnums.PropertyIdentifier.MODEL_NAME:
                        {
                            if (TagList.List[index + 1].TagList.List.Count > 0)
                                BACnetDriver.Addinfo(string.Format(Properties.Resources.ModelName, TagList.List[index + 1].TagList.List[0].CharacterString));
                            else
                                BACnetDriver.Addinfo(string.Format(Properties.Resources.ModelName, "Error"));
                        }
                        break;
                    case (uint)BACnetEnums.PropertyIdentifier.PROTOCOL_SERVICES_SUPPORTED:
                        {
                            if (TagList.List[index + 1].TagList.List.Count > 0)
                            {
                                //-1 because is a terminate byte
                                byte length = (byte)(TagList.List[index + 1].TagList.List[0].Size - 1);
                                byte[] buff = new byte[length];
                                Array.Copy(TagList.List[index + 1].TagList.List[0].OctetString, 1, buff, 0, length);
                                String bbs = GetStingServicesSupported(buff);
                                BACnetDriver.Addinfo(Properties.Resources.ProtocolServicesSupported + " : ");
                                for (int i = 0; i < bbs.Length; i++)
                                {
                                    try
                                    {
                                        String text = Enum.GetName(typeof(BACnetEnums.ListService), i); // for 'classic' Enum like 0,1,2,3 ...
                                        if (!String.IsNullOrEmpty(text))
                                        {
                                            text = string.Format("{0}={1}",text, (bbs[i] == '1' ? "True" : "False"));
                                            BACnetDriver.Addinfo(text);
                                        }
                                    }
                                    catch { }
                                }
                                ...
                                BACnetDriver.Addinfo("Protocol Services Supported: Error");
                        }
                        break;
        ...
        public string GetStingServicesSupported(byte[] DataArray)
        {
            string ret = "";
            int count = DataArray.Length * 8;
            for (int i = 0; i < count; i++)
            {
                ret = ret + ((DataArray[i / 8] & (0x80 >> (i % 8))) > 0 ? "1" : "0");
            }
            return ret;
        }
    }
    ...
    public class APDU : IPackable, IExpandable
    {
       ...
       //Fields
        ...
        public bool MORESEG
        {
            set { _bytePduType = (byte)((Convert.ToByte(value) << 2) + (_bytePduType & ((1 << 2) ^ 0xFF))); }
            get { return (Convert.ToBoolean(_bytePduType & (1 << 2))); }
        }
        ...
        public List<byte> Data { set { _Data = value; } get { return _Data; } }
            ...
            switch (PduType)
            {
                ...
                case BACnetEnums.BACnetPDU.COMPLEX_ACK:
                    ...
                    if (SEG)
                    {
                        ...
                        _WindowSize = BufferExpand.toByte(ref DataArray, ref Offset);
                        _ServiceChoice = BufferExpand.toByte(ref DataArray, ref Offset);
                        _Data.AddRange(DataArray.Skip(Offset).Take(DataArray.Length - Offset));
                    }
                    else
                    {
                        _ServiceChoice = BufferExpand.toByte(ref DataArray, ref Offset);
    ...
    public static partial class IPFrameFactory
    {
        ...
        /// <summary>
        /// creation of the device information request
        /// </summary>
        /// <param name="job"></param>
        /// <returns></returns>
        public static BACnetIPFrame confirmedReadPropertyInfo(BACnetStation Station, BACnetEnums.PropertyIdentifier propertyIdentifier)
        {
            ...
            //BACnetIPFrame.Npdu.Apdu.ServiceRequest.Add(new ServiceTag((uint)propertyIdentifier, 1));
            BACnetIPFrame BACnetIPFrame = new BACnetIPFrame(BACnetEnums.ConfirmedService.READ_PROP_MULTIPLE);
            if (Station.DestinationSpecifierPresent)
            {
                BACnetIPFrame.Npdu.DestinationSpecifierPresent = Station.DestinationSpecifierPresent;
                BACnetIPFrame.Npdu.DNET = Station.DNET;
            ...
            BACnetIPFrame.Npdu.Apdu.ServiceRequest.Add(new ServiceTag(((BACnetStation)Station).DeviceIdentifier, 0));
            //Prepare the list of properties to request from the device
            ServiceTagList inTagList = new ServiceTagList();
            inTagList.Add(new ServiceTag((uint)BACnetEnums.PropertyIdentifier.OBJECT_NAME, 0));
            inTagList.Add(new ServiceTag((uint)BACnetEnums.PropertyIdentifier.VENDOR_NAME, 0));
            inTagList.Add(new ServiceTag((uint)BACnetEnums.PropertyIdentifier.FIRMWARE_REVISION, 0));
            inTagList.Add(new ServiceTag((uint)BACnetEnums.PropertyIdentifier.MODEL_NAME, 0));
            inTagList.Add(new ServiceTag((uint)BACnetEnums.PropertyIdentifier.MAX_APDU_LENGTH_ACCEPTED, 0));
            inTagList.Add(new ServiceTag((uint)BACnetEnums.PropertyIdentifier.MAX_SEGMENTS_ACCEPTED, 0));
            inTagList.Add(new ServiceTag((uint)BACnetEnums.PropertyIdentifier.PROTOCOL_SERVICES_SUPPORTED, 0));
        ...
        public static BACnetIPFrame InfoDevice(BACnetStation Station)
        {
            BACnetIPFrame BACnetIPFrame = new BACnetIPFrame(BACnetEnums.ConfirmedService.READ_PROPERTY);
            if (Station.DestinationSpecifierPresent)
            {
            ...
            BACnetIPFrame.Npdu.Apdu.InvokeId = ((BACnetChannel)Station.GetChannel()).getInvokeID();
            BACnetIPFrame.Npdu.Apdu.ServiceRequest.Add(new ServiceTag((uint)BACnetEnums.PropertyIdentifier.OBJECT_TYPE, 0));
            byte[] arrayshort = new byte[2] { (byte)BACnetEnums.PropertyIdentifier.OBJECT_NAME, 0x29 };
            BACnetIPFrame.Npdu.Apdu.ServiceRequest.Add(new ServiceTag(BitConverter.ToInt16(arrayshort,0), 0));
            //BACnetIPFrame.Npdu.Apdu.ServiceRequest.Add(new ServiceTag((uint)BACnetEnums.PropertyIdentifier., 0));
            //switch (job.PropertyIdentifier)
            //{
            //    case BACnetEnums.PropertyIdentifier.STATE_TEXT:
            //    case BACnetEnums.PropertyIdentifier.WEEKLY_SCHEDULE:
    ...
    public partial class BACnetProtocol
    {
        #region const
        ...
        public static void BACnetParseDataImportVariable(object remoteEndPoint, byte[] packet, DateTime timeStamp, Dictionary<string, BACnetStation> allowedIp, out ReceiveItem receiveItem)
        {
            receiveItem = new ReceiveItem(new BACnetIPFrame(), new IPEndPoint(0, 0), new DateTime());
    ...
    public static class BacNextProtocol
    {
        public const string TEST_COMM_DYNAMIC_SETTINGS = "BACnet.Station={0}|LinkType=1|OBT=2|OBN=OBJECT_ANALOG_VALUE|PRI=85|CE=False|DS=0|AI=1|DLM=0|PL=8|INN=0";
