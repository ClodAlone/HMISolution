...
namespace BACnet
{
    ...
    public class BACnetDriver : CommunicationDriver
    {
        ...
        public BACnetDriver(string strSettingPath)
            : base(strSettingPath)
        {
        }
        ...
        public override void GetDriverErrorInfo(int errorcode, out uint quality, out string error)
        {
            ...
            switch ((BACnetErrorCodes)errorcode)
            {
                ...
                case BACnetErrorCodes.ErrorWhoHisFailed:
                    ...
                    break;
                case BACnetErrorCodes.ErrorRegisterToBBMD:
                    quality = StatusCodes.BadConnectionRejected;
                    error = Properties.Resources.ErrorRegisterToBBMD;
                    break;
                //case BACnetErrorCodes.ErrorAllStationsOnError:
                //    quality = StatusCodes.BadConnectionRejected;
                //    error = Properties.Resources.ErrorAllStationsOnError;
                //    break;
                case BACnetErrorCodes.ErrorInvalidStation:
                    quality = StatusCodes.BadConnectionRejected;
                    error = Properties.Resources.ErrorInvalidStation;
                    break;
                case BACnetErrorCodes.ErrorUnreachableStation:
                    quality = StatusCodes.BadConnectionRejected;
                    error = Properties.Resources.ErrorUnreachableStation;
                    break;
                case BACnetErrorCodes.ErrorUnknownObject:
                    quality = StatusCodes.BadNotFound;
                    error = Properties.Resources.ErrorUnknownObject;
                    break;
            }
        }
        #endregion
        ...
        public override bool TestComm(ChannelSettings ch, StationSettings st, out string error)
        {
            ...
            //To run the test without creating duplicate code, a dummy job is created to trigger all the initialization sequences for communication with the device.
            IList<object> dataValues = new List<Object>();
            dataValues.Add(new Variant((float)0));
            info = string.Empty;
            // force drive to terminated execution after pls station's info is retried --> don't continue tryiing to subscribe job
            TestInfoDevice = true;

            uint conn = OnReadValues(string.Format(BacNextProtocol.TEST_COMM_DYNAMIC_SETTINGS, station.Name), dataValues);
        ...
        private static object lockAddinfo = new object();
        ...
        public static bool TestInfoDevice = false;
        #endregion
    }
}
