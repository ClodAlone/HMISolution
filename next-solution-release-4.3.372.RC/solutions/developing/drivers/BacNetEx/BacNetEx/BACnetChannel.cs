...
namespace BACnet
{
    ...
    public partial class BACnetChannel : BacknetUdpChannel
    {
            ...
            else
            {
                ...
                switch (mJob.BacnetState)
                {
                    ...
                    case BACnetCommJob.BACnetState.Polling:
                        ...
                        break;
                     case BACnetCommJob.BACnetState.CheckDeviceInfo:
                        {
        ...
        public override bool ProcessNewData(DriverErrorCodes conn, CommJob exjob)
        {
            ...
            switch (conn)
            {
                case DriverErrorCodes.ErrorDeviceOpenFailed:
                    ...
                    BACnetDriver.Addinfo(String.Format("{0} Error: {1}", exjob.Station.Name, Properties.Resources.ErrorBACnetTimeOut));
                ...
                default:
                    ...
                    else
                    {
                        ...
                        if (IPFrameValidator.isErrorFrame(receiveItem.Frame))
                        {
                            ...
                            //string errormsg = String.Format("{0} Error Frame: {1}",exjob.Station.Name, error.ToString);
                            BACnetDriver.Addinfo(String.Format("{0} Error Frame: {1}", exjob.Station.Name, error.ToString()));
                        }
                        else
                        {
            ...
            switch (mJob.BacnetState)
            {
                ...
                case BACnetCommJob.BACnetState.CheckDeviceInfo:
                    {
                        if ((conn == DriverErrorCodes.ErrorTimeOut) || ((byte)receiveItem.Frame.Npdu.Apdu.ConfirmedServiceChoice == ((byte)BACnetEnums.ConfirmedService.READ_PROP_MULTIPLE)) &&
                                      receiveItem.Frame.Npdu.Apdu.ServiceRequest.List.Count > 1 && receiveItem.Frame.Npdu.Apdu.ServiceRequest.List[1].isTagList)
                        {                        
                            //end of life of the dummy job used to establish the connection with the device.
        ...
        public override bool Startup()
        {
            if (!bChannelStarted)
            {
                ...
                if (RequestObjectList)
                    this.NewDataReceived += BACnetChannel_ObjectListImport;
                else
                    this.NewDataReceived += BACnetChannel_NewDataReceived;

                CyclicCheckStart(Properties.Settings.Default.WhoIsRepeatDelaySlow, Properties.Settings.Default.WhoIsRepeatDelaySlow); // System.Threading.Timeout.Infinite);
        ...
        private void CopyTagValueFromAnotherJob(BACnetCommJob jobSource, BACnetCommJob jobDest)
        {
            for (int i = 0; i < jobSource.TagsList.Count; i++)
            {
                if (jobDest.TagsList.Count >= i)
                    jobDest.TagsList[i].SetValue(jobSource.TagsList[i].GetValue());
            }
        ...
        public void RequestInfoDevice(Station s, BACnetCommJob mJob, BACnetEnums.PropertyIdentifier propertyIdentifier)
        {
            ResetNewDataEvent();
            mJob.InvokeID = getInvokeID();
            DriverErrorCodes conn = DriverErrorCodes.ErrorNoError;
            ...
            if (!DeviceWrite(IPFrameFactory.ReadProp((BACnetStation)mJob.Station, inTagList), (mJob.Station as BACnetStation).RemoteEndPoint, mJob))
            {
                BACnetDriver.Addinfo(Properties.Resources.ErrorImpossibleToRetrieveDeviceInformation);
                conn = (DriverErrorCodes)BACnetErrorCodes.ErrorTxWrite;
                // manage error into ProcessNewData()
                RequestObjectList = false;
            }            
        }
        ...
        private void BACnetChannel_NewDataReceived(object sender, NewDataReceivedArgs e)
        {
            ...
            lock (currentJobLock)
            {
                ...
                if (BACnetDriver.TestInfoDevice)
                {
                    if (IPFrameValidator.IAm(receiveItem.Frame, out IAmAnswer iAmAnswer))
                    {
                        BACnetDriver.Addinfo(string.Format(Properties.Resources.InstanceFound, iAmAnswer.DeviceIdentifier.instance));
                    }
                }
        ...
        List<Station> stationsToCheck = new List<Station>();
