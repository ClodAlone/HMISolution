...
namespace BACnet
{
    ...
    public class BACnetCommJob : CommJob
    {
        ...
        public void CheckBacNetState()
        {
            ...
            if (BacnetState == BACnetState.WhoIs)
            {
                if (((BACnetStation)Station).WhoIsDisabled)
                {
                    ...
                    //Request info when request the test
                    if (BACnetDriver.TestInfoDevice)
                    {
                        BacnetState = BACnetState.CheckDeviceInfo;
                        ((BACnetChannel)Station.GetChannel()).RequestInfoDevice(this.Station, this, BACnetEnums.PropertyIdentifier.OBJECT_NAME);
                    }
                    else
            ...
            if (BacnetState == BACnetState.CheckDeviceInfo)
            {
                return; // send read info request
            }
        ...
        {
            return (TagsList[0].GetValue() != null);
        }
