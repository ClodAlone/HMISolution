...
namespace MQTTClient
{
    ...
    public class MQTTClientCommJob : CommJob
    {
            ...
            : base(station, settings)
        {
            ...
            if (IsValid)
            {
                MQTTClientTag defTag = null;
                if (StructTag != null)
                    defTag = StructTag as MQTTClientTag;
                else
                    defTag = TagsList[0] as MQTTClientTag;
                if (defTag != null)
                    InitJSONFormatProperties(station, defTag);
            }
        }
        ...
        public void SetError(DriverErrorCodes errorCode)
        {
            ...
            //{
                if (!InErrorState && Station.FirstTime)
                {
                    ...
                    Station.GetCommDriver().OnSystemEvent(null, String.Format(DriverCodeBaseEx.Properties.Resources.StationResumeError, Station.GetCommDriver().DriverName/*DriverInfo.GetDriverName()*/, Station.Name), EventSeverity.Low);
                }
            //}
        }
        ...
        public bool MustPublish()
        {
            ...
            if ((Type != LinkType.Input) &&
                ((TagsListOnWriting.Count > 0) || (Type == LinkType.UnconditionalOutput)))
            {
                returnValue = true;
            }
        ...
        public string GetPublishMessagePayload()
        {
            ...
            cand.Value.Value = cand.WriteVal;
            DataValue valueTobeSent = new DataValue(cand.Value);
        ...
        public uint SetJobValue(string receivedValue, ref List<Tag> changed)
        {
            MQTTClientStation mqttStation = (MQTTClientStation)Station;
            if (mqttStation.MQTTClientMessageFormat == MQTTClientMessageFormats.XML)
            {
                DataValue receivedData = new DataValue();
                try
                {
                    receivedData = receivedValue.FromXml<DataValue>();
                }
                catch 
                {
                    return (uint)(MQTTClientErrorCodes.ErrorCodeErrorBadDecodingError);
                }
        ...
        public void RestoreInitialStatus()
        {
            if(InErrorState)
            {
                ...
                {
                    SetQuality(StatusCodes.Uncertain);
                }
                else
                {
                    SetQuality(StatusCodes.BadWaitingForInitialData);
                }
                InErrorState = false;
            }
        }
        ...
        /// <summary>
        /// Init JSON format properties
        /// </summary>
        /// <param name="station"></param>
        private void InitJSONFormatProperties(Station station, MQTTClientTag defTag)
        {
        ...
        public override bool DuplicatedTagValueRemoved(NodeId tagnodeid, ref object value)
        {
            lock (lockListObject)
            {
                bool result = base.DuplicatedTagValueRemoved(tagnodeid, ref value);
                if (!result)
                {
                    // Driver specific: for jobs of type ExceptionOutput check also the Hysteresis value
                    if (Type == LinkType.ExceptionOutput)
                    {
                        if (!CheckHysteresis(ref value))
                        {
