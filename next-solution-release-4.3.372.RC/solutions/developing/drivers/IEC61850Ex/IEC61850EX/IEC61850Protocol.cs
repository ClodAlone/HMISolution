...
namespace IEC61850
{
    ...
    public class IEC61850Protocol
    {
        ...
        static public byte[] PrepareRequest(IEC61850CommJob job, uint invokeID)
        {
            if (job.ReadRequest())
            {
        ...
        static public byte[] BuildWriteRequest(IEC61850CommJob job, uint invokeID)
        {
            byte[] mmsRequestBuffer = null;
            if (job.MMSDataType != MMSDataTypes.Structure)
                mmsRequestBuffer = BuildMMSWriteRequest(job, invokeID);
            else
                mmsRequestBuffer = BuildMMSStructuredWriteRequest(job, invokeID);
        ...
        static public byte[] BuildMMSWriteRequest(IEC61850CommJob job, uint invokeID)
        {
        ...
        static uint GetStructureFieldsTypes(Channel ch, IEC61850CommJob job)
        {
            uint nNumberOfTypes = 0;            
            byte[] pTypesBuffer = job.StructureFieldsTypes;
            uint nMaxNumberOfTypes = job.GetTheNumberOfStructureFields();
            uint[] pBitStringInfos = job.StructureBitStringInfos;

            // Set the initial value for the types            
            for (uint i = 0; i < 2 * nMaxNumberOfTypes; i += 2)
            {
                pTypesBuffer[i] = 0;
                pTypesBuffer[i + 1] = 0xff;
            }

            // Build the read request message
            byte[] pRequestBuffer = BuildReadRequest(job.MMSDataItemId, job.LogicalDeviceName, RequestTypes.ReadVariableAccessAttributes, ((IEC61850Channel)ch).GetNextInvokeID());

            // Send the Read request
            if (!ch.DeviceWrite(pRequestBuffer, (uint)pRequestBuffer.Count()))
                return 0;

            ((IEC61850Channel)ch).SyncronizeWriteWithBeginDeviceRead();
                        
            // Get the device reply
            // Synchronous read of the reply
            byte[] replyMessage = ((IEC61850Channel)ch).ReadDeviceReply();
            if (replyMessage == null)
                return 0;

            uint nTotalCheckedBytes = 4;
            uint nBytesToBeChecked = (uint)replyMessage.Length - nTotalCheckedBytes;
            if (nBytesToBeChecked == 0)
                return 0;

            // Check the COTP header
            uint nCheckedBytes = CheckCOTPHeader(replyMessage, nTotalCheckedBytes, nBytesToBeChecked, out byte cotpSegmentType);
            if (nCheckedBytes == 0)
                return (0);

            nTotalCheckedBytes += nCheckedBytes;
            nBytesToBeChecked -= nCheckedBytes;
            if (nBytesToBeChecked == 0)
                return 0;

            // Check the session PDU
            nCheckedBytes = CheckSessionLayer(replyMessage, nTotalCheckedBytes, nBytesToBeChecked, 0x01, (IEC61850Station)job.Station);
            if (nCheckedBytes == 0)
                return (0);
            ...
            nCheckedBytes = CheckPresentationLayer(replyMessage, nTotalCheckedBytes, nBytesToBeChecked, 0x61, (IEC61850Station)job.Station);
            if (nCheckedBytes == 0)
                return (0);
            ...
            // Check the MMS PDU
            nCheckedBytes = CheckMMSLayer(replyMessage, nTotalCheckedBytes, nBytesToBeChecked, (IEC61850Channel)ch, out byte mpduType); //0xa1
            if (nCheckedBytes == 0)
                return 0;
            ...
            // Check the MMS reply
            uint nDataLength = 0;            
            nCheckedBytes = CheckMMSGetVarAccessAttribReply(replyMessage, nTotalCheckedBytes, nBytesToBeChecked, out nDataLength);
            if (nCheckedBytes == 0)
                return 0;

            nTotalCheckedBytes += nCheckedBytes;
            nBytesToBeChecked -= nCheckedBytes;
            if ((nBytesToBeChecked == 0) || (nBytesToBeChecked != nDataLength))
                return 0;

            // Parse the types
            uint nTotalNumberOfStructures = 0;
            nNumberOfTypes = ParseStructureFieldsTypes(replyMessage, nTotalCheckedBytes, nDataLength, pTypesBuffer, 0, pBitStringInfos, 0, nMaxNumberOfTypes, 0, ref nTotalNumberOfStructures);
            job.TotalNumberOfStructures = nTotalNumberOfStructures;

            return (nNumberOfTypes);
        }
        
        static public byte[] BuildMMSStructuredWriteRequest(IEC61850CommJob job, uint invokeID)
        {
            uint nNumberOfFields = job.GetTheNumberOfStructureFields();            

            if (job.StructureFieldsTypes == null)
                job.StructureFieldsTypes = new byte[2 * nNumberOfFields];

            if (job.StructureBitStringInfos == null)
                job.StructureBitStringInfos = new uint[nNumberOfFields];

            if (job.NumberOfParsedStructureFieldsTypes == 0)
                job.NumberOfParsedStructureFieldsTypes = GetStructureFieldsTypes(job.Station.GetChannel(), job);

            if (job.NumberOfParsedStructureFieldsTypes == 0)
                return null;

            // invokeID  was used during GetStructureFieldsTypes; generate a new one
            invokeID = ((IEC61850Channel)job.Station.GetChannel()).GetNextInvokeID();

            // Calculate the message length            
            uint encodedDataLength = job.CalculateWriteStructureDataMMSLength();
            uint dataLength = (uint)encodedDataLength + 1 + BerEncoderCheckLengthSize((uint)encodedDataLength);
            uint dataTagLength = dataLength + 1 + BerEncoderCheckLengthSize(dataLength);
            // Item ID
            uint itemIdLength = (uint)job.MMSDataItemId.Length;
            ...
            requestBuffer[messageLength++] = 0xa0;
            BerEncoderAddLength(ref requestBuffer, ref messageLength, dataLength);
            if (!AddWriteStructureData(ref requestBuffer, messageLength, job))
            {
                return (null);
            }
        ...
        static public bool AddWriteData(ref byte[] requestBuffer, ref uint requestLength, IEC61850CommJob job, uint encodedDataLength)
        {
            ...
            switch (job.MMSDataType)
            {
                ...
                case MMSDataTypes.UnsignedInteger32Bits:
                    ...
                    if (encodedDataLength > 0)
                    {
                        ...
                        byte[] valueBuffer = IEC61850Protocol.AsnEncoderUnsigned(uintAux);
                        for (int i = 0; i < encodedDataLength; i++)
                        {
                            requestBuffer[requestLength++] = valueBuffer[i];
                        }
                ...
                case MMSDataTypes.FloatingPoint32Bits:
                    {
                        if (dataBufferLength >= 4)
                        {
                            ...
                            byte[] valueBuffer = IEC61850Protocol.ReverseBytes(auxdata);
        ...
        static uint AddWriteSingleStructureData(byte[] pRequestBuffer, uint messageLength, IEC61850CommJob job, ref uint nStructureIndex, ref uint nFieldIndex, byte nPreviousNestingLevel)
        {
            uint nLength = 0;
            if (nStructureIndex >= job.TotalNumberOfStructures)
                return 0;

            uint nCurrentStructureIndex = nStructureIndex;

            // Data type = Structure
            pRequestBuffer[messageLength + nLength++] = 0xa2;

            // Structure data size
            uint nStructSize = job.StructuresLengths[nCurrentStructureIndex];

            byte[] data = BerEncoderLength((int)nStructSize);
            Array.Copy(data, 0, pRequestBuffer, messageLength + nLength, data.Length);            
            nLength += (uint)data.Length;
            
            bool bFieldFound = true;
            while (bFieldFound && (nFieldIndex < job.NumberOfParsedStructureFieldsTypes))
            {
                // Build the structure field name
                nFieldIndex++;

                if (nFieldIndex > job.GetTagCount())
                    continue;

                // Check the nesting level of the field
                if (job.StructureFieldsTypes[2 * (nFieldIndex - 1)] > nPreviousNestingLevel)
                {
                    // Add the values of the fields of the nested structure
                    nFieldIndex--;
                    nStructureIndex++;
                    uint nStructLength = AddWriteSingleStructureData(pRequestBuffer, messageLength + nLength, job, ref nStructureIndex, ref nFieldIndex, job.StructureFieldsTypes[2 * nFieldIndex]);
                    if (nStructLength == 0)
                    {
                        // Parsing terminated
                        nLength = 0;
                    ...
                    nLength += nStructLength;
                }
                else if (job.StructureFieldsTypes[2 * (nFieldIndex - 1)] < nPreviousNestingLevel)
                {
                    // Parsing completed for a nested structure
                    nFieldIndex--;
                    break;
                }
                else
                {
                    // Add the data of the current field
                    nLength += AddWriteSingleFieldData(pRequestBuffer, messageLength + nLength, job, nFieldIndex - 1);
                }
            }
        ...
        static uint AddWriteSingleFieldData(byte[] pRequestBuffer, uint messageLength, IEC61850CommJob job, uint nFieldIndex)
        {
            if (nFieldIndex >= job.GetTagCount())
                return 0;

            uint nLength = AddWriteFieldData(pRequestBuffer, messageLength, job, nFieldIndex, job.StructureFieldsTypes[2 * nFieldIndex + 1], job.StructureBitStringInfos[nFieldIndex]);

            return (nLength);
        }

        static uint AddWriteFieldData(byte[] pRequestBuffer, uint initRequestBuffer, IEC61850CommJob job, uint nFieldIndex, byte nMMSType, uint nBitStringInfo)
        {
            object tagValue = null;
            job.GetJobDataMemberOfStruct(job.TagsList[(int)nFieldIndex], ref tagValue);
            
            uint nLength = initRequestBuffer;
            switch (nMMSType)
            {
                case (byte)MMSDataTypes.Boolean:
                    pRequestBuffer[nLength++] = 0x83;
                    pRequestBuffer[nLength++] = 1;
                    pRequestBuffer[nLength++] = (byte)((bool)tagValue ? 0xff : 0);
                    break;
                ...
                case (byte)MMSDataTypes.Integer32Bits:
                    {
                        pRequestBuffer[nLength++] = 0x85;
                        int nIntValue = 0;
                        switch ((uint)job.TagsList[(int)nFieldIndex].TagNode.DataType.Identifier)
                        {
                            case (uint)BuiltInType.Byte://VT_UI1:
                                nIntValue = (int)(byte)tagValue;
                                break;

                            case (uint)BuiltInType.SByte: // VT_I1:
                                nIntValue = (int)(sbyte)tagValue;
                                break;

                            case (uint)BuiltInType.UInt16: // VT_UI2:
                                nIntValue = (int)(UInt16)tagValue;
                                break;

                            case (uint)BuiltInType.Int16: // VT_I2:
                                nIntValue = (int)(Int16)tagValue;
                                break;

                            case (uint)BuiltInType.UInt32: // VT_UI4:
                                nIntValue = (int)(UInt32)tagValue;
                                break;

                            default:
                                nIntValue = (int)tagValue;
                                break;
                        }

                        byte[] pValueBuffer = AsnEncoderLong(nIntValue);
                        int nDataLength = pValueBuffer.Length;
                        pRequestBuffer[nLength++] = (byte)nDataLength;
                        for (int i = 0; i < nDataLength; i++)
                            pRequestBuffer[nLength++] = pValueBuffer[i];
                    }
                    break;
                ...
                case (byte)MMSDataTypes.UnsignedInteger32Bits:
                    {
                        pRequestBuffer[nLength++] = 0x86;
                        uint nUintValue = 0;
                        switch ((uint)job.TagsList[(int)nFieldIndex].TagNode.DataType.Identifier)
                        {
                            case (uint)BuiltInType.Byte: // VT_UI1:
                                nUintValue = (uint)(byte)tagValue;
                                break;
                        ...
                        byte[] pValueBuffer = AsnEncoderUnsigned(nUintValue);
                        int nDataLength = pValueBuffer.Length;
                        pRequestBuffer[nLength++] = (byte)nDataLength;
                        for (int i = 0; i < nDataLength; i++)
                            pRequestBuffer[nLength++] = pValueBuffer[i];
                ...
                case (byte)MMSDataTypes.FloatingPoint32Bits:
                    {
                        float fFloatValue = (float)tagValue;
                        byte[] pValueBuffer = BitConverter.GetBytes(fFloatValue);
                        
                        // Added in version 11.4.1.11 (FOGBUGZ 21227)
                        ReverseBytes(pValueBuffer);

                        pRequestBuffer[nLength++] = 0x87;
                        pRequestBuffer[nLength++] = 5;
                        pRequestBuffer[nLength++] = 8; // Exponent width
                        for (int i = 0; i < 4; i++)
                            pRequestBuffer[nLength++] = pValueBuffer[i];
                    }
                    break;
                ...
                case (byte)MMSDataTypes.FloatingPoint64Bits:
                    {
                        double flDoubleValue = (double)tagValue;
                        byte[] pValueBuffer = BitConverter.GetBytes(flDoubleValue);

                        // Added in version 11.4.1.11 (FOGBUGZ 21227)
                        ReverseBytes(pValueBuffer);

                        pRequestBuffer[nLength++] = 0x87;
                        pRequestBuffer[nLength++] = 9;
                        pRequestBuffer[nLength++] = 11; // Exponent width
                        for (int i = 0; i < 8; i++)
                            pRequestBuffer[nLength++] = pValueBuffer[i];
                    }
                    break;
                ...
                case (byte)MMSDataTypes.BitString:
                    {
                        pRequestBuffer[nLength++] = 0x84;
                        uint nBitLength = nBitStringInfo & 0xffffff;
                        UInt16 wAux = HIWORD(nBitStringInfo);
                        byte nPadding = HIBYTE(wAux);
                        nPadding %= 8;
                        int nSize = (int)nBitLength / 8;
                        if (nBitLength % 8 != 0)
                            nSize++;
                        int nDataLength = 0;
                        int i = 0;
                        ...
                        {
                            nDataLength = nSize;
                            pRequestBuffer[nLength++] = (byte)(nDataLength + 1);
                            pRequestBuffer[nLength++] = (nPadding);
                            uint nUintValue = 0;
                            switch ((uint)job.TagsList[(int)nFieldIndex].TagNode.DataType.Identifier)
                            {
                                case (uint)BuiltInType.Byte: // VT_UI1:
                            ...
                            // Modified in version 11.4.1.6 (FOGBUGZ 20116)
                            //nUintValue <<= nPadding;
                            nUintValue <<= nPadding + 8 * (4 - nSize);

                            byte[] pValueBuffer = BitConverter.GetBytes(nUintValue);
                            ReverseBytes(pValueBuffer);
                        ...
                        {
                            string szAux = tagValue.ToString();
                            int nNonNullChar = szAux.Length;
                            nDataLength = nNonNullChar / 8;
                            if (nNonNullChar % 8 != 0)
                            {
                                nDataLength++;
                            }
                            ...
                            //nLength += m_pIEC61850Station->BerEncoderAddLength(&(pRequestBuffer[nLength]), nDataLength + 1);
                            BerEncoderAddLength(ref pRequestBuffer, ref nLength, (uint)nDataLength + 1);

                            pRequestBuffer[nLength++] = nPadding;

                            byte nByteValue = 0;
                            uint nBitIndexLimit = 0;
                            int nBitIndex;
                            for (i = 0, nBitIndex = 0; (i < nDataLength) && (nBitIndex < (int)nNonNullChar); i++)
                            {
                                nByteValue = 0;
                                if (i == (nDataLength - 1))
                                {
                                    nBitIndexLimit = nPadding;
                                }
                                for (int j = 7; j >= nBitIndexLimit; j--, nBitIndex++)
                                {
                                    if (szAux[nBitIndex]  == 0)
                                    {
                                        break;
                                    }
                                    else if (szAux[nBitIndex] != '0')
                                    {
                                        nByteValue |= (byte)(1 << j);
                                    }
                                }
                                pRequestBuffer[nLength++] = nByteValue;
                            }
                        }

                        byte nPaddingMask = 0;
                        for (i = 0; i < nPadding; i++)
                            nPaddingMask += (byte)(1 << i);

                        nPaddingMask = (byte)~nPaddingMask;
                        pRequestBuffer[nLength - 1] &= nPaddingMask;
                    }
                    break;

                case (byte)MMSDataTypes.OctetString:
                    {
                        pRequestBuffer[nLength++] = 0x89;
                        ...
                        uint nDataLength = (uint)nNonNullChar / 3;
                        if (nNonNullChar % 3 != 0)
                            nDataLength++;

                        //nLength += m_pIEC61850Station->BerEncoderAddLength(&(pRequestBuffer[nLength]), nDataLength);
                        BerEncoderAddLength(ref pRequestBuffer, ref nLength, (uint)nDataLength);
                        if (nDataLength > 0)
                        {                            
                            int nCharIndex = 0;
                            uint nByteValue = 0;
                            string szByteValue;
                            for (int i = 0; (i < nDataLength) && (nCharIndex < (int)nNonNullChar); i++)
                            {
                                nByteValue = 0;
                                szByteValue = string.Empty;
                                while (szAux[nCharIndex] != 0)
                                {
                                    if (szAux[nCharIndex] != ' ')
                                    {
                                        szByteValue += szAux[nCharIndex];
                                        nCharIndex++;
                                    }
                                    else
                                    {
                                ...
                                if (!String.IsNullOrEmpty(szByteValue))
                                {
                                    //if (_stscanf(szByteValue, _T("%x"), &nByteValue) == 1)
                                    if (szByteValue.IndexOf(nByteValue.ToString("%x")) >=0)
                                    {
                                        pRequestBuffer[nLength++] = (byte)nByteValue;
                                    }
                                }
                            }
                        }
                ...
                case (byte)MMSDataTypes.VisibleString:
                    {
                        pRequestBuffer[nLength++] = 0x8a;
                        string szAux = tagValue.ToString();
                        int nNonNullChar = szAux.Length;
                        //nLength += m_pIEC61850Station->BerEncoderAddLength(&(pRequestBuffer[nLength]), nNonNullChar);
                        BerEncoderAddLength(ref pRequestBuffer, ref nLength, (uint)nNonNullChar);
                        if (nNonNullChar > 0)
                        {
                            for (int i = 0; i < (int)nNonNullChar; i++)
                                pRequestBuffer[nLength++] = (byte)szAux[i];
                        }
                    }
                    break;
                ...
                case (byte)MMSDataTypes.UTCTime:
                    {
                        pRequestBuffer[nLength++] = 0x91;
                        pRequestBuffer[nLength++] = 8;
                        byte[] pDataBuffer = new byte[8];

                        string szTimeValue = tagValue.ToString();
                        if (!String.IsNullOrEmpty(szTimeValue))
                            ConvertASCIIStringToUTCTime(szTimeValue, ref pDataBuffer, 8);

                        for (int i = 0; i < 8; i++)
                            pRequestBuffer[nLength++] = pDataBuffer[i];
                    }
                    break;

                case (byte)MMSDataTypes.BinaryTime:
                    {
                        pRequestBuffer[nLength++] = 0x8c;
                        nLength += 6;
                        byte[] pDataBuffer = new byte[6];

                        string szTimeValue = tagValue.ToString();
                        if (!String.IsNullOrEmpty(szTimeValue))
                            ConvertASCIIStringToBinaryTime(szTimeValue, ref pDataBuffer, 6);
            ...
            return (nLength - initRequestBuffer);
        }

        static bool AddWriteStructureData(ref byte[] pRequestBuffer, uint messageLength, IEC61850CommJob job)
        {
            ...
            if (job.NumberOfParsedStructureFieldsTypes == 0|| (job.NumberOfParsedStructureFieldsTypes > job.NumberOfStructureFields))
                return false;

            // Check the numbers of parsed structures (it must be at least 1)
            if (job.TotalNumberOfStructures < 1)
                return false;

            uint nStructureIndex = 0;
            uint nFieldIndex = 0;
            byte nPreviousNestingLevel = 1;

            return (AddWriteSingleStructureData(pRequestBuffer, messageLength, job, ref nStructureIndex, ref nFieldIndex, nPreviousNestingLevel) > 0);
        }

        static void ConvertASCIIStringToBinaryTime(string timeString, ref byte[] timeByteBuffer, uint bufferLength)
        {
        ...
        static void ConvertASCIIStringToUTCTime(string timeString, ref byte[] timeByteBuffer, uint bufferLength)
        {
        ...
        static public byte[] BuildReadRequest(IEC61850CommJob job, uint invokeID)
        {
        ...
        static public byte[] BuildReadRequest(string mmsDataItemId, string LogicalDeviceName, RequestTypes requestType, uint invokeID)
        {
        ...
        static byte[] BuildMMSReadVariableAccessAttributesRequest(string mmsDataItemId, string logicalDeviceName, uint invokeID)
        {
            // Calculate the message length
            // Item ID
        ...
        static byte[] BuildMMSReadDataSetDirectoryRequest(string mmsDataItemId, string logicalDeviceName, uint invokeID)
        {
            // Calculate the message length
            // Item ID
            uint itemIdLength = (uint)mmsDataItemId.Length;
        ...
        static byte[] BuildMMSReadRequest(string mmsDataItemId, string logicalDeviceName, uint invokeID)
        {
            // Calculate the message length
            // Item ID
            uint itemIdLength = (uint)mmsDataItemId.Length;
        ...
        static byte[] BuildMMSReadIdentifyRequest(uint invokeID)
        {
            // Calculate the message length
        ...
        static void AddRCBItemValueToWriteRequest(ref byte[] pRequestBuffer, MMSDataTypes nItemType,uint nEncodedDataLength,byte nPadding, byte[] pDataBuffer, uint nDataOffset)
        {
            switch (nItemType)
            {
                case MMSDataTypes.Boolean:
                    ...
                    pRequestBuffer[nDataOffset++] = (pDataBuffer[0] != 0 ? (byte)1 : (byte)0);
                    break;
        ...
        static byte[] BuildMMSWriteRCBItemRequest(string szDeviceID,string szItemID, MMSDataTypes nItemType, byte[] pDataBuffer, byte nPadding, uint invokeID)
        {
            // Calculate the message length
            // Data
        ...
        static public byte[] BuildWriteRCBItemRequest(string szDeviceID, string szItemID, MMSDataTypes nItemType, byte[] pDataBuffer, byte nPadding, uint invokeID)
        {
        ...
        static void BerEncoderAddLength(ref byte[] buffer, ref uint bufferSize, uint length)
        {
        ...
        static RequestTypes GetJobReadRequestType(IEC61850CommJob job)
        {
        ...
        static public uint CheckMMSInitiatePDU(byte[] replyMessage, uint totalCheckedBytes, uint bytesToBeChecked)
        {
        ...
        static public uint CheckAssociationControlService(byte[] replyMessage, uint totalCheckedBytes, uint bytesToBeChecked)
        {
            if (bytesToBeChecked < 1)
            {
                return (0);
        ...
        static public uint CheckPresentationLayer(byte[] replyMessage, uint totalCheckedBytes, uint bytesToBeChecked, byte expectedPDUType, IEC61850Station station)
        {
            uint checkedBytes = 0;
            // Check the PDU type
        ...
        static public uint CheckMMSConfirmedReadResponse(byte[] replyMessage, uint totalCheckedBytes, uint bytesToBeChecked, out uint dataLength)
        {
            uint checkedBytes = 0;
            byte responseType = 0;
            dataLength = 0;
        ...
        static public uint CheckMMSIdentifyResponse(byte[] replyMessage, uint totalCheckedBytes, uint bytesToBeChecked)
        {
            uint checkedBytes = 0;
            byte responseType = 0;
            if (bytesToBeChecked < 2)
        ...
        static public uint CheckMMSVarAccessReply(byte[] replyMessage, uint totalCheckedBytes, uint bytesToBeChecked, out uint dataLength)
        {
            dataLength = 0;
            if (bytesToBeChecked < 2)
            {
        ...
        static public uint CheckMMSNamedVarListReply(byte[] replyMessage, uint totalCheckedBytes, uint bytesToBeChecked, out uint dataLength)
        {
            dataLength = 0;
            if (bytesToBeChecked < 2)
            {
        ...
        static public uint CheckMMSGetGetVarAccessAttribReply(byte[] replyMessage, uint totalCheckedBytes, uint bytesToBeChecked, out uint dataLength)
        {
            uint checkedBytes = 0;
            byte responseType = 0;
            dataLength = 0;
        ...
        static public uint CheckMMSGetNamedVarListAttribReply(byte[] replyMessage, uint totalCheckedBytes, uint bytesToBeChecked, out uint dataLength)
        {
            uint checkedBytes = 0;
            byte responseType = 0;
            dataLength = 0;
        ...
        static public uint CheckMMSConfirmedResponse(byte[] replyMessage, uint totalCheckedBytes, uint bytesToBeChecked, IEC61850CommJob job, out uint dataLength, out DriverErrorCodes errorCode)
        {
            uint checkedBytes = 0;
            byte responseType = 0;
            dataLength = 0;
        ...
        static bool CheckMMSWriteResult(byte[] replyMessage, uint initialIndex, uint bytesToBeChecked, out DriverErrorCodes errorCode)
        {
        ...
        static bool CheckMMSResult(byte[] replyMessage, uint initialIndex, uint bytesToBeChecked, out DriverErrorCodes errorCode)
        {
            bool returnValue = true;
            errorCode = DriverErrorCodes.ErrorNoError;
            switch(replyMessage[initialIndex])
        ...
        static public bool CheckMMSWriteResult(byte[] replyMessage, uint totalCheckedBytes, uint bytesToBeChecked, out uint errorCode)
        {            
        ...
        static public uint ParseInfoReportID(byte[] replyBuffer, uint totalCheckedBytes, uint nReplyLength, out string szReportID)
        {
        ...
        static public uint CheckMMSReadReply(byte[] replyMessage, uint totalCheckedBytes, uint bytesToBeChecked, out uint dataLength)
        {
            dataLength = 0;
            if (bytesToBeChecked < 2)
            {
        ...
        static public uint CheckMMSIdentifyReply(byte[] replyMessage, uint totalCheckedBytes, uint bytesToBeChecked)
        {
            if (bytesToBeChecked < 2)
            {
                return (0);
        ...
        static public uint CheckMMSWriteReply(byte[] replyMessage, uint totalCheckedBytes, uint bytesToBeChecked)//, out uint dataLength)
        {
        ...
        static public uint CheckInformationReport(byte[] replyBuffer)
        {
        ...
        static public uint CheckMMSLayer(byte[] replyMessage, uint totalCheckedBytes, uint bytesToBeChecked, IEC61850Channel channel, out byte mpduType)
        {
            uint checkedBytes = 0;
        ...
        static public uint CheckMMSInvokeId(byte[] replyMessage, uint totalCheckedBytes, uint bytesToBeChecked, IEC61850Channel channel)
        {
            uint checkedBytes = 0;
        ...
        static public uint CheckPresentationLayer(byte[] replyMessage, uint totalCheckedBytes, uint bytesToBeChecked, IEC61850Station station, out byte ppduType)
        {
            uint checkedBytes = 0;
            // Parse and check the PDU type
        ...
        static uint CheckPresentationPDU(byte[] replyMessage, uint initIndex, uint bytesToBeChecked, IEC61850Station station)
        {
        ...
        static uint BerDecodeUint32(byte[] replyMessage, uint initIndex, uint valueSize)
        {
            uint value = 0;
        ...
        static public uint BerDecodeLength(byte[] replyMessage, uint initIndex, uint bytesToBeChecked, out uint lengthSize)
        {
            lengthSize = 0;
            if(bytesToBeChecked < 1)
            {
        ...
        static public uint CheckSessionLayer(byte[] replyMessage, uint totalCheckedBytes, uint bytesToBeChecked, byte expectedPDUType, IEC61850Station station)
        {
            uint checkedBytes = 0;
            // Check the PDU type
            if (replyMessage[totalCheckedBytes + checkedBytes++] != expectedPDUType)
        ...
        static public uint CheckSessionLayer(byte[] replyMessage, uint totalCheckedBytes, uint bytesToBeChecked, IEC61850Station station, out byte spduType)
        {
            uint checkedBytes = 0;
            // Check the PDU type
        ...
        static public uint CheckCOTPHeader(byte[] replyMessage, uint totalCheckedBytes, uint bytesToBeChecked, out byte cotpSegmentType)
        {
            cotpSegmentType = 0;
        ...
        static public byte[] BuildCOTPConnectionRequest(IEC61850Station station)
        {
        ...
        static public byte[] BuildAssociateRequest(IEC61850Station station, IEC61850Channel channel)
        {
        ...
        static byte[] AddSessionConnectionRequest(IEC61850Station station, IEC61850Channel channel, byte[] requestDataPart)
        {
        ...
        static byte[] AddSessionReadRequest(byte[] requestDataPart)
        {
        ...
        static byte[] AddPresentationReadRequest(byte[] requestDataPart)
        {
            // Calculate the length of the encoded message
            uint dataLength = (uint)requestDataPart.Length;
        ...
        static public byte[] AddPresentationConnectionRequest(IEC61850Station station, IEC61850Channel channel, byte[] requestDataPart)
        {
        ...
        static public byte[] AddAssociationControlRequest(IEC61850Station station, IEC61850Channel channel, byte[] requestDataPart)
        {
        ...
        static public uint AsnEncoderCheckLongSize(int intValue)
        {
        ...
        static public uint AsnEncoderCheckUnsignedSize(uint uintValue)
        {
        ...
        static public byte[] AsnEncoderLong(int intValue)
        {
            byte[] buffer = BitConverter.GetBytes(intValue);
            int bufferIndex = 0;
            for (bufferIndex = 3; bufferIndex > 0; bufferIndex--)
        ...
        static public byte[] AsnEncoderUnsigned(uint uintValue)
        {
            byte[] buffer = BitConverter.GetBytes(uintValue);
            int bufferIndex = 0;
            for (bufferIndex = 3; bufferIndex > 0; bufferIndex--)
        ...
        static byte[] BerEncoderUint32Value(uint value)
        {
        ...
        static uint BerEncoderCheckUint32Size(uint value)
        {
            byte[] valueBuffer = new byte[5];
            valueBuffer[0] = 0;
            ushort auxWord = (ushort)(value >> 16);
        ...
        static uint BerEncoderCheckCompressedIntegerSize(byte[] valueBuffer)
        {
        ...
        public static uint BerEncoderCheckLengthSize(uint length)
        {
            uint lengthSize = 0;
        ...
        static public byte[] BuildMMSInitiateRequest()
        {
            // MMS Initiate PDU
        ...
        static public byte[] BuildMMSInitiatePDU()
        {
            uint pduLength = 0;
        ...
        static public byte[] BerEncoderLength(int length)
        {
            uint bufferLength = 0;
            if (length < 128)
            {
        ...
        static public byte[] BuildMMSInitiateDetails()
        {
            uint pduLength = 0;
        ...
        static public byte[] AddISOHeader(byte[] requestBuffer)
        {
            int dataLength = 0;
        ...
        static public byte[] AddCOTPHeader(byte[] requestBuffer)
        {
            int dataLength = 0;
        ...
        static byte[] SetSelectorArray(string selectorString)
        {
        ...
        static byte[] SetOidArray(string oidString)
        {
        ...
        static uint CheckApplicationIdString(string oidString)
        {
        ...
        static public byte[] ReverseBytes(byte[] dataBuffer)
        {
        ...
        static public bool ParseData(byte[] receivebuffer, ref IEC61850CommJob job, ref List<object> items)
        {
        ...
        static public bool ParseMMSData(byte[] receivebuffer, out MMSDataTypes dataType, out uint dataBufferLength, out uint dataOffset, out uint padding)
        {
        ...
        static public DriverErrorCodes ParseJobMMSData(byte[] receivebuffer, bool areArguments, ref IEC61850CommJob job, ref List<object> items, ref List<Tag> changed)
        {
            ...
            uint padding = 0;
            // receivebuffer[0] == Data Access Type
            if(!IEC61850Protocol.ParseMMSData(receivebuffer, out dataType, out dataBufferLength, out dataOffset, out padding))
            {
                return ((DriverErrorCodes)IEC61850ErrorCodes.ErrorUnexpectedReply);
            }
        ...
        static byte[] GetDataBuffer(MMSDataTypes receivedDataType, byte[] dataBuffer, ref IEC61850CommJob job, uint padding, ref List<Tag> changed)
        {
            byte[] jobData = null;
        ...
        static byte[] CopyStructuredData(MMSDataTypes dataType, uint dataBufferLength, byte[] dataBuffer, ref IEC61850CommJob job, ref uint fieldIndex, ref List<Tag> changed)
        {
        ...
        static byte[] CopyStructureData(uint dataBufferLength, byte[] dataBuffer, ref IEC61850CommJob job, ref uint dataOffset, ref uint fieldIndex, ref List<Tag> changed)
        {
            List<byte> copiedData = new List<byte>();
            uint fieldNumber = (uint)job.TagsList.Count();
        ...
        static byte[] CopyStructureBinaryTime(uint dataBufferLength, byte[] dataBuffer, ref IEC61850CommJob job, ref uint dataOffset, ref uint fieldIndex, ref List<Tag> changed)
        {
        ...
        static byte[] CopyStructureUTCTime(uint dataBufferLength, byte[] dataBuffer, ref IEC61850CommJob job, ref uint dataOffset, ref uint fieldIndex, ref List<Tag> changed)
        {
            byte[] copiedData = null;
            uint dataLength = 0;
            if ((dataBufferLength - dataOffset) >= 2)
        ...
        static byte[] CopyStructureBitString(uint dataBufferLength, byte[] dataBuffer, ref IEC61850CommJob job, ref uint dataOffset, ref uint fieldIndex, ref List<Tag> changed)
        {
            byte[] copiedData = null;
            int dataLength = -1;
            uint padding = 0;
        ...
        static byte[] CopyStructureVisibleString(uint dataBufferLength, byte[] dataBuffer, ref IEC61850CommJob job, ref uint dataOffset, ref uint fieldIndex, ref List<Tag> changed)
        {
            byte[] copiedData = null;
            int dataLength = -1;
        ...
        static byte[] CopyStructureOctetString(uint dataBufferLength, byte[] dataBuffer, ref IEC61850CommJob job, ref uint dataOffset, ref uint fieldIndex, ref List<Tag> changed)
        {
            byte[] copiedData = null;
            int dataLength = -1;
            if ((dataBufferLength - dataOffset) >= 1) // Empty string is an acceptable case
        ...
        static byte[] GetUintStructTagBuffer(uint dataValue, ref IEC61850CommJob job, int tagIndex)
        {
            byte[] dataArray = null;
        ...
        static byte[] GetIntStructTagBuffer(int dataValue, ref IEC61850CommJob job, int tagIndex)
        {
            byte[] dataArray = null;
            uint tagType = (uint)Opc.Ua.DataTypes.UInt32;
            //lock (job.retLockList())
        ...
        static byte[] CopyStructureUnsignedInteger(uint dataBufferLength, byte[] dataBuffer, ref IEC61850CommJob job, ref uint dataOffset, ref uint fieldIndex, ref List<Tag> changed)
        {
            byte[] copiedData = null;
            uint dataLength = 0;
            MMSDataTypes dataType = MMSDataTypes.Boolean;
        ...
        static byte[] CopyStructureInteger(uint dataBufferLength, byte[] dataBuffer, ref IEC61850CommJob job, ref uint dataOffset, ref uint fieldIndex, ref List<Tag> changed)
        {
            byte[] copiedData = null;
            uint dataLength = 0;
            MMSDataTypes dataType = MMSDataTypes.Boolean;
        ...
        static byte[] CopyStructureBoolean(uint dataBufferLength, byte[] dataBuffer, ref IEC61850CommJob job, ref uint dataOffset, ref uint fieldIndex, ref List<Tag> changed)
        {
            byte[] copiedData = null;
            uint dataLength = 0;
            if ((dataBufferLength - dataOffset) >= 2)
        ...
        static byte[] CopyStructureFloat(uint dataBufferLength, byte[] dataBuffer, ref IEC61850CommJob job, ref uint dataOffset, ref uint fieldIndex, ref List<Tag> changed)
        {
            byte[] copiedData = null;
            uint dataLength = 0;
        ...
        static byte[] CopyBitStringToASCIIString(uint dataBufferLength, byte[] dataBuffer, uint padding)
        {
        ...
        static byte[] CopyBinaryTimeToASCIIString(uint dataBufferLength, byte[] dataBuffer)
        {
        ...
        static byte[] CopyUTCTimeToASCIIString(uint dataBufferLength, byte[] dataBuffer)
        {            
            byte[] buffer = new byte[0];
        ...
        public static void ConvertBinaryTimeToDateTime(byte[] dataBuffer, ref DateTime dt)
        {
        ...
        static public DateTime ConvertBinaryTimeToDateTime(uint dataBufferLength, byte[] dataBuffer)
        {            
        ...
        static public void ConvertUTCTimeToDateTime(byte[] dataBuffer, ref DateTime dt)
        {
            uint dataBufferLength = (uint)dataBuffer.Length;
            if(dataBufferLength <= 2)
            {
        ...
        static DateTime ConvertUTCTimeToDateTime(uint dataBufferLength, byte[] dataBuffer)
        {            
        ...
        static byte[] CopyOctetStringToASCIIString(uint dataBufferLength, byte[] dataBuffer)
        {
        ...
        static uint CheckMMSGetVarAccessAttribReply(byte[] pReplyBuffer, uint nInitialTotalCheckedBytes, uint nReplyLength, out uint nDataLength)
        {
            nDataLength = 0;
            if (nReplyLength < 2)
                return 0;
            ...
            uint nTotalCheckedBytes = 0;
            if (pReplyBuffer[nInitialTotalCheckedBytes + nTotalCheckedBytes++] != 0xa6)
                return 0;

            if (nReplyLength < (nTotalCheckedBytes + 2))
                return 0;

            uint nLengthSize = 0;
            uint nLength = BerDecodeLength(pReplyBuffer, (nInitialTotalCheckedBytes + nTotalCheckedBytes), nReplyLength - nTotalCheckedBytes, out nLengthSize);
            if (nLength == 0)
                return 0;

            nTotalCheckedBytes += nLengthSize;

            if (nReplyLength < (nTotalCheckedBytes + nLength))
                return 0;
            ...
            // Skip the Deletable attribute
            nTotalCheckedBytes += 3;

            if (pReplyBuffer[nInitialTotalCheckedBytes + nTotalCheckedBytes++] != 0xa2)
                return (0);

            nLength = BerDecodeLength(pReplyBuffer, (nInitialTotalCheckedBytes + nTotalCheckedBytes), nReplyLength - nTotalCheckedBytes, out nLengthSize);
            if (nLength == 0)
                return 0;
        ...
        static uint ParseStructureFieldsTypes(byte[] pReplyBuffer, uint pReplyBufferInit, uint nReplyLength, byte[] pTypesBuffer, uint pTypesBufferInit, uint[] pBitStringInfo, uint pBitStringInfoInit, uint nMaxNumberOfTypes, byte nNestingLevel, ref uint nNumberOfStructures)
        {
            uint nDataOffset = 0;
            uint nNumberOfParsedTypes = 0;
            byte nFieldLevel = (byte)(nNestingLevel + 1);

            // Is the device tag a Structure?
            if (pReplyBuffer[pReplyBufferInit + nDataOffset++] != 0xa2)
                return 0;

            // Check the structure size
            uint nLengthSize = 0;
            uint nLength = BerDecodeLength(pReplyBuffer, (pReplyBufferInit + nDataOffset), nReplyLength - nDataOffset, out nLengthSize);
            if (nLength == 0)
                return 0;

            nDataOffset += nLengthSize;
            if (nReplyLength < (nDataOffset + nLength))
                return 0;

            // Parse the structure and set the field types
            while ((nDataOffset < nReplyLength) && (nNumberOfParsedTypes < nMaxNumberOfTypes))
            {
                if (nReplyLength < (nDataOffset + 2))
                {
                    // Parsing completed
                    nDataOffset = nReplyLength;
                    continue;
                }

                // Skip the information about the field name
                if (pReplyBuffer[(pReplyBufferInit + nDataOffset)] == 0xa1)
                {
                    nDataOffset++;
                    nLength = BerDecodeLength(pReplyBuffer, (pReplyBufferInit + nDataOffset), nReplyLength - nDataOffset, out nLengthSize);
                    nDataOffset += nLengthSize;
                    if (nReplyLength < (nDataOffset + nLength))
                    {
                        // Parsing completed
                ...
                else if (pReplyBuffer[(pReplyBufferInit + nDataOffset)] == 0x30)
                {
                    nDataOffset++;
                    nLength = BerDecodeLength(pReplyBuffer, (pReplyBufferInit + nDataOffset), nReplyLength - nDataOffset, out nLengthSize);
                    nDataOffset += nLengthSize;
                ...
                else if (pReplyBuffer[(pReplyBufferInit + nDataOffset)] == 0x80)
                {
                    nDataOffset++;
                    nLength = BerDecodeLength(pReplyBuffer, (pReplyBufferInit + nDataOffset), nReplyLength - nDataOffset,out nLengthSize);
                    nDataOffset += nLengthSize;
                ...
                switch (pReplyBuffer[(pReplyBufferInit + nDataOffset)])
                {
                    // Float
                    case 0x87:
                        nDataOffset++;
                        ...
                        nLength = BerDecodeLength(pReplyBuffer, (pReplyBufferInit + nDataOffset), nReplyLength - nDataOffset,out nLengthSize);
                        nDataOffset += nLengthSize;
                        if ((nReplyLength < nDataOffset) || (nLength != 1))
                        {
                            // Parsing completed
                            nDataOffset = nReplyLength;
                            continue;
                        }
                        nLength = pReplyBuffer[nDataOffset];
                        switch (nLength)
                        {
                            case 0x20:
                                nDataOffset++;
                                pTypesBuffer[pTypesBufferInit + 2 * nNumberOfParsedTypes] = nFieldLevel;
                                pTypesBuffer[pTypesBufferInit + 2 * nNumberOfParsedTypes + 1] = (byte)MMSDataTypes.FloatingPoint32Bits;
                                pBitStringInfo[pBitStringInfoInit + nNumberOfParsedTypes] = 0;
                                nNumberOfParsedTypes++;
                                break;

                            case 0x40:
                                nDataOffset++;
                                pTypesBuffer[pTypesBufferInit + 2 * nNumberOfParsedTypes] = nFieldLevel;
                                pTypesBuffer[pTypesBufferInit + 2 * nNumberOfParsedTypes + 1] = (byte)MMSDataTypes.FloatingPoint64Bits;
                                pBitStringInfo[pBitStringInfoInit + nNumberOfParsedTypes] = 0;
                                nNumberOfParsedTypes++;
                                break;
                    ...
                    case 0x83:
                        nDataOffset += 2;
                        pTypesBuffer[pTypesBufferInit + 2 * nNumberOfParsedTypes] = nFieldLevel;
                        pTypesBuffer[pTypesBufferInit + 2 * nNumberOfParsedTypes + 1] = (byte)MMSDataTypes.Boolean;
                        pBitStringInfo[pBitStringInfoInit + nNumberOfParsedTypes] = 0;
                        nNumberOfParsedTypes++;
                        break;
                    ...
                    case 0x85:
                        ...
                        nLength = pReplyBuffer[(pReplyBufferInit + nDataOffset)];
                        if ((nLength >= 1) && (nLength <= 32))
                        {
                            nDataOffset++;
                            byte nDataType = (byte)MMSDataTypes.Integer8Bits;
                            if (nLength <= 8)
                            {
                                nDataType = (byte)MMSDataTypes.Integer8Bits;
                            }
                            else if (nLength <= 16)
                            {
                                nDataType = (byte)MMSDataTypes.Integer16Bits;
                            }
                            else
                            {
                                nDataType = (byte)MMSDataTypes.Integer32Bits;
                            }
                            pTypesBuffer[pTypesBufferInit + 2 * nNumberOfParsedTypes] = nFieldLevel;
                            pTypesBuffer[pTypesBufferInit + 2 * nNumberOfParsedTypes + 1] = nDataType;
                            pBitStringInfo[pBitStringInfoInit + nNumberOfParsedTypes] = 0;
                            nNumberOfParsedTypes++;
                        }
                        else
                    ...
                    case 0x86:
                        ...
                        {
                            nDataOffset++;
                            byte nDataType = (byte)MMSDataTypes.UnsignedInteger8Bits;
                            if (nLength <= 8)
                            {
                                nDataType = (byte)MMSDataTypes.UnsignedInteger8Bits;
                            }
                            else if (nLength <= 16)
                            {
                                nDataType = (byte)MMSDataTypes.UnsignedInteger16Bits;
                            }
                            else
                            {
                                nDataType = (byte)MMSDataTypes.UnsignedInteger32Bits;
                            }
                            pTypesBuffer[pTypesBufferInit + 2 * nNumberOfParsedTypes] = nFieldLevel;
                            pTypesBuffer[pTypesBufferInit + 2 * nNumberOfParsedTypes + 1] = nDataType;
                            pBitStringInfo[pBitStringInfoInit + nNumberOfParsedTypes] = 0;
                    ...
                    case 0x89:
                        ...
                        nLength = BerDecodeLength(pReplyBuffer, (pReplyBufferInit + nDataOffset), nReplyLength - nDataOffset, out nLengthSize);
                        // Empty string is an acceptable case
                        nDataOffset += 2 * nLengthSize;
                        pTypesBuffer[pTypesBufferInit + 2 * nNumberOfParsedTypes] = nFieldLevel;
                        pTypesBuffer[pTypesBufferInit + 2 * nNumberOfParsedTypes + 1] = (byte)MMSDataTypes.OctetString;
                        pBitStringInfo[pBitStringInfoInit + nNumberOfParsedTypes] = 0;
                        nNumberOfParsedTypes++;
                        break;
                    ...
                    case 0x8a:
                        ...
                        nDataOffset += 2 * nLengthSize;
                        pTypesBuffer[pTypesBufferInit + 2 * nNumberOfParsedTypes] = nFieldLevel;
                        pTypesBuffer[pTypesBufferInit + 2 * nNumberOfParsedTypes + 1] = (byte)MMSDataTypes.VisibleString;
                        pBitStringInfo[pBitStringInfoInit + nNumberOfParsedTypes] = 0;
                        nNumberOfParsedTypes++;
                        break;
                    ...
                    case 0x90:
                        ...
                        nDataOffset += 2 * nLengthSize;
                        pTypesBuffer[pTypesBufferInit + 2 * nNumberOfParsedTypes] = nFieldLevel;
                        pTypesBuffer[pTypesBufferInit + 2 * nNumberOfParsedTypes + 1] = (byte)MMSDataTypes.MMSString;
                        pBitStringInfo[pBitStringInfoInit + nNumberOfParsedTypes] = 0;
                        nNumberOfParsedTypes++;
                        break;
                    ...
                    case 0x84:
                        ...
                        else
                        {
                            ...
                            nDataOffset++;
                            pTypesBuffer[pTypesBufferInit + 2 * nNumberOfParsedTypes] = nFieldLevel;
                            pTypesBuffer[pTypesBufferInit + 2 * nNumberOfParsedTypes + 1] = (byte)MMSDataTypes.BitString;
                            uint nBitLength = 0;
                            if (nLength <= 127)
                            {
                                nBitLength = nLength;
                            ...
                            {
                                nBitLength = 256 - nLength;
                            }
                            byte nPadding = 0;
                            if (nBitLength > 0)
                            {
                                nPadding = (byte)(8 - (nBitLength % 8));
                            }
                            nBitLength &= 0xffffff;
                            uint nInfoValue = nPadding;
                            nInfoValue <<= 24;
                            nInfoValue += nBitLength;
                            pBitStringInfo[pBitStringInfoInit + nNumberOfParsedTypes] = nInfoValue;
                            nNumberOfParsedTypes++;
                        }
                        break;
                    ...
                    case 0x91:
                        nDataOffset += 2;
                        pTypesBuffer[pTypesBufferInit + 2 * nNumberOfParsedTypes] = nFieldLevel;
                        pTypesBuffer[pTypesBufferInit + 2 * nNumberOfParsedTypes + 1] = (byte)MMSDataTypes.UTCTime;
                        pBitStringInfo[pBitStringInfoInit + nNumberOfParsedTypes] = 0;
                        nNumberOfParsedTypes++;
                        break;
                    ...
                    case 0x8c:
                        nDataOffset += 2;
                        pTypesBuffer[pTypesBufferInit + 2 * nNumberOfParsedTypes] = nFieldLevel;
                        pTypesBuffer[pTypesBufferInit + 2 * nNumberOfParsedTypes + 1] = (byte)MMSDataTypes.BinaryTime;
                        pBitStringInfo[pBitStringInfoInit + nNumberOfParsedTypes] = 0;
                        nNumberOfParsedTypes++;
                        break;
                    ...
                    // Structure
                    case 0xa2:
                        nLength = BerDecodeLength(pReplyBuffer, (pReplyBufferInit + nDataOffset + 1), nReplyLength - nDataOffset,out nLengthSize);
                        if (nLength > 0)
                        {
                            // Parse the nested structure
                            uint nStructSize = nLengthSize + 1 + nLength;
                            nNumberOfParsedTypes += ParseStructureFieldsTypes(pReplyBuffer, (pReplyBufferInit + nDataOffset), nStructSize, pTypesBuffer, pTypesBufferInit + (2 * nNumberOfParsedTypes), pBitStringInfo, pBitStringInfoInit + nNumberOfParsedTypes, nMaxNumberOfTypes - nNumberOfParsedTypes, nFieldLevel, ref nNumberOfStructures);
                            nDataOffset += nStructSize;
                        }
                        else
                        {
                            // Parsing operation terminated
                    ...
                    // Added in version 11.4.1.9 (FOGBUGZ 21227)
                    // Float (constructed type)
                    case 0xa7:
                        nDataOffset++;
                        // Check the data length size
                        nLength = BerDecodeLength(pReplyBuffer, (pReplyBufferInit + nDataOffset), nReplyLength - nDataOffset, out nLengthSize);
                        nDataOffset += nLengthSize;
                        if ((nReplyLength < nDataOffset) || (nLength != 6))
                        {
                            // Parsing completed
                            nDataOffset = nReplyLength;
                            continue;
                        }
                        // Parse the size in bit
                        if (pReplyBuffer[nDataOffset] != 2)
                        {
                            // Parsing completed
                            nDataOffset = nReplyLength;
                            continue;
                        ...
                        {
                            case 0x20:
                                // Skip the information about the size of the exponent
                                nDataOffset += 4;
                                pTypesBuffer[pTypesBufferInit + 2 * nNumberOfParsedTypes] = nFieldLevel;
                                pTypesBuffer[pTypesBufferInit + 2 * nNumberOfParsedTypes + 1] = (byte)MMSDataTypes.FloatingPoint32Bits;
                                pBitStringInfo[pBitStringInfoInit + nNumberOfParsedTypes] = 0;
            ...
            if (nNumberOfParsedTypes > 0)
            {
                // Increase the number of parsed structures
                nNumberOfStructures++;
            }

            return (nNumberOfParsedTypes);
        }

        static UInt32 LOWORD(UInt32 l)
        {
            return (UInt16)(l & 0xffff);
        }

        static UInt16 HIWORD(UInt32 l)
        {
            return (UInt16)(l >> 16);
        }

        static byte LOBYTE(UInt32 w)
        {
            return (byte)(w & 0xff);
        }

        static byte HIBYTE(UInt32 w)
        {
            return (byte)(w >> 8);
        }
        #endregion
