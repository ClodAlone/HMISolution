...
namespace IEC61850
{
    public class IEC61850Channel : TcpChannel
    {
        ...
        private TimeZoneInfo channelTimeZoneInfo;
        ...
        private CommJob activeJob = null;
        ...
        /// <returns></returns>
        private IEC61850EventID WaitNewEvent(int sleepCycle)
        {
            IEC61850EventID eventID = (IEC61850EventID)CheckPriorityEvents();
            if (eventID == (IEC61850EventID)EventID.None)
            {
                //wait for answer
                int eventNr = WaitHandle.WaitAny(waitHandles, sleepCycle);
                eventID = (IEC61850EventID)CheckPriorityEvents();
                if (eventID == (IEC61850EventID)EventID.None)
                {
                    ...
                    {
                        case 0:
                            eventID = (IEC61850EventID)EventID.NewJobExecution;
                        ...
                        case 1:
                            ...
                            lock (lockThreadObject)
                            {
                                ...
                                {
                                    eventID = IEC61850EventID.TimeOut;
                                    ResetNewDataEvent();
                                }
                                else
                                {
                                    eventID = IEC61850EventID.NewDataToAnalize;
                                }
                            }
                            break;
        ...
        IEC61850EventID eventID = (IEC61850EventID)EventID.None;
        protected override void CommunicationToDeviceThread(object data)
        {
            ...
            if (_ActiveCommunicationCheckTime > 0)
                CyclicCheckStart((int)_ActiveCommunicationCheckTime, (int)_ActiveCommunicationCheckTime);

            while (true)
            {
                // wait some events/condition to proceed 
                eventID = WaitNewEvent(sleepCycle);
                if (eventID == (IEC61850EventID)EventID.ThreadTerminate)
                    break;

                switch (eventID)
                {
                    case (IEC61850EventID)EventID.NewJobExecution:
                        if (activeJob == null)
                        {
                            //if (SynchroJob != null)
                            //    nextjob = SynchroJob;
                            //else
                            activeJob = GetNextPendingJob();
                            if (activeJob != null)
                            {
                                DriverErrorCodes conn = CheckDevice(activeJob, this);
                                if (!ExecuteJob(ref conn, activeJob))
                                    sleepCycle = ResetActiveJob(ref activeJob);
                            }
                        }
                        break;
                    case (IEC61850EventID)EventID.CyclicCheckExecution:
                ...
                dataReceived = false;
                lock (lockThreadObject)
                    dataReceived = (ReceiveBuffer.Count > 0);
                ...
                if (dataReceived)
                {
                    ...
                    if (ManageReceivedMessage(out byte[] receivedMessage, out ReplyType replyType))
                    {
                        switch (replyType)
                        {
                            case ReplyType.ConfirmedResponse:
                                if (activeJob != null)
                                    if (ProcessConfirmedReply(activeJob, receivedMessage))
                                        sleepCycle = ResetActiveJob(ref activeJob);
                                break;

                            case ReplyType.Unconfirmed:
                                ProcessUnconfirmedMessage(receivedMessage, activeJob != null);
                                break;
                        }
                    }
                    else
                    {
                        ...
                        if (activeJob != null)
                        {
                            ProcessNewData(DriverErrorCodes.ErrorNoError, activeJob);
                            sleepCycle = ResetActiveJob(ref activeJob);
                        }
                    }
                }
                ...
                if (activeJob != null)
                {
                    if (DateTime.UtcNow.Subtract(activeJob.StartExecutionTime).TotalMilliseconds > Timeout)
                    {
                        //error
                        LastErrorCode = (GetPendingUnconfirmedMessageCount() > 0 ? (DriverErrorCodes)IEC61850ErrorCodes.ErrorTimeout : DriverErrorCodes.ErrorTimeOut);
                        ProcessNewData(LastErrorCode, activeJob);
                        sleepCycle = ResetActiveJob(ref activeJob);
        ...
        protected override bool CyclicCheck()
        {
            ...
            if (!dataReceived && activeJob != null)
            {
        ...
        public override DriverErrorCodes CheckDevice(CommJob exjob, object thischannel)
        {
            ...
            if (!IEC61850IsDeviceOpen(iec61850Station))
            {
                if (!IEC61850DeviceOpen(iec61850Station))
        ...
        public override bool ExecuteJob(ref DriverErrorCodes conn, CommJob job)
        {
            if (conn != DriverErrorCodes.ErrorNoError)
            {
                ...
                eJob.Job = activeJob;
                eJob.ErrorCode = (DriverCodeBaseEx.Enumerators.DriverErrorCodes)IEC61850ErrorCodes.ErrorConnectFailed;
                OnJobExecuted(eJob);
                return false;
            }
             
            IEC61850CommJob iec61850Job = (IEC61850CommJob)job;
            // Job, associated to a report, that does not require the initialization of data --> Do nothing
            if (((iec61850Job.Type == LinkType.Input) || ((iec61850Job.Type == LinkType.InputOutput) && (iec61850Job.GetTagListOnWritingCount() == 0))) && !string.IsNullOrEmpty(iec61850Job.MMSReportID) && !iec61850Job.InitializeData)
            {
            ...
            try
            {
                requestBuffer = IEC61850Protocol.PrepareRequest(iec61850Job, GetNextInvokeID());
            }
            catch
            {
                requestBuffer = null;
        ...
        private int ResetActiveJob(ref CommJob job)
        {
            job = null;
        ...
        */
        ...
        protected void ManageTimeoutError()
        {
            doBeginDeviceRead = false;
        }

        public uint GetNextInvokeID()
        {
            return (++InvokeID);
        }
        ...
        bool ManageReceivedMessage(out byte[] receivedMessage, out ReplyType replyType)
        {
            ...
            uint checkedBytes = IEC61850Protocol.CheckCOTPHeader(replyMessage, totalCheckedBytes, bytesToBeChecked, out cotpSegmentType);
            ...
            byte spduType = 0;
            checkedBytes = IEC61850Protocol.CheckSessionLayer(replyMessage, totalCheckedBytes, bytesToBeChecked, iec61850Station, out spduType);
            totalCheckedBytes += checkedBytes;
            bytesToBeChecked -= checkedBytes;
            if ((checkedBytes == 0) || (bytesToBeChecked == 0))
            {
                ...
                case 1: // Give tokens
                    {
                        ...
                        checkedBytes = IEC61850Protocol.CheckPresentationLayer(replyMessage, totalCheckedBytes, bytesToBeChecked, iec61850Station, out ppduType);
                        totalCheckedBytes += checkedBytes;
                        bytesToBeChecked -= checkedBytes;
                        if ((checkedBytes == 0) || (bytesToBeChecked == 0))
                        {
                        ...
                        switch (ppduType)
                        {
                            case 0x61: // Fully encoded data
                                checkedBytes = IEC61850Protocol.CheckMMSLayer(replyMessage, totalCheckedBytes, bytesToBeChecked, this, out mpduType);
                                totalCheckedBytes += checkedBytes;
                                bytesToBeChecked -= checkedBytes;
                                if ((checkedBytes == 0) || (bytesToBeChecked == 0))
                                {
        ...
        UnconfirmedMessageError ProcessSingleUnconfirmedMessage(byte[] receivedMessage)
        {
            ...
            uint checkedBytes = IEC61850Protocol.CheckInformationReport(receivedMessage);
            if (checkedBytes == 0)
            {
            ...
            checkedBytes = IEC61850Protocol.ParseInfoReportID(receivedMessage, totalCheckedBytes, bytesToBeChecked, out szReportID);
            if (checkedBytes == 0)
            {
        ...
        bool ProcessConfirmedReply(CommJob pendingjob, byte[] receivedMessage)
        {
            ...
            uint checkedBytes = IEC61850Protocol.CheckMMSConfirmedResponse(receivedMessage, 0, (uint)receivedMessage.Length, (IEC61850CommJob)pendingjob, out dataLength, out errorCode);
            if (checkedBytes == 0)
            {
        ...
        public override bool ProcessNewData(DriverErrorCodes conn, CommJob pendingjob)
        {
            ...
            {
                ExecutedJobArgs eJob = new ExecutedJobArgs { ErrorCode = conn, Job = pendingjob };
                OnJobExecuted(eJob);
                            ...
                            case 0x61: // Fully encoded data
                                ...
                                switch (mpduType)
                                {
                                    case 0xa1: // Confirmed Response
                                        ...
                                        {
                                            checkedBytes = IEC61850Protocol.CheckMMSConfirmedResponse(replyMessage, totalCheckedBytes, bytesToBeChecked, (IEC61850CommJob)pendingjob, out dataLength, out errorCode);
                                            if (checkedBytes == 0)
                                            {
                                                if (pendingjob != null)
                                                {
        ...
        bool Associate(IEC61850Station station)
        {
            ...
            byte[] associateRequestBuffer = IEC61850Protocol.BuildAssociateRequest(station, this);
            ...
            // Check the session PDU
            checkedBytes = IEC61850Protocol.CheckSessionLayer(replyMessage, totalCheckedBytes, bytesToBeChecked, 0x0e, station);
            if (checkedBytes == 0)
            {
                return (false);
            }
            ...
            checkedBytes = IEC61850Protocol.CheckPresentationLayer(replyMessage, totalCheckedBytes, bytesToBeChecked, 0x31, station);
            if (checkedBytes == 0)
            {
                return (false);
            }
            ...
            checkedBytes = IEC61850Protocol.CheckAssociationControlService(replyMessage, totalCheckedBytes, bytesToBeChecked);
            if (checkedBytes == 0)
            {
                return (false);
            }
            ...
            checkedBytes = IEC61850Protocol.CheckMMSInitiatePDU(replyMessage, totalCheckedBytes, bytesToBeChecked);
            if (checkedBytes == 0)
            {
                return (false);
            }
        ...
        public byte[] ReadVariableAccessAttributes(string szDeviceID, string szMMSDataItem)
        {
            ...
            byte[] requestBuffer = IEC61850Protocol.BuildReadRequest(szMMSDataItem, szDeviceID, RequestTypes.ReadVariableAccessAttributes, GetNextInvokeID());
                    ...
                    case 0xa1: // Confirmed Response
                        {
                            ...
                            // To be modified
                            checkedBytes = IEC61850Protocol.CheckMMSGetGetVarAccessAttribReply(replyMessage, totalCheckedBytes, bytesToBeChecked, out dataBufferLength);
                            totalCheckedBytes += checkedBytes;
                            bytesToBeChecked -= checkedBytes;
                            if ((checkedBytes == 0) || (bytesToBeChecked == 0))
                            {
        ...
        {
            byte[] dataBuffer = null;
            byte[] requestBuffer = IEC61850Protocol.BuildReadRequest(szDataSet, szDeviceID, RequestTypes.ReadDataSetDirectory, GetNextInvokeID());
            if ((requestBuffer == null) || (requestBuffer.Length < 1))
            {
                //System.Diagnostics.Debug.WriteLine("IEC61850 DEBUG - ReadDataSetDirectory - Error in BuildReadRequest");
                return (null);
                    ...
                    case 0xa1: // Confirmed Response
                        {
                            uint dataBufferLength = 0;
                            // To be modified
                            checkedBytes = IEC61850Protocol.CheckMMSGetNamedVarListAttribReply(replyMessage, totalCheckedBytes, bytesToBeChecked, out dataBufferLength);
                            totalCheckedBytes += checkedBytes;
                            bytesToBeChecked -= checkedBytes;
                            if ((checkedBytes == 0) || (bytesToBeChecked == 0))
                            {
        ...
        {
            byte[] dataBuffer = null;
            byte[] requestBuffer = IEC61850Protocol.BuildReadRequest(mmsReportID, deviceID, RequestTypes.ReadValue, GetNextInvokeID());
            if ((requestBuffer == null) || (requestBuffer.Length < 1))
            {
                                        ...
                                        case 0xa1: // Confirmed Response
                                            {
                                                ...
                                                checkedBytes = IEC61850Protocol.CheckMMSConfirmedReadResponse(replyMessage, totalCheckedBytes, bytesToBeChecked, out mmsDataLength);
                                                totalCheckedBytes += checkedBytes;
                                                bytesToBeChecked -= checkedBytes;
                                                if ((checkedBytes == 0) || (bytesToBeChecked == 0))
                                                {
                                                ...
                                                uint padding = 0;
                                                if (!IEC61850Protocol.ParseMMSData(receivedMessage, out dataType, out dataBufferLength, out dataOffset, out padding))
                                                {
        ...
        {
            byte[] requestBuffer = IEC61850Protocol.BuildReadRequest(String.Empty, String.Empty, RequestTypes.ReadIdentify, GetNextInvokeID());
            if ((requestBuffer == null) || (requestBuffer.Length < 1))
            {
                            ...
                            switch (ppduType)
                            {
                                case 0x61: // Fully encoded data
                                    ...
                                    switch (mpduType)
                                    {
                                        case 0xa1: // Confirmed Response
                                            {
                                                checkedBytes = IEC61850Protocol.CheckMMSIdentifyResponse(replyMessage, totalCheckedBytes, bytesToBeChecked);
                                                totalCheckedBytes += checkedBytes;
                                                bytesToBeChecked -= checkedBytes;
                                                if (checkedBytes == 0)
                                                {
        ...
        bool ConnectCOTP(IEC61850Station station)
        {
            ...
            byte[] cotpRequestBuffer = IEC61850Protocol.BuildCOTPConnectionRequest(station);
            ...
            // Add the ISO header
            byte[] isoRequestBuffer = IEC61850Protocol.AddISOHeader(cotpRequestBuffer);
        ...
        byte[] ReadDeviceCompleteReply()
        {
            if (doBeginDeviceRead == false)
            {
                ...
                //if (NewDataToAnlyze.WaitOne(Timeout) || ReceiveBuffer.Count > 0)
                lock (lockThreadObject)
                {
                    newDataToAnlyze = NewDataToAnlyze;
                }
                doBeginDeviceRead = (newDataToAnlyze.WaitOne(Timeout) || ReceiveBuffer.Count > 0);
                if (doBeginDeviceRead)
        ...
        {            
            ...
            lock (lockThreadObject)
            {
                ...
                {
                    isoHeader = new byte[4];                    
                    Array.Copy(ReceiveBuffer.ToArray(), isoHeader, 4);
                    ReceiveBuffer.Clear();
                }
            }
        ...
        public bool WriteRCBItem(string szDeviceID, string szMMSDataItem, MMSDataTypes nItemType, byte[] pDataBuffer, byte nPadding /*= 0*/) {
            ...
            byte[] requestBuffer = IEC61850Protocol.BuildWriteRCBItemRequest(szMMSDataItem, szDeviceID, nItemType, pDataBuffer, nPadding, GetNextInvokeID());
            if ((requestBuffer == null) || (requestBuffer.Length < 1))
            {
            ...
            while (!bReplyReceived && (numberOfConsecutiveUnsolicitedMessages < maxNumberOfConsecutiveUnsolicitedMessages))
            {
                ...
                {
                    case 0xa1: // Confirmed Response
                        checkedBytes = IEC61850Protocol.CheckMMSWriteReply(replyMessage, totalCheckedBytes, bytesToBeChecked);
                        totalCheckedBytes += checkedBytes;
                        bytesToBeChecked -= checkedBytes;
                        if ((checkedBytes == 0) || (bytesToBeChecked == 0))
                        {
                        ...
                        uint nErrorCode = 0;
                        if (!IEC61850Protocol.CheckMMSWriteResult(replyMessage, totalCheckedBytes + checkedBytes, bytesToBeChecked - checkedBytes, out nErrorCode))
                        {
        ...
        private int GetReceiveBufferCount()
        {
            lock (lockThreadObject)
        ...
        /// </summary>
        /// <returns></returns>
        public bool SetIec61850Station()
        {
            List<Station> st = CommDriver.GetChannelStations(this);
            if (st != null && st.Count > 0)            
                iec61850Station = (IEC61850Station)st[0];

            return (iec61850Station != null);
        }

        public bool SyncronizeWriteWithBeginDeviceRead()
        {
            DriverErrorCodes conn = DriverErrorCodes.ErrorNoError;
            if (!IsBeginDeviceReadPending())
