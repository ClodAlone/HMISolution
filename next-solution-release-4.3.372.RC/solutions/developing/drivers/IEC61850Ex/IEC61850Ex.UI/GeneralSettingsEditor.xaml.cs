    ...
    public partial class GeneralSettingsEditor : UserControl, IDisposable
    {
        ...
        private void ReadSettings()
        {
            ...
            if (!configurationEditor.IsValid)
                configurationEditor.Init<IEC61850DriverSettings>(comSetContext.ConnectionString, DriverName, comSetContext.bProtected, comSetContext.protectionCode);
        ...
        private void SetCurrentData()
        {
            ReadSettings();
            DataContext = null;
            DataContext = configurationEditor.Configuration as IEC61850DriverSettings;
            if (configurationEditor.Configuration is IEC61850DriverSettings && (configurationEditor.Configuration as IEC61850DriverSettings).ChannelSettings != null)
            {
                cht = new DriverCodeBaseEx.UI.Controls.BaseChannelTree() { DataContext = configurationEditor.Configuration as IEC61850DriverSettings };
            ...
            baseGeneralSetting.AggregationLimitText.Visibility = Visibility.Collapsed;
            baseGeneralSetting.AggregationLimit.Visibility = Visibility.Collapsed;
            baseGeneralSetting.AggregationLimitErr.Visibility = Visibility.Collapsed;
            baseGeneralSetting.AggregationThresholdText.Visibility = Visibility.Collapsed;
            baseGeneralSetting.AggregationThreshold.Visibility = Visibility.Collapsed;
            baseGeneralSetting.AggregationThresholdErr.Visibility = Visibility.Collapsed;
            baseGeneralSetting.DataContext = DataContext;
        ...
        {
            if (!(cht.GetSelecteItem() is IEC61850StationSettings))
                return;

            var stationSettings = cht.GetSelecteItem() as IEC61850StationSettings;
            var channelSettings = cht.GetSelectedChannel() as IEC61850ChannelSettings;

            using (IEC61850Driver driver = new IEC61850Driver(comSetContext.ConnectionString))
            {
