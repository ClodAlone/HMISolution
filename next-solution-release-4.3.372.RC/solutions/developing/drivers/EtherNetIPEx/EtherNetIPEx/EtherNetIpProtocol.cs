   ...
   // Constructor:
    ...
    public class EtherNetIpProtocol
    {
        ...
        public static void Logix5550GetCpuInformation(EtherNetIPStation s, ref byte[] pdu, ref ushortUnion pduPointer, EtherNetIPChannel c)
        {
            ...
            if (!c.EtherNetIpDeviceWrite(ref pdu, ref pduPointer))
            {
                EtherNetIPDriver.Addinfo(Properties.Resources.ErrorSTSCode05);
                return;
            }
            ...
            if (c.AnalyzeReplyHeader(ref pdu, ecSendRRData.USHORT, c.SessionHandle.UINT) !=
                (EtherNetIpErrorCodes)DriverErrorCodes.ErrorNoError)
            {
                EtherNetIPDriver.Addinfo(Properties.Resources.ErrorAnalyzeTheReplyHeader);
                return;
            }
            ...
            {
                EtherNetIPDriver.Addinfo(Properties.Resources.ErrorCheckTheReplyServiceCode);
                return;
            }
            ...
            if (wStatus.USHORT != 0)
            {
                EtherNetIPDriver.Addinfo(Properties.Resources.ErrorCheckTheStatus);
                return;
            }
            ...
            {
                EtherNetIPDriver.Addinfo(Properties.Resources.ErrorDataParsing);
                return;
            }

            s.FirmwareVersion = pdu[MR_SVC_REPLY_CODE_OFFS + 10];
            s.FirmwareVersion_Minor = pdu[MR_SVC_REPLY_CODE_OFFS + 11];
            ...
            if (eDataLen.USHORT >= (35 + nProductDescrLength))
            {
                ...
                s.ProducName = szProductDescription;
                Int32 serial = BitConverter.ToInt32(pdu, MR_SVC_REPLY_CODE_OFFS + 14);
     
                Byte status1 = pdu[MR_SVC_REPLY_CODE_OFFS + 12];
                Byte status2 = pdu[MR_SVC_REPLY_CODE_OFFS + 13];

                EtherNetIPDriver.Addinfo(string.Format(Properties.Resources.PLCProductName, s.ProducName));
                EtherNetIPDriver.Addinfo(string.Format(Properties.Resources.PLCFirmware, s.FirmwareVersion, s.FirmwareVersion_Minor));
                EtherNetIPDriver.Addinfo(string.Format(Properties.Resources.PLCSerialNumber, serial));
                EtherNetIPDriver.Addinfo(string.Format(Properties.Resources.PLCDeviceStatusField, status2, status1));
                ...
                station.PlcType == PlcTypes.ControlLogix_CompactLogix)
            {
                ...
                switch (station.PlcType)
                {
                    case PlcTypes.SLC500_MicroLogix:
                        ...
                        // bit reading optimization in 16 / 32bit data areas
                        if (job.CommandType == CommandTypes.ReadCmd && job.DataFormat == DataFormats.BIT && job.TagsList.Count> 0 && (job.FileType == FileTypes.Integer || job.FileType == FileTypes.LongInteger))
                        {
                            uint jobByteSize = job.TotalJobSize / 8;                            
                            switch (job.FileType)
                            {
                                case FileTypes.Integer:
                                    if (jobByteSize == 0 || jobByteSize % 2 != 0)
                                        jobByteSize += (2 - jobByteSize % 2);
                                    break;
                                case FileTypes.LongInteger:
                                    if (jobByteSize == 0 || jobByteSize % 4 != 0)
                                        jobByteSize += (4 - jobByteSize % 4);
                                    break;
                            }
                            pdu[pduPointer.USHORT++] = (byte)jobByteSize;
                        }
                        else
                        {
                            //For arrays of LongInteger type booleans, 
                        ...
                        {
                            //Element Number
                            if (job.CommandType == CommandTypes.WriteCmd && job.DataFormat == DataFormats.BIT)
                            {
                                if (job.TagsList.Count == 1)
                                    writeUshort((ushort)job.Element, ref pdu, ref pduPointer);
                                else
                                    writeUshort((ushort)job.GetFirstElementFromLastTagToWrite(), ref pdu, ref pduPointer);
                            }
                            else
                            {
                        ...
                        else if (job.IsFileTypeIO())
                        {
                                ...
                                (job.DataFormat == DataFormats.BIT))
                            {
                                ...
                                WordNumber = job.Word + (ushort)((job.GetNrBitsFromInitialAddressFrom1stTagToWrite() + offset) >> 4);
                            }
                            else
                            {
                        ...
                        else
                        {
                            ...
                            else if (!((job.DataFormat == DataFormats.BIT) && (job.GetLastBitNrFromLastTagToWrite() > 15)))
                            { 
                                 pdu[pduPointer.USHORT++] = 0;
                            }
                            else
                        ...
                        if (job.CommandType == CommandTypes.WriteCmd)
                        {
                            ...
                            if ((job.DataFormat == DataFormats.BIT) && (job.TagsList[0].TagNode.ArrayDimension < 17))
                            {
                                ...
                                {
                                    if (job.TagsList.Count > 0 && (job.FileType == FileTypes.Integer || job.FileType == FileTypes.LongInteger))
                                        BitToWrite = (byte)((job.GetFirstBitNrFromLastTagToWrite()) % 16);
                                    else
        ...
        public static void MicroLogixGetCpuInformationDP1(EtherNetIPStation s, EtherNetIPChannel c,ref DriverErrorCodes conn)
        {
            s.FirmwareVersion = 0;
            s.CpuModel = 0;
            ...
            // Ethernet/IP Encapsulation Header
            PrepareEncapsulationHeaderMicroLogix(ecSendRRData, c, ref requestBuffer, ref requestBufferPointer);
           
            MicroLogixNonBlockPhAddPrepareGetCpuTypeRequest(c, s, ref requestBuffer, ref requestBufferPointer);

            c.ReceiveClear();
            // Send the request
            if (!c.EtherNetIpDeviceWriteNoInsertLength(ref requestBuffer, ref requestBufferPointer))
            {
                EtherNetIPDriver.Addinfo(Properties.Resources.ErrorSTSCode05);
                return;
            }
            ...
            // First read the fixed-size header
            if (c.DeviceRead(requestBuffer, ENCAPSULATION_HEADER_SIZE))
            {
                // Read the variable size data part
                ushortUnion DataLen = new ushortUnion(requestBuffer, EDATA_LEN_OFFS);
                if (DataLen.USHORT > 0)
                {
                    // Read the variable size data part
                    byte[] pduData = new byte[DataLen.USHORT];
                    ...
                    pduData.CopyTo(requestBuffer, ENCAPSULATION_HEADER_SIZE);
                }
            ...
            // Analyze the reply header
            if (c.AnalyzeReplyHeader(ref requestBuffer, ecSendRRData.USHORT, c.SessionHandle.UINT) !=
                (EtherNetIpErrorCodes)DriverErrorCodes.ErrorNoError)
            {
                EtherNetIPDriver.Addinfo(Properties.Resources.ErrorAnalyzeTheReplyHeader);
                return;
            ...
            // Check the reply service code
            ushortUnion eDataLen = new ushortUnion(requestBuffer, EDATA_LEN_OFFS ) ;
            eDataLen.USHORT += 24;
            if (eDataLen.USHORT < 20)
            {
                EtherNetIPDriver.Addinfo(Properties.Resources.ErrorCheckTheReplyServiceCode);
            ...
            if (requestBuffer[MR_SVC_REPLY_CODE_OFFS] != 0xCB)
            {
                EtherNetIPDriver.Addinfo(Properties.Resources.ErrorCheckTheReplyServiceCode);
                return;
            }
            ...
            // Check the status
            byte wStatus = requestBuffer[ MR_SVC_REPLY_CODE_OFFS + 12];
            if (wStatus != 0)
            {
                EtherNetIPDriver.Addinfo(string.Format("{0} {1}", Properties.Resources.ErrorCheckTheStatus, wStatus));
                return;
            }
            ...
            // Get the product description
            byte nProductDescrLength = 11;
            if (eDataLen.USHORT >= (MR_SVC_REPLY_CODE_OFFS + 19 + nProductDescrLength))
            {
                byte[] auxArray = new byte[nProductDescrLength];
                Array.Copy(requestBuffer, MR_SVC_REPLY_CODE_OFFS + 20, auxArray, 0, nProductDescrLength);
                String szProductDescription = ASCIIEncoding.ASCII.GetString(auxArray);
                s.ProducName = szProductDescription;
        ...
        public static void PrepareEncapsulationHeaderMicroLogix(ushortUnion Cmd, EtherNetIPChannel c, ref byte[] pdu, ref ushortUnion pduPointer)
        {
            pduPointer.USHORT = 0;
        ...
        public static void MicroLogixNonBlockPhAddPrepareGetCpuTypeRequest(EtherNetIPChannel c, EtherNetIPStation s, ref byte[] pdu, ref ushortUnion pduPointer)
        {
            ...
            //the length field
            ushort DataLen = pduPointer.USHORT++;
            pduPointer.USHORT++;
            ...
            pdu[pduPointer.USHORT++] = 0x07;
            //CIP Vendor ID: 0x004d
            pdu[pduPointer.USHORT++] = 0x4d;
            pdu[pduPointer.USHORT++] = 0x00;
            //CIP Serial Number: 0x3a376d17
            pdu[pduPointer.USHORT++] = 0x17;
            pdu[pduPointer.USHORT++] = 0x6d;
            pdu[pduPointer.USHORT++] = 0x37;
            pdu[pduPointer.USHORT++] = 0x3a;
            //Command Code: 0x06
            pdu[pduPointer.USHORT++] = 0x06;
            //Status: Success (0x00)
            pdu[pduPointer.USHORT++] = 0x00;

            pdu[pduPointer.USHORT++] = s.GetTransaction().LOBYTE;       // TNS
            pdu[pduPointer.USHORT++] = s.GetTransaction().HIBYTE;      // TNS

            //Function Code: Diagnostic status (0x03)
            pdu[pduPointer.USHORT++] = 0x03;

            ushortUnion value = new ushortUnion();
            //Subtract 2 bytes from the length field   
            value.USHORT = (ushort)(pduPointer.USHORT - DataLen  - 2);
            pdu[DataLen ] = value.LOBYTE ;// LOBYTE(wAux);
            pdu[DataLen + 1] = value.HIBYTE; // (wAux);

        }

        /// Get attribute PLC Micro 800
        public static bool Logix5000V21GetTagInstances800(EtherNetIPStation s, ref byte[] pdu, ref ushortUnion pduPointer, EtherNetIPChannel c)
        {
            s.FirmwareVersion = 0;
            s.CpuModel = 0;

            if (EtherNetIpProtocol.Logix5550ForwardOpen(s, ref pdu, ref pduPointer, c))
            {

                // Prepare the request message
                Logix5000V21PrepareGetAttrubutAll800(c, s, ref pdu, ref pduPointer, new ushortUnion(01));
                c.ReceiveClear();
                ...
                if (pdu[LOGIX5550_REP_TNS_OFFS + 2] != 0x81)
                {
                    Logix5550ForwardClose(s, ref pdu, ref pduPointer, c);
                    return false;
                }
                ...
                // Check the status
                ushortUnion wStatus = new ushortUnion(pdu, LOGIX5550_REP_GENSTS_OFFS);
                ushort  inStatus = wStatus.USHORT;
                if (inStatus != 0)
                {
                    return false;
                }
                ...
                uintUnion uintAux = new uintUnion(pdu, LOGIX5550_REP_GENSTS_OFFS + 8);
                uint nAddress = uintAux.UINT;

                byte[] auxArray = new byte[4];
                Array.Copy(pdu, LOGIX5550_REP_GENSTS_OFFS + 22, auxArray, 0, 4);

                s.FirmwareVersion = pdu[LOGIX5550_REP_TNS_OFFS + 12];
                s.FirmwareVersion_Minor = pdu[LOGIX5550_REP_TNS_OFFS + 13];
                // Get the product description
                byte nProductDescrLength = pdu[LOGIX5550_REP_TNS_OFFS + 20];
                if ( nProductDescrLength != 0x00)
                {
                    auxArray = new byte[nProductDescrLength];
                    Array.Copy(pdu, LOGIX5550_REP_TNS_OFFS + 21, auxArray, 0, nProductDescrLength);
                    String szProductDescription = ASCIIEncoding.ASCII.GetString(auxArray);
                    s.ProducName = szProductDescription;
                    Int32 serial = BitConverter.ToInt32(pdu, MR_SVC_REPLY_CODE_OFFS + 14);
        ...
        public static ushort Logix5000V21PrepareGetAttrubutAll800(EtherNetIPChannel c, EtherNetIPStation s, ref byte[] pdu, ref ushortUnion pduPointer,
                                                              ushortUnion nInstance)
        {
            s.LastTransOK = true;
            // Add the header
            ...
            // Fill in packet data length
            ushortUnion wAux = new ushortUnion((ushort)(nLength));
            pdu[EDATA_LEN_OFFS] = wAux.LOBYTE;
            pdu[EDATA_LEN_OFFS + 1] = wAux.HIBYTE;
        ...
        //public static void Logix5550NonBlockPhAddPrepareGetCpuTypeRequest800(EtherNetIPChannel c, EtherNetIPStation s, ref byte[] pdu, ref ushortUnion pduPointer)
        //{
        //    s.LastTransOK = true;
        //    // Ethernet/IP Encapsulation Header
        //    PrepareEncapsulationHeader(ecSendRRData, c, ref pdu, ref pduPointer);
        ...
        //    // Common Industrial Protocol request
        //    ushortUnion nDataLen = new ushortUnion(Logix5550NonBlockPhAddPrepareGetCpuTypeCPF800(s, ref pdu, ref pduPointer));
        ...
        //}
        //public static ushort Logix5550NonBlockPhAddPrepareGetCpuTypeCPF800(EtherNetIPStation s, ref byte[] pdu, ref ushortUnion pduPointer)
        //{
        //    ushort nStartCh = pduPointer.USHORT;
        ...
        //    ushortUnion nPDULen = new ushortUnion(Logix5550NonBlockPhAddPrepareGetCpuTypeTPDU800(s, ref pdu, ref pduPointer));
        ...
        //public static ushort Logix5550NonBlockPhAddPrepareGetCpuTypeTPDU800(EtherNetIPStation s, ref byte[] pdu, ref ushortUnion pduPointer)
        //{
        //    ushort nStartCh = pduPointer.USHORT;
        ...
        //    // Message Request
        //    ushortUnion nMsgLen = new ushortUnion(Logix5550NonBlockPhAddPrepareGetCpuTypeMsgRequest800(ref pdu, ref pduPointer));
        ...
        //    return (ushort)(pduPointer.USHORT - nStartCh);
        //}
        //public static ushort Logix5550NonBlockPhAddPrepareGetCpuTypeMsgRequest800(ref byte[] pdu, ref ushortUnion pduPointer)
        //{
        //    ushort nStartCh = pduPointer.USHORT;
