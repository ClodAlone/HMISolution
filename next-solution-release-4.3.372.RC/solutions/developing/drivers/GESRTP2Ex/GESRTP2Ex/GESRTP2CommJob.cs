...
namespace GESRTP2
{
    ...
    public class GESRTP2CommJob : CommJob
    {
        ...
        private void CheckJobValidDataArea()
        {
            ...
            if (!(IsStructAtomic() || IsStructMixed()))
            {
            ...
            // set the offset of each tag of mixed struct to parse quickly data
            if (IsStructMixed())
                MixedStructBufferOffset();            
        ...
        public override void SetJobData(object jobData, ref List<Tag> changed)
        {
            ...
            lock (lockListObject)
            {
                ...
                {
                    if (IsStructMixed())
                        ((GESRTP2Tag)TagsList[TagIndex]).BackUpOffset();
                    ...
                    if (IsStructMixed())
                        ((GESRTP2Tag)TagsList[TagIndex]).RestoreOffset();
                }
        ...
        /// Set the offset of each tag of mixed struct to parse quickly data
        /// </summary>
        private void MixedStructBufferOffset()
        {            
            Tag startTag = TagsList[0];
            for (int i = 0; i < TagsList.Count; i++)
            {
                Tag endTag = TagsList[i];

                ((GESRTP2Station)Station).GetStructMixedTagByteStartAddressAndSize(startTag, out uint startTagAddress, out uint startTagSize);
                ((GESRTP2Station)Station).GetStructMixedTagByteStartAddressAndSize(endTag, out uint endTagAddress, out uint endTagSize);

                ((GESRTP2Tag)endTag).StructMixedBufferByteOffset = (int)(endTagAddress - startTagAddress);
                ((GESRTP2Tag)endTag).StructMixedBufferByteSize = (int)endTagSize;
            }            
        }
        ...
        {
            return (candTag != null && candTag.TagNode.DataType.IdType == IdType.Guid && !((GESRTP2DynTagSettings)candTag.DynSettings).StructMixed);
        }

        public bool IsStructMixed(Tag defTag = null)
        {
            if (defTag == null)
                defTag = StructTag;

            return (defTag != null && defTag.TagNode.DataType.IdType == IdType.Guid && ((GESRTP2DynTagSettings)defTag.DynSettings).StructMixed);
        }

        public override bool IsCustomJob(Tag defTag)
        {
            return (IsStructAtomic(defTag) || IsStructMixed(defTag));
        }

        public override void CreateChildJobsRead(Tag defTag, List<Tag> membersList)
        {
            if (IsStructMixed(defTag))
            {
                SetAsACustomJob();
                ...
                string dynamicSettings = ((GESRTP2DynTagSettings)(membersList[0]).DynSettings).ToString();
                
                // create an array with data type compatible with data area type
                uint areaDataType = GESRTP2Protocol.GetOpcUaDataTypeFromBuiltType(GESRTP2Protocol.DataType(((GESRTP2DynTagSettings)(membersList[0]).DynSettings).AreaType));

                // defTag.Size is always in byte
                int arrayDimension = Math.DivRem((int)defTag.Size, GESRTP2Protocol.GetAreaDataSize(((GESRTP2DynTagSettings)(membersList[0]).DynSettings).AreaType), out int rest);
                if (rest != 0)
                    arrayDimension++;

                NodeId dt = new NodeId(areaDataType);
                
                NamedTag sourcevar = new NamedTag() { ArrayDimension = (uint)arrayDimension, Name = "tagToRead", DataType = dt, NodeId = new NodeId(Guid.NewGuid()) };
                ...
                //NodeId dt = Opc.Ua.DataTypes.GetDataTypeId(sourcevar.DataType.Identifier);
        ...
        private Dictionary<NodeId, CommJob> CreateChildJobBigArray(Tag defTag)
        {
            ...
            Tag ntag = Station.GetCommDriver().CreateTag(new TagDefinition
            {
                ...
                DynamicSettings = defTag.DynSettings.ToString(),
                DataType = defTag.TagNode.DataType,
                SamplingInterval = defTag.SamplingInterval,
                ArrayDimension = defTag.TagNode.ArrayDimension,
        ...
        public override void CreateChildJobsWrite(Tag defTag, List<Tag> membersList)
        {
            if (IsStructMixed(defTag))
            {
                ...
                foreach (var member in membersList)
                {
                    ...
                    {
                        // use a mew nodeid to avoid to reuse an existing tag and update Tag.ByteOffset and Tag.BitOffset
                    ...
                    Tag candTag = Station.GetCommDriver().CreateTag(memberDefinition);
                    if (!candTag.bIsValid)
                        continue;

                    CommJob candJob = Station.CreateJob(candTag);
                    ...
                    foreach (Tag tag in candJob.TagsList)
                        ChildJobsWrite[tag.TagNode.NodeId] = candJob;                    
                }                
            }
        }

        protected override void CreateCustomJob(Tag defTag)
        {
            // when job is crated from .DynJobSetting fill (create) internal prototypes using tag list
            if (IsStructMixed())
                ((GESRTP2Driver)Station.GetCommDriver()).CreateInternalPrototypeFromTagsList(StructTag.TagNode.NodeId, this.TagsList);
            
            base.CreateCustomJob(defTag);            
        }

        public override bool IsJobAggregable()
        {
            return (!GESRTP2Protocol.IsSymbolic(this));
        }
