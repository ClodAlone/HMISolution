...
namespace Databoom
{
    ...
    //    ErrorStationID,
    //    ErrorFunctionCode,
    //    ErrorProtocolIllegalFunction = DataboomProtocol.PROTOCOL_ERROR + 1,
    //    ErrorProtocolIllegalDataAddress = DataboomProtocol.PROTOCOL_ERROR + 2,
    //    ErrorProtocolIllegalDataValue = DataboomProtocol.PROTOCOL_ERROR + 3,
    //    ErrorProtocolSlaveDeviceFailure = DataboomProtocol.PROTOCOL_ERROR + 4,
    //    ErrorProtocolAcknowledge = DataboomProtocol.PROTOCOL_ERROR + 5,
    //    ErrorProtocolSlaveDeviceBusy = DataboomProtocol.PROTOCOL_ERROR + 6,
    //    ErrorProtocolMemoryParityError = DataboomProtocol.PROTOCOL_ERROR + 8,
    //    ErrorProtocolGatewayPathUnavailable = DataboomProtocol.PROTOCOL_ERROR + 10,
    //    ErrorProtocolGatewayTargetFailResponse = DataboomProtocol.PROTOCOL_ERROR + 11
    //}
    ...
    public class DataboomProtocol
    {
        ...
        public uint PrepareRequest(DataboomCommJob job, ref byte[] buffer)
        {
            return (0);
        }
        
        static int ChecKAnswer(byte[] receivebuffer, DataboomCommJob job)
        {
        ...
        public static bool ParseData(string receivedValue, ref DataboomCommJob job, ref List<object> items)
        {
            List<Tag> changed = new List<Tag>();
            bool writeOperation = false;
            //if ((job.Status == DataboomCommJobStatus.PublishReplyReceived) ||
            //   (job.Status == DataboomCommJobStatus.PublishError))
            ...
            System.Diagnostics.Debug.WriteLine(String.Format("Databoom DBG - {0} ParseData: processed subscription data for var {1}",
        ...
        public static bool ParseData(byte[] receivebuffer, ref DataboomCommJob job, ref List<object> items)
        {
            List<Tag> changed = new List<Tag>();
