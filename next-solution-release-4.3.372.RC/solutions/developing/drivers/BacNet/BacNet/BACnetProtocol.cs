...
namespace BACnet
{
    ...
    public class ServiceTag : IPackable, IExpandable
    {
        ...
        public ServiceTag(BACnetCommJob job, byte contextTag,out bool noDataToWrite, out string error)
        {
            ...
            if (job.TagsListToWrite.Count == 0)
            {
                error = string.Format("job.TagsListToWrite.Count == 0 on ServiceTag");
                return;
            }
    ...
    public static class IPFrameFactory
    {
        ...
        public static WriteFormatResult confirmedWriteProperty(out BACnetIPFrame BACnetIPFrame, BACnetCommJob job, out string error)
        {
            error = string.Empty;
    ...
    public static class IPFrameValidator
    {
        ...
        public static bool IAm(BACnetIPFrame frame, ref BACnetObjectIdentifier outDeviceID,
            ref UInt32 outMaxAPDULength, ref uint outSegmentationSupported, ref UInt16 outVendorIdentifier, 
            ref bool DestinationSpecifierPresent, ref UInt16 DNET, ref byte DLEN, ref byte[] DADR )
        {
            if (!isIAm(frame))
                return false;

            outDeviceID = frame.Npdu.Apdu.ServiceRequest.List[0].ObjectID;
            outMaxAPDULength = frame.Npdu.Apdu.ServiceRequest.List[1].UInt;
            outSegmentationSupported = frame.Npdu.Apdu.ServiceRequest.List[2].Enumerated.Uint;
            outVendorIdentifier = (UInt16)frame.Npdu.Apdu.ServiceRequest.List[3].UInt;
            if(frame.Npdu.SourceSpecifierPresent)
            {
                DestinationSpecifierPresent = true;
                DNET = frame.Npdu.SNET;
                DLEN = frame.Npdu.SLEN;
                DADR = frame.Npdu.SADR;
            }
            else
        ...
        public static bool isIHave(BACnetIPFrame frame)
        {
        ...
        public static bool isIHave(BACnetIPFrame frame, BACnetObjectIdentifier DeviceID, string ObjectName)
        {
            if (!isIHave(frame))
                return false;
        ...
        public static bool IHave(BACnetIPFrame frame, BACnetObjectIdentifier DeviceID, string ObjectName,
            ref BACnetObjectIdentifier outObjectID)
        {
        ...
        public static bool UnconfirmedCovNotification(BACnetIPFrame frame, ref UInt32 outSubscriberID, ref BACnetObjectIdentifier outDeviceID,
            ref BACnetObjectIdentifier outObjectID, ref UInt32 outTimeRemaining, ref ServiceTagList TagList)
        {
        ...
        public static bool ConfirmedReadProperty(BACnetIPFrame frame, byte InvokeID, BACnetObjectIdentifier ObjectID,
            BACnetEnums.PropertyIdentifier property, ref byte[] answer, ushort ArrayIndex, BACnetCommJob job)
        {
            ...
            foreach (ServiceTag Tag in frame.Npdu.Apdu.ServiceRequest.List[indxTagValue].TagList.List)
            {
                switch (property)
                {
                    ...
                    case BACnetEnums.PropertyIdentifier.PRIORITY_ARRAY:
                        ...
                        {
                            //array element cannot be null --> set to 0 
                            if (job.TagsList[0].TagNode.ArrayDimension != 0)
                            {
                                byte[] array_priority_val = null;
                                int DataSize = (int)((BACnetDynTagSettings)job.TagsList[0].DynSettings).GetProtocolDataByteSize();
                                Array.Resize(ref array_priority_val, DataSize);
                                retVal.AddRange(array_priority_val);
                            } else
                            {
                                //leave value as null
                            }
