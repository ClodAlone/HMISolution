...
{
    public class DriverTcpExampleCommJob : CommJob
    {
                #region Constructors
        public DriverTcpExampleCommJob(Station station, DriverTcpExampleCommJobSettings settings)
            : base(station, settings)
        {
            _FunctionCode = settings.FunctionCode;
            _StartAddress = settings.StartAddress;
            DriverTcpExampleStation modbusStation = (DriverTcpExampleStation)station;
        ...
        public DriverTcpExampleCommJob(Station station, DriverTcpExampleTag defTag)
            : base(station, defTag)
        {
            _FunctionCode = defTag.DriverTcpExampleDynSettings.FunctionCode;
            _StartAddress = defTag.DriverTcpExampleDynSettings.StartAddress;
            DriverTcpExampleStation modbusStation = (DriverTcpExampleStation)station;
            AddressType = modbusStation.AddressType;
            _FileNumber = defTag.DriverTcpExampleDynSettings.FileNumber;
            _SwapDWords = defTag.DriverTcpExampleDynSettings.SwapDWords;

            _StringLength = defTag.DriverTcpExampleDynSettings.StringLength;
            _BroadCast = defTag.DriverTcpExampleDynSettings.BroadCast;

            if (TotalJobSize == 0)
                TotalJobSize = (uint)((defTag.DriverTcpExampleDynSettings.StringLength + 1) / 2) * 2;
        ...
        public DriverTcpExampleCommJob(Station station)
            : base(station)
        {
            DriverTcpExampleStation modbusStation = (DriverTcpExampleStation)station;
            AddressType = modbusStation.AddressType;
        ...
        protected DriverTcpExampleCommJob()
        {
            CheckJobValid();
        }
        #endregion
        ...
        {
            if (((Executioncode != (byte)ExecutionCodes.ForceSingleCoil) && 
                (Executioncode != (byte)ExecutionCodes.PresetSingleRegister) && 
                (Executioncode != (byte)ExecutionCodes.MaskWrite4xRegister)) || (TagsListToWrite.Count == 0))
            {
                base.ResetConditionalVariable();
            }
        }
        ...
        {
            return DriverTcpExampleProtocol.GetMaxJobSize(FunctionCode, Type);
        }
        ...
        public override JobAggregationType TestAggregateJob(CommJob candJob, out uint ExtraBytes)
        {
            ...
            DriverTcpExampleCommJob testJob = candJob as DriverTcpExampleCommJob;
            if (testJob == null)
        ...
        public JobAggregationType TestAggregateJobForMaskWriteRegister(DriverTcpExampleCommJob testJob, out uint ExtraBytes)
        {
            ExtraBytes = 0;
        ...
        public override bool AggregateJob(CommJob candJob, JobAggregationType AggType, uint ExtraBytes)
        {
            switch(AggType)
            {
                case JobAggregationType.JobAggregFits:
                    ...
                    {
                        var t = new DriverTcpExampleTag(tag.TagNode, tag.ByteOffset, 0);
                ...
                case JobAggregationType.JobAggregBackward:
                    ...
                    StartAddress = ((DriverTcpExampleCommJob)candJob).StartAddress;

                    if(FunctionCode == FunctionCodes.MaskWriteRegister)
                    {
                        ElementNumber = ((DriverTcpExampleCommJob)candJob).ElementNumber;
                    }
                    break;
                default:
                    return false;
            ...
            lock (lockListObject)
            {
                ...
                for (int TagIndex = 0; TagIndex < TagsList.Count; TagIndex++)
                {
                    if (TagsList[TagIndex].TagNode.DataType == Opc.Ua.DataTypes.Boolean)
                    {
                        if (isProtocolBool())
                        {
                            if (((DriverTcpExampleTag)(TagsList[TagIndex])).SetTagValue(ref rec, (int)TagsList[TagIndex].ByteOffset))
                        ...
                        else
                        {
                            ...
                            if (((DriverTcpExampleTag)(TagsList[TagIndex])).SetTagValue(ref tmpData, 0))
                                changed.Add(TagsList[TagIndex]);
                        }
                    }
                    else
                    {
                        if (isProtocolBool())
                        {
                            if (((DriverTcpExampleTag)(TagsList[TagIndex])).SetTagValue(ref rec, (int)TagsList[TagIndex].ByteOffset, (uint)(ElementNumber > 0 ? 1 : 0)))
                                changed.Add(TagsList[TagIndex]);
                        }
                        else
                        {
                            if (!ProtocolDataSizeBig())
                            {
                                ...
                                if (((DriverTcpExampleTag)(TagsList[TagIndex])).SetTagValue(ref rec, (int)TagsList[TagIndex].ByteOffset, elemsize))
                                    changed.Add(TagsList[TagIndex]);
                            }
                            else
                            {
