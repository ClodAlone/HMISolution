...
namespace EtherNetIP
{
    public class EtherNetIPCommJob : CommJob
    {
        ...
        public void InitEtherNetIPCommJob()
        {
        ...
        public bool IsDataToWriteChanged(Tag cand)
        {
            lock (lockListObject)
            {
        ...
        public ushort GetAsciiTagname(EtherNetIPStation s,ref byte[] pdu, ref ushortUnion pduPointer , Tag tag, string subName = "")
        {
                            ...
                            if (indexLength == 0)
                            {
                                ...
                                else
                                {
                                    if (nParentTemplateInstance > 0 && s.m_mapPlcTemplateInfo.ContainsKey(nParentTemplateInstance))
                                    {
                                        ...
                                        {
                                            writeArrayIndex(ref pdu, ref  pduPointer, Convert.ToUInt16(s.m_mapPlcTemplateInfo[nParentTemplateInstance].m_mapFieldInfo[match.Value].m_nIndex));
            ...
            //System.Diagnostics.Debug.WriteLine(String.Format("DBG REQ LENGTH - {0} - GetAsciiTagname - pduPointer.USHORT = {1} - sizeWords = {2} - returning = {3}", DateTime.UtcNow.ToString("HH:mm:ss.fff"), pduPointer.USHORT, sizeWords, pduPointer.USHORT - sizeWords));
        ...
        private void CheckJobValid()
        {
            ...
            if (((Station as EtherNetIPStation).PlcType != PlcTypes.SLC500_MicroLogix) &&
        ...
        public override JobAggregationType TestAggregateJob(CommJob candJob, out uint ExtraBytes)
        {
            ...
            foreach (var tag in TagsList)
            {
                ...
                uint endTag = startTag + (tag.Size / _ElemSize) - 1;
                ...
                if (cand != null)
                {
                    if ((cand.ByteOffset + cand.Size) != listToWrite[0].ByteOffset)
                    {
                        ...
                        if ((((EtherNetIPStation)Station).PlcType == PlcTypes.SLC500_MicroLogix) &&
                            ...
                            (((EtherNetIPTag)cand).EtherNetIPDynSettings.DataFormat == DataFormats.BIT) &&
                            (((EtherNetIPTag)cand).EtherNetIPDynSettings.ElementNumber == ((EtherNetIPTag)listToWrite[0]).EtherNetIPDynSettings.ElementNumber) &&
                            (((EtherNetIPTag)cand).EtherNetIPDynSettings.Word == ((EtherNetIPTag)listToWrite[0]).EtherNetIPDynSettings.Word) &&
                            (((EtherNetIPTag)cand).EtherNetIPDynSettings.Slot == ((EtherNetIPTag)listToWrite[0]).EtherNetIPDynSettings.Slot))
                        {
                            if ((((EtherNetIPTag)cand).EtherNetIPDynSettings.Bit > 15) && (((EtherNetIPTag)listToWrite[0]).EtherNetIPDynSettings.Bit < 16)||
                                (((EtherNetIPTag)cand).EtherNetIPDynSettings.Bit < 16) && (((EtherNetIPTag)listToWrite[0]).EtherNetIPDynSettings.Bit > 15))
                            {
                                break;
                            }
                        } 
        ...
        /// <summary>
        /// Move data from TagsListOnWriting (filled during preparerequest) to TagListToWrite (so next time will be written)
        /// </summary>
        public void MoveTagsFromListOnWritingToListToWrite() {
            lock (lockListObject)
            {
                if (TagsListOnWriting.Count > 0)
                {                    
                    TagsListOnWriting.ForEach((tag) =>
                    {
                        if (!TagsListToWrite.Contains(tag))
                            TagsListToWrite.Add(tag);
                    });
