...
namespace EIB
{
    ...
    public class EIBCommJob : CommJob
    {
            ...
            : base(station, settings)
        {
            ...
            _ErrorCode = (EIB_ERROR_CODES)DriverErrorCodes.ErrorNoError;
        ...
        public override void ManageUpdatedValueForTheConditionalVariable(DataValue value)
        {
            ...
            if (WriteCanBePerformed(true))
            {
                if (TagsList[0].Value.Value != null)
                {
                    ...
                    OnWriteTag(TagsList[0].TagNode.NodeId, ref WriteValue);
                }
            }
        ...
        public override void GetJobData(ref object jobData)
        {
            ...
            lock (lockListObject)
            {
                ...
                if (tagData != null)
                { 
                    if ((_DataFormat != (int)EISDATAFORMAT.EISDFBit)&&
                    (_DataFormat != (int)EISDATAFORMAT.EISDFByte)&&
                    (_DataFormat != (int)EISDATAFORMAT.EISDFEIS6))
                    {
                        if (SwapBytes)
                        {
                            SwapByteBuffer(ref tagData);
        ...
        public bool CheckAndUpdateNewData(ExecutedJobArgs eJob)
        {
            EIBChannel EibJobChannel = (EIBChannel)Station.GetChannel();
            if (EibJobChannel.mapEIBNewData.ContainsKey((UInt16)PollingGroupIntValue))
            {
                GroupValue e = EibJobChannel.mapEIBNewData[(UInt16)PollingGroupIntValue];
                eJob.Job = this;
                //eJob.ErrorCode = (DriverCodeBase.Enumerators.DriverErrorCodes)e.ErrorCode;
                eJob.Values = e.Value;
                return (true);
            }
        ...
        public bool MustSendARequest()
        {
            ...
            if (PollingCanBePerformed() && EIBCommJob.IsValidAddress(PollingGroup))
            {
                if(PollingIsForced())
                {
                    returnValue = true;
                ...
                {
                    double dtime = (DateTime.UtcNow - LastReadRequestTimeStamp).TotalMilliseconds;
                    if (dtime > PollingTime)
                    {
                        returnValue = true;
                    }
                }
            ...
            else if (WriteCanBePerformed() && EIBCommJob.IsValidAddress(OutputGroup))
            {
                ...
                else if (TagsListToWrite.Count > 0)
                {
                    returnValue = true;
                }
            }
        ...
        {
            return PollingCanBePerformed(false);
        }

        private bool PollingCanBePerformed(bool fromConditionalVar)
        {
            bool returnValue = false;

            if (fromConditionalVar)
            {   
                if (PollingIsForced() && TagsListToWrite.Count == 0)
                {
            ...
            else
            {
                ...
                if ((Type == LinkType.Input) ||
                    ((Type == LinkType.InputOutput) &&
                        (TagsListToWrite.Count == 0) && !WriteIsForced()))
                {
                    if (((PollingEnabled == true) && !PollingOnlyOnRequest) ||
                       ((conditionalVariableHasBeenSet == true) && (PollingIsForced() == true)))
                    {
        ...
        private bool WriteCanBePerformed(bool fromConditionalVar)
        {
            ...
            if (fromConditionalVar)
            {
                ...
                if (WriteIsForced())
                {
                    if ((Type != LinkType.Input) && ((!OutputOnlyOnRequest && TagsListToWrite.Count == 0)))
                        returnValue = true;
                }
            }
            else
            {
                // standard loop
                if ((Type != LinkType.Input) &&
                    ((!OutputOnlyOnRequest && (TagsListToWrite.Count > 0)) ||
                     WriteIsForced()))
                {
                    returnValue = true;
                }
        ...
        /// ErrorCode
        /// </summary>
        private EIB_ERROR_CODES _ErrorCode;
        public EIB_ERROR_CODES ErrorCode
        {
            get
            {
                return _ErrorCode;
