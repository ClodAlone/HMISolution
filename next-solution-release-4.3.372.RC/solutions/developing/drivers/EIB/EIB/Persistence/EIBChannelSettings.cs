...
namespace EIB
{
    ...
    public class EIBChannelSettings : ChannelSettings
    {
            ...
            : base(session)
        {
            ...
            session.UpdateSchema(typeof(EIBChannelSettings));
        }
        ...
        public void DefaultSettings()
        {
            ...
            _SerialCommPort = COMMPORT.Com1;
            _ConnectorType = ConnectorTypes.KnxIpRouting;
        ...
        public void CopyProperties(EIBChannelSettings ch)
        {
            base.CopyProperties(ch);
            MinBusInactivityTime = ch.MinBusInactivityTime;
            SerialCommPort = ch.SerialCommPort;
            ConnectorType = ch.ConnectorType;
            MulticastAddress = ch.MulticastAddress;
            EIBNetFriendlyName = ch.EIBNetFriendlyName;
            LocalHostName = ch.LocalHostName;
            EIBNetIpAddress = ch.EIBNetIpAddress;
            EIBNetTcpPort = ch.EIBNetTcpPort;
            ConnectionResetTime = ch.ConnectionResetTime;
        }
        ...
        protected override String PerformValidation(String propertyName)
        {
            ...
            switch (propertyName)
            {
                case "EIBNetIpAddress":
                    if (_ConnectorType == ConnectorTypes.KnxIpTunneling)
                    {
                        //validate IP address
