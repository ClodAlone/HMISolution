...
namespace IEC60870_5_104
{
    ...
    public class IEC60870_5_104Driver : CommunicationDriver
    {
        ...
        public IEC60870_5_104Driver(string strSettingPath)
            : base(strSettingPath)
        {
        }
        ...
        public bool TestDriver(string strProjectPath)
        {
                ...
                foreach (var ch in GetChannels())
                {
                    foreach (var st in GetChannelStations(ch))
                    {
                        TagDefinition tgdf = new TagDefinition()
                        {
                            ...
                            DynamicSettings = string.Format("IEC60870_5_104.Station={0}|LinkType=1|ASDU=1|IOAM=100|IOAC=100|QOC=0|QOP=0|CA=0|UTS=False|WTS=False", st.Name),
                            SamplingInterval = 0,
                            ArrayDimension = 0,
                            InitialValue = null,
                            MemberOrder = 0,
                            DataType = (int)BuiltInType.Boolean,
