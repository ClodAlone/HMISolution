...
namespace IEC60870_5_104
{
    ...
    public class IEC60870_5_104CommJob : CommJob
    {
        ...
        public void CheckIECState()
        {
            if (_IecState == IECStates.None)
            {
                if (((IEC60870_5_104Station)Station).IsInitialized())
                {
                    if (IsFileManagementASDUTypes())
                        _IecState = IECStates.FileManagement;
                    else
                        _IecState = IECStates.WaitData;
                }
                else
                {
                    System.Diagnostics.Debug.WriteLine("IEC60870-5.104-CheckIECState-->set IECStates.StationInitialization");
                    _IecState = IECStates.StationInitialization;
                }            
            }

            if (_IecState == IECStates.StationInitialization)
            {
                if (!((IEC60870_5_104Station)Station).IsInitialized())
                {
                    _IecState = ((IEC60870_5_104Station)Station).CheckInitializationState();
                }
                else
                {
                    _IecState = IECStates.WaitData;
                    System.Diagnostics.Debug.WriteLine("IEC60870-5.104-CheckIECState-->set IECStates.WaitData");
                }                
            }
         

            if (_IecState ==  IECStates.FileManagement)
            {
                if (IsFileManagementTerminated())
                    _IecState = IECStates.WaitData;
            }

            if (_IecState == IECStates.WaitData)
            {
                // do nothing 
            }
        }

        public bool IsIECJobState()
        {
            return (_IecState >= IECStates.FileManagement);
        }
        ...
        private bool IsFileManagementASDUTypes()
        {
            return (_ASDUType == ASDUSelectableTypes.UploadFile || _ASDUType == ASDUSelectableTypes.ReadDirectoryContents);
        }

        private bool IsFileManagementTerminated()
        {
            return ((IEC60870_5_104Channel)Station.GetChannel()).IsFileManagementTerminated();
        }
        ...
        public void SetIecNetStateInError()
        {
        ...
        private IECStates _IecState = IECStates.None;
        ////////////////////////////////////////////////////////////////////////////////////////////////////
        /// <summary>   Instance Number. </summary>
        ///
        /// <value> The Instance Number. </value>
        ////////////////////////////////////////////////////////////////////////////////////////////////////
        public IECStates IecState
        {
            get { return _IecState; }
            set { _IecState = value; }
        }
