...
namespace OmronEthernetIP
{
   ...
   // Constructor:
    ...
    public class OmronEthernetIPProtocol
    {
        ...
        //Position CRC
        private static ushort posByteCrc;
        //last CRC
        private static ushortUnion LastCrc;
        ...
        public static bool GetDeviceIdentity(ref byte[] pdu, ref ushortUnion pduIndex, OmronEthernetIPChannel c, OmronEthernetIPStation s, ref ushort vendorID, ref string productName, ref byte Major, ref byte Minor, ref uint serialNumber)
        {
            // Prepare the request message
            ...
            if (!ParseDeviceIdentityAttributes(ref pdu, lenData.USHORT, ref vendorID, ref productName, ref Major, ref Minor, ref serialNumber, s))
            {
                c.ReceiveClear();
                return (false);
            }
        ...
        private static bool ParseDeviceIdentityAttributes(ref byte[] pdu, ushort pduLength, ref ushort vendorID, ref string productName, ref byte Major, ref byte Minor, ref uint serialNumber, OmronEthernetIPStation s)
        {
            ...
            //Parse Major and Minor Revision
            Major = pdu[REPLY_CIP_SERVICE_OFFSET + 10];
            Minor = pdu[REPLY_CIP_SERVICE_OFFSET + 11];
            //Parse Serial Number
            uint[] array = new uint[1];
            Buffer.BlockCopy(pdu, ALL_ATTRIBUTES_MIN_LENGTH + 10, array, 0, 4);
        ...
        private static void PrepareRequest(ref byte[] pdu, ref ushortUnion pduIndex, OmronEthernetIPChannel c, OmronEthernetIPStation s, ref OmronEthernetIPChannel.RequestOfDevice request)
        {
            ...
            // Service: Get Attributes All
            if((request.Request != OmronEthernetIPChannel.CommandRequestOfDevice.GetTheTypeOfDeviceCJ2M) /*&&
                (request.Request != OmronEthernetIPChannel.CommandRequestOfDevice.GetNumberOfItemCJ2M)*/)
                pdu[pduIndex.USHORT++] = bServiceAttribute;
            else
                pdu[pduIndex.USHORT++] = 0x01;
            ...
            switch(request.Request)
            {
                ...
                case OmronEthernetIPChannel.CommandRequestOfDevice.GetTheTypeOfDeviceCJ2M:
                    {
                        // Path Size: 2 words
                        pdu[pduIndex.USHORT++] = 0x02;
                        ...
                        // Request Path: Identity, Instance 1
                        pdu[pduIndex.USHORT++] = 0x20; // Class Segment
                        pdu[pduIndex.USHORT++] = 0x01; // Identity

                        pdu[pduIndex.USHORT++] = 0x24; // Instance Segment
                        pdu[pduIndex.USHORT++] = 0x01; // Instance ID
                    }
                    break;
                //case OmronEthernetIPChannel.CommandRequestOfDevice.GetNumberOfItemCJ2M:
                //    {
                //        // Path Size: 2 words
                //        pdu[pduIndex.USHORT++] = 0x02;
        ...
        private static bool ParseTheDeviceResponse(ref byte[] pdu, ushort pduLength,  OmronEthernetIPStation s, ref OmronEthernetIPChannel.RequestOfDevice request)
        {
            ...
            if (request.Request == OmronEthernetIPChannel.CommandRequestOfDevice.GetTheTypeOfDeviceCJ2M)
            {
                request.DeviceType = Encoding.UTF8.GetString(pdu, 41, pdu[40]);
                return string.IsNullOrWhiteSpace(request.DeviceType)? false : true;
            }
        ...
        public static bool CheckDeviceIdentity(ushort vendorID, string productName)
        {
            ...
            if ((vendorID != OmronEthernetIPProtocol.OMRON_VENDOR_ID) || ((!productName.Contains("NJ") && (!productName.Contains("CJ2M")))))
            {
                return (false);
            }
            ...
            ref byte[] pdu, ref ushortUnion pduPointer)
        {
            ...
            foreach (OmronEthernetIPCommJob j in List)
            {
                ...
                {
                    // CIP Sequence Count:
                    //Attention that the reading must be done with a single Job,
                    //because no document describing a multiple request has been found.
                    pdu[OffsetIndex.USHORT++] = s.GetTransaction().LOBYTE;
                    pdu[OffsetIndex.USHORT++] = s.GetTransaction().HIBYTE;
                }
                ...
                OffsetValue.USHORT += j.GetWriteNumOfElements(ref pdu, ref pduPointer, tag, (s.PlcType == PlcTypes.CJ2M));
        ...
        public static ushort PrepareEncapsulatedFragmentRequest(ref byte[] pdu, ref ushortUnion pduIndex, ref byte[] fragmentBuffer, ushort fragmentBufferLength, bool isTheLastFragment, ref OmronEthernetIPFragmentInfo fragmentInfo)
        {
            ...
            if (fragmentInfo.FType == FragmentType.First)
            {
                ...
                // CRC
                OmronEthernetIPProtocol.posByteCrc = pduIndex.USHORT;
                pduIndex.USHORT++;
                pduIndex.USHORT++;
                LastCrc.USHORT = 0;
            ...
            LastCrc.USHORT = CRCCalculator(fragmentBuffer, LastCrc.USHORT);
            pdu[posByteCrc] = LastCrc.LOBYTE;
            pdu[posByteCrc + 1] = LastCrc.HIBYTE;
        ...
        //CRC16 ARC Calculation 
        public static ushort CRCCalculator(byte[] data, ushort star)
        {
            ushort crc = star;
