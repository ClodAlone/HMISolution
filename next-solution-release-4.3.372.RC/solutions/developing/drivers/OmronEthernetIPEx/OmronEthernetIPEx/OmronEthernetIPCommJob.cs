...
namespace OmronEthernetIP
{
    public class OmronEthernetIPCommJob : CommJob
    {
        ...
        {            
            ...
            ResetOmronEthernetIPCommJob();            
            answer = null;
            //ExecuteTask = false;
            //TerminateTask = false;
            TagNameFragmentedRequestBuffer = null;
        }

        public void ResetOmronEthernetIPCommJob()
        {
            _RequestCorrectDataTypeToPlcSequence = false;
            ReadTagStart = 0;
            ReadTagEnd = 0;
            PartialArrayStart = 0;
            PartialArrayEnd = 0;
        ...
        public byte[] TagNameFragmentedRequestBuffer;
        public int TagIndexFragmentedRequestSize = 0;
        public ushort TagIndexFragmentedRequest = 0;

        #endregion

        #region Methods

        public Tag GetFirstTagToWrite()
        {
            lock (lockListObject)
            {
                ...
                return TagsListOnWriting[0];
            }
        }
        public void MoveFromToWriteToOnWriting(Tag tag)
        {
            lock (lockListObject)
            {
        ...
        public ushort adjSizeWrite(Tag cand)
        {
            ...
            {
                tagSize = (ushort)Encoding.UTF8.GetBytes(cand.WriteVal.ToString()).Length;
            }
            else
                tagSize = (ushort)cand.Size;
        ...
        public ushort GetNumOfElements(ref byte[] pdu, ref ushortUnion pduPointer, Tag cand)
        {
...
#if DEBUG
            {
                ...
                System.Diagnostics.Debug.WriteLine(String.Format("OMRON EthernetIP DBG - {0} - GetNumOfElements - NumOfElements: {1} - PartialArrayStart: {2} - PartialArrayEnd: {3}",
                                                   currentTime, NumOfElements.USHORT, PartialArrayStart, PartialArrayEnd));
            }
#endif
        ...
        public ushort GetWriteNumOfElements(ref byte[] pdu, ref ushortUnion pduPointer, Tag cand, bool isCJ2PLC = false)
        {
            ushortUnion NumOfElements;
            ...
            {
                NumOfElements = new ushortUnion((ushort)Encoding.UTF8.GetBytes(cand.WriteVal.ToString()).Length);
            ...
            else
            {
                ...
                if (PartialArrayEnd == PartialArrayStart)
                {
                    if(ABAddress[ABAddress.Length - 1].Equals(']'))
                    {
                        NumOfElements = new ushortUnion((ushort)(adjSize(cand) / ElemSize));
                    }
                    else
        ...
        public ushort GetTagWriteData(ref byte[] pdu, ref ushortUnion pduPointer, Tag cand)
        {
            ...
            else if (isProtocolBool())
            {
                ...
                if ((DataFormat == DataFormats.BIT) && (TagsList[0].TagNode.ArrayDimension > 0) && (!ABAddress[ABAddress.Length - 1].Equals(']')))
                {
                    byte[] tmpData = new byte[(((ArraySize + 7) / 8 + ElemSize - 1) / ElemSize) * ElemSize];
                    for (int ArrayIndex = 0; ArrayIndex < ArraySize; ArrayIndex++)
                    {
            ...
            if ((TagFormat != TagFormats.BOOL) || ((ArraySize > 1) && (ABAddress[ABAddress.Length - 1].Equals(']'))))
            {
            ...
            else if ((DataFormat == DataFormats.BIT) && (TagsList[0].TagNode.ArrayDimension > 0) && (!ABAddress[ABAddress.Length -1].Equals(']')))
            {
                int i = 0;
        ...
        public ushort GetFragmentedAsciiTagname(ref byte[] pdu, ref ushortUnion pduPointer, Tag tag, string subName = "")
        {
            ushort sizeBytes = pduPointer.USHORT;
            if (TagNameFragmentedRequestBuffer == null)
            {
                // The length (two bytes) of the variable name specification will be added at the end of the method
                pdu[pduPointer.USHORT++] = 0;
                pdu[pduPointer.USHORT++] = 0;
                        ...
                        if (TagTreeNodeIndexMatch.Success)
                        {
                            ...
                            else
                            {
                                ...
                                if (TagTreeIndexMatch.Success)
                                {
                                    ...
                                    {
                                        int memIndex = pduPointer.USHORT;
                                        TagIndexFragmentedRequest = Convert.ToUInt16(TagTreeIndexMatch.Groups["index1"].Value);
                                        writeArrayIndex(ref pdu, ref pduPointer, TagIndexFragmentedRequest);
                                        TagIndexFragmentedRequestSize = pduPointer.USHORT - memIndex;
                                        if (TagTreeIndexMatch.Groups["index2"].Value.Length > 0)
                                        {
                                            memIndex = pduPointer.USHORT;
                                            TagIndexFragmentedRequest = Convert.ToUInt16(TagTreeIndexMatch.Groups["index2"].Value.Remove(0, 1));
                                            writeArrayIndex(ref pdu, ref pduPointer, TagIndexFragmentedRequest);
                                            TagIndexFragmentedRequestSize = pduPointer.USHORT - memIndex;
                                            if (TagTreeIndexMatch.Groups["index3"].Value.Length > 0)
                                            {
                                                memIndex = pduPointer.USHORT;
                                                TagIndexFragmentedRequest = Convert.ToUInt16(TagTreeIndexMatch.Groups["index3"].Value.Remove(0, 1));
                                                writeArrayIndex(ref pdu, ref pduPointer, TagIndexFragmentedRequest);
                                                TagIndexFragmentedRequestSize = pduPointer.USHORT - memIndex;
                                            }
                                        }
                ...
                TagNameFragmentedRequestBuffer = new byte[pduPointer.USHORT - sizeBytes];

                Array.Copy(pdu, sizeBytes, TagNameFragmentedRequestBuffer, 0, TagNameFragmentedRequestBuffer.Count());
            }
            else
            {
                Array.Copy(TagNameFragmentedRequestBuffer, 0, pdu, pduPointer.USHORT, TagNameFragmentedRequestBuffer.Count());
                pduPointer.USHORT += (ushort)TagNameFragmentedRequestBuffer.Count();
            }

            if (PartialArrayEnd != PartialArrayStart)
            {
                pduPointer.USHORT = (ushort)(pduPointer.USHORT - TagIndexFragmentedRequestSize);
                writeArrayIndex(ref pdu, ref pduPointer, (ushort)(PartialArrayStart + TagIndexFragmentedRequest));
            }
            ...
            // Add the length at the beginning of the variable name specification
            ushort variableNameLength = (ushort)(pduPointer.USHORT - sizeBytes - 2);
            pdu[sizeBytes] = (byte)variableNameLength;
            pdu[sizeBytes + 1] = (byte)(variableNameLength >> 8);

            return (ushort)(pduPointer.USHORT - sizeBytes);
        }
        ...
        public ushort GetTagFragmentedReadBufferSize(Tag tag, string subName = "")
        {
            ushort outValue = 0;
            if (TagNameFragmentedRequestBuffer == null)
            {
                outValue = GetFragmentedAsciiTagnameLength(tag, subName);
            }
            else
            {
                outValue = (ushort)TagNameFragmentedRequestBuffer.Count();
                if (!string.IsNullOrEmpty(subName))
                    outValue += (ushort)(((subName.Length + 3) / 2) * 2);
            }
        ...
        public ushort GetFragmentedAsciiTagnameLength(Tag tag, string subName = "")
        {
            ushort sizeBytes = 0;
            ushort tagIndexFragmentedRequest = 0;
            ushort tagIndexFragmentedRequestSize = 0;
            if (TagNameFragmentedRequestBuffer == null)
            {
                // The length (two bytes) of the variable name specification
                sizeBytes = 2;
                        ...
                        if (TagTreeNodeIndexMatch.Success)
                        {
                            ...
                            if (indexLength == 0)
                            {
                                if (level == 0)
                                {
                                    ...
                                    if (_ModuleName != string.Empty)
                                    {
                                        sizeBytes += GetSymbolSize(_ModuleName);
                                    }
                                    sizeBytes += GetSymbolSize(match.Value);
                                }
                                else
                                {
                                    sizeBytes += GetSymbolSize(match.Value);
                            ...
                            else
                            {
                                ...
                                sizeBytes += GetSymbolSize(match.Value.Remove(match.Value.Length - indexLength, indexLength));
                                Match TagTreeIndexMatch = TagTreeIndexValidator.Match(TagTreeNodeIndexMatch.Groups["index"].Value);
                                if (TagTreeIndexMatch.Success)
                                {
                                    if (TagTreeIndexMatch.Groups["index1"].Value.Length > 0)
                                    {
                                        int memIndex = sizeBytes;
                                        tagIndexFragmentedRequest = Convert.ToUInt16(TagTreeIndexMatch.Groups["index1"].Value);
                                        if (tagIndexFragmentedRequest > 0xff)
                                        {
                                            sizeBytes += 4;
                                        }
                                        else
                                        ...
                                        tagIndexFragmentedRequestSize = (ushort)(sizeBytes - memIndex);
                                        if (TagTreeIndexMatch.Groups["index2"].Value.Length > 0)
                                        {
                                            //writeArrayIndex(ref pdu, ref  pduPointer, Convert.ToUInt16(TagTreeIndexMatch.Groups["index2"].Value.Remove(0, 1)));
                                            memIndex = sizeBytes;
                ...
                if (_TagFormat == TagFormats.STRUCTURE)
                {
                    ...
                    while (!string.IsNullOrEmpty(Tree))
                    {
                        sizeBytes += GetSymbolSize(OmronEthernetIPProtocol.getOffTree(ref Tree));
                    }
                }
            }
            else
            {
                sizeBytes = (ushort)TagNameFragmentedRequestBuffer.Count();
            }

            if (PartialArrayEnd != PartialArrayStart)
            {
                sizeBytes = (ushort)(sizeBytes - tagIndexFragmentedRequestSize);
                if ((PartialArrayStart + tagIndexFragmentedRequest) > 0xff)
                {
                    sizeBytes += 4;
                }
                else
            ...
            if (!string.IsNullOrEmpty(subName))
            {
                sizeBytes += GetSymbolSize(subName);
            }

            return (sizeBytes);
        }

        public bool IsTooBigForSingleFragment()
        {
            if ((TagsList == null) || (TagsList.Count() < 1))
            {
                return (false);
            }

            OmronEthernetIPStation omronStation = (OmronEthernetIPStation)Station;
            // Size of the tag + Maximum overhead for the PDU of a fragment (16 bytes for the first fragment) + 2 bytes for the CIP_SEQUENCE_COUNT_SIZE
            if ((adjSize(TagsList[0]) + OmronEthernetIPProtocol.FRAGMENT_REQUEST_FIX_OVERHEAD + OmronEthernetIPProtocol.CIP_SEQUENCE_COUNT_SIZE) > omronStation.MaxPduSize)
            {
                return (true);
            }
        ...
        public bool ArrayIsRequestedWithItemIndex()
        {
            if ((TagsList[0].TagNode.ArrayDimension > 0) && (ABAddress.Last() == ']'))
            {
                return (true);
            }
            return (false);
        ...
        private bool _RequestCorrectDataTypeToPlcSequence;
        public bool RequestCorrectDataTypeToPlcSequence
        {
            get { return _RequestCorrectDataTypeToPlcSequence; }
            set { _RequestCorrectDataTypeToPlcSequence = value; }            
        }
