...
namespace OmronEthernetIP
{
    ...
    public class OmronEthernetIPChannel : TcpChannelList, IDisposable
    {
        ...
        private void CheckIfVariableMonitoringObjectIsSupported(List<CommJob> joblist)
        {
            ...
            if (!OmronEthernetIPProtocol.GetDeviceIdentity(ref requestBuffer, ref requestBufferPointer, this, s, ref vendorID, ref productName, ref Major, ref Minor, ref serialNumber))
            {
            ...
            if(TestConnection)
            {
                OmronEthernetIPDriver.Addinfo(string.Format(Properties.Resources.PLCFamily, productName.ToString()));
                OmronEthernetIPDriver.Addinfo(string.Format(Properties.Resources.PLCSerialNumber, serialNumber.ToString("X")));
                OmronEthernetIPDriver.Addinfo(string.Format(Properties.Resources.PLCRevision, Major, Minor));
            }
        ...
        public bool OmronEthernetIPForwardOpen(List<CommJob> list)
        {
            ...
            if ((s.PlcType == PlcTypes.NJ) || (s.PlcType == PlcTypes.NX) || (s.PlcType == PlcTypes.CJ2M))
            {
                    ...
                    if (j.Station.Name == station)
                    {
                        ...
                        if (write && jwrite)
                        {
                            if (OmronEthernetIPProtocol.getWriteListLimitate(ref j, ref frameSize, OmronEthernetIPProtocol.ReadWriteLimitate.EvaluateOnly))
                            {
                                ...
                                jobIndex--;
                                //The CJ2 PLC writes one variable at a time
                                if (((OmronEthernetIPStation)j.Station).PlcType == PlcTypes.CJ2M)
                                {
                                    break;
                                }
#if DEBUG
        ...
        private void ExecuteJobList(List<CommJob> list)
        {
            ...
            if (FragmentProtocolCanBeApplied(list)/* || (s.PlcType == PlcTypes.CJ2M)*/)
            {
        ...
        {
            //Using the CJ2M PLC the write does not include the Transaction in the request
            if (s.GetTransaction().USHORT == new ushortUnion(pdu, 33).USHORT)
            {
            ...
            else if (s.GetTransaction().USHORT == new ushortUnion(pdu, OmronEthernetIPProtocol.LOGIX5550_REP_TNS_OFFS).USHORT)
            {
                // when return false, all jobs are already set in error with OnJobExecuted(...);
                s.LastTransOK = Logix5550TagsCheckReply(ref pdu, s, list);
            }
        ...
        void CheckTheWritingResponse(ref byte[] pdu, List<CommJob> list, OmronEthernetIPStation s)
        {
            ...
            // Check the number of replied services
            //ushortUnion recieveReply = new ushortUnion(pdu, OmronEthernetIPProtocol.LOGIX5550_REP_SERNUM_OFFS);
            ushortUnion recieveReply;
            if(s.PlcType != PlcTypes.CJ2M)
            {
                recieveReply = new ushortUnion(pdu, OmronEthernetIPProtocol.LOGIX5550_REP_SERNUM_OFFS);
            }
            else
            {
                recieveReply = new ushortUnion(1);
            }
            for (int i = 0; i < recieveReply.USHORT; i++)
            {
                OmronEthernetIPCommJob j = list[i] as OmronEthernetIPCommJob;
                int cipErrorCode = (int) DriverErrorCodes.ErrorNoError;
                if (s.PlcType != PlcTypes.CJ2M)
                {
                    ushortUnion ReplyOffset = new ushortUnion(pdu, (ushort)(OmronEthernetIPProtocol.LOGIX5550_REP_SERNUM_OFFS + 2 + i * 2));
                    ReplyOffset.USHORT += OmronEthernetIPProtocol.LOGIX5550_REP_SERNUM_OFFS;

                    cipErrorCode = Logix5550TagsCheckSingleReply(ref pdu, list, ReplyOffset, j.CommandType);
                }
                else
                {
                    ushortUnion ReplyOffset = new ushortUnion(pdu, (ushort)(46 + 2 + 0 ));
                    //ReplyOffset.USHORT += OmronEthernetIPProtocol.LOGIX5550_REP_SERNUM_OFFS;
                    cipErrorCode = Logix5550TagsCheckSingleReplyCJ2M(ref pdu, list, ReplyOffset, j.CommandType);
                }
        ...
        private int Logix5550TagsCheckSingleReplyCJ2M(ref byte[] pdu, List<CommJob> list, ushortUnion ReplyOffset, CommandTypes CommandType)
        {
            int omronErrorCode = (int)DriverErrorCodes.ErrorNoError;

            if ((CommandType == CommandTypes.ReadCmd && pdu[46] != 0xCC) &&
                (CommandType == CommandTypes.WriteCmd && pdu[46] != 0xCD))
            {
                omronErrorCode = (int)OmronEthernetIPErrorCodes.ErrorRepSerCode;
            }
            else
            {
                byte ReplyStatus = pdu[46 + 2];
                if (ReplyStatus != 0)
                {
                    switch ((int)ReplyStatus)
                    {
                    ...
                    // Length (in words) of the additional status  
                    byte AddStatusSize = pdu[42];
                    if ((AddStatusSize > 6) && (omronErrorCode != (int)OmronEthernetIPErrorCodes.ErrorCipStatus))
                    {
                        ushortUnion addInfo = new ushortUnion(pdu, (ushort)(46+ 4));
                        switch ((int)addInfo.USHORT)
                        {
                            case 0x102:
                                omronErrorCode += (int)OmronEthernetIPErrorCodes.ErrorCipAddStatus102;
