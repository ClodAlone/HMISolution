...
namespace S7TIASymbolic
{
    public class S7TIACommJob : CommJob
    {
        ...
        public bool LoadAccessHandleAndSize(IntPtr pRootHandle, string StationName, string addres, ref string szError)
        {
            ...
            {
                // get string type from .tia file (strunct's member string can be STRING/WSTRING)
                AGL4.ValueType valueType = AGL4.ValueType.UNDEFINED;
                result = AGL4.Symbolic_GetAccessElementValueType(m_stSymbolicRW.AccessHandle, ref valueType);
                if (result != AGL4Sym.AGLSYM_SUCCESS)
                {
                    szError = string.Format("{0} {1} {2}", StationName, _StartAddress, string.Format(Properties.Resources.ErrorFromDllAGLink, result));
                    return (false);
                }
                if (valueType == AGL4.ValueType.STRING16 || valueType == AGL4.ValueType.STRING32)
                    _S7DataFormat = S7DataFormats.WString;
                
                SetStringTagDataFormat(_S7DataFormat);

                if ((uint)m_stSymbolicRW.BufferLen != 0)
                {
                    uint arrayDimension = (TagsList[0].TagNode.ArrayDimension == 0 ? 1 : TagsList[0].TagNode.ArrayDimension);
                    
                    if (_S7DataFormat == S7DataFormats.String)
                        StringLength = (uint)(m_stSymbolicRW.BufferLen / arrayDimension);
                    else
                        StringLength = (uint)(m_stSymbolicRW.BufferLen / arrayDimension) / 2;

                    // buffer has 1 byte more to manage \0
                    StringLength--;
                    TagsList[0].Size = StringLength * arrayDimension;

                    TotalJobSize = TagsList[0].Size;
                }
            }
