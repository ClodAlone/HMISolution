...
namespace ModbusTCPSlave.UI
{
    public class CommunicationDriverWpfEditing : ICommunicationDriverWpfEditing3
    {
        ...
        public bool CopyFile(string sourceconn, string targetconn)
        {
            ...
            bool result = DriverCodeBaseEx.CommunicationDriver.CopyFile<ModbusTCPSlaveDriverSettings>(sourceconn, targetconn, drivername);
            result &= DriverCodeBaseEx.CommunicationDriver.CopyFile<ModbusTCPSlaveChangeTag>(sourceconn, targetconn, drivername, DriverCodeBaseEx.Helpers.DriverInfo.GetChangeDynTagsExtension());
            return result;
        }
        ...
        {
            ModbusTCPSlave.ModbusTCPSlaveDynTagSettings dynTag = new ModbusTCPSlave.ModbusTCPSlaveDynTagSettings();
        ...
        {
            ModbusTCPSlaveDynTagSettings dynTag = new ModbusTCPSlaveDynTagSettings();
        ...
        {
            var genSet = generalSettings as ModbusTCPSlaveDriverSettings;
            if (genSet == null)
                return null;

            if (channelEditor == null)
                channelEditor = new ChannelDetails();
            var ch = new ModbusTCPSlaveChannelSettings(genSet.Session);
        ...
        public UserControl NewStationSettingsEditor(object generalSettings)
        {
            ...
            var st = new ModbusTCPSlaveStationSettings(genSet.Session);
        ...
        public bool DisableDriver(string connectionString, bool bProtected, Guid protectionCode)
        {
            ...
            {
                string drivername = DriverCodeBaseEx.Helpers.DriverInfo.GetDriverName(this).Replace(".UI", "");
                configurationEditor.Init<ModbusTCPSlaveDriverSettings>(connectionString, drivername, bProtected, protectionCode);
