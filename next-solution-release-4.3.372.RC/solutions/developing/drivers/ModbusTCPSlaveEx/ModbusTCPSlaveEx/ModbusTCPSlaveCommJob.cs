...
namespace ModbusTCPSlave
{
    public class ModbusTCPSlaveCommJob : CommJob
    {
        ...
        /// <summary>
        /// Set internal memory value using inital job value
        /// </summary>
        /// <returns></returns>
        public bool SetJobInitialValue(out bool initialValueForced)
        {
            initialValueForced =false;
            lock (lockListObject)
            {
                // Working on the first tag of the job is sufficient because this driver does not aggregate tags
                // (a job cannot be associated to more than one tag)
                if (TagsList.Count > 0)
                {
                    if (TagsList[0].Value == null || TagsList[0].Value.Value == null)
                    {
                        ((ModbusTCPSlaveTag)TagsList[0]).ForceDefaultInitialValue();
                        initialValueForced = true;
                    }

                    TagsListToWrite.Add(TagsList[0]);
                    byte[] writeData;
                    if (PrepareData(out writeData))
                    {
                        List<ModbusTCPSlaveCommJob> changedJob;
                        ModbusTCPSlaveStation modStation = (ModbusTCPSlaveStation)Station;
                        modStation.setMemoryData(this, ref writeData, out changedJob);
                        TagsListOnWriting.Remove(TagsList[0]);
                        return (true);
                    }
                }
            }
