...
namespace ModbusTCP
{
    public sealed class ModbusTCPDynTagSettings : DynTagSettings
    {
        ...
        #region Data Member
        // keep next address of structure
        public UInt16 _NextStartAddress;
        #endregion
        ...
        private bool IsBitArea()
        {
            return (FunctionCode == FunctionCodes.Coils || FunctionCode == FunctionCodes.SingleCoil || FunctionCode == FunctionCodes.DiscreteInputs);
        }

        public override string GetFirstDynSetting(Tag tag, TagDefinition thistagdefinition, int stringLength)
        {
            string result = base.GetFirstDynSetting(tag, thistagdefinition, stringLength);
            ...
            {                
                if (stringLength > 0)
                {
                    StringLength = (uint)stringLength;
                    // align string to register size
                    if (!bits && StringLength % 2 != 0)
                        StringLength++;
                }

                // (StringLength / 2) --> size is in register and not bytes
                _NextStartAddress = (ushort)(StartAddress + (StringLength / 2));
            }
            else
            {
                _NextStartAddress = StartAddress;
                // call GetNextDynSetting method to calculate _NextStartAddress value starting from current
                string dummy = GetNextDynSetting(thistagdefinition, thistagdefinition, 0);
            }
        ...
        {
            StartAddress = _NextStartAddress;
            ...
            if (thistagdefinition.DataType.IdType == IdType.Numeric)
            {
                switch ((uint)thistagdefinition.DataType.Identifier)
                {
                    case (uint)BuiltInType.Boolean:
                        if (bits)
                            _NextStartAddress += (thistagdefinition.ArrayDimension > 0 ? (UInt16)thistagdefinition.ArrayDimension : (UInt16)1);
                        else
                            _NextStartAddress += (thistagdefinition.ArrayDimension > 0 ? (UInt16)(thistagdefinition.ArrayDimension / 16 + (thistagdefinition.ArrayDimension % 16 > 0 ? 1 : 0)) : (UInt16)1);
                        break;
                    case (uint)BuiltInType.SByte:
                    case (uint)BuiltInType.Byte:
                        if (bits)
                            _NextStartAddress += (UInt16)((thistagdefinition.ArrayDimension > 0 ? thistagdefinition.ArrayDimension : 1) * 8);
                        else
                            _NextStartAddress += (UInt16)((thistagdefinition.ArrayDimension > 0 ? (thistagdefinition.ArrayDimension / 2 + (thistagdefinition.ArrayDimension % 2 > 0 ? 1 : 0)) : 1));
                        break;
                    case (uint)BuiltInType.Int16:
                    case (uint)BuiltInType.UInt16:
                        if (bits)
                            _NextStartAddress += (UInt16)((thistagdefinition.ArrayDimension > 0 ? thistagdefinition.ArrayDimension : 1) * 2 * 8);
                        else
                            _NextStartAddress += (UInt16)((thistagdefinition.ArrayDimension > 0 ? (thistagdefinition.ArrayDimension) : 1));
                        break;
                    case (uint)BuiltInType.Float:
                    case (uint)BuiltInType.UInt32:
                    case (uint)BuiltInType.Int32:
                        if (bits)
                            _NextStartAddress += (UInt16)((thistagdefinition.ArrayDimension > 0 ? thistagdefinition.ArrayDimension : 1) * 4 * 8);
                        else
                            _NextStartAddress += (UInt16)((thistagdefinition.ArrayDimension > 0 ? (thistagdefinition.ArrayDimension * 2) : 2));
                        break;
                    case (uint)BuiltInType.UInt64:
                    case (uint)BuiltInType.Int64:
                    case (uint)BuiltInType.Double:
                        if (bits)
                            _NextStartAddress += (UInt16)((thistagdefinition.ArrayDimension > 0 ? prevtagdefinition.ArrayDimension : 1) * 8 * 8);
                        else
                            _NextStartAddress += (UInt16)((thistagdefinition.ArrayDimension > 0 ? (prevtagdefinition.ArrayDimension * 4) : 4));
                        break;
                    case (uint)BuiltInType.String:
                        ...
                        if (bits)
                            _NextStartAddress += (UInt16)((thistagdefinition.ArrayDimension > 0 ? thistagdefinition.ArrayDimension : 1) * (StringLength * 2) * 8);
                        else // (StringLength / 2) --> size is in register and not bytes
                            _NextStartAddress += (UInt16)((thistagdefinition.ArrayDimension > 0 ? (thistagdefinition.ArrayDimension * (StringLength / 2)) : (StringLength/2)));
                        break;
                }
            }
        ...
        protected override String PerformValidation(String propertyName)
        {
            ...
            if (propertyName == "FunctionCode")
            {
                ...
                if (InvalidFunctionCodeInutType())
                    return string.Format(Properties.Resources.FunctionCodeRequireInput, DriverCodeBaseEx.Properties.Resources.LinkType_Input);

                if (InvalidFunctionCodeOutputType())
                    return string.Format(Properties.Resources.FunctionCodeRequireOutput, DriverCodeBaseEx.Properties.Resources.LinkType_Input);
        ...
        private bool InvalidStringSize()
        {
            ...
            return ((VarType == UFUAModel.DataType.String) &&
                    ((StringLength > ModbusProtocol.GetMaxJobSize(FunctionCode, (LinkType)TagLinkType)) ||
                    (StringLength < 1)));
        }
