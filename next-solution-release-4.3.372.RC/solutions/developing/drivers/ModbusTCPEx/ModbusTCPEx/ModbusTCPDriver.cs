...
namespace ModbusTCP
{
    public class ModbusTCPDriver : CommunicationDriver
    {
        ...
        public ModbusTCPDriver(string strSettingPath)
            : base(strSettingPath)
        {
        }
        ...
        public override bool LoadDriverSettings(IDataLayer idl)
        {
            ...
            using (UnitOfWork ufw = new UnitOfWork(idl))
            {
                try
                {
                    var configuration = (from tag in new XPQuery<ModbusTCPDriverSettings>(ufw).AsParallel() select tag).Single();
        ...
        public override void LoadDriverSettings(DriverSettings settings)
        {
            ...
            var conf = settings as ModbusTCPDriverSettings;
            if (conf == null)
        ...
        public override void SaveDriverSettings(IDataLayer idl)
        {
            using (UnitOfWork ufw = new UnitOfWork(idl))
            {
                var configuration = (from tag in new XPQuery<ModbusTCPDriverSettings>(ufw).AsParallel() select tag).ToList();

                if (configuration.Count == 0)
                    configuration.Add(new ModbusTCPDriverSettings(ufw));
         ...
         public override List<ChangeTag> LoadChangeSettings(IDataLayer idl)
        {
            using (UnitOfWork ufw = new UnitOfWork(idl))
            {
                return new List<ChangeTag>((new XPQuery<ModbusTCPChangeTag>(ufw)/*.AsParallel()*/ ).ToList());
            }
        }
