...
namespace TwinCAT
{
    public class TwinCATDriver : CommunicationDriver
    {
        ...
        public override bool Init(string strSettingPath, bool isProtected, Guid protectionCode)
        {
            ...
            // check library presence before to start
            if (!TwinCATProtocol.CheckDll())
            {
                OnSystemEvent(ObjectIds.Server, Properties.Resources.ErrorAMSTwincatRouter, EventSeverity.Max);
                return (false);
            }
