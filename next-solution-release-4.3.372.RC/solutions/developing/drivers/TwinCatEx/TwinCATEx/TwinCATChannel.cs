    ...
    {                
        ...
        public override void SplitInExecutionLists(List<CommJob> jobList, ref List<List<CommJob>> exList)
        {
            ...
            while (jobIndex < jobList.Count())
            {
                ...
                int PrevieusNumberTagsOnWrite= 0;
                    ...
                    if (j != null)
                    {
                        ...
                        if (write)
                        {
                            if (j.Station != null && j.Station.Name == station && jwrite)
                            {
                                ...
                                int maxNumberOfJobWritableTags = (int)(TwinCATMaxNumberOfAggregatedRequests - PrevieusNumberTagsOnWrite);
                                if (j.GetTagListOnWritingCount() > maxNumberOfJobWritableTags)
                                {
                                    if (j.GetTagListToWriteCount() != 0)
                                        j.UpdateTagsListOnWriting(); // This method moves into j.TagsListOnWriting the tags of j.TagsListToWrite and empties j.TagsListToWrite

                                    lock (((TwinCATCommJob)j).retLockList())
                                    {
                                        j.TagsListToWrite.AddRange(j.TagsListOnWriting);
                                        j.TagsListToWrite.RemoveRange(0, maxNumberOfJobWritableTags);
                                        j.TagsListOnWriting.RemoveRange(maxNumberOfJobWritableTags, (j.TagsListOnWriting.Count - maxNumberOfJobWritableTags));
                                    }
                                }
                                PrevieusNumberTagsOnWrite += j.GetTagListOnWritingCount();
                                list.Add(j);
                                jobList.RemoveAt(jobIndex);
                                jobIndex--;
                                if (PrevieusNumberTagsOnWrite >= (int)TwinCATMaxNumberOfAggregatedRequests)
                                {
                                    break;
                                }                                  
                            }
        ...
        {
            // if the management of the numerical addressing of the tags is used, these,
            // if consecutive, are aggregated in a single job,
            // therefore for correct management it is necessary to consider the list of tags and not that of the Jobs.
            int totalWriteRequestLength = 0;
            for (int i = 0; i < list.Count; i++)
            {
                totalWriteRequestLength += list[i].GetTagListOnWritingCount();
            }
        ...
        protected void ReadData(List<CommJob> l)
        {
            ...
            foreach (TwinCATCommJob job in list)
                base.ExecuteJob(job);
