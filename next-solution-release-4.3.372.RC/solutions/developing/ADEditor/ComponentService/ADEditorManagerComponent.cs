...
{
    public class ADEditorManagerComponent : ComponentBase<IADEditorManager>, IADEditorManager, IDocumentManager, IDisposable
#if !NET_STANDARD
        , ICrossReference
#endif
    {
...
#region IDocumentManager Members
        ...
        public void CleanCoreFiles(String projectPath)
        {
            var fileCore = String.Format("{0}\\{1}\\{2}{3}{4}",
                                ...
                                FileName,
                                FileType,
                                ADServerInfo.ADServerInfo.GetServerCoreExtension());

            if (System.IO.File.Exists(fileCore))
                System.IO.File.Delete(fileCore);

                //I delete all driver "* .core" files
                string settingsPath = string.Format("{0}\\{1}\\", projectPath, TypeLabel);
            Directory.GetFiles(settingsPath/*ADServerInfo.ADServerInfo.GetServerFolder()*/, 
                "*" + ADServerInfo.ADServerInfo.GetServerCoreExtension()).ToList().ForEach(f => File.Delete(f));
