...
namespace ADEditor.TreeDocumentManagers
{
    class EventListDocumentManager : IDocumentManager
    {
        ...
        ADModel.ADNotification Tag;
        ADModel.ADFolder Folder;
        ...
        public EventListDocumentManager(ADEditorManagerComponent c, ADEditorDocument doc)
        {
        ...
        internal EventListDocumentManager(ADEditorManagerComponent c, ADEditorDocument doc, IDocumentManager parent)
            : this(c, doc)
        {
        ...
        internal EventListDocumentManager(ADEditorManagerComponent c, ADEditorDocument doc,
            ADModel.ADNotification tag, IDocumentManager parent) 
            : this(c, doc)
        {
            Tag = tag;
            Parent = parent;
        ...
        internal EventListDocumentManager(ADEditorManagerComponent c, ADEditorDocument doc,
            ADModel.ADFolder folder, IDocumentManager parent)
            : this(c, doc)
        {
            DocumentEditorManagerComponent = c;
            Document = doc;
        ...
        public void Edit(Uri uri, IDocument parent)
        {
            if (bIsMaxItemsReached)
                ((EventListDocumentManager)Parent).GetChilds(uri, parent, false);
            else
            {
                DocumentEditorManagerComponent.Edit(uri, parent);
                DocumentEditorManagerComponent.GetViewFromUri(uri).tabNotifications.IsSelected = true;
            }
        }
        ...
        ObservableCollection<IDocumentManager> GetChilds(Uri uri, IDocument parent, bool bCap = true)
        {
            if (isGettingChilds)
                return null;
                ...
                using (new WaitCursor())
                {
                    if (Document == null || Document.IsDisposed)
                    {
                        ...
                        if (Folder != null)
                            ...
                            Tag = Document.FindNotificationByNodeId(Tag.NodeId);
                    }
                    ...
                    i = 0;
                    foreach (var name in Document.GetNotificationsCollection(Folder))
                    {
                        list.Add(new TreeDocumentManagers.EventListDocumentManager(DocumentEditorManagerComponent,
                                                    Document, name, this));
                        if (!bGetAll && ++i >= maxItems)
        ...
        public string TypeLabel
        {
            get
            {
                if (Tag != null)
                    ...
                    return Properties.Resources.tabNotifications;
            }
        }
        ...
        public BitmapImage TypeIcon
        {
            get
            {
                if (Tag != null)
                {
                    return ADEditorManagerComponent.GetBitmapImage("ADNotificationSmall");
                }
                else if (Folder != null)
                {
                    return ADEditorManagerComponent.GetBitmapImage("CloseFolderSmall", true);
                }
                else
                    return ADEditorManagerComponent.GetBitmapImage("ADNotificationSmall");
            }
        ...
        public BitmapImage TypeIconOpen
        {
            get
            {
                if (Folder != null)
                {
                    return ADEditorManagerComponent.GetBitmapImage("OpenFolderSmall", true);
                }
                else
                    return TypeIcon;
            }
        ...
        public bool IsResourceExpandable(IDocument document = null)
        {
            ...
            else if (Document != null)
            {
                ...
                if (list.Count > 0)
                    return true;
                var list1 = Document.GetNotificationsCollection(Folder);
