...
namespace DeployServer.Hubs
{
    ...
    public class DeployServerHub : Hub
    {
        ...
        //    await Task.Run(() =>
        //    {
        //        string projectFolder = (pathSourceDeploy + path).Replace('/', Path.DirectorySeparatorChar);
        //        var tempFileName = Path.GetFileName(Path.GetTempFileName());
        //        ZipFileHelper.CreateArchive(projectFolder, tempFileName);
        //        var archivePath = Path.Combine(Path.GetFullPath(projectFolder), tempFileName);
        //        ret = File.ReadAllBytes(archivePath);
        //        File.Delete(archivePath);
        //    });
        //    return ret;
        //}

        public async Task<FileInfoSerializable> CreateProjectZipFile(String path)
        {
            FileInfoSerializable zipFileInfo = null;
            await Task.Run(() =>
            {
                string projectFolder = (pathSourceDeploy + path).Replace('/', Path.DirectorySeparatorChar);
                var tempFileName = Path.GetFileName(Path.GetTempFileName());
                ZipFileHelper.CreateArchive(projectFolder, tempFileName);
                var zipPath = Path.Combine(Path.GetFullPath(projectFolder), tempFileName);
                zipFileInfo = new FileInfoSerializable(zipPath, new FileInfo(zipPath).Length);
            });
            return zipFileInfo;
        }

        public async Task<List<string>> ScanRemoteProjects(String projectsPath)
        {
            List<string> projectsList = new List<string>();
            await Task.Run(() =>
            {
                string projectFolder = (pathSourceDeploy + projectsPath).Replace('/', Path.DirectorySeparatorChar);
                foreach (var dir in Directory.GetDirectories(projectFolder))
                    projectsList.Add(new DirectoryInfo(dir).Name);
            });
            return projectsList;
        }

        public async Task<byte[]> DownloadFileChunk(string path, int maxSize, int offset)
        {
            byte[] bytes = null;
            await Task.Run(() =>
            {
                ...
                try
                {
                    using (var stream = File.OpenRead(path))
                    {
                        ...
                        chunkSize = Math.Min(maxSize, stream.Length - offset);
                        bytesread = stream.Read(bytes, 0, (int)chunkSize);
