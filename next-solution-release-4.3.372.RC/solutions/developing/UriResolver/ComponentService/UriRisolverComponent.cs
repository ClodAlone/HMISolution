...
#region IUFInterfaceBase Members
        ...
        private void GetComponentInterfaces()
        {
            ...
            if (dirInfo.GetDirectories(Properties.Settings.Default.DocumentManagersPath, SearchOption.TopDirectoryOnly).FirstOrDefault() == null)
                dirInfo = dirInfo.Parent;
            baseFolderDesigner = String.Format("{0}{1}", Path.Combine(dirInfo.FullName, Properties.Settings.Default.DocumentManagersPath), Path.DirectorySeparatorChar);
#endif
            // RegisterFileTypes();
        }
        ...
        public List<IDocumentManager> GetListInstalledDocumentManagers()
        {
            ...
            lock(lockObject)
            {
                if (listInstalledComponent == null)
                {
                    ...
                    var list = FindAndLoadDLL.LoadDLLs<IComponent>(baseFolderDesigner, "*.dll");
