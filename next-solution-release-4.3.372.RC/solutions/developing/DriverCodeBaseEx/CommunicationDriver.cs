...
#if !NETSTANDARD
        , ICrossReferenceAware, IInvalidateDynJobs
#endif
    {
        #region Constructors
        ...
        protected readonly Dictionary<String, Channel> Channels = new Dictionary<String, Channel>();
        /// <summary>   The stations. </summary>
        protected readonly Dictionary<String, Station> Stations = new Dictionary<String, Station>();
        /// <summary>   The tag to station map. </summary>
        readonly Dictionary<NodeId, Station> TagToStationMap = new Dictionary<NodeId, Station>();
        /// <summary>   All driver's tags</summary>
        ...
        public void OnRefreshDiagnostic(object sender, System.Timers.ElapsedEventArgs e)
        {
            ...
            try
            {
                ...
                {
                    StatisticsData.Update(StatisticSetting.NodeDataNames.ElapsedTime.ToString(), StatisticsData.GetTotalTimeOn().ToString(@"d\.hh\:mm\:ss"), send: true);

                    long totalJobsInUse = Channels.Values.Sum(c => c.InUseJobs);                    
                    StatisticsData.Update(StatisticSetting.NodeDataNames.TotalJobsInUse.ToString(), totalJobsInUse, send: true);
                    StatisticsData.NotifyValue(StatisticSetting.NodeDataNames.TotalTags.ToString());
                    StatisticsData.NotifyValue(StatisticSetting.NodeDataNames.TotalTagsInUse.ToString());
                    StatisticsData.NotifyValue(StatisticSetting.NodeDataNames.PeakOfTagsInUse.ToString());
                    StatisticsData.NotifyValue(StatisticSetting.NodeDataNames.TotalJobs.ToString());
                }
        ...
        public virtual bool Startup()
        {
            ...
            lock (lockStartupSuspend)
            {
                ...
                ResumeAllStatistics();
                // Startup will be excecuted on fly if necessary
        ...
        public virtual bool Terminate()
        {
            ...
            //Unlike the case of Suspend, the workingThread isn't stopped, in this case...
        ...
        {            
            ...
            ParseDynamicTags(newTags);

            return newTags;
        }

        private IList<Tag> CreateTagsFromChangeSettings(IList<Tag> tags, Dictionary<NodeId, string> tagChangeSettingMap)
        {            
            List<Tag> newTags = new List<Tag>();
        ...
        public virtual bool AddDynamics(IList<TagDefinition> tags)
        {
            ...
            {
                IList<Tag> processedTags = new List<Tag>();

                // try to load job's aggregation from storage
                if (!bLoadedPersistenceDynJobs)
                {                    
                    bLoadedPersistenceDynJobs = LoadDynamicJobs(out List<TagDefinition> invalidTagsLoaded, tags);

                    // some invalid tags ?
                    if (invalidTagsLoaded.Count > 0)
                    {
                        // try to reprocess these and then add to tags loaded from storage
                        processedTags = CreateTags(invalidTagsLoaded);
                        // if some tags are "created" from invalid, force configuration saving
                        if (processedTags.Count > 0)
                            bLoadedPersistenceDynJobs = false;
                    }
                    else
                    {
                        // no tags Loaded ? process movicon's tags
                        processedTags = GetAllTags();
                        if (processedTags.Count == 0)
                        {
                            processedTags = CreateTags(tags);
                            bLoadedPersistenceDynJobs = false;
                        }
                    }                    
                }
                ...
                // return list of invalid tags and set opc and log to BadConfigurationError error
                IList<TagDefinition> invalidTags = GetInvalidTags(processedTags); 

                // if tags were not loaded from storage (or need to be updated), save it
                if (!bLoadedPersistenceDynJobs)
                    bLoadedPersistenceDynJobs = SaveDynamicJobs(invalidTags);
                ...
                if (StatisticsData != null)
                {
                    ...
                    {
                        totalJobs += Stations[stationname].GetListWholeJobCount();
                        totalTags += Stations[stationname].GetAllTagsCount();
                    }
                    StatisticsData.Update(StatisticSetting.NodeDataNames.TotalJobs.ToString(), totalJobs);
                    StatisticsData.Update(StatisticSetting.NodeDataNames.TotalTags.ToString(), totalTags);
                }
            }
        ...
        public virtual bool InUseDynamics(IList<TagDefinition> tags, bool bInUse)
        {
            ...
            Station.InUseStates inUseState = Station.InUseStates.None;
            Dictionary<Channel, bool> setInUseChannel = new Dictionary<Channel, bool>();
            foreach (var tag in tags)
            {
                if (copyTagToStationMap.ContainsKey(tag.NodeId))
                {
                    inUseState = copyTagToStationMap[tag.NodeId].SetInUse(tag.NodeId, bInUse, tag.SamplingInterval);
                    if (inUseState == Station.InUseStates.InUse || inUseState == Station.InUseStates.NotInUse)
                    {
                        // wake up scheduler only for channel when tag get in use
                        if (inUseState == Station.InUseStates.InUse)
                        {
                            if (!setInUseChannel.ContainsKey(copyTagToStationMap[tag.NodeId].GetChannel()))
                                setInUseChannel[copyTagToStationMap[tag.NodeId].GetChannel()] = true;
                        }
                ...
                else if (tag.NodeId.IdType == IdType.String)
                {
                    ...
                    {
                        inUseState = copyTagToStationMap[n].SetInUse(tag.NodeId, bInUse, tag.SamplingInterval);
                        if (inUseState == Station.InUseStates.InUse || inUseState == Station.InUseStates.NotInUse)
                        {
                            // wake up scheduler only for channel when tag get in use
                            if (inUseState == Station.InUseStates.InUse)
            ...
            lock (lockMaps)
            {
                Dictionary<Channel, bool> channelInUse = new Dictionary<Channel, bool>();
            ...
            // wake up scheduler only for channel when tag get in use
            if (setInUseChannel.Values.Count > 0)
            {
                foreach (Channel ch in setInUseChannel.Keys)
                    ch.SetRequestSchedule();
            }
        ...
        /// <returns>   An uint. </returns>
        ////////////////////////////////////////////////////////////////////////////////////////////////////
        public virtual uint OnWriteTag(TagDefinition tag, ref object value, ref StatusCode statusCode, ref DateTime timestamp, bool ignoreWriteAsync = false, bool forceSynchWrite = false)
        {
            /*
            ...
            if (station.IsStateCommandVariableBit((UInt16)StationVariableBits.StationActiveCommand, true))
            {
                //station suspended, do not write!!
        ...
        public virtual uint OnWriteValues(String startingAddress, IList<object> dataValues)
        {
            ...
            var channel = station.GetChannel();
            Monitor.Enter(channel);
                try
                {
#if DEBUG
                    System.Diagnostics.Trace.TraceInformation("{2} OnWriteValues {0} n:{1} {3}.{4}", startingAddress, dataValues.Count, Thread.CurrentThread.ManagedThreadId, DateTime.Now.ToLongTimeString(), DateTime.Now.Millisecond);
#endif
                    if (station.IsStateCommandVariableBit((UInt16)StationVariableBits.StationActiveCommand, true))
                    {
                        //station suspended, do not write!!
                    ...
                    for (int i = 0; i < dataValues.Count; i++)
                    {
                        ...
                        int vdim = ((value.Value as Array) != null ? (value.Value as Array).Length : 0);
                        ...
                        if (isString)
                        {
                            ...
                            else
                            {
                                ...
                                if (bstr != null)
                                {
                                    stringLength = bstr.Length;
                                }
                            }
                        }

                        dtCalc.VarType = GetUFUAModelDataTypeFromBuiltType((BuiltInType)((uint)(dt.Identifier)));
                        ...
                        if (i == 0)
                        {
                            ...
                            var tag = CreateTag(tdef);

                            tdef.DynamicSettings = dtCalc.GetFirstDynSetting(tag, tdef, stringLength);
                        }
                        else
                        {
                            tdef.DynamicSettings = dtCalc.GetNextDynSetting(previousTDef, tdef, stringLength);
                        }
                        previousTDef = tdef;
...
#if DEBUG
                            MainJob.Name = "MainJob";
#endif
                            if (i == dataValues.Count - 1)
                            {
                                //last
                                result = station.ExecuteSyncroJob(MainJob, false, 0, outputvalues, false);
                        ...
                        else
                        {
                            ...
                            uint newIndex = 0;
                            // if tag data size is smaller that device protocol data type, don't aggregate
                            if (testJob.ProtocolDataSizeBig())
                                aggType = JobAggregationType.JobAggregImpossible;
                            else

                                aggType = MainJob.TestAggregateJob(testJob, out newIndex);
                            ...
                            else
                            {
                                ...
                                {
                                    //to end
                                    result = station.ExecuteSyncroJob(testJob, false, 0, outputvalues, false);
                                    if (result != StatusCodes.Good)
                                        return (error == 0 ? result : StatusCodes.Bad);
                ...
                try
                {
                    ...
                    for (int i = 0; i < dataValues.Count; i++)
                    {
                        ...
                        if (i == 0)
                        {
                            ...
                            {
                                //last
                                result = station.ExecuteSyncroJob(MainJob, false, 0, dataresult);
                                if (result != StatusCodes.Good)
                                    return (error == 0 ? result : StatusCodes.Bad);
                        ...
                        else
                        {
                            ...
                            else
                            {
                                ...
                                {
                                    //to end
                                    result = station.ExecuteSyncroJob(testJob, false, 0, dataresult);
                                    if (result != StatusCodes.Good)
                                        return (error == 0 ? result : StatusCodes.Bad);
                                    if (Convert.ToUInt32(dataresult[0]) == (int)DriverErrorCodes.ErrorNoError)
                                    {
        ...
        ////////////////////////////////////////////////////////////////////////////////////////////////////
        /// <summary>   Test communican with device . </summary>
        ///
        /// <returns>   The result of communication test (0 === Good). </returns>
        ////////////////////////////////////////////////////////////////////////////////////////////////////
        public virtual bool TestComm(ChannelSettings ch, StationSettings st, out string error)
        {
            error = null;
        ...
        /// <returns>   The result of communication test (0 === Good). </returns>
        ////////////////////////////////////////////////////////////////////////////////////////////////////
        public virtual bool TestComm(ChannelSettings ch, StationSettings st, BuiltInType tagDataType, string dynamicSettings, out string error)
        {
            error = null;
        ...
        {
            return (OnTagChanged(node, value, default(DateTime), false, false));
        }
        ////////////////////////////////////////////////////////////////////////////////////////////////////
        /// <summary>   Executes the tag changed action. </summary>
        ///
        /// <param name="node" type="NodeId">       The value. </param>
        /// <param name="value" type="DataValue">   . </param>
        /// <param name="disableQualityUpdate" type="bool"> Disable quality update. </param>
        ////////////////////////////////////////////////////////////////////////////////////////////////////
        public bool OnTagChanged(NodeId node, DataValue value, bool disableQualityUpdate)
        {
            return (OnTagChanged(node, value, default(DateTime), disableQualityUpdate, false));
        }
        ...
        {
            return OnTagChanged(node, value, Timestamp, false, false);
        }
        ...
        /// <param name="disableQualityUpdate" type="bool"> Disable quality update. </param>
        ////////////////////////////////////////////////////////////////////////////////////////////////////
        public bool OnTagChanged(NodeId node, DataValue value, DateTime Timestamp, bool disableQualityUpdate = false, bool allowNullValues = false)
        {
            var temp = TagChanged;
            if (temp != null)
            {
                ...
                e.DisableQualityUpdate = disableQualityUpdate;
                e.NodeId = node;
        ...
        ////////////////////////////////////////////////////////////////////////////////////////////////////
        public virtual void OnTagPrototypeQuery(NodeId sourceNode, ref List<TagDefinition> listTags)
        {
        ...
        public List<NodeId> GetObservingNodes()
        {
            ...
            foreach (var station in stationList)
            {
                ...
                if(stationNodeIDs.Count > 0)
                {
                    ...
                    foreach(var node in stationNodeIDs)
                    {
                        ...
                        if(!ObservedTagToStationsMap[node].Contains(station))
                        {
            ...
            {
                nodeIDs.AddRange(driverStateCommandVariable.GetNodesId());
            }
            return nodeIDs;
        }
        ...
        public void UpdateObservedTag(NodeId node, DataValue value)
        {
            ...
            if(ObservedTagToStationsMap.Keys.Contains(node) == true)
            {
                ...
                Dictionary<string, Channel> suspendedChannels = new Dictionary<string, Channel>();
                foreach (var station in stationList)
            ...
            if ((driverStateCommandVariable.hasBeenSet == true) && driverStateCommandVariable.ContainsNodeId(node))
            {
        ...
        {
            driverStateCommandVariable.SetValueDataType(node, value.WrappedValue.TypeInfo.BuiltInType);
            driverStateCommandVariable.SetValue(node, value);
        }

        public void ManageUpdatedValueForTheStateCommandVariable(NodeId node, DataValue value)
        {
            bool changeBitSavedValue = false;
            bool changeBitCanBeManaged = false;
            bool bGetOldStatValue = false;

            bGetOldStatValue = driverStateCommandVariable.GetStateCommandVariableBit(ref changeBitSavedValue, (UInt16)DriverVariableBits.ChangeTagsSettings);
            SetStateCommandVariableValue(node, value);

            if (!bPendingChange && bGetOldStatValue == true)
            {
            ...
            if (changeBitCanBeManaged)
            {
                ...
                bool bGetStateValue = false;
                bGetStateValue = driverStateCommandVariable.GetStateCommandVariableBit(ref suspendBitNewValue, (UInt16)DriverVariableBits.ChangeTagsSettings);

                if (bGetStateValue == true)
                {
                        ...
                        SmartThreadPool.QueueWorkItem(() =>
                        {
                            ...
                            if (LoadChangeSettings(TagChangeSettingMap))
                            {
                                if (checkChangeFile(TagChangeSettingMap))
                                {
                                    if (Suspend())
                                    {
                                        List<Tag> allDrivertags = GetAllTags();

                                        RemoveAllJobs();

                                        IList<Tag> processedTags = CreateTagsFromChangeSettings(allDrivertags, TagChangeSettingMap);
                                        // publish invalid tags
                                        GetInvalidTags(processedTags);
        ...
        public bool checkChangeFile(Dictionary<NodeId, string> TagChangeSettingMap)
        {
            ...
            {
                List<Tag> tags = station.GetAllTags();
                foreach (var tag in tags)
                {
                    if (TagChangeSettingMap.ContainsKey(tag.TagNode.NodeId))
        ...
        #region IInvalidateDynJobs
#if !NET_STANDARD
        public void InvalidateDynamicSettings(String connectionString)
        {
            InvalidateDynJobSettings(DriverName, connectionString);
        }
#endif
        #endregion

        void RemoveDynJobSettings()
        {
            InvalidateDynJobSettings(DriverName, strConnectionString, true);
        }

        static void InvalidateDynJobSettings(string driverName, string strConnectionString, bool forceDelete = false)
        {
            string conn = GetConnectionString(strConnectionString, Properties.Settings.Default.TypeLabel, driverName, Properties.Settings.Default.DefaultDynFileExt);
            using (IDataLayer idl = GetSpecificDataLayer(conn, out string fileBase, out InMemoryDataStore InMemory, out bool targetIsFile))
            {
                if (targetIsFile)
                {                    
                    try
                    {
                        // windows --> delete .dynJobs , Linux --> delete .dynJobs.core (in linux don't delete .dynJobs file so driver cab re-use its name to create aggregation job file)
                        if (File.Exists(fileBase))                        
                            File.Delete(fileBase);

                        if (!forceDelete)
                        {
                            // force to create an empty file for linux project --> this force driver at start-up to recreate .dynJobs file
                            File.Create(fileBase);

                            string fileBaseCore = string.Format("{0}{1}", fileBase, Properties.Settings.Default.DefaultDrvFileExtCore);
                            if (!File.Exists(fileBaseCore))
                                File.Create(fileBaseCore);
                        }
                    }
                    catch (Exception ex)
                    { }                    
                ...
                else
                {
                    using (UnitOfWork ufw = new UnitOfWork(idl))
                    {
                        try
                        {
                            // delete all element referr to this driver 
                            var dy = (from dyn in new XPQuery<DynamicJobs>(ufw) where dyn.DriverName == driverName select dyn).ToList()[0];
                            if (dy != null)
                                ufw.Delete(dy);
                        }
                        catch (Exception ex)
        ...
        private List<Tag> GetAllTags()
        {
            ...
            foreach (Station station in Stations.Values)
                tags.AddRange(station.GetAllTags());            
...
#region Virtual Methods
        ...
        public virtual void LoadDriverSettings(DriverSettings settings)
        {
            ...
            {
                driverStateCommandVariable = new StateCommandVariable(StateCommandVariable.ObjectTypes.Driver, _StateCommandTag, _StateTag, _CommandTag);
            }
        }
        ...
        public virtual void GetDriverErrorInfo(int errorcode, out uint quality, out string error)
        {
            switch ((DriverErrorCodes)errorcode)
            {
                ...
                case DriverErrorCodes.ErrorDeviceOpenFailed:
                    quality = StatusCodes.BadCommunicationError;
                    error = Properties.Resources.ErrorDeviceOpenFailed;
                    break;
                default:
...
#region Internal Methods
            ...
            using (UnitOfWork ufw = new UnitOfWork(idl))
            {
                ...
                foreach (var settings in channelsettings)
                {
                    ...
                    if (!Channels[settings.Name].Init())
                        OnSystemEvent(null, String.Format(Properties.Resources.StationUninitailized, channel.Name), EventSeverity.High);
                }
            }
        }
        ...
        private void BuildListOfStations()
        {
            ...
            using (UnitOfWork ufw = new UnitOfWork(idl))
            {
                var listsettings = (from drvsettings in new XPQuery<DriverSettings>(ufw)
                                    select drvsettings).ToList();
                ...
                foreach (var settings in stationsettings)
                {
                    ...
                    if (!Stations[settings.Name].Init(channel))
                        OnSystemEvent(null, String.Format(Properties.Resources.StationUninitailized, station.Name), EventSeverity.High);
                }
            }
        }
        ...
        private bool StartupAllStations()
        {
            ...
            foreach (var station in Stations.Values)
            {
                if (!station.Startup())
                {
                    OnSystemEvent(null, String.Format(Properties.Resources.StationStartupFailed, station.Name), EventSeverity.High);
                    bRet = false;
                }
            };
            return bRet;
        ...
        private bool SuspendAllStations()
        {
            ...
            foreach (var station in Stations.Values)
            {
                if (!station.Suspend(false))
                {
                    OnSystemEvent(null, String.Format(Properties.Resources.StationSuspendFailed, station.Name), EventSeverity.High);
                    bRet = false;
                }
            };
            return bRet;
        ...
        private bool TerminateAllStations()
        {
            ...
            foreach (var station in Stations.Values)
            {
                ...
                {
                    OnSystemEvent(null, String.Format(Properties.Resources.StationTerminateFailed, station.Name), EventSeverity.High);
                    bRet = false;
                }
            };
            return bRet;
        ...
        /// <summary>
        /// OnLogEvent  Method used for writing on log file.
        /// Please note if movicon is in drawing the message is saved in the file: "C: \ ProgramData \ Progea \ Movicon.NExT.Log \ ClientLog.xml", 
        /// while if movicon is in runtime the message is saved at inside the file "Name Project \ Log \ SysLog.xml"
        /// </summary>
        /// <param name="message">Message string to pass</param>
        /// <param name="entryType">Category type of the message</param>
        /// <param name="bEnableEventLogsWindow"> This flag by default is false if enabled it writes to the windows event log </param>
        public static void OnLogEvent( String message, EventLogEntryType entryType, bool bEnableEventLogsWindow = false)
        {
            Utilities.Logger.Logger.WriteToEventLog(Properties.Resources.LoggerSource, message,
                            entryType, LoggerDestination.Drivers , bEnableEventLogsWindow);
        }
        ////////////////////////////////////////////////////////////////////////////////////////////////////
        ...
        {            
            ...
            if (tags.Count > 0)
            {
                ...
                Dictionary<string, List<Tag>> stationList = (from tag in tags.AsParallel() where tag.bIsValid select tag).GroupBy(x => x.DynSettings.StationName)
                ...
                foreach (var station in stationList)
                {
                    ...
                    var tagsStation = (from tag in tags.AsParallel()
                                       where tag.bIsValid && tag.DynSettings.StationName == station.Key
                                       select tag).ToList();

                    if (Stations.ContainsKey(station.Key))
                    {
                        if (Stations[station.Key].ParseDynamicTags(tagsStation))
                            totalTags += tagsStation.Count;                        
                    }
                    else
                    {
                        // invalid station
                        OnSystemEvent(null, string.Format(Properties.Resources.ErrorStationNameNotExist, station.Key, DriverName), EventSeverity.Low);
                        // force station's tags to invalid state
                        Parallel.ForEach(tagsStation, tag =>
                        {
                            tag.bIsValid = false;
                            tag.InvalidReason = string.Format(Properties.Resources.ErrorTagStationNameNotExist, station.Key);
                        });
                    }
                }
            }
        ...
        private UFUAModel.DataType GetUFUAModelDataTypeFromBuiltType(BuiltInType type)
        {
            switch (type)
            {
        ...
        private object lockSmartThreadPool = new object();
            ...
            if (providerType == InMemoryDataStore.XpoProviderTypeString)
            {
                ...
                if (index != -1)
                {
                    ...
                    if (runningFilePath == null)
                    {
                        ...
                        var fileBase = String.Format("{0}{1}{2}{3}{4}", path.Replace('\\', Path.DirectorySeparatorChar), Path.DirectorySeparatorChar, baseName, xmlExt, Properties.Settings.Default.DefaultDrvFileExtCore);
                        if (!File.Exists(fileBase))
                        {
                            ...
                            if (!File.Exists(fileBase))
                            {
                                xmlExtNetCore = Properties.Settings.Default.DefaultDrvFileExtCore;
                            }
                        }
                        else
                        {
...
#else
                        var fileBase = String.Format("{0}{1}{2}{3}", path.Replace('\\', Path.DirectorySeparatorChar), Path.DirectorySeparatorChar, baseName, xmlExt, Properties.Settings.Default.DefaultDrvFileExtCore);
                        if (!File.Exists(fileBase))
                        {
                            fileBase = String.Format("{0}{1}{2}{3}", path.Replace('\\', Path.DirectorySeparatorChar), Path.DirectorySeparatorChar, baseName, xmlExt);
                        }                        
...
#else
                        ds = String.Format("{0}{1}{2}{3}{4}", path.Replace('\\', Path.DirectorySeparatorChar), Path.DirectorySeparatorChar, baseName, xmlExt, xmlExtNetCore);
#endif
                }
        ...
        private IDataLayer GetDriverDataLayer(bool running = false)
        {
            if (drvDataLayer == null)
            {
                try
                {
                    ...
                    drvDataLayer = GetSpecificDataLayer(conn); 
                }
                catch (Exception ex)
                {
                    drvDataLayer = null;
...
#else
                    if(!fileBase.EndsWith(Properties.Settings.Default.DefaultDrvFileExtCore))
                        fileBase = String.Format($"{fileBase}{Properties.Settings.Default.DefaultDrvFileExtCore}");
#endif
                    retMess = ex.Message;
        ...
        ////////////////////////////////////////////////////////////////////////////////////////////////////
        /// <summary>   Make a copy (.bak) of driver settings and the delete it.                 </summary>
        ////////////////////////////////////////////////////////////////////////////////////////////////////
        public static void BackupAndDeleteDriverSettings(bool protection, Guid code, string strConnectionString, string driverName, out string backupFile)
        {
            backupFile = string.Empty;
            string conn = GetConnectionString(strConnectionString, Properties.Settings.Default.TypeLabel, driverName, Properties.Settings.Default.DefaultDrvFileExt);
            using (IDataLayer idl = GetSpecificDataLayer(conn, out string fileBase, out InMemoryDataStore InMemory, out bool targetIsFile))
            {
                if (targetIsFile)
                {
                    try
                    {
                        backupFile = string.Format("{0}{1}", fileBase, Properties.Settings.Default.BackupDrvFileExt);
                        File.Copy(fileBase, backupFile, true);
        ...
        ////////////////////////////////////////////////////////////////////////////////////////////////////
        private IDataLayer GetDynamicJobsDataLayer(out string filebase, out InMemoryDataStore InMemory, out bool targetIsFile)
        {
            string conn = GetConnectionString(strConnectionString, Properties.Settings.Default.TypeLabel, DriverName, Properties.Settings.Default.DefaultDynFileExt);

            return GetSpecificDataLayer(conn, out filebase, out InMemory, out targetIsFile);
        }
        ...
        {
            string conn = GetConnectionString(strConnectionString, Properties.Settings.Default.TypeLabel, DriverName, Properties.Settings.Default.DefaultExchFileExt);
        ...
        private IDataLayer GetChangeSettingsDataLayer(out string filebase, out InMemoryDataStore InMemory)
        {
            ...
            return GetSpecificDataLayer(conn, out filebase, out InMemory, out bool targetIsFile); 
        }
        ...
        /// <returns>   true if it succeeds, false if it fails. </returns>
        ////////////////////////////////////////////////////////////////////////////////////////////////////
        private bool LoadDynamicJobs(out List<TagDefinition> invalidTags, IList<TagDefinition> serverTags)
        {
            bool loaded = false;
            invalidTags = new List<TagDefinition>();
            try
            {
                using (IDataLayer idl = GetDynamicJobsDataLayer(out string fileBase, out InMemoryDataStore InMemory, out bool targetIsFile))
                {
                    using (UnitOfWork ufw = new UnitOfWork(idl))
                    {                    
                        // during deserialization . 
                        XPCollection<CommJobSettings> jobs = null;
                        XPCollection<TagSettings> tags = null;
                        Dictionary<CommJobSettings, List<TagSettings>> mapTags = null;
                        XPCollection<TagSettings> structTags = null;
                        Dictionary<CommJobSettings, List<TagSettings>> mapStructTags = null;
                        try
                        {
                            // check if dynjob file was generated by the same driver version
                            bool correctDriverVersion = ((from dyn in new XPQuery<DynamicJobs>(ufw) where dyn.DriverName == DriverName && dyn.DriverVersion == DriverVersion() select dyn.JobSettings).ToList().Count > 0);
                            if (correctDriverVersion)
                            {
                                jobs = (from dyn in new XPQuery<DynamicJobs>(ufw) where dyn.DriverName == DriverName select dyn.JobSettings).ToList()[0];

                                tags = (from dyn in new XPQuery<DynamicJobs>(ufw) where dyn.DriverName == DriverName select dyn.Tags).ToList()[0];

                                // from list of valid tags create a dyctionary with key=job, values = tags associated to job
                                mapTags = (from tag in tags where tag.IsValid select tag).GroupBy(x => x.CommJobSettings)
                                                .ToDictionary(k => k.Key, v => v.ToList());

                                structTags = (from dyn in new XPQuery<DynamicJobs>(ufw) where dyn.DriverName == DriverName select dyn.StructTags).ToList()[0];
                                // from list of valid tags create a dyctionary with key=job, values = tags associated to job
                                mapStructTags = (from tag in structTags where tag.IsValid select tag).GroupBy(x => x.CommJobSettings)
                                                .ToDictionary(k => k.Key, v => v.ToList());
                            }
                        }
                        catch (Exception ex)
                        ...
                        if ((tags != null && tags.Count > 0) || (structTags != null && structTags.Count > 0))
                        {
                            // check if some tags are invalid
                            if (tags != null && mapTags.Values.Count != tags.Count)
                            {
                                loaded = true;
                                var badTags = (from tag in tags where !tag.IsValid select tag).ToList();
                                foreach (var tag in badTags)
                                    invalidTags.Add(tag.GetTagDefinition());                        
                            }
                            object Lock = new object();                            
                            Dictionary<NodeId, TagSettings> mapTagsNodeId = null;
                            if (jobs.Count > 0)
                            {
                                try
                                {
                                    // set tag initial value using value of tags definition from server tags
                                    mapTagsNodeId = (from tag in tags where tag.IsValid select tag).ToDictionary(k => k.NodeId, v => v);
                                }
                                catch (Exception ex)
                                {
                                    // if there are some duplicate tags it may fail: try creating the map manually
                                    mapTagsNodeId = new Dictionary<NodeId, TagSettings>();
                                    foreach (var tag in tags)
                                    {
                                        if (!mapTagsNodeId.ContainsKey(tag.NodeId))
                                            mapTagsNodeId[tag.NodeId] = tag;
                                    }
                                }
                                var dt = DateTime.UtcNow;
                                int counter = 0;
                                int totalservertags = serverTags.Count;
                                Parallel.ForEach(serverTags, tag =>
                                {
                                    if (tag.InitialValue != null)
                                    {
                                        if (mapTagsNodeId.ContainsKey(tag.NodeId))
                                            mapTagsNodeId[tag.NodeId].InitialValue = tag.InitialValue;
                                    }
                                    lock (Lock) //to avoid different threads print the same percentage at the same time
                                    {
                                        int counterNow = Interlocked.Increment(ref counter);
                                        if (dt <= DateTime.UtcNow)
                                        {
                                            Console.WriteLine(String.Format("{0} - {1} -> {2} 1 ({3}%)", DateTime.Now,
                                                Properties.Resources.LoadingDynamicJobs, Properties.Resources.Step, Math.Round((float)((float)counterNow / (float)totalservertags * 100))));
                                ...
                                counter = 0;
                                loaded = true;
                                foreach (var jobsettings in jobs)
                                {
                                    bool isValid = false;

                                    if (Stations.ContainsKey(jobsettings.Station))
                                    {
                                        if (mapStructTags.Count > 0 && mapStructTags.ContainsKey(jobsettings))
                                        {
                                            List<TagSettings> structMembers = new List<TagSettings>();
                                            structMembers = mapStructTags[jobsettings].OrderBy(t => t.MemberOrder).ToList();
                                            if (structMembers.Count > 0)
                                            {
                                                jobsettings.StructTag = structMembers[0];
                                                structMembers.RemoveAt(0);
                                                if (structMembers.Count > 0)
                                                    jobsettings.Tags.AddRange(structMembers);
                                            }
                                        }
                                        else
                                        {
                                            if (mapTags[jobsettings].Count == 1)
                                                jobsettings.Tags.AddRange(mapTags[jobsettings]);
                                            else
                                                jobsettings.Tags.AddRange(mapTags[jobsettings].OrderBy(t => t.Oid).ToList());
                                        }
                                    ...
                                    if (!isValid)
                                    {
                                        if (mapTags.ContainsKey(jobsettings))
                                        {
                                            foreach (var tag in mapTags[jobsettings])
                                                invalidTags.Add(tag.GetTagDefinition());
                                        }
                                    }
                                    ...
                                    if (dt <= DateTime.UtcNow)
                                    {
                                        Console.WriteLine(String.Format("{0} - {1} -> {2} 2 ({3}%)", DateTime.Now,
                                            Properties.Resources.LoadingDynamicJobs, Properties.Resources.Step, Math.Round(((float)counter / (float)jobs.Count * 100))));
                                        dt = DateTime.UtcNow.AddMilliseconds(2000);
                                    }
                                    counter++;
                                }
            ...
            {
                OnLogEvent(string.Format(Properties.Resources.ErrorLoadingDynamicJobs, DriverName, ex.Message), EventLogEntryType.Error);
                loaded = false;
            }
        ...
        private IList<TagDefinition> GetInvalidTags(IList<Tag> tags)
        {
            ...
            {
                result.Add(tag.TagNode);
                OnTagChanged(tag.TagNode.NodeId, new DataValue(StatusCodes.BadConfigurationError));
                OnLogEvent(string.Format(Properties.Resources.InvalidTagDefinition, DriverName, tag.TagNode.NodeId.Identifier, tag.InvalidReason),EventLogEntryType.Error);
            }
        ...
        /// <returns>   true if it succeeds, false if it fails. </returns>
        ////////////////////////////////////////////////////////////////////////////////////////////////////
        private bool SaveDynamicJobs(IList<TagDefinition> invalidTags)
        {
            RemoveDynJobSettings();            
            ...
            using (IDataLayer idl = GetDynamicJobsDataLayer(out string filebase, out InMemoryDataStore InMemory, out bool targetIsFile))
            {
                using (UnitOfWork ufw = new UnitOfWork(idl))
                {
                    ...
                    dynamicJobs.DriverName = DriverName;
                    dynamicJobs.DriverVersion = DriverVersion();
                    dynamicJobs.LastInteraction = DateTime.UtcNow;

                    foreach (var tag in invalidTags)
                        dynamicJobs.Tags.Add(new TagSettings(ufw, tag , false));    // set tag invalid
                    ...
                    foreach (var stationname in Stations.Keys)
                    {
                        ...
                        {
#if DEBUG
                            job.Name = string.Format("ST{0}_{1}", stationname, job.TagsList[0].TagNode.Name);
#endif
                            ...
                            if (job.IsStruct())
                            {
                                var tagStruct = CreateTagSettings(ufw, job.StructTag);
                                tagStruct.MemberOrder = -1;
                                tagStruct.CommJobSettings = commJobSettings;
                                dynamicJobs.StructTags.Add(tagStruct);
                                foreach (var tag in job.TagsList)
                                {
                                    var tagSettings = CreateTagSettings(ufw, tag);
                                    tagSettings.CommJobSettings = commJobSettings;
                                    dynamicJobs.StructTags.Add(tagSettings);
                                }
                            }
                            else
                            {
                                foreach (var tag in job.TagsList)
                                {
                                    var tagSettings = CreateTagSettings(ufw, tag);
                                    tagSettings.CommJobSettings = commJobSettings;
                                    dynamicJobs.Tags.Add(tagSettings);
                                }
                            }
                        }
                    }                   
                    ...
                    if(!String.IsNullOrEmpty(filebase) && InMemory != null)
                    {
                        try
                        {
                            InMemory.WriteXml(filebase);
        ...
        {
            return Utilities.AssemblyInfo.FileVersion;
        }
        ...
        /// <summary>
        /// Indicates whether the driver has been started (running) or not (suspended or still starting)
        /// </summary>
        /// <returns></returns>
...
#region ICrossReferenceAware
        ...
        public Dictionary<string, string> GetTagList(ComunicationSettingsContext2 settingsContext)
        {
            ...
            using (IDataLayer idl = DriverCodeBaseEx.CommunicationDriver.GetDriverDataLayer(settingsContext.ConnectionString, DriverName, out string fileBase, out InMemoryDataStore InMemory))
            {
                using (UnitOfWork ufw = new UnitOfWork(idl))
                {
                    //try
        ...
        public void SetTagList(List<string> tagMap, ComunicationSettingsContext2 settingsContext)
        {
            ...
            using (IDataLayer idl = DriverCodeBaseEx.CommunicationDriver.GetDriverDataLayer(settingsContext.ConnectionString, DriverName, out string fileBase, out InMemoryDataStore InMemory))
            {
                using (UnitOfWork ufw = new UnitOfWork(idl))
                {
                    ...
                    {
                        // General
                        if (StateCommandVariable.IsTagsSet(configuration.StateCommandTag))
                        {
                        ...
                        if (StateCommandVariable.IsTagsSet(configuration.StateTag))
                        {
                            var tag = tagEntityReferenceMap.FirstOrDefault(t => t.NodeId == configuration.StateTag.NodeId);
                            if (tag != null)
                                configuration.StateTag = tag;
                        }

                        if (StateCommandVariable.IsTagsSet(configuration.CommandTag))
                        {
                            var tag = tagEntityReferenceMap.FirstOrDefault(t => t.NodeId == configuration.CommandTag.NodeId);
                            if (tag != null)
                                configuration.CommandTag = tag;
                        }
                        ...
                        foreach (var ch in channels)
                        {
                            if (StateCommandVariable.IsTagsSet(ch.StateCommandTag))
                            {
                            ...
                            if (StateCommandVariable.IsTagsSet(ch.StateTag))
                            {
                                var tag = tagEntityReferenceMap.FirstOrDefault(t => t.NodeId == ch.StateTag.NodeId);
                                if (tag != null)
                                    ch.StateTag = tag;
                            }

                            if (StateCommandVariable.IsTagsSet(ch.CommandTag))
                            {
                                var tag = tagEntityReferenceMap.FirstOrDefault(t => t.NodeId == ch.CommandTag.NodeId);
                                if (tag != null)
                                    ch.CommandTag = tag;
                            }
                        }
                        ...
                        foreach (var st in stations)
                        {
                            ...
                            if (StateCommandVariable.IsTagsSet(st.StateTag))
                            {
                                var tag = tagEntityReferenceMap.FirstOrDefault(t => t.NodeId == st.StateTag.NodeId);
                                if (tag != null)
                                    st.StateTag = tag;
                            }

                            if (StateCommandVariable.IsTagsSet(st.CommandTag))
                            {
                                var tag = tagEntityReferenceMap.FirstOrDefault(t => t.NodeId == st.CommandTag.NodeId);
                                if (tag != null)
                                    st.CommandTag = tag;
                            }
                        }
...
#region save data into storage
        ...
        protected ManualResetEvent ScheduleEvents;
        protected object lockSchedulerEvents = new object();        
        AutoResetEvent SchedulerWaitHandle;
        bool schedulerLaunchedAtStartUp = true;
        bool schedulerTerminated = true;

        protected virtual bool StartUpScheduler()
        {
            bool started = false;

            schedulerLaunchedAtStartUp = true;
            schedulerTerminated = false;            

            lock (lockSchedulerEvents)
            {
                foreach (Channel ch in Channels.Values)
                    ch.InitScheduling();

                if (SchedulerWaitHandle == null)
                    SchedulerWaitHandle = new AutoResetEvent(false);
                else
                    SchedulerWaitHandle.Reset();

                if (SchedulerThread == null)
                {
                    SchedulerThread = new Thread(DriverSchedulingThread);
                    SchedulerThread.IsBackground = true;
                    SchedulerThread.Priority = ThreadPriority.BelowNormal;
                    SchedulerThread.Name = "DriverScheduler";

                    if (!SchedulerThread.IsAlive)
                        SchedulerThread.Start(0);
                    started = SchedulerThread != null && (SchedulerThread.ThreadState & (System.Threading.ThreadState.Stopped | System.Threading.ThreadState.Unstarted)) == 0;
                }
            }
        ...
        public virtual bool RestartScheduler()
        {
            bool started = false;
            if (!schedulerLaunchedAtStartUp)
                return started;
        ...
        protected virtual void TerminateScheduler()
        {
            ...
            lock (lockSchedulerEvents)
            {
                ...
                if (SchedulerWaitHandle != null)
                {
                    SchedulerWaitHandle.Dispose();
                    SchedulerWaitHandle = null;
                }
            }
        }   

        public void ScheduleChannelEventSet()
        {
            lock (lockSchedulerEvents)
            {
                if (SchedulerWaitHandle != null)
        ...
        public virtual void SuspendScheduler()
        {
            TerminateScheduler();
        }

        protected void DriverSchedulingThread(object data)
        {
            int minWaitTimeOut = 0; // force to execute immediatly schedule
            DateTime scheduledDT = DateTime.UtcNow;

            bool raised = false;
            while (true)
            {
                // wait some channel to raise event for schedule/write/ecc
                raised = SchedulerWaitHandle.WaitOne(minWaitTimeOut);
                if (bDisposed || schedulerTerminated)
                    break;
                
                scheduledDT = DateTime.UtcNow;
                
                //System.Diagnostics.Debug.WriteLine("DriverSchedulingThread DriverSchedulingThread start=" + DateTime.UtcNow.ToString("hh:mm:ss.fff tt"));
                //System.Diagnostics.Stopwatch s = new System.Diagnostics.Stopwatch();
                //s.Start();

                minWaitTimeOut = -1;
                
                // retrieve all channel with request to schedule (write, no jobs to process, etc) and with next rescheduling set
                List<Channel> channels = (from channel in GetChannels()
                                          where channel.IsRequestScheduleSet() || channel.IsNextScheduleSet()
                                         select channel).ToList();

                foreach (Channel channel in channels)
                {
                    // try to schedule jobs and/or retrieve next schedule time
                    int nextSchedule = channel.Schedule(scheduledDT);
                    // evaluate next scheduling "delay"
                    if (nextSchedule != -1)
                    {
                        if (minWaitTimeOut == -1)
                        {
                            minWaitTimeOut = nextSchedule;
                        }
                        else
                        {
                            if (nextSchedule < minWaitTimeOut)
                                minWaitTimeOut = nextSchedule;
                        }
                    }
                }

                //Channel.PerformanceLogToFile(string.Format("DriverSchedulingThread nextSchedule:{0} ", minWaitTimeOut));

                //System.Diagnostics.Debug.WriteLine("DriverSchedulingThread DriverSchedulingThread stop=" + DateTime.UtcNow.ToString("hh:mm:ss.fff tt") + ", minWaitTimeOut =" + minWaitTimeOut.ToString() + ",TotalScheduleTime=" + s.ElapsedMilliseconds.ToString());
            }
        }
        #endregion
    }
