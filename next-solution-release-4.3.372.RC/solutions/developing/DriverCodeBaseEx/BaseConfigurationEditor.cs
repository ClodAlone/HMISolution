...
namespace DriverCodeBaseEx
{
    public abstract class BaseConfigurationEditor : IDisposable
    {
        ...
        public List<object> GetStationList(string channelname)
        {
            if (IsValid)
            {
                var l = new List<object>((from s in configuration.StationSettings
                         where s.Channel == channelname 
                         select s).ToList());
                return l;
            }
            return null;
        ...
        /// <summary>
        /// <summary>   Make a copy (.bak) of driver settings and the delete it.
        /// </summary>
        ...
        public void BackupAndDeleteDriverSettings(string connectionString, string drivername, bool protection, Guid Code, out string backupFile)
        {
            DriverCodeBaseEx.CommunicationDriver.BackupAndDeleteDriverSettings(protection, Code, connectionString, drivername, out backupFile);
        }
