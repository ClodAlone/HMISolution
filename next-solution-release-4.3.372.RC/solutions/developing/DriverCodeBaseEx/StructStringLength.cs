...
namespace DriverCodeBaseEx
{
    public class StructStringLength
    {
        ...
        public StructStringLength(uint maxStringLength = 255)
        {
            _MAX_STRING_LENGTH = maxStringLength;
            InitBase();
        }
        ...
        {
            return (stringLength > 0 && stringLength <= _MAX_STRING_LENGTH);
        }
        ...
        private Dictionary<string, ProMemberView> ParseStructStringLength(string structStringLength, out bool error)
        {
            ...
            if (defaultStringLength == 0 && members.Count == 0)
                defaultStringLength = _MAX_STRING_LENGTH;
        ...
        private void ImportPrototypeMembers(string ancestorPrototypeName, IDynamicSettingsEditing tag, ref ProMemberView memberView)
        {
            ...
            {                
                ...
                {
                    ProMemberView newMemberView = new ProMemberView(member.Name, memberView.FullName, _MAX_STRING_LENGTH,UFUAModel.ModelType.Variable);
                    memberView.Members.Add(newMemberView);
                }
                else if (member.IsObjectType)   // nested struct's element
                {
                    ProMemberView newMemberView = new ProMemberView(member.Name, memberView.FullName, _MAX_STRING_LENGTH, UFUAModel.ModelType.ObjectType);
        ...
        private ProMemberView GetMemberView(string memberName)
        {
            ...
            return getMemberView(memberName, MemberView, memberLevel, ref currentLevel);
        }
        ...
        public string UnSplitToStructString(ProMemberView memberView)
        {
            ...
            StructStringLength.ProMemberView Member0 = members.First().Value;
