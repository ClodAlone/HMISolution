...
using AuthenticationCredentialsProvider.Model;
#endif
...
using ExternalAuthentication.Browser;
...
namespace AuthenticationCredentialsProvider.ComponentService
{
...
#endif
    {
        ...
        private IOidcClientServiceExtension _oidcClientServiceExtension;
        private IExternalLoginService _externalLoginService;
        private IExternalLogoutService _externalLogoutService;
        private bool _isExternalAuthActive;
        private ExternalIdPUserSettings _externalIdPUserSettings;
        private string _applicationName;
#if !IO_Server
        IWorkspace workspace;
        static List<String> listFaulted = new List<String>();
        static Dictionary<String, LoginDetails> usersDictionary = new Dictionary<String, LoginDetails>();
        static System.Timers.Timer timerCleanFaulted;
...
#if !IO_Server
        public AuthenticationCredentialsProviderComponent() {}
#endif
        #endregion
        ...
        static void StartFualtedCleaner()
        {
            lock (listFaulted)
            {
                ...
                {
                    timerCleanFaulted = new System.Timers.Timer(10000);
    ...
    #region IAuthenticationCredentialsProvider Members
        ...
        /// </summary>
        void OnUserOnline(String applicationName, String user, String password, bool isRefreshing = false, String role = null, UserIdentity userIdentity = null, bool isExternalAuthenticationActive = false)
        {
            var e = UserOnline;
            if (e != null)
            {
                e (this, new LoginInfoEventArgs() { User = user, Password = password, Role = role, UserIdentity = userIdentity, ApplicationName = applicationName, IsRefreshing = isRefreshing, IsExternalAuthenticationActive = isExternalAuthenticationActive });
            }
        }
        #endregion
        ...
        /// <summary>
        /// Triggers the FaultedProvider event.
        /// </summary>
        void OnFaultedProvider(String applicationName, string errorInfo = null)
        {
            var e = FaultedProvider;
            if (e != null)
            {
                e(this, new LoginInfoEventArgs() { ApplicationName = applicationName, ErrorInfo = errorInfo });
            }
        }
...
#region OnPromptUserLogin
        ...
        private void InitializeExternalAuthenticationServices(ExternalIdPUserSettings externalIdPUserSettings)
        {
            _oidcClientServiceExtension = new OidcClientServiceExtension(new SetOidcClient(), new DesktopWebBrowser(), externalIdPUserSettings);

            _externalLoginService =
                new ExternalLoginService(
                    _oidcClientServiceExtension,
                    ...
                    new SetOidcClientLoginResultService(
                        new CreateUserTokenFromJwtService(),
                        new GetRolesListService()));

            _externalLoginService.LoginProcedureCompleted += (o, e) =>
            {
                using (new WaitCursor())
                {
                    var loginResultArgs = e as LoginProcedureCompletedArgs;

                    if (loginResultArgs != null && loginResultArgs.LoginResult.IsLoginSuccessful)
                    {
                        OnUserOnline(
                               _applicationName,
                               loginResultArgs.LoginResult.Username,
                               null,
                               false,
                               loginResultArgs.LoginResult.Roles[0],
                               loginResultArgs.LoginResult.UserIdentity, 
                               true);
                    ...
                    {
                        OnFaultedProvider(_applicationName, loginResultArgs.LoginResult.ErrorMessage);
                    }
                }
            };

            _externalLogoutService = new ExternalLogoutService(_oidcClientServiceExtension);

            _externalLogoutService.LogoutProcedureCompleted += (o, e) =>
            {
                OnUserOnline(_applicationName, String.Empty, String.Empty);
                RemoveItemFromUsersMap(_applicationName);
            };
        }

        private void CheckIfExternalAuthenticationIsEnabledAndActivateServices(IDocument document)
        {
            if (_externalLoginService == null || _externalLogoutService == null)
            {
                var parentDocument = DocumentManager.ComponentService.Helpers.DocumentHelper.GetRootParent(document, true);

                if (parentDocument != null)
                {
                    IUFUserEditorManager userManager = parentDocument.GetService(typeof(IUFUserEditorManager)) as IUFUserEditorManager;

                    if (userManager != null)
                    {
                        var getExternalAuthenticationSettings =
                            new GetExternalAuthenticationSettings(new ConvertGeneralSettingsToExternalIdPUserSettings(), userManager);

                        _externalIdPUserSettings = getExternalAuthenticationSettings.Execute(parentDocument);

                        _isExternalAuthActive = _externalIdPUserSettings.ExternalIdentityProviderActive;

                        if (_isExternalAuthActive)
                        {
                            InitializeExternalAuthenticationServices(_externalIdPUserSettings);
                        }
                    }
                }
            }
        ...
        { 
            ...
            try
            {
                CheckIfExternalAuthenticationIsEnabledAndActivateServices(document);

                if (_externalLoginService != null && _isExternalAuthActive)
                {
                    _applicationName = applicationName;
                    _externalLoginService.Execute(_externalIdPUserSettings);
                    return null;
                }
        ...
        LoginAttemptResult ProcessCredentials(bool bWithUI, String applicationName, IDocument document = null, String requestedRoleOrUser = null, int requestedLevel = 0, Window owner = null, String user = null, String psw = null, bool bRetryOnFailed = false, bool bApplyUserIndentity = true)
        {
            ...
            lock (lockObject)
            {
                ...
                if (isAuthorized && bApplyUserIndentity)
                {
                    ...
                    AddItemToUsersMap(applicationName, ret.UserName, ret.Password, null, null);
                }
            }
        ...
        private void AddItemToUsersMap(string applicationName, string username, string password, string role, UserIdentity userIdentity)
        {
            var loginDetails = new LoginDetails(username, password, role, userIdentity); 
            lock (usersDictionary)
            {
                if (usersDictionary.ContainsKey(applicationName))
                    usersDictionary.Remove(applicationName);
                usersDictionary.Add(applicationName, loginDetails);
            }
        }
        ...
        public string RefreshCurrentUser(String applicationName)
        {
            ...
            try
            {
                lock (lockObject)
                {
                    ...
                    lock (usersDictionary)
                    {
                        var userDetails = (from c in usersDictionary where c.Key.Equals(applicationName) select c.Value).FirstOrDefault();
                        if (userDetails != null)
                        {
                            username = userDetails.Username;
                            userPsw = userDetails.Password;
                            userRole = userDetails.Role;
                            userIdentity = userDetails.UserIdentity;
                        }
                    }

                    if (!String.IsNullOrEmpty(username))
                        OnUserOnline(applicationName, username, userPsw, true, userRole, userIdentity, _isExternalAuthActive);   
                    else
        ...
        public int GetNumberOfUsersOnline(String applicationName)
        {
            ...
            try
            {
                lock (lockObject)
                {
                    lock (usersDictionary)
                    {
                        var list = (from c in usersDictionary where c.Key.Equals(applicationName) select c).ToList();
                        if (list.Count > 0)
                            return list.Count;
                    }
        ...
        public void Logout(String applicationName, bool openBrowserOnExternalAuth = true)
        {
            if (IsFaulted(applicationName))
                return;
            ...
            try
            {
                if (_externalLogoutService != null && _isExternalAuthActive)
                {
                    _applicationName = applicationName;
                    _externalLogoutService.Execute(openBrowserOnExternalAuth);
                    return;
                }
                ...
                lock (lockObject)
                {
                    ...
                    try
                    {
                        ...
                        if (ret != null)
                        {
                            ...
                            RemoveItemFromUsersMap(applicationName);
                        }
                        else
                        {
                            OnUserOnline(applicationName, String.Empty, String.Empty);
                            ...
                            lock(usersDictionary)
                            {
                                var list = (from c in usersDictionary where c.Key.Equals(applicationName) select c.Key).ToList();
                                list.ForEach(app => usersDictionary.Remove(app));
                            }
                        }
        ...
        private void RemoveItemFromUsersMap(string applicationName)
        {
            lock (usersDictionary)
            {
                if (usersDictionary.ContainsKey(applicationName))
