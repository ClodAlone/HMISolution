...
{
    public class PrototypeMemberDesc
    {
        public PrototypeMemberDesc(string name, string type)
        {
            Name = name;
            Type = type;
        }
        ...
        IDocument, IXpoDocument
    {
        ...
        internal void RenameReferences(Dictionary<string, string> nodeIdMap, UFInterfaces.Editors.CrossReferenceModel model)
        {
            ...
            using (var cursor = new WaitCursor())
            {
                ...
                var historicalSettings = (from historian in GetHistoricalSettings()
                                          where 
                                          historian.EnableTag != null ||
                                          historian.ResettingTag != null
                ...
                historicalSettings.ForEach(historian =>
                {
                    ...
                    if (historian.ResettingTag != null && !historian.ResettingTag.IsEmpty())
                        list.Add(historian.ResettingTag);
                });
                ...
                var alarmThresholds = (from alarmThreshold in GetAlarmThresholds(false)
                                       where
                                        ...
                                        alarmThreshold.SeverityTag != null ||
                ...
                alarmThresholds.ForEach(alarmThreshold =>
                {
                    ...
                    if (model.ApplyNewNames)
                    {
                        ...
                        if (RenameCommand(command, model))
                        {
                            bDirty = true;
                    ...
                    //if(getTags)
                    {
                        ...
                        if (alarmThreshold.SeverityTag != null && !alarmThreshold.SeverityTag.IsEmpty())
                            list.Add(alarmThreshold.SeverityTag);
                ...
                foreach (var node in mapNodes.Keys)
                {
                    ...
                    found.ForEach(fitem =>
                    {
                        ...
                        if (newEntity != null)
                        {
                            ...
                            (from item in historicalSettings where item.ResettingTag != null && item.ResettingTag.ResolveNodeId(ns).ToString() == nodeid select item).ToList().ForEach(item =>
                            {
                                item.ResettingTag = newEntity;
                                bDirty = true;
                            });
                            ...
                            (from item in alarmThresholds where item.SeverityTag != null && item.SeverityTag.ResolveNodeId(ns).ToString() == nodeid select item).ToList().ForEach(item =>
                            {
                                item.SeverityTag = newEntity;
                                bDirty = true;
                            });
        ...
        private bool RenameCommand(CommandManagerList commandList, UFInterfaces.Editors.CrossReferenceModel model)
        {
            bool bDirty = false;
            if(commandList != null)
        ...
        internal List<DataLoggerSettings> GetDataLoggersWithTag(UFUAModel.UFUATag tag)
        {
            List<DataLoggerSettings> ret = new List<DataLoggerSettings>();

            //Recupero tutti i DataLogger
            var dlrList = (from p in new XPQuery<DataLoggerModel.DataLoggerSettings>(uow, true).AsParallel()
                                                                select p);

            //Per tutti i DataLogger, cerco nelle colonne se è usata la tag che sto cercando
            foreach (DataLoggerModel.DataLoggerSettings dlr in dlrList)
            {
                DataLoggerColumn dlrColumn = (from p in new XPQuery<DataLoggerModel.DataLoggerColumn>(uow, true).AsParallel()
                                                                        where p.DataLoggerReference == dlr &&
                                                                              p.ColumnTag.NodeId.Identifier.ToString() == tag.NodeId.ToString()
                                                                        select p).FirstOrDefault();
        ...
        public bool IsDriverNameUsed(String name)
        {
            ...
            return IsDriverNameUsed(name, ref tags);
        }

        public bool IsDriverNameUsed(string name, ref List<UFUATag> tags)
        {
            tags = (from tag in new XPQuery<UFUAModel.UFUATag>(uow, true).AsParallel()
        ...
        internal static void DeleteDriverSettings(UFUAModel.UFUACommunicationDriver driver, UFUAServerDocument doc)
        {
            String path = String.Format("{0}\\{1}\\{2}\\", doc.rootBase, Properties.Settings.Default.TypeLabel, UFUAServerInfo.UFUAServerInfo.GetDriverRootName());
            var filesBelongingToDriver = Directory.EnumerateFiles(path).Where(f => f.Contains(driver.PathIdentifier + ".")).ToList();
            foreach(var file in filesBelongingToDriver)
            {
                File.Delete(file);
            }
        }

        internal static void UpdateDynamicSettingsAfterDriverCancellation(UFUACommunicationDriver deletedDriver, List<UFUATag> tags)
        {
            var affectedTags = 
                    tags.Where(tag => tag.DynamicSettings.Contains(deletedDriver.UniqueIdentifier))
                        .AsParallel()
                        .ToList();
            ...
            Parallel.ForEach(affectedTags, affectedTag =>
            {
                var newDynamicSettings = string.Empty;

                if (affectedTag.DynamicSettings.Contains('$'))
                {
                    var singleTagAddresses = affectedTag.DynamicSettings.Split('$').ToList();

                    var singleTagAddressesNotContainingUniqueIdentifier =
                            singleTagAddresses.Where(address => address.Contains(deletedDriver.UniqueIdentifier) == false)
                                              .ToList();

                    singleTagAddressesNotContainingUniqueIdentifier.ForEach(tagAddress =>
                    {
                        newDynamicSettings = tagAddress + "$";
                    });

                    newDynamicSettings = newDynamicSettings.Substring(0, newDynamicSettings.LastIndexOf("$"));
                }

                affectedTag.DynamicSettings = newDynamicSettings;
            });   
        }
        ...
        {
            return GetNewThresholdList(hs, tag, model.AssignAlarmType, model.PrefixAlarmText, model.SetAlarmText);
        }
#endif

        public List<UFUAModel.UFUAAlarmThreshold> GetNewThresholdList(UFUAModel.UFUAAlarmDefinition hs, UFUAModel.UFUATag tag, AssignAlarmType alrType, string alrText, bool setText = false)
        {
            var ret = new List<UFUAModel.UFUAAlarmThreshold>();
            ...
            if (setText)
            {
                ...
                ret.ForEach((alr) =>
                {
                    if (alrText == null || String.IsNullOrEmpty(alrText.Trim()))
                    {
                        alr.UFUATagAss = tag;
                    ...
                    {
                        alr.AlarmText = (ret.Count > 1 ? String.Format("{0}.{1}", alrText, ++counter) :
                            alrText);
                    }
                });
            }
            ...
            using (CachedUnitOfWork task = cachedUnitOfWorks.BeginUnitOfWork())
            {
                ...
                else
                {
                    ...
                    if (tagfound != null)
                    {
                        ...
                        {
                            refTagFound = GetTagOPCUAEntityReference(tagfound, null, false)?.ToXml();
                            result = string.IsNullOrEmpty(refTagFound);
                        }
                    }   
                }
            }
        ...
        internal IDictionary<String, IList<string>> GetListPrototypesDesc()
        {
            using (var task = cachedUnitOfWorks.BeginUnitOfWork())
            {
                var map = new Dictionary<String, IList<string>>();
                (from tag in new XPQuery<UFUAModel.UFUATagPrototype>(task.UnitOfWork, true).AsParallel()
                 ...
                 select tag).ToList().ForEach(prototype =>
                 {
                     var members = (from tag in prototype.Members select (string.IsNullOrEmpty(tag.PrototypeName) ? tag.Name : string.Format("{0}|{1}", tag.Name, tag.PrototypeName))).ToList();
                     if (!map.ContainsKey(prototype.Name))
                         map.Add(prototype.Name, members);
                 });
        ...
        public UFUAModel.UFUATag GetUFUATag(string tagname, string membername, bool bForceSubPrototypeCreation, bool inExecution = false)
        {
            ...
            return GetUFUATag(ref tagString, ref instance, inExecution: inExecution, bForceSubPrototypeCreation: bForceSubPrototypeCreation);
        }
        ...
        private UFUATag GetUFUATag(ref String tagName, ref String instance, bool inExecution, List<UFUAModel.UFUATag> list = null, bool bForceSubPrototypeCreation = false)
        {
            ...
            try
            {
                ...
                if (tagfound == null)
                {
                    var tags = new XPCollection<UFUATag>(uow, true)
                        .Where(tag => !tag.IsPrototypeMember)
                        .AsQueryable();

                    if (tagString.Contains('\\'))
                    {
                        tagfound = tags.FirstOrDefault(t => Compare($"{t.FolderPath}\\{t.Name}", tagString)) ??
                                   tags.FirstOrDefault(t =>
                                       Compare($"{t.FolderPath}\\{t.OriginalName}", tagString) ||
                                       Compare($"{t.OriginalFolderPath}\\{t.Name}", tagString) ||
                                       Compare($"{t.OriginalFolderPath}\\{t.OriginalName}", tagString));
                    }
                    else
                    {
                        tagfound = tags.FirstOrDefault(t => CompareNotEmpty(t.Name, t.FolderPath, tagString)) ??
                                   tags.FirstOrDefault(t => CompareNotEmpty(t.OriginalName, t.FolderPath, tagString));
                    }

                    if (tagfound == null && tagString.Contains('_'))
                    {
                        tagfound = tags.FirstOrDefault(t => CompareAndReplace(t.FolderPath, t.Name, tagString)) ??
                                   tags.FirstOrDefault(t =>
                                       CompareAndReplace(t.FolderPath, t.OriginalName, tagString) ||
                                       CompareAndReplace(t.OriginalFolderPath, t.Name, tagString) ||
                                       CompareAndReplace(t.OriginalFolderPath, t.OriginalName, tagString));
                    }
                }
        ...
        private static bool CompareNotEmpty(string name, 
            string folderPath,
            string tagString)
        {
            return string.IsNullOrEmpty(folderPath) && Compare(name, tagString);
        }

        private static bool CompareAndReplace(string folderPath,
            string name,
            string tagString)
        {
            return Compare($"{folderPath}\\{name}".Replace('\\', '_'), tagString);
        }

        private static bool Compare(string compare1, string tagString)
        {
            return string.Compare(compare1, tagString, StringComparison.OrdinalIgnoreCase) == 0;
        }
        ...
        internal OPCUAEntityReference GetWriteVarValuesOPCUAEntityReference()
        {
            string relativepath = string.Empty;
            Opc.Ua.NamespaceTable n = new Opc.Ua.NamespaceTable();
            UInt16 ns = (UInt16)(n.Count + 2 - 1);
            ...
            var applicationName = GetAplicationName();
            var nodeId = new NodeId(UFUAServerInfo.BrowserNames.WriteVarValues, ns); // Second parameter should be replaced with 'NamespaceIndex'
            return new OPCUAEntityReference(null, applicationName, GetDefaultLocalEndpoint(),
                                            null, nodeId, string.Format("{0} ({1})", UFUAServerInfo.BrowserNames.WriteVarValues, applicationName), null, null);
        }
        ...
        private void AddDynamicSettings(UFUAModel.UFUATag tag, UFUAModel.ImportTag itag)
        {
            // Check if the dynamic link of the imported variable is valid
            if(String.IsNullOrWhiteSpace(itag.DynSettings))
            {
                // The dynamic link of the imported tag is invalid
                return;
            }
            int index = itag.DynSettings.IndexOf('.');
            if (index <= 0)
            {
                // The dynamic link of the imported tag is invalid
                return;
            ...
            // The dynamic link of the existing tag is empty --> Replace it with the one of the imported tag
            if (String.IsNullOrWhiteSpace(tag.DynamicSettings))
            {
            ...
            // Driver name + '.'
            string driverName = itag.DynSettings.Substring(0, index + 1);

            // Settings for this driver are already present in the dynamic link of the existing tag?
            index = tag.DynamicSettings.IndexOf(driverName);
            if (index < 0)
            {
                // Add the dynamic link of the imported tag to the one of the existing tag
                tag.DynamicSettings += "$" + itag.DynSettings;
            }
            else
            {
                // Replace the driver settings in the dynamic link of the existing tag with the dynamic link of the imported tag
                string[] driverSettings = tag.DynamicSettings.Split('$');
                if(driverSettings.Length <= 1)
                {
                    // Only one driver is present in the dynamic link of the existing tag --> Replace it with the one of the imported tag
                    tag.DynamicSettings = itag.DynSettings;
                }
                else
                {
                    // Recompose the dynamic link of the existing tag substituting only the settings of the driver of the imported tag
                    tag.DynamicSettings = String.Empty;
                    int i = 0;
                    for (i = 0; i < driverSettings.Length; i++)
                    {
                        if (i > 0)
                        {
                            tag.DynamicSettings += "$";
                        }
                        if (driverSettings[i].IndexOf(driverName) >= 0)
                        {
                            tag.DynamicSettings += itag.DynSettings;
                        }
                        else
                        {
                            tag.DynamicSettings += driverSettings[i];
                        }
                    }
                }
            }
        ...
        internal List<UFUAModel.UFUATag> AddImportedTag(List<UFUAModel.ImportTag> itags, UFUAModel.UFUAFolder root, ref CustomDialogResults dialogRetValue)
        {
            ...
            foreach (var itag in itags)
            {
                ...
                if (tag != null)
                {
                    if (itag.BehaviorForExistingTags == BehaviorExistingTagsValues.Ignore)
                    {
                        continue;
                    }
                    else if (itag.BehaviorForExistingTags == BehaviorExistingTagsValues.Overwrite)
                    {
                        ...
                        if (itag.BehaviorForDynamicLink == BehaviorDynamicLinkValues.ReplaceExisting)
                        {
                            tag.DynamicSettings = itag.DynSettings;
                        }
                        else
                        {
                            AddDynamicSettings(tag, itag);
                        }
        ...
        internal UFUAModel.UFUATagPrototype AddImportedPrototype(UFUAModel.ImportPrototype iproto,
            TagsImportProgressViewModel importVeiwModel,
            ref CustomDialogResults dialogRetValue)
        {
            var editView = ActiveView as UFUAEditorControl;
            ...
            if (prototype != null)
            {
                    ...
                    dialogRetValue != CustomDialogResults.Cancel)
                {
                    ...
                    if (uiMsgBox != null)
                    {
                        importVeiwModel?.HideProgressBarImport();

                        using (new ResetCursor())
                        {
                            dialogRetValue = uiMsgBox.ShowYesNoAllCancel(string.Format(Properties.Resources.ImportPrototypeNameExist, protoname), CustomDialogIcons.Question);
                        }

                        if (importVeiwModel != null)
                        {
                            if (dialogRetValue == CustomDialogResults.Cancel)
                                importVeiwModel.CloseProgressBarImport();
                            else
                                importVeiwModel.ShowProgressBarImport();
                        }
                    }
                }
                ...
                if (dialogRetValue == CustomDialogResults.No || dialogRetValue == CustomDialogResults.NoAll)
                {
                    if (editView != null)
        ...
        internal void AddImportedPrototypes(List<UFUATagPrototype> prototypes)
        {
            var editView = ActiveView as UFUAEditorControl;
            ...
            if (editView != null)
                editView.AddPrototypeList(list);
        }
...
#if !NET_STANDARD
        internal IList<UFUAEngineeringUnit> AddImportedEngineeringUnits(List<ImportEngineeringUnit> items)
        {
            IList<UFUAEngineeringUnit> currentEUnits = GetEngineeringUnits();
            List<UFUAEngineeringUnit> eUnits = new List<UFUAEngineeringUnit>();
            foreach (var item in items)
            {
                if (!currentEUnits.Any(x => x.Name == item.UNECECode))
                {
                    var eu = new UFUAEngineeringUnit(uow)
                    {
                        Name = item.UNECECode,
                        UnitId = item.UnitId,
            ...
            return eUnits.OrderBy(x => x.Name).ToList();
        }

        internal void RemoveImportedPrototypes(List<UFUATagPrototype> prototypesToDelete)
        {
            var editView = ActiveView as UFUAEditorControl;
            if (editView != null)
            {
                editView.RemoveImportedPrototypes();

                var prototypes = GetPrototypes();
                foreach (var prototype in prototypes)
                {
                    if (prototypesToDelete.Contains(prototype))
                    {
                        prototype.Delete();
                    }
                }

                editView.UpdateUserControl();
            }
