...
namespace DriverCodeBase
{
    ...
    public abstract class DriverSettings : XPObject, IDataErrorInfo
    {
        ...
        public virtual string GetNewChannelName()
        {
            ...
            do
            {
                if ((from c in ChannelSettings/*.AsParallel()*/ where c.Name == chName select c).ToList().Count == 0)
                    return chName;
        ...
        public virtual string GetNewStationName()
        {
            ...
            do
            {
                if ((from c in StationSettings/*.AsParallel()*/ where c.Name == stName select c).ToList().Count == 0)
                    return stName;
