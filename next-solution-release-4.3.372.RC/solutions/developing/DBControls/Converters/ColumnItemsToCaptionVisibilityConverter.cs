...
namespace DBControls.Converters
{
    public class ColumnItemsToCaptionVisibilityConverter : IMultiValueConverter
    {
        ...
        {
            if (values.Length < 3 || values[1] as ImageThresholdCollection == null || (values[1] as ImageThresholdCollection).Count == 0)
                return Visibility.Visible;

            bool bIsInEditMode = values.Length > 4 && values[3] is Boolean && (Boolean)values[3];
            if (bIsInEditMode)
            ...
            if (values.Length >= 4)
            {
                ...
                var columnName = values[4].ToString();

                var thc = new ThresholdToImageConverter();
                var thr = thc.FindMatchingThreshold(cellValue, columnName, imgThresholds);
