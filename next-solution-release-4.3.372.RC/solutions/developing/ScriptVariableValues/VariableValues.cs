...
namespace ScriptVariableValues
{
    ...
    public class VariableValues : IReDispatch, System.Runtime.InteropServices.ComTypes.ITypeInfo, IDisposable
    {
        ...
        readonly HashSet<String> resolvedItems = new HashSet<string>();
        readonly HashSet<String> needToBeresolvedItems = new HashSet<String>();
        readonly HashSet<String> isReadOnlyItems = new HashSet<String>();
        // begin inner support - TCB
        readonly Dictionary<int, VariableValues> innerVariableValues = new Dictionary<int, VariableValues>();
        // end inner support - TCB

        readonly String instancename;
        bool bResolveVariables;
        // bool bFoundVariableNameSuspect;

        public static readonly string ParentPrefix = @"..\";
        public static readonly String initialQuality = new Opc.Ua.StatusCode(Opc.Ua.StatusCodes.BadWaitingForInitialData).ToString();

        public VariableValues(bool bResolveVar = true, VariableValues parent = null)
        {
        ...
        public VariableValues(String n, bool bResolveVar = true, VariableValues parent = null)
        {
        ...
        public int GetInnerVarCount()
        { 
            return innerVariableValues.Count; 
        }

        public VariableValues GetInnerVariable(int index)
        { 
            if(index >= 0 && index < innerVariableValues.Values.Count)
                return innerVariableValues.Values.ToList()[index];
            return null;
        }

        public VariableValues GetInnerVariable(string name)
        {
            var res = (from c in innerVariableValues.Values where c.instancename == name select c).ToList();
            if (res.Count > 0)
                return res[0];
            return null;
        }
        ...
        public string RuntimeRealName { get; set; }
        public string RuntimeRealInstance { get; set; }


        public string GetCompleteName()
        {
            if (Parent == null)
                return isChildProjectTagsContainer ? _childProjectName : instancename;
            else
                return string.Format("{0}\\{1}", Parent.GetCompleteName(), instancename);
        }
        public string GetChildProject()
        {
            if (Parent == null)
                return isChildProjectTagsContainer ? _childProjectName : string.Empty;
            else
                return Parent.GetChildProject();
        }
        public string GetFirstParentName()
        {
            if (IsRuntimeOnly && RuntimeRealInstance != null && RuntimeRealInstance.IndexOf(VariableValues.ParentPrefix) != -1)
                return string.Empty;
            if (IsRuntimeOnly)
                return RuntimeRealInstance;
            if (Parent == null)
                return instancename;
            else
                return Parent.isChildProjectTagsContainer ? instancename : Parent.GetFirstParentName();
        }

        public string GetRealName(string name)
        {
            if (name.IndexOf(VariableValues.ParentPrefix) != -1)
                return name;
            string instance = name;
            if (IsRuntimeOnly)
            {
                return RuntimeRealName;
                return string.Format("{0}\\{1}", RuntimeRealName, name);
            }
            if (!String.IsNullOrEmpty(GetName()))
            {
                instance = string.Format("{0}\\{1}", GetCompleteName(), name);
            }
            if (!string.IsNullOrEmpty(ChildProjetName) && instance.StartsWith(ChildProjetName))
                instance = instance.Substring(ChildProjetName.Length);
            if(instance.StartsWith("\\"))
                instance = instance.Substring(1);
            var fpn = GetFirstParentName();
            if(instancename != null && instance.StartsWith(fpn))
                instance = instance.Substring(fpn.Length);
            if (instance.StartsWith("\\"))
                instance = instance.Substring(1);
            return instance;
        }
        public string GetKeyInstanceName(string name)
        {
            if(name.IndexOf(VariableValues.ParentPrefix) != -1)
                return name;
            var instance = name;
            if (!String.IsNullOrEmpty(GetName()))
            {
                string rightInstance = GetCompleteName();
                rightInstance = rightInstance.Replace("\\", "-");
                instance = String.Format("{0}-{1}", rightInstance/*variableValue.GetName()*/, name);
            }
            //if (!string.IsNullOrEmpty(ChildProjetName))
            //    instance = String.Format("{0}-{1}", ChildProjetName, instance);
            return instance;
        }

        public VariableValues Parent { get; private set; }
        public bool isDataService { get; set; }
        
        public bool isChildProjectTagsContainer { get; set; }
        string _childProjectName = string.Empty;
        public string ChildProjetName
        {
            get
            {
                if (string.IsNullOrEmpty(_childProjectName) && Parent != null)
                    return Parent.ChildProjetName;
                else
                    return _childProjectName;
            }
            set { _childProjectName = value; }
        }

        HashSet<String> idLookupChecking;
        int nCounter = 0;
        public void PrepareAddNewVariable()
        {
            idLookupChecking = new HashSet<String>();
        ...
        // begin inner support - TCB
        public VariableValues AddInnerVariable(String Name, bool bResolverVar = false/*true*/)
        {
            var values = new VariableValues(Name /*+ "Struct"*/, bResolverVar, this);
            Name = Name.Replace('\\', '_');
            AddVariable(Name);
            SetVariableValue(Name, values);
            if (!isReadOnlyItems.Contains(Name))
                isReadOnlyItems.Add(Name);

            innerVariableValues.Add(idLookup[Name], values);
            return values;
        }

        // end inner support - TCB

        public VariableValues GetVariableValueByName(String name)
        { 
            ...
            if(members.Length == 1)
            {
                if (idLookup.ContainsKey(name))
                {
                    return this;// innerVariableValues[idLookup[name]];
                }
            }
            else
            {
                if (idLookup.ContainsKey(members[0]))
                {
                    string newname = name.Replace(string.Format("{0}-", members[0]), "");
                    return innerVariableValues[idLookup[members[0]]].GetVariableValueByName(newname);
                }
            }
            return null;
        }
        ...
        public void ForceResolveVariables()
        {
            ...
            foreach (var variableValue in innerVariableValues.Values)
        ...
        bool isMemberStruct(int idx)
        {
            lock (values)
            {
                return (values.ContainsKey(idx) && values[idx] is VariableValues);
            }
        }
        ...
        {
            Object value = null;
            bool bStruct = isMemberStruct(dispIdMember);
            if (bResolveVariables || bStruct)
            {
            ...
            if ((wFlags & INVOKE_PROPERTYPUTREF) != 0)
            {
                ...
                if (pDispParams.cArgs == 1)
                {
                    ...
                    OnWriteVariable(new WriteVariableEventArgs(idReverseLookup[dispIdMember], NewValue, this));
                }
                else
                {
                    // array specific code
                    ...
                    OnWriteVariable(new WriteVariableEventArgs(idReverseLookup[dispIdMember], array, this));
                }
            }
        ...
        public void GetDocumentation(int index, out string strName, out string strDocString, out int dwHelpContext, out string strHelpFile)
        {
            ...
            else if (!bResolveVariables || !isMemberStruct(index + 1))
            {
                // index is an id
        ...
        public void GetTypeAttr(out IntPtr ppTypeAttr)
        {
            ...
            typeattr.wTypeFlags = System.Runtime.InteropServices.ComTypes.TYPEFLAGS.TYPEFLAG_FAPPOBJECT;
        ...
        public void GetVarDesc(int index, out IntPtr ppVarDesc)
        {
            ...
            // start inner support - TCB
            if (!innerVariableValues.ContainsKey(index + 1))
                vardesc.elemdescVar.tdesc.vt = (short)System.Runtime.InteropServices.VarEnum.VT_VARIANT;
            else
            {
                vardesc.elemdescVar.tdesc.vt = (short)System.Runtime.InteropServices.VarEnum.VT_USERDEFINED;
                vardesc.elemdescVar.tdesc.lpValue = new IntPtr(index + 1);
            }
            // end inner support - TCB
