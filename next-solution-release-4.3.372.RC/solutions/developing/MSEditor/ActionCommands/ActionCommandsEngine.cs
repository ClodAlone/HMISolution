    ...
    public class ActionCommandsEngine : IDisposable
    {
        ...
        void EvaluateCommandExecution(MonitoredItemViewModel m, OPCUAEntityReference schedulerEntityReference)
        {
            ...
            if (bool.TryParse(enableItem.Value, out enabledState) && enabledState)
            {
                ...
                {
                    byte outval;
                    if (byte.TryParse(m.Value, out outval))
                    {
                        bool actionState = (outval == (Byte)ActionCommandsEventType.CommandsOn || 
                            outval == (Byte)ActionCommandsEventType.CommandsOnEx);
                        bool exception = (outval == (Byte)ActionCommandsEventType.CommandsOffEx || 
                            outval == (Byte)ActionCommandsEventType.CommandsOnEx);
#if DEBUG
                        System.Diagnostics.Debug.WriteLine("command executer action:{0} exc:{1} outval:{2} Value:{3}",
                            actionState, exception, outval, m.Value);
#endif
                        ...
                        if (!actionPreviousStates.ContainsKey(nodeId) || actionPreviousStates[nodeId] != actionState)
                        {
                            ...
                            if (actionState && (commandsExecuter[nodeId].ExecOnAtStartup || m.Quality == goodQuality))
                            {
                                ...
                                commandsExecuter[nodeId].Execute(exception ? ActionCommandsEventType.CommandsOnEx : ActionCommandsEventType.CommandsOn);
                            ...
                            {
                                actionPreviousStates[nodeId] = actionState;
                                commandsExecuter[nodeId].Execute(exception ? ActionCommandsEventType.CommandsOffEx : ActionCommandsEventType.CommandsOff);
