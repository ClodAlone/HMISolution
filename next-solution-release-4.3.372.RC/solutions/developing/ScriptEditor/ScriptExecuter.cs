...
namespace ScriptManager
{
    class ScriptExecuter : IDisposable
    {
        ...
        public ScriptExecuter(ScriptDocument doc, ScriptManagerComponent c)
        {
            ...
            {
                ev.list = ScriptComponent.UFUAEditor.GetFlatListTags(scriptDocument, inExecution: true);
            };
            scriptDocument.GetPrototypeList += (o, ev) =>
            {
                ev.mapDefinitions = ScriptComponent.UFUAEditor.GetListPrototypesDesc(scriptDocument);
                ev.mapPrototypes = ScriptComponent.UFUAEditor.GetFlatListPrototypeInstances(scriptDocument, inExecution: true);
            };
            Document.GetChildsTagList += (o, ev) =>
            {
                ev.map = ScriptComponent.UFUAEditor.GetFlatListChildTags(Document);
            };
            Document.GetChildsPrototypeList += (o, ev) =>
            {
                ev.mapChildsDefinitions = ScriptComponent.UFUAEditor.GetListChildsPrototypesDesc(Document);
                ev.mapChildsPrototypes = ScriptComponent.UFUAEditor.GetFlatListChildsPrototypeInstances(Document);
            };
            scriptDocument.GetTagEntityReference += (o, ev) =>
            {
                var erString = ScriptComponent.UFUAEditor.GetTagEntityReference(scriptDocument, ev.Name, ev.Instance, inExecution: true, Project: ev.ChildProject);
