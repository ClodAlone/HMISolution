...
#if !WINDOWS_UWP
#if !NET_STANDARD
using OPCUAViewModel.UserControls;
...
#region Declarations
        ...
        public OPCUAEntityReference(OPCUAEntityReference template)
        {
            ...
            IsSystemVariable = template.IsSystemVariable;
...
*/
        ...
        public bool Edit(
            ...
            bool noDataSinks = false, 
            bool noLocalServer = false, 
            bool useChildSelector = true)
        {
            ...
            if (Editor != null)
            {
                var tabModel = new ServerTagSelectionModel(this, Editor);
                result = null;

                if (wnd == null)
                {
                    var tabControl = new DXTabControl();
                    var dialog = new ServerTagSelectionTab(tabModel);
                ...
                if (tabbers != null)
                {
                    ...
                    foreach (var t in tabbers.Items)
                    {
                        ...
                        else if (tt.Tag == localServerTag)
                        {
                            ...
                            {
                                IDocument doc = null;
                                if(appName != null && tabModel?.DocumentProjectsList != null)
                                    tabModel.DocumentProjectsList.TryGetValue(appName, out doc);
                                ts.Add(tt, Editor.GetAplicationName(doc ?? Document));
                            }
                        }
                        ...
                        if (tt.Content is ServerTagSelectionTab)
                        {
                            var tTab = tt.Content as ServerTagSelectionTab;

                            if (tTab.DataContext is ServerTagSelectionModel)
                            {
                                var sModel = tTab.DataContext as ServerTagSelectionModel;
                                sModel.IsCmbBoxVisible = sModel.DocumentProjectsList.Count > 1 && useChildSelector;

                                if (!(sModel.DocumentProjectsList.Count > 1 && useChildSelector))
                                {
                                    tTab.ChangeTagViewByAppName(sModel, sModel.AppNameParent);
                                }
                            }
                        }
                    ...
                    else
                    {
                        ...
                        if (tt != null)
                        {
                            ...
                            {
                                var tab = tt.Content as ServerTagSelectionTab;
                                if (tab != null)
                                {
                                    IDocument doc = null;
                                    if (appName != null && tabModel?.DocumentProjectsList != null)
                                        tabModel.DocumentProjectsList.TryGetValue(appName, out doc);
                                    tab.BringIntoView(doc ?? document, this);
                                }
                            }
                        }
                    }
                ...
                */
                ...
                if (bRet)
                {
                    var tabCntrl = wnd.DialogContent as DXTabControl;
                    var updateValueAction = new Action(() =>
                    {
                        if((tabCntrl.SelectedItem as DXTabItem).Content is ISelectEntityReference)
                        {
                            ScheduleValueUpdate((tabCntrl.SelectedItem as DXTabItem).Content as ISelectEntityReference);
                        }
                        else if((tabCntrl.SelectedItem as DXTabItem).Content is ServerTagSelectionTab)
                        {
                            var serverTag = (tabCntrl.SelectedItem as DXTabItem).Content as ServerTagSelectionTab;
                            var entityRef = serverTag.contentTab.Content as ISelectEntityReference;
                            ScheduleValueUpdate(entityRef);
                        }
                    });
...
#region Data Sink Interfaces 
        ...
        static public void StartDataSinkInterfaces(IDocument parent)
        {
            lock (mapDataSinkInterfaces)
            {
                mapDataSinkInterfaces.Values.ToList().ForEach(data => data.Start(parent));
            }
        }
        ...
        public static List<String> GetDataSinkInterfaces()
        {
            lock (mapDataSinkInterfaces)
            {
        ...
        public static DataSinkInterface GetDataSinkInterface(String name)
        {
            lock (mapDataSinkInterfaces)
            {
                if (mapDataSinkInterfaces.ContainsKey(name))
        ...
        public bool Resolve(String sessionName, MonitoredItemViewModel temporary = null, IDocument parent = null)
        {
            ...
            if (!bFirstTime && temporary == null && (bForceResolve || MonitoredItemViewModel == null))
            {
                NormalizeHost();
                var monitoredItem = GetRealTimeMonitoredItem(sessionName);
                if (bForceResolve && monitoredItem == null && MonitoredItemViewModel != null)
                    temporary = MonitoredItemViewModel;
                else
                {
                    MonitoredItemViewModel = monitoredItem;
                    if (MonitoredItemViewModel != null)
                        NodeIdViewModel = GetRealTimeNodeIdItem(sessionName);
                }
            }

            if (temporary != null && (bForceResolve || MonitoredItemViewModel == null))
            {
...
//                        }
        ...
        /// <summary>
        /// Set the state of the keep alive flag. When 'true' the object will not be destroyed ar runtime. 
        /// </summary>
        public void SetKeepAlive(bool bNewState)
        {
            if (bKeepAlive == bNewState)
                return;
            bKeepAlive = bNewState;

            var counter = Interlocked.Read(ref inUseCounter);
            if (!bKeepAlive && counter == 0)
            {
                bFirstTime = false;
                bForceResolve = true;
                bIsTemporary = false;
            ...
            lock (lockObject)
            {
                if (embeddedRedundantList != null)
                    embeddedRedundantList.ForEach(el => el.SetKeepAlive(bNewState));
            }
        }
        ...
        void ProcessInUse()
        {
            ...
            if (Interlocked.Read(ref inUseCounter) <= 0 && !bKeepAlive)
            {
                if (lastTimeUsed.AddSeconds(10) < DateTime.UtcNow)
                    OnDispose();
                else
...
#region Properties
        ...
        public bool InBlackList
        {
            ...
            set
            {
                if (value == bInBlackList)
                    return;
                bInBlackList = value;
                OnPropertyChanged("InBlackList");
            }
        }
...
#region Override Methods
        ...
        {
            return !string.IsNullOrEmpty(HumanReadableNoProject) 
                ? HumanReadableNoProject 
                : base.ToString();
        }

        public string ToString(string format, IFormatProvider formatProvider)
        {
            return !string.IsNullOrEmpty(ReadablePath) 
                ? ReadablePath 
                : base.ToString();
        }

        public string ToExpressionString(string defaultValue = null)
        {
            ...
            lock (mapDataSinkInterfaces)
            {
                if (mapDataSinkInterfaces.ContainsKey(AppName))
                {
                    if (!string.IsNullOrWhiteSpace(ReadablePath))
                        expressionValue = $"{AppName}.{ReadablePath}";
                    else if (!string.IsNullOrWhiteSpace(HumanReadableNoProject))
                        expressionValue = $"{AppName}.{HumanReadableNoProject}";
                    else
                        expressionValue = defaultValue ?? base.ToString();
            ...
            if (IsSystemVariable)
                expressionValue = $"{SysNames.dataSynkName}.{HumanReadableNoProject}".Replace('&', '\\');
            else if (defaultValue != null)
                expressionValue = defaultValue;
            else
                expressionValue = !string.IsNullOrWhiteSpace(ReadablePath)
                    ? ReadablePath
                    : base.ToString();
