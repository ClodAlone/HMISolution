...
//#endif
        ...
        */
        ...
        void ElaborateInUseRequests(Dictionary<String, List<IEntityReference>> tempInUse, bool bAddReferences)
        {
            ...
            var toUpdateMonitoringMode = new Dictionary<MonitoringMode, List<MonitoredItem>>();
                ...
                listToAddVariables.ForEach(nodeid =>
                {
                    lock (lockObject)
                    {
                        ...
                        else
                        {
                            ...
                            {
                                var monitoringMode = inUse ? MonitoringMode.Reporting : MonitoringMode.Disabled;
                                UpdateItemMonitoringMode(toUpdateMonitoringMode, nodeid, monitoringMode);
                            }
                            else
                            {
                                UpdateItemMonitoringMode(toUpdateMonitoringMode, nodeid, MonitoringMode.Reporting);
                            }

                            listChangedMonitoredItems.Add(MonitoredItemMapped[nodeid].monitoredItem);
                            MonitoredItemMapped[nodeid].SamplingInterval = inUse ? fastSamplingInterval : slowSamplingInterval;
                        }
                    }
                });
...
//#endif
            ...
            {
                foreach (var monitoringMode in toUpdateMonitoringMode.Keys)
                    SubscriptionViewModel.SetMonitoringMode(monitoringMode, toUpdateMonitoringMode[monitoringMode]);
...
//#endif
        ...
        private void UpdateItemMonitoringMode(Dictionary<MonitoringMode, List<MonitoredItem>> monitoredItemDict,
            NodeId nodeId,
            MonitoringMode monitoringMode)
        {
            var item = MonitoredItemMapped[nodeId];
            if (item.MonitoringMode == monitoringMode) return;
            if (!monitoredItemDict.ContainsKey(monitoringMode))
                monitoredItemDict.Add(monitoringMode, new List<MonitoredItem>());
            if (!monitoredItemDict[monitoringMode].Contains(item.monitoredItem))
                monitoredItemDict[monitoringMode].Add(item.monitoredItem);
