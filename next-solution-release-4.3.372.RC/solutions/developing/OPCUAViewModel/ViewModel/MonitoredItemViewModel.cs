...
namespace OPCUAViewModel
{
    public class MonitoredItemViewModel : TreeViewItemViewModel, IEntityReference
    {
        ...
        private readonly IOPCSessionService _opcSessionService = new OPCSessionService();
        ...
        public IList<ConditionStateViewModel> GetCurrentConditionStateList(bool bSortByTimeDescending = false)
        {
            ...
            return bSortByTimeDescending ? list.OrderByDescending(x => x.TimeOn).ToList() : list;
        }
        ...
        public bool WriteSynchValue(object v)
        {
            if(NodeIdModel != null && NodeIdModel.nodeId != null)
                return _opcSessionService.WriteSynchValue(((NodeId)NodeIdModel.nodeId), v, this);
            return false;
        }

        public bool WriteValue(object v, int indexArray = -1, int indexBit = -1)
        {
            return _opcSessionService.WriteValue(v, this, lockObject, indexArray, indexBit);
        }
        ...
        /// <summary>
        /// Allow to apply a filter based to an access mask and an access level
        /// </summary>
        /// <param name="accessMask">
        /// The access mask to use as BitwiseAnd matching with the condition UserReadAccessMask property
        /// </param>
        /// <param name="accessLevel">
        /// The access level to use as LessThanOrEqual matching with the condition UserAccessLevel property
        /// </param>
        /// <returns>
        /// Return true if the new filter condition was applied
        /// </returns>
        /// <exception cref="NotSupportedException"></exception>
        public bool ApplyConditionStateFilter(int accessMask, int accessLevel)
        {
            lock (lockObject)
            {
                UserAccessMask = accessMask;
...
#endif
                    _ConditionStateViewModel.UserAccessMask = UserAccessMask = accessMask;
                    _ConditionStateViewModel.UserAccessLevel = UserAccessLevel = accessLevel;
                    _ConditionStateViewModel.Filter = _ConditionStateViewModel.ConstructFilter();
        ...
        public EventFilter ConstructFilter()
        {
            ...
            if (UserAccessMask != defaultAccessMask)
            {
                // select the UserAccessMask property of the event.
                SimpleAttributeOperand operand1 = new SimpleAttributeOperand
                {
                    TypeDefinitionId = ObjectTypeIds.BaseEventType
                };
                operand1.BrowsePath.Add(UFUAServerInfo.BrowserNames.UserReadAccessMask);
                operand1.AttributeId = Attributes.Value;

                // specify the value to compare the UserAccessMask property with.
                LiteralOperand operand2 = new LiteralOperand
                {
                    Value = new Variant(UserAccessMask)
                };

                LiteralOperand zeroOperand = new LiteralOperand
                {
                    Value = new Variant(0)
                };

                // specify that the UserAccessMask property must Less or Equal the value specified.
                var filter1 = whereClause.Push(FilterOperator.IsNull, operand1);
                var filter2 = whereClause.Push(FilterOperator.BitwiseAnd, operand1, operand2);
                var filter3 = whereClause.Push(FilterOperator.GreaterThan, filter2, zeroOperand);
                element2 = whereClause.Push(FilterOperator.Or, filter1, filter3);
            ...
            {
                // select the UserAccessLevel property of the event.
                SimpleAttributeOperand operand1 = new SimpleAttributeOperand
                {
                    TypeDefinitionId = ObjectTypeIds.BaseEventType
                };
                operand1.BrowsePath.Add(UFUAServerInfo.BrowserNames.UserAccessLevel);
                operand1.AttributeId = Attributes.Value;

                // specify the value to compare the UserAccessLevel property with.
                LiteralOperand operand2 = new LiteralOperand
                {
                    Value = new Variant(UserAccessLevel)
                };

                // specify that the UserAccessLevel property must Less or Equal the value specified.
                var filter1 = whereClause.Push(FilterOperator.IsNull, operand1);
                var filter2 = whereClause.Push(FilterOperator.LessThanOrEqual, operand1, operand2);
                element2 = whereClause.Push(FilterOperator.Or, filter1, filter2);
        ...
        /// <summary>
        /// The user access mask to use to filter the events of interest.
        /// </summary>
        public int UserAccessMask = defaultAccessMask;
        /// <summary>
        /// The user access level to use to filter the events of interest.
        /// </summary>
            ...
            get
            {
                lock (lockObject)
                {
                    ...
                    if (_ConditionStateViewModel == null)
                    {
                        ...
                        var map = GetSubscriptionViewModelParent().AddAlarmEventMonitoredItem(listCreateMonItem, UserAccessMask, UserAccessLevel);
        ...
        public bool IsValueEqual(String value)
        {
            ...
            if (_dataValue != null && _dataValue.WrappedValue.TypeInfo.BuiltInType == BuiltInType.Boolean)
            {
                var v = ChangeTypeHelper.ChangeType(value, BuiltInType.Boolean);
                newvalue = Convert.ToString(v);
            }
            ...
            set
            {
                ...
                else
                {
                    ...
                    catch
                    {
                        ...
                        OnPropertyChanged(nameof(InvariantCultureValue));
                    }
                }
            }
        }
...
#region Validations
        ...
        {
            get => !string.IsNullOrWhiteSpace(_description) ? _description : NodeIdModel?.Description?.Text;
