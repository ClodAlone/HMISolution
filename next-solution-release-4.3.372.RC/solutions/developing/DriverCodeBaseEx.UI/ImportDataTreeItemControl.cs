...
namespace DriverCodeBaseEx.UI
{
    public class ImportDataTreeItemControl : TreeItemControl
    {
        #region DP
        #region ImportSortId
        public static readonly DependencyProperty ImportSortIdProperty = DependencyProperty.Register("ImportSortId", typeof(uint), typeof(ImportDataTreeItemControl), new UIPropertyMetadata((uint)0, new PropertyChangedCallback(OnImportSortIdChanged), new CoerceValueCallback(OnCoerceImportSortId)));

        private static object OnCoerceImportSortId(DependencyObject o, object value)
        {
            ImportDataTreeItemControl importDataTreeItemControl = o as ImportDataTreeItemControl;
            if (importDataTreeItemControl != null)
                return importDataTreeItemControl.OnCoerceImportSortId((uint)value);
            else
                return value;
        }

        private static void OnImportSortIdChanged(DependencyObject o, DependencyPropertyChangedEventArgs e)
        {
            ImportDataTreeItemControl importDataTreeItemControl = o as ImportDataTreeItemControl;
            if (importDataTreeItemControl != null)
                importDataTreeItemControl.OnImportSortIdChanged((uint)e.OldValue, (uint)e.NewValue);
        }

        protected virtual uint OnCoerceImportSortId(uint value)
        {
            // TODO: Keep the proposed value within the desired range.
            return value;
        }

        protected virtual void OnImportSortIdChanged(uint oldValue, uint newValue)
        {
            // TODO: Add your property changed side-effects. Descendants can override as well.
            OnPropertyChanged("ImportSortId");
        }
        ...
        public uint ImportSortId
        {
            ...
            get
            {
                return (uint)GetValue(ImportSortIdProperty);
            }
            set
            {
                SetValue(ImportSortIdProperty, value);
            }
        }
        #endregion
        #region TagName
        ...
        public virtual void UpdateAllColumns()
        {
            ...
            {
                ImportSortId = ((ImportData)Tag).ImportSortId;
