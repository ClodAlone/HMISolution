...
namespace DriverCodeBaseEx.UI.SettingsControls
{
    ...
    /// </summary>
    public partial class PrototypeMemberStringLength : UserControl
    {
        private bool alreadyLoaded = false;

        TreeListNode itemRoot;
        bool cellEditPendingError = false;
        private uint _MAX_STRING_LENGTH = 0;

        public PrototypeMemberStringLength(uint maxStringLength)
        {
            InitializeComponent();
            Loaded += (o, e) =>
            {
                ...
                StructStringLength.ProMemberView proto = this.DataContext as StructStringLength.ProMemberView;
        ...
        private BitmapImage GetBitmapImageSourceFromMemberViewType(StructStringLength.ProMemberView tag)
        {
        ...
        private TreeListNode AddRootTreeItem(StructStringLength.ProMemberView tag)
        {
            return AddTreeItem(null, tag, false);
        }

        private TreeListNode AddTreeItem(TreeListNode parent, StructStringLength.ProMemberView tag, bool includeChildren = true)
        {
            ...
            var t = tag as StructStringLength.ProMemberView;
        ...
        private void FillItems(ref TreeListNode parent, StructStringLength.ProMemberView tag)
        {
            ...
            if (parent == null)
            {
                if (tag != null)
                {
                    itemRoot = AddRootTreeItem(new StructStringLength.ProMemberView(tag.Name, UFUAModel.ModelType.ObjectType));
                    parent = itemRoot;
                }
            }
        ...
        {
            var ic = e.Node.Tag as StructStringLength.ProMemberView;
            if (ic != null)
            {
        ...
        private void treeListView_ValidateCell(object sender, DevExpress.Xpf.Grid.TreeList.TreeListCellValidationEventArgs e)
        {
            ...
            if (e.Column.FieldName == "StringLength") {
                ...
                {
                    e.SetError(string.Format(DriverCodeBaseEx.Properties.Resources.ErrorInvalidStringLength, _MAX_STRING_LENGTH));
                    e.IsValid = false;
                    cellEditPendingError = true;
                    return;
                }

                if (!StructStringLength.IsStringLengthInRange(newValue)) {
                    e.SetError(string.Format(DriverCodeBaseEx.Properties.Resources.ErrorInvalidStringLength, _MAX_STRING_LENGTH));
                    e.IsValid = false;
                    cellEditPendingError = true;
                    return;
