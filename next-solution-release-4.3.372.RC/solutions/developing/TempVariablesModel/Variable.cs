    ...
    public class Variable : XPObject, IDataErrorInfo, INotifyPropertyVisibilityChanged, XpoHelpers.UndoRedoIXPSimpleObjectHelper.IUniqueIdentifier
    {
        ...
        public string GetRelativePath(UInt16 ns, string separator, bool bRelative = false)
        {
            ...
            else
            {
                ...
                if (Folder != null)
                {
                    path = string.Format("{0}{3}{2}:{1}", Folder.GetRelativePath(ns, separator, bRelative), Name, ns, separator);
                }
                return path;
            }
        }

        public string GetRelative(string separator)
        {
            string name = string.Format("{0}", Name);
            if (Folder != null)
            {
                name = string.Format("{0}{2}{1}", Folder.GetRelativeName(separator), Name, separator);
            }
            return name;
        }
