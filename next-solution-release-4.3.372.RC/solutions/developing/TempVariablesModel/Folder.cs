...
namespace TempVariablesModel
{
    public class Folder : XPObject, IDataErrorInfo, XpoHelpers.UndoRedoIXPSimpleObjectHelper.IUniqueIdentifier
    {
        ...
        public string GetRelativePath(UInt16 ns, string separator, bool bRelative = false)
        {
            if (bRelative)
            {
            ...
            {
                string path = string.Format("{0}:{1}{3}{0}:{2}", ns, Properties.Resources.Tags, Name, separator);
                if (FolderAss != null)
                {
                    path = string.Format("{0}{3}{2}:{1}", FolderAss.GetRelativePath(ns, separator, bRelative), Name, ns, separator);
                }
                return path;
            }
        }

        public string GetRelativeName(string separator = "&")
        {
            string name = string.Format("{0}", Name);
            if (FolderAss != null)
            {
                name = string.Format("{0}{2}{1}", FolderAss.GetRelativeName(separator), Name, separator);
            }
            return name;
        }
