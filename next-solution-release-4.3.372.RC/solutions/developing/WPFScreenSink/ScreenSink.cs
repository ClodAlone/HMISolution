    ...
    public class ScreenSink : IDisposable
    {
        ...
        static String GetValueProvider(HitTestResult res, Point pt, bool bIsDown, ref bool bStopPropagation)
        {
            try
            {
                ...
                if (valueProv != null)
                {
                    if (autoElement.Owner is ComboBox && !(autoElement.Owner is ComboWeb))
                    {
                        var combo = autoElement.Owner as ComboBox;
                        ...
                        foreach(var item in combo.Items)
                        {
                            ...
                            else if (item is IFormattable)
                            {
                                var txt = ((IFormattable)item).ToString(null, CultureInfo.InvariantCulture);
                                if (ret.Length != 0)
                                    ret.Append(ListSerializationHelper.tagSelectionSeparator);
                                ret.Append(txt);
                            }
        ...
        */
        ...
        private void GetAllSessionsNames(ScreenDocument Document, String sessionname)
        {
            var map = new Dictionary<String, UIElement>();
            
            foreach (var name in Document.MapScreenEntities.Keys)
            {
                var entity = Document.MapScreenEntities[name];
                var listCommands = entity.CommandList as CommandManagerList;
                listCommands.ForEach(command => { if (!allSessions.Contains(command.SessionName)) allSessions.Add(command.SessionName); });
            }

            if (!allSessions.Contains(ClientSessionName))
                allSessions.Add(ClientSessionName);
            if (ClientSessionName != sessionname && !allSessions.Contains(sessionname))
                ...
                lock (lockStaticObject)
                {
                    ...
                    else
                    {
                        ...
                        if (Document != null)
                        {
                            ...
                            try
                            {
                                GetAllSessionsNames(Document, sessionName);
                                
                                foreach (string session in allSessions)
                                {
                                    if (String.IsNullOrEmpty(User))
                                        RealTimeConnectionManagerViewModel.SetUserIdentity(session, null, new StringCollection());
                                    else
                                        RealTimeConnectionManagerViewModel.SetUserIdentity(session, UserIdentity, new StringCollection());
                                }
                            }
                            catch (Exception ex)
                            {
...
//                                    };
        ...
        void Document_GetPrototypeList(object sender, GetPrototypeListEventArgs ev)
        {
            ...
            if (service == null)
                return;
            ev.mapDefinitions = service.GetFlatListPrototypes(Document, inExecution: true);
            ev.mapPrototypes = service.GetFlatListPrototypeInstances(Document, inExecution: true);
        }
        ...
        void Document_GetTagList(object sender, GetTagListEventArgs ev)
        {
            ...
            if (service == null)
                return;
            ev.list = service.GetFlatListTags(Document, inExecution: true);
        }
