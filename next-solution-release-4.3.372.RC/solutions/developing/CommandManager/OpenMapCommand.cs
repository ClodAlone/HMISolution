...
#if !WINDOWS_UWP && !NET_STANDARD
        [DataMember]
        private Point3D geoMapOpeningParameters = new Point3D(0, 0, 1);
        public Point3D GeoMapOpeningParameters
        {
            get 
            { 
                return geoMapOpeningParameters; 
            }
            set
            {
                if (geoMapOpeningParameters == value)
                    return;
                geoMapOpeningParameters = value;

                OnPropertyChanged("GeoMapOpeningParameters");
                OnPropertyChanged("CommandSummary");           
            }
        }
#endif
