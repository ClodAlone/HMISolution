...
namespace CommandManager
{
    ...
    public class ValueCommand : CommandManager
    {
        ...
        ExpressionValueConverter expValueConverter = null;
#if !NET_STANDARD
        ...
        [DataMember]
        private String idUnitConverter;
        public String IdUnitConverter
        {
            get
            {
                return idUnitConverter;
            }
            set
            {
                if (value == idUnitConverter)
                    return;
                idUnitConverter = value;
#if !WINDOWS_UWP && !NET_STANDARD
        ...
        [DataMember]
        private bool inheritFromControl = true;
        public bool InheritFromControl
        {
            get { return inheritFromControl; }
            set
            {
                if (inheritFromControl == value)
                    return;
                inheritFromControl = value;
#if !WINDOWS_UWP && !NET_STANDARD
                OnPropertyChanged("InheritFromControl");
                OnPropertyVisiblityChanged("InheritFromControl");
#endif
            }
        }

        [DataMember]
        private bool showTagDescription = false;
        public bool ShowTagDescription
        {
            get { return showTagDescription; }
            set
            {
                if (showTagDescription == value)
                    return;
                showTagDescription = value;
#if !WINDOWS_UWP && !NET_STANDARD
                OnPropertyChanged("ShowTagDescription");
#endif
            }
        }
        ...
        void AppendDecimalOFF(MonitoredItemViewModel model)
        {
        ...
        String GetWriteValue()
        {
            ...
            String padTagDescription = null;
            if (ShowTagDescription)
                padTagDescription = GetLocalizedText(model.ReferenceViewModel?.Description ?? model.Description);

            switch (Type)
            {
                case ValueCommandType.TransferValue:
                    AppendDecimalOFF(model);
                    return stringValue;
                ...
                case ValueCommandType.Set:
                    ...
                    if (Type == ValueCommandType.Set)
                    {
#if !NET_STANDARD
                        if (InheritFromControl && Control is IInheritPropertiesFromControl)
                            IdUnitConverter = (this.Control as IInheritPropertiesFromControl).GetUnitConverterFromControl(Control);

                        if (!String.IsNullOrEmpty(IdUnitConverter))
                        {
                            double? value = Convert.ToDouble(Value, CultureInfo.InvariantCulture);
                            var unitConverterEditorManager = Parent.GetService(typeof(UnitConverterManager.ComponentService.IUnitConverterEditorManager)) as UnitConverterManager.ComponentService.IUnitConverterEditorManager;
                            if (unitConverterEditorManager != null)
                            {
                                var converter = unitConverterEditorManager.GetActiveConverter(Parent);
                                var input = unitConverterEditorManager.GetInputExpression(Parent, IdUnitConverter, converter);
                                var output = unitConverterEditorManager.GetOutputExpression(Parent, IdUnitConverter, converter);
                                value = ConvertValue(input, output, value);
                                if (value != null)
                ...
                case ValueCommandType.NumericPad:
                    {
                        ...
                        if (InheritFromControl)
                        {
                            if (Control is IInheritPropertiesFromControl)
                            {
                                Decimals = (uint)(Control as IInheritPropertiesFromControl).GetPrecisionDigit();
                                UseEUnit = (Control as IInheritPropertiesFromControl).GetUseEngineeringUnit();
                                IdUnitConverter = (this.Control as IInheritPropertiesFromControl).GetUnitConverterFromControl(Control);
                            }
                        }
                        ...
                        else
                        {
                            ...
                            if (InheritFromControl)
                            {
                                if (Control is IInheritPropertiesFromControl)
                                {
                                    minValue = Convert.ToDouble((Control as IInheritPropertiesFromControl).GetMinValue());
                                    maxValue = Convert.ToDouble((Control as IInheritPropertiesFromControl).GetMaxValue());
                                }
                            }
                        ...
                        if (value.HasValue)
                        {
                            ...
                            string converter = String.Empty, input = String.Empty, output = String.Empty; 
                            var unitConverterEditorManager = Parent.GetService(typeof(UnitConverterManager.ComponentService.IUnitConverterEditorManager)) as UnitConverterManager.ComponentService.IUnitConverterEditorManager;
                            if (unitConverterEditorManager != null )
                            {
                                converter = unitConverterEditorManager.GetActiveConverter(Parent);
                                input = unitConverterEditorManager.GetInputExpression(Parent, IdUnitConverter, converter);
                                output = unitConverterEditorManager.GetOutputExpression(Parent, IdUnitConverter, converter);

                                minValue = ConvertValue(input, output, minValue);
                                maxValue = ConvertValue(input, output, maxValue);
                            ...
                            var ret = AsPassword 
                                ? Pads.Pads.ShowNumericPasswordPad(X, Y, IsRelative, padValue, owner, OpcuaEntityReference?.HumanReadable, minValue, maxValue, padTagDescription, ShowTagDescription) 
                                : Pads.Pads.ShowNumericPad(X, Y, IsRelative, padValue, owner, OpcuaEntityReference?.HumanReadable, minValue, maxValue, padTagDescription, ShowTagDescription);
                            ...
                            try
                            {
                                ...
                                if (OpcuaEntityReference != null)
                                    value = ConvertValue(input, output, value, true);
                            ...
                            return AsPassword 
                                ? Pads.Pads.ShowNumericPasswordPad(X, Y, IsRelative, stringValue, owner, OpcuaEntityReference?.HumanReadable, minValue, maxValue, padTagDescription, ShowTagDescription) 
                                : Pads.Pads.ShowNumericPad(X, Y, IsRelative, stringValue, owner, OpcuaEntityReference?.HumanReadable, minValue, maxValue, padTagDescription, ShowTagDescription);
                ...
                case ValueCommandType.AlphaNumericPad:
                    {
                        ...
                        if (InheritFromControl)
                        {
                            if (Control is IInheritPropertiesFromControl)
                            {
                                maxValue = Convert.ToInt32((Control as IInheritPropertiesFromControl).GetMaxValue());
                                if (maxValue == 0)
                                    maxValue = null;
                            }
                        }
                        ...
                        return AsPassword 
                            ? Pads.Pads.ShowPasswordPad(X, Y, IsRelative, stringValue, owner, OpcuaEntityReference?.HumanReadable, maxValue, padTagDescription, ShowTagDescription) 
                            : Pads.Pads.ShowAlphaNumericPad(X, Y, IsRelative, stringValue, owner, OpcuaEntityReference?.HumanReadable, maxValue, padTagDescription, ShowTagDescription);
#else
                        throw new Exception(Properties.Resources.PadCommandNotSupported);
#endif
                    }
                case ValueCommandType.AppendValueFromString:
                    {
                        AppendDecimalOFF(model);
                        string translatedString = null;
                        IStringEditorManager stringEditor = Parent.GetService(typeof(IStringEditorManager)) as IStringEditorManager;
                        if (stringEditor != null)
                        {
                            var dictCulture = stringEditor.GetListStringForCulture(Parent, stringEditor.GetActiveCulture(Parent));

                            translatedString = TranslationHelpers.TranslationHelper.TranslateComposedText(Value, dictCulture, "");
                        }
                        if (!String.IsNullOrEmpty(translatedString))
                        {
                            return $"{model.DataValue.Value}{translatedString}";
                        }
                        else
                        {
                            return $"{model.DataValue.Value}";
                        }
                    }
            }
        ...
        private string GetLocalizedText(string txt)
        {
            if (!string.IsNullOrWhiteSpace(txt) &&
                Parent.GetService(typeof(IStringEditorManager)) is IStringEditorManager stringEditorManager)
            {
                var activeCulture = stringEditorManager.GetActiveCulture(Parent);
                var mapValues = stringEditorManager.GetListStringForCulture(Parent, activeCulture);
                if (mapValues != null && mapValues.ContainsKey(txt))
                    return mapValues[txt];
            }

            return txt;
        }

        private double? ConvertValue(string input, string output, double? valToConvert, bool convertBack = false)
        {
            if (!String.IsNullOrEmpty(IdUnitConverter) && !String.IsNullOrEmpty(input) && !String.IsNullOrEmpty(output))
            {
                bool parseFormula = false;

                if (expValueConverter == null)
                {
                    expValueConverter = new ExpressionValueConverter()
                    {
                        Formula = input,
                        ReverseFormula = output
                    };
                    parseFormula = true;
                }
                
                if (expValueConverter.Formula != input)
                {
                    expValueConverter.Formula = input;
                    parseFormula = true;
                }
                if (expValueConverter.ReverseFormula != output)
                {
                    expValueConverter.ReverseFormula = output;
                    parseFormula = true;
                }
                    
                if (parseFormula)
                    expValueConverter.ParseFormula();

                if (!convertBack)
                    return Convert.ToDouble(expValueConverter.Convert(valToConvert, typeof(double), null, CultureInfo.CurrentCulture));
                else
                    return Convert.ToDouble(expValueConverter.ConvertBack(valToConvert, typeof(double), null, CultureInfo.CurrentCulture));
            }
            return valToConvert;
        }
        ...
        private bool MustForceDecimals(MonitoredItemViewModel monitor)
        {
            ...
            if (monitor.DataType != null)
                builtInType = MonitoredItemViewModel.GetBuiltInType(monitor.DataType);
            else if (monitor.ReferenceViewModel != null)
                builtInType = MonitoredItemViewModel.GetBuiltInType(monitor.ReferenceViewModel.DataType);
            else
                return false;
        ...
        public override List<OPCUAEntityReference> ListTags
        {
            get
            {
                ...
                if (TagMaxValue != null &&
                    ...
                    ret.Add(TagMaxValue);
                if (expressionEntity != null)
                    ret.AddRange(expressionEntity.ListTags);
                return ret;
            }
        }
        ...
        public override bool this[string propertyName]
        {
            get
            {
                bool inheritVisibility = (Control is IInheritPropertiesFromControl) && InheritFromControl;
                ...
                else if (propertyName == "ReverseExpression" || propertyName == "AsPassword" || propertyName == "ShowTagDescription")
                {
                    return Type == ValueCommandType.AlphaNumericPad ||
                           Type == ValueCommandType.NumericPad;
                }
                else if (propertyName == "MaxValue" || propertyName == "TagMaxValue")
                {
                    return (Type == ValueCommandType.AlphaNumericPad || Type == ValueCommandType.NumericPad) && !inheritVisibility;
                }
                else if (propertyName == "MinValue" || propertyName == "TagMinValue" || propertyName == "Decimals" || propertyName == "UseEUnit")
                {
                    return Type == ValueCommandType.NumericPad && !inheritVisibility;
                }
                else if (propertyName == "IdUnitConverter")
                {
                    return (Type == ValueCommandType.NumericPad || Type == ValueCommandType.Set) && !inheritVisibility;
                }
                else if (propertyName == "InheritFromControl")
                {
                    return (Type == ValueCommandType.NumericPad || Type == ValueCommandType.AlphaNumericPad || Type == ValueCommandType.Set) && 
                        Control is IInheritPropertiesFromControl;
                }
        ...
        public override void Terminate()
        {
            ...
            if (expValueConverter != null)
            {
                expValueConverter.Dispose();
                expValueConverter = null;
            }
