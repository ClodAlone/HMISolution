...
namespace UFUserEditor.Document
{
        ...
        IDocument, IXpoDocument
    {
        ...
        {
            var list = (from tag in new XPQuery<UFUserModel.UFUserSettings>(uow, true)/*.AsParallel() */select tag).ToList();
            if (list.Count == 0)
        ...
        public IList<UFUserModel.UFRole> GetRoles(UnitOfWork u = null)
        {
            ...
            return (from p in new XPQuery<UFUserModel.UFRole>(currentuow, true)// .AsParallel()
                    orderby p.Name
                    select p).ToList();
        }
        ...
        {
            return (from role in new XPQuery<UFUserModel.UFRole>(uow, true)
                    where role.Name == roleName
                    select role.AccessLevel).FirstOrDefault();
        }

        internal int GetRoleAccessMask(string roleName)
        {
            return (from role in new XPQuery<UFUserModel.UFRole>(uow, true)
                    where role.Name == roleName
                    select role.AccessMask.Value).FirstOrDefault();
        }

        internal int GetRoleAutoLogoutSeconds(string roleName)
        {
            using (var task = cachedUnitOfWorks.BeginUnitOfWork())
            {
                var rolesList = (from role in new XPQuery<UFUserModel.UFRole>(task.UnitOfWork, true).AsParallel()
                                where String.Compare(role.Name, roleName, true) == 0 || String.Compare(role.Name, roleName.Trim(), true) == 0
                                select role).ToList();

                if (rolesList.Count > 0)
                {
                    if (rolesList[0].AutoLogoutSeconds > 0)
                        return rolesList[0].AutoLogoutSeconds;
                }
            }
                ...
                using (var task = cachedUnitOfWorks.BeginUnitOfWork())
                {
                    ...
                    if (list.Count > 0)
                    {
                        ...
                        if (bExpired)
                        {
                            ...
                            var captions = GeneralDialogContent.GetDefaultButtonCaptions(stringlist);
        ...
        internal Dictionary<String, String> GetExternalAuthenticationSettings()
        {
            using (var task = cachedUnitOfWorks.BeginUnitOfWork())
            {
                UFUserModel.UFUserSettings configuration;

                var externalIdPSettingsList = new Dictionary<String, String>();
                var userSettings = (from item in new XPQuery<UFUserModel.UFUserSettings>(task.UnitOfWork, true) select item).ToList();

                if (userSettings.Count == 0)
                    configuration = new UFUserModel.UFUserSettings(task.UnitOfWork);
                else
                    configuration = userSettings[0];

                externalIdPSettingsList.Add("AccessTokenValidIssuer", configuration.AccessTokenValidIssuer);
                externalIdPSettingsList.Add("AppKeysBaseEndpoint", configuration.AppKeysBaseEndpoint);
                externalIdPSettingsList.Add("Authority", configuration.Authority);
                externalIdPSettingsList.Add("AuthorizeEndpoint", configuration.AuthorizeEndpoint);
                externalIdPSettingsList.Add("ClientId", configuration.ClientId);
                externalIdPSettingsList.Add("EndInfoEndpoint", configuration.EndInfoEndpoint);
                externalIdPSettingsList.Add("ExternalIdentityProviderActive", configuration.ExternalIdentityProviderActive.ToString());
                externalIdPSettingsList.Add("LogoutEndpoint", configuration.LogoutEndpoint);
                externalIdPSettingsList.Add("Nonce", configuration.Nonce);
                externalIdPSettingsList.Add("RedirectUriNativeClient", configuration.RedirectUriNativeClient);
                externalIdPSettingsList.Add("ResponseType", configuration.ResponseType);
                externalIdPSettingsList.Add("ReturnUrlAfterLogin", configuration.ReturnUrlAfterLogin);
                externalIdPSettingsList.Add("ReturnUrlAfterLogout", configuration.ReturnUrlAfterLogout);
                externalIdPSettingsList.Add("Scope", configuration.Scope);
                externalIdPSettingsList.Add("TenantId", configuration.TenantId);
                externalIdPSettingsList.Add("TokenEndpoint", configuration.TokenEndpoint);
                externalIdPSettingsList.Add("UserInfoEndpoint", configuration.UserInfoEndpoint);
                externalIdPSettingsList.Add("ValidIssuer", configuration.ValidIssuer);
                externalIdPSettingsList.Add("ClientSecret", configuration.ClientSecret);

                return externalIdPSettingsList;
            }
        }

        internal string GetExternalAuthenticationDefaultRole()
        {
            using (var task = cachedUnitOfWorks.BeginUnitOfWork())
            {
                var defaultRole = 
                    (from item in new XPQuery<UFUserModel.UFUserSettings>(task.UnitOfWork, true)
                    select item.DefaultExternalAuthenticationRole).FirstOrDefault();
                
                return defaultRole;
            }
        }

        internal string GetExternalAuthenticationUserRole(string externalRole)
        {
            var currentlyExistingRoles = GetRoleNames();
            var defaultExternalAuthenticationRole = GetExternalAuthenticationDefaultRole();

            if (currentlyExistingRoles != null && currentlyExistingRoles.Count > 0)
            {
                if (string.IsNullOrEmpty(externalRole) == false)
                {
                    if (currentlyExistingRoles.Contains(externalRole))
                    {
                        return externalRole;
                    }
                    else
                    {
                        if (string.IsNullOrEmpty(defaultExternalAuthenticationRole) == false && currentlyExistingRoles.Contains(defaultExternalAuthenticationRole))
                        {
                            return defaultExternalAuthenticationRole;
                        }
                    }
                }
                else
        ...
        internal String GetRoleCultureName(String roleName)
        {
            return (from role in new XPQuery<UFUserModel.UFRole>(uow, true)
                    where role.Name == roleName
                    select role.CultureName).FirstOrDefault();
        }
        ...
        internal String GetRoleConverterName(String roleName)
        {
            using (var task = cachedUnitOfWorks.BeginUnitOfWork())
            {
                var rolesList = (from p in new XPQuery<UFUserModel.UFRole>(task.UnitOfWork, true).AsParallel()
                                where String.Compare(p.Name, roleName, true) == 0 || String.Compare(p.Name, roleName.Trim(), true) == 0
                                select p).ToList();

                if (rolesList.Count > 0)
                    return rolesList[0].ConverterName;
            }
        ...
        public void MoveUsersToSharedRepository(UFUserDocument document, string targetPath, bool copySettings, bool copyUsersAndGroup)
        {
            MergeFile(document, targetPath, copySettings, copyUsersAndGroup);
        }
        ...
        internal void TerminateAndLogoff()
        {
            try
            {
                EditorManagerComponent.AuthenticationCredentialsProvider.Logout(parent.Title, false);
            }
            catch (Exception ex)
            {
        ...
        internal void AddSharedRepositoryTriggers()
        {
            ...
            if (membershipConnectionString != null)
            {
                ...
                if (serverName != null)
                {
                    //sqlQuery = String.Format(Properties.Settings.Default.sp_dropserver, serverName);
                    //sqlQueries.AddRange(sqlQuery.Split(splitString, StringSplitOptions.None));
                    sqlQuery = String.Format(Properties.Settings.Default.sp_addlinkedserver, serverName, userName != null ? "FALSE" : "TRUE", userName ?? "NULL", password ?? "NULL");
                    sqlQueries.AddRange(sqlQuery.Split(splitString, StringSplitOptions.None));
                    //sqlQuery = String.Format(Properties.Settings.Default.sp_droplinkedsrvlogin, serverName);
                    //sqlQueries.AddRange(sqlQuery.Split(splitString, StringSplitOptions.None));
                    //sqlQuery = String.Format(Properties.Settings.Default.sp_addlinkedsrvlogin, serverName, userName != null ? "FALSE" : "TRUE", userName ?? "NULL", password ?? "NULL");
                    //sqlQueries.AddRange(sqlQuery.Split(splitString, StringSplitOptions.None));

                    sqlQuery = String.Format(Properties.Settings.Default.tr_removemembership_drop, serverName);
                    sqlQueries.AddRange(sqlQuery.Split(splitString, StringSplitOptions.None));
                    sqlQuery = String.Format(Properties.Settings.Default.tr_removemembership, serverName, sharedApplicationName.ToLower());
                    sqlQueries.AddRange(sqlQuery.Split(splitString, StringSplitOptions.None));
                }
                else
                    sqlQueries.Clear();
            ...
            if (sqlQueries.Count > 0)
            {
                ...
                if (!String.IsNullOrEmpty(connectionStringBackup))
                    ExecuteNonQuery(connectionStringBackup, sqlQueries.ToArray());
            }
            else
