...
namespace ExpressionManager
{
    public class ExpressionEntity : IDisposable
    {
        ...
        public ReadOnlyList<OPCUAEntityReference> ListTags
        {
            get
            {
                return new ReadOnlyList<OPCUAEntityReference>(inuseVariableValueReferencesExpression);
            }
        }
        ...
        public bool ExpressionTagsInIdle { private get; set; }
        ...
        internal void Initialize()
        {
            ...
            var type = ExpressionValueConverter.GetFormulaType(converter.Formula, 
                out var arrayIndex, 
                out var bitNumber);
            ...
            if (documentParent?.GetService(typeof(IUFUAEditorManager)) is IUFUAEditorManager ufuaEditor)
            {
                ...
                listVariables.ForEach(variable =>
                {
                    ...
                    if (reference == null)
                    {
                        reference = _localTagParser.Parse(variable);
                        isLocalTag = reference != null;
                    }
                    ...
                    if (reference != null)
                    {
                        ...
                        reference.SetKeepAlive(ExpressionTagsInIdle);
                        reference.SetInUse(referenceItemViewModel, !ExpressionTagsInIdle);
                    }
                });
            }
            ...
            lock (lockObject)
            {
                ...
                try
                {
                    ...
                    if (bConvertBack)
                    {
                        ...
                        if (newValue != null)
                        {
                            if (converter.Type == ExpressionType.Expression)
                            {
                                if (sourceType == null && referenceItemViewModel.DataValue.WrappedValue.TypeInfo.ValueRank == ValueRanks.Scalar)
                                {
                                    sourceType = TypeInfo.GetSystemType(referenceItemViewModel.DataValue.WrappedValue.TypeInfo.BuiltInType,
                                        referenceItemViewModel.DataValue.WrappedValue.TypeInfo.ValueRank);
                                }
                                ...
                                if (sourceType != null)
                                {
                                    try
                                    {
                                        newValue = System.Convert.ChangeType(newValue, sourceType);
                                    }
                                    catch (System.FormatException ex)
                                    {
                                        BuiltInType type = referenceItemViewModel.DataValue.WrappedValue.TypeInfo.BuiltInType;
                                        bool isInteger = type >= BuiltInType.SByte && type <= BuiltInType.UInt64;
                                        if (!isInteger)
                                            throw;
                                        newValue = Math.Round(Convert.ToDouble(newValue));
                                    }
                                }
                            }
        ...
        void UpdateMapDynamics(OPCUAEntityReference reference)
        {
            ...
            {
                mapDynamics[reference.ToExpressionString(reference.RelativePath)] = value;
            }
            mapDynamics[reference.ToExpressionString(reference.HumanReadableNoProject)] = value;
        }
