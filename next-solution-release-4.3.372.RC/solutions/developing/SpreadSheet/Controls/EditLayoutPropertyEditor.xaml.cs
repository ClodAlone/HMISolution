...
namespace SpreadSheet.Controls
{
    ...
    public partial class EditLayoutPropertyEditor : UserControl
    {
        ...
        public EditLayoutPropertyEditor()
        {
            ...
            PropertyChangeNotifier propertyChangedNotifier = null; 

            Loaded += (sender, e) =>
             {
                 SpreadSheet obj = GetContextObject() as SpreadSheet;
                 if (obj != null)
                 {
                     IsEnabled = !string.IsNullOrEmpty(obj.CheckFilePath(obj.DocumentPath));

                     propertyChangedNotifier = new PropertyChangeNotifier(obj as SpreadSheet,SpreadSheet.DocumentPathProperty.Name);

                     propertyChangedNotifier.ValueChanged += (s, ea) =>
                     {
            ...
            Unloaded += (sender, e) =>
            {
                if(propertyChangedNotifier != null) 
                    propertyChangedNotifier.Dispose();
        ...
        private void EditLayout_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (Workspace != null)
            {
                ...
                if (contextObject is SpreadSheet)
                {
                    ...
                    { 
                        // Set document for cloned control
                        ScreenSettings.ScreenDocument.SetScreenDocument(lcontrol, Document);
                        lcontrol.DocumentPath = obj.DocumentPath;
                        ...
                        {
                            // save document.
                            lcontrol.spreadsheetControl1.SaveDocument();
                            // Set excel document source to trigger refresh.
                            obj.spreadsheetControl1.DocumentSource = UriToAbsoluteUriConverter.Convert(lcontrol.DocumentPath, Document, SpecialFolders.Documents);
                        }
                    }
                }
            }
