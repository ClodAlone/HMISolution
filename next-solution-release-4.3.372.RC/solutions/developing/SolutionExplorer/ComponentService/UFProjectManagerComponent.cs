        ...
        public IReportManager ReportManager
        {
            get
            {
                if (reportManager == null)
                    reportManager = GetService(typeof(IReportManager)) as IReportManager;
        ...
        public void Execute(Uri uri, IDocument parent, ExecutionMode mode, Object Context)
        {
            ...
            {   
                ...
                foreach (var docManager in listsingle)
                {
                    ...
                    try
                    {
                        var stringManager = docManager as IStringEditorManager;
                        if(stringManager != null && doc.ForceStartupCultureName)
                            docManager.Execute(new Uri(doc.GetProjectCulture(), UriKind.RelativeOrAbsolute), doc, ExecutionMode.Synchro, null);
                        else
            ...
            if (mode != ExecutionMode.Shared)
            {
                ...
                if (bmode == false)
                {
                    ...
                    if (timer == null)
                    {
                        ...
                        if (bFirstCheck && !MSZ.MSZView.IsSuspended() && !MSZ.MSZView.IsForciblySuspended())
                            {
                                ...
                                {
                                    logLicense.Info(String.Format(Properties.Resources.LicenseSiteCode, siteCode));
                        ...
                        logLicense.Warn(String.Format(Properties.Resources.NoRuntimeLicenseTerminating, counter * 10, dongleInfo));
                        doc.AddLogEntity(null, DateTime.UtcNow, 
                            String.Format(Properties.Resources.NoRuntimeLicenseTerminating, counter * 10, dongleInfo), 
                            System.Diagnostics.EventLogEntryType.Warning);
...
#region IDisposable Members
                                     ...
                                     System.Security.AccessControl.AuthorizationRuleCollection acl)
        {
            ...
            for (int i = 0; i < acl.Count; i++)
            {
                ...
                bool user = _currentUser.User.Equals(currentRule.IdentityReference);
                bool isinrole = false;
                try
                {
                    isinrole = _currentPrincipal.IsInRole(currentRule.IdentityReference.ToString());
                }
                catch 
                {
                    //this could happen if a deleted user is still present in folder access roles
                }
...
#region IUFProjectManager
        ...
        public List<CrossReferenceResultModel> GetCRObjects(CrossReferenceModel model)
        {
            ...
            if(getScreen)
            {
                parent.AutoloadScreenList.ForEach(screen =>
                {
                    ...
                    var name = System.IO.Path.GetFileNameWithoutExtension(pathString);
                    //lock (result)
                ...
                parent.ListStartupLogics.ForEach(logic =>
                {
                    var pathString = logic.Uri.GetPathString();
                    var name = System.IO.Path.GetFileNameWithoutExtension(pathString);
                    //lock (result)
                    result.Add(new UFInterfaces.Editors.CrossReferenceResultModel()
                    {
                        ...
                        AppName = parent.Title,
                        CReferenceType = CrossReferenceType.Resources,
                        Description = string.Format("{0}", Properties.Resources.StartupLogicList),
                        Settings = string.Format("{0}", parent.FilePath),
                        ContainerDoc = TypeScheme,
                        IconType = docType
                    });
                });
                parent.ListStartupScripts.ForEach(script =>
                {
                    var pathString = script.Uri.GetPathString();
                    var name = System.IO.Path.GetFileNameWithoutExtension(pathString);
                    //lock (result)
                    result.Add(new UFInterfaces.Editors.CrossReferenceResultModel()
                    {
                        ...
                        AppName = parent.Title,
                        CReferenceType = CrossReferenceType.Resources,
                        Description = string.Format("{0}", Properties.Resources.StartupScriptList),
                        Settings = string.Format("{0}", parent.FilePath),
                        ContainerDoc = TypeScheme,
                        IconType = docType
                    });
        ...
        public Dictionary<string, IDocument> GetAllProjectsDocuments(IDocument parent)
        {
            var doc = GetOrCreateDocument(parent);
            if (doc == null)
                return null;
            return doc.GetAllProjectsDocuments();
        }

        public bool ExportToSvg(Uri uri, IDocument parent)
        {
            UFProjectDocument doc = null;
            if (!mapActiveDocuments.TryGetValue(uri.GetPathString(), out doc))
            {
            ...
            var control = new Controls.ProgressDialog(doc, projectManagerComponent, bSilent: true);
