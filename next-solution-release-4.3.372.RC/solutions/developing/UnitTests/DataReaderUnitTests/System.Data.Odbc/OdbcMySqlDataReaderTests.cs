...
{
    [TestClass]
    public class OdbcMySqlDataReaderTests
    {
        [TestMethod]
        [Ignore]
        public void TestMySqlOdbcSchemaInfo()
        {
            using (var config = new MySqlConfigDataSource("5.3", MySqlDriverType.Unicode) { UserName = "root", Password = "1-1-91" })
            {
                if (!config.Add())
