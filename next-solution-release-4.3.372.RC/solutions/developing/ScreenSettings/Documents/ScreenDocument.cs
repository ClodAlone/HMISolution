...
{
    [DataContract(Name = "ScreenDocument", Namespace = UFInterfaces.Constants.Namespaces.UriProgea)]
...
#endif
    {
        ...
        private void PostInitialize(StreamingContext context)
        {
            ...
            else
            {
                ...
                foreach (var usedVarList in mapScriptVariableUsed.Values)
                {
                    foreach(var uVar in usedVarList)
                    {
                        var i = uVar.IndexOf(idSeparator);
                        list.Add((i >= 0 ? uVar.Substring(0, i) : uVar));
                    }
                }
                if (list.Count > 0)
                {
        ...
        public static readonly DependencyProperty DisableShowMenuOnLefProperty = DependencyProperty.RegisterAttached("DisableShowMenuOnLef", typeof(bool), typeof(ScreenDocument),
#if !WINDOWS_UWP
        ...
        [DesignerSerializationVisibility(DesignerSerializationVisibility.Hidden)]
#endif
        public static void SetDisableShowMenuOnLef(UIElement element, bool value)
        {
            element.SetValue(DisableShowMenuOnLefProperty, value);
        }

        public static bool GetDisableShowMenuOnLef(UIElement element)
        {
            return (bool)element.GetValue(DisableShowMenuOnLefProperty);
        }
#endif
        #endregion
...
#if !WINDOWS_UWP && !NET_STANDARD
        public static void RemoveFile(string fullPath, IDocument parent, FileSystemProviderBase fileSystemProvider = null)
        {
            ...
            {
                TryDeleteIfExists(settingsFileName, parent);
                TryDeleteIfExists(styleFileName, parent);
                TryDeleteIfExists(GetSettingsBinaryFileName(fullPath), parent);
                TryDeleteIfExists(fullPath, parent);
                TryDeleteIfExists(file, parent);
                ...
                if (webExtensions.Length > 0)
                {
                    ...
                    foreach (var extension in webExtensions)
                    {
                        ...
                        TryDeleteIfExists(webFile, parent);
                    };
                }
            }
        }
        ...
        /// <summary>
        /// Tries to delete a file if it exists, handling the possibile exceptions
        /// </summary>
        /// <param name="filePath">The path of the file to delete.</param>
        static void TryDeleteIfExists(string filePath, IDocument parent)
        {
            ...
            catch (Exception ex)
            {
                ...
                if (uiMsgBox != null)
                    uiMsgBox.ShowError(String.Format(Properties.Resources.ErrorDeletingDocument, filePath, ex.Message));
#else
                var syslog = LogManager.GetLogger(System.Reflection.Assembly.GetEntryAssembly(), parent.Title);
                syslog.ErrorFormat(Properties.Resources.ErrorDeletingDocument, filePath, ex.Message);
#endif
            }
        }
        ...
        public static void RemoveAllCompiledScreenFiles(string fullPath)
        {
            ScreenCompilerManager.ScreenCompilerManager.RemoveCompiledFiles(fullPath);
        }
        ...
        static void ScreenDocumentFileCopy(string sourceFileName, string destFileName, IDocument parent, FileSystemProviderBase targetVFS, bool bCopy)
        {
            ...
            if (!bCopy)
            {
                RemoveFile(sourceFileName, parent);
            }
        }
        ...
        public static void CopyFile(String fullPath, String newPath, bool bCopy, IDocument parent, bool bUploading)
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    if (!bCopy)
                        RemoveFile(sourcesettingsFileName, parent);
                    ...
                    if (!bCopy)
                        RemoveFile(sourcestyleFileName, parent);
                    ...
                    if (targetVFS == null)
                    {
                        ...
                        if (!bCopy)
                            RemoveFile(sourcesettingsBinaryFileName, parent);
                    }
                    ...
                    if (File.Exists(fullPath))
                    {
                        if (targetVFS != null)
                        {
                            ...
                            try
                            {
                                ...
                                ConvertToDB(fileTemp, parent, newPath);

                                var text = File.ReadAllText(fileTemp);
                                ...
                                var textSettings = File.ReadAllText(fileTempsettings);
                                var decodedSettings = System.Text.Encoding.UTF8.GetBytes(textSettings);
                                targetVFS.UploadFile(null, sourcesettingsFileName.Replace(parent.rootBase, parent.rootBaseDB), decodedSettings);
                            }
                            finally
                            {
                                try
                    ...
                    if (!bCopy)
                        RemoveFile(sourcelayoutFileName, parent);
        ...
        static void ConvertToDB(String file, IDocument parent, String newpath)
        {
            ...
            if (list.Count > 0)
            {
                using (var cursor = new WaitCursor())
                {
                    list.ForEach(entry =>
                    {
                        //some symbols must be loaded from file system, so it is not needed the source symbol provider 
                        var symbolPath = CryptString.DecryptString(entry.Value.SourceSymbolPath);
                        if (File.Exists(symbolPath))
                            return;
                        try
                        {
                            entry.Value.SourceSymbolProvider = CryptString.EncryptString(newpath);
                        }
                        catch (Exception ex)
                        {
        ...
        Dictionary<OPCUAEntityReference, IEntityReference> mapInIdleOPCItemToEntity;
                ...
                lock (lockObject)
                {
                    ...
                    if (mapResolvedVariables.ContainsKey(instanceName))
                    {
                        ...
                        if (!inuseVariableValueReferences.Contains(mapResolvedVariables[instanceName]))
                        {
                            ...
                            mapOPCItemToEntity.Add(mapResolvedVariables[instanceName], this);
            ...
            IDictionary<string, IList<string>> dictStructDefinition, ref List<VariableValues> ret)
        {
            ...
            var childTags = new VariableValues(childProject, true);
            ...
            if (dictVariable != null && dictVariable.Count > 0 && dictStructDefinition != null && dictStructDefinition.Count > 0)
            {
                foreach (var inst in dictVariable.Keys)
                {
                    if (dictVariable[inst] == null || !dictStructDefinition.ContainsKey(dictVariable[inst]))
                        continue;
                    var prototypesValues = childTags.AddInnerVariable(inst, true);
            ...
            if (listScriptVariableUsed != null)
            {
                ...
                if (mapLocalResolvedVariables.Count > 0)
                {
                    foreach (var var in mapLocalResolvedVariables.Keys)
                    {
                        ...
                        if (rightVal != null)
                            rightVal.OnResolveVariable(new ResolveVariableEventArgs(resName));
                    }
            ...
            varvalues.ResolveVariable += (o, e) =>
            {
                ...
                if (mapResolvedVariables.ContainsKey(instanceName))
                {
                    ...
                    if (!inuseVariableValueReferences.Contains(mapResolvedVariables[instanceName]))
                    {
                        ...
                        mapResolvedVariables[instanceName].SetKeepAlive(inIdle);
                        mapResolvedVariables[instanceName].SetInUse(reference, !inIdle);
                ...
                if (mapLocalResolvedVariables.Count > 0)
                {
                    foreach (var var in mapLocalResolvedVariables.Keys)
                    {
                        ...
                        else
                        {
                            ...
                            //var intreference = (index >= 0 ? var.Substring(0, index) : var);
                            //string resName = intreference;
                            //string resInstance = intreference;
                            //int nFound = intreference.IndexOf("-");
                            //if (nFound != -1)
                            //{
                            //    resInstance = intreference.Substring(0, nFound);
                            //    resName = intreference.Substring(++nFound);
                            //}
                            var name = var.Split('-');
                        ...
                        if (eap.mapPrototypes != null && eap.mapDefinitions != null)
                        {
                            foreach (var instance in eap.mapPrototypes.Keys)
                            {
                                if (eap.mapPrototypes[instance] == null || !eap.mapDefinitions.ContainsKey(eap.mapPrototypes[instance]))
                                    continue;
                                var prototypesValues = SubscribeStructVariableValueChanged(instance, eap.mapPrototypes[instance], eap.mapDefinitions, ref ret);//SubscribeVariableValuesChanged(reference, instance);
                                    ...
                                    ecpl.mapChildsDefinitions.ContainsKey(c) && ecpl.mapChildsPrototypes.ContainsKey(c))
                                {
                                    ...
                                    foreach (var inst in listofvariables.Keys)
                                    {
                                        if (listofvariables[inst] == null || !listofStructDefinitions.ContainsKey(listofvariables[inst]))
                                            continue;
                                        var prototypesValues = childTags.AddInnerVariable(inst, inExecution);
                                        {
                                        ...
                                        AddStructure(ref prototypesValues, ref ret, listofvariables[inst], listofStructDefinitions, inExecution);
                                        //ret.Add(prototypesValues);
                                    }
            ...
            foreach (var member in dictDefinitions[name])
            {
                ...
                if (idx != -1)
                {
                    ...
                    if (!dictDefinitions.ContainsKey(prototypename))
                        continue;
                    var innerObj = values.AddInnerVariable(membername, inExecution);
                    if(runtime)
                    ...
                    if (listScriptVariableUsed != null)
                    {
                        var found = (from c in listScriptVariableUsed.AsParallel()
                                     where (String.IsNullOrEmpty(instance) && !c.Contains("-") ||
                                          c.StartsWith(String.Format("{0}-", startInstance)))
                                     select c).ToList();
                        ...
                        found.ForEach(var =>
                        {
                            ...
                            var instanceName = reference;
                            if (!String.IsNullOrEmpty(startInstance))
                            {
                                string prefix = String.Format("{0}-", startInstance);
                                if (reference.StartsWith(prefix))
                                {
                                    reference = reference.Substring(prefix.Length);
                                }
        ...
        public Object SetVariableValue(String Name, Object value, int timeoutMS = -1, bool sync = false)
        {
            ...
            if (variableValue == null ||
                (variableValue != null && !string.IsNullOrEmpty(variableValue.ChildProjetName)))
            {
                //this tag is in a child project
                variableValue = GetChildVariableValue(Name);
                name = Name;
        ...
        public void SetImagesBaseUri(List<UIElement> listChildren)
        {
            ...
            var listImagesToUpdate = new List<Image>();
            ...
            listImagesToUpdate.ForEach(image =>
            {
                var map = image.GetAllValueEntries();
                
                var listExpression = map.Keys
                        .Where(key =>
                            (key.Value as BindingExpression)?.ParentBinding?.Converter is IUriToUriAbsoluteImageConverter)
                        .Select(key =>
                            (key.Value as BindingExpression))?
                        .ToList();
        ...
        public void SetImageBinding(List<Image> listImages, IUriToUriAbsoluteImageConverter uriToUriAbsoluteImageConverter)
        {
            ...
            listImages.ForEach(image =>
            {
                ...
                var binding = new Binding()
                {
                    ...
                    ConverterParameter = System.IO.Path.GetFileName(image.Source.ToString())
                };

                (binding.Converter as IUriToUriAbsoluteImageConverter).FileSystemProviderBase = fileSystemProviderBase;
                (binding.Converter as IUriToUriAbsoluteImageConverter).AbsolutePath = AbsolutePath;
                (binding.Converter as IUriToUriAbsoluteImageConverter).AbsolutePath2 = AbsolutePath2;
        ...
        public void RenameReferences(Dictionary<string, string> nodeIdMap, UFInterfaces.Editors.CrossReferenceModel model)
        {
            using (var cursor = new WaitCursor())
            {
                ...
                if (model.ApplyNewNames)
                {
                    ...
                    foreach (var screen in MapScreenEntities.Values)
                    {
                        //Check menu
                        var newUri = MakeRelativeUri(MakeAbosoluteUri(screen.MenuName));
                        if (newUri != screen.MenuName)
                        {
                            screen.MenuName = newUri;
                            bDirty = true;
                        }

                        
                        //Check other associated Uris
                        if(screen.MapProblematicXamlWriterProperties != null)
                        {
                            Dictionary<string, Uri> mapUris = new Dictionary<string, Uri>();
                            foreach (var property in screen.MapProblematicXamlWriterProperties)
                            {
                                ...
                                try
                                {
                                    using (var reader = new StringReader(value))
                                    {
                                        ...
                                        {
                                            var uri = XamlReader.Load(textReader) as Uri;
                                            if (uri != null)
                                                mapUris.Add(property.Key, uri);
                                        }
                                    }
                                }
                                catch
                            ...
                            if (mapUris.Count > 0)
                            {
                                foreach (var mapUri in mapUris)
                                {
                                    newUri = MakeRelativeUri(MakeAbosoluteUri(mapUri.Value));
                                    screen.MapProblematicXamlWriterProperties[mapUri.Key] =
                                        screen.MapProblematicXamlWriterProperties[mapUri.Key].Replace(mapUri.Value.ToString(), newUri.ToString());
                                }
                            }
                        }
                ...
                if(mapScriptVariableUsed != null && mapScriptVariableUsed.Count > 0)
                {
                    Dictionary<string, string> dictScriptVariableUsed = new Dictionary<string, string>();

                    foreach (var entity in MapScreenEntities.Values)
                    {
                        if(mapScriptVariableUsed.ContainsKey(entity.EntityName))
                        {
                            dictScriptVariableUsed.Clear();
                            foreach (var variable in mapScriptVariableUsed[entity.EntityName])
                            {
                                var idx = variable.IndexOf(idSeparator);
                                dictScriptVariableUsed[idx >= 0 ? variable.Substring(0, idx) : variable] =
                                    idx >= 0 ? variable.Substring(idx + 1) : variable;
                            }
                            if (entity.RenameReferences(dictScriptVariableUsed, UFUAEditor, this))
                                bDirty = true;
                        }
                    }
                    dictScriptVariableUsed.Clear();
                    ...
                    {
                        foreach (var variable in mapScriptVariableUsed[innerEntityNameFormat])
                        {
                            var idx = variable.IndexOf(idSeparator);
                            dictScriptVariableUsed[idx >= 0 ? variable.Substring(0, idx) : variable] =
                                idx >= 0 ? variable.Substring(idx + 1) : variable;
                            listv.Add(idx >= 0 ? variable.Substring(idx + 1) : variable);
                        }
                    }

                    var mapNodes = UFUAEditor.GetListNodeNames(this, listv);
                    string szCode = Code;
                    foreach (var key in dictScriptVariableUsed.Keys)
                    {
                        string searchid = dictScriptVariableUsed[key];
...
#if DEBUG
                            System.Diagnostics.Debug.WriteLine("Replacing {0} with {1}", key, newname);
#endif
                ...
                if (bDirty)
                {
                    bool t = bRecalculateListScriptVariableUsed;
                    bRecalculateListScriptVariableUsed = true;
                    var canvas = GetCurrentXamlDocument();
                    RecalculateListVariableUsed(canvas);
                    SaveToFile();
                    bRecalculateListScriptVariableUsed = t;
                    canvas.Dispose();
                }
            }
        }
                        ...
                        Action OnControlFirstLoaded = () =>
                        {
                            ...
                            if (!bAnimate || IsOnBlindServer() || DelayLoadSymbols == 0 || contentControl.Visibility != Visibility.Visible)
                            {
                                if (inExecution || IsOnBlindServer()) // wait for the symbol been loaded
                                {
                                    ...
                                    var mergedInners = new List<String>(listInners);
                                    mergedInners.AddRange(postlistInners);
                                    if (mergedInners.Count > 0)
                                    {
                                        //ret = fe.Dispatcher.BeginInvokeAsynchronouslyInBackground(() =>
                                        //{
                                        //    if (!bDisposed)
                                        //    {
                                        PrepareExecution(canvas, currentSessionName, mergedInners, true);
        ...
        public void RecalculateListVariableUsed(Canvas cv)
        {
            ...
            if (MapScreenEntities != null)
            {
                ...
                listScripts.ForEach(key =>
                {
                    ...
                    UpdateMapScriptVariableUsed(map, key);
                });
            }

            var list = GetListUsedVariables(skipDataService: true);
            UpdateMapScriptVariableUsed(list, innerEntityNameFormat);
        }
        void UpdateMapScriptVariableUsed(Dictionary<string, OPCUAEntityReference> map, string key)
        {
            if (map != null && map.Count > 0)
            {
                ...
                if (!mapScriptVariableUsed.ContainsKey(key))
                {
                    ...
                    foreach(var vu in map)
                    {
                        ls.Add(string.Format(varUsedFormat, vu.Key, vu.Value.ResolvedNodeId));
                    }
                    mapScriptVariableUsed.Add(key, ls);
                }
            }
        }
        ...
        */
        ...
        public bool SaveToFile(bool forceEncryption = false, bool bUploading = false, bool bThrowOnError = false)
        {
            try
            {
                ...
                if (Parent != null)
                {
                    ...
                    if (rootParent != null)
                        ScreenCompilerManager.ScreenCompilerManager.CompileScreen(FullPath, rootParent.FilePath);
                }
            }
            catch (Exception ex)
            {
                        ...
                        !dpd.IsReadOnly && IsValidPropertyXamlWriter(dpd.Name))
                    {
                        ...
                        notifier.ValueChanged += (o, e) =>
                        {
                            ...
                            if (MapScreenEntities.ContainsKey(key) && !MapScreenEntities[key].bUpdatingProperty)
                            {
                                ...
                                if (value == null && dpd.DependencyProperty.PropertyType == typeof(String))
                                    value = String.Empty;
        ...
        public IEnumerable<dynamic> GetAllSourceScreenStringDetails(string name, CancellationToken quitEvent, Canvas activeLayer, CRMapsHeler cRMapsHeler = null)
        {
            ...
            if (entity != null && entity.Entity != null)
            {
                ...
                if ((_fe is ListBox || _fe is ComboBox) && entity.IsItemSourceEntity())
                {
                    ...
                    {
                        var logger = LogManager.GetLogger(Title);
        ...
        public IEnumerable<dynamic> GetAllSourceScreenConnectionDetails(ConnectionSettings connectionSettings, string name, CancellationToken quitEvent, bool resolveEntities = false, CRMapsHeler cRMapsHeler = null)
        {
            ...
            if (entity?.Entity != null)
            {
                ...
                if (entity.Entity is IConnectionAware connectionAware)
                {
                ...
                if (entity.Entity is IDisposable disposable)
                    disposable.Dispose();
            }
        ...
        internal Dictionary<String, OPCUAEntityReference> GetListUsedVariables(string key = null, ScreenEntity sentity = null, CRMapsHeler cRMapsHeler = null, bool skipDataService = false)
        {
            ...
            if (sentity != null && !string.IsNullOrEmpty(sentity.Code))
            {
                            ...
                            else
                            {
                                var nameToCheck = e.Name;
                                {
                                        ...
                                        childProject: variableValue.ChildProjetName))
                                    {
                                        ...
                                        if (erString != null)
                                            entityReference = erString.FromXml<OPCUAEntityReference>();

                                        if (entityReference != null)
                                            mapResolvedVariables.Add(instanceName, entityReference);
                                        else
                                        {
                                            if (correctName.EndsWith(qualityTagName))
                                            {
                                                var nameNoQuality = correctName.Substring(0, correctName.Length - qualityTagName.Length);
                                                erString = service.GetTagEntityReference(this, correctName,
                                                    correctInstance,
                                                    useCachedUow: true,
                                                    Project: variableValue.ChildProjetName);
                                                if (erString != null)
                                            ...
                                            else if (correctName.EndsWith(timestampTagName))
                                            {
                                                var nameNoTimestamp = correctName.Substring(0, correctName.Length - timestampTagName.Length);
                                                erString = service.GetTagEntityReference(this, correctName,
                                                    correctInstance,
                                                    useCachedUow: true,
                                                    Project: variableValue.ChildProjetName);
                    ...
                    if (!string.IsNullOrEmpty(entity.Code) && mapScriptVariableUsed != null && service != null)
                    {
                        ...
                        if (!bRecalculateListScriptVariableUsed)
                        {
                            ...
                            if (map != null)
                            {
                                ...
                                {
                                    var dict = GetCRScriptTags(listReference[i], service, scriptTagMap, listDataSinkInterfaces);
                                    if(dict != null)
                                        yield return dict;                                    
                                }
                            }
                            ...
                            foreach (var propName in listofkeys)
                            {
                                ...
                                if (!String.IsNullOrEmpty(valueString))
                                {
                                    ...
                                    if (obj != null)
                                    {
                                        ...
                                        {
                                            var dictionary = new Dictionary<string, OPCUAEntityReference>();
                                            dictionary[string.Format("{0}\\{1}", Properties.Resources.DynamicTagHeader, propName)] = entity.ReplaceAlias((obj as OPCUAXMLEntityReference).TagReference);
                                            yield return dictionary;
                                        }
                                        else if (obj is OPCUAEntityReference)
                                        {
                                            var dictionary = new Dictionary<string, OPCUAEntityReference>();
                                            dictionary[string.Format("{0}\\{1}", Properties.Resources.DynamicTagHeader, propName)] = entity.ReplaceAlias((obj as OPCUAEntityReference));
                                            yield return dictionary;
                                        }
                                        else if (obj is IList)
                                        {
                                            ...
                                            for (int i = 0; i < objlist.Count; i++)
                                            {
                                                ...
                                                {
                                                    var dictionary = new Dictionary<string, OPCUAEntityReference>();
                                                    dictionary[string.Format("{0}\\{1}", Properties.Resources.DynamicTagHeader, propName)] = entity.ReplaceAlias((prop.GetValue(o) as OPCUAEntityReference));
                                                    yield return dictionary;
                                                };
                                                foreach (var prop in opcXMLProperties)
                                                {
                                                    var dictionary = new Dictionary<string, OPCUAEntityReference>();
                                                    dictionary[string.Format("{0}\\{1}", Properties.Resources.DynamicTagHeader, propName)] = entity.ReplaceAlias((prop.GetValue(o) as OPCUAXMLEntityReference)?.TagReference);
                                                    yield return dictionary;
                                                };
                                            }
                                        }
                    ...
                    if (!string.IsNullOrEmpty(sCode) && mapScriptVariableUsed != null && service != null)
                    {
                        ...
                        else
                        {
                            ...
                            if (smap != null && smap.Count > 0)
                            {
                                ...
                                {
                                    List<string> ls = new List<string>();
                                    foreach(var v in smap)
                                    {
                                        ls.Add(string.Format(varUsedFormat, v.Key, v.Value.ResolvedNodeId));
                                    }
                                    mapScriptVariableUsed.Add(innerEntityNameFormat, ls);
                                }
        ...
        Dictionary<string, OPCUAEntityReference> GetCRScriptTags(string referenceItem, IUFUAEditorManager service, Dictionary<string, object> scriptTagMap, List<string> listDataSinkInterfaces)
        {
            bool bExist = false;
            OPCUAEntityReference tag = null;
            var idx = referenceItem.IndexOf(idSeparator);
            var reference = (idx >= 0 ? referenceItem.Substring(0, idx) : referenceItem);
            var nodeid = (idx >= 0 ? referenceItem.Substring(idx + 1) : string.Empty);
            lock (lockObject)
            {
            ...
            else
            {
                ...
                else
                {
                    ...
                    var listv = new List<string>() { nodeid };
                    var mapNodes = service.GetListNodeNames(this, listv);
                    if(mapNodes.Count > 0 && mapNodes.ContainsKey(nodeid))
                    {
                        ...
                        var xml = service.GetTagEntityReference(this, tname, tinstance, useCachedUow: true);
                        ...
                        if(tag != null && tag.HumanReadable.Length > 0)
                        {
                            ...
                            lock (lockObject)
                            {
                                if (!scriptTagMap.ContainsKey(tname/*reference*/))
                                    scriptTagMap.Add(tname/*reference*/, oTag);
                            }
                            dictionary[string.Format("{0}", Properties.Resources.ItemScriptHeader)] = oTag;
                            return dictionary;
                        }
                    }
                }
        ...
        /// <summary>
        /// Applies the redundancy logic on the control's ConnectionString after it was eventually customized depending on control's configuration
        /// </summary>
        /// <param name="list">The list of the controls defining a ConnectionString</param>
        [EditorBrowsable(EditorBrowsableState.Never)]
        public void PostBindConnectionStringSource(List<FrameworkElement> list)
        {
            try
            {
                foreach (var fe in list)
                    BindConnectionStringSource(fe, null, bOverwrite: true);
            }
            catch (Exception ex)
            {
...
#endif
                    BindConnectionStringSource(fe, connectionString, bDesign);
                }
            }
            catch (Exception ex)
            {
        ...
        void BindConnectionStringSource(FrameworkElement fe, String connectionString, bool bDesign = false, bool bOverwrite = false)
        {
            var t = fe.GetType();
            var p = t.GetProperty(bDesign ? "ConnectionStringDesignMode" : "ConnectionString");
            if (p != null)
            {
                if (bOverwrite) //Replacing the hostname of the actual connectionstring property value to target the active server
                {
                    connectionString = RealTimeConnectionManagerViewModel.ReplaceServerRenamedOnDataSource(p.GetValue(fe) as String, SessionString);
                    p.SetValue(fe, connectionString, null);
                }
                else if (String.IsNullOrEmpty(p.GetValue(fe, null) as String))
                    p.SetValue(fe, connectionString, null);
            }
        }
...
#if !NET_STANDARD
        internal const string sExpression = "{0}-Expression";
        internal const string sInternalExpression = "{0}-{1}-{2}-Expression";
        internal const string sReverseExpression = "{0}-ReverseExpression";
        internal const string sEntityReference = "{0}-{1}";
        internal const string sInternalEntityReference = "{0}-{1}-{2}";
        Dictionary<string, string> dictOriginalAliases = null;

        internal bool TryGetAlias(string sKey, out string sAlias)
        {
            if (dictOriginalAliases == null)
            {
                sAlias = string.Empty;
                return false;
            }
            return dictOriginalAliases.TryGetValue(sKey, out sAlias);
        }
        internal void AddAlias(string sKey, string sAlias)
        {
            if (dictOriginalAliases == null)
                dictOriginalAliases = new Dictionary<string, string>();

            dictOriginalAliases[sKey] = sAlias;
        }
        List<string> GetEntityWithAliases()
        {
            var list = new List<String>();
            var screenEntityList = mapScreenEntities.Values.ToList();
            string sAlias;
            foreach(var seVal in MapScreenEntities)
            {
                string seName = seVal.Key;
                ScreenEntity e = seVal.Value;
                if ((e.OpcuaEntityReference != null &&
                    (TryGetAlias(string.Format(sEntityReference, e.ID, e.OpcuaEntityReference.RelativePath), out sAlias) ||
                        AliasHelper.ContainsAlias(e.OpcuaEntityReference.RelativePath))) ||
                    (e.OpcuaEntityReference3DZoom != null &&
                    (TryGetAlias(string.Format(sEntityReference, e.ID, e.OpcuaEntityReference3DZoom.RelativePath), out sAlias) ||
                        AliasHelper.ContainsAlias(e.OpcuaEntityReference3DZoom.RelativePath))) ||
                    (AliasHelper.ContainsAlias(e.Expression)) ||
                    (TryGetAlias(string.Format(sExpression, e.ID), out sAlias)) ||
                    (AliasHelper.ContainsAlias(e.ReverseExpression)) ||
                    (TryGetAlias(string.Format(sReverseExpression, e.ID), out sAlias))
                    )
                {
                    list.Add(seName);
                    continue;
                ...
                {
                    bool bFound = false;
                    foreach (CommandManager.CommandManager command in e.CommandList)
                    {
                        var tags = command.ListTags;
                        foreach (var r in tags)
                        {
                            if (AliasHelper.ContainsAlias(r.RelativePath) ||
                                (TryGetAlias(string.Format(sInternalEntityReference, e.ID, command.Name, r.RelativePath), out sAlias))
                                )
                            {
                                bFound = true;
                                list.Add(seName);
                        ...
                        if (AliasHelper.ContainsAlias(command.Expression) ||
                            (TryGetAlias(string.Format(sInternalExpression,
                                e.ID, command.Name, command.Expression), out sAlias))
                            )
                        {
                            list.Add(seName);
                            break;
                ...
                if (e.HasAnimations)
                {
                    foreach (AnimationManager.AnimationManager animation in e.AnimationList)
                    {
                        if (animation.OpcuaEntityReference != null &&
                            (AliasHelper.ContainsAlias(animation.OpcuaEntityReference.RelativePath) ||
                            (animation.OpcuaEntityReference != null && TryGetAlias(string.Format(sInternalEntityReference,
                            e.ID, animation.Name, animation.OpcuaEntityReference.RelativePath), out sAlias))
                            ))
                        {
                            list.Add(seName);
                            break;
                        ...
                        (TryGetAlias(string.Format(sInternalExpression,
                            e.ID, animation.Name, animation.Expression), out sAlias))
                        )
                        {
                            list.Add(seName);
                            break;
                ...
                if (e.Element is IDynamicTagAware)
                {
                    var dyamicAware = e.Element as IDynamicTagAware;
                    ...
                    foreach (var key in dynamicMap.Keys)
                    {
                        ...
                        string sId = string.Format(sInternalEntityReference, e.ID, key, reference.RelativePath);
                        if (AliasHelper.ContainsAlias(reference.RelativePath) ||
                            (TryGetAlias(sId, out sAlias))
                            )
                        {
                            list.Add(seName);
                            break;
        ...
        /// <summary>
        /// Apply changes made to the Screen Alias table (or screen object Alias table) to all the screen objects containing an alias.
        /// </summary>
        public void ApplyAlias()
        {
            var listEntityWithAlias = GetEntityWithAliases();

            if (inExecution)
            {
                CleanInIdle();

                listEntityWithAlias.ForEach(key =>
                {
#if !WINDOWS_UWP
                                    ...
                                    if (!bDisposed)
                                    {
                                        pendingTerminatingScript.ToList().ForEach(element =>
                                        {
                                                ...
                                                MapScreenEntities[element].TerminateScriptCode())
                                            {
                                                ...
                                                MapScreenEntities[element].ReplaceAllAliasInOnce();
                ...
                listEntityWithAlias.ForEach(key =>
                {
                    if (pendingTerminatingScript == null || !pendingTerminatingScript.Contains(key))
                    {
                        MapScreenEntities[key].ReplaceAllAliasInOnce();
        ...
        /// <summary>
        /// Set the document in idle state or restore the running state.
        /// </summary>
        /// <param name="newValue"> true if the idle state have to be set. </param>
        /// <param name="bTerminating"></param>
        [EditorBrowsable(EditorBrowsableState.Never)]
        public void SetInIdle(bool newValue, bool bTerminating = false)
        {
            if (!inExecution || KeepAlwaysTagsAlive || inIdle == newValue)
                return;
            inIdle = newValue;
            
            if (mapInIdleOPCItemToEntity == null)
            {
                mapInIdleOPCItemToEntity = new Dictionary<OPCUAEntityReference, IEntityReference>();
                
                var map = GetDynamicMapForElementAndChilds();
                foreach (var el in map.Keys)
                {
                    foreach (var item in map[el])
                    {
                        item.SetKeepAlive(inIdle);
                        item.SetInUse(MapScreenEntities[el], !inIdle);

                        if (MapScreenEntities[el] is ScreenEntity screenEntity)
                            screenEntity.ExpressionTagsInIdle = inIdle;

                        if (!mapInIdleOPCItemToEntity.ContainsKey(item))
                            mapInIdleOPCItemToEntity.Add(item, MapScreenEntities[el]);
                    }
                }

                if (inuseVariableValueReferences != null && mapOPCItemToEntity != null)
                {
                    inuseVariableValueReferences.ForEach(item =>
                    {
                        item.SetKeepAlive(inIdle);
                        item.SetInUse(mapOPCItemToEntity[item], !inIdle);

                        if (!mapInIdleOPCItemToEntity.ContainsKey(item))
                            mapInIdleOPCItemToEntity.Add(item, mapOPCItemToEntity[item]);
                    });
                }
            }
            else
            {
                foreach (var item in mapInIdleOPCItemToEntity.Keys)
                {
                    item.SetKeepAlive(inIdle);
                    item.SetInUse(mapInIdleOPCItemToEntity[item], !inIdle);
                }
            }

            if (bTerminating && mapInIdleOPCItemToEntity != null)
            {
                mapInIdleOPCItemToEntity.Clear();
                mapInIdleOPCItemToEntity = null;
            }
        }

        void CleanInIdle()
        {
            if (!inExecution || KeepAlwaysTagsAlive)
                return;

            if (mapInIdleOPCItemToEntity != null)
            {
                foreach (var item in mapInIdleOPCItemToEntity.Keys)
                    item.SetKeepAlive(false);
        ...
        public bool KeepAlwaysInMemory
        {
            ...
            set
            {
                ...
                OnPropertyVisiblityChanged(nameof(KeepAlwaysInMemory));
                NeedsSave = true;
            }
#endif
        }
        ...
        public bool KeepAlwaysTagsAlive
        {
            get
            {
                return keepAlwaysTagsAlive;
            }
#if !WINDOWS_UWP && !NET_STANDARD
            set
            {
                if (keepAlwaysTagsAlive == value)
                    return;
                keepAlwaysTagsAlive = value;
                OnPropertyChanged(nameof(KeepAlwaysTagsAlive));
                NeedsSave = true;
            }
#endif
        }
        ...
        protected override String PerformValidation(String propertyName)
        {
            ...
            else if (propertyName == "WriteTimeout")
            {
        ...
        List<IDynamicTagAware> listDynamicTagElement;
        [EditorBrowsable(EditorBrowsableState.Never)]
        public void ChangeUnitConverter(String converter, IUnitConverterEditorManager converterEditor, string elementName = null)
        {
            ...
            if (listDynamicTagElement == null)
                listDynamicTagElement = (from c in MapScreenEntities.Values.AsParallel()
                                         where c.Element is IDynamicTagAware
                                         select c.Element as IDynamicTagAware).ToList();

            List<IDynamicTagAware> _listConverterSystem = new List<IDynamicTagAware>();

            if (!string.IsNullOrEmpty(elementName))
            {
                if (MapScreenEntities.ContainsKey(elementName))
                {
                    var entityElement = MapScreenEntities[elementName].Element;
                    if(entityElement is IDynamicTagAware)
                    {
                        _listConverterSystem.Add(entityElement as IDynamicTagAware);
                        GetListInners(elementName).ForEach(I =>
                        {
                            if(MapScreenEntities[I].Element is IDynamicTagAware)
                            {
                                _listConverterSystem.Add(MapScreenEntities[I].Element as IDynamicTagAware);
                            }
                        });
                    }
                ...
                _listConverterSystem = listDynamicTagElement;

            foreach (var entity in _listConverterSystem)
            {
                entity?.SetUnitConverterSystem(converter);
            }
        }
#endif
#if !NET_STANDARD
                ...
                action = new Action<bool>((bInit) =>
                {
                    if (control.ItemsSource != null)
                    {
                        ...
                        if (list.Count > 0)
                        {
                            ...
                            else if (!bInit && CheckComboItemsChanged(list, mapControlListId[control].translatedStrings, mapControlListId[control].untranslatedStrings))
                            {
                                bInit = true;
                                mapControlListId[control] = new ElementTranslation(list);
                            }
                            ...
                            mapControlListId[control].untranslatedStrings.ForEach(id =>
                            {
                                if (map.ContainsKey(id) &&
                                    ...
                                    newSource.AddRange(TranslationHelpers.TranslationHelper.TranslateComposedText(map[id], map, map[id]).Split(new Char[] { '|', '\n', '\r' }));
                                else
                                    newSource.AddRange(TranslationHelpers.TranslationHelper.TranslateComposedText(id, map, id).Split(new Char[] { '|', '\n', '\r' }));
                            });
                            if (bInit)
                            ...
                            if (oldSource == null || !oldSource.SequenceEqual(newSource))
                            {
                                ...
                                if (mapItemSourceDispatcherOperation != null && 
                                    mapItemSourceDispatcherOperation.ContainsKey(_control) && 
                                    mapItemSourceDispatcherOperation[_control] != null &&
                                    mapItemSourceDispatcherOperation[_control].Status != DispatcherOperationStatus.Aborted &&
                                    mapItemSourceDispatcherOperation[_control].Status != DispatcherOperationStatus.Completed)
                                        mapItemSourceDispatcherOperation[_control].Abort();
        ...
        static bool CheckComboItemsChanged(List<string> actualItems, List<string> translatedItems, List<string> untranslatedItems)
        {
            bool bRet = false;
            if (actualItems.Count != untranslatedItems.Count)
                bRet = true;
            else
                foreach (var id in actualItems)
                {
                    if (!translatedItems.Contains(id) && !untranslatedItems.Contains(id))
                    {
                        bRet = true;
                        break;
                    }
        ...
        bool INotifyPropertyVisibilityChanged.this[string propertyName]
        {
            get
            {
                ...
                //else if (propertyName == nameof(KeepAlwaysTagsAlive))
                //{
                //    return KeepAlwaysInMemory;
                //}
