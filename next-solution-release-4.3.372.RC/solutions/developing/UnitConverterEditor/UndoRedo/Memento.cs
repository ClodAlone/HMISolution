...
namespace UnitConverterManager.UndoRedo
{
    internal class Memento : IEquatable<Memento>
    {
        ...
        #region Properties
        public List<ExpandoObject> Converters
        {
            get
            {
                return listConverters.ToList();
            }
        }
        ...
        {
            return listConverters.GetHashCode() ^ listConverters.GetHashCode();
        }
        ...
        public bool Equals(Memento other)
        {
            ...
            if (Converters.Count != other.Converters.Count ||
            ...
            {
                var source = Converters[ii] as IDictionary<string, object>;
                var target = other.Converters[ii] as IDictionary<string, object>;
                ...
                foreach (var key in source.Keys)
                {
                    ...
                    if (source[key] is UnitConverterModel.UFConverterItem)
                    {
                        var itemSource = source[key] as UnitConverterModel.UFConverterItem;
                        var itemTarget = target[key] as UnitConverterModel.UFConverterItem;

                        if (itemSource.Name != itemTarget.Name ||
                            itemSource.InputExpression != itemTarget.InputExpression ||
                            itemSource.OutputExpression != itemTarget.OutputExpression ||
                            itemSource.InputUnit != itemTarget.InputUnit ||
                            itemSource.Description != itemTarget.Description ||
                            itemSource.Locale != itemTarget.Locale ||
                            itemSource.Culture != itemTarget.Culture)
                            return false;
                    }
