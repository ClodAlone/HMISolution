	...
	public static class NetworkStatus
	{
		...
		private static bool IsNetworkAvailable ()
		{
			...
			if (NetworkInterface.GetIsNetworkAvailable())
			{
				...
				{
					// filter so we see only Internet adapters and not Wireless connection
					if (face.OperationalStatus == OperationalStatus.Up)
					{
						if ((face.NetworkInterfaceType != NetworkInterfaceType.Tunnel) &&
							(face.NetworkInterfaceType != NetworkInterfaceType.Loopback) &&
                            (face.NetworkInterfaceType != NetworkInterfaceType.Wireless80211))
						{
                            // testing gateway addresses for excluding virtual adapter.
                            IPInterfaceProperties properties = face.GetIPProperties();
