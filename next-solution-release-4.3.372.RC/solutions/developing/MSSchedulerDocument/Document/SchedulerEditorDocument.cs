...
namespace MSSchedulerSettings.Document
{
    public class SchedulerEditorDocument : ViewModelBase, ICloneable, IDocument
    {
#region Declarations
        ...
        {
            var ufuaEditorService = GetService(typeof(IUFUAEditorManager)) as IUFUAEditorManager;
            if (ufuaEditorService != null)
            {
                using (var cursor = new WaitCursor())
                {
                    ...
                    List<MSModel.MSScheduledAction> taglist = GetCRFlatTagCollection().ToList();
                    bool bDirty = false;
                    ...
                    taglist.ForEach(item =>
                    {
                        ...
                        {
                            CommandManagerList commandsOn = null;
                            CommandManagerList commandsOff = null;
                            CommandManagerList commandsOnEx = null;
                            CommandManagerList commandsOffEx = null;
                            if (!string.IsNullOrEmpty(item.CommandsOn))
                                commandsOn = item.CommandsOn.FromXml<CommandManagerList>();
                            if (!string.IsNullOrEmpty(item.CommandsOff))
                                commandsOff = item.CommandsOff.FromXml<CommandManagerList>();
                            if (!string.IsNullOrEmpty(item.ExceptionCommandsOn))
                                commandsOnEx = item.ExceptionCommandsOn.FromXml<CommandManagerList>();
                            if (!string.IsNullOrEmpty(item.ExceptionCommandsOff))
                                commandsOffEx = item.ExceptionCommandsOff.FromXml<CommandManagerList>();
                            if (!string.IsNullOrEmpty(item.CommandsOn))
                            {
                                if(RenameCommand(commandsOn, model))
                                {
                                    bDirty = true;
                                    item.CommandsOn = commandsOn.ToXml();
                                }
                            }
                            if (!string.IsNullOrEmpty(item.CommandsOff))
                            {
                                if(RenameCommand(commandsOff, model))
                                {
                                    bDirty = true;
                                    item.CommandsOff = commandsOff.ToXml();
                                }
                            }
                            if (!string.IsNullOrEmpty(item.ExceptionCommandsOn))
                            {
                                if (RenameCommand(commandsOnEx, model))
                                {
                                    bDirty = true;
                                    item.ExceptionCommandsOn = commandsOnEx.ToXml();
                                }
                            }
                            if (!string.IsNullOrEmpty(item.ExceptionCommandsOff))
                            {
                                if (RenameCommand(commandsOffEx, model))
                                {
                                    bDirty = true;
                                    item.ExceptionCommandsOff = commandsOffEx.ToXml();
                                }
                            }
                        }
                    });
                    ...
                    foreach (var node in mapNodes.Keys)
                    {
                        ...
                        found.ForEach(fitem =>
                        {
                            ...
                            taglist.ForEach(item =>
                            {
                                bool bTag = false;
                                bool bETag = false;
                                string nodeid = fitem.ResolvedNodeId.ToString();
                                bTag = !string.IsNullOrEmpty(item.ScheduleItem) && item.ScheduleItem.Contains(nodeid);
                                bETag = !string.IsNullOrEmpty(item.EnableVariable) && item.EnableVariable.Contains(nodeid);

                                if (bTag)
                                {
                                    bDirty = true;
                                    item.ScheduleItem = postChanged;
                                }
                                if (bETag)
                                {
                                    bDirty = true;
                                    item.EnableVariable = postChanged;
                                }
                                CommandManagerList commandsOn = null;
                                CommandManagerList commandsOff = null;
                                CommandManagerList commandsOnEx = null;
                                ...
                                if (commandsOff != null)
                                {
                                    commandsOff.ForEach(command =>
                                    {
                                        command.UpdateTags(preChanged.FromXml<OPCUAEntityReference>(), postChanged.FromXml<OPCUAEntityReference>());
                                    });
                                    item.CommandsOff = commandsOff.ToXml();
                                }
                                if (commandsOnEx != null)
                                {
                                    commandsOnEx.ForEach(command =>
                                    {
                                        command.UpdateTags(preChanged.FromXml<OPCUAEntityReference>(), postChanged.FromXml<OPCUAEntityReference>());
                                    });
                                    item.ExceptionCommandsOn = commandsOnEx.ToXml();
                                }
                                if (commandsOffEx != null)
                                {
                                    commandsOffEx.ForEach(command =>
                                    {
                                        command.UpdateTags(preChanged.FromXml<OPCUAEntityReference>(), postChanged.FromXml<OPCUAEntityReference>());
                                    });
                                    item.ExceptionCommandsOff = commandsOffEx.ToXml();
        ...
        private List<OPCUAEntityReference> GetOPCReferenceList(MSModel.MSScheduledAction eventitem)
        {
            var list = new List<OPCUAViewModel.OPCUAEntityReference>();
            if (!string.IsNullOrEmpty(eventitem.ScheduleItem))
            {
                var tag = eventitem.ScheduleItem.FromXml<OPCUAEntityReference>();
                list.Add(tag);
            }
            if (!string.IsNullOrEmpty(eventitem.EnableVariable))
            {
                var tag = eventitem.EnableVariable.FromXml<OPCUAEntityReference>();
                list.Add(tag);
            }
            CommandManagerList commandsOn = null;
            CommandManagerList commandsOff = null;
            CommandManagerList commandsOnEx = null;
            CommandManagerList commandsOffEx = null;
            if (!string.IsNullOrEmpty(eventitem.CommandsOn))
                commandsOn = eventitem.CommandsOn.FromXml<CommandManagerList>();
            if (!string.IsNullOrEmpty(eventitem.CommandsOff))
                commandsOff = eventitem.CommandsOff.FromXml<CommandManagerList>();
            if (!string.IsNullOrEmpty(eventitem.ExceptionCommandsOn))
                commandsOnEx = eventitem.ExceptionCommandsOn.FromXml<CommandManagerList>();
            if (!string.IsNullOrEmpty(eventitem.ExceptionCommandsOff))
                commandsOffEx = eventitem.ExceptionCommandsOff.FromXml<CommandManagerList>();
            commandsOn?.ForEach(command => list.AddRange(command.ListTags));
            commandsOff?.ForEach(command => list.AddRange(command.ListTags));
            commandsOnEx?.ForEach(command => list.AddRange(command.ListTags));
            commandsOffEx?.ForEach(command => list.AddRange(command.ListTags));
            return list;
        }
#endif
...
#region Clipboard
#if !NET_STANDARD
        public void CopyListFoldersToClipbaord(List<MSModel.MSFolder> list)
        {
            list.ForEach(tag =>
            {
        ...
        public void CopyListEventsToClipbaord(List<MSModel.MSScheduledAction> list)
        {
            list.ForEach(tag =>
            {
                CloneToClipboard(tag, false);
        ...
        public bool ClipboardContainsEvents()
        {
            if (uowClipboard == null)
                return false;
            ...
            try
            {
                return (from c in new XPQuery<MSModel.MSScheduledAction>(uowClipboard).AsParallel() select c).ToList().Count > 0;
            }
            catch
            {
                return false;
        ...
        public bool ClipboardContainsFolders()
        {
            if (uowClipboard == null)
                return false;
            ...
            try
            {
                return (from c in new XPQuery<MSModel.MSFolder>(uowClipboard).AsParallel() select c).ToList().Count > 0;
            }
            catch
            {
                return false;
        ...
        public List<MSModel.MSScheduledAction> PasteClipboardEvents(MSModel.MSFolder folder, MSModel.MSFolder parent = null)
        {
            var ret = new List<MSModel.MSScheduledAction>();
            var mapStartCounter = new Dictionary<string, ulong>();
            var listNodeId = GetEventsNodeIdList();
            var listName = GetEventsNameList(folder);
            var listToCopy = (from c in new XPQuery<MSModel.MSScheduledAction>(uowClipboard).AsParallel() where c.MSFolderAss == parent orderby c.Oid select c).ToList();
            listToCopy.ForEach(tag =>
            {
                ...
                var newtag = CloneFromClipboard(tag, exist) as MSModel.MSScheduledAction;
                newtag.Name = newName;
                if (folder != null)
                    folder.MSScheduledActions.Add(newtag);
                ret.Add(newtag);
            });
            return ret;
        }

        public List<MSModel.MSFolder> PasteClipboardFolders(MSModel.MSFolder folder, MSModel.MSFolder parent = null)
        {
            var ret = new List<MSModel.MSFolder>();
            var mapStartCounter = new Dictionary<string, ulong>();
            var listNodeId = GetFoldersNodeIdList();
            var listName = GetFoldersNameList(folder);
            var listToCopy = (from c in new XPQuery<MSModel.MSFolder>(uowClipboard).AsParallel() where c.MSFolderAss == parent orderby c.Oid select c).ToList();
            listToCopy.ForEach(dir =>
            {
                ...
                var newfolder = CloneFromClipboard(dir, exist) as MSModel.MSFolder;
                newfolder.Name = newName;
                if (folder != null)
                    folder.MSFolders.Add(newfolder);
                ret.Add(newfolder);
            });
            return ret;
        }
...
#region Undo/Redo
        ...
        public void AddExistingObject(XPObject obj, object parent)
        {
            ...
            if (obj is MSModel.MSScheduledAction)
            {
                var tag = obj as MSModel.MSScheduledAction;
                if (parent is MSModel.MSFolder)
                {
                    var root = parent as MSModel.MSFolder;
                    AddExistingObject(tag, FindFolderByNodeId(root.NodeId));
                }
            }
            else if (obj is MSModel.MSFolder)
            {
                var folder = obj as MSModel.MSFolder;
                if (parent is MSModel.MSFolder)
                {
                    var root = parent as MSModel.MSFolder;
        ...
        internal void AddExistingObject(MSModel.MSScheduledAction tag, MSModel.MSFolder root)
        {
            if (root != null)
                root.MSScheduledActions.Add(tag);
        }

        internal void AddExistingObject(MSModel.MSFolder tag, MSModel.MSFolder root)
        {
            if (root != null)
                root.MSFolders.Add(tag);
        }
#endif
#endregion
        ...
        public static SchedulerEditorDocument FromConnectionString(string conn, IDocument parent)
        {
            try
            {
                ...
                var doc = new SchedulerEditorDocument(null)
                {
        ...
        public static SchedulerEditorDocument FromFile(String path, IDocumentManager c, IDocument parent, 
...
#region ICloneable Members
        ...
        SchedulerEditorDocument(SchedulerEditorDocument template)
        {
            if (template == null)
                return;
        ...
        public object Clone()
        {
            return new SchedulerEditorDocument(this);
        }
