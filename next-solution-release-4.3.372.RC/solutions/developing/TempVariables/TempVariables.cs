...
#region Declarations
        ...
        List<IDocument> activePopupDocuments = new List<IDocument>();
...
#region Methods
        ...
        public bool AddTempVariable(Variable x)
        {
            lock (lockObject)
            {
                ...
                string relativepath = x.GetRelative(FolderSeparator); // x.GetRelativeName();
        ...
        void RemoveTempVariable(Variable x)
        {
            lock (lockObject)
            {
                string varname = x.GetRelative(FolderSeparator);
...
#region DataSinkInterface
        ...
        /// <summary>
        /// Start a new instance of TempVariables using the document of the calling window session
        /// </summary>
        /// <param name="parent">document of the calling window session</param>
        public void Start(IDocument parent)
        {
            lock (lockObject)
            {
                if (currentDocument == null)
                ...
                //if this is a popup, the TempVariables of the calling window must be assign to this instance
                //to avoid values incogruences
                var rootAncestor = DocumentHelper.GetRootParent(parent, traverse: false);
                if (mapRunningVariables.ContainsKey(rootAncestor))
                {
                    mapRunningVariables.Add(rootParent, mapRunningVariables[rootAncestor]);
                    mapRunningVariables[rootParent].SetDocumentParent(rootAncestor);
                    activePopupDocuments.Add(rootParent);
                }
            }
        }
        ...
        public OPCUAEntityReference GetReference(string name)
        {
            ...
            return new OPCUAEntityReference(null, DataSynkName, null, name, null, $"{name} ({DataSynkName})", null);
        }
        ...
        public OPCUAEntityReference GetReference(Variable tagFound)
        {
            Opc.Ua.NamespaceTable n = new Opc.Ua.NamespaceTable();
            UInt16 ns = (UInt16)(n.Count + 2 - 1);
            string relativepath = tagFound.GetRelativePath(ns, FolderSeparator);
            string relativename = tagFound.GetRelative(FolderSeparator);
        ...
        public void DisposingDocumentParent(DocumentManager.ComponentService.IDocument parent)
        {
            lock (lockObject)
            {
                ...
                if (mapRunningVariables.ContainsKey(rootParent))
                {
                    if (!activePopupDocuments.Contains(rootParent))
                    {
                        mapRunningVariables[rootParent].Dispose();
                    }
                    else
                    {
                        activePopupDocuments.Remove(rootParent);
                    }
...
#region IDisposable
        ...
        public void Dispose()
        {
            ...
            if (currentDocument != null)
            {
                ...
                if (!singletonInstance.activePopupDocuments.Contains(rootParent))
                { 
                    currentDocument.Dispose();
                }
                else
                {
                    singletonInstance.activePopupDocuments.Remove(rootParent);
                }
            }
        }
#endregion
