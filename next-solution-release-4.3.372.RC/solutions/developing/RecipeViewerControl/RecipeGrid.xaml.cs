    ...
    public partial class RecipeGrid : UserControl, IContainPropertyEditors, ICheckUserCallable, IDisposable
    {
            ...
            Loaded += (ob, ev) =>
            {
                ...
                else
                {
                    ...
                    if (!RunningOnServer)
                        ...
                        recipeIndexUI.RunningOnServer = true;
        ...
        Task Initialize()
        {
            ...
            var task = Task.Factory.StartNew(() =>
            {
                ...
                dataSet = recipeExecuter.CreateDataSet(token, fill: true, out fillError, ref mapWrongValues, bContinueOnError: true);
                return new RecipeGridViewModel(recipeDocument, dataSet, mapWrongValues);
            });
            return task.ContinueWith(ret =>
            {
                ...
                if (ret.Exception != null)
                {
                    try
                    {
                        dataSet = recipeExecuter.CreateDataSet(token, ref mapWrongValues);
                        taskResult = new RecipeGridViewModel(recipeDocument, dataSet, mapWrongValues);
                ...
                if (taskResult != null)
                {
                    ...
                    if (!String.IsNullOrEmpty(fillError))
                    {
                        ...
                        viewModel.MapWrongValues = mapWrongValues;
                    }
            ...
            get
            {
                if (reloadCommand == null)
                {
                    reloadCommand = new RelayCommand(param =>
                    {
                        ...
                        {
                            dataSet.Clear();
                            fillError = recipeExecuter.FillDataSet(dataSet, token, ref mapWrongValues, bContinueOnError: true);
                        });
                        ...
                        task.ContinueWith(ret =>
                        {
                            ...
                            if (ret.Exception != null)
                                ...
                                viewModel.LastError = Properties.Resources.RecipeReadingError;
                        }, TaskScheduler.FromCurrentSynchronizationContext());
        ...
        /// <summary>
        /// Allow to know if a recipe could write on a device
        /// </summary>
        /// <returns>True in case the recipe is writable</returns>
        private bool isSelectedRecipeWritable()
        {
            if(String.IsNullOrEmpty(fillError) || !viewModel.MapWrongValues.ContainsKey(recipeIndexUI.SelectedRecipeId.ToString()))
                return true;
            else if(viewModel.MapWrongValues.ContainsKey(recipeIndexUI.SelectedRecipeId.ToString()) &&
                    viewModel.MapWrongValues[recipeIndexUI.SelectedRecipeId.ToString()].Count() == 0)
                viewModel.MapWrongValues.Remove(recipeIndexUI.SelectedRecipeId.ToString());
