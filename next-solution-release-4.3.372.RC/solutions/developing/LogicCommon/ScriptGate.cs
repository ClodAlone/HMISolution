...
namespace LogicCommon
{
...
#endif
    {
        ...
        static string referenceStartKey = @"'#Reference";

        static string referenceNameSapaceKey = @" #";
        static string referenceVersionKey = @"Version=";
        static string referenceCultureKey = @"Culture=";
        static string referencePublicKeyTokenKey = @"PublicKeyToken=";
        static string referenceProcessorArchitectureKey = @"processorArchitecture=";
        static string referencePathKey = @"Path=";
        static string referenceAppDirKey = @"<APPDIR>";
        #endregion
        ...
        System.Reflection.Assembly[] ParseScriptReferences(string scriptReferences)
        {
            if (String.IsNullOrEmpty(ScriptReferences) || !ScriptReferences.Contains(referenceStartKey))
                return null;

            var assemblies = new List<System.Reflection.Assembly>();
            var referencelines = scriptReferences.Split(new string[] { Environment.NewLine }, StringSplitOptions.RemoveEmptyEntries);
            foreach (var reference in referencelines)
            {
                if (reference.StartsWith(referenceStartKey))
                {
                    string referenceNameSapace = null;
                    string referenceVersion = null;
                    string referenceCulture = null;
                    string referencePublicKeyToken = null;
                    string referenceProcessorArchitecture = null;
                    string referencePath = null;

                    var row = reference.Substring(referenceStartKey.Length);
                    var elements = row.Split(new string[] { ", " }, StringSplitOptions.RemoveEmptyEntries);
                    foreach (var element in elements)
                    {
                        if (element.StartsWith(referenceNameSapaceKey))
                            referenceNameSapace = element.Substring(referenceNameSapaceKey.Length);
                        else if (element.StartsWith(referenceVersionKey))
                            referenceVersion = element.Substring(referenceVersionKey.Length);
                        else if (element.StartsWith(referenceCultureKey))
                            referenceCulture = element.Substring(referenceCultureKey.Length);
                        else if (element.StartsWith(referencePublicKeyTokenKey))
                            referencePublicKeyToken = element.Substring(referencePublicKeyTokenKey.Length);
                        else if (element.StartsWith(referenceProcessorArchitectureKey))
                            referenceProcessorArchitecture = element.Substring(referenceProcessorArchitectureKey.Length);
                        else if (element.StartsWith(referencePathKey))
                            referencePath = element.Substring(referencePathKey.Length);
                    }

                    if (referencePath != null)
                    {
                        if (referencePath.StartsWith(referenceAppDirKey))
                        {
                            var currentDir = System.IO.Path.GetDirectoryName(System.Reflection.Assembly.GetEntryAssembly().Location);
                            referencePath = referencePath.Replace(referenceAppDirKey, currentDir);
                        }

                        try
                        {
                            assemblies.Add(System.Reflection.Assembly.LoadFile(referencePath));
                        }
                        catch { }
                    }
                    else if (referenceNameSapace != null)
                    {
                        var assemblyString = new StringBuilder(referenceNameSapace);
                        if (referenceVersion != null)
                            assemblyString.AppendFormat(", {0}{1}", referenceVersionKey, referenceVersion);
                        if (referenceCulture != null)
                            assemblyString.AppendFormat(", {0}{1}", referenceCultureKey, referenceCulture);
                        if (referencePublicKeyToken != null)
                            assemblyString.AppendFormat(", {0}{1}", referencePublicKeyTokenKey, referencePublicKeyToken);
                        if (referenceProcessorArchitecture != null)
                            assemblyString.AppendFormat(", {0}{1}", referenceProcessorArchitectureKey, referenceProcessorArchitecture);

                        try
                        {
                            assemblies.Add(System.Reflection.Assembly.Load(assemblyString.ToString()));
                        }
                        catch { }
                    }
                }
        ...
        public override void Init(IDocument doc) 
        {
            ...
            engine = new Jint.Engine(cfg =>
            {
                ...
                var assemblies = ParseScriptReferences(ScriptReferences);
                if (assemblies != null)
                    cfg.AllowClr(assemblies); // Access to .net
                else
        ...
        public override bool Execute(WireData[] links)
        {
            ...
            catch (Exception ex)
            {
                ...
                if (ex is Jint.Runtime.JavaScriptException)
                {
                    var lineNumber = (ex as Jint.Runtime.JavaScriptException).LineNumber;
                    message = String.Format(Properties.Resources.JavaScriptRunningException, lineNumber, message);
                }
        ...
        public override void OnDblClick() 
        {
            ...
            var code = ScriptCode;
            var references = ScriptReferences;
            Dialog.Closing += (o, e) =>
            {
                code = control.Text;
        ...
        String _ScriptReferences;
        [Browsable(false)]
        public String ScriptReferences
        {
            get { return _ScriptReferences; }
            set
            {
                var old = _ScriptReferences;
                if (old != value)
                {
                    _ScriptReferences = value;
                    RaisePropertyChanged("ScriptReferences", old, value);
                }
            }
        }
        ...
        public override XElement MakeXElement(XName n)
        {
            ...
            e.Add(XHelper.Attribute("ScriptReferences", this.ScriptReferences, ""));
        ...
        public override void LoadFromXElement(XElement e)
        {
            ...
            this.ScriptReferences = XHelper.Read("ScriptReferences", e, "");
