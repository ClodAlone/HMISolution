...
namespace Utilities
{
    public class RegistryKeysHelper
    {
        ...
        public static string ProEnergyProductKey
        {
            get
            {
                if (!string.IsNullOrWhiteSpace(AssemblyInfo.Company))
                    return string.Format(@"SOFTWARE\{0}\{1} {2}", AssemblyInfo.Company, "Pro.Energy", AssemblyInfo.FileFormatMainVersion);
                else
                    return string.Format(@"SOFTWARE\{0} {1}", "Pro.Energy", AssemblyInfo.FileFormatMainVersion);
            }
        }

        public static string ProLeanProductKey
        {
            get
            {
                if (!string.IsNullOrWhiteSpace(AssemblyInfo.Company))
                    return string.Format(@"SOFTWARE\{0}\{1} {2}", AssemblyInfo.Company, "Pro.Lean", AssemblyInfo.FileFormatMainVersion);
                else
                    return string.Format(@"SOFTWARE\{0} {1}", "Pro.Lean", AssemblyInfo.FileFormatMainVersion);
            }
        }
        #endregion
    }
