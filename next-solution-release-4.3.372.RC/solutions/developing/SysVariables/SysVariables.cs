...
#region DataSinkInterface
        ...
        public void Start(IDocument parent)
        {
            lock (lockObject)
            {
                ...
                {
                    mapRunningVariables.Add(currentDocument, mapRunningVariables[rootAncestor]);
                    mapRunningVariables[currentDocument].SetDocumentParent(rootAncestor);
                    mapRunningVariables[currentDocument].StartSysVariables();
                    activePopupDocuments.Add(currentDocument);
                }
        ...
        public void DisposingDocumentParent(DocumentManager.ComponentService.IDocument parent)
        {
            lock (lockObject)
            {
                ...
                {
                    if (!activePopupDocuments.Contains(root))
                    {
                        mapRunningVariables[root].Dispose();
                    }
                    else
                    {
                        activePopupDocuments.Remove(root);
                    }
