...
{
    class TreeEntityDocManager : IDocumentManager
    {
        #region Declarations
        ...
        public TreeEntityDocManager(ScreenManagerComponent c, ScreenDocument doc, String n)
        {
        ...
        public void Edit(Uri uri, IDocument parent)
        {
            ...
            screenManagerComponent.GetActiveView(uri).SelectElement(Name);
        }
        ...
        public ObservableCollection<IDocumentManager> GetChildDocumentManagers()
        {
            using (new WaitCursor())
            {
                ...
                if (screenDocument.MapScreenEntities.ContainsKey(Name))
                {
                    var listRef = screenDocument.GetDynamicMapForElement(Name);
                    listRef.ForEach(reference => list.Add(new TreeDocumentManagers.TreeEntityReferenceDocManager(screenManagerComponent, screenDocument, reference)));
                    return list;
                }
        ...
        {
            var listRef = screenDocument.GetDynamicMapForElement(Name);
            return listRef != null && listRef.Count > 0; 
        }
