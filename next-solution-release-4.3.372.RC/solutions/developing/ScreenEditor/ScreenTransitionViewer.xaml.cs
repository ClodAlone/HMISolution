...
namespace ScreenManager
{
    ...
    public partial class ScreenTransitionViewer : UserControl, INotifyPropertyChanged, IDisposable
    {
        ...
        readonly bool bDisablePageTransitions;
            ...
            IDocument parent, IScreenController screenController = null, bool test = false)
        {
            ...
            if (!Properties.Settings.Default.DisablePageTransitions)
                bDisablePageTransitions = false;
            else
                bDisablePageTransitions = ScreenController?.DisablePageTransitions ?? true;

            if (bDisablePageTransitions)
            ...
            if (Properties.Settings.Default.EnablePerformancesLog)
                navigationControl.ContentRendered += OnNavigationControlContentRendered;
        ...
        void OnNavigated(object sender, NavigationEventArgs e)
        {
            if (prevSelectedIndex != -1 && prevSelectedIndex < navigatedItems.Items.Count)
            {
                ...
                if (prevcontrol is ScreenViewer && navigatedItems.CurrentItem.Content != prevcontrol)
                {
        ...
        void ShowUserLogin()
        {
            ...
            if (!bLoaded && !bLoadingAutoLoadingScreen && EditorComponent.AuthenticationCredentialsProvider != null && DocumentHelper.GetRootParent(DocumentParent, traverse: true) != null)
            {
            ...
            Dispatcher.BeginInvokeIfRequired(() =>
                {
                    ...
                    {
                        if (!ev.IsRefreshing && EditorComponent.UserEditor != null)
                        {
                            if (!ev.IsPasswordExpired.HasValue)
                                ev.IsPasswordExpired = EditorComponent.UserEditor.VerifyPasswordExpired(DocumentParent, ev.User);
                            if (!ev.IsPasswordExpired.Value)
                            {
                        ...
                        {
                            role = EditorComponent.UserEditor.GetExternalAuthenticationUserRole(DocumentParent, ev.Role);
                            SetAutoLogout(string.Empty, role);
                        }
                        else
                        {
                        ...
                        if (!ev.IsRefreshing)
                        {
                            ...
                            if (EditorComponent.UserEditor != null)
                            {
                                ...
                                if (ev.IsExternalAuthenticationActive)
                                {
                                    level = EditorComponent.UserEditor.GetRoleAccessLevel(DocumentParent, role);
                                    mask = EditorComponent.UserEditor.GetRoleAccessMask(DocumentParent, role);
                                }
                                else
                                {
                                    role = EditorComponent.UserEditor.GetUserRole(DocumentParent, ev.User);
                                    level = EditorComponent.UserEditor.GetUserAccessLevel(DocumentParent, ev.User);
                                    mask = EditorComponent.UserEditor.GetUserAccessMask(DocumentParent, ev.User);   
                                }
        ...
        void SetAutoLogout(String user, string role = null)
        {
            if (EditorComponent.UserEditor == null)
                return;
            ...
            {
                autoLogoutSeconds = EditorComponent.UserEditor.GetAutoLogoutSeconds(DocumentParent, user);
            }
            else
            {
                autoLogoutSeconds = EditorComponent.UserEditor.GetRoleAutoLogoutSeconds(DocumentParent, role);
            }
        ...
        */
        ...
        private void OnNavigationControlContentRendered(object sender, EventArgs _ = null)
        {
            if (!Properties.Settings.Default.EnablePerformancesLog ||
                DXSplashScreen.IsActive ||
                EditorComponent?.performanceWatcher == null ||
                !(sender is NavigationFrameEx navigationFrameEx) ||
                !(navigationFrameEx.Content is FrameworkElement fe)) return;

            Dispatcher.BeginInvokeAsynchronouslyInRender(fe, () =>
            {
                EditorComponent.performanceWatcher.Dispose();
                EditorComponent.performanceWatcher = null;
            });
        }
        ...
        internal void OpenMap(IDocument parent, Rect openingArea, bool enableZoomingScrolling,
            bool showMiniMap, bool showNextButton, Point3D openingParameters)
        {
            ...
            try
            {
                ...
                {
                    geoViewControl.ZoomTo(openingArea, openingParameters);
        ...
        private void OnShowClientStatus(object sender, System.Windows.Input.ExecutedRoutedEventArgs e)
        {
            ...
            else
            {
                ...
                wndClientStatus.SetDialogTheme(ApplicationPropertiesHelper.GetProperty("SolutionSkin") as String);
        ...
        private void OnShowLog(object sender, System.Windows.Input.ExecutedRoutedEventArgs e)
        {
            ...
            else
            {
                ...
                wndLog.SetDialogTheme(ApplicationPropertiesHelper.GetProperty("SolutionSkin") as String);
        ...
        private void OnShowCrossReference(object sender, System.Windows.Input.ExecutedRoutedEventArgs e)
        {
            ...
            else
            {
                ...
                wndCrossReference.SetDialogTheme(ApplicationPropertiesHelper.GetProperty("SolutionSkin") as String);
        ...
        private void OnShowWatchWindow(object sender, ExecutedRoutedEventArgs e)
        {
            ...
            else
            {
                ...
                wndWatch.SetDialogTheme(ApplicationPropertiesHelper.GetProperty("SolutionSkin") as String);
                wndWatch.Show();
            }
        }
        ...
        void LoadAutoLoadScreens()
        {
            ...
            var busyComponent = DocumentParent.GetService(typeof(IBusyComponent)) as IBusyComponent;
            int counter = 0;
            int maxCounter = listAutoLoadedOrdered.Count();
            string busyText = null;
            if (busyComponent != null)
            {
                busyComponent.SetProgress(counter, maxCounter);
                busyText = busyComponent?.BusyContent;
                busyComponent.BusyContent = Properties.Resources.PreloadingScreens;
            }
            try
            {
                ...
                listAutoLoadedOrdered.ForEach(uri =>
                {
                    if (!mapLoaded.ContainsKey(uri))
                    {
                        ...
                        try
                        {
                            ...
                            if (control is ScreenViewer viewer)
                            {
                                viewer.ForceLoadingNow();
                                if (viewer.Document != null)
                                {
                                    viewer.Document.KeepAlwaysInMemory = true;
                                    viewer.Document.SetInIdle(viewer.Document.KeepAlwaysTagsAlive);
                                }
                                //if (!viewer.IsLoaded)
                                //{
                                //    viewer.Loaded += viewer_Loaded;
        ...
        ScreenViewer SetLayoutUri(Uri uri, DevExpress.Xpf.LayoutControl.Dock dock)
        {
            ...
            var item = new ScreenViewer(EditorComponent, uri, DocumentParent, this, bTest, true, bIsLayout: true, dockType: dock);
