...
namespace ScreenManager.SpecialObjects
{
    ...
    public partial class EmbeddedTabScreen : UserControl, IRemotelyCommandable, IContainPropertyEditors, IDisposable, IStringIDAware
    {
        ...
        List<ScreenViewer> PageViewsScreenLoaded = new List<ScreenViewer>();
        #endregion

        #region EventHandlers
        public event RoutedEventHandler EmbeddedScreenLoaded;
        public void ScreenLoadedDelegate(object o, RoutedEventArgs e)
        {
            if (o is ScreenViewer && !PageViewsScreenLoaded.Contains(o))
            {
                EmbeddedScreenLoaded?.Invoke(o, e);
                PageViewsScreenLoaded.Add(o as ScreenViewer);
            }
        }
        #endregion
        ...
        void UpdateTabs(List<String> screens)
        {
            ...
            {
                pageView.BeginInit();
                var screenProperties = GetScreenPropertiesFromScreen(screens[0]);
                if (screenProperties.Length > 0)
                {
                    PopulatePageView(screens[0], null);
                }
                pageView.EndInit();
            }
            else
            {
                ...
                {
                    var screenProperties = GetScreenPropertiesFromScreen(screen);
                    if (screenProperties.Length > 0)
                    {
                        var header = screenProperties.Length > 2 ? screenProperties[2] : System.IO.Path.GetFileNameWithoutExtension(screenProperties[0]);
                        PopulatePageView(screen, header);
                    }
                });
                pageView.EndInit();
            }
            ...
            try
            {
                if (screens != null && screens.Count > 0)
                {
                    if (bDesignerMode && screens.Count == 1)
                    {
                        ...
                        var screenProperties = GetScreenPropertiesFromScreen(screens[0]); 
                        if(screenProperties.Length > 0)
                        txtHelp.Text = System.IO.Path.GetFileNameWithoutExtension(screenProperties[0]);
                    }
                    else
                    {
                        if (txtHelp != null)
        ...
        private string[] GetScreenPropertiesFromScreen(string screen)
        {
            return screen.Split('?');
        }
            ...
            Dispatcher.BeginInvokeIfRequired((Action)(() =>
            {
                ...
                list.ForEach(screen =>
                {
                    if (mapItemTitles.ContainsKey(screen))
                    {
                        ...
                        if (bUntranslated && !String.IsNullOrEmpty(header))
                        {
                        ...
                        else if (!String.IsNullOrEmpty(path))
                        {
                            ...
                            if (map != null)
                            {
                                ...
                                {
                                    screenHeader = TranslationHelpers.TranslationHelper.TranslateComposedText(header, map, header);
                                }
                                else
                                {
                                    if (relative != null)
                                    {
                                        ...
                                        else
                                        {
                                            ...
                                            if (map.ContainsKey(relativeWithoutExtension))
                                                ...
                                                screenHeader = System.IO.Path.GetFileNameWithoutExtension(relative);
                                        }
                                    }
                                }
                            }
                            ...
                            pageView.SelectionChanged += (ob, ev) =>
                                {
                                        ...
                                        selected.Content == null)
                                    {
                                        ...
                                        using (var cursor = new WaitCursor())
                                        {
                                            ...
                                            var tags = selected.Tag.ToString().Split('?');                                                                                        
                                            if (tags.Length > 1)                                                                                            
                                            {
                                                screen = tags[0];
                                                fileParameter = tags[1];                                                                                            
                                            }
                                            ...
                                            if (doc != null)
                                            {
                                                ...
                                                {
                                                    var parent = doc.UpdateParentFromUri(uri);
                                                    
                                                    var view = new ScreenViewer(editor as ScreenManagerComponent, uri,
                                                                                    parent, doc.ActiveView, bTest, true, 
                                                                                    bIsLayout: true, bRunningOnServer:RunningOnServer,
                                                                                    parameterFile: !String.IsNullOrEmpty(fileParameter) ? fileParameter : (doc as ScreenDocument)?.ParameterFile);
                                                    selected.Content = view;
                                                    viewDocument = view.Document;
                                                    view.Loaded += ScreenLoadedDelegate;
                                                }
                                            }
                                        }
                                    }
        ...
        void ClearItems()
        {
            ...
            foreach (var disp in pageView.Items.OfType<PageViewItem>())
            {
                ...
                {
                    (disp.Content as ScreenViewer).Loaded -= ScreenLoadedDelegate;
            ...
            pageView.TemplateReady -= OnTemplateLoaded;
            mapItemTitles.Clear();
            PageViewsScreenLoaded.Clear();
        }

        /// <summary>
        /// Call this method to refresh embedded screens.
        /// </summary>
        public void Refresh()
        {
            UpdateHelpTextVisibility(Screens);
