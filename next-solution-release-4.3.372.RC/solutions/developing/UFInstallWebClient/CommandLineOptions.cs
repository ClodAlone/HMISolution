...
namespace UFInstallWebClient
{
    internal class CommandLineOptions
    {
        ...
        const string parReports = "/B";
        const string parUsers = "/U";
        const string parRoles = "/R";
        #endregion
        ...
        public static void Parse(string[] args, IISViewModel viewModel)
        {
            ...
            if (argTable.ContainsKey(parReports))
            {
                viewModel.ReportList = argTable[parReports].Split('|');
            }

            if (argTable.ContainsKey(parUsers))
            {
                viewModel.Users = argTable[parUsers].Split('|');
            }

            if (argTable.ContainsKey(parRoles))
            {
                viewModel.Roles = argTable[parRoles].Split('|');
            }
