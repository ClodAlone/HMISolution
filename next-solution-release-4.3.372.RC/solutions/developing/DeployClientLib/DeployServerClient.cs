...
namespace DeployClientLib
{
    public sealed class DeployServerClient: Observable, IDisposable
    {
        ...
        public async Task<BoolTaskResult> CopyFiles(String sourcepath, String destpath, string extensionsPattern = null)
        {
            var ret = new BoolTaskResult();
            ret.Success = false;
            if (!Active)
        ...
        public async Task<FileInfoSerializable> CreateProjectZipFile(String filePath)
        {
            if (!Active)
                return null;

            return await connection.InvokeAsync<FileInfoSerializable>("CreateProjectZipFile", filePath);
        }
        ...
        public async Task<List<string>> ScanRemoteProjects(String projectsPath)
        {
            ...
            return await connection.InvokeAsync<List<string>>("ScanRemoteProjects", projectsPath);
        }

        public async Task<byte[]> DownloadFileChunk(String filePath, int maxSize, int offset)
        {
            if (!Active)
                return null;

            return await connection.InvokeAsync<byte[]>("DownloadFileChunk", filePath, maxSize, offset);
        }
