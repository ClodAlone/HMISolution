...
namespace IpDriverCodeBaseEx
{
    public class TCPManager : IDisposable
    {
            ...
            lock (lockStream)
            {
                if (TcpChannelClient != null)
                {
                    try
                    {
                        ...
                        if (Properties.Settings.Default.TcpChannelSettingsDeviceCloseSleepTime > 0)
                            Thread.Sleep((int)Properties.Settings.Default.TcpChannelSettingsDeviceCloseSleepTime);
                    }
                    catch
                    {
        ...
        public bool IsBeginDeviceReadPending()
        {
            return doBeginDeviceRead;
        }
        #endregion
