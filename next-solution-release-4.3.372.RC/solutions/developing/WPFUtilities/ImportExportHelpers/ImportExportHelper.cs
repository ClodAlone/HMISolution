...
namespace WPFUtilities.ImportExportHelpers
{
    ...
    public class ImportExportHelper<T>
    {
        ...
        PropertyInfo extRefTagOwnerPath;
        ...
        public ImportExportHelper(ImportExportBaseOptions options)
        {
            ...
            if (!string.IsNullOrEmpty(assSubProtTagOwnerPath))
                extRefTagOwnerPath = (from p in properties where p.Name == assSubProtTagOwnerPath select p).FirstOrDefault();
        ...
        private IEnumerable<string> GetPropValues(T customObject, Dictionary<string, string> externalReferences)
        {
            ...
            string extRefPropTagOwnerPath = null;
                ...
                else
                {
                    ...
                    if (bUpdateExternalRef && externalPropertyReferences.Contains(p.Name) && !string.IsNullOrWhiteSpace(value))
                    {
                        ...
                        if(!bInitExtKeys)
                        {
                            ...
                            if (extRefTagOwnerPath != null)
                                extRefPropTagOwnerPath = extRefTagOwnerPath.GetValue(customObject, null)?.ToString();
                            bInitExtKeys = true;
                        }
                        ...
                        string guid = (bUseSubPrototype &&
                            !string.IsNullOrEmpty(extRefPropTagOwnerPath) &&
                            !string.IsNullOrEmpty(extRefKeyValue) &&
                            !string.IsNullOrEmpty(extRefPropValue)) ?
                            $"{extRefPropTagOwnerPath}_{extRefKeyValue}_{extRefPropValue}" :
                            (!string.IsNullOrEmpty(extRefKeyValue) ?
                            !string.IsNullOrEmpty(extRefPropValue) ?
                                $"{extRefKeyValue}_{extRefPropValue}" :
                                $"{extRefKeyValue}_{Guid.NewGuid().ToString()}" :
                ...
                try
                {
                    if (data.ContainsKey(propname))
                    {
                        ...
                        if(t == typeof(System.String))
                            safeValue = StripImportQuotes(safeValue);
        ...
        string quote = @"""";
        string doublequote = @"""""";
        object StripImportQuotes(object value)
        {
            if (value is String && !String.IsNullOrEmpty(value.ToString()))
            {
                string sVal = value.ToString();
                
                if(sVal.StartsWith(quote) && sVal.EndsWith(quote) && sVal.Contains(doublequote))
                {
                    sVal = sVal.Trim('"');
                    sVal = sVal.Replace(doublequote, quote);
                }
                return sVal;
            }
            return value;
