...
namespace WPFUtilities.Controls
{
    public class TimeSpanMaskManager : MaskManagerSelectAllEnhancer<TimeSpanMaskManagerCore> {
        public TimeSpanMaskManager(TimeSpanMaskManagerCore coreManager)
            : base(coreManager) { }
        public TimeSpanMaskManager(string mask, bool isOperatorMask, CultureInfo culture, bool allowNull, TimeSpan? minValue, TimeSpan? maxValue)
            : this(new TimeSpanMaskManagerCore(mask, isOperatorMask, culture, allowNull, minValue, maxValue)) { }
    ...
    public class TimeSpanMaskManagerCore : MaskManager {
        ...
        protected readonly TimeSpan? minValue;
        protected readonly TimeSpan? maxValue;

        public TimeSpanMaskManagerCore(string mask, bool isOperatorMask, CultureInfo culture, bool allowNull, TimeSpan? minValue, TimeSpan? maxValue) {
        ...
        public override bool SpinUp() {
            ...
            if (currentEditor == null)
                ...
                if (result && currentEditor.IsMinimum) {
                    ...
                    if (oldValue.HasValue && maxValue.HasValue && newValue > maxValue.Value)
        ...
        public override bool SpinDown() {
            ...
            if (currentEditor == null)
                ...
                if (result && currentEditor.IsMaximum) {
                    ...
                    if (oldValue.HasValue && minValue.HasValue && newValue < minValue.Value)
                        newValue = oldValue.Value;
                    ApplyNewValue(newValue);
                }
                return result;
        ...
        protected bool ApplyNewValue(TimeSpan newValue, bool raiseTextChanged = true) {
            ...
            if (minValue.HasValue && minValue.Value > newValue)
                newValue = minValue.Value;
            else if (maxValue.HasValue && maxValue.Value < newValue)
