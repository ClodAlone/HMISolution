...
#if !NET_STANDARD
        public bool UpdateMembershipUsersRole(List<string> errList = null)
        {
            Membership.ApplicationName = document.Parent.Title;

            var users = GetUserList();
            var roles = GetRoleList();
            var helper = new CredentialsProviderHelper(Membership.ApplicationName,
                new AuthenticationCredentialsProvider.ComponentService.AuthenticationCredentialsProviderComponent(), users, roles);
            ...
            if(errList != null && err.Count > 0)
                errList.AddRange(err);
            return err.Count == 0;
        }
#endif

        /// <summary>
        /// Method that move users, groups and settings configuration from file to DB
        /// </summary>
        /// <param name="connectionString"></param>
        /// <param name="copySettings">optional parameter to copy settings, default true</param>
        /// <param name="copyUsersAndGroups">optional parameter to copy users, default true</param>
        public UsersReturnCode MoveUsersToSharedRepository(string connectionString, bool copySettings = true, bool copyUsersAndGroups = true)
        {
            if (document == null)
                return UsersReturnCode.NoDocument;
            ...
            if (document != null)
            {
                try
                {
                    document.MoveUsersToSharedRepository(document, connectionString, copySettings, copyUsersAndGroups);
                    document.GetGeneralUserSettings().SharedConnectionRepository = connectionString;                    
