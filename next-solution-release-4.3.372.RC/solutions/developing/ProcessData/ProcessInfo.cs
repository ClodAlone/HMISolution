...
namespace DeployServer.Processes
{
    ...
    public class ProcessInfo : IEquatable<ProcessInfo>
    {
        ...
        [IgnoreDataMember]
        public bool volatileAutoRestart;

        internal int autoRestartsCounter;
        internal DateTime firstProcessStartDateTime;
        ...
        static public int MaxProcessAutorestarts = 3;
        static public TimeSpan ProcessAutorestartCounterLifetimeTimespan = TimeSpan.FromDays(1);
                ...
                processes.Values.SelectMany(x => x).ToList().ForEach(p =>
                {
                    try
                    {
                        ...
                        if (!bAlreadyRunning) {
                            ...
                            p.autoRestart = p.volatileAutoRestart = p.autoRestart && !CheckIsInDemo(p.fileName, p.commandLine);
        ...
        public static List<ProcessData> GetProcessesData()
        {
            ...
            lock (processes)
            {
                listToProcess.AddRange(processes.Values.SelectMany(x => x).Where(p => p.IsProcessInspectable()));
            }
        ...
        public static void Terminate()
        {
            ...
            lock (processes)
            {
                processes.Values.SelectMany(x => x).Where(p => p.IsProcessInspectable()).ToList().ForEach(p =>
                {
                    StopProcessInner(p, true);
                });
            }
        }
        ...
        async void UpdateProcessData()
        {
            ...
            if (bHasExited)
            {
                ...
                if (autoRestart && volatileAutoRestart && MaxProcessAutorestarts > 0 && status != ProcessStatus.Starting)
                {
                    try
                    {
                        process.Close();
                    ...
                    if (now - firstProcessStartDateTime > ProcessAutorestartCounterLifetimeTimespan)
                    {
//#if DEBUG
//                            Console.WriteLine("ProcessAutorestartCounterLifetimeTimespan exceeded from the first autorestart: resetting autorestarts counter. Elapsed seconds since the first autorestart: " + (now - firstProcessStartDateTime).TotalSeconds);
//#endif
                        firstProcessStartDateTime = now;
                        autoRestartsCounter = 0;
                    }
                    else if (autoRestartsCounter >= MaxProcessAutorestarts)
                    {
//#if DEBUG
//                            Console.WriteLine("MaxProcessAutorestarts exceeded: autorestart was canceled. Elapsed seconds since the first autorestart: " + (now - firstProcessStartDateTime).TotalSeconds);
//#endif
                        status = ProcessStatus.Stopped;
                        volatileAutoRestart = false;
                        return;
                    }
                    ...
                    {
//#if DEBUG
//                        Console.WriteLine("[[ Autorestarting occurred at: " + DateTime.Now.ToLongTimeString() + " ]]");
//#endif
                        autoRestartsCounter++;
        ...
        static void StartProcess(ProcessInfo p)
        {
            p.firstProcessStartDateTime = DateTime.Now;
            p.autoRestartsCounter = 0; 
            p.process.Start();
        ...
        public static bool AddProcess(String name, String filename, string commandline, bool autorestart, string documentTitle)
        {
            var p = new ProcessInfo()
            {
                ...
                volatileAutoRestart = autorestart,
            ...
            lock (processes)
            {
                ...
                processes[documentTitle].RemoveAll(item => item.Equals(p));
        ...
        static void StopProcessInner(ProcessInfo p, bool bForceKill)
        {
            try
            {
                if (RuntimeInformation.IsOSPlatform(OSPlatform.Windows))
        ...
        public bool Equals(ProcessInfo other)
        {
            ...
            return fileName == other.fileName && commandLine == other.commandLine;
        }
    }
}