...
namespace UFProjectManager.Controls
{
    ...
    public partial class DeployClient : UserControl
    {
            ...
            using (var cursor = new WaitCursor())
            {
                ...
                if (uiInterface.ShowYesNo(Properties.Resources.UploadProjectOnTargetMachine, CustomDialogIcons.Question) == CustomDialogResults.Yes)
                {
                    ...
                    else
                    {
                        ...
                        var ret = await deployServerClient.MoveDirectory(String.Format("{0}/{1}", projectMainFolder, DocumentManager.ComponentService.SpecialFolders.Images.ToString()), remoteImagesDirectory);
                        if (!ret.Success)
                            deployErrors = String.Format("{0}{1}{2}", deployErrors, Environment.NewLine, String.Format(Properties.Resources.WebHMIProjectImagesDeployFailed, ret.Message));

                        var bDocTransferSuccess = await deployServerClient.CopyFiles(String.Format("{0}/{1}", projectMainFolder, DocumentManager.ComponentService.SpecialFolders.Documents.ToString()), remoteDocumentsDirectory, Properties.Settings.Default.WebHMIWwwrootDocumentsExtensionsPattern);
                        
                        if (!bDocTransferSuccess)
                            deployErrors = String.Format("{0}{1}{2}", deployErrors, Environment.NewLine, String.Format(Properties.Resources.WebHMIProjectDocumentsDeployFailed, ret.Message));
                    }
        ...
        void PersistProfiles(bool bSilent = false)
        {
            try
            {
                var profilesFile = Path.Combine(Utilities.ApplicationPropertiesHelper.GetProperty("CommonFolder").ToString(), Properties.Settings.Default.DeployProfilesFileName);
