...
namespace UFUAModel
{
...
#endif
    {
        ...
        public String AlarmOptions
        {
            get
            {
                string alarmOptions = $"{Expression}#{AddTagDescription}#{Enabled}#{EnableTag}#{EnableQualityGood}#{ActivationLowValueTag}#{ActivationValueTag}#{Beep}#{Oid}#{HighHighLimitTag}#{HighLimitTag}#{LowLimitTag}#{LowLowLimitTag}";
        ...
        public string GetAlarmMessage(String startName)
        {
            ...
            var alarmTextBuilder = new System.Text.StringBuilder();
            if (AddTagDescription && UFUATagAss != null && !String.IsNullOrEmpty(UFUATagAss.Description))
                alarmTextBuilder.Append(UFUATagAss.Description);
            
            if (alarmTextBuilder.Length == 0)
                alarmTextBuilder.Append(String.IsNullOrEmpty(AlarmText) ? suffix.ToString() : AlarmText);
            else if (!String.IsNullOrEmpty(AlarmText))
                alarmTextBuilder.AppendFormat(" - {0}", AlarmText);

            return alarmTextBuilder.ToString(); ;
        }
