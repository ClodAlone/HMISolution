    ...
    public class SNMPDriver : CommunicationDriver
    {
        ...
        private void BuildSNMPChannelDictionary()
        {
            ...
            foreach (var channel in channelList)
            {
                ...
                if(snmpChannel != null && !String.IsNullOrEmpty(snmpChannel.UdpChannelHostName))
                {
                    IPAddress objIpAddress = new IPAddress(0);
                    if (UdpChannel.GetResolvedConnecionIPAddress(snmpChannel.UdpChannelHostName, out objIpAddress))
                    {
                        string ipAddress = objIpAddress.ToString();
                        if(!String.IsNullOrWhiteSpace(ipAddress))
                        {
                            ChannelsByIPAddress[ipAddress] = snmpChannel;
                        }
                    }
                }
            }
