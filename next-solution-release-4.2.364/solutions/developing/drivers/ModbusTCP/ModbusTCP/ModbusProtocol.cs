...
namespace ModbusTCP
{
    ...
    public enum ModbusErrorCodes : int
    {
        ...
        ErrorStationID,
        ErrorFunctionCode,
        ErrorProtocolIllegalFunction = ModbusProtocol.PROTOCOL_ERROR + 1,
        ErrorProtocolIllegalDataAddress = ModbusProtocol.PROTOCOL_ERROR + 2,
        ErrorProtocolIllegalDataValue = ModbusProtocol.PROTOCOL_ERROR + 3,
        ErrorProtocolSlaveDeviceFailure = ModbusProtocol.PROTOCOL_ERROR + 4,
        ErrorProtocolAcknowledge = ModbusProtocol.PROTOCOL_ERROR + 5,
        ErrorProtocolSlaveDeviceBusy = ModbusProtocol.PROTOCOL_ERROR + 6,
        ErrorProtocolMemoryParityError = ModbusProtocol.PROTOCOL_ERROR + 8,
        ErrorProtocolGatewayPathUnavailable = ModbusProtocol.PROTOCOL_ERROR + 10,
        ErrorProtocolGatewayTargetFailResponse = ModbusProtocol.PROTOCOL_ERROR + 11
    }
    ...
    public class ModbusProtocol
    {
        ...
        public uint GetFrameLength(ModbusTCPCommJob job)
        {
            if (job.ReadRequest())
        ...
        public uint PrepareRequest(ModbusTCPCommJob job, ref byte[] buffer, out bool isBroadcast)
        {
            isBroadcast = false;
            if (job == null || (job.Station as ModbusTCPStation == null))
                return 0;
            uint WriteCh = 0;
            ...
            if (job.ReadRequest())
            {
                ...
                buffer[WriteCh++] = (byte)((ModbusTCPStation)job.Station).StationID;
...
//#if DEBUG
//                    byte[] jobDataArray = (byte[])jobData;
//                    string currentTime = DateTime.Now.ToString("HH:mm:ss.fff");
//                    string stringArrayData = String.Empty;
//                    foreach(var byteVal in jobDataArray)
//                    {
//                        stringArrayData += String.Format("[{0}]", byteVal);
//                    }
//                    System.Diagnostics.Debug.WriteLine(String.Format("MDBG- {0} - PrepareRequest - jobData size: {1} - listOnWriting size: {2}, jobData: {3}, listOnWriting[0].Offset: {4}",
//                                                       currentTime, jobDataArray.Count(), listOnWriting.Count(), stringArrayData, listOnWriting[0].ByteOffset));
...
//#endif
                ...
                if (job.SwapDWords)
                    ModbusTCPCommJob.SwapDWordBuffer(ref writeData);
        ...
        static int ChecKAnswer(byte[] receivebuffer, ModbusTCPCommJob job)
        {
            if (receivebuffer[0] != (byte)((ModbusTCPStation)job.Station).StationID)
            {
        ...
        /*
        public static void ParseDataToArguments(byte[] receivebuffer, ModbusTCPCommJob job, ref List<object> args)
        {
            if (args.Count == 0)
                return;

            BuiltInType bt = job.Station.GetBuiltInType(args[0].GetType());
            ...
            if (args.Count < 2)
            {
                ...
                args[0] = 11;//ModbusErrorCodes.;
                return;

            }
            args[0] = ChecKAnswer(receivebuffer, job);
                ...
                case 7://Read Exception Status
                    {
                        ...
                        List<Tag> ttag = new List<Tag>();
                        job.SetJobData(jobdata, ref ttag);
                        if (job.TagsList.Count == args.Count - 1)
                        {
                            for (int i = 0; i < job.TagsList.Count; i++)
                            {
                                args[i + 1] = job.TagsList[i].Value.Value;
                            }
                        }
                    }
                    break;
        ...
        public static bool ParseData(byte[] receivebuffer, ref ModbusTCPCommJob job, ref List<object> items)
        {
            ...
            switch (receivebuffer[1])
            {
                ...
                case 7://Read Exception Status
                    {
                        //System.Diagnostics.Trace.TraceInformation("{0} - Read: ", DateTime.UtcNow.ToString("hh:mm:ss:ffff"));
                        byte[] jobdata = new byte[bytenumber];
                        receivebuffer.ToList().CopyTo(3, jobdata, 0, bytenumber);
