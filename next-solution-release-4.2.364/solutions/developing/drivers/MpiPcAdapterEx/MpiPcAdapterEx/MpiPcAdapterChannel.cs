        ...
        */
        ...
        public bool InitPlcConnection(MpiPcAdapterStation s)
        {
            if (!s.ConnEstablished)
            {
                ...
                //System.Diagnostics.Debug.WriteLine($@"-> {DateTime.Now.ToString("mm:ss.fff")} : GestCreateConnection 1 Send");
                if (GestCreateConnection(s, 1))
                    if (!GestResponse(MpiResponses.MPI_CREATECONN1_RESP, s))
                    {
                        //System.Diagnostics.Debug.WriteLine($@"-> {DateTime.Now.ToString("mm:ss.fff")} : GestCreateConnection 1 Failed");
                        CloseConnection(s);
                    }
                    else
                    {
                        //System.Diagnostics.Debug.WriteLine($@"-> {DateTime.Now.ToString("mm:ss.fff")} : GestCreateConnection 1 Ok");
                        ...
                        if (GestCreateConnection(s, 2))
                            if (!GestResponse(MpiResponses.MPI_CREATECONN2_RESP, s))
                                ...
                                {
                                    //System.Diagnostics.Debug.WriteLine($@"-> {DateTime.Now.ToString("mm:ss.fff")} : GestCreateConnection 1 - 2 - 3 Ok");
                                    s.ConnEstablished = true;
                                }
                    }
        ...
        {
            //System.Diagnostics.Debug.WriteLine($@"-> {DateTime.Now.ToString("mm:ss.fff")} : CloseConnection - ClosePlcConnection(s) init ");
            if (!ClosePlcConnection(s))
            {
               // System.Diagnostics.Debug.WriteLine($@"-> {DateTime.Now.ToString("mm:ss.fff")} : CloseConnection - ClosePlcConnection(s) failed ");
                CloseMpiAdapter();
            }
            else if (ConnCounter == 0)
            {
                //System.Diagnostics.Debug.WriteLine($@"-> {DateTime.Now.ToString("mm:ss.fff")} : CloseConnection - (ConnCounter == 0)  ClosePlcConnection() Ok and inti  CloseMpiAdapter");
                CloseMpiAdapter();
            }
           // System.Diagnostics.Debug.WriteLine($@"-> {DateTime.Now.ToString("mm:ss.fff")} : CloseConnection  end ");
        }
        ...
        public override bool DeviceOpen()
        {
            ...
            //Waiting time needed to restore the MPIAdapter cable
            Thread.Sleep(Properties.Settings.Default.WaitingForResetMpiPcAdapterCable);
            return true;
        }
