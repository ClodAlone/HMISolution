...
{
    public sealed class MpiPcAdapterTag : Tag
    {
        #region Constructors

        public MpiPcAdapterTag(TagDefinition tag, uint byteoffset, uint bitoffset)
            : base(tag, byteoffset, bitoffset)
        {

        }

        public MpiPcAdapterTag(TagDefinition tag)
            : base(tag)
        {
        ...
        {
            get { return (DynTagSettings)MpiPcAdapterDynSettings; }
        }
                ...
                (uint)TagNode.DataType.Identifier == (uint)Opc.Ua.DataTypes.Boolean)
            {
                ...
                if (buffer != null && buffer.Length >= size)
                {
                    ...
                    else
                    {
                        ...
                        //    }
                        //}
                        size = (TagNode.ArrayDimension + 7) / 8;
                    }
                }
            }
            else
        ...
        private readonly MpiPcAdapterDynTagSettings _MpiPcAdapterDynSettings = new MpiPcAdapterDynTagSettings();
        public MpiPcAdapterDynTagSettings MpiPcAdapterDynSettings
        {
            get { return _MpiPcAdapterDynSettings; }
        }
