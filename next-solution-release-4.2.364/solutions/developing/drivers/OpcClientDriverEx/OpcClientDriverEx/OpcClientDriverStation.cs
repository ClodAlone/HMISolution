        ...
        public void ExecuteSyncroJob(CommJob job)
        {
            ...
            if (!read)
            {
                ...
                else
                {
                    lock (lockListObject)
                    {
                        jobDataNodeId = ((Tag)jobData).TagNode.NodeId;
                        jobData = ((Tag)jobData).Value.Value;
                    }
                }
            }
                    ...
                    else
                    {
                        if (j.IsExtensionObject)
                        {
                            ...
                            foreach (Tag tag in j.TagsListOnWriting)
                            {
                                ...
                                tagnodevalues.Add(tag.Value.Value);
                                lock (lockListObject)
                        ...
                        {
                            jobs.Add(j);
                            values.Add(j.TagsList[0].Value.Value);
                            lock (lockListObject)
