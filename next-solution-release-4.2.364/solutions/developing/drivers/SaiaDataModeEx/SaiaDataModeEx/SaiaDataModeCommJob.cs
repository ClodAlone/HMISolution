...
namespace SaiaDataMode
{
    public class SaiaDataModeCommJob : CommJob
    {
        ...
        public ushort GetWriteAddres()
        {
            if (IsBitDataFormat())
            {
                if ((uint)TagsListOnWriting[0].TagNode.DataType.Identifier == (uint)BuiltInType.Boolean ||
                    ElementNumber != 0)
                {
                    return (ushort)(StartAddress.USHORT + TagsListOnWriting[0].ByteOffset);
                }
                else
                {
                    return (ushort)(StartAddress.USHORT + TagsListOnWriting[0].ByteOffset * 8);
                }
            }
            else
            {
                return (ushort)(StartAddress.USHORT + TagsListOnWriting[0].ByteOffset / 4);
            }
        }
        ...
        {
            byte ArrayDimension = (byte)(TagsListOnWriting[0].TagNode.ArrayDimension == 0 ? 1 : TagsListOnWriting[0].TagNode.ArrayDimension);
            if ((uint)TagsListOnWriting[0].TagNode.DataType.Identifier == (uint)BuiltInType.Boolean ||
                ElementNumber > 0)
            {
                return (byte)(TagsListOnWriting.Count * ArrayDimension - 1);
            }
            else
            {
                return (byte)(TagsListOnWriting.Count * GetDataTypeBitSize((uint)TagsListOnWriting[0].TagNode.DataType.Identifier) * ArrayDimension - 1);
            }
        }
