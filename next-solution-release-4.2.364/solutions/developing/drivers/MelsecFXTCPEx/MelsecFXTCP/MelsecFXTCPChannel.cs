        ...
        public override bool ProcessNewData(DriverErrorCodes conn, CommJob pendingjob)
        {
            ...
            lock (lockThreadObject)
            {
                   ...
                   (subHeader > (byte)(0x80 + MelsecFXCommandCode.Command_BatchWrite_WordUnits)))
                {
                    ...
                    eAJob.ErrorCode = (DriverCodeBaseEx.Enumerators.DriverErrorCodes)MelsecFXErrorCodes.ErrorCodeUnknownSubHeader;
                    ReceiveBuffer.Clear();
                    Flush();
                ...
                {
                    ExecutedJobArgs eAJob = new ExecutedJobArgs { ErrorCode = LastErrorCode, Job = pendingjob };
                    eAJob.ErrorCode = (DriverCodeBaseEx.Enumerators.DriverErrorCodes)MelsecFXErrorCodes.ErrorCodeMismatchSubheader;
                    ReceiveBuffer.Clear();
                    Flush();
                    eAJob.Job = pendingjob;
                    OnJobExecuted(eAJob);
                ...
                switch (completeCode)
                {
                    ...
                    case 0x50:
                    {
                        ...
                        eAJob.ErrorCode = (DriverCodeBaseEx.Enumerators.DriverErrorCodes)MelsecFXErrorCodes.ErrorCodeInvalidCommand;
                        ReceiveBuffer.Clear();
                        Flush();
                        eAJob.Job = pendingjob;
                        OnJobExecuted(eAJob);
                    ...
                    {
                        ExecutedJobArgs eAJob = new ExecutedJobArgs { ErrorCode = conn, Job = pendingjob };
                        eAJob.ErrorCode = (DriverCodeBaseEx.Enumerators.DriverErrorCodes)MelsecFXErrorCodes.ErrorCodeIncorrectDeviceDesignation;
                        ReceiveBuffer.Clear();
                        Flush();
                        eAJob.Job = pendingjob;
                        OnJobExecuted(eAJob);
                    ...
                    {
                        ExecutedJobArgs eAJob = new ExecutedJobArgs { ErrorCode = conn, Job = pendingjob };
                        eAJob.ErrorCode = (DriverCodeBaseEx.Enumerators.DriverErrorCodes)MelsecFXErrorCodes.ErrorCodeExceedingAddOrNumOfPoints;
                        ReceiveBuffer.Clear();
                        Flush();
                        eAJob.Job = pendingjob;
                        OnJobExecuted(eAJob);
                    ...
                    {
                        ExecutedJobArgs eAJob = new ExecutedJobArgs { ErrorCode = conn, Job = pendingjob };
                        eAJob.ErrorCode = (DriverCodeBaseEx.Enumerators.DriverErrorCodes)MelsecFXErrorCodes.ErrorCodeWrongHeadDeviceNum;
                        ReceiveBuffer.Clear();
                        Flush();
                        eAJob.Job = pendingjob;
                        OnJobExecuted(eAJob);
                    ...
                    case 0x5B:
                    {
                        ...
                        switch (abnormalCode)
                        {
                            // Wrong PC number
                            case 0x10:
                                eAJob.ErrorCode = (DriverCodeBaseEx.Enumerators.DriverErrorCodes)MelsecFXErrorCodes.ErrorCodePCNumberError;
                            break;

                            // Mode error
                            case 0x11:
                                eAJob.ErrorCode = (DriverCodeBaseEx.Enumerators.DriverErrorCodes)MelsecFXErrorCodes.ErrorCodeModeError;
                            break;

                            // Remote error
                            case 0x18:
                                eAJob.ErrorCode = (DriverCodeBaseEx.Enumerators.DriverErrorCodes)MelsecFXErrorCodes.ErrorCodeRemoteError;
                            break;

                            // Unrecognized abnormal code
                            default:
                                eAJob.ErrorCode = (DriverCodeBaseEx.Enumerators.DriverErrorCodes)MelsecFXErrorCodes.ErrorCodeAbnormalCode;
                            break;
                        }
                    ...
                    {
                        ExecutedJobArgs eAJob = new ExecutedJobArgs { ErrorCode = conn, Job = pendingjob };
                        eAJob.ErrorCode = (DriverCodeBaseEx.Enumerators.DriverErrorCodes)MelsecFXErrorCodes.ErrorCodeMonitoringTimerExceeded;
                        ReceiveBuffer.Clear();
                        Flush();
                        eAJob.Job = pendingjob;
                        OnJobExecuted(eAJob);
                    ...
                    {
                        ExecutedJobArgs eAJob = new ExecutedJobArgs { ErrorCode = conn, Job = pendingjob };
                        eAJob.ErrorCode = (DriverCodeBaseEx.Enumerators.DriverErrorCodes)MelsecFXErrorCodes.ErrorCodeGenericError;
                        ReceiveBuffer.Clear();
                        Flush();
                        eAJob.Job = pendingjob;
                        OnJobExecuted(eAJob);
                ...
                {
                    ExecutedJobArgs eAJob = new ExecutedJobArgs { ErrorCode = conn, Job = pendingjob };
                    eAJob.ErrorCode = (DriverCodeBaseEx.Enumerators.DriverErrorCodes)MelsecFXErrorCodes.ErrorCodeIncompleteFrame;
                    ReceiveBuffer.Clear();
                    Flush();
                    eAJob.Job = pendingjob;
                    OnJobExecuted(eAJob);
