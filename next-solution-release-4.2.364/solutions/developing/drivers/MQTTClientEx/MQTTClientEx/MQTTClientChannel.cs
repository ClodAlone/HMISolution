    ...
    class MQTTClientChannel : ChannelList
    {
        ...
        private void StartTmrCheckDeviceCyclically()
        {
            lock (lockThreadObject)
            {
                ...
                {
                    _TmrCheckConnectionCyclically = new Timer(CheckDeviceCyclically, this, _MQTTClientKeepAliveTime*1000, System.Threading.Timeout.Infinite);
                }
            }
        }
        ...
        private bool StopTmrCheckDeviceCyclically(int maxWaitTime)
        {
            WaitHandle waitHandle = null;
            lock (lockThreadObject)
            {
            ...
            if (waitHandle != null)
            {
                if( maxWaitTime > 0)
                {
                    ...
                    {
                        string currentTime = DateTime.Now.ToString("HH:mm:ss.fff");
                        System.Diagnostics.Debug.WriteLine(String.Format("MQTTClient DBG - {0} - StopTmrCheckDeviceCyclically before waitHandle.WaitOne(maxWaitTime) - maxWaitTime: {1}",
                                                           currentTime, maxWaitTime));
                    }
#endif
                    stopped = waitHandle.WaitOne(maxWaitTime);
#if DEBUG
                    {
                        string currentTime = DateTime.Now.ToString("HH:mm:ss.fff");
                        System.Diagnostics.Debug.WriteLine(String.Format("MQTTClient DBG - {0} - StopTmrCheckDeviceCyclically after waitHandle.WaitOne(maxWaitTime) - stopped: {1}",
                                                           currentTime, stopped));
                    }
#endif
                }
        ...
        {
            if(StopTmrCheckDeviceCyclically(Timeout) == true)
            {
        ...
        protected override void StopTimers(bool bTerminate = true)
        {
            ...
            lock (lockThreadObject)
            {
                if (bTerminate && allRepliesReceived != null)
                {
                    allRepliesReceived.Set();
                }
            }
