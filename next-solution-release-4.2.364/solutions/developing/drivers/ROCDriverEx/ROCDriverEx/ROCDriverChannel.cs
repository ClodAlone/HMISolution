...
namespace ROCDriver
{
    public class ROCDriverChannel : ChannelList, IDisposable
    {
        ...
        private void StartTmrCheckDeviceCyclically()
        {
            ...
            {
                if ((_TmrCheckConnectionCyclically == null) && (CheckConnectionFrequency > 0))
                    // To be more precise, use the half frequency of check as due time and period of the timer 
                    _TmrCheckConnectionCyclically = new Timer(CheckDeviceCyclically, this, CheckConnectionFrequency * 500, CheckConnectionFrequency * 500); // System.Threading.Timeout.Infinite);
            }
        }
            ...
            try
            {
                ...
                if (stationsToCheck == null || stationsToCheck.Count == 0)
                {
#if DEBUG
                    {
                        ...
                        System.Diagnostics.Debug.WriteLine(String.Format("ROC DBG - {0} - CheckDeviceCyclically - No stations to be checked",
                                                           currentTime));
                    }
#endif
                    return;
