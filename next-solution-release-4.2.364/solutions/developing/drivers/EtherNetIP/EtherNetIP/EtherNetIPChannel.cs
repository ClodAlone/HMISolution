...
{
    public class EtherNetIPChannel : TcpChannel, IDisposable
    {
        //InputOutput write job management with movicon data type smaller than device
        private enum IOSmallBigJobSchedule {
            JobNotScheduled,            
            JobAddOnlyAndExit,
            JobAddRemoveAndExit,
            JobAddAndRemove,
        ...
        List<EtherNetIPCommJob> dequeueListJob = new List<EtherNetIPCommJob>();
            ...
            while (true)
            {
                ...
                if (ListJobPending.Count == 0)
                {
                    ...
                    if (nextlist.Count > 0)
                    {
                        ...
                        if (isOpen)
                        {
                            EtherNetIPStation eipStation = nextlist[0].Station as EtherNetIPStation;
                            if((nextlist[0].AddressType == AddressTypes.TagName) && (eipStation.ConnectionIDSet != true))
                            {
                            ...
                            if((nextlist[0].AddressType != AddressTypes.TagName) || ((nextlist[0].AddressType == AddressTypes.TagName) && (eipStation.ConnectionIDSet == true)))
                            {
                                lock (lockThreadObject)//201011
                                {
                                    //dbgCount = 0;
                ...
                lock (lockThreadObject)
                {
                    ...
                    {
                        //System.Diagnostics.Trace.TraceInformation("WTh NewData:{0} #data:{1} :{2}", bNew, ReceiveBuffer.Count, DateTime.UtcNow.ToString());
                        ...
                        if (ListJobPending.Count > 0)
                        {
                            ...
                            {
                                ReceiveBuffer.Clear();
                                //System.Diagnostics.Trace.TraceInformation("0 ReceiveBuffer.Clear()");
                                foreach (EtherNetIPCommJob job in ListJobPending)
                                {
                                    //System.Diagnostics.Trace.TraceInformation("ProcessNewData {0} :{1} :{2}", job.GroupString, job.LastExecutionTime.ToString(), DateTime.UtcNow.ToString());
                                    ListJobExecuted.Add(job);
                                }
                            }
                            foreach (EtherNetIPCommJob job in ListJobExecuted)
                            {
        ...
        protected override void OnJobExecuted(ExecutedJobArgs e)
        {
            ...
            if (!j.TerminateTask)
            {
                ...
                //{
                //    byte[] b = (byte[])e.Values;
                //    System.Diagnostics.Debug.WriteLine(string.Format("Ethernet IP job executed ReadForRW {0} for tag {1} EN {2}", j.RWState, j.TagsList[0].TagNode.Name, j.ElementNumber));
                //}
        ...
        #region methods
        protected void GetNextPendingList(ref List<EtherNetIPCommJob> inList)
        {
            bool write = false;
            ...
            inList.Clear();
            List<EtherNetIPCommJob> renoveListJob = new List<EtherNetIPCommJob>();
            foreach (EtherNetIPCommJob j in dequeueListJob)
            {
                if (j.ReadTagEnd == 0 && j.PartialArrayEnd == 0)
                    renoveListJob.Add(j);
                //System.Diagnostics.Trace.TraceInformation("--DEBUG-- EthernetIpChannel  GetNextPendingList PartialArrayEnd:{0} PartialArrayStart:{1}   :{2}", j.PartialArrayEnd, j.PartialArrayStart, DateTime.UtcNow.ToString());
            }

            foreach (EtherNetIPCommJob j in renoveListJob)
            {
            ...
            if (dequeueListJob.Count() == 0)
            {
                lock (lockScheduleFlag)
                {
                    ...
                    int jobIndex = 0;
                    while (jobIndex < queue.Count)
                    {
                        bool jobScheduled = false;
                        CommJob dequeuedJob = null;
                        EtherNetIPCommJob j = queue.ElementAt(jobIndex) as EtherNetIPCommJob;

                        #region InputOut Job with Movicon data type smaller than plc data type --> force 1 read cycle, and than write
                        IOSmallBigJobSchedule JobAddMode = IOSmallBigJobSchedule.JobNotScheduled;
                        //for InputOutput jobs with movicon data type smaller than plc data type, before write, read it --> only 1 job at time
                        if (j.Type == LinkType.InputOutput && (j.GetDataTypeByteSize((uint)j.TagsList[0].TagNode.DataType.Identifier) < j.GetProtocolDataByteSize()) && j.TagsListToWrite.Count > 0)
                        {
                            // no jobs added to the list
                            if (dequeueListJob.Count == 0)
                            {
                                // first force to execute as a read Job --> don't dequeue from jobs list 
                                if (j.RWState == CommJob.RWStates.Standard)
                                {
                                    j.RWState = CommJob.RWStates.ReadForRW;
                                    //queue.Enqueue(j);
                                    JobAddMode = IOSmallBigJobSchedule.JobAddOnlyAndExit;
                                    //System.Diagnostics.Debug.WriteLine(string.Format("Ethernet IP SpliJobs assigne ReadForRW for tag {0} EN {1}", j.TagsList[0].TagNode.Name, j.ElementNumber));
                                }
                                else
                                {
                                    // then, execute as a write job --> and after remove from list
                                    j.RWState = CommJob.RWStates.WriteForRW;
                                    JobAddMode = IOSmallBigJobSchedule.JobAddRemoveAndExit;
                                    //System.Diagnostics.Debug.WriteLine(string.Format("Ethernet IP SpliJobs assigne WriteForRW for tag {0} EN {1}", j.TagsList[0].TagNode.Name, j.ElementNumber));
                                }
                            } else
                            {
                                //exit and wait until previous job are executed
                                break;
                            }
                        }
                        else // manage as usual
                        {
                            JobAddMode = IOSmallBigJobSchedule.JobAddAndRemove;
                        }
                        #endregion

                        //if (j != null && (j.IsPending == false) && !j.ExecuteTask && (j.ReadTagStart == 0) && (j.ReadTagEnd == 0)) Case 20074 It does not write tag
                        if (j != null && (j.IsPending == false) && (j.ReadTagStart == 0) && (j.ReadTagEnd == 0)) {
                            if (j.AddressType == AddressTypes.DataFile || (j.Station as EtherNetIPStation).PlcType == PlcTypes.Micro800_series) // for DataFile and Micro800_series only one job
                            {
                                if (!j.ReadRequest())
                                {
                                    write = true;
                        ...
                        if (jobScheduled)
                        {
                            // add job without dequeue --> write job force to be executed as read job
                            if (JobAddMode == IOSmallBigJobSchedule.JobAddOnlyAndExit)
                                break;

                            // execute only 1 write job (before executed as read)
                            if (JobAddMode == IOSmallBigJobSchedule.JobAddRemoveAndExit) { 
                                queue.TryDequeue(out dequeuedJob);
                                break;
                            }
                            ...
                            // standard job
                            if (JobAddMode == IOSmallBigJobSchedule.JobAddAndRemove)
                                queue.TryDequeue(out dequeuedJob);
                        }
                        else
                        {
                            // move to next queue's job
                            jobIndex++;
                        }
                    }
        ...
        ///// <summary>
        ///// Aggregate write jobs 
        ///// </summary>
        ///// <param name="inList"></param>
        ///// <returns></returns>
        //private ConcurrentQueue<CommJob> AggregateDequeueListJob(ref ConcurrentQueue<CommJob> inList)
        //{
        //    ConcurrentQueue<CommJob> Jobs = new ConcurrentQueue<CommJob>();
        //    CommJob j;

        //    while (inList.Count>0)
        //    {                
        //        inList.TryPeek(out j);
        ...
        //                    j.RWState = CommJob.RWStates.WriteForRW;
        //                    CommJob dequeuedJob = null;
        //                    inList.TryDequeue(out dequeuedJob);
        //                    if (dequeuedJob != null)
        //                        Jobs.Enqueue(dequeuedJob);
        ...
        private void GetNextSynchroJob(ref List<EtherNetIPCommJob> inList)
        {
            EtherNetIPCommJob job = SynchroJob as EtherNetIPCommJob;
            ...
            if (job.AddressType == AddressTypes.TagName && job.answer == null)
                job.answer = new byte[job.TotalJobSize];

            inList.Add(job);
        }

        private List<EtherNetIPCommJob> processDequeueListJob(List<EtherNetIPCommJob> inList, bool write, ConcurrentQueue<CommJob> queue)
        {
            if (dequeueListJob[0].AddressType != AddressTypes.DataFile)
            {
                if (write)
                {
                    List<EtherNetIPCommJob> queueEmptyJobs = new List<EtherNetIPCommJob>();
                    EtherNetIpProtocol.getWriteListLimitate(dequeueListJob, ref inList, ref queueEmptyJobs);
                    if (queueEmptyJobs.Count > 0)
                    {
                        foreach (EtherNetIPCommJob j in queueEmptyJobs) {
                            ExecutedJobArgs eJob = new ExecutedJobArgs { ErrorCode = DriverErrorCodes.ErrorNoError, Job = j };                            

                            //System.Diagnostics.Debug.WriteLine(string.Format("processDequeueListJob CleanUp Job with tagWriteCount == 0 {0}", j.TagsList[0].DynSettings.ToString()));
                            OnJobExecuted(eJob);
                    ...
                    EtherNetIpProtocol.getReadListLimitate(dequeueListJob, ref inList);
            }
            else
                inList.Add(dequeueListJob[0]);

            foreach (EtherNetIPCommJob j in inList)
            {
                //j.ExecuteTask = true; Case 20074 It does not write tag
                j.TerminateTask = false;
                ...
                else
                {
                    ...
                    if (j.AddressType == AddressTypes.TagName && j.answer == null)
        ...
        protected void ExecuteJobList(ref List<EtherNetIPCommJob> list)
        {
            ...
            switch (((EtherNetIPCommJob)list[0]).AddressType)
            {
                case AddressTypes.DataFile:
                    ...
                    {
                        list[0].MoveTagsFromListOnWritingToListToWrite();
                        RemovePendingJob(list[0]);
                        list[0].IsPending = false;
                        return;
                    }
                ...
                case AddressTypes.TagName:
                    if (EtherNetIpProtocol.Logix5550PrepareRequest(ref list, this, ref s, ref requestBuffer, ref requestBufferPointer) == 0)
                    {
                        foreach (EtherNetIPCommJob j in list)
                        {
                            j.MoveTagsFromListOnWritingToListToWrite();
                            RemovePendingJob(j);
                            j.IsPending = false;
                        }
                        return;
            ...
            //System.Diagnostics.Trace.TraceInformation(string.Format("{0} ExecuteJobList ends", DateTime.Now.ToLongTimeString()));
        }
        ...
        bool ProcessNewDataList(List<CommJob> list)
        {
            ...
            if (LastErrorCode != (int)DriverErrorCodes.ErrorNoError)
            {
                ...
                LastErrorCode = DriverErrorCodes.ErrorNoError;
                ReceiveBuffer.Clear();
                //System.Diagnostics.Trace.TraceInformation("3 ReceiveBuffer.Clear()");
                Flush();
                return true;
            }
                ...
                lock (lockList)
                {
                    ...
                    if (s != null)
                    {
                        ...
                        if (ReceiveBuffer.Count >= EtherNetIpProtocol.ENCAPSULATION_HEADER_SIZE)
                        {
                            ...
                            ushortUnion dim = new ushortUnion(ReceiveBuffer, EtherNetIpProtocol.EDATA_LEN_OFFS);
                            if (dim.USHORT != 0)
                            {
                                byte[] pdu = new byte[EtherNetIpProtocol.ENCAPSULATION_HEADER_SIZE + dim.USHORT];
        ...
        void GetReadData(ref byte[] pdu,List<CommJob> list, EtherNetIPStation s)
        {
            ...
            byte[] pduLocal = pdu;
            List<ExecutedJobArgs> LocalListExecutedJob = new List<ExecutedJobArgs>();
            ...
            Parallel.ForEach(list, (jComm, state, index) => 
                {
                    ...
                    readJobTag = 0;
                    outErrorCode = copyTagData(j.TagsList[0], pduLocal, (ushort)(totalPduTag + tagIndex - j.ReadTagStart), j, out GeneralStatusError, out ExtendedError);
            ...
            //foreach (var ( jb ,  index) in list.Select((v, i) => (v, i)))
            //{
            //    byte GeneralStatusError = 0;
            //    readJobTag = 0;
            //    EtherNetIPCommJob j = (EtherNetIPCommJob)jb;
            //    ushort tagIndex = (ushort)index;
            ...
            //    //        outErrorCode = copyTagData(tag, ref pdu, (ushort)(totalPduTag + tagIndex - j.ReadTagStart), j, out GeneralStatusError, out  ExtendedError);
            //    //    }
            //    //    readJobTag++;
            //    //}
            //    outErrorCode = copyTagData(j.TagsList[0], ref pdu, (ushort)(totalPduTag + tagIndex - j.ReadTagStart), j, out GeneralStatusError, out ExtendedError);
            //    totalPduTag += readJobTag;
            //    ExecutedJobArgs eJob;
            //    if (outErrorCode == DriverErrorCodes.ErrorNoError)
            //    {
        ...
        void GetReadDataMicro800(ref byte[] pdu, List<CommJob> list, EtherNetIPStation s)
        {
            ...
            //long array case --> close job when alla data was readed
        ...
        {
            EtherNetIP.Properties.Resources.ErrorSTSCode01,
            EtherNetIP.Properties.Resources.ErrorSTSCode02,
            EtherNetIP.Properties.Resources.ErrorSTSCode03,
            EtherNetIP.Properties.Resources.ErrorSTSCode04,
            EtherNetIP.Properties.Resources.ErrorSTSCode05,
            EtherNetIP.Properties.Resources.ErrorSTSCode06,
            EtherNetIP.Properties.Resources.ErrorSTSCode07,
            EtherNetIP.Properties.Resources.ErrorSTSCode08,
            EtherNetIP.Properties.Resources.ErrorSTSCode09Spare,
        };

        protected string[] PlcResponseErrorList2 = 
        {
            EtherNetIP.Properties.Resources.ErrorSTSCode10,
            EtherNetIP.Properties.Resources.ErrorSTSCode20,
            EtherNetIP.Properties.Resources.ErrorSTSCode30,
            EtherNetIP.Properties.Resources.ErrorSTSCode40,
            EtherNetIP.Properties.Resources.ErrorSTSCode50,
            EtherNetIP.Properties.Resources.ErrorSTSCode60,
            EtherNetIP.Properties.Resources.ErrorSTSCode70,
            EtherNetIP.Properties.Resources.ErrorSTSCode80,
            EtherNetIP.Properties.Resources.ErrorSTSCode90,
            EtherNetIP.Properties.Resources.ErrorSTSCodeA0,
            EtherNetIP.Properties.Resources.ErrorSTSCodeB0,
            EtherNetIP.Properties.Resources.ErrorSTSCodeC0,
            EtherNetIP.Properties.Resources.ErrorSTSCodeD0,
            EtherNetIP.Properties.Resources.ErrorSTSCodeE0,
        };

        protected string[] PlcResponseErrorList3 = 
        {
            EtherNetIP.Properties.Resources.ErrorEXTSTS01,
            EtherNetIP.Properties.Resources.ErrorEXTSTS02,
            EtherNetIP.Properties.Resources.ErrorEXTSTS03,
            EtherNetIP.Properties.Resources.ErrorEXTSTS04,
            EtherNetIP.Properties.Resources.ErrorEXTSTS05,
            EtherNetIP.Properties.Resources.ErrorEXTSTS06,
            EtherNetIP.Properties.Resources.ErrorEXTSTS07,
            EtherNetIP.Properties.Resources.ErrorEXTSTS08,
            EtherNetIP.Properties.Resources.ErrorEXTSTS09,
            EtherNetIP.Properties.Resources.ErrorEXTSTS0A,
            EtherNetIP.Properties.Resources.ErrorEXTSTS0B,
            EtherNetIP.Properties.Resources.ErrorEXTSTS0C,
            EtherNetIP.Properties.Resources.ErrorEXTSTS0D,
            EtherNetIP.Properties.Resources.ErrorEXTSTS0E,
            EtherNetIP.Properties.Resources.ErrorEXTSTS0F,
            EtherNetIP.Properties.Resources.ErrorEXTSTS10,
            EtherNetIP.Properties.Resources.ErrorEXTSTS11,
            EtherNetIP.Properties.Resources.ErrorEXTSTS12,
            EtherNetIP.Properties.Resources.ErrorEXTSTS13,
            EtherNetIP.Properties.Resources.ErrorEXTSTS14,
            EtherNetIP.Properties.Resources.ErrorEXTSTS15,
            EtherNetIP.Properties.Resources.ErrorEXTSTS16,
            EtherNetIP.Properties.Resources.ErrorEXTSTS17,
            EtherNetIP.Properties.Resources.ErrorEXTSTS18,
            EtherNetIP.Properties.Resources.ErrorEXTSTS19,
            EtherNetIP.Properties.Resources.ErrorEXTSTS1A,
            EtherNetIP.Properties.Resources.ErrorEXTSTS1B,
            EtherNetIP.Properties.Resources.ErrorEXTSTS1C,
            EtherNetIP.Properties.Resources.ErrorEXTSTS1D,
            EtherNetIP.Properties.Resources.ErrorEXTSTS1E,
            EtherNetIP.Properties.Resources.ErrorEXTSTS1F,
            EtherNetIP.Properties.Resources.ErrorEXTSTS20,
            EtherNetIP.Properties.Resources.ErrorEXTSTS21,
            EtherNetIP.Properties.Resources.ErrorEXTSTS22,
            EtherNetIP.Properties.Resources.ErrorEXTSTS23,
            EtherNetIP.Properties.Resources.ErrorEXTSTS24,
         };
        ...
        public override void Dispose()
        {
            ...
            EtherNetIpProtocol.CloseSession(ref requestBuffer, ref requestBufferPointer, this);
