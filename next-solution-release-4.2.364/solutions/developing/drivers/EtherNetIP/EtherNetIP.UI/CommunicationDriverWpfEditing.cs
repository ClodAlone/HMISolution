...
namespace EtherNetIP.UI
{
    public class CommunicationDriverWpfEditing : ICommunicationDriverWpfEditing2, IDisposable
    {
        ...
        public bool CopyFile(string sourceconn, string targetconn)
        {
            ...
            bool result = DriverCodeBase.CommunicationDriver.CopyFile<EtherNetIPDriverSettings>(sourceconn, targetconn, drivername);
            result &= DriverCodeBase.CommunicationDriver.CopyFile<EtherNetIPChangeTag>(sourceconn, targetconn, drivername, DriverCodeBase.Helpers.DriverInfo.GetChangeDynTagsExtension());
            ...
            using (ufw = new UnitOfWork(idl))
            {
                ...
                if ((configuration != null) && (configuration.StationSettings.Count > 0))
                {
                    foreach (EtherNetIPStationSettings settings in configuration.StationSettings)
                    {
                        ...
                        Result &= DriverCodeBase.CommunicationDriver.CopyFile<EtherNetIPOptimizedTagsMap>(sourceconn, targetconn, EtherNetIPChannel.GetOptimizedTagsMapFileNameOnly(settings.Name), ".xml");
                    }
                }
            }
