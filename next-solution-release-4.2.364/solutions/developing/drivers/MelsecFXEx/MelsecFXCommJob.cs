...
namespace MelsecFX
{
    ...
    public class MelsecFXCommJob : CommJob
    {
        ...
        internal CommJob CreateChildJobWriteArrayBit(MelsecFXTag.ChangedBit changedBit)
        {
            MelsecFXDynTagSettings startDyn = new MelsecFXDynTagSettings();
            startDyn.TryParse(TagsList[0].DynSettings.ToString());

            // define the tag start address using bit nr
            MelsecFXAddress newAddressObj = new MelsecFXAddress(startDyn.Address);
            newAddressObj.StartAddress = (newAddressObj.StartAddress + changedBit.BitNr);
            startDyn.Address = newAddressObj.ToString();
        ...
        public bool IsBitArray(Tag defTag = null)
        {
            ...
            return MelsecFXProtocol.IsBitArrayTag(defTag);
        }

        public override bool IsCustomJob(CommJob.CustomJobCheckStates state = CustomJobCheckStates.None)
        {
           switch (state)
           {
               case CustomJobCheckStates.None:
                case CustomJobCheckStates.Write:                
                    return _IsCustomJob;
                case CustomJobCheckStates.Read:
                    if (IsBitArray())
                        return false;
                    else
                        return _IsCustomJob;
