...
#define L4_VERSION_MINOR  0x02
#define L4_VERSION ((L4_VERSION_MAJOR<<8)|L4_VERSION_MINOR)

#define GET_L4VERSION_MAJOR(v)			((v>>8)&0xFF)
#define GET_L4VERSION_MINOR(v)			(v&0xFF)

/* List of all released L4 protocol versions */
#define L4_VERSION_FIRST			0x0100 /* introduced with V3.0.1.0, first version */
#define L4_VERSION_FIRST_SWAPPED	0x0001 /* badly swapped version 1.0 */
#define L4_VERSION_MAX_ADDRESS_SIZE	0x0101 /* introduced with V3.2.1.10, improves parallel routing, which was started with V3.2.0.0 */
#define L4_VERSION_CHANNEL_PIN		0x0102 /* introduced with V3.5.18.0, additional pin to secure channel id */

/* Version notes: 
   1. Versions V3.2.1.10 and < V3.5.18.0 of the channel server running on Motorola byte order systems discard 
      open channel packages of protocol version 1.0 because of an swapping issue.      
	  
   2. Starting from version V3.5.18.0, the channel server has a well defined behavior regarding 
      the open channel request protocol versions:
      - If the major version is not supported, the server does not handle the request and answers
	    with OPENC_VERSION_NOT_SUPPORTED.
	  - The minor protocol version is negotiated. The highest available version of both peers is used.   
*/
...
#define PTF_NONE			0x00	/* No flag set */
#define PTF_FIRSTPACKAGE    0x01	/* Used in PT_DATA packages. Marks the first package of a layer-7 message. */ 
#define PTF_MISSINGPACKAGE  0x02	/* Currently not used. Signals in PT_DATA and PT_ACK packages that the next package 
									   after the acknowledged package probably has been lost and should be retransmitted. */
#define PTF_DATADIRTOSERVER 0x80	/* Used for channel (DATA, ACK, KEEPALIVE, ...) packages. 
...
typedef struct
{
	...
	RTS_UI16 usTypeAndFlags;
#else
	RTS_UI8 byPkgType; 				/* Type of this pkg (one of the PT_ constants) */
	RTS_UI8 byFlags;				/* PTF_ flags dependent on the PkgType */
#endif
}L4PACKAGE;

/* Header for all layer4 data packages (an extension of L4PACKAGE) */
typedef struct
{
	L4PACKAGE L4Pkg;
	RTS_UI16 wChannelId;			/* Channel id of the package */
}L4DATAPACKAGE;

/* Header for all layer4 data packages (an extension of L4PACKAGE), used since version 1.2 */
typedef struct
{
	L4PACKAGE L4Pkg;
	RTS_UI16 wChannelId;			/* Channel id of the package */
	RTS_UI32 dwChannelPin;			/* Channel pin to secure the channel id */
}L4DATAPACKAGE2;

/* ACK block for a layer4 data acknowledge (PT_ACK) package */
typedef struct
{
	RTS_UI32  dwAckBlock;			/* number of last acknowledged block */
}L4DATAPACKAGEACK;

/* Layer4 data acknowledge (PT_ACK) package (an extension of L4DATAPACKAGE) */
typedef struct
{
	L4DATAPACKAGE dataPkg;
	L4DATAPACKAGEACK ack;
}L4ACKNOWLEDGE;

/* Layer4 data acknowledge (PT_ACK) package (an extension of L4DATAPACKAGE2), used since version 1.2 */
typedef struct
{
	L4DATAPACKAGE2 dataPkg;
	L4DATAPACKAGEACK ack;
}L4ACKNOWLEDGE2;


/* Data block for a layer4 data (PT_DATA) package */
typedef struct
{
	RTS_UI32 dwBlock;				/* number of the contained data block */
	RTS_UI32 dwAckBlock;			/* number of last acknowledged block */
	RTS_UI8 byData[1];
}L4DATAPACKAGEDATA;

/* Data block for a first layer4 data (PT_DATA) package */
typedef struct
{
	RTS_UI32 dwBlock;				/* number of the contained data block */
	RTS_UI32 dwAckBlock;			/* number of last acknowledged block */
	RTS_UI32 dwMsgLength;			/* Length of the complete (L7) message */
	RTS_UI32 dwMsgChecksum;			/* CRC32 checksum of the complete (L7) message */
	RTS_UI8 byData[1];
}L4DATAPACKAGEDATAFIRST;

/* Layer4 data (PT_DATA) package (an extension of L4DATAPACKAGE) */
typedef struct
{
	L4DATAPACKAGE dataPkg;
	L4DATAPACKAGEDATA data;
}L4DATA;

/* Layer4 data (PT_DATA) package (an extension of L4DATAPACKAGE2), used since version 1.2 */
typedef struct
{
	L4DATAPACKAGE2 dataPkg;
	L4DATAPACKAGEDATA data;
...
/* First layer4 data (PT_DATA) package of a layer 7 message (an extension of L4DATAPACKAGE) */
typedef struct
{
	L4DATAPACKAGE dataPkg;
	L4DATAPACKAGEDATAFIRST data;
}L4DATAFIRST;

/* First layer4 data (PT_DATA) package of a layer 7 message (an extension of L4DATAPACKAGE), used since version 1.2 */
typedef struct
{
	L4DATAPACKAGE2 dataPkg;
	L4DATAPACKAGEDATAFIRST data;
...
/* Layer4 data keep alive (PT_KEEPALIVE) package - it is simply a L4DATAPACKAGE */
typedef L4DATAPACKAGE L4KEEPALIVE;

/* Layer4 data keep alive (PT_KEEPALIVE) package - it is simply a L4DATAPACKAGE2, used since version 1.2 */
typedef L4DATAPACKAGE2 L4KEEPALIVE2;
...
{
	L4PACKAGE L4Pkg;
	RTS_UI16 wVersion;	  			/* L4 Protocol version */
	RTS_UI32 dwChecksum;			/* CRC32 checksum of the meta package */
}L4METAPACKAGE;
...
typedef struct
{
	...
	RTS_UI32 dwRequestId;			/* Client request id to identify repeated or different requests */
	RTS_UI32 dwCommBufferSize; 		/* Requested size of communication buffer */
}L4OPENCHANNEL_REQ;

/* Request for a new channel, used since version 1.1 */
typedef struct
{
	L4METAPACKAGE metaPkg;
	RTS_UI32 dwRequestId;			/* Client request id to identify repeated or different requests */
...
#else
	RTS_UI8 byMaxAddressSize; 		/* Bytes needed to store sender and receiver address form the clients point of view */
	RTS_UI8 byDummy;				/* Dummy alignment byte, must be set to 0 */
#endif
	RTS_UI16 wDummy;				/* Dummy alignment word, must be set to 0 */
}L4OPENCHANNEL_REQ2;
...
#define OPENC_VERSION_NOT_SUPPORTED     3	/* Server does not support the protocol major version */

/* Definition of the open channel response flags */
#define CHF_NONE					0x00	/* No flag set */
...
{
	L4METAPACKAGE metaPkg;
	RTS_UI32 dwRequestId;			/* References the request */
	RTS_UI16 wResult;				/* One of the OPENC_xxx constants */
	RTS_UI16 wChannelId;			/* The id of the created channel. */
	RTS_UI32 dwCommBufferSize;		/* Granted size of communication buffer. Less or equal to the requested size */
}L4OPENCHANNEL_RESP;

/* Response to an open channel request, used since version 1.1 */
typedef struct
{
	L4METAPACKAGE metaPkg;		
	RTS_UI32 dwRequestId;			/* References the request */
...
#else	
	RTS_UI8 byFlags;				/* A combination of the CHF_xxx flags */
	RTS_UI8 byMaxAddressSize;		/* Stores the sum of sender and receiver address lengths */
#endif
	RTS_UI16 wDummy;				/* Dummy alignment word, must be zero */
}L4OPENCHANNEL_RESP2;

/* Response to an open channel request, used since version 1.2 */
typedef struct
{
	L4METAPACKAGE metaPkg;		
	RTS_UI32 dwRequestId;			/* References the request */
...
/* Close channel request or response */
typedef struct
{
	L4METAPACKAGE metaPkg;
	RTS_UI16 wChannelId;			/* The id of the channel to close */
	RTS_UI16 wError;				/* The cause for closing the channel (ERR_OK for close by request)*/
}L4CLOSECHANNEL_REQ;

/* Close channel request or response, used since version 1.2 */
typedef struct
{
	L4METAPACKAGE metaPkg;
	RTS_UI16 wChannelId;			/* The id of the channel to close */
	RTS_UI16 wError;				/* The cause for closing the channel (ERR_OK for normal shutdown)*/
	RTS_UI32 dwChannelPin;			/* Channel pin to secure the channel id */
}L4CLOSECHANNEL_REQ2;
...
typedef struct 
{
	...
	RTS_UI32 dwBlockNr;
	RTS_UI16 wLength;
	RTS_UI16 wFlags;				/* Combination of the BF_xxx flags */
}BLOCKINFO;
...
{
	/* Channel "name" */
	RTS_UI8 ucAddrOffset;			/* Offset of relative addresses. Valid if CF_ADDRRELATIVE is set in wFlags */
	RTS_UI8 ucAddrPeerLen;			/* Address length of the communication partner */
	ADDRESSCOMPONENT addrPeer[MAX_NODEADDR_LEN]; /* Address of the communication partner */
	RTS_UI16 wChannelId;			/* Channel id - identifies the channel */
	RTS_HANDLE hRouter;				/* Handle of the router instance, to which this channel is assigned */

	/* Channel properties (static) */
	RTS_UI32 dwCommBufferSize;		/* Maximum buffer size available for L7 services */
	RTS_UI16 wNumBlockBuffers;		/* Number of blocks needed to transmit the complete communication buffer */
	RTS_UI16 wMaxBlockSize;			/* Maximum size of a L4 package */
	RTS_UI16 wFlags;				/* Combination of the CF_xxx constants */
	
	/* Connection properties (dynamic adaptation of package window size) */
	RTS_UI16 wCWnd;					/* Current window size, means max. number of unACKed packages allowed to send */
	RTS_UI16 wCWndCheckCount;		/* Number of sent data packages since last window check */
	RTS_UI16 wLastCWnd;				/* Last found "good" value for window size */
	RTS_UI16 wGoodCWnd;				/* Window size found as a "good" value for this channel */
	RTS_UI16 wGoodCWndCount;		/* Counts the number of times, the same "good" value was found */ 
	RTS_UI32 dwCWndFreezeTime;		/* Time stamp of the last freeze of the window size */
	RTS_UI32 dwRTT;					/* Round trip time - measured from sending a package until it is ACKed */
	RTS_UI32 dwRTO;					/* Retransmission timeout value - calculation is based on the weighted RTTs */ 

	/* The channels buffers */
	RTS_UI8 *pReceiveBuffer;		/* Pointer to the start of the receive message */
	RTS_UI8 *pSendBuffer;			/* Pointer to the start of the send message */
		
	CHANNELSENDSTATE sendState;		/* Structure to manage sending of messages */
	CHANNELRECEIVESTATE recState;	/* Structure to manage receiving of messages */

	/* Extensions added in version V3.5.18.0 */
	RTS_UI16 wL4Version;			/* Version of the L4 protocol */
	RTS_UI32 dwChannelPin;			/* Channel pin to secure the channel id */

	/* Block info to manage the blocks to send or receive */
	BLOCKINFO blockBuffer[1]; 
		/* The communication buffer starts after the last element of the blockBuffer array.
		   ...
		   i.e. the first element after the blockBuffer. This must be aligned on a 4 byte boundary.
		*/
}CHANNELBUFFER;
	...
	#define sizeof_L4DATAPACKAGE			RTS_GET_BYTE_SIZE(sizeof(L4DATAPACKAGE))
	#define sizeof_L4DATAPACKAGE2			RTS_GET_BYTE_SIZE(sizeof(L4DATAPACKAGE2))
	#define sizeof_L4DATAPACKAGEACK			RTS_GET_BYTE_SIZE(sizeof(L4DATAPACKAGEACK))
	#define sizeof_L4ACKNOWLEDGE			RTS_GET_BYTE_SIZE(sizeof(L4ACKNOWLEDGE))
	#define sizeof_L4ACKNOWLEDGE2			RTS_GET_BYTE_SIZE(sizeof(L4ACKNOWLEDGE2))
	#define sizeof_L4DATAPACKAGEDATA		RTS_GET_BYTE_SIZE(sizeof(L4DATAPACKAGEDATA))
	#define sizeof_L4DATAPACKAGEDATAFIRST	RTS_GET_BYTE_SIZE(sizeof(L4DATAPACKAGEDATAFIRST))
	#define sizeof_L4DATA					RTS_GET_BYTE_SIZE(sizeof(L4DATA))
	#define sizeof_L4DATA2					RTS_GET_BYTE_SIZE(sizeof(L4DATA2))
	#define sizeof_L4DATAFIRST				RTS_GET_BYTE_SIZE(sizeof(L4DATAFIRST))
	#define sizeof_L4DATAFIRST2				RTS_GET_BYTE_SIZE(sizeof(L4DATAFIRST2))
	#define sizeof_L4KEEPALIVE				RTS_GET_BYTE_SIZE(sizeof(L4KEEPALIVE))
	#define sizeof_L4KEEPALIVE2				RTS_GET_BYTE_SIZE(sizeof(L4KEEPALIVE2))
	#define sizeof_L4METAPACKAGE			RTS_GET_BYTE_SIZE(sizeof(L4METAPACKAGE))
	#define sizeof_L4OPENCHANNEL_REQ		RTS_GET_BYTE_SIZE(sizeof(L4OPENCHANNEL_REQ))
	#define sizeof_L4OPENCHANNEL_REQ2		RTS_GET_BYTE_SIZE(sizeof(L4OPENCHANNEL_REQ2))
	#define sizeof_L4OPENCHANNEL_RESP		RTS_GET_BYTE_SIZE(sizeof(L4OPENCHANNEL_RESP))
	#define sizeof_L4OPENCHANNEL_RESP2		RTS_GET_BYTE_SIZE(sizeof(L4OPENCHANNEL_RESP2))
	#define sizeof_L4OPENCHANNEL_RESP3		RTS_GET_BYTE_SIZE(sizeof(L4OPENCHANNEL_RESP3))
	#define sizeof_L4CLOSECHANNEL_REQ		RTS_GET_BYTE_SIZE(sizeof(L4CLOSECHANNEL_REQ))
	#define sizeof_L4CLOSECHANNEL_REQ2		RTS_GET_BYTE_SIZE(sizeof(L4CLOSECHANNEL_REQ2))

	/* L4PACKAGE */
	#define GET_L4PACKAGE_TYPE(pkg)			(RTS_UI8)(((L4PACKAGE*)pkg)->usTypeAndFlags & 0x00FF)
	#define GET_L4PACKAGE_FLAGS(pkg)		(RTS_UI8)((((L4PACKAGE*)pkg)->usTypeAndFlags >> 8) & 0x00FF)
	#define SET_L4PACKAGE_TYPE(pkg,nType)	((L4PACKAGE*)pkg)->usTypeAndFlags = ((RTS_UI16) (nType) & 0x00FF) | (((L4PACKAGE*)pkg)->usTypeAndFlags & 0xFF00)
	#define CLEAR_L4PACKAGE_FLAGS(pkg)		((L4PACKAGE*)pkg)->usTypeAndFlags = (((L4PACKAGE*)pkg)->usTypeAndFlags & 0x00FF)
	#define SET_L4PACKAGE_FLAG(pkg,nFlag)	((L4PACKAGE*)pkg)->usTypeAndFlags = (((RTS_UI16)(GET_L4PACKAGE_FLAGS(pkg) | (nFlag)) << 8) & 0xFF00) | (((L4PACKAGE*)pkg)->usTypeAndFlags & 0x00FF)

	/* L4OPENCHANNEL_REQ2 */
	#define GET_L4OPENCHANNEL_REQ2_MAX_ADDR_SIZE(pkg)	(RTS_UI8)(((L4OPENCHANNEL_REQ2*)pkg)->usMaxAddressSize & 0x00FF)
	#define SET_L4OPENCHANNEL_REQ2_MAX_ADDR_SIZE(pkg,nMaxAddrSize)	((L4OPENCHANNEL_REQ2*)pkg)->byMaxAddressSize = (RTS_UI16) (nMaxAddrSize & 0x00FF) | (RTS_UI16)(((L4OPENCHANNEL_REQ2*)pkg)->byMaxAddressSize & 0xFF00)

	/* L4OPENCHANNEL_RESP3 */
	#define GET_L4OPENCHANNEL_RESP_FLAGS(pkg)	(RTS_UI8)(((L4OPENCHANNEL_RESP3*)pkg)->usFlagsAndMaxAddrSize & 0x00FF)
	#define GET_L4OPENCHANNEL_RESP_MAX_ADDR_SIZE(pkg)	(RTS_UI8)((((L4OPENCHANNEL_RESP3*)pkg)->usFlagsAndMaxAddrSize >> 8) & 0x00FF)
	#define CLEAR_L4OPENCHANNEL_RESP_FLAGS(pkg)		((L4OPENCHANNEL_RESP3*)pkg)->usFlagsAndMaxAddrSize = (((L4OPENCHANNEL_RESP3*)pkg)->usFlagsAndMaxAddrSize & 0xFF00)
	#define SET_L4OPENCHANNEL_RESP_FLAG(pkg,nFlag)	((L4OPENCHANNEL_RESP3*)pkg)->usFlagsAndMaxAddrSize = (((RTS_UI16)(GET_L4PACKAGE_FLAGS(pkg) | nFlag)) & 0x00FF) | (((L4OPENCHANNEL_RESP3*)pkg)->usFlagsAndMaxAddrSize & 0xFF00)
	#define SET_L4OPENCHANNEL_RESP_MAX_ADDR_SIZE(pkg,nMaxAddrSize)	((L4OPENCHANNEL_RESP3*)pkg)->usFlagsAndMaxAddrSize = (RTS_UI16)((nMaxAddrSize << 8) & 0xFF00) | (RTS_UI16)(((L4OPENCHANNEL_RESP3*)pkg)->usFlagsAndMaxAddrSize & 0x00FF)															

#else

	/* L4PACKAGE */
	#define GET_L4PACKAGE_TYPE(pkg) 		(RTS_UI8)((L4PACKAGE*)pkg)->byPkgType
	#define GET_L4PACKAGE_FLAGS(pkg) 		(RTS_UI8)((L4PACKAGE*)pkg)->byFlags
	#define SET_L4PACKAGE_TYPE(pkg,nType) 	((L4PACKAGE*)pkg)->byPkgType = (RTS_UI8) nType
	#define CLEAR_L4PACKAGE_FLAGS(pkg)		((L4PACKAGE*)pkg)->byFlags = 0
	#define SET_L4PACKAGE_FLAG(pkg,nFlag)	((L4PACKAGE*)pkg)->byFlags |= nFlag

	/* L4OPENCHANNEL_REQ2 */
	#define GET_L4OPENCHANNEL_REQ2_MAX_ADDR_SIZE(pkg)	(RTS_UI8)((L4OPENCHANNEL_REQ2*)pkg)->byMaxAddressSize
	#define SET_L4OPENCHANNEL_REQ2_MAX_ADDR_SIZE(pkg,nMaxAddrSize)	((L4OPENCHANNEL_REQ2*)pkg)->byMaxAddressSize = (RTS_UI8) nMaxAddrSize

	/* L4OPENCHANNEL_RESP3 */
	#define GET_L4OPENCHANNEL_RESP_FLAGS(pkg)	(RTS_UI8)((L4OPENCHANNEL_RESP3*)pkg)->byFlags
	#define GET_L4OPENCHANNEL_RESP_MAX_ADDR_SIZE(pkg)	(RTS_UI8)((L4OPENCHANNEL_RESP3*)pkg)->byMaxAddressSize
	#define CLEAR_L4OPENCHANNEL_RESP_FLAGS(pkg)		((L4OPENCHANNEL_RESP3*)pkg)->byFlags = 0
	#define SET_L4OPENCHANNEL_RESP_FLAG(pkg,nFlag)	((L4OPENCHANNEL_RESP3*)pkg)->byFlags |= nFlag
	#define SET_L4OPENCHANNEL_RESP_MAX_ADDR_SIZE(pkg,nMaxAddrSize)	((L4OPENCHANNEL_RESP3*)pkg)->byMaxAddressSize = (RTS_UI8) nMaxAddrSize
 ...
 * <param name="wMaxBlockSize" type="IN">Maximum size of a layer4 block (as returned by RouterGetMaxBlockSize)</param>
 * <param name="wL4Version" type="IN">L4 protocol version</param>
 * <param name="pnNumBlocks" type="OUT">Required number of block buffers</param>
 * <result>
 *   The size of the channel buffer in bytes.
 * </result>
*/
RTS_UI32 CDECL NetworkGetChBufferSize(RTS_UI32 dwCommBufferSize, RTS_UI16 wMaxBlockSize, RTS_UI16 wL4Version, int *pnNumBlocks);
typedef RTS_UI32 (CDECL * PFNETWORKGETCHBUFFERSIZE) (RTS_UI32 dwCommBufferSize, RTS_UI16 wMaxBlockSize, RTS_UI16 wL4Version, int *pnNumBlocks);
	...
	#define CAL_NetworkGetChBufferSize(p0,p1,p2,p3)  (RTS_UI32)ERR_NOTIMPLEMENTED
...
/**
 * <description>
 * Initializes the provided channel buffer with a given maximum block size and further parameters.
 * </description>
 * <param name = "pChBuffer" type="IN"> Pointer to the channel buffer to initialize </param>
 * <param name = "hRouter" type="IN"> Handle of the assigned router instance </param>
 * <param name = "dwBufferSize" type="IN"> Size of the memory pChBuffer points to </param>
 * <param name = "wChannelId" type="IN"> The id of the channel </param>
 * <param name = "addrSender" type="IN"> The CODESYS address of this channels peer </param>
 * <param name = "byChFlags" type="IN"> Initial channel flags (CF_SERVERCHANNEL for a server channel, CF_SENDMODE for a client channel) </param>
 * <param name = "dwMaxCommBuffer" type="IN"> Maximum size of the communication buffer to use (usually the requested communication buffer)  </param>
 * <param name = "wMaxBlockSize" type="IN"> Maximum size of one block </param>
 * <param name = "wL4Version" type="IN"> Version of the L4 protocol </param>
 * <param name = "dwChannelPin" type="IN"> Channel pin to secure the channel id </param>
 * <result>
 *   The size of the communication buffer for this channel buffer.
 * </result>
*/
RTS_UI32 CDECL NetworkInitChannelBuffer(CHANNELBUFFER *pChBuffer, RTS_HANDLE hRouter, RTS_UI32 dwBufferSize, RTS_UI16 wChannelId, PEERADDRESS addrSender, RTS_UI8 byChFlags, RTS_UI32 dwMaxCommBuffer, RTS_UI16 wMaxBlockSize, RTS_UI16 wL4Version, RTS_UI32 dwChannelPin);
typedef RTS_UI32 (CDECL * PFNETWORKINITCHANNELBUFFER) (CHANNELBUFFER *pChBuffer, RTS_HANDLE hRouter, RTS_UI32 dwBufferSize, RTS_UI16 wChannelId, PEERADDRESS addrSender, RTS_UI8 byChFlags, RTS_UI32 dwMaxCommBuffer, RTS_UI16 wMaxBlockSize, RTS_UI16 wL4Version, RTS_UI32 dwChannelPin);
	...
	#define CAL_NetworkInitChannelBuffer(p0,p1,p2,p3,p4,p5,p6,p7,p8,p9)  (RTS_UI32)ERR_NOTIMPLEMENTED
 ...
 * </result>
*/
int CDECL NetworkSendMetaPkg(RTS_HANDLE hRouter, PEERADDRESS addrReceiver, L4METAPACKAGE *pMetaPkg, RTS_UI32 nPkgSize);
typedef int (CDECL * PFNETWORKSENDMETAPKG) (RTS_HANDLE hRouter, PEERADDRESS addrReceiver, L4METAPACKAGE *pMetaPkg, RTS_UI32 nPkgSize);
...
typedef int (CDECL * PFNETWORKGETSTATUS) (CHANNELBUFFER *pChBuffer, RTS_UI16 *pusStatus, RTS_UI8 *pbyScalingFactor, RTS_I32 *pnItemsComplete, RTS_I32 *pnTotalItems);
	...
	#define EXTREF_ITF_NetworkGetStatus {(RTS_VOID_FCTPTR)NetworkGetStatus, "NetworkGetStatus", 0, 0}
#elif defined(MIXED_LINK) && !defined(CMPCHANNELMGR_EXTERNAL)
	#define USE_NetworkGetStatus
	#define EXT_NetworkGetStatus
	#define GET_NetworkGetStatus(fl)  CAL_CMGETAPI( "NetworkGetStatus" ) 
...
class ICmpChannelMgr : public IBase
{
	public:
		...
		virtual RTS_UI32 CDECL INetworkGetChBufferSize(RTS_UI32 dwCommBufferSize, RTS_UI16 wMaxBlockSize, RTS_UI16 wL4Version, int *pnNumBlocks) =0;
		virtual RTS_UI32 CDECL INetworkInitChannelBuffer(CHANNELBUFFER *pChBuffer, RTS_HANDLE hRouter, RTS_UI32 dwBufferSize, RTS_UI16 wChannelId, PEERADDRESS addrSender, RTS_UI8 byChFlags, RTS_UI32 dwMaxCommBuffer, RTS_UI16 wMaxBlockSize, RTS_UI16 wL4Version, RTS_UI32 dwChannelPin) =0;
		virtual int CDECL INetworkSendMetaPkg(RTS_HANDLE hRouter, PEERADDRESS addrReceiver, L4METAPACKAGE *pMetaPkg, RTS_UI32 nPkgSize) =0;
