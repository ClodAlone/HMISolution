 ...
 * <result>Pointer to the security setting which is selected on the target</result>
 */
const SecurityEditableSetting * CDECL SecManGetEditableSetting(RTS_HANDLE hSecuritySetting, RTS_RESULT *pResult);
typedef const SecurityEditableSetting * (CDECL * PFSECMANGETEDITABLESETTING) (RTS_HANDLE hSecuritySetting, RTS_RESULT *pResult);
	...
	#define CAL_SecManGetEditableSetting(p0,p1)  (const SecurityEditableSetting *)ERR_NOTIMPLEMENTED
...
class ICmpSecurityManager : public IBase
{
	public:
		...
		virtual const SecurityEditableSetting * CDECL ISecManGetEditableSetting(RTS_HANDLE hSecuritySetting, RTS_RESULT *pResult) =0;
