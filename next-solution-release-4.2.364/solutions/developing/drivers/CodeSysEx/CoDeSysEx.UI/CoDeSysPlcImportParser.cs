...
namespace CoDeSys.UI
{
    public partial class CoDeSysPlcImportParser : CoDeSysImportBase, IDisposable
    {
        ...
        public ImportDataModel Import(GetStationName readStationName, string station, string deviceHostName, string pLCAddress, ulong nTypeConnection, ulong port, string username, string passwordPLC, string GatewayPassword, int timeout, int ulNumTries)
        {
            ...
            Ret = PLCHandler.CheckConnectionAndRunningState(deviceHostName, nTypeConnection, pLCAddress, port, username, passwordPLC, GatewayPassword, timeout, ulNumTries);
            if (Ret == CoDeSysProtocol.PLCHandlerErrors.RESULT_OK)
            {
