...
namespace MelsecQEth
{
    ...
    public class MelsecQEthCommJob : CommJob
    {
            ...
            : base(station, settings)
        {
            ...
            AddressObj = new MelsecQAddress(_Address);
            _BitVariables = false;
            _PointCount = 0;
        ...
        public override void SetJobData(object jobData, ref List<Tag> changed)
        {
            ...
            lock (lockListObject)
            {
                ...
                for (int TagIndex = 0; TagIndex < TagsList.Count; TagIndex++)
                {
                    ...
                    {
                        if(CommandCode == (ushort)MelsecQCommandCode.BitUnits)
                        {
                        ...
                        else if (CommandCode == (ushort)MelsecQCommandCode.WordUnits)
                        {
       ...
       uint GetTagSize(DriverBaseInterfaces.TagDefinition t)
        ...
        public bool ParseData(byte[] receiveBuffer, ref List<object> items)
        {
            ...
            if (areArguments)
            {
                ...
                if ((CommandCode == (ushort)MelsecQCommandCode.BitUnits) ||
                    (CommandCode == (ushort)MelsecQCommandCode.WordUnits))
                {
                    if (items.Count < 2)
                    {
                        items[0] = 11;
        ...
        public uint SetModuleTarget(ref byte[] buffer, ref uint nByteCount)
        {
            switch (CpuTarget)
            {
                case CpuTargets.Plc1:
                    buffer[nByteCount++] = 0xE0; // Dest. mod. I/O N. (l. byte).
                    break;
                case CpuTargets.Plc2:
                    buffer[nByteCount++] = 0xE1; // Dest. mod. I/O N. (l. byte).
                    break;
                case CpuTargets.Plc3:
                    buffer[nByteCount++] = 0xE2; // Dest. mod. I/O N. (l. byte).
                    break;
                case CpuTargets.Plc4:
                    buffer[nByteCount++] = 0xE3; // Dest. mod. I/O N. (l. byte).
                    break;

                default:
                case CpuTargets.ControlPLC:
                    buffer[nByteCount++] = 0xFF; // Dest. mod. I/O N. (l. byte).
                    break;

            }
            buffer[nByteCount++] = 0x03; // Dest. mod. I/O N. (h. byte).
            buffer[nByteCount++] = 0; // Dest. module station number.
        ...
        /// <returns></returns>
        public static uint GetStringJobSize(bool unicode, uint stringLength)
        {
            if (unicode)
