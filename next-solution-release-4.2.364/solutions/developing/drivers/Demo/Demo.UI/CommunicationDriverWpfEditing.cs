...
namespace Demo.UI
{
    public class CommunicationDriverWpfEditing : ICommunicationDriverWpfEditing2, IDisposable
    {
        ...
        public bool CopyFile(string sourceconn, string targetconn)
        {
            ...
            bool result = DriverCodeBase.CommunicationDriver.CopyFile<DemoDriverSettings>(sourceconn, targetconn, drivername);
            result &= DriverCodeBase.CommunicationDriver.CopyFile<DemoChangeTag>(sourceconn, targetconn, drivername, DriverCodeBase.Helpers.DriverInfo.GetChangeDynTagsExtension());
            return result;
        }
        ...
        {
            DemoDynTagSettings dynTag = new DemoDynTagSettings();
        ...
        {
            var genSet = generalSettings as DemoDriverSettings;
            if(genSet==null)
                return null;

            if (channelEditor == null)
                channelEditor = new ChannelDetails();
            var ch = new DemoChannelSettings(genSet.Session);
        ...
        public UserControl NewStationSettingsEditor(object generalSettings)
        {
            ...
            var st = new DemoStationSettings(genSet.Session);
