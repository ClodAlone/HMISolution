...
{
    public class PhoenixContactPLCIChannel : ChannelList
    { 
        #region Constructors
        ...
        private PhoenixContactPLCIProtocol.ConnectionState _CurrentConnectionState = PhoenixContactPLCIProtocol.ConnectionState.Disconnected;
        private PhoenixContactPLCIProtocol.ConnectionState _PreviousConnectionState = PhoenixContactPLCIProtocol.ConnectionState.Disconnected;
        //private DateTime _LastDeviceCompletedRequest = DateTime.MinValue;
        private bool _FirstManageConnection = true;
        ...
        private void StartTmrCheckDeviceCyclically()
        {
            lock (lockThreadObject)
            {
                if (_TmrCheckConnectionCyclically == null)
                    _TmrCheckConnectionCyclically = new Timer(CheckDeviceCyclically, this, Properties.Settings.Default.CheckStateNoJobsPollingTime, System.Threading.Timeout.Infinite);
            }
        }
        ...
        private bool ConnectionStateChanged(out PhoenixContactPLCIProtocol.ConnectionState previousConnectionState, bool firstTime = false)
        {
            bool statusChanged = false;
            
            bool IsRunning = (GetState() == PlcState.PlcRunning);

            previousConnectionState = _PreviousConnectionState;
        ...
        {            
            ...
            while (jobIndex < jobList.Count())
            {
                ...
                while (jobIndex < jobList.Count())
                {
                    PhoenixContactPLCICommJob j = jobList.ElementAt(jobIndex) as PhoenixContactPLCICommJob;
                    // ignore job with unmapped tag on device (checked on DeviceOpen)
                    if (first)
                    {
                    ...
                    else
                    {
                        if (j.Station != null && j.Station.Name == station && !jwrite && j.Type != LinkType.ExceptionOutput)
                        {
                            ...
                            else
                            {
                                ...
                                {
                                    // 1f the 1 job is a big job/array, add to list
                                    if (list.Count == 0)
                                    {
                                        list.Add(j);
                                        jobList.RemoveAt(jobIndex);
                                    }
                                    // message too long, stop 
                                    break;
                                }
                            }
                        }
        ...
        private DriverErrorCodes WriteData(List<PhoenixContactPLCICommJob> list) {
            ...
            foreach (PhoenixContactPLCICommJob j in list)
            {
                ...
                {
                    ExecutedJobArgs eJob = new ExecutedJobArgs();
                    eJob.ErrorCode = (DriverErrorCodes)PhoenixContactPLCIProtocol.ErrorCodes.ErrorPoorlyFormattedData;
                    j.LastExecutionTime = DateTime.UtcNow;
                    eJob.Job = j;
                    OnJobExecuted(eJob);
                }
        ...
        public override bool ExecuteJobList(ref DriverErrorCodes conn, List<CommJob> l)
        {
            List<PhoenixContactPLCICommJob> list = l.ConvertAll(x => (PhoenixContactPLCICommJob)x);

            if (list[0].Type == DriverCodeBaseEx.Enumerators.LinkType.Input || (list[0].Type == DriverCodeBaseEx.Enumerators.LinkType.InputOutput && list[0].TagsListOnWriting.Count == 0))
                conn = ReadData(list);
            else
                conn = WriteData(list);
            ...
            // for driver with library set to false
            return false;
        }
