    ...
    public class BrPviChannel : ChannelList
    {
        ...
        public override void SplitInExecutionLists(List<CommJob> jobList, ref List<List<CommJob>> exList)
        {
            ...
            while (jobIndex < jobList.Count)
            {
                ...
                while (jobIndex < jobList.Count)
                {
                    ...
                    if (j != null)
                    {
                        // if connection is not yet initializad, schedule on job at time
                        if (!((BrPviStation)j.Station).pviObjectsInitialized)
                        {
                            exList.Add(new List<CommJob>() { j });
                        ...
                        else
                        {
                            ...
                            bool jsubscribe = j.IsSubscribeState();
                            if (subscribe && jsubscribe)
                            {
                            ...
                            {
                                // write operation,ecc are performed "individually"
                                exList.Add(new List<CommJob>() { j });
                                jobList.RemoveAt(jobIndex);
                                jobIndex--;
                            }
        ...
        public override bool ExecuteJobList(ref DriverErrorCodes conn, List<CommJob> list)
        {
            ...
            foreach (BrPviCommJob brJob in list)
            {
                ...
                // Pvi server don't accept/respond a request of subscription/string length of variable (name) already suscbribed; used a settings/value of subscribed job (with same variable name)
        ...
        public DriverErrorCodes TestComm(BrPviStation station)
        {
            //DriverErrorCodes conn = DriverErrorCodes.ErrorNoError;

            //List<CommJob> exjoblist = new List<CommJob>();

            //BrPviTag defTag = new BrPviTag(new TagDefinition());

            //BrPviCommJob exjob = new BrPviCommJob(station, );
            //exjob

            //while (conn != DriverErrorCodes.ErrorNoError && exjoblist.Count(c => c.IsPending) > 0)
            //{
            //    conn = CheckDevice(exjoblist, this);
            ...
            //    if (!bDisposed && exjoblist.Count(c => c.IsPending) > 0)
            ...
            try
            {
                ...
                if (pviObj != null)
                {
                    ...
                    switch (pviEvent.EventInfo.nMode)
                    {
                        ...
                        case (uint)PviObjectModes.POBJ_MODE_DELETE: // not supported
                            break;
                    }
                }
            }
