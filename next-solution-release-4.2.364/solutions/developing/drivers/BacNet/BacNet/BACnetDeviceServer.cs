...
namespace BACnet
{
    ...
    public class BACnetDeviceServer
    {
        ...
        public BACnetDeviceServer(BACnetChannel channel,int deviceId, string localHostName)
        {
            DefaultSettings();
            _Channel = channel;
            _DeviceId = deviceId;
        ...
        public IPAddress LocalHostIP
        {
            get
            {
                ...
                if (!string.IsNullOrEmpty(_LocalHostName))
                {
                    ...
                    if (UdpChannel.GetResolvedConnecionIPAddress(_LocalHostName, out resolvedIPAddress))
