...
namespace MelsecQEth
{
    ...
    public class MelsecQAddress : Object
    {
        ...
        /// </summary>
        public MelsecQAddress(MelsecQEthProtocol.AddressTypes addressTypes, string QAddress)
        {
            Parse(addressTypes, QAddress);
        }
        ...
        /// Parse an address.
        /// </summary>
        void Parse(MelsecQEthProtocol.AddressTypes addressType, string QAddress, bool bHexFormat = true)
        {
            _AddressType = addressType;

            ResetAddress();
            string TempAddress = String.Copy(QAddress).Trim();
            ...
            if (addressType == MelsecQEthProtocol.AddressTypes.DataArea)
            {
                ...
                string StartAddressString = TempAddress.Substring(1).Trim();
                string StartAddressString1 = TempAddress.Substring(2).Trim();
                ...
                switch (TempAddress[0])
                {
                    case 'X':
                        ...
                        {
                            _DataAreaString = TempAddress[0].ToString();
                    ...
                    case 'S':
                        switch (TempAddress[1])
                        {
                            case 'B':
                                ...
                                {
                                    _DataAreaString = TempAddress[1].ToString();
        ...
        public void Set(MelsecQEthProtocol.AddressTypes addressType, string QAddress)
        {
            Parse(addressType, QAddress);
        }

        public bool TryToSet(MelsecQEthProtocol.AddressTypes addressType, int StartAddress)
        {
            return TryToSet(addressType, StartAddress.ToString());
        }

        public bool TryToSet(MelsecQEthProtocol.AddressTypes addressType, string StartAddress)
        {
            _AddressType = addressType;
            if (addressType == MelsecQEthProtocol.AddressTypes.DataArea)
            {
                ...
                Parse(addressType, newAddress);
            } 
            else 
            { 
                _IsValid = !string.IsNullOrEmpty(StartAddress); 
            }
        ...
        public string GetNewAddress(MelsecQEthProtocol.AddressTypes addressType, string Address)
        {
            if (addressType == MelsecQEthProtocol.AddressTypes.DataArea)
            {
                // Build the string of the new address and set it
                ...
                switch (DataArea)
                {
                    ...
                    case DataArea.SW:
                        // Hexadecimal format
                        strNewAdd = string.Format("{0:X}", Address);
                        break;

                    default:
                        // Decimal format
                        strNewAdd = string.Format("{0}", Address);
                        break;
                }
        ...
        public int GetLabelAddressLength()
        {
            return (_Address.Length * 2);
        }
        #endregion
        ...
        /// Data area code
        /// </summary>
        private string _DataAreaString;
        public string DataAreaString
        {
            get
            {
                return _DataAreaString;
            }
        }
