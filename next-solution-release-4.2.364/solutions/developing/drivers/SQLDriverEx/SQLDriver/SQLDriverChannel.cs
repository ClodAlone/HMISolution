    ...
    protected void SetConnectionError(List<SQLDriverCommJob> list)
        ...
        private void ExecuteJobList(List<CommJob> l)
        {
            ...
            for (int index = 0; index < list.Count; index++)
            {
                ...
                {
                    list[index].FillWholeTagsListOnWriting();
                }
            }
        ...
        protected int WriteToDatabase(SQLDriverStation s, List<SQLDriverCommJob> pendingjob)
        {
            ...
            for (int i = 0; i < pendingjob.Count; i++)
            {
                ...
                try
                {
                    ...
                    {
                        WriteDataSet.Tables[0].Rows.Add(pendingjob[i].TagName, pendingjob[i].TagsList[0].Value.ToString(null, CultureInfo.InvariantCulture));
                    }
                    else
                    {
                        ((DataRow) WriteDataSet.Tables[0].Rows.Find(pendingjob[i].TagName))[pendingjob[i].SQLDriverColumnValue] = pendingjob[i].TagsList[0].Value.ToString(null, CultureInfo.InvariantCulture);                  
                    }
                }
                catch (Exception ex)
                {
                ...
                using (writer)
                {
                    ...
                    try
                    {
                        ...
                        for (int ix = 0; ix < pendingjob.Count; ix++)
                        {
                            ...
                            OnJobExecuted(eJob);
                            j.lastExecutionTimeSchede = date;
                            if((j.TagsList[0].Value != null) && (j.TagsList[0].Value.Value != null))
                                j.TagsList[0].LastValue = j.TagsList[0].Value.Value;
                        }
                    }
                    catch (Exception ex)
                    {
