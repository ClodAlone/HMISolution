...
{
    sealed class ExampleDemoChannel : Channel
    {
        #region constants
        ...
        /// </summary>
        public ExampleDemoChannel(CommunicationDriver commdriver, ExampleDemoChannelSettings settings)
        ...
        void UpdateSin(Tag ctag, ExampleDemoCommJob job)
        {
        ...
        void UpdateRamp(Tag ctag, ExampleDemoCommJob job)
        {
        ...
        void UpdateCos(Tag ctag, ExampleDemoCommJob job)
        {
        ...
        {
            ExampleDemoCommJob dj = job as ExampleDemoCommJob;
            if (dj == null) return;

            base.ExecuteJob(job);//steve 230811
            ...
            if (SimulationRunning)
            {
                ...
                {
                    var simtime = (dj.SimulationInterval < 0 ? Convert.ToDouble(((ExampleDemoStation)dj.Station).SimulationInterval) : Convert.ToDouble(dj.SimulationInterval));
                    DateTime UtcNow = DateTime.UtcNow;
                    if ((UtcNow - dj.lastTrigoTime).TotalMilliseconds >= simtime)
                    {
                        if (!dj.CommonVal.HasValue)
                        {
                            if (dj.DemoType == ExampleDemoTypes.Ramp)
                        ...
                        {
                            double d = 1.0 / StepSimulazione;
                            d += Convert.ToDouble(dj.CommonVal);
                            if (d > 1)
                                d = 0;
                            dj.CommonVal = d;
                        }
                    ...
                    if ((UtcNow - dj.LastExecutionTime).TotalMilliseconds < simtime)
                    {
                        ...
                        ExecutedJobArgs evJob = new ExecutedJobArgs { ErrorCode = LastErrorCode, Job = job };
                        OnJobExecuted(evJob);
                        return;
                    }
                    else
                    {
                        foreach (var t in dj.TagsList)
                        {
                            ...
                            {
                                if (dj.DemoType == ExampleDemoTypes.Sin)
                                    UpdateSin(t, dj);
                                else if (dj.DemoType == ExampleDemoTypes.Cos)
                                    UpdateCos(t, dj);
                                else if (dj.DemoType == ExampleDemoTypes.Ramp)
                                    UpdateRamp(t, dj);
                                else if (dj.DemoType == ExampleDemoTypes.Random)
                                    UpdateRandom(t);
                            }
                        }
