...
namespace LacbusPC.UI
{
    public class CommunicationDriverWpfEditing : ICommunicationDriverWpfEditing2, IDisposable
    {
        ...
        public bool CopyFile(string sourceconn, string targetconn)
        {
            ...
            bool result = DriverCodeBase.CommunicationDriver.CopyFile<LacbusPCDriverSettings>(sourceconn, targetconn, drivername);
            result &= DriverCodeBase.CommunicationDriver.CopyFile<LacbusPCChangeTag>(sourceconn, targetconn, drivername, DriverCodeBase.Helpers.DriverInfo.GetChangeDynTagsExtension());
            return result;
        }
        ...
        {
            LacbusPCDynTagSettings dynTag = new LacbusPCDynTagSettings();
        ...
        {
            var genSet = generalSettings as LacbusPCDriverSettings;
            if (genSet == null)
                return null;

            if (channelEditor == null)
                channelEditor = new ChannelDetails();
            var ch = new LacbusPCChannelSettings(genSet.Session);
        ...
        public UserControl NewStationSettingsEditor(object generalSettings)
        {
            ...
            var st = new LacbusPCStationSettings(genSet.Session);
