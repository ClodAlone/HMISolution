...
namespace S7TCP
{
    ...
    public static class S7Protocol
    {
        ...
        public const uint INITAL_REQUEST_DIM = 12;
        ...
        public static uint PrepareRequest(S7TCPChannel ch, List<CommJob> ListJobPending, ref List<CommJob> ListJobExec, ref byte[] buffer)
        {
            ...
            uint nRes = 0;
            if (ListJobPending[0].ReadRequest())
                nRes = PrepareReadRequest(ch, ListJobPending, ref ListJobExec, ref buffer);
            else
                nRes = PrepareWriteRequest(ch, ListJobPending, ref ListJobExec, ref buffer);
            return nRes;
        }
        ...
        static uint PrepareWriteRequest(S7TCPChannel ch, List<CommJob> ListJobPending, ref List<CommJob> ListJobExec, ref byte[] buffer)
        {
            ...
            try
            {
                ...
                foreach (S7TCPCommJob j in ListJobPending)
                {
                    ...
                    LastAddedTag = (byte)j.GetTagListOnWritingCount();
                    ...
                    while (j.tmpTagsListToWrite.Count > 0)
                    {
                        ...
                        if (!j.LimitatedGetJobData(ch, ref objectData, nTotalByteAnyPointerAndVarValue + dataOverheadSize, out firstTag))
                        {
                            RequestEnd = true;
