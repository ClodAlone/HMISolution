...
namespace S7TCP
{
    internal class S7TCPStructStringLength
    {
        ...
        public class ProMemberView
        {
            ...
            public ProMemberView(string name, UFUAModel.ModelType modelType) : this()
            {
                Name = name;
                ModelType = modelType;
            }

            public ProMemberView(string name, string path, uint stringLength, UFUAModel.ModelType modelType) : this()
            {                
                Name = name;
                Path = path;
                ...
                if (StringLength == 0)
                    StringLength = S7Protocol.DEFAULTSTRINGLENGTH_TIAPORTAL;
                ModelType = modelType;
        ...
        public S7TCPStructStringLength()
        {
            InitBase();
        }
        ...
        public void Parse(IDynamicSettingsEditing thisTag, string structStringLength)
        {
            ...
            ImportPrototypeMembers(null, thisTag, ref memberV);
        ...
        {
            return (stringLength > 0 && stringLength <= S7Protocol.DEFAULTSTRINGLENGTH_TIAPORTAL);
        }
        ...
        private Dictionary<string, ProMemberView> ParseStructStringLength(string structStringLength, out bool error)
        {
                ...
                if (structStringLength.Substring(0, 1) == STRING_SIZE_SEPARATOR)
                {
                    if (uint.TryParse(structStringLength.Substring(1, structStringLength.Length - 1), out uint Dummy))
                    {
                        ...
                        if (IsStringLengthInRange(Dummy))
                        {
                            ...
                            members = GetMembersAsAList(memberV);
                    ...
                    foreach (string member in structStringLengthMembers)
                    {
                        if (!string.IsNullOrEmpty(member))
                        {
                            ...
                            if (memberData.Length == 2)
                            {
                                ...
                                {
                                    if (!IsStringLengthInRange(dummy))
                                        dummy = S7Protocol.DEFAULTSTRINGLENGTH_TIAPORTAL;
            ...
            if (defaultStringLength == 0 && members.Count == 0)
                defaultStringLength = S7Protocol.DEFAULTSTRINGLENGTH_TIAPORTAL;
        ...
        private void ImportPrototypeMembers(string ancestorPrototypeName, IDynamicSettingsEditing tag, ref ProMemberView memberView)
        {
            ...
            {                
                ...
                {
                    ProMemberView newMemberView = new ProMemberView(member.Name, memberView.FullName, S7Protocol.DEFAULTSTRINGLENGTH_TIAPORTAL, UFUAModel.ModelType.Variable);
        ...
        private Dictionary<string, ProMemberView> MergeStructStringLengthWithPrototypeMembers(string structStringLength)
        {
            ...
            return GetMembersAsAList(memberV);
        }

        private ProMemberView getMemberView(string memberName, ProMemberView memberView, int memberLevel, ref int currentLevel)
        {
            ...
            {
                foundMember = (from m in memberView.Members.AsParallel() where m.FullName.ToLower() == memberName.ToLower() && m.ModelType == UFUAModel.ModelType.Variable select m).FirstOrDefault();
            }
            else
            {
                string memberNameStruct = GetMemberLevel(memberName, currentLevel);
                ProMemberView member = (from m in memberView.Members.AsParallel() where m.Name.ToLower() == memberNameStruct.ToLower() && m.ModelType == UFUAModel.ModelType.ObjectType select m).FirstOrDefault();
                if (member != null)
                    foundMember = getMemberView(memberName, member, memberLevel, ref currentLevel);
            }

            return foundMember;
        }

        private int CalculateTotalMemberLevels(string memberName)
        {
            return memberName.Count(f => f == STRUCT_FIELD_SEPARATOR);
        }

        private string GetMemberLevel(string memberName, int level)
        {
            string subStruct = string.Empty;

            var subStructs = memberName.Split(STRUCT_FIELD_SEPARATOR);
            if (subStructs != null && level < subStructs.Length)
                subStruct = subStructs[level];
        ...
        {
            int memberLevel = CalculateTotalMemberLevels(memberName);
            int currentLevel = -1;

            return getMemberView(memberName, memberV, memberLevel, ref currentLevel);
        }
        ...
        private void getMembersAsAList(ProMemberView memberView, Dictionary<string, ProMemberView> resultMembers)
        {
            if (memberView == null)
                return;
            ...
            foreach (var member in memberView.Members)
            {
                switch (member.ModelType)
                {
                    ...
                    case UFUAModel.ModelType.ObjectType:
                        getMembersAsAList(member, resultMembers);
                        break;
                }
            }            
        }
        ...
        private Dictionary<string, ProMemberView> GetMembersAsAList(ProMemberView memberView)
        {
            ...
            getMembersAsAList(memberView, resultMembers);
        ...
        public string UnSplitToStructString(ProMemberView memberView)
        {
            ...
            S7TCPStructStringLength.ProMemberView Member0 = members.First().Value;
