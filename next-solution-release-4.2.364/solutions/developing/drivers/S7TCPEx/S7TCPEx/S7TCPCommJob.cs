...
namespace S7TCP
{
    ...
    {
        public List<byte> ListRecievedBigArrary = new List<byte>();
        public uint ExecutionOffset;
        public int ExecutionOffsetWrite;
            ...
            : base(station, settings)
        {
            ...
            tmpTagsListToWrite = new List<Tag>();
            _SwapDWords = settings.SwapDWords;
            ExecutionOffset = 0;
            ExecutionOffsetWrite = 0;
        ...
        /// <summary>   The temporary tags list to write. </summary>
        public List<Tag> tmpTagsListToWrite;
        ...
        public bool IsBigArray()
        {
            ...
            bool ret = false;
            uint val = (uint)TagsList[0].TagNode.DataType.Identifier;
            if (TagsList[0].TagNode.ArrayDimension != 0)
            {
                switch(val)
                {
                    case (uint)Opc.Ua.DataTypes.Boolean:
                        {
                            uint byteSize = TagsList[0].Size / 8;
                            if ((TagsList[0].Size % 8) > 0)
                            {
                                byteSize++;
                            }
                    ...
                    case (uint)Opc.Ua.DataTypes.String:
                        break;
                    default:
                        if(TagsList[0].Size > ((S7TCPChannel)Station.GetChannel()).MAX_DATA_BYTES)
                        {
                            ret = true;
                        }
                        break;
            ...
            {
                uint sizeToBeChecked = TagsList[0].Size;
                if(isLenStringEnable == true)
                {
                    sizeToBeChecked++;
                }
        ...
        {
            return (isProtocolBool() && TagsList[0].TagNode.ArrayDimension != 0);
        }
        ...
        public bool LimitatedGetJobData(S7TCPChannel ch, ref object jobData, long RequestSize,out S7TCPTag firstTag)
        {
            ...
            do
            {
                if (cand != null)
                {
                    if(cand.ByteOffset + cand.Size != tmpTagsListToWrite[0].ByteOffset)
                        break;
                }
                else
                    firstTag = (tmpTagsListToWrite[0] as S7TCPTag);

                cand = tmpTagsListToWrite[0];

                if (currentRequestSize >= ch.MAX_TEL_LENGTH && isProtocolBool() && cand.TagNode.ArrayDimension != 0)
                {
                    rt = false;
                    break;
                }
                ...
                {                    
                    ...
                    // special case --> array of bit
                    if ((uint)cand.TagNode.DataType.Identifier == (uint)BuiltInType.Boolean && cand.TagNode.ArrayDimension != 0) {
                        #region special case array of bit
                        _ArrayBoolWriteCheck = true;
                        // have to write value ? 
                        if (Type == LinkType.UnconditionalOutput || (Station.RewritingOfTheSameValue && (Type == LinkType.ExceptionOutput || Type == LinkType.InputOutput))) {                            
                            if (!(cand as S7TCPTag).S7TCPArrayBoolWriteManualCheck)
                                (cand as S7TCPTag).S7TCPInitArrayBoolWriteManualCheck();                            
                        }

                        // get changed bit
                        tsize = cand.GetTagBuffer(ref jobdata, false, 0, (ElementNumber > 0 && !ProtocolDataSizeBig() ? GetProtocolDataByteSize() : 0));
                        // no bit changed
                        if (tsize == unchecked((uint)(-1)))
                        {
                            _ArrayBoolWriteCheck = false;
                            (cand as S7TCPTag).S7TCPResetArrayBoolWriteManualCheck();
                            tmpTagsListToWrite.RemoveAt(0);
                            continue;
                            //rt = false;
                            //break;
                        }
                        ...
                        {
                            // last bit changed ?
                            if ((cand as S7TCPTag).S7TCPArrayBitWriteIsChangedElementLast)
                            {
                                _ArrayBoolWriteCheck = false;
                                // don't full reset all flags
                                (cand as S7TCPTag).S7TCPResetArrayBoolWriteManualCheck(false);
                            }
                            tsize = 1;
                        }
                        #endregion
                ...
                //if ((uint)cand.TagNode.DataType.Identifier == (uint)BuiltInType.Boolean && cand.TagNode.ArrayDimension != 0 && tsize == unchecked((uint)(-1)))
                //{
                //    rt = false;
                //    break;
                //}
                ...
                //if (outData.Count() + jobdata.Length +  RequestSize >= S7Protocol.MAX_MPI_TLG_LEN - 28)
                if (IsBigArray() && (!isProtocolBool()))
                {
                    if((jobdata.Length - ExecutionOffsetWrite) > (ch.MAX_MPI_TLG_LEN - currentRequestSize))
                    {
                        byte[] subArray = new byte[(ch.MAX_MPI_TLG_LEN - currentRequestSize)];
                        Array.Copy(jobdata, ExecutionOffsetWrite, subArray, 0, (ch.MAX_MPI_TLG_LEN - currentRequestSize));
                        Array.Clear(jobdata, 0, jobdata.Length);
                        jobdata = subArray;
                    }
                    else
                    {
                        int size = jobdata.Length - ExecutionOffsetWrite;
                        byte[] subArray = new byte[size];
                        Array.Copy(jobdata, ExecutionOffsetWrite, subArray, 0, size);
                        Array.Clear(jobdata, 0, jobdata.Length);
                        jobdata = subArray;
                    }

                }
                if ( jobdata.Length + currentRequestSize > ch.MAX_MPI_TLG_LEN )
                {
                    rt = false;
                    break;
                }
                ...
                if (!listOnWriting.Contains(tmpTagsListToWrite[0]))
                    listOnWriting.Add(tmpTagsListToWrite[0]);
                if ((uint)cand.TagNode.DataType.Identifier != (uint)BuiltInType.Boolean || cand.TagNode.ArrayDimension == 0 )
                {
                    tmpTagsListToWrite.RemoveAt(0);
                }
            ...
            } while (tmpTagsListToWrite.Count > 0);
        ...
        public void ResetArrayBoolWriteManualCheck()
        {
            if (Type != LinkType.UnconditionalOutput)
            {
                ...
                lock (lockListObject)
                {
                    tagslist.AddRange(TagsList);
                }
                Parallel.ForEach(tagslist, tag =>
                {
                    if (tag.TagNode.DataType.IdType == IdType.Numeric && (uint)tag.TagNode.DataType.Identifier == (uint)Opc.Ua.DataTypes.Boolean && tag.TagNode.ArrayDimension > 0)
                        ((S7TCPTag)tag).S7TCPResetArrayBoolWriteManualCheck();
                });
            }
        }
        ...
        public override uint OnWriteTag(NodeId tagnodeid, ref object value)
        {
            lock (lockListObject)
            {
                ...
                if ((Type == LinkType.ExceptionOutput || Type == LinkType.InputOutput) && !TagsListToWrite.Contains(tag) && !tmpTagsListToWrite.Contains(tag))
                {
                ...
                System.Diagnostics.Debug.WriteLine("- {0}.{1} CommJob.OnWriteTag '{4}' tag:{2} value {3}", DateTime.Now.ToString(),
                                                                                                        ...
                                                                                                        tagnodeid.ToString(), 
                                                                                                        tag.Value.Value.ToString(), Name);
#endif      
                return StatusCodes.Good;
            }
        }

        public override bool DuplicatedTagValueRemoved(NodeId tagnodeid, ref object value)
        {
            ...
            if ((Station.RewritingOfTheSameValue == false) && (Type == LinkType.ExceptionOutput || Type == LinkType.InputOutput))
            {
                lock (lockListObject)
                {
                    ...
                    if (t != null)
                    {
                        ...
                        if (!TagsListToWrite.Contains(t) && !TagsListOnWriting.Contains(t) && !tmpTagsListToWrite.Contains(t))
                        {
                            DataValue objTmp = new DataValue();
                            objTmp.Value = Utils.Clone(value);

                            if (StatusCode.IsGood(t.Value.StatusCode) && (t.LastValue != null) && (t.LastValue.Equals(objTmp.Value)))
                            {
        ...
        public void CleartmpTagsListToWrite()
        {
            lock (lockListObject)
            {
                tmpTagsListToWrite.Clear();
            }
        }

        public void AddtmpTagsListToWrite(List<Tag> tagList)
        {
            lock (lockListObject)
            {
                tagList.ForEach(tag =>
                {
                    if (!tmpTagsListToWrite.Contains(tag))
                    {
                        tmpTagsListToWrite.Add(tag);
                    }
                });
            }
        }

        public void SorttmpTagsListToWrite()
        {
            lock (lockListObject)
            {
                tmpTagsListToWrite.Sort(S7TCPCommJob.CompareTagByOffset);
            }
        }

        public int GettmpTagsListToWriteCount()
        {
            lock (lockListObject)
            {
                return(tmpTagsListToWrite.Count);
            }
        }
        public List<Tag> GettmpTagListToWrite()
        {
            List<Tag> lTags = new List<Tag>();
            lock (lockListObject)
            {
                lTags.AddRange(tmpTagsListToWrite);
            }
            return lTags;
        }
        ...
        /// </summary>
        public void CalculateAddressWithOffsetValues()
        {
        ...
        private bool _ArrayBoolWriteCheck = false;
        public bool ArrayBoolWriteCheck
        {
            get { return _ArrayBoolWriteCheck; }
            set { _ArrayBoolWriteCheck = value; }
        }
        ...
        public uint GetReadResponseLength()
        {
            ...
            if (IsBigArray())
            {
                uint remain = TagsList[0].Size - ExecutionOffset;
                if(isLenStringEnable == true)
                {
                    remain++;
                }
                if (remain > ((S7TCPChannel)Station.GetChannel()).MAX_DATA_BYTES)
                {
                    nBytes = ((S7TCPChannel)Station.GetChannel()).MAX_DATA_BYTES;
                }
                else
                {
                    nBytes = remain;
        ...
        public uint GetWriteRequestLength(ref List<Tag> listToWrite, ref List<Tag> listToWriteLater)
        {
            ...
            if (this.Type == LinkType.UnconditionalOutput)
            {
                lock (lockListObject)
                {
                    ...
                    listToWriteLater.Clear();
                }
                return (HeaderWriteAnyPointer + HeaderWriteVarValue + TotalJobSize);
            }

            if (Area != Step7Area.aT && Trans != Step7WordTrans.wtT)
            {
                List<Tag> actuallist = new List<Tag>();
                bool isBigArrayFlag = false;
                lock (lockListObject)
                {
                    actuallist.AddRange(GetTagListOnWriting()) ;
                    isBigArrayFlag = IsBigArray();
                }
                ...
                listToWrite.Clear();
                listToWriteLater.Clear();
                listToWriteLater.AddRange(actuallist);
                while (i < actuallist.Count)
                {
                    ...
                    if (i < actuallist.Count - 1)
                    {
                        ...
                        {
                            listToWrite.Add(actuallist[i]);
                            listToWriteLater.Remove(actuallist[i]);
                            break;
                        }
                    }
                    listToWrite.Add(actuallist[i]);
                ...
                if ((Format == Step7Format.frmBit))
                {
                    ...
                    if ((size % 8) != 0)
                        size++;
                    size /= 8;
                    // when size > 1, is array of bit with more than 8 elements --> driver will split dinamically on S7Protocol.PrepareWriteData (don't allow to aggregate bit array)                    
                }
                ...
                if (isBigArrayFlag)
                {
                    uint remain = size - ExecutionOffset;
                    if (remain > ((S7TCPChannel)Station.GetChannel()).MAX_DATA_BYTES)
                    {
        ...
        public int GetDataLength()
        {
            ...
            if (IsBigArray())
            {
                uint byteSize = TagsList[0].Size;
                if (TagsList[0].TagNode.DataType == Opc.Ua.DataTypes.Boolean)
                {
                    byteSize = (((uint)(Bit % 8) + byteSize) / 8);
                    if (((uint)(Bit % 8) + byteSize) > 0)
                    {
                        byteSize++;
                    }
                }
                ...
                uint remain = byteSize - ExecutionOffset;
                if (remain > ((S7TCPChannel)Station.GetChannel()).MAX_DATA_BYTES)
                {
                    remain = ((S7TCPChannel)Station.GetChannel()).MAX_DATA_BYTES;
                }
                return (int)remain;
            }
            else
        ...
        public override bool IsReadRWReady()
        {
            lock (lockListObject)
            {
                ...
                (tmpTagsListToWrite.Count != 0 || TagsListOnWriting.Count != 0) && (ProtocolDataSizeBig()));
            }
        }
        public override bool ReadRequest()
        {
            bool bRead = Type == DriverCodeBaseEx.Enumerators.LinkType.Input ||
                (Type == DriverCodeBaseEx.Enumerators.LinkType.InputOutput &&
                (tmpTagsListToWrite.Count == 0 || RWState == RWStates.ReadForRW));
            return bRead;
        }
