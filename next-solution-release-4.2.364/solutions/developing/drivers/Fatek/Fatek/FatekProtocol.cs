...
namespace Fatek
{
    public class FatekProtocol
    {
        ...
        private static string FormatValueToWriteIntoRegister(Tag tag, byte[] data, uint startPosition, int ElementNumber)
        {
        ...
        private static bool GetDataToWriteFormatted(FatekCommJob job, out uint itemCount, out string startAddress, out string dataToWrite)
        {
            ...
            if (FatekProtocol.IsBitDataArea(job.Area))
            {
                if ((uint)job.TagsListOnWriting[0].TagNode.DataType.Identifier == (uint)Opc.Ua.DataTypes.Boolean)
                {
                ...
                else
                {
                    ...
                    areaAddress = (ushort)(job.AreaAddress + job.TagsListOnWriting[0].ByteOffset * 8);
                }
            ...
            else
            {
                if (job.TagsList[0].TagNode.ArrayDimension == 0)
                {
                    ...
                    {
                        dataToWrite += FormatValueToWriteIntoRegister(tag, jobdata, startPosition, job.ElementNumber);
                        startPosition += (tag.Size < 2 ? 2 : tag.Size);
                        itemCount += (uint)(jobdata.Length / PpotocolByteSize) + ((uint)jobdata.Length % 2);
                        if (((job.ElementNumber == 2) && ((uint)job.TagsList[0].TagNode.DataType.Identifier == (uint)BuiltInType.Float)) && (jobdata.Length == 4))
                        {
                            itemCount *= (uint)job.ElementNumber;
                        }
                    }
                }
                else
        ...
        public static DriverErrorCodes ParseAnswer(FatekCommJob job, List<byte> reciveBuffer, out byte[] dataBuffer)
        {
            ...
            if (reciveBuffer.Count < FatekProtocol.PROTOCOL_ANSWER_MIN_SIZE)
                return (DriverCodeBase.Enumerators.DriverErrorCodes)FatekProtocol.FatekErrorCodes.ErrorReceiveFrameError;
            ...
            if (Encoding.ASCII.GetString(reciveBuffer.GetRange(reciveBuffer.Count - 3, 2).ToArray()) != crcX)
                return (DriverCodeBase.Enumerators.DriverErrorCodes)FatekProtocol.FatekErrorCodes.ErrorCRCInvalid;
            #endregion
        ...
        {
            // read command
            job.CommandType = job.GetRequestType();
