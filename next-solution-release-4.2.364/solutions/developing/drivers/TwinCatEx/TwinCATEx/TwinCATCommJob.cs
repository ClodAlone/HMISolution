...
namespace TwinCAT
{
    public class TwinCATCommJob : CommJob
    {
        ...
        public void ResetTwinCatElement()
        {
            // for symbolic address only
            if (!AddressObj.IsNumeric)
            {
            ...
            AddressObj.InvalidErrorCode = AdsErrorCode.NoError;
        }

        public override bool IsJobAggregable()
        {
            return (AddressObj != null && AddressObj.IsNumeric);
        }
        
        public uint GetTotalJobSize(CommJob j)
        {
            uint localTotalJobSize = TotalJobSize;
            if (j.ElementNumber > 0 || j.ProtocolDataSizeBig())
            {
                if (j.TagsList[0].TagNode.ArrayDimension == 0)
                {
                    if (j.ProtocolDataSizeBig())
                    {
                        //localTotalJobSize expresses the number of bytes to request if the job has aggregated,
                        //the byte offset returns the number of bytes minus one.
                        localTotalJobSize = j.GetProtocolDataByteSize() + (uint)j.TagsList[j.TagsList.Count()-1].ByteOffset ;
                    }
                }
                else
                    localTotalJobSize = j.GetProtocolDataByteSize() * j.TagsList[0].TagNode.ArrayDimension;
            }
            return localTotalJobSize;
        }
