...
namespace ModBus
{
    public class ModbusCommJob : CommJob
    {
        #region Constructors
        public ModbusCommJob(Station station, ModbusCommJobSettings settings)
        ...
        public ModbusCommJob(Station station, ModbusTag defTag)
            : base(station, defTag)
        {
            //steve 170611
            /*
             * verify, in case of prototype, the possibility to communicate...
             * 
             */
            ...
            //    {
            //        System.Diagnostics.Trace.TraceInformation(string.Format("member: {2} type:{0} Settings:{1}", tag.DataType.Identifier, tag.DynamicSettings, tag.NodeId.ToString()));
            //    }
            //}



            _FunctionCode = defTag.ModbusDynSettings.FunctionCode;
            _StartAddress = defTag.ModbusDynSettings.StartAddress;
            _FileNumber = defTag.ModbusDynSettings.FileNumber;
            _BroadCast = defTag.ModbusDynSettings.BroadCast;
        ...
        public ModbusCommJob(Station station)
            : base(station)
        {
            // FOGBUGZ 11354
            _LastWrittenSingleCoil = 0;
        ...
        public override JobAggregationType TestAggregateJob(CommJob candJob, out uint ExtraBytes)
        {
            ...
            ModbusCommJob testJob = candJob as ModbusCommJob;
            if (testJob == null)
                return JobAggregationType.JobAggregImpossible;
            ...
            if (startTest >= start && endTest <= end)
            {
                // Modified to solve FOGBUGZ 12116
        ...
        public override bool AggregateJob(CommJob candJob, JobAggregationType AggType, uint ExtraBytes)
        {
            switch(AggType)
            {
                case JobAggregationType.JobAggregFits:
                    ...
                    {
                        var t = new ModbusTag(tag.TagNode, tag.ByteOffset, 0);
                ...
                case JobAggregationType.JobAggregBackward:
                    ...
                    TotalJobSize += ExtraBytes;
                    StartAddress = ((ModbusCommJob)candJob).StartAddress;
                    break;
                default:
                    return false;
            }
