...
#region Methods
        ...
        public Dictionary<ExpandedNodeId, MonitoredItemViewModel> AddAlarmEventMonitoredItem(ExpandedNodeIdCollection list)
        {
            return AddEventMonitoredItem(list, new NodeId[] { ObjectTypeIds.ConditionType },
        ...
        {
            return AddEventMonitoredItem(list, new NodeId[] { ObjectTypeIds.AuditUpdateMethodEventType });
        }

        public Dictionary<ExpandedNodeId, MonitoredItemViewModel> AddSystemEventMonitoredItem(ExpandedNodeIdCollection list)
        {
            return AddEventMonitoredItem(list, new NodeId[] { ObjectTypeIds.SystemEventType });
        }
        ...
        public Dictionary<ExpandedNodeId, MonitoredItemViewModel> AddEventMonitoredItem(ExpandedNodeIdCollection list,
            NodeId[] conditionType, params NodeId[] eventTypeIds)
        {
            var ret = new Dictionary<ExpandedNodeId, MonitoredItemViewModel>();
        ...
        public void RecreateMonitoredItems(Session session)
        {
            ...
            lock (lockObject)
            {
                ...
                foreach (var m in subscription.MonitoredItems)
                {
                    ...
                    mvm.monitoredItem = m;
                    mvm.MonitoringMode = MonitoringMode.Reporting;
                }
                    ...
                    try
                    {
                        ...
                        if (maxNodesPerRead > 0 && nodesToRead.Count > maxNodesPerRead)
                        {
                            for (long i = 0; i < nodesToRead.Count; i += maxNodesPerRead)
                            {
                                ...
                                for (int ii = 0; ii < maxnodeList.Count; ii++)
                                {
                                    ...
                                    if (itemsRange[ii].DataValue != null && 
                                        values[ii].SourceTimestamp == itemsRange[ii].DataValue.SourceTimestamp)
                                        values[ii].ServerTimestamp = itemsRange[ii].DataValue.ServerTimestamp;

                                    if (!values[ii].Equals(itemsRange[ii].DataValue))
                                        itemsRange[ii].DataValue = values[ii];
                                }
                            }
                        }
...
#endif
                        {
                            ...
                            for (int ii = 0; ii < nodesToRead.Count; ii++)
                            {
                                ...
                                if (list[ii].DataValue != null && 
                                    values[ii].SourceTimestamp == list[ii].DataValue.SourceTimestamp)
                                    values[ii].ServerTimestamp = list[ii].DataValue.ServerTimestamp;

                                if (!values[ii].Equals(list[ii].DataValue))
                                    list[ii].DataValue = values[ii];
                            }
                        }
                    }
