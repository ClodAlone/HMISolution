...
namespace ADPluginBase
{
    public class PluginBase : IPlugin, IDisposable
    {
        ...
        public static string GetFileBase(string conn, string drivername)
        {
            ...
            ConnectionStringParser helper = new ConnectionStringParser(connect);
            string filebase = string.Empty;
            if (helper.GetPartByName(DataStoreBase.XpoProviderTypeParameterName) == InMemoryDataStore.XpoProviderTypeString)
                filebase = helper.GetPartByName(DataSourceHeader);
            return filebase;
        }
