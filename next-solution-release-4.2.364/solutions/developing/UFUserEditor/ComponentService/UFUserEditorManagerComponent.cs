...
namespace UFUserEditor.ComponentService
{
    public class UFUserEditorManagerComponent : ComponentBase<IUFUserEditorManager>, IUFUserEditorManager, IDocumentManager, IDisposable
    {
        ...
        {
            var doc = UFUserDocument.FromFile(uri.GetPathString(), this, parent, bCheckShared: bCheckShared);
            if (doc == null)
                return null;
            doc.Parent = parent;
...
#if !NET_STANDARD
                    if (wasDocumentDisposed && listRunningDocuments.Contains(uri.GetPathString()))
                    {
                        doc.StartSharedRepositoriesPollingSyncronization();
                    }
#endif
                }
            }
