    ...
    public class SourceState : BaseObjectState
    {
        ...
        static SourceState()
        {
            ...
            var startupInfo = new STPStartInfo()
            {
                ...
                MaxWorkerThreads = Math.Max(1, currentMaxWkThreads / 4)
            };
        ...
        private IDataLayer CreateDataLayer(AlarmStatus alarmstatus, out IDisposable[] objectsToDisposeOnDisconnect, bool retry = true, string filename = null)
        {
            ...
            try
            {
                conn = XpoHelper.GetConnectionString(dlConnString, Properties.Settings.Default.TypeLabel, filename ?? alarmstatus.Name, Properties.Settings.Default.DefaultFileExt);
                    ...
                    try
                    {
                        using (UnitOfWork ufw = new UnitOfWork(dl))
                        {
                            var alarm = (from persistence in new XPQuery<AlarmPersistence>(ufw)/*.AsParallel()*/
                                         ...
                                         persistence.ParentId == null
                                         select persistence).ToList();

                            if (alarm.Count > 0)
                            {
                                alarmstatus.Comment = alarm[0].Comment;
                                alarmstatus.UserName = alarm[0].UserName;
                                alarmstatus.TagAliasShowValues = alarm[0].TagAliasValue;
                                alarmstatus.Time = alarm[0].TimeStamp;
                                alarmstatus.parentId = alarm[0].ParentId;
                                alarmstatus.Occurence = alarm[0].Occurence;
                                alarmstatus.Sequence = alarm[0].Sequence;

                                alarmstatus.lastValue = alarm[0].Value;
                                alarmstatus.lastQuality = alarm[0].Quality;
                                alarmstatus.lastState = alarmstatus.state;
                                alarmstatus.lastTimeStateChanged = alarmstatus.lastTimeUpdated;
                                alarmstatus.lastTimeUpdated = alarm[0].LastTimeUpdated;

                                if (minTimeStamp == DateTime.UtcNow ||
                                    alarm[0].TimeStamp == DateTime.UtcNow ||
                                    alarm[0].TimeStamp > minTimeStamp)
                                {
                                    alarmstatus.state = alarm[0].State;
                                    alarmstatus.EnableTime = alarm[0].EnableTime;
                                    alarmstatus.AcknowledgeTime = alarm[0].AcknowledgeTime;
                                    alarmstatus.ConfirmTime = alarm[0].ConfirmTime;
                                    alarmstatus.SuppressTime = alarm[0].SuppressTime;
                                    alarmstatus.ActiveTime = alarm[0].ActiveTime;
                                    alarmstatus.ShelvingTime = alarm[0].ShelvingTime;
                                    alarmstatus.TimeOnShelf = alarm[0].TimeOnShelf;
                                    alarmstatus.isOffline = alarm[0].Offline;
                                }
                                else
                            ...
                            alarmstatus.CheckResetCounters(alarm.Count == 0);

                            var alarmbranches = (from persistence in new XPQuery<AlarmPersistence>(ufw)/*.AsParallel()*/
                                                 where persistence.ParentId == alarmstatus.nodeId.ToString()
                                                 select persistence).ToList();
