    ...
    public class TypeInfo : IFormattable
    {
        ...
        public static Type GetSystemType(BuiltInType builtInType, int valueRank)
        {
            if (valueRank == ValueRanks.Scalar)
            {
                ...
                {
#if SILVERLIGHT
                    case BuiltInType.LocalizedText: return typeof(String); 
#endif
...
#if !SILVERLIGHT
                // check for encodeable object.
                if (systemType.IsSubclassOf(typeof(EncodeableObject)))
                {
...
#if !SILVERLIGHT
                // check for encodeable object.
                if (systemType.GetElementType().IsSubclassOf(typeof(EncodeableObject)))
                {
        ...
        {
            TypeInfo sourceType = TypeInfo.Construct(source);
        ...
        public string ToString(string format, IFormatProvider formatProvider)
        {
            if (format == null)
            {
                if (m_valueRank < 0)
                {
                    return String.Format("{0}", m_builtInType);
                }
                
                return String.Format("{0}[{1}]", m_builtInType, m_valueRank);
            }
