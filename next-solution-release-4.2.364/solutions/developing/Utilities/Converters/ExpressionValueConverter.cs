...
namespace Utilities.Converters
{
    ...
    /// A Value converter
    /// </summary>
    public class ExpressionValueConverter : IValueConverter, IDisposable
    {
        #region Declarations
        long lastValue;
        ...
        */
        ...
        public void ParseFormula()
        {
            ...
            if (sFormula.StartsWith(expressionTag))
            {
                ...
                sFormulaParsed = sFormula.Replace(String.Format("{0}{1}{2}", openTag, xTag, closeTag), GetValidConstantName(xTag));
                ...
                {
                    var defName = GetValidConstantName(tagName);
                    sFormulaParsed = sFormulaParsed.Replace(String.Format("{0}{1}{2}", openTag, tagName, closeTag), defName);
                });
                if (!String.IsNullOrEmpty(ParserError))
                {
                ...
                if (!String.IsNullOrEmpty(sReverseFormula) && sReverseFormula.StartsWith(expressionTag))
                {
                    ...
                    sReverseFormulaParsed = sReverseFormula.Replace(String.Format("{0}{1}{2}", openTag, xTag, closeTag), GetValidConstantName(xTag));
                    ...
                    {
                        var defName = GetValidConstantName(tagName);
                        sReverseFormulaParsed = sReverseFormulaParsed.Replace(String.Format("{0}{1}{2}", openTag, tagName, closeTag), defName);
                    });
                }
                else
        ...
        {
            return formula.Replace(openTag, String.Format("\"{0}", openTag))
                .Replace(closeTag, String.Format("{0}\"", closeTag));
        }

        string GetValidConstantName(string varName)
        {
            return String.Format("_{0}", System.Text.RegularExpressions.Regex.Replace(varName, validCharsConstantName, "_"));
        }

        string GetValidConstantValue(string constValue)
        {
            double dvalue;
            if (double.TryParse(constValue, out dvalue))
                    ...
                    case ExpressionType.Expression:
                        {
                            ...
                            try
                            {
                                ...
                                if (listVariables != null && listVariables.Count > calcEngine.DefinedNames.Count - 1)
                                    return null;
                    ...
                    case ExpressionType.Expression:
                        {
                            ...
                            try
                            {
                                ...
                                if (listReverseVariables != null && listReverseVariables.Count > calcEngine.DefinedNames.Count - 1)
                                    return null;
