...
namespace UFEventModel
{
...
#endif
    {
        ...
        public void GetAllSourceEntityReferencesDetails(CRMapsHelper cRMapsHeler, bool bForceOp = false)
        {
            ...
            bool getScreens = cRMapsHeler.CrossReferenceTypes.Contains(CrossReferenceType.Screens);
            var commands = CommandList as CommandManagerList;
            foreach (var command in commands)
            {
                if (getScreens && command is OpenScreenCommand && (command as OpenScreenCommand).ScreenName != null)
                {
                    var listDynamic = GetOpenScreenCommand(command as OpenScreenCommand);
                    cRMapsHeler.ScreenLinks.AddRange(listDynamic);
                }
                if(getTags || bForceOp)
                {
        ...
        public List<object> GetOpenScreenCommand(OpenScreenCommand command)
        {
            List<object> list = new List<object>();
            var dictionary = new Dictionary<string, string>();
            dictionary[string.Format("{0}\\{1}", Properties.Resources.CommandTagHeader, command.Name)] = command.ScreenName.GetPathString();
            list.Add(dictionary);
            return list;
        }
#endif
