...
namespace DriverCodeBaseEx
{
...
#endif
    {
        ...
        public virtual uint OnWriteTag(TagDefinition tag, ref object value, ref StatusCode statusCode, ref DateTime timestamp)
        {
            ...
            if (bRunning)
                return station.OnWriteTag(tag.NodeId, ref value, ref statusCode, ref timestamp);
            else
        ...
        {
            return (OnTagChanged(node, value, default(DateTime), false, disableQualityUpdate));
        }
        ...
        /// <param name="disableQualityUpdate" type="bool"> Disable quality update. </param>
        ////////////////////////////////////////////////////////////////////////////////////////////////////
        public bool OnTagChanged(NodeId node, DataValue value, DateTime Timestamp, bool disableQualityUpdate)
        {
            return OnTagChanged(node, value, Timestamp, false, disableQualityUpdate);
        }
        ...
        /// <param name="disableQualityUpdate" type="bool"> Disable quality update. </param>
        ////////////////////////////////////////////////////////////////////////////////////////////////////
        public bool OnTagChanged(NodeId node, DataValue value, DateTime Timestamp, bool allowNullValues = false, bool disableQualityUpdate = false)
        {
            var temp = TagChanged;
            if (temp != null)
            {
        ...
        public void ManageUpdatedValueForTheStateCommandVariable(DataValue value)
        {
            ...
            driverStateCommandVariable.varDataType = value.WrappedValue.TypeInfo.BuiltInType;
            driverStateCommandVariable.SetValue(value);
...
#region Properties
        ...
        public bool bTerminating
        { 
            get { return _bTerminating;  }
            internal set { _bTerminating = value; }         
        }
#endregion
...
#region Persistence Data
        ...
        private bool LoadDynamicJobs(out List<TagDefinition> invalidTags, IList<TagDefinition> serverTags)
        {
                        ...
                        if ((tags != null && tags.Count > 0) || (structTags != null && structTags.Count > 0))
                        {
                            ...
                            if (jobs.Count > 0)
                            {
                                ...
                                foreach (var jobsettings in jobs)
                                {
                                    ...
                                    {
                                        if (mapStructTags.ContainsKey(jobsettings))
                                        {
            ...
            {
                log.Error(string.Format(Properties.Resources.ErrorLoadingDynamicJobs, DriverName, ex.Message));
                loaded = false;
            }
        ...
        public void SetTagList(List<string> tagMap, ComunicationSettingsContext2 settingsContext)
        {
            ...
            using (IDataLayer idl = DriverCodeBaseEx.CommunicationDriver.GetDriverDataLayer(settingsContext.ConnectionString, DriverName, out string fileBase, out InMemoryDataStore InMemory))
            {
                using (UnitOfWork ufw = new UnitOfWork(idl))
                {
                    ...
                    {
                        // General
                        if (StateCommandVariableTag.IsTagsSet(configuration.StateCommandTag))
                        {
                        ...
                        foreach (var ch in channels)
                        {
                            if (StateCommandVariableTag.IsTagsSet(ch.StateCommandTag))
                            {
                        ...
                        foreach (var st in stations)
                        {
                            if (StateCommandVariableTag.IsTagsSet(st.StateCommandTag))
                            {
