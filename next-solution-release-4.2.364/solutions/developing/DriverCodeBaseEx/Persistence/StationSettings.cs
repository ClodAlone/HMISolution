...
namespace DriverCodeBaseEx
{
    ...
    public abstract class StationSettings : XPObject, IDataErrorInfo, INotifyPropertyChanged
    {
        ...
        {
            if (DriverSettings != null && StateCommandTag != null && !StateCommandTag.IsEmpty())
            {
                var listStationStateTags = (from s in DriverSettings.StationSettings where s != this && StateCommandVariableTag.IsTagsEquals(s.StateCommandTag, StateCommandTag) select s.Name).ToList();
                var listChannelStateTags = (from c in DriverSettings.ChannelSettings where StateCommandVariableTag.IsTagsEquals(c.StateCommandTag, StateCommandTag) select c.Name).ToList();
