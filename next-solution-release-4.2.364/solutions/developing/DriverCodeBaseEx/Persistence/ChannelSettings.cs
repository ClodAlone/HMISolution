...
namespace DriverCodeBaseEx
{
    ...
    public abstract class ChannelSettings : XPObject, IDataErrorInfo
    {
        ...
        {
            if (DriverSettings != null && StateCommandVariableTag.IsTagsSet(StateCommandTag))
            {
                var listStationStateTags = (from s in DriverSettings.StationSettings where StateCommandVariableTag.IsTagsEquals(s.StateCommandTag, StateCommandTag) select s.Name).ToList();
                var listChannelStateTags = (from c in DriverSettings.ChannelSettings where c != this && StateCommandVariableTag.IsTagsEquals(c.StateCommandTag, StateCommandTag) select c.Name).ToList();
                return (listStationStateTags.Count > 0 || 
                    listChannelStateTags.Count > 0 || 
                    (DriverSettings.StateCommandTag != null && StateCommandVariableTag.IsTagsEquals(DriverSettings.StateCommandTag, StateCommandTag))
                    );
            }
            return false;
        }
