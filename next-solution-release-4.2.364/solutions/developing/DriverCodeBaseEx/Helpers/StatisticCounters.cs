...
namespace DriverCodeBaseEx.Helpers
{
    ...
    public sealed class StatisticCounters : IDisposable
    {
        ...
        {
            Variant val = GetValue(key);
            if (val != Variant.Null)
                OnChangedCounter(key, val);
        }
        ...
        public void Update(string key, long newValue, bool send = false)
        {
            ...
            if(updateChangedCounter && send)
                OnChangedCounter(key, new Variant(newValue));

        }

        public void Update(string key, bool newValue, bool send = false)
        {
            bool updateChangedCounter = false;
            lock (lockObject)
            {
        ...
        public void Update(string key, string newValue, bool send = false)
        {
            bool updateChangedCounter = false;
            lock (lockObject)
            {
        ...
        public void IncreaseCounter(string key, long value = 1, bool send = false)
        {
            bool updateChangedCounter = false;
            long newValue;
            lock (lockObject)
        ...
        public void DecreaseCounter(string key, long value, bool send = false)
        {
            bool updateChangedCounter = false;
            long newValue;
            lock (lockObject)
