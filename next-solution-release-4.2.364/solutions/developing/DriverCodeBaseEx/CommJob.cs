...
namespace DriverCodeBaseEx
{
    ...
    public abstract class CommJob
    {
            ...
            : this(station)
        {
            ...
            if (StateCommandVariableTag.IsTagsSet(_ConditionalVariableName, _ConditionalVariableId))
            {
            ...
            if (StateCommandVariableTag.IsTagsSet(_OffsetVariableName, _OffsetVariableId))
            {
        ...
        private bool _IsChildJob = false;
        public bool IsChildJob
        {
            get { return _IsChildJob; }
        ...
        ////////////////////////////////////////////////////////////////////////////////////////////////////////////
        public virtual void UpdateTagsListOnWriting()
        {
            lock (lockListObject)
            {
                ...
                {
                    if (TagsListOnWriting.Count != TagsList.Count)
                    {
                        TagsListOnWriting.Clear();
        ...
        public virtual void UpdateTagsListToWrite()
        {
            lock (lockListObject)
            {
                if (Type == LinkType.Input)
        ...
        public virtual void ManageUpdatedValueForTheConditionalVariable(DataValue value)
        {
            ...
            {
                jobConditionalVariable.varDataType = value.WrappedValue.TypeInfo.BuiltInType;
                jobConditionalVariable.SetValue(value);
        ...
        {
            jobOffsetVariable.SetValue(value);
        }
        ...
        public virtual void ResetConditionalVariable()
        {
            ...
            {                
                //uint uintValue = 0;
                var newVal = new DataValue(GetVarianForConditionalVariable(jobConditionalVariable.varValue.WrappedValue.TypeInfo.BuiltInType));
                jobConditionalVariable.SetValue(newVal);
                
                Station.GetCommDriver().OnTagChanged(jobConditionalVariable.varNodeId, newVal);
            }
        }
        ////////////////////////////////////////////////////////////////////////////////////////////////////
        ...
        public virtual bool SetInUse(NodeId node, bool inUse, double samplinginterval = -1)
        {
            ...
            lock (lockListObject)
            {
                ...
                listaordinata = (from taguse in TagsList.AsParallel()
                                 where taguse.InUse == true
        ...
        public virtual bool GetCustomJobsChangedTags(ref List<object> items)
        {
            ...
            {
                if (tag.IsChildReadValueChanged)
                {
                    tag.IsChildReadValueChanged = false;
                    items.Add(tag);
                }
            }
        ...
        /// <summary>
        /// Reset child job of Big job before each execution
        /// </summary>
        public virtual void InitChildJobBeforeExecution()
        {
            IsPending = true;
            InErrorState = false;
            ChildJobErrorCode = DriverErrorCodes.ErrorNoError;
            foreach (Tag t in TagsList)
                t.InitChildReadValueBeforeExecution();
        }
