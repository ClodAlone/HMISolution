...
namespace Microsoft.Samples.Kinect.InteractionGallery.Utilities
{
    ...
    public class WindowBezelHelper : DependencyObject
    {
        ...
        /// </param>
        static WindowStyle oldStyle = WindowStyle.None;
        static Cursor oldCursor = Cursors.None;
        public static void UpdateBezel(Window window, bool showBezel, bool bChangeStyle = true)
        {
            if (window == null)
            {
            ...
            {
                if (bChangeStyle)
                    window.WindowStyle = oldStyle;
                window.Cursor = Cursors.Arrow;
                if (oldCursor != Cursors.None)
