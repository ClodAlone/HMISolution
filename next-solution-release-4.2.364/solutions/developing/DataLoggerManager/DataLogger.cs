...
#region Public Methods
        ...
        public void AddDataLoggerEntry(DataLoggerEntity entity)
        {
            ...
            lock (lockObject)
            {
                ...
                if (dataLoggerState.EnableFlag && dataLoggerState.ValidFlag)
                {
                    try
                    {
        ...
        public void UpdateDataLoggerEntry(DataLoggerEntity entity, params string[] columnNamesChanged)
        {
            if (bDisposed || ExitMode)
                return;
...
#endif
                {
                    CheckRecordOnDataChange(entity, columnNamesChanged);
                }
            }
        }
#endregion
...
#region Private Methods
        ...
        void CheckRecordOnDataChange(DataLoggerEntity entity, params string[] columnNamesChanged)
        {
            lock (lockObject)
            {
                var dataLoggerState = mapDataLoggerState[entity.dataLoggerName];
        ...
        void AddRecordingTimeEntry(DataLoggerEntity entity)
        {
            if (bDisposed || ExitMode)
                return;
        ...
        {
            return String.Format("{0}.{1}", dataLoggerState.DataProvider.ToLower(), dataLoggerState.Connection.ToLower());
        }
