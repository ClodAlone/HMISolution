...
namespace WebNExTHMI.PlatformComponents
{
    ...
    static public class PlatformComponents
    {
        ...
        {
            return (int)projectDocument.StartType;
        }
        ...
        static List<string> screensInfo;
        ...
        static internal List<string> GetProjectScreensList()
        {
            if (screensInfo != null)
                return screensInfo;

            screensInfo = new List<string>();
            projectDocument.GetScreenControllers(ScreenComponent).ForEach(controller =>
            {
        ...
        static public ServerAuditDataItemModel LoadHistoricalViewerData(string connection, bool showAuditTrace, List<string> historicalList, string selecteditem, int maxRows, int commandTimeout, DateSpan rangeType, long? startTimestamp, long? endTimestamp, string sessionString, CancellationToken ct)
        {
            ...
            foreach (var entry in _list)
            {
                ...
                var _entry = new AuditDataItemModel()
                {
                    ...
                    RecordDateTime = entry.RecordDateTimeUtc.ToLocalTime().ToString("G"),
        ...
        static public StatesChartConfigData InitStatesChartHistoricalData(string connString, string historicalname, bool bDlrSource, string sessionString)
        {
            //InitServerDocument
            ...
            {
                InitHistorianConnections(sessionString, connString);
                lock (MapToHistoricalConnectsions)
                {
                    scData.connectionString = (MapToHistoricalConnectsions != null && !string.IsNullOrEmpty(historicalname) && MapToHistoricalConnectsions.ContainsKey(historicalname) && !string.IsNullOrEmpty(MapToHistoricalConnectsions[historicalname])) ? MapToHistoricalConnectsions[historicalname] : connString;
                }
            }
        ...
        static public AggregatedValues LoadHistoricalData(string connString, int commandTimeout, long? startTimestamp, long? endTimestamp, bool usesourcetimestamp, int maxRecord, bool aggregate, int maxaggregationfactor, int maxDeadLockRetry, bool localize, TimeSpan viewTimeFrame, bool minAggregation, bool maxAggregation, bool avgAggregation, string tagName, string columnTagName, Dictionary<string, Tuple<string, string>> parametersMap, string serieTitle, string nodeID, string historicalname, bool bDlrSource, string sessionString, string conditionalString, System.Threading.CancellationToken ct)
        {
            ...
            if (!bDlrSource)
            {
                var _dl = GetHistoricalConnectionStringDataLayer(historicalname, sessionString, commandTimeout);
                if (_dl == null)
