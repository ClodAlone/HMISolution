...
function buildAnimationChildren(animation, control, controlNode) {
    if (!animation.children) {
        ...
        animation.children = filledNodes;
    }
}
    ...
    execute: function (animation, control, axis) {
        ...
        if (animation.Autoreverse)
            animation[propAnimation].after(function (sit) {
                ...
                animator.animate(animation.AnimationTime).transform(newTransform, true);
