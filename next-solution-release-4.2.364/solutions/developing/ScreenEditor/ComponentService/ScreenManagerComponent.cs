       ...
       public static ScreenManagerComponent screenManagerComponent { get; protected set; }
        ...
        private void GetComponentInterfaces()
        {
            ...
            if (PropertyControl != null)
            {
                ...
                dt = new DataTemplate();
                factory = new FrameworkElementFactory(typeof(PropertyDataTemplate.ListItemsSourcePropertyEditor));
                dt.VisualTree = factory;
...
#region IDocumentManager Members
        ...
        public void Delete(Uri uri, IDocument parent)
        {
            ...
            using (new WaitCursor())
            {
                lock (lockObject)
                {
                    ...
                    ScreenDocument.RemoveFile(uri.GetPathString(), parent.fileSystemProviderBase);
                }
            }
        }
        ...
        public bool OpenMap(IDocument parent, ExecutionMode mode, double X, double Y, 
            bool IsRelative, IEntityReference Entity, Rect zoomTo, bool enableZoomingScrolling,
            bool showMiniMap, bool showNextButton)
        {
            ...
            if (mapMainWindows.ContainsKey(rootParent))
            {
                ...
                {
                    mapMainWindows[rootParent].Dispatcher.Invoke((Action)(() => OpenMap(parent, mode, X, Y, IsRelative, Entity, zoomTo, enableZoomingScrolling,
                                                                                showMiniMap, showNextButton)));
                    return true;
                }
            }
            ...
            else if (activeView is ScreenTransitionViewer)
            {
                ...
                containerView.OpenMap(rootParent, zoomTo, enableZoomingScrolling,
                        showMiniMap, showNextButton);
            }
                        ...
                        var cRMapsHeler = new CRMapsHeler() { UpdateStrings = updateStrings };
                        {
                            ...
                            Parallel.ForEach(cRMapsHeler.ScreenLinks.Keys, reference =>
                            {
                                ...
                                foreach (var collectionref in cRMapsHeler.ScreenLinks[reference])
                                {
                                    ...
                                    {
                                        var rpath = refscreendetails.First().Value.Remove(refscreendetails.First().Value.LastIndexOf(".xaml"));
            ...
            try
            {
...
#endif
                {
                    ...
                    for (int i = 0; i < model.ResourceList.Count(); i++)
                    {
                        ...
                        if (p.fileSystemProviderBase != null && resource.StartsWith(p.rootBaseDB))
                            ...
                            using (ScreenDocument doc = CreateDocument(model.Parent, uri))
                            {
                                ...
                                {
                                    errorMessages = true;
                                    logDeploy.Error(string.Format(Properties.Resources.ErrorExportingDocument, doc.Title));
                                }
                            }
                    ...
                    if(screenobjects.Count > 0)
                    {
                        ...
                        {
                            errorMessages = true;
                            logDeploy.Error(string.Format(Properties.Resources.ErrorExportingDocument, p.Title));
                        }
                    }
