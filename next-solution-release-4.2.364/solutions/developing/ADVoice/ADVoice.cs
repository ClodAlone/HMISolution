...
namespace ADVoice
{
    ...
    public class ADVoice : PluginBase
    {
        ...
        //List<ADPluginBase.Message> ListMessagesWaiting = new List<ADPluginBase.Message>();
        ...
        public override int SendMessage(object msg)
        {
            ...
            /**/
            // Consistecy chek on messages, if appropriate...
            /**/
            lock(lockListObj)
            {
                if (messaggio != null)
        ...
        private void CloseCallResources()
        {
            ...
            if (_call != null)
            {
                ...
                //_call.DtmfStarted -= _call_DtmfStarted;
            }
        ...
        //void UpdateMessage(VoiceMessage msg)
        //{
        //    lock(lockListObj)
        //    {
        //        if (!string.IsNullOrEmpty(msg.GroupId))
        //        {
        //            //get all the grouped messages...
        //            var list = (from m in ListMessagesWaiting
        //                        where m.GroupId == msg.GroupId
        //                        select m).ToList();
        //            foreach (var m in list)
        //            {
        //                currentMessage.Add(m);
        //                ListMessagesWaiting.Remove(m);
        //            }
        //        }
        //        else
        //        {
        //            currentMessage.Add(msg);
        //            ListMessagesWaiting.Remove(msg);
        //        }
        //    }
        //}
            ...
            while (true)
            {
                ...
                switch(currentState)
                {
                    ...
                    case VoiceStates.StateWaitIntro:
                        if (callDTMFReceivedEvent.WaitOne(0)/*DTMFReceived != DTMFNONE*/)
                        {
                            callDTMFReceivedEvent.Reset();
                            StopSpeak();
                            currentState = VoiceStates.StateIntroMessagePause/*StateMessage*/;
                    ...
                    case VoiceStates.StateWaitMessage:
                        ...
                        //        //OnSystemEvent(null, String.Format(Properties.Resources.MessageSent, currentMessage.Text, currentMessage.Addresses[currAddress]),
                        //        //    EventSeverity.Min, null, null, null, (int)System.Diagnostics.EventLogEntryType.Information,
                        //        //    (int)LoggerDestination.AlarmDispatcher);
                        //        break;
                        //    }
                        ...
                        if (ttsSpeechStoppedEvent.WaitOne(0))
                        {
                            ...
                            //System.Threading.Thread.Sleep(500);
                            //StartSpeak();
                            currentState = VoiceStates.StateMessagePause/*StateMessage*/;
                            lastPauseTime = DateTime.Now;
                        }
                        break;
                    ...
                    case VoiceStates.StateListMessagesToAck:
                        ...
                        //AcknowledgeMessage(currentMessage, currAddress);
                        currentState = VoiceStates.StateBye;
                        break;
                    case VoiceStates.StateAckMessages:
                        if(listToAck.Count > 0)
                        {
                            foreach(var m in listToAck)
                            {
                                ...
                                {
                                    //back in queue again, to  send to remmaining recipients
                ...
                if(currentState == VoiceStates.StateWaitIntro || 
                    currentState == VoiceStates.StateWaitMessage || 
                    currentState == VoiceStates.StateWaitNext ||
                    currentState == VoiceStates.StateWaitBye)//precisare condizioni
                {
                    if (((DateTime.Now - lastOperationTime).TotalMilliseconds / 1000) > TimeOut)
                    {
                        //scaduto, abort this send operation
                        StopSpeak();
                        if (currentState == VoiceStates.StateWaitIntro)
                        {
                            currentState = VoiceStates.StateCallFailed;
        ...
        bool InitVoIP()
        {
            ...
            if(!VoIPOnLine)
            {
                ...
                if (_textToSpeech == null)
                {
                    ...
                    //_textToSpeech.MediaDataSent += _textToSpeech_MediaDataSent;
                    _textToSpeech.Stopped += _textToSpeech_Stopped;
                }
        ...
        {
            //System.Diagnostics.Trace.TraceInformation("line_RegStateChanged {0} to {1} err:{2}", e.PrevState, e.State, e.Error);
        ...
        //private void _call_DtmfStarted(object sender, VoIPEventArgs<DtmfInfo> e)
        //{
        ...
        private void _textToSpeech_MediaDataSent(object sender, AudioData e)
        {
            //SpeechSent = true;
        }
