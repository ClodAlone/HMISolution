...
#region Methods
        ...
        public ExecutionResult Execute(Variant[] values)
        {
            ...
            if (CanExecute())
            {
                var monitoredItems = (from c in listTagIOEntityReference
                                      where c.MonitoredItemViewModel != null
                                      select c.MonitoredItemViewModel).ToList();

                if (monitoredItems.Count != values.Length)
                    return executionResult;

                try
                {
                    bExecuting = true;
                    executionResult.OutputValues = new Variant[monitoredItems.Count];
                    for (int ii = 0; ii < listTagIOEntityReference.Count; ii++)
                    {
                        if (bWrite)
                            monitoredItems[ii].WriteValue(values[ii].Value);
                        else
                        {
                            if (!IsGoodDataValue(monitoredItems[ii].DataValue))
                                return executionResult;
                            executionResult.OutputValues[ii] = new Variant(monitoredItems[ii].DataValue.Value);
                        }
                    }
