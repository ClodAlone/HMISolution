...
#region Public Methods
        ...
        public DataSet CreateDataSet(CancellationToken token, bool fill, out String fillError, bool bContinueOnError = false)
        {
            ...
            if (fill)
                fillError = FillDataSet(ds, token, bContinueOnError);
                            ...
                            using (var dbdapater = DataReader.DataReader.CreateDbDataAdapter(dataProvider))
                            {
                                using (var cmdBuilder = DataReader.DataReader.CreateDbCommandBuilder(dataProvider))
                                {
                                    ...
                                    for (int cc = 0; cc < ds.Tables.Count; cc++)
                                    {
                                        ...
                                        dbdapater.SelectCommand.CommandText = String.Format("SELECT * FROM {0}", ds.Tables[cc].TableName);
        ...
        public void AcceptUpdateData(DataSet dataSet)
        {
            AcceptUpdateData(dataSet, Guid.Empty);
        }

        public void AcceptUpdateData(DataSet dataSet, Guid guid, string userName = null, string userComment = null)
        {
            if (recipeUAConnector != null)
            {
            ...
            else
            {
                using (var writer = new DataWriter.DataSetWriter(dataProvider, connectionString))
                {
                    ...
                    for (int cc = 0; cc < dataSet.Tables.Count; cc++)
                    {
                        ...
                        bool bAuditTrace = RecipeDocument.RecipeEntity.AuditTraceEnabled && dataView.Table.TableName == DataSetHelper.TableName(RecipeDocument.RecipeEntity);
