...
namespace PropertyControl.Localization
{
    ...
    public sealed class LocalizablePropertyDescriptor : PropertyDescriptor
    {
        ...
        internal void UpdateLocalizationResourceFiles()
        {
            ...
            var key = BasePropertyDescriptor.Name;
            if (!String.IsNullOrEmpty(DisplayName))
                items[key] = DisplayName;
            else
                items[key] = BasePropertyDescriptor.DisplayName;

            key = String.Format("{0}_Help", BasePropertyDescriptor.Name);
            if (!String.IsNullOrEmpty(Description))
                items[key] = Description;
            else
                items[key] = items[BasePropertyDescriptor.Name];

            key = String.Format("{0}_Category", BasePropertyDescriptor.Name);
