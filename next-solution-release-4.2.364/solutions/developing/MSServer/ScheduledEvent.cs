...
#region Methods
        ...
        public void Scheduling(DateTime present)
        {
            ...
            if (!Enable || !RuntimeEnable || IsOnHoliday(present))
                return;
        ...
        void SetOn(bool write = true)
        {
            ...
            bool setVar = (write && !string.IsNullOrEmpty(ValueOn));
            dofirstexecution = false;
            nodeManager.UpdateSchedulerVariable(NodeId, new Opc.Ua.DataValue(true));
            executed = (setVar ? WriteOPCItemValue(ValueOn) : true);
            LastExecutionTime = DateTime.Now;
            SaveSchedulerStatus(LastExecutionTime);

            string evDetails = string.Empty;
            if (!string.IsNullOrEmpty(ValueOn) && HasCommandOn)
            {
                evDetails = string.Format(Properties.Resources.And,
                    string.Format(Properties.Resources.VariableSet, ValueOn),
                    Properties.Resources.CommandListExecuted);
            }
            else if (!string.IsNullOrEmpty(ValueOn))
            {
                evDetails = string.Format(Properties.Resources.VariableSet, ValueOn);
            }
            else if (HasCommandOn)
            {
        ...
        void SetOff(bool write = true)
        {
            ...
            bool setVar = (write && !string.IsNullOrEmpty(ValueOff));
            dofirstexecution = false;
            nodeManager.UpdateSchedulerVariable(NodeId, new Opc.Ua.DataValue(false));
            executed = (setVar ? !WriteOPCItemValue(ValueOff) : false);
            LastExecutionTime = DateTime.Now;
            SaveSchedulerStatus(LastExecutionTime);

            string evDetails = string.Empty;
            if (!string.IsNullOrEmpty(ValueOff) && HasCommandOff)
            {
                evDetails = string.Format(Properties.Resources.And,
                    string.Format(Properties.Resources.VariableSet, ValueOff),
                    Properties.Resources.CommandListExecuted);
            }
            else if (!string.IsNullOrEmpty(ValueOff))
            {
                evDetails = string.Format(Properties.Resources.VariableSet, ValueOff);
            }
            else if (HasCommandOff)
            {
                evDetails = Properties.Resources.CommandListExecuted;
            }
                ...
                { 
                    ...
                    //if (status.Count > 0)
                    {
                        ...
                        if (Type == ScheduleType.calendar || Type == ScheduleType.weeklyPlan)
                        {
                            ...
                            if (lista != null && lista.Count > 0)
                            {
                                foreach (var si in lista)
                                {
                                    ...
                                    {
                                        if (CheckInsideDates(LastExecutionTime, si))
                                        {
                                            dofirstexecution = ExecOnAtStartup/*true*/;
                                            break;
                                        }
                        ...
                        else
                        {
                            ...
                            if (executed && DateTime.Now > off)
                            {
                                dofirstexecution = true;
                            }
                            else if (!executed && (DateTime.Now > on && DateTime.Now < off))
                            {
                                dofirstexecution = true;
                            }
                        }
