...
namespace UFWebClient.Service
{
    public class ScreenHub : Hub
    {
        ...
        public ElementData Register(int width, int height, bool bMobile, String user, String password)
        {
            return Register(width, height, bMobile, user, password, 0);
        }

        public ElementData Register(int width, int height, bool bMobile, String user, String password, int clientTimezoneOffset)
        {
            var session = GetSession();
            if (session == null)
                return null;
            ...
            if (UFWebClientService.UFUserEditorComponent.GetEnableUserManager(UFWebClientService.projectDocument))
            {
                try
                {
                    ...
                    if (bValidated)
                    {
                        ...
                        session.ScreenSink.Role = UFWebClientService.UFUserEditorComponent.GetUserRole(UFWebClientService.projectDocument, session.ScreenSink.User);
                        session.ScreenSink.AccessMask = UFWebClientService.UFUserEditorComponent.GetUserAccessMask(UFWebClientService.projectDocument, session.ScreenSink.User);
                        session.ScreenSink.AccessLevel = UFWebClientService.UFUserEditorComponent.GetUserAccessLevel(UFWebClientService.projectDocument, session.ScreenSink.User);
