    ...
    public class StorageHelper
    {
        ...
        public static Dictionary<string, T> LoadMemoryMaps<T>(IDocument document, string defaultName) where T : class
        {
            ...
            try
            {
                ...
                var di = new DirectoryInfo(Path.Combine(document.GetSpecialFolder(SpecialFolders.Documents).OriginalString, subFolder));
                var settingsFullFilePrefix = $"{di.FullName}{Path.DirectorySeparatorChar}{settingsFilePrefix}";
                var searchPattern = $"{settingsFilePrefix}*{Properties.Settings.Default.StorageFileExtension}";

                foreach (var settingsFile in di.GetFiles(searchPattern))
                {
                    var userName = "";
                    try
                    {
                        userName = Path.GetFileNameWithoutExtension(settingsFile.FullName.Substring(settingsFullFilePrefix.Length));
                        if (!String.IsNullOrEmpty(userName))
