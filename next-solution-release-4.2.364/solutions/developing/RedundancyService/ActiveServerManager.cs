...
namespace RedundancyService
{
    ...
    public class ActiveServerManager : Observable, IRedundancyService, IDisposable
    {
            ...
            if (bIsStarting)
            {
                ...
                for (int i = 0; i < activeServers.Length; ++i)
                {
                    if (activeServers[i] != hostName)
                    {
                        ...
                        lock (locktcpObject)
                        {
                            try
                            {
                                ...
                                clientTcp = ChannelFactory<IRedundancyService>.CreateChannel(
                                    new BinaryNetTcpBinding(NetTcpSecurityMode) { MaxReceivedMessageSize = MaxReceivedMessageSize },
                                    new EndpointAddress(address));
                                ((IContextChannel)clientTcp).OperationTimeout = SynchronizeTimeout;
                        ...
                        if (!SkipHistoryDataSynchronization)
                            OnSynchronizeHistoryData(historySettings, DateTime.MinValue, startTimeActivation, server);
                ...
                lock (locktcpObject)
                {
                    ...
                    timer = new Timer((o) =>
                    {
                        try
                        {
                            ...
                            if (pendingPings == 1 && !bDisposed && !bStopping && !IsActiveServer)
                            {
                                lock (locktcpObject)
                                {
                                    if (errorCounter < 3)
                                    {
                                        ...
                                        catch (Exception ex)
                                        {
                                            ...
                                            if (bRecreateChannelOnPingError)
                                            {
                                                CleanClientTcp();
                                                clientTcp = ChannelFactory<IRedundancyService>.CreateChannel(
                                                    new BinaryNetTcpBinding(NetTcpSecurityMode) { MaxReceivedMessageSize = MaxReceivedMessageSize },
        ...
        public Opc.Ua.ServiceResult SendCallingMethod(CallMethod callMethod)
        {
            lock (locktcpObject)
            {
                if (clientTcp == null)
        ...
        bool OnSynchronizeHistoryData(HistorySettings hs, DateTime st, DateTime et, String server)
        {
        ...
        Opc.Ua.ServiceResult OnCallingMethod(CallMethod callMethod)
        {
            var t = CallingMethod;
            if (t != null)
            {
        ...
        public Opc.Ua.ServiceResult CallMethod(CallMethod callMethod)
        {
            try
            {
