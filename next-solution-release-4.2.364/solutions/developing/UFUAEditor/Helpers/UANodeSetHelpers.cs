...
{    
    ...
    public partial class UANodeSet: IDisposable
    {
        ...
        public ImportExportResult Import(UFUAServerDocument document)
        {
            ...
            if (Items.Count() > 0)
            {
                ...
                workspace?.UpdateProgressState(0,
                                               rootTags.Count + rootUAMethods.Count + rootObjects.Count + rootAlarmObjects.Count,
        ...
        private UAObject GetRootFolder(string rootKind)
        {
            ...
            if (prototype == null)
            {
                ...
                if (listNames != null && listNames.Contains(tagName))
                {
                    ...
                    if (tag != null)
                    {
                        ...
                        {
                            workspace?.ResetBusy();
                            dialogRetValue = document.AskCreateVariable(string.Format(Properties.Resources.ImportTagNameExist, objectFullName), Properties.Resources.ImportTagInProgress);
        ...
        private void CreateObject(UAObject node, UFUAModel.UFUAFolder root = null, UFUAModel.UFUATagPrototype prototypeModel = null)
        {
            ...
            if (refType == ExportAlias(folderType))
            {
        ...
        private UFUAHistorianSettings CreateHistorian(UAObject typedef)
        {
            ...
            if (historianprototype != null)
            {
                ...
                {
                    workspace?.ResetBusy();
                    dialogRetValue = document.AskCreateVariable(string.Format(Properties.Resources.ImportPrototypeNameExist, typedefName), Properties.Resources.ImportTagInProgress);
                    workspace?.RestoreBusy();
                }
        ...
        private UFUAEngineeringUnit CreateEUnit(UAVariable euRange, UAVariable instrumentRange, UAVariable engineeringUnit)
        {
            ...
            if (eUnit != null)
            {
                ...
                {
                    workspace?.ResetBusy();
                    dialogRetValue = document.AskCreateVariable(string.Format(Properties.Resources.ImportEUnitExists, typedefName), Properties.Resources.ImportTagInProgress);
                    workspace?.RestoreBusy();
                }
