...
#endif
    {
        ...
        private void GetComponentInterfaces()
        {
            ...
            if (PropertyControl != null)
            {
                ...
                dt = new DataTemplate();
                factory = new FrameworkElementFactory(typeof(WPFUtilities.PropertyDataTemplate.ExpressionPropertyEditor));
        ...
        public IEnumerable<String> GetFlatListTags(IDocument parent)
        {
            var doc = GetOrCreateDocument(parent);
            if (doc == null)
                return null;
            try
            {
                return doc.GetFlatTagNameCollection();
            }
            catch (DevExpress.Xpo.DB.Exceptions.UnableToOpenDatabaseException ex)
            {
#if !NET_STANDARD
        ...
        public IDictionary<String, IList<String>> GetFlatListPrototypes(IDocument parent)
        {
            var doc = GetOrCreateDocument(parent);
            if (doc == null)
                return null;
        ...
        public IDictionary<String, String> GetFlatListPrototypeInstances(IDocument parent)
        {
            var doc = GetOrCreateDocument(parent);
            if (doc == null)
                return null;
        ...
        public String GetTagEntityReference(IDocument parent, String tagName, String instance, bool inExecution = false, bool useCachedUow = false)
        {
            var p = DocumentHelper.GetRootParent(parent, traverse: false);
            ...
            try
            {
                var tag = doc.GetTagOPCUAEntityReference(tagName, instance, inExecution: inExecution || useCachedUow);
                if (tag == null)
                    return null;
                var ret = tag.ToXml();
                ...
                if (doc != null)
                {
                    ...
                    if (getConnections || getTags)
                    {
                        ...
                        for (int i = 0; i < historicalSettings.Count(); i++)
                        {
                            ...
                            if (getTags && historian.EnableTag != null && !historian.EnableTag.IsEmpty())
        ...
        private void AddCommandsTag(string endpointUrl, UFInterfaces.Editors.CrossReferenceModel model, string applicationName, IDocument doc, string descr, string commands, List<UFInterfaces.Editors.CrossReferenceResultModel> result, string name, string pathString, string tagType)
        {
            ...
            if (!string.IsNullOrEmpty(commands))
            {
                ...
                for (int i = 0; i < commandList.Count(); i++)
                {
                    ...
                    if (getScreens && command is CommandManager.OpenScreenCommand && (command as CommandManager.OpenScreenCommand).ScreenName != null)
                    {
                        string relativePath = (command as CommandManager.OpenScreenCommand).ScreenName.GetPathString();
                        relativePath = relativePath.Remove(relativePath.LastIndexOf(".xaml"));
                        var screenname = relativePath.Split('/').LastOrDefault();
                        result.Add(new UFInterfaces.Editors.CrossReferenceResultModel()
                        {
