    ...
    internal class UFDataValueEntityMap : UFBaseEntityMap<UFDataValueEntity>
    {
        ...
        public void Merge(UFDataValueEntity csvEntity, UFDataValueEntity entity)
        {
            ...
            if (document != null)
            {
                ...
                {
                    var tag = GetTagEntityReference(document, csvEntity.TagDataValue);
                    if (tag != null)
                        entity.TagDataValue = tag.FromXml<OPCUAEntityReference>();
                }
                if (csvEntity.TagIODataValue != null)
                {
                    var tag = GetTagEntityReference(document, csvEntity.TagIODataValue);
                    if (tag != null)
                        entity.TagIODataValue = tag.FromXml<OPCUAEntityReference>();
                }
            }
        }
    }
