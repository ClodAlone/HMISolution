...
{   
    ...
    public class CREditorDocument : ViewModelBase, IDocument, ICloneable
    {
        ...
        internal string GetRelativePath(string value)
        {
            ...
            {
                string newValue = NamespaceTableConverter.GetSanitizedValue(value);
                if (newValue.StartsWith("Tags/", StringComparison.OrdinalIgnoreCase))
                    newValue = newValue.Remove(0, 5);
                if (newValue.StartsWith("Tag/", StringComparison.OrdinalIgnoreCase))
                    newValue = newValue.Remove(0, 4);
                if (newValue.StartsWith("Screen/", StringComparison.OrdinalIgnoreCase))
                    newValue = newValue.Remove(0, 7);
        ...
        internal void AddEntities(List<UFInterfaces.Editors.CrossReferenceResultModel> rawdata, CancellationToken ct)
        {
            ...
            while (!ct.IsCancellationRequested && rawdata.Count > 0)
            {
                ...
                {
                    string relativePath = GetRelativePath(rawdata[0].RelativePath);
                    ...
                    if (aggregated.Count > 0)
                    {
                        if(rawdata[0].CReferenceType == CrossReferenceType.Tags && !string.IsNullOrEmpty(relativePath))
                        {
                            ...
                            if (dataSincInterfaces.Contains(appName))
                            {
                                ...
                                relativePath = mapDataSinkInterfaces.ContainsKey(appName) ? mapDataSinkInterfaces[appName].DataSynkName + "\\" + readablePathNoProject : string.Empty + "\\" + readablePathNoProject;
                        ...
                        else if (rawdata[0].CReferenceType == CrossReferenceType.Screens)
                        {
        ...
        internal void UpdateItemExistence(CRTag item)
        {
            ...
            else
            {
                ...
                if (EditorManagerComponent.UFUAEditorManager.CheckVariable(Parent, _readablepath, item.EndpointUrl, prototypelist, out tagFound))
                {
                    dynamicList.Add(_readablepath);
                    item.IsNotValid = true;
                    return;
        ...
        internal void UpdateItemExistence(CRScreen item)
        {
            if(screenList == null)
            {
                ...
                if (iUFProjectManager != null)
                {
                    screenList = iUFProjectManager.GetResourceList(parent, "ScreenManager").ToList();
                }
            }

            item.IsNotValid = screenList == null || !screenList.Contains("Screen/" + item.ReadablePath + ".xaml");
        }
        ...
        public void UpdateItemExistence(CancellationToken ct, bool hastTypedef)
        {
...
#endif
            {
                ...
                IsNotValidCountMap[CrossReferenceType.Screens] = (from t in screenRefMap.Values where t.IsNotValid select t).Count();
