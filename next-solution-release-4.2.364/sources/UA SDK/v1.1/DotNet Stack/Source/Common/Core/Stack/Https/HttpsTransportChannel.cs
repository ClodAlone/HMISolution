...
namespace Opc.Ua.Bindings
{
    ...
    public class HttpsTransportChannel : ITransportChannel
    {
        ...
        public static IEncodeable ReadSoapMessage(
            ...
            ServiceMessageContext messageContext)
        {
			long initialPosition = istrm.Position;

			//first check if it's a Fault message
			...
			if( reader.IsStartElement( "Fault", "http://www.w3.org/2003/05/soap-envelope" ) )
            {
				reader.ReadToDescendant( "Detail", "http://www.w3.org/2003/05/soap-envelope" );

				XmlDecoder faultDecoder = new XmlDecoder( null, reader, messageContext );

				faultDecoder.PushNamespace( Namespaces.OpcUaXsd );
				ServiceFault fault = faultDecoder.ReadEncodeable( "ServiceFault", typeof( ServiceFault ) ) as ServiceFault;
				faultDecoder.PopNamespace();

				if( fault != null )
					throw new ServiceResultException( fault.ResponseHeader.ServiceResult );
				else
					throw new ServiceResultException( StatusCodes.Bad );
            }

			//recreate the XmlReader to read the message
			reader.Close();
			istrm.Position = initialPosition;
			reader = XmlReader.Create( istrm );
			reader.MoveToContent();
			reader.ReadStartElement( "Envelope", "http://www.w3.org/2003/05/soap-envelope" );
			reader.MoveToContent();
