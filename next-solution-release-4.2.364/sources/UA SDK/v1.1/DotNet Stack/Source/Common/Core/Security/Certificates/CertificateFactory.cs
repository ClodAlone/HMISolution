    ...
    public class CertificateFactory
    {
        ...
        public static X509Certificate2 Load(X509Certificate2 certificate, bool ensurePrivateKeyAccessible)
        {
            ...
            lock (m_certificates)
            {
                ...
                if (currentUser.User.IsWellKnown(System.Security.Principal.WellKnownSidType.LocalSystemSid))
                    rootDir.Append("\\Microsoft\\Crypto\\RSA\\S-1-5-18\\");
                if (currentUser.User.IsWellKnown(System.Security.Principal.WellKnownSidType.LocalServiceSid))
                    rootDir.Append("\\Microsoft\\Crypto\\RSA\\S-1-5-19\\");
                if (currentUser.User.IsWellKnown(System.Security.Principal.WellKnownSidType.NetworkServiceSid))
                    rootDir.Append("\\Microsoft\\Crypto\\RSA\\S-1-5-20\\");
                else
