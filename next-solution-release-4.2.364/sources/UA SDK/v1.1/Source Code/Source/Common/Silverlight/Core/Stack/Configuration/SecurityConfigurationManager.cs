...
namespace Opc.Ua.Security
{
    ...
    public class SecurityConfigurationManager : ISecurityConfigurationManager
    {
        ...
        public SecuredApplication ReadConfiguration(string filePath)
        {
            ...
            try
            {
                ...
                try
                {
                    ...
                    if (reader.Name == "SecuredApplication" && reader.NamespaceURI == Namespaces.OpcUaSecurity)
                    {
        ...
        public void WriteConfiguration(string filePath, SecuredApplication configuration)
        {
            ...
            if (document.DocumentElement.LocalName == "SecuredApplication" && document.DocumentElement.NamespaceURI == Namespaces.OpcUaSecurity)
            {
                document.DocumentElement.InnerXml = SetObject(typeof(SecuredApplication), configuration);
            }

            // update only the portions of the document that change.
            else
            {
