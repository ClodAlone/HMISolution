        ...
        public object ReadVariantContents(out TypeInfo typeInfo)
        {
            ...
            try
            {
                ...
                if (typeName.StartsWith("ListOf", StringComparison.Ordinal))
                {
                    ...
                    {
                        case "Boolean":         { typeInfo = TypeInfo.Arrays.Boolean; return ReadBooleanArray(typeName).ToArray(); }
                        case "SByte":           { typeInfo = TypeInfo.Arrays.SByte; return ReadSByteArray(typeName).ToArray(); }
                        case "Byte":            { typeInfo = TypeInfo.Arrays.Byte; return ReadByteArray(typeName).ToArray(); }
                        case "Int16":           { typeInfo = TypeInfo.Arrays.Int16; return ReadInt16Array(typeName).ToArray(); }
                        case "UInt16":          { typeInfo = TypeInfo.Arrays.UInt16; return ReadUInt16Array(typeName).ToArray(); }
                        case "Int32":           { typeInfo = TypeInfo.Arrays.Int32; return ReadInt32Array(typeName).ToArray(); }
                        case "UInt32":          { typeInfo = TypeInfo.Arrays.UInt32; return ReadUInt32Array(typeName).ToArray(); }
                        case "Int64":           { typeInfo = TypeInfo.Arrays.Int64; return ReadInt64Array(typeName).ToArray(); }
                        case "UInt64":          { typeInfo = TypeInfo.Arrays.UInt64; return ReadUInt64Array(typeName).ToArray(); }
                        case "Float":           { typeInfo = TypeInfo.Arrays.Float; return ReadFloatArray(typeName).ToArray(); }
                        case "Double":          { typeInfo = TypeInfo.Arrays.Double; return ReadDoubleArray(typeName).ToArray(); }
                        case "String":          { typeInfo = TypeInfo.Arrays.String; return ReadStringArray(typeName).ToArray(); }
                        case "DateTime":        { typeInfo = TypeInfo.Arrays.DateTime; return ReadDateTimeArray(typeName).ToArray(); }
                        case "Guid":            { typeInfo = TypeInfo.Arrays.Guid; return ReadGuidArray(typeName).ToArray(); }
                        case "ByteString":      { typeInfo = TypeInfo.Arrays.ByteString; return ReadByteStringArray(typeName).ToArray(); }
                        case "XmlElement":      { typeInfo = TypeInfo.Arrays.XmlElement; return ReadXmlElementArray(typeName).ToArray(); }
                        case "NodeId":          { typeInfo = TypeInfo.Arrays.NodeId; return ReadNodeIdArray(typeName).ToArray(); }
                        case "ExpandedNodeId":  { typeInfo = TypeInfo.Arrays.ExpandedNodeId; return ReadExpandedNodeIdArray(typeName).ToArray(); }
                        case "StatusCode":      { typeInfo = TypeInfo.Arrays.StatusCode; return ReadStatusCodeArray(typeName).ToArray(); }
                        case "DiagnosticInfo":  { typeInfo = TypeInfo.Arrays.DiagnosticInfo; return ReadDiagnosticInfoArray(typeName).ToArray(); }
                        case "QualifiedName":   { typeInfo = TypeInfo.Arrays.QualifiedName; return ReadQualifiedNameArray(typeName).ToArray(); }
                        case "LocalizedText":   { typeInfo = TypeInfo.Arrays.LocalizedText; return ReadLocalizedTextArray(typeName).ToArray(); }
                        case "ExtensionObject": { typeInfo = TypeInfo.Arrays.ExtensionObject; return ReadExtensionObjectArray(typeName).ToArray(); }
                        case "DataValue":       { typeInfo = TypeInfo.Arrays.DataValue; return ReadDataValueArray(typeName).ToArray(); }
                        case "Variant":         { typeInfo = TypeInfo.Arrays.Variant; return ReadVariantArray(typeName).ToArray(); }
                    }              
                }
