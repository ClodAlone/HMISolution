...
#endif
{
    ...
    public class GridCurrentCell
    {
        ...
        internal int MoveToRowIndex
        {
            get
            {
                return this.moveToRowIndex;
            }
        }
        ...
        {
            if (cellRowColumnIndex.RowIndex < 0 || cellRowColumnIndex.ColumnIndex < 0
        ...
        public bool Activate(RowColumnIndex cellRowColumnIndex, GridActivateCurrentCellOptions activateOptions)
        {
            ...
            try
            {
                ...
                if (skipActivate || activateOptions == null)
                {
                    return false;
                }
#endif
                ...
                grid.Focus();
#else
                grid.Focus(FocusState.Keyboard);
#endif
        ...
        private void BeginEditHelper(GridActivateCurrentCellOptions activateOptions)
        {
            ...
            if (!cellRenderer.ShouldSupportOptimization())
            {
            ...
            if (activateOptions.IsActivateTriggeredByMouseDownIntoUIElement
                || activateOptions.IsActivateTriggeredByGotFocus
                || (activateOptions.ShouldBeginEdit && !this.Grid.Model[this.rowIndex, this.columnIndex].ReadOnly)
                )
                BeginEdit(true);
        }
        ...
        public bool Deactivate()
        {
            ...
            bool suspendMoveTo;
            return Deactivate(false, out isValidationMsgShown, out suspendMoveTo);
        }
        ...
        /// <returns>True if current cell can be deactivated; False otherwise.</returns>
        public bool Deactivate(bool discardChangesIfCommitFails, out bool isValidationMsgShown, out bool suspendMoveTo)
        {
            isValidationMsgShown = false;
            suspendMoveTo = false;
            ...
            try
            {
                ...
                if (IsModified)
                {
                    if (!discardChangesIfCommitFails)
                    {
                        ...
                        if (!this.ConfirmChanges(out isValidationMsgShown, out suspendMoveTo))
                            return false;
                    }
                    else
        ...
        /// failed.</returns>
        public bool ConfirmChanges(out bool isValidationMsgShown, out bool suspendMoveTo)
        {
            isValidationMsgShown = false;
            suspendMoveTo = false;
            ...
            try
            {
                if (cellRenderer != null)
                {
                    ...
                    if (IsModified)
                    {
                        Validate(out isValidationMsgShown, out suspendMoveTo);
                        if (!IsValid)
                        {
        ...
        public bool CreateCurrentCellUIElements()
        {
            ...
            if (cellUI == null)
            {
                ...
                if (this.HasCurrentCell && this.IsEditing)
                {
                    this.Grid.DelayedSwapCellUIElements(this.CellRowColumnIndex, true);
                }
#endif
        ...
        {
            return BeginEdit(true);//grid.IsKeyboardFocusWithin);
        }
            ...
            try
            {
                if (IsValidCellRowColumnIndex(CellRowColumnIndex))
                {
                    if (cellRenderer != null && !IsEditing && cellRenderer.IsEditable)
                    {
                        ...
                        //if (cellRenderer.ShouldSupportOptimization())
                        {
                            cellRenderer.RaiseInitialize(this.activateOptions);
                        }

                        if (cellRenderer.IsFocusable && focusCellUIElement)
                            cellRenderer.IsFocused = true;
                    }
                    if (!cellRenderer.IsEditable)
                        cellRenderer.CopyCurrentStyle();
                    return true;
                }
            }
            finally
        ...
        public void EndEdit()
        {
            ...
            try
            {
                ...
                {
                    // Fires NotifyValidate, should CloseDropDown() possibly fires ControlLostFocus event.
                    ConfirmChanges(out isValidationMsgShown, out suspendMoveTo);
                }
                CancelEdit();
            }
            finally
        ...
        {
            // only raise event first time cell contents are changed.
            if (notifyChangingCalled) return true;
        ...
        {
            bool isValidationMsgShown = false;
            return MoveTo(rowIndex, columnIndex, new GridActivateCurrentCellOptions(), out isValidationMsgShown);
        }
        ...
        /// selection should be ignored and more.</param>
        /// <returns>True if the current cell could be moved; False otherwise.</returns>
        public bool MoveTo(RowColumnIndex cellRowColumnIndex, GridActivateCurrentCellOptions activateOptions, out bool isValidationMsgShown)
        {
            return MoveTo(cellRowColumnIndex.RowIndex, cellRowColumnIndex.ColumnIndex, activateOptions, out isValidationMsgShown);
        }
        ...
        {
            bool isValidationMsgShown = false;
            return MoveTo(cellRowColumnIndex.RowIndex, cellRowColumnIndex.ColumnIndex, new GridActivateCurrentCellOptions(), out isValidationMsgShown);
        }
        ...
        /// selection should be ignored and more.</param>
        /// <returns>True if the current cell could be moved; False otherwise.</returns>
        public bool MoveTo(int rowIndex, int columnIndex, GridActivateCurrentCellOptions activateOptions, out bool isValidationMsgShown)
        {
            isValidationMsgShown = false;
            ...
            try
            {
                ...
                if (!Deactivate(activateOptions.DiscardChanges, out isValidationMsgShown, out suspendMoveTo) || !style.Enabled)
                    return false;
                ...
                success = true;
                //Whole row was invalidated, while moving from one cell to another cell – this lead to an performance problem
                //if (savedRowIndex >= 0)
                //    Grid.InvalidateCell(GridRangeInfo.Row(savedRowIndex));
                this.inMoved = true;
                //  RaiseCurrentCellMoved method should be invoked only after the CurrentCell.MoveTo() method is processed. IT;s better to invoke from GridSelectCellMouseController.cs
                //  Since all the process like adding the selections in Selected Ranges and selections should be completed before invoking RaiseCurrentCellMoved() method. Or else it selectedItems will be Empty.
                // Hence this is commented and invoked after Completing this process in SelectCellmouseController
        ...
        public void Validate(out bool isValidationMsgShown, out bool suspendMoveTo)
        {
            isValidationMsgShown = false;
            suspendMoveTo = false;
            if (!HasCurrentCell || IsLocked) return;
            ...
            try
            {
                var style = this.gridModel[this.RowIndex, this.ColumnIndex];
                CurrentCellValidateEventArgs e = new CurrentCellValidateEventArgs(style, style.CellValue, cellRenderer.ControlValue);
                e = Grid.RaiseCurrentCellValidating(e);
                if (cellRenderer.ControlValue != null && !cellRenderer.ControlValue.Equals(e.NewValue))
                {
                    cellRenderer.ControlValue = e.NewValue;
                }
                bool valid = !e.Cancel;
                suspendMoveTo = e.SuspendMoveTo;
                /// isValidationMsgShown is True when the validation message shown. In Silverlight we dont have the support for checking the button state
                /// so that this flag was used to remove the selection while mouse over the grid.
                isValidationMsgShown = e.Handled;
                // Possibly shows a dialog box (but user can also set CurrentCell.ErrorMessage).
                IsValid = valid && (cellRenderer == null || cellRenderer.RaiseValidate());
                ...
                else
                {
                    ...
                    this.gridModel.SelectedRanges.Add(GridRangeInfo.Cell(this.RowIndex, this.ColumnIndex));
                }
            }
            catch (Exception ex)
            {
...
#else
            MoveDown(1, (Window.Current.CoreWindow.GetAsyncKeyState(VirtualKey.Shift) == CoreVirtualKeyStates.Down));
#endif
        }
        /// <summary>
...
#else
            MoveDown(num, Window.Current.CoreWindow.GetAsyncKeyState(VirtualKey.Shift) == CoreVirtualKeyStates.Down);
#endif
        }
        /// <summary>
...
#else
            MoveUp(1, Window.Current.CoreWindow.GetAsyncKeyState(VirtualKey.Shift) == CoreVirtualKeyStates.Down);
#endif
        }
        /// <summary>
...
#else
            MoveUp(num, Window.Current.CoreWindow.GetAsyncKeyState(VirtualKey.Shift) == CoreVirtualKeyStates.Down);
#endif
        }
        /// <summary>
        ...
        {
#if !WinRT
            MoveLeft(1, (Keyboard.Modifiers & ModifierKeys.Shift) != 0);
#else
            MoveLeft(1, Window.Current.CoreWindow.GetAsyncKeyState(VirtualKey.Shift) == CoreVirtualKeyStates.Down);
#endif
        }
        /// <summary>
...
#else
            MoveLeft(num, Window.Current.CoreWindow.GetAsyncKeyState(VirtualKey.Shift) == CoreVirtualKeyStates.Down);
#endif
        }
        /// <summary>
...
#else
            MoveRight(1, Window.Current.CoreWindow.GetAsyncKeyState(VirtualKey.Shift) == CoreVirtualKeyStates.Down);
#endif
        }
        /// <summary>
...
#else
            MoveRight(num, Window.Current.CoreWindow.GetAsyncKeyState(VirtualKey.Shift) == CoreVirtualKeyStates.Down);
#endif
        }
        /// <summary>
...
#else
            PageUp(Window.Current.CoreWindow.GetAsyncKeyState(VirtualKey.Shift) == CoreVirtualKeyStates.Down);
#endif
        }
        /// <summary>
        /// Moves the current cell up one page to the next enabled row.
...
#else
            PageDown(Window.Current.CoreWindow.GetAsyncKeyState(VirtualKey.Shift) == CoreVirtualKeyStates.Down);
#endif
        }
        /// <summary>
        /// Moves the current cell down one page to the next enabled row.
...
#else
            PageLeft(Window.Current.CoreWindow.GetAsyncKeyState(VirtualKey.Shift) == CoreVirtualKeyStates.Down);
#endif
        }
        /// <summary>
        /// Moves the current cell left one page to the next enabled row.
...
#else
            PageRight(Window.Current.CoreWindow.GetAsyncKeyState(VirtualKey.Shift) == CoreVirtualKeyStates.Down);
#endif
        }
        /// <summary>
        /// Moves the current cell right one page to the next enabled row.
...
#else
            MoveToRightEnd(Window.Current.CoreWindow.GetAsyncKeyState(VirtualKey.Shift) == CoreVirtualKeyStates.Down);
#endif
        }
        /// <summary>
        /// Moves the current cell down one page to the next enabled row.
...
#else
            MoveToBottom(Window.Current.CoreWindow.GetAsyncKeyState(VirtualKey.Shift) == CoreVirtualKeyStates.Down);
#endif
        }
        /// <summary>
        /// Moves the current cell down one page to the next enabled row.
...
#else
            MoveToLeftEnd(Window.Current.CoreWindow.GetAsyncKeyState(VirtualKey.Shift) == CoreVirtualKeyStates.Down);
#endif
        }
        /// <summary>
        /// Moves the current cell down one page to the next enabled row.
...
#else
            MoveToTop(Window.Current.CoreWindow.GetAsyncKeyState(VirtualKey.Shift) == CoreVirtualKeyStates.Down);
#endif
        }
        /// <summary>
        /// Moves the current cell down one page to the next enabled row.
...
#else
            MoveToTopLeft(Window.Current.CoreWindow.GetAsyncKeyState(VirtualKey.Shift) == CoreVirtualKeyStates.Down);
#endif
        }
...
#else
            MoveToBottomRight(Window.Current.CoreWindow.GetAsyncKeyState(VirtualKey.Shift) == CoreVirtualKeyStates.Down);
#endif
        }
        ...
        public bool Move(GridDirectionType direction, int num, bool extendSelection, bool wrapCell)
        {
            ...
            try
            {
                ...
                GridMoveCurrentCellDirectionEventArgs qme = new GridMoveCurrentCellDirectionEventArgs(direction, num, extendSelection, this.RowIndex, this.ColumnIndex, this.Grid);
                    ...
                    if (direction == GridDirectionType.Right)
                    {
                        ...
                        if (isLastCol)
                        {
                            ...
                            if (isLastRow)
                            {
                                ...
                                {
                                    GridWrapCellNextControlInFormEventArgs e = new GridWrapCellNextControlInFormEventArgs(true, false, this.Grid);
                    ...
                    else if (direction == GridDirectionType.Left)
                    {
                        ...
                        if (isFirstCol)
                        {
                            ...
                            if (isFirstRow)
                            {
                                ...
                                {
                                    GridWrapCellNextControlInFormEventArgs e = new GridWrapCellNextControlInFormEventArgs(false, false, this.Grid);
                                    Grid.RaiseWrapCellNextControlInForm(e);
                                    if (!e.Handled)
                                    {
        ...
        public bool InternalMove(GridDirectionType direction, int num, GridActivateCurrentCellOptions options)
        {
                ...
                case GridDirectionType.Up:
                    ...
                    while (num > 0)
                    {
                        ...
                        else if (newRowIndex <= regularCells.Top || vscrollup > 0)
                        {
                            ...
                            vscrollup++;
                            if (currentRow >= notFrozenCells.Top && notFrozenCells.Top > 1)
                ...
                case GridDirectionType.Left:
                    while (num > 0)
                    {
                        ...
                        else if (newColumnIndex <= regularCells.Left || hscrollleft > 0)
                        {
                            ...
                            hscrollleft++;
                            if (currentCol >= notFrozenCells.Left && notFrozenCells.Left > 1)
                                newColumnIndex = grid.GetCol(grid.GetClientCol(notFrozenCells.Left - 1)) - hscrollleft;
                        }
                ...
                case GridDirectionType.Right:
                    ...
                    while (num > 0)
                    {
                        ...
                        if (newColumnIndex < notFrozenCells.Left)
                        {
                            ...
                            newColumnIndex = Math.Min(regularCells.Right, newColumnIndex + 1);
            ...
            isCtrlKey = ((Keyboard.Modifiers & ModifierKeys.Control) != ModifierKeys.None);
#else
                ...
                || !hasCurrentCell))
            {
                try
                {
                    ...
                    else if ((isShiftKey || isCtrlKey) && (gridModel.Selections.Ranges.AnyRangeContains(GridRangeInfo.Cell(targetRowIndex, targetColumnIndex))) && gridModel.Options.ListBoxSelectionMode == GridSelectionMode.None)
                        grid.Model.Selections.Clear();
                    ...
                    if (this.MoveTo(_targetRowIndex, _targetColumnIndex))
                    {
