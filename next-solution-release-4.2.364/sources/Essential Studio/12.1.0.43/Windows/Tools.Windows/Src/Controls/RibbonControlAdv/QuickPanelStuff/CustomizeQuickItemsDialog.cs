...
namespace Syncfusion.Windows.Forms.Tools
{
	...
	/// </summary>
	internal partial class CustomizeQuickItemsDialog:
		Office2007Form
	{
		...
		/// <summary>
		/// Creates & initializes new instance of CustomizeQuickItemsDialog.
		/// </summary>
		/// <param name="header">RibbonControlAdvHeader instance to work with.</param>
		public CustomizeQuickItemsDialog( RibbonControlAdvHeader header )
		{
			InitializeComponent();
            this.MinimumSize = new Size(659, 547);
		...
		{
			using( CustomizeQuickItemsDialog dlg = new CustomizeQuickItemsDialog( header ) )
			{
        ...
        protected override void OnResize(EventArgs e)
        {
            ...
            using (Bitmap bit = new Bitmap(10, 10))
            {
                using (Graphics g = Graphics.FromImage(bit))
                {
                    if (g.DpiY > 96)
                    {
                        label3.Text = WordWrap(QuickAccessToolBarLabel, this.lstChosenItems.Width / 4);
		...
		private void UpdateLabels()
		{
			...
			if( ribbonControl != null )
			{
                ...
                btnAdd.Text = SR.GetString(SR.QuickAccessDialogButtonAdd, ribbonControl);
                btnRemove.Text = SR.GetString(SR.QuickAccessDialogButtonRemove, ribbonControl);
                btnReset.Text = SR.GetString(SR.QuickAccessDialogButtonReset, ribbonControl);
                btnOK.Text = SR.GetString(SR.QuickAccessDialogButtonOk, ribbonControl);
                btnCancel.Text = SR.GetString(SR.QuickAccessDialogButtonCancel, ribbonControl);
                chkPlaceBelowRibbon.Text = SR.GetString(SR.QuickAccessPlaceBelowRibbon, ribbonControl);
                lblChooseCommandsFrom.Text = SR.GetString(SR.QuickAccessDialogCommands, ribbonControl);
                label1.Text = SR.GetString(SR.CustomizationLabel, ribbonControl);
                label2.Text = SR.GetString(SR.CustomizeQuickAccessLabel, ribbonControl);
                QuickAccessToolBarLabel = SR.GetString(SR.QuickAccessToolBarLabel, ribbonControl);
                label3.Text = WordWrap(QuickAccessToolBarLabel, 45);
        ...
        void ListBox_DrawItem(object sender, DrawItemEventArgs e)
        {
            ...
            if ((e.State & DrawItemState.Selected) != 0)
            {
                ...
                {
                    g.DrawLine(borderPen, rect.X + 1, rect.Y, rect.Right - 1, rect.Y);
                ...
                {
                    foreRectTop = new RectangleF(foreRectTop.X - 1, foreRectTop.Y - 1, foreRectTop.Width + 2, foreRectTop.Height + 1);
