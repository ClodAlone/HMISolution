...
namespace Syncfusion.Data
{
    ...
    public class Group : GroupEntry, IEnumerable<NodeEntry>
    {
        ...
        internal protected TopLevelGroup toplevelGroup;
        ...
        public object GetSummaryValue(string ColumnName, string AggregateKey)
        {
            if (this.toplevelGroup != null && !string.IsNullOrEmpty(ColumnName) && this.SummaryDetails != null)
            {
                var summaryRecordEntry = this.SummaryDetails;
                if (summaryRecordEntry != null)
                    return SummaryCreator.GetSummaryDisplayValue(summaryRecordEntry, ColumnName, AggregateKey);
                else
                    return string.Empty;
            }
            return string.Empty;
        ...
        public virtual void CreateDetailsForRecords(Group parent, IEnumerable source, int level)
        {
            ...
            this.Details = new GroupRecordEntry(parent, level, source, topLevelGroup.CollectionView.FilterRecord);
        }
        ...
        internal virtual object[] GetGroupItems()
        {
            var groupRecordEntry = this.Details as GroupRecordEntry;
            return groupRecordEntry.ToArray();
        }
        ...
        {
            var group1 = this.Details as GroupRecordEntry;
        ...
        {
            int FilteredCount = 0;
            var group = this.Details as GroupEntry;
            if (this.Details.IsGroups)
            {
        ...
        private void MakeDirty(Group group)
        {
            if (group != null && !group.IsTopLevelGroup)
            {
                group.SetDirty();
                if (group.Parent != null)
                    MakeDirty(group.Parent);
            }
        }
    ...
    public class GroupComparer : IComparer<Group>
    {
        ...
        public int Compare(Group x, Group y)
        {
            ...
            {
#if WinRT
                var xIsNull = (x.Key == null) || (x.Key is Nullable) || !(x.Key is IComparable);
                var yIsNull = (y.Key == null) || (y.Key is Nullable) || !(y.Key is IComparable);
#else
                ...
                {
#if WinRT
                    if ((x.Key == null && y.Key == null) || (x.Key is Nullable && y.Key is Nullable) ||
