...
namespace Syncfusion.Pdf
{
    ...
    {
        internal Stream FromStream(Stream stream)
        {
            ...
            Guid globalPixelFormat;
            if (numComps == 4)
            {
                globalPixelFormat = PixelFormat.Format32bppCMYK;
            }
            else
            {
                globalPixelFormat = PixelFormat.Format24bppRGB;
            }
            ...
            blankBitmapFrameEncode.SetSize((int)decodedImage.ImgWidth, (int)decodedImage.ImgHeight);
            ...
            int blankStride = PixelFormat.GetStride(globalPixelFormat, (int)decodedImage.ImgWidth);
            var blankBufferSize = (int)decodedImage.ImgHeight * blankStride;

            byte[] blankOutputBytes = new byte[blankBufferSize];

            blankBitmapFrameEncode.WritePixels((int)decodedImage.ImgHeight, blankStride, blankBufferSize, blankOutputBytes);
            ...
            blankWicImageStream.Seek(0);
            var bitmapDecoder = factory.CreateBitmapDecoder(blankWicImageStream, DecodeOptions.CacheOnDemand);
            ...
            BitmapLock inputImageData = null;

            JPXImageCoordinates numTiles = decodedImage.getNumTiles(null);
            int tIdx = 0;
            
            byte[] rowvalues = new byte[decodedImage.ImgWidth * numComps];
            for (int y = 0; y < numTiles.y; y++)
            {
                for (int x = 0; x < numTiles.x; x++, tIdx++)
                {
                    ...
                    for (int l = 0; l < height; l++)
                    {
                        ...
                        rowvalues = new byte[width * bytesPerPixel];
                                ...
                                case 4:
                                    {
                                        ...
                                        rowvalues[offset + 0] = (byte)(255 - r1);
                                        rowvalues[offset + 1] = (byte)(255 - g1);
                                        rowvalues[offset + 2] = (byte)(255 - b1);
                                        break;
                                    }
                            }
                        }

                        inputImageData = inputImage.Lock(new global::Windows.Foundation.Rect(tOffx, tOffy + l, width, 1), BitmapLockFlags.Read);
            ...
            inputImageData = inputImage.Lock(new global::Windows.Foundation.Rect(0, 0, (int)decodedImage.ImgWidth, (int)decodedImage.ImgHeight), BitmapLockFlags.Read);
            int strideLength = (int)(inputImageData.Stride * (int)decodedImage.ImgHeight);

            int imgsize;
            DataPointer dImgPtr = inputImageData.GetDataPointer(out imgsize);

            IntPtr imgptr = new IntPtr(dImgPtr.Pointer);
            byte[] jpxImageBytes = new byte[strideLength];
            
            Marshal.Copy(imgptr, jpxImageBytes, 0, strideLength);
           
            ImageStream jpxStream = null;
            IRandomAccessStream jpxWicImageStream = new InMemoryRandomAccessStream();
            jpxStream = factory.CreateStream(jpxWicImageStream);

            var jpxEncoder = factory.CreateJpegEncoder();
            jpxEncoder.Initialize(jpxStream);

            var jpxBitmapFrameEncode = jpxEncoder.CreateBitmapFrameEncode();

            jpxBitmapFrameEncode.Initialize();

            jpxBitmapFrameEncode.SetSize((int)decodedImage.ImgWidth, (int)decodedImage.ImgHeight);
            jpxBitmapFrameEncode.SetPixelFormat(globalPixelFormat);
            jpxBitmapFrameEncode.WritePixels((int)decodedImage.ImgHeight, blankStride, jpxImageBytes.Length, jpxImageBytes);

            jpxBitmapFrameEncode.Commit();
            jpxEncoder.Commit();
            jpxBitmapFrameEncode.Dispose();
            jpxEncoder.Dispose();
            jpxStream.Dispose();
            return jpxWicImageStream.AsStream();
        }
