...
namespace Syncfusion.Pdf
{
    internal class XObjectElement
    {
        ...
        public PdfDocumentPage Render(PdfPageResources resources, Syncfusion.DirectXWrapper.WinRT.Matrix matrix)
        {
            PdfDocumentPage returnpage = new PdfDocumentPage(null);
            if (this.ObjectType == "Form")
            {
                ...
                if (xobjects.ContainsKey(DictionaryProperties.Matrix))
                {
                    ...
                    if (xobjects[DictionaryProperties.Matrix] is PdfArray)
                    {
                        ...
                        if (matrixArray != null)
                        {
                            ...
                            returnpage.FormMatrix = new Syncfusion.DirectXWrapper.WinRT.Matrix(matrix.M11 * a, -b, -c, matrix.M22 * d, matrix.OffsetX + e, matrix.OffsetY + (matrix.M22 - (f + d)));
                ...
                {
                    returnpage.m_recordCollection = contentTree;
                    returnpage.m_resources = childResource;
                    //DeviceCMYK cmyk = new DeviceCMYK();
                    //WinRTRenderer renderer = new WinRTRenderer(contentTree, childResource);
...
namespace Syncfusion.Pdf
{
    internal class XObjectElement
    {
        ...
        public PdfDocumentPage Render(PdfPageResources resources, global::Windows.UI.Xaml.Media.Matrix matrix)
        {
            PdfDocumentPage returnpage = new PdfDocumentPage(null);
            if (this.ObjectType == "Form")
            {
                ...
                if (xobjects.ContainsKey(DictionaryProperties.Matrix))
                {
                    ...
                    if (xobjects[DictionaryProperties.Matrix] is PdfArray)
                    {
                        ...
                        if (matrixArray != null)
                        {
                            ...
                            returnpage.FormMatrix = new global::Windows.UI.Xaml.Media.Matrix(matrix.M11 * a, -b, -c, matrix.M22 * d, matrix.OffsetX + e, matrix.OffsetY + (matrix.M22 - (f + d)));
                            //check for rotate transform
                            double rad = Math.Acos(a);
                            double degree = Math.Round((180 / Math.PI) * rad);
