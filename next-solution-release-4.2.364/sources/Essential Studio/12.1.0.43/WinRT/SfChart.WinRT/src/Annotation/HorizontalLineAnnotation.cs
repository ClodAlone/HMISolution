...
namespace Syncfusion.UI.Xaml.Charts
{
    public class HorizontalLineAnnotation : StraightLineAnnotation
    {
                ...
                switch (CoordinateUnit)
                {
                    case CoordinateUnit.Axis:
                        if (XAxis != null && YAxis != null)
                        {
                            ...
                            if (ShowLine)
                            {
                                Point point = (XAxis.Orientation == Orientation.Horizontal) ? new Point(this.Chart.ValueToPointRelativeToAnnotation(XAxis, x1) - XAxis.PlotOffset,
                                                        this.Chart.ValueToPointRelativeToAnnotation(YAxis, y1)) : new Point(this.Chart.ValueToPointRelativeToAnnotation(YAxis, y1) - YAxis.PlotOffset,
                                                        this.Chart.ValueToPointRelativeToAnnotation(XAxis, x1));
                                Point point2 = (XAxis.Orientation == Orientation.Horizontal) ? new Point(Chart.ValueToPointRelativeToAnnotation(XAxis, x2) + XAxis.PlotOffset,
                                                         this.Chart.ValueToPointRelativeToAnnotation(YAxis, y2)) : new Point(this.Chart.ValueToPointRelativeToAnnotation(YAxis, y2) + YAxis.PlotOffset,
                    ...
                    case CoordinateUnit.Pixel:
                        if (ShowLine && this.Chart != null && this.Chart.AnnotationManager!=null)
                        {
                                this.DraggingMode = AxisMode.Vertical;
                                if (Y1 == null)
                                    Y1 = 0;
        ...
        internal Point GetAxisLabelPosition(Size desiredSize, Point originalPosition, Size textSize)
        {
            ...
            string name = (XAxis.Orientation == Orientation.Vertical) ? XAxisName : YAxisName;
