...
namespace Syncfusion.UI.Xaml.Charts
{
    public class DateTimeCategoryAxis : ChartAxisBase2D
    {
        ...
        public static readonly DependencyProperty IntervalProperty =
            DependencyProperty.Register("Interval", typeof(double), typeof(DateTimeCategoryAxis), new PropertyMetadata(0d, OnIntervalChanged));
        ...
        public static readonly DependencyProperty IntervalTypeProperty =
            DependencyProperty.Register("IntervalType", typeof(DateTimeIntervalType), typeof(DateTimeCategoryAxis), new PropertyMetadata(DateTimeIntervalType.Auto, new PropertyChangedCallback(OnIntervalTypeChanged)));
        ...
        {
            if ((d as DateTimeCategoryAxis) != null && e.NewValue != null && e.NewValue!=e.OldValue)
            {
                (d as DateTimeCategoryAxis).ActualIntervalType = (d as DateTimeCategoryAxis).IntervalType;
                (d as DateTimeCategoryAxis).OnIntervalChanged(e);
            }
        }
                ...
                if (XValuesCount!=null && XValuesCount.Count > 0)
                {
                    ...
                    switch (ActualIntervalType)
                    {
                                ...
                                if (VisibleRange.Inside(position) && pos > -1 && pos < xValues.Count)
                                {
                                    ...
                                    if (xValue.Year == year)
                                    {
                                        ...
                                        if (xValue.Month == month)
                                        {
                                            ...
                                            VisibleLabels.Add(new ChartAxisLabel(pos, obj, pos));
                                ...
                                if (VisibleRange.Inside(position) && pos > -1 && pos < xValues.Count)
                                {
                                    ...
                                    if (xValue.Date == date)
                                    {
                                        ...
                                        if (xValue.Hour == hour)
                                        {
                                            object obj = GetLabelContent(pos, actualSeries);
                                            VisibleLabels.Add(new ChartAxisLabel(pos, obj, pos));
                                            var diff = hour - xValue.AddHours((int)interval).Hour;
                        ...
                        case DateTimeIntervalType.Minutes:
                            ...
                            {
                                int pos = ((int)Math.Round(position));
                                //if (pos > -1 && pos < xValues.Count)
                                //    var xValue = xValues[pos].FromOADate();
                                //else
                                //    xValue = DateTimeAxisHelper.IncreaseInterval(xValues[0].FromOADate(), pos, DateTimeIntervalType.Minutes);
                                if (VisibleRange.Inside(position) && pos > -1 && pos < xValues.Count)
                                {
                                    var xValue = xValues[pos].FromOADate();
                                    if (xValue.Date > date)
