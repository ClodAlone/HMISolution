    ...
    public static class ChartTransform
    {
        ...
        public class ChartCartesianTransformer : IChartTransformer
        {
            ...
            public ChartCartesianTransformer(Size viewport, ChartSeriesBase series)
            {
                ...
                ChartAxis logXAxis = XAxis as LogarithmicAxis3D;
                logXAxis = logXAxis ?? XAxis as LogarithmicAxis;

                ChartAxis logYAxis = YAxis as LogarithmicAxis3D;
                logYAxis = logYAxis ?? YAxis as LogarithmicAxis;
                ...
                m_IsRoated = series.IsActualTransposed
                             && series.ActualArea.VisibleSeries[0].IsActualTransposed;
            }
            #endregion
            ...
            public Point TransformToVisible(double x, double y)
            {
                if (XAxis != null && YAxis != null && double.IsNaN(m_viewport.Width) == false && double.IsNaN(m_viewport.Height) == false)
                {
                    ...
                    if (this.m_IsRoated)
                    {
                        double left = area is SfChart ? YAxis.RenderedRect.Left - XAxis.Area.SeriesClipRect.Left : YAxis.RenderedRect.Left;
                        double top = area is SfChart ? XAxis.RenderedRect.Top - YAxis.Area.SeriesClipRect.Top : XAxis.RenderedRect.Top;
                    ...
                    {
                        double left = area is SfChart ? XAxis.RenderedRect.Left - XAxis.Area.SeriesClipRect.Left : XAxis.RenderedRect.Left;
                        double top = area is SfChart ? YAxis.RenderedRect.Top - YAxis.Area.SeriesClipRect.Top : YAxis.RenderedRect.Top;
        ...
        public class ChartPolarTransformer : IChartTransformer
        {
            ...
            public Point TransformToVisible(double x, double y)
            {
                ...
                double radius = m_radius * m_yAxis.ValueToPolarCoefficient(y);
        ...
        public class ChartTransform3D : IChartTransformer
        {
            ...
            {
                SetCenter(new Vector3D(mViewport.Width / 2, mViewport.Height / 2, Depth / 2));
                View = Matrix3D.Transform(0, 0, Depth);
                View *= Matrix3D.Turn(-ChartMath.ToRadial * Rotation);
                View *= Matrix3D.Tilt(-ChartMath.ToRadial * Tilt);
