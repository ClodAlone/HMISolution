...
namespace Syncfusion.XlsIO.FormatParser.FormatTokens
{
  ...
  public class GeneralToken : FormatTokenBase
  {
    ...
    public override int TryParse( string strFormat, int iIndex )
    {
      ...
      if( string.Compare( strFormat, iIndex, DEF_FORMAT, 0, DEF_FORMAT.Length, StringComparison.CurrentCultureIgnoreCase ) == 0 )
      {
        iIndex += DEF_FORMAT.Length;
      }
    ...
    {
        //MS Excel, Display with Exponent when 
        // the double value contains leading four zeros in mantissa part
        //also the digit count should be more than 12.
        string strValue;
        try
        {
            strValue = Convert.ToDecimal(value).ToString();
        }
        catch(System.OverflowException)
        {
            strValue = value.ToString();
        }
        if (strValue.Contains("0.0000"))
        {
