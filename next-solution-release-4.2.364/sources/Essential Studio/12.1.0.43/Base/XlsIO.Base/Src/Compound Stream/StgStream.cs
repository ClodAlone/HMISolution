...
namespace Syncfusion.XlsIO.IO.Stream
{
  /// <summary>
  ...
  public class StgStream : System.IO.Stream, IDisposable
  {
    ...
    /// </summary>
    private ArrayList m_arrStreams = new ArrayList();
    /// <summary>
    /// List of storage names found in current storage.
    /// </summary>
    private ArrayList m_arrStorages = new ArrayList();
    /// <summary>
    ...
    public string[] Streams
    {
      get
      {
        return ( string[] )m_arrStreams.ToArray( typeof( string ) );
      }
    }
    ...
    public string[] Storages
    {
      get
      {
        return ( string[] )m_arrStorages.ToArray( typeof( string ) );
      }
    }
    /// <summary>
    /// Reference in COM interface which provide access to storage.
    ...
    public override void Close()
    {
      ...
      if( m_stream != null )
      {
        m_stream.Commit( ( uint )STGC.STGC_CONSOLIDATE );
    ...
    public virtual void Commit( STGC code )
    {
      if( ( m_modeStream & STGM.STGM_TRANSACTED ) == STGM.STGM_TRANSACTED )
      {
        ...
        if( error != 0 )
          throw new ExternalException( "Commit Operation failed", error );
      }
    ...
    {
      if( ( m_modeStream & STGM.STGM_TRANSACTED ) != STGM.STGM_TRANSACTED ) return;
    ...
    public override long Seek( long offset, SeekOrigin origin )
    {
      ...
      STREAM_SEEK seek = ( origin == SeekOrigin.Begin ) ?
        STREAM_SEEK.STREAM_SEEK_SET :
        ( ( origin == SeekOrigin.Current ) ?
        STREAM_SEEK.STREAM_SEEK_CUR : STREAM_SEEK.STREAM_SEEK_END );

      int error = m_stream.Seek( offset, seek, ref ulSkip );

      if( error != 0 )
        throw new ExternalException( "Seek Operation failed.", error );

      m_lPosition = ( long )ulSkip;
    ...
    public override void SetLength( long value )
    {
      ...
      int error = m_stream.SetSize( ( _ULARGE_INTEGER )value );
    ...
    public override int Read( byte[] buffer, int offset, int count )
    {
      ...
      if( m_lLength == m_lPosition ) return 0;
...
//      if( offset > 0 ) buffer.CopyTo( tmpBuff, offset );
      ...
      if( offset > 0 ) Array.Copy( buffer, offset, tmpBuff, 0, count );
      ...
      if( m_lLength < m_lPosition ) m_lLength = m_lPosition;
    }
#if DEBUG
    /// <summary>
    ///
    ...
    public StgStream( string fileName, STGM flags )
    {
      ...
      int error = Win32.API.StgOpenStorage( fileName,
    ...
    public StgStream( System.IO.Stream stream, STGM flags )
    {
      ...
      error = m_lockBytes.WriteAt( ( _ULARGE_INTEGER )( long )0, buffer, ( uint )buffer.Length,
        out uiWritten );
    ...
    public void SaveILockBytesIntoStream( System.IO.Stream stream )
    {
      ...
      {
        int result = m_lockBytes.ReadAt( ( _ULARGE_INTEGER )position, buffer, DEF_BUFFER_SIZE, out readBytes );
        ...
        if( readBytes < DEF_BUFFER_SIZE ) break;
      }
    }
    /// <summary>
    ...
    {
      if( strStreamName == null || strStreamName.Length == 0 ) return false;
    ...
    {
      if( strStorageName == null || strStorageName.Length == 0 ) return false;
    ...
    /// </exception>
    static public StgStream CreateStorage( string fileName )
    {
      IStorage storage;

      int error = Win32.API.StgCreateDocfile( fileName,
    ...
    /// </exception>
    private ArrayList CalculateStorageStreams()
    {
    ...
    private ArrayList CalculateStorageSubStorages()
    {
      ArrayList list = new ArrayList();
    ...
    private void ByTypeAccumulate_Streams( Win32.STATSTG item, object userData )
    {
      if( item.type == STGTY.STGTY_STREAM )
      {
        (( ArrayList )userData).Add( item.pwcsName );
      }
    }
    /// <summary>
    ...
    private void ByTypeAccumulate_Storages( Win32.STATSTG item, object userData )
    {
    ...
    /// <param name="item">Item to add.</param>
    /// <param name="userData">Collection where adding is.</param>
    private void ByTypeAccumulate_All( Win32.STATSTG item, object userData )
    {
      if( item.type == STGTY.STGTY_STREAM )
      {
        if( m_arrStreams != null ) m_arrStreams.Add( item.pwcsName );
      }
      else if( item.type == STGTY.STGTY_STORAGE )
      {
        if( m_arrStorages != null ) m_arrStorages.Add( item.pwcsName );
      }
    }
    /// <summary>
    /// Calculates subItems.
    ...
    private void CalculateSubItems( SubItemNameEventHandler caller, object userData )
    {
      ...
      if( error != 0 ) throw new ExternalException( DEF_FAIL, error );
      ...
      Win32.STATSTG item = new Win32.STATSTG();
      uint fetch = 0;
      ...
      if( error > 1 || error < 0 ) throw new ExternalException( DEF_FAIL, error );
    ...
    /// </summary>
    internal delegate void SubItemNameEventHandler( Win32.STATSTG item, object userData );
    #endregion
  }
}