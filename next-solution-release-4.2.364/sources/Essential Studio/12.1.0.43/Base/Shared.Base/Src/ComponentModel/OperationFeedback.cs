...
namespace Syncfusion.ComponentModel
{
	...
	public sealed class OperationFeedback: IDisposable
	{
		...
		public string Description
		{
			...
			{
#if DEBUG
				if (Switches.OperationFeedback.TraceVerbose)
...
#if DEBUG
			if (Switches.OperationFeedback.TraceVerbose)
			    TraceUtil.TraceCurrentMethodInfo(counter);
#endif
...
#if obsolete
	/// <summary>
	/// Use the Dispatcher if you have several objects providing feedback but
	/// only one resource to display the feedback. For example, the statusbar
	/// in a splitter control.
	/// </summary>
	public class OperationFeedbackDispatcher: IDisposable, IOperationFeedbackProvider
	{
		// REVIEW: OperationFeedbackDispatcher might be obsolete.
		// Why not simply implement the same mechanism directly in DelayedWaitCursor
		// and add a AddProvider method? - Have to think about this again, but for
		// now no harm is done by leaving this class.
		...
		public void AddProvider(IOperationFeedbackProvider provider)
		{
			...
			provider.OperationFeedback += new OperationFeedbackEventHandler(Progress);
			// association will go away when provider is removed
			providers.Add(new WeakReference(provider));
		}
		...
		private void Progress(object sender, OperationFeedbackEventArgs e)
		{
			if (OperationFeedback != null)
				OperationFeedback(sender, e);
		}
	}
#endif
