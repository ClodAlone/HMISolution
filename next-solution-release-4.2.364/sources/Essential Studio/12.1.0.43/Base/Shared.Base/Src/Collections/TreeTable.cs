...
namespace Syncfusion.Collections.BinaryTree
{
	...
	/// <exclude/>
	public interface IIsDisposedProperty
	{
		bool IsDisposed { get; } 
	}
	...
	public abstract class TreeTableNode : Disposable, ITreeTableNode
	{
		...
		public virtual string GetNodeInfo()
		{
			...
			if (Parent != null)
			{
				if (Object.ReferenceEquals(Parent.Left, this))
					side = "L";
				else
					side = "R";
			}
	...
	public class TreeTableBranch : TreeTableNode, ITreeTableBranch
	{
		...
		/// </summary>
		public TreeTableNodeColor Color
		{
			get { return _redBlack; }
			set { _redBlack = value; }
		}
	...
	public class TreeTable : Disposable, ITreeTable
	{
		...
		static BitVector32.Section __sorted;
		static BitVector32.Section __inAddMode;
		...
		static BitVector32.Section __reservedShort;


		static TreeTable()
		{
			__sorted = BitVector32.CreateSection(1);
			__inAddMode = BitVector32.CreateSection(1, __sorted);
			__reserved1 = BitVector32.CreateSection(1, __inAddMode);
			...
			__reservedShort = BitVector32.CreateSection(0xfff, __reserved4);
		}
		...
		bool _sorted
		{
			get
			{
				return bv[__sorted] != 0;
			}
			set
			{
				bv[__sorted] = value ? 1 : 0;
			}
		}
		...
		bool inAddMode
		{
			get
			{
				return bv[__inAddMode] != 0;
			}
			set
			{
				bv[__inAddMode] = value ? 1 : 0;
			}
		}
		...
		/// <internalonly/>
		public int ReservedShort
		{
			get
			{
				return bv[__reservedShort];
			}
			set
			{
				bv[__reservedShort] = value;
			}
		}
...
//		ITreeTableEntry lastInsertEntry = null;
		...
		{
			ITreeTableBranch y = x.Right as ITreeTableBranch;
			if (y == null)
				return;
		...
		{
			ITreeTableBranch y = x.Left as ITreeTableBranch;
			if (y == null)
				return;
		...
		private void InsertFixup(ITreeTableBranch x, bool inAddMode)
		{
				...
				&& x.Parent.Parent != null)
			{
				...
				{
					ITreeTableBranch y = x.Parent.Parent.Right as ITreeTableBranch;
				...
				else
				{
					...
					ITreeTableBranch y = x.Parent.Parent.Left as ITreeTableBranch;
					if (y != null && y.Color == TreeTableNodeColor.Red)
					{
						// uncle is red
						x.Parent.Color = TreeTableNodeColor.Black;
		...
		private void DeleteFixup(ITreeTableBranch x, bool isLeft)
		{
			...
			while (!Object.ReferenceEquals(x, _root) && x.Color == TreeTableNodeColor.Black)
			{
				if (isLeft)
				{
					ITreeTableBranch w = x.Parent.Right as ITreeTableBranch;
				...
				{
					ITreeTableBranch w = x.Parent.Left as ITreeTableBranch;
					if (w != null && w.Color == TreeTableNodeColor.Red)
					{
						w.Color = TreeTableNodeColor.Black;
		...
		ITreeTableEntry GetEntryAt(int index)
		{
			...
			if (index < 0 || index >= treeCount)
				throw new ArgumentOutOfRangeException("index", index, "must be between 0 and " + (treeCount-1).ToString());
...
//				lastCount = treeCount;
		...
		public void Insert(int index, ITreeTableNode value)
		{
			...
			int treeCount = GetCount();
			if (index < 0 || index > treeCount)
				throw new ArgumentOutOfRangeException("index", index, "must be between 0 and " + treeCount.ToString());
...
//					lastInsertEntry = null;
		...
		public void Remove(ITreeTableNode value)
		{
			_Remove(value, true);
		}
		...
		internal void _Remove(ITreeTableNode value, bool resetParent)
		{
			if (value == null)
				return;

			if (!Contains(value))
				throw new ArgumentException("is not a node in this tree", "value");
...
//			lastInsert = -1;
		...
		ITreeTableNode GetSisterNode(ITreeTableBranch leafsParent, ITreeTableNode node)
		{
			...
			if (Object.ReferenceEquals(leafsParent.Left, node))
				sisterNode = leafsParent.Right;
			else
				sisterNode = leafsParent.Left;
			return sisterNode;
		}
		...
		public int Add(ITreeTableNode value)
		{
			...
			if (Sorted && !this.inAddMode)
			{
				...
				// throw new InvalidOperationException("This tree is sorted - Use AddSorted instead or call this fn only after a BeginInit()");
			}
			...
			else
			{
				...
				ITreeTableBranch branch = null;
				ITreeTableNode current = lastAddBranch == null ? _root : lastAddBranch;
				...
				ITreeTableEntry leaf = (ITreeTableEntry) current;
				//if (branch != null)
				...
				else
				{
					...
					{
						Trace.WriteLine(branch.Parent.Parent.ToString());
						Trace.WriteLine(branch.Right.ToString());
					}
					if (!(branch.Parent == null || branch.Parent.Left.IsEntry() || ((TreeTableBranch)branch.Parent.Left).Right != branch))
					{
						Trace.WriteLine(branch.Parent.Left.ToString());
				...
				if (value.Parent != null && value.Parent.Parent != null)
				{
					if (value.Parent.Parent.Right == value)
					{
						...
						Trace.WriteLine(value.Parent.ToString());
						Trace.WriteLine(value.Parent.Parent.ToString());
		...
		public virtual void EndInit()
		{
			...
			TreeTableBranch branch = this._root as TreeTableBranch;
	...
	/// </summary>
	public class TreeTableEnumerator : IEnumerator
	{
	...
	/// </summary>
	public class TreeTableEntrySourceCollection : IList
	{
		...
		public ITreeTableEntrySource this[int index]
		{
			get
			{
				ITreeTableEntry entry = (ITreeTableEntry) _inner[index];
				if (entry == null)
					return null;
