...
namespace Syncfusion.Collections.BinaryTree
{
	...
	public class TreeTableWithCounterBranch : TreeTableWithSummaryBranch, ITreeTableCounterNode
	{
		...
		public override void InvalidateCounterBottomUp(bool notifyCounterSource)
		{
			...
			{
				if (TreeTableWithCounterEntry.traceVisibleCount)
				{
					Trace.WriteLine("InvalidateCounterBottomUp " + this.Tree.GetType());
					if (Tree != null && Tree.Tag != null)
						Trace.WriteLine(Tree.Tag.GetType());
				}

				if (Tree != null && Tree.Tag is ITreeTableCounterSource)
				{
					((ITreeTableCounterSource) Tree.Tag).InvalidateCounterBottomUp();
				}
			}
		}
	...
	public class TreeTableWithCounterEntry : TreeTableWithSummaryEntry, ITreeTableCounterNode
	{
		...
		public ITreeTableCounter GetCounterTotal()
		{
			if (counter == null)
			{
				...
				if (traceVisibleCount)
					Trace.Indent();
				if (source != null)
					counter = source.GetCounter();
				if (traceVisibleCount)
				{
					Trace.WriteLine("Counter Total: " + counter.ToString() + " / " + this.Value.GetType().Name);
					Trace.Unindent();
				}
			}
			return counter;
		...
		public override void InvalidateCounterBottomUp(bool notifyCounterSource)
		{
            if (traceVisibleCount)
            {
                Trace.WriteLine("InvalidateCounterBottomUp " + this.Value.GetType());
	...
	public class TreeTableWithCounter : TreeTableWithSummary
	{
		...
		TreeTableWithCounterEntry GetEntryAtCounterPosition(ITreeTableCounter start, ITreeTableCounter searchPosition, int cookie, bool preferLeftMost)
		{
			...
			{
				Trace.WriteLine(searchPosition.ToString());
				Trace.WriteLine(GetCounterTotal().ToString());
		...
		TreeTableWithCounterEntry GetEntryAtCounterPosition(ITreeTableNode currentNode, ITreeTableCounter start, ITreeTableCounter searchPosition, int cookie, bool preferLeftMost, out ITreeTableCounter currentNodePosition)
		{
			...
			while (!currentNode.IsEntry())
			{
				...
				ITreeTableCounterNode leftB = (ITreeTableCounterNode) branch.Left;
...
//			}
...
#if DEBUG
		/// <internalonly/>
		public static void DumpNode(ITreeTableNode currentNode)
		{
			if (currentNode.IsEntry())
			{
				Trace.WriteLine(currentNode.GetPosition() + ": " + ((ITreeTableEntry) currentNode).Value + " Pos " + ((TreeTableWithCounterEntry) currentNode).GetCounterPosition().GetValue(0x08));
			}
			else
			{
				Trace.WriteLine("Left");
				Trace.Indent();
				DumpNode(((ITreeTableBranch) currentNode).Left);
				Trace.Unindent();

				Trace.WriteLine("Right");
				Trace.Indent();
				DumpNode(((ITreeTableBranch) currentNode).Right);
				Trace.Unindent();
			}
		}
#endif
		...
		public ITreeTableEntry GetNextNotEmptyCounterEntry(ITreeTableEntry current, int cookie)
		{
			...
			else
			{
					...
					else
					{
						...
						else
						{
							...
							if (next == parentParent.Right)
							{
								...
								Trace.WriteLine(parentParent.ToString());
				...
				while (!next.IsEntry())
				{
					ITreeTableBranch branch = (ITreeTableBranch) next;
					if (!((ITreeTableCounterNode) branch.Left).GetCounterTotal().IsEmpty(cookie))
						next = branch.Left;
					else
						next = branch.Right;
				}
        ...
        public ITreeTableEntry GetPreviousNotEmptyCounterEntry(ITreeTableEntry current, int cookie)
        {
            ...
            else
            {
                ...
                {
                    ITreeTableBranch branch = (ITreeTableBranch)next;
                    if (!((ITreeTableCounterNode)branch.Right).GetCounterTotal().IsEmpty(cookie))
                        next = branch.Right;
                    else
                        next = branch.Left;
                }
		...
		/// </summary>
        /// <param name="value">The value.</param>
		public void Remove(TreeTableWithCounterEntry value)
		{
			base.Remove(value);
		}
