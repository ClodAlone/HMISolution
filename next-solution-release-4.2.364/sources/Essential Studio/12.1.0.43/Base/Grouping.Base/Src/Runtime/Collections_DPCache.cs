...
using Syncfusion.Grouping.Counters;
using Syncfusion.Grouping.Summaries;
using Syncfusion.Grouping.Filters;

namespace Syncfusion.Grouping
{
	#region UnsortedRecords
	public class UnsortedRecordsCollection : IList
	{
		...
		public UnsortedRecordsCollectionEnumerator GetEnumerator()
		{
			_table.EnsureInitialized(this);
...
//					allowCache = true;
				...
				if (cachedElement != null && version == _table.version)
				{
					...
					else  
					{
						...
						else if (/*_table.FilteredChildTable == null && */adjustedTableIndex == cachedAdjustedTableIndex+cachedElementCount)
						{
							...
							cachedElement = ElementHelper.GetNextElementStepIn(cachedElement, GroupByVisibleCounterCookies.CountDisplayElements, null, this._stepInNestedTables, this._table);
							...
							if (true)
							{
								...
								if (testIndexOf == -1 || testIndexOf != index)
								{
									...
									cachedElement = ElementHelper.GetNextElementStepIn(elx, GroupByVisibleCounterCookies.CountDisplayElements, null, this._stepInNestedTables, this._table);
								}
				...
				Element result = ElementHelper.FindElement(_table, GroupByVisibleCounter.CreateDisplayElementCounter(adjustedTableIndex), _stepInNestedTables);
				if (allowCache)
				{
		...
		int IndexOfHelper(Element value, int index)
		{
			...
			{
				Trace.WriteLine(index.ToString(), cachedAdjustedTableIndex.ToString());
			}
		...
		public Element GetItemAtDisplayMetrics(int index)
		{
			int count = Count;
			int index2 = index;
			if (_childTable != null)
				index2 += _childTable.GetDisplayMetricsPosition();
			else if (_table.FilteredChildTable != null)
				index2 += _table.FilteredChildTable.GetDisplayMetricsPosition();

			index2 = Math.Min(index2, _table.GetDisplayMetricsCount());
			return ElementHelper.FindElement(_table, GroupByVisibleCounter.CreateDisplayMetricsCounter(index2), _stepInNestedTables);
		}

		public int GetDisplayMetricsIndexOf(Element value)
		{
...
//				value = ElementHelper.GetNextElementStepIn(value, GroupByVisibleCounterCookies.CountDisplayElements, null, _stepInNestedTables);

			if (!Contains(value))
				return -1;

			int index = value.GetDisplayMetricsPosition();
			if (!_stepInNestedTables)
			{
				if (_childTable != null)
					index -= _childTable.GetDisplayMetricsPosition();
				else if (_table.FilteredChildTable != null)
					index -= _table.FilteredChildTable.GetDisplayMetricsPosition();
			}
			else
			{
				ChildTable childTable = value.ParentChildTable;
				while (childTable != null)
				{
					index -= childTable.GetDisplayMetricsPosition();
					NestedTable nestedTable = childTable.ParentNestedTable;
					if (nestedTable != null)
					{
						index += nestedTable.GetDisplayMetricsPosition();
						childTable = nestedTable.ParentChildTable;
					}
					else
						break;
	...
	public class DisplayElementsInTableCollectionEnumerator : IEnumerator 
	{
		...
		public bool MoveNext()
		{
			...
			{
				_next = ElementHelper.GetNextElementStepIn(_next, GroupByVisibleCounterCookies.CountDisplayElements, null, _coll._stepInNestedTables, _coll._table);
	...
	public class ElementsInTableCollection : IList
	{
			...
			get
			{
				...
				if (cachedElement != null && version == _table.version)
				{
					...
					else 
					{
						...
						{
							cachedAdjustedTableIndex += cachedElementCount;
							cachedElement = ElementHelper.GetNextElementStepIn(cachedElement, GroupByVisibleCounterCookies.CountElements, null, this._stepInNestedTables, this._table);
				...
				// Look up the element and cache it together with adjusted table index.
				Element result = ElementHelper.FindElement(_table, GroupByVisibleCounter.CreateElementCounter(adjustedTableIndex), _stepInNestedTables);
				if (cachedElement != null)
				{
					//lastCount = count;
					//lastFilteredChildTable = _table.FilteredChildTable;
		...
		/// <summary>
		/// Checks if the group belongs to the details section and is an element.
		/// </summary>
		/// <param name="value"></param>
		/// <returns></returns>
		public bool Contains(Element value)
	...
	public class ElementsInTableCollectionEnumerator : IEnumerator 
	{
		...
		public bool MoveNext()
		{
			...
			if (_coll._stepInNestedTables || --skipElements < 0)
			{
				_next = ElementHelper.GetNextElementStepIn(_next, GroupByVisibleCounterCookies.CountElements, null, _coll._stepInNestedTables, _coll._table);
		...
		public Record this[int index]
		{
			get
			{
				...
				if (lastIndex != -1 && lastCount == count && version == _table.version)
				{
					...
					{
						lastIndex++;
						lastElement = (Record) ElementHelper.GetNextElementStepIn(lastElement, GroupByVisibleCounterCookies.CountFilteredRecords, typeof(Record));
						Debug.Assert(lastElement == ElementHelper.__FindElement(_table, GroupByVisibleCounter.CreateFilteredRecordCounter(index), typeof(Record), false));
						return lastElement;
					}
				}

				lastElement = (Record) ElementHelper.__FindElement(_table, GroupByVisibleCounter.CreateFilteredRecordCounter(index), typeof(Record), false);
				lastCount = count;
				lastIndex = index;
				version = _table.version;
				return lastElement;
	...
	public class FilteredRecordsInTableCollectionEnumerator : IEnumerator 
	{
		...
		public bool MoveNext()
		{
			...
			_next = (Record) ElementHelper.GetNextElementStepIn(_next, GroupByVisibleCounterCookies.CountFilteredRecords, typeof(Record));
			...
			get
			{
				...
				if (lastIndex != -1 && lastCount == count && version == _table.version)
				{
					...
					else if (index == lastIndex+1)
					{
						...
						lastElement = (Record) ElementHelper.GetNextElementStepIn(lastElement, GroupByVisibleCounterCookies.CountRecords, typeof(Record));
						Element el = ElementHelper.__FindElement(_table, GroupByVisibleCounter.CreateRecordCounter(index), typeof(Record), false);
						if (lastElement != el)
						{
							...
							el = ElementHelper.__FindElement(_table, GroupByVisibleCounter.CreateRecordCounter(index), typeof(Record), false);
							el = (Record) ElementHelper.GetNextElementStepIn(prev, GroupByVisibleCounterCookies.CountRecords, typeof(Record));
							Trace.WriteLine(el.GetSortedRecordPosition());
							Debug.Assert(false);
						}
#else
				...
				lastElement = (Record) ElementHelper.__FindElement(_table, GroupByVisibleCounter.CreateRecordCounter(index), typeof(Record), false);
				lastCount = count;
				lastIndex = index;
				version = _table.version;
				return lastElement;
	...
	public class RecordsInTableCollectionEnumerator : IEnumerator 
	{
		...
		public bool MoveNext()
		{
			...
			_next = (Record) ElementHelper.GetNextElementStepIn(_next, GroupByVisibleCounterCookies.CountRecords, typeof(Record));
	...
	{
		internal SortedRecordsTree _inner;
...
//			product21.Records["Spegesild"].SetCurrent();
		...
		public Record this[int index]
		{
			get
			{
				...
				SortedRecordsTreeEntry entry = _inner[index];
				if (entry == null)
					return null;
	...
	public class FilteredRecordsInDetailsCollection : IList
	{
		...
		public Record this[int index]
		{
			get
			{
				...
				if (lastIndex != -1 && lastCount == count)
				{
					...
					{
						lastIndex++;
						lastElement = (Record) ElementHelper.GetNextSiblingElement(lastElement, GroupByVisibleCounterCookies.CountFilteredRecords);
						Debug.Assert(lastElement == ((SortedRecordsTreeEntry) _inner.GetEntryAtCounterPosition(GroupByVisibleCounter.CreateFilteredRecordCounter(index))).Element);
						return lastElement;
					}
				}

				SortedRecordsTreeEntry entry = (SortedRecordsTreeEntry) _inner.GetEntryAtCounterPosition(GroupByVisibleCounter.CreateFilteredRecordCounter(index));
		...
		public int IndexOf(Record value)
		{
			if (!Contains(value))
				return -1;
			return ((GroupByVisibleCounter) value.SortedEntry.GetCounterPosition()).FilteredRecordCount;
		}
	...
	public class FilteredRecordsInDetailsCollectionEnumerator : IEnumerator 
	{
		...
		public bool MoveNext()
		{
			...
			_next = (Record) ElementHelper.GetNextSiblingElement(_next, GroupByVisibleCounterCookies.CountRecords);
	...
	#region Groups
	/// <summary>
	/// A Read-only list of groups within a details section.
	/// </summary>
	public class GroupsInDetailsCollection : IList
	{
		internal GroupsTreeTable _inner;
		...
		internal GroupsInDetailsCollection(GroupsDetails groupWithGroups)
		{
			...
			if (_groupWithGroups != null)
				_inner = groupWithGroups.GroupTreeEntries;
//			else
//				throw new ArgumentNullException();
		...
		public Group this[int index]
		{
			get
			{
				...
				GroupsTreeTableEntry entry = _inner[index];
				if (entry == null)
					return null;
				return entry.Element;
			}
		...
		public int FindGroup(object category)
		{
			if (_groupWithGroups != null && _groupWithGroups.ParentGroup != null)
			{
				...
				Type type = this._groupWithGroups.ParentGroup.SortColumns[0].FieldDescriptor.GetPropertyType();
				object key = Convert.ChangeType(category, type);
				return _inner.TreeTable.IndexOfKey(new object[] { key });
			}
			return -1;
		}
...
//
//			Table productsTable = categoriesTable.RelatedTables["Category_Products"];
...
//			Console.WriteLine(product21.Records[0].ToString());
			...
			if (_groupWithGroups != null && _groupWithGroups.ParentGroup != null)
			{
				...
				int c = this._groupWithGroups.ParentTable.GetVisibleCount();
				//Console.WriteLine(this._groupWithGroups.ParentTable);
				return _inner.TreeTable.IndexOfKey(categoryKeys);
			}
			return -1;
		}
		...
		public Group this[string category]
		{
			get
			{
				...
				if (index == -1)
					throw new ArgumentException(category.ToString() + " not found in " + this._groupWithGroups.ParentGroup.SortColumns[0].Name);
				return this[index];
			}
		}
		...
		/// <summary>
		/// Checks if the group belongs to the details section.
		/// </summary>
		/// <param name="value"></param>
		/// <returns></returns>
		...
		/// <summary>
		/// Returns the index of the group within the details section.
		/// </summary>
		/// <param name="value"></param>
		/// <returns></returns>
		public int IndexOf(Group value)
		{
			if (!Contains(value))
				return -1;
			return value.GroupEntry.GetPosition();//InnerElementPosition;
		}
	...
	#endregion
	#region Sections
	public class SectionInGroupCollection : IList
	{
	...
	public class SectionInGroupCollectionEnumerator : IEnumerator 
	{
		...
		public SectionInGroupCollectionEnumerator(IList collection)
		{
			_coll = collection;
			_cursor = null;
			if (_coll.Count > 0)
		...
		public bool MoveNext()
		{
			...
			_next = (Section) ElementHelper.GetNextSibling(_next); //_next.GetNextSibling();
	...
	#endregion
	#region RecordParts
	public class RecordPartInRecordCollection : IList
	{
	...
	public class RecordPartInRecordCollectionEnumerator : IEnumerator 
	{
		...
		public RecordPartInRecordCollectionEnumerator(IList collection)
		{
			_coll = collection;
			_cursor = null;
			if (_coll.Count > 0)
