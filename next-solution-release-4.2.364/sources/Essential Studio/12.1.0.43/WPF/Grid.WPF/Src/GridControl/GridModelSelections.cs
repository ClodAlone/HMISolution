    ...
    public class GridModelSelections : IEnumerable
    {
        ...
        public void SelectRange(GridRangeInfo range, bool bSelect)
        {
                ...
                && StoreSelectRange(range, bSelect))
            {
                ...
                if (bSelect)
                    ...
                    RaiseSelectionChanged(range, oldRanges, GridSelectionReason.Clear);
        ...
        bool StoreSelectRange(GridRangeInfo range, bool bSelect)
        {
            ...
            else
            {
                ...
                // GridRangeInfo rangeEx = range.ExpandRange(1, 1, model.RowCount, model.ColumnCount); Unused local variable
                foreach (GridRangeInfo item in rl)
                {
                    ...
                    else
                    {
                        ...
                        if (item.Top < intersect.Top)
                        {
                            if (item.IsRows)
                                pSelList.Add(GridRangeInfo.Rows(item.Top, intersect.Top - 1));
                            else if (item.IsTable && intersect.IsRows)
                            {
                                pSelList.Add(GridRangeInfo.Rows(item.Top, intersect.Top - 1));
                                pSelList.Add(GridRangeInfo.Rows(intersect.Top + 1, model.RowCount));
                            }
                            else
        ...
        {
            ChangeSelection(oldRange, newRange, raiseEvents, null, reason);
        }
        public void ChangeSelection(GridRangeInfo oldRange, GridRangeInfo newRange, bool raiseEvents, MouseControllerEventArgs e, GridSelectionReason reason)
        {
            ...
            else
            {
                if (!raiseEvents || this.RaiseSelectionChanging(ref newRange, reason))
                {
                    ...
                    if (index != -1 && !(e != null && e.Button == MouseButton.Right && this.Model.EnableContextMenu))
                        pSelList.RemoveAt(index);
                    else if (pSelList.Count > 0 && !pSelList.AnyRangeContains(newRange) && (e != null && e.Button == MouseButton.Right && this.Model.EnableContextMenu))//clears the previous selections when right clicks on new selected row
                        pSelList.Clear();

                    GridRangeInfo pRange = newRange;
                    /*if (newRange.IsCells) -- new code in GridSelectCellsMouseController that uses ActiveRangeFunc to determine the click range
                    {
                        if (newRange.Top == 0 && newRange.Left == 0 && model.HeaderColumns > 0 && model.HeaderRows > 0)
                            pRange = GridRangeInfo.Table();
                        else if (newRange.Left == 0 && model.HeaderColumns > 0)
        ...
        /// <param name="count">No. of rows to insert.</param>
        /// <param name="moveState">Move state.</param>
        public void InsertRows(int insertAtRowIndex, int count, GridRangeInfoList moveState)
        {
            ...
            //this.RaiseSelectionChanged(GridRangeInfo.Empty, GridSelectionReason.Clear); //This code was added for SD7967, since it breaks the basic behavior, it was reverted temporarily. 
        ...
        /// <param name="count">No. of rows to remove.</param>
        /// <param name="moveState">Move state.</param>
        public void RemoveRows(int removeAtRowIndex, int count, GridRangeInfoList moveState)
        {
            //var selecrtedRange = this.Model.SelectedCells;
            //if (this.RaiseSelectionChanging(ref selecrtedRange, GridSelectionReason.Clear))
            //{
                for (int n = 0; n < Ranges.Count; n++)
                {
                    GridRangeInfo range = Ranges[n];
            ...
            if(this.Model is GridTreeModel)
                this.RaiseSelectionChanged(GridRangeInfo.Empty, GridSelectionReason.Clear);
            //}
        }
        ...
        /// <param name="count">No. of columns to insert.</param>
        /// <param name="moveState">Move state.</param>
        public void InsertColumns(int insertAtColumnIndex, int count, GridRangeInfoList moveState)
        {
            for (int n = 0; n < Ranges.Count; n++)
            {
                ...
                if (!range.IsEmpty && range.Left >= insertAtColumnIndex)
                {
                    ...
                    if (range.Right <= this.Model.ColumnCount)
                        Ranges[n] = range;
                }
            }
        ...
        /// <param name="count">No. of columns to remove.</param>
        /// <param name="moveState">Move state.</param>
        public void RemoveColumns(int removeAtColumnIndex, int count, GridRangeInfoList moveState)
        {
            ...
            {
                GridRangeInfo range = Ranges[n];
                if (!range.IsEmpty && (range.Left >= removeAtColumnIndex || range.Right >= removeAtColumnIndex))
                {
                    ...
                    else 
                    {
                        if (moveState != null)
                        {
                            moveState.Add(range.OffsetRange(0, 0));
                            range = GridRangeInfo.Empty;
                        }
                    }
                    Ranges[n] = range;
