...
namespace Syncfusion.Windows.Controls.Grid
{
    ...
    public class GridControlDragAutoScroller : AutoScroller
    {
        ...
        public GridControlDragAutoScroller(IScrollBarProvider scroll)
            : base(scroll)
        {
            this.grid = scroll as GridControlBase;
        }

        /// <summary>
        /// 
        /// </summary>
        /// <param name="mousePoint">Ckecks to scroll with Current mouse point</param>
        protected override void CheckAutoScroll(Point mousePoint)
        {
            if (this.grid.IsInDrag)
            {
                base.CheckAutoScroll(this.grid.DragPoint);
            }
            else if (Mouse.LeftButton == MouseButtonState.Pressed)
            {
                base.CheckAutoScroll(mousePoint);
            }
        }

        public override Rect InsideScrollBounds
        {
            get
            {
                if ((grid.FrozenColumns > 0 || grid.FrozenRows > 0 || grid.FooterColumns > 0 || grid.FooterRows > 0)
                     && grid.Model.GraphicModel.SelectedGraphicCells.Count <= 0)
                    return grid.GetClipRect(ScrollAxisRegion.Body, ScrollAxisRegion.Body);
                return base.InsideScrollBounds;
            }
        }
    }
    ...
    public class GridOleDropTarget : GridSubComponent, IGridOleDragDropEventsTarget
    {
        ...
        public virtual void OnDragEnter(DragEventArgs drgevent)
        {
            ...
            {
                //System.Diagnostics.Debug.WriteLine("OnDragEnter");
        ...
        public virtual void OnDragDrop(DragEventArgs drgevent)
        {
            ...
            Point pt = drgevent.GetPosition(grid);
            this.OutlineDropTargetRange(pt, true, false);

            // Something is wrong here, but at least we cleaned up everything.
            if (drgevent.Effects == DragDropEffects.None)
            {
                return;
            }

            DragDropEffects dropEffect = drgevent.Effects;

            // Check if the source grid is also the target grid and
            // drop cell is the same as the start cell.
            RowColumnIndex targetCell = grid.PointToCellRowColumnIndex(drgevent.GetPosition(this.grid));
                ...
                try
                {
                    ...
                    else
                    {
                        ...
                        try
                        {
                            ...
                            else if ((grid.Model.Options.DragDropDropTargetFlags & GridDragDropFlags.Move) != 0)
                            {
        ...
        public virtual void OnDragOver(DragEventArgs drgevent)
        {
            ...
            {
                drgevent.Effects = this.dragDropDropeffect;
                return;
            }

            // Outline the cells which will be replaced.
            Point pt = new Point(drgevent.GetPosition(this.grid).X, drgevent.GetPosition(this.grid).Y);
