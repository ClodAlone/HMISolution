...
namespace Syncfusion.RDL.Internal
{
    class ExpressionEngine
    {
        ...
        internal Dictionary<string, int> GroupLevels { get; set; }

        internal Dictionary<string, List<DataField>> FieldInformations { get; set; }

        internal Dictionary<string, ExpressionEval> Expressions { get; set; }

        internal List<IDictionary> CacheFieldValues { get; set; }
        ...
        internal ExpressionEngine()
        {
            ...
            this.GroupLevels = new Dictionary<string, int>();
            this.FieldInformations = new Dictionary<string, List<DataField>>();
            this.Expressions = new Dictionary<string, ExpressionEval>();
            this.CacheFieldValues = new List<IDictionary>();
        }
        ...
        internal ExpressionEngine(ReportModel pagemodel)
        {
            ...
            this.Expressions = new Dictionary<string, ExpressionEval>();
            this.CacheFieldValues = new List<IDictionary>();
            this.GenerateCodeClasses();
        }

        internal void NewExpressionFields()
        {
            this.CacheFieldValues.Add(this.FieldValues);
            this.FieldValues = new Dictionary<string, object>();
        }

        internal void ResetExpressionFields()
        {
            this.FieldValues.Clear();
            this.FieldValues = null;

            if(this.CacheFieldValues != null && this.CacheFieldValues.Count > 0)
            {
                this.FieldValues = this.CacheFieldValues.Last() as Dictionary<string,object>;
            }
        }

        internal void AddFieldValue(string key,object value)
        {
        ...
        internal void ResetExpressionCache()
        {
        ...
        internal void AddTablixDataFields(string key,IReportItemModeler reportItem)
        {
            if (!string.IsNullOrEmpty(key) && this.FieldInformations.ContainsKey(key))
            {
                var fields = this.FieldInformations[key];

                foreach (var field in fields)
                {
                    var fieldsCount = (from expField in reportItem.DataSetFields
                    ...
                    if (fieldsCount == 0)
                    {
                        reportItem.DataSetFields.Add(field);
                    }
                }
            }
        }

        internal void UpdateExpressionFieldValues(string key)
        {
            if (!string.IsNullOrEmpty(key) && this.Expressions.ContainsKey(key))
            {
                if (this.FieldInformations.ContainsKey(key))
                {
                    var fieldInfos = this.FieldInformations[key];
                    var fields = (from field in fieldInfos
                                  where !this.FieldValues.ContainsKey(field.Name) && field.IsDataSetField
                                  select field).GroupBy(group => group.DataSetName);
                    ...
                    if (fields.Count() > 0)
                    {
                        foreach (var fieldKey in fields)
                        {
                            ...
                            {
                                this.FieldValues.Add(field.Name, field.Value.Value.GetResult());
                            }
        ...
        public object GetEvalExpression(string key)
        {
            try
            {
                if (!string.IsNullOrEmpty(key) && this.Expressions.ContainsKey(key))
                {
                    this.UpdateExpressionFieldValues(key);
                    ExpressionEval eval = this.Expressions[key];
            ...
            {
                throw new Exception("Unsupported expression " + this.Expressions[key].Expression);
            }
        }

        public string GetEvalExpressionString(string key)
        {
            object value = this.GetEvalExpression(key);
        ...
        {
            return this.GetExpressionKey(expression, false);
        }

        public string GetExpressionKey(string expression, string dataSetName)
        {
            return this.GetExpressionKey(expression, dataSetName, false);
        }

        public string GetExpressionKey(string expression, bool isChildItem)
        {
            return this.GetExpressionKey(expression, null, isChildItem);
        }

        public string GetExpressionKey(string expression, string dataSetName, bool isChildItem)
        {
            ...
            if (expression.Contains("User!Language"))
                expression = expression.Replace("User!Language", "\"" + CultureInfo.CurrentCulture.Name + "\"");
            if (expression.Contains("Globals.ReportName"))
                expression = expression.Replace("Globals.ReportName", "\"" + this.pageModel.GetReportName() + "\"");
            if (expression.Contains("Globals!ReportName"))
                expression = expression.Replace("Globals!ReportName", "\"" + this.pageModel.GetReportName() + "\"");
            ...
            if (expression.Contains("Globals!PageNumber"))
                expression = expression.Replace("Globals!PageNumber", "\"" + "Globals.PageNumber" + "\"");
            if (expression.Contains("Globals.OverallPageNumber"))
                expression = expression.Replace("Globals.OverallPageNumber", "\"" + "Globals.PageNumber" + "\"");
            if (expression.Contains("Globals!OverallPageNumber"))
                expression = expression.Replace("Globals!OverallPageNumber", "\"" + "Globals.PageNumber" + "\"");
            ...
            {
                if (pageModel.IsServerReport)
                {
                   expression = expression.Replace("User!UserID", "\"" + pageModel.ReportServerCredential.GetCredential(null,null).Domain + "\\" + pageModel.ReportServerCredential.GetCredential(null,null).UserName + "\"");
                }
                else
                {
            ...
            List<DataField> DataFields = this.UpdateExpressionFields(expression, dataSetName, out evaluatedExp, isChildItem);
            ExpressionEval eval = new ExpressionEval(evaluatedExp, this);

            this.FieldInformations.Add(key, DataFields);
            this.Expressions.Add(key, eval);
        ...
        internal string GetTypeName(string fieldName, string datasetName)
        {
            var fieldValue = this.pageModel.ReportDataSets.Where(dataset => dataset.Name.Equals(datasetName)).First()
                                                        .Fields.Where(field => field.Name.Equals(fieldName)).FirstOrDefault();

            if (fieldValue != null && fieldValue.TypeName != null)
            {
        ...
        internal string GetFieldName(string fieldName, string datasetName)
        {
            var fieldValue = this.pageModel.ReportDataSets.Where(dataset => dataset.Name.Equals(datasetName)).First()
                                                        .Fields.Where(field => field.Name.Equals(fieldName)).FirstOrDefault();

            if (fieldValue != null && fieldValue.DataField != null)
            {
                return fieldValue.DataField;
            }
        ...
        public List<DataField> UpdateExpressionFields(string expression, string dataSetName, out string resultExpression, bool isChildItem)
        {
            try
            {
                    ...
                    if (match.Success)
                    {
                        if (this.IsAggregateFunction(match.Groups["Function"].Value))
                        {
                            ...
                            if (parameters.Length == 3)
                            {
                                ...
                                Match subMatch = new Regex(@"^((?i)Fields!)\s*(?<Field>\w+)\s*((?i).Value)$").Match(subexp);

                                bool isRecursive = false;
                                string groupName = parameters[1].Trim();

                                if (parameters[2].Trim() == "Recursive")
                                {
                                    isRecursive = true;
                                }
                                ...
                                if (subMatch.Success)
                                {
                                    ...
                                    var sumField = this.UpdateExpressionFields(subexp, dataSourceName, out subEvaluatedExpression, true).First();
                                    ...
                                    sumField.IsDataSetField = true;
                                    sumField.GroupName = groupName;
                                    sumField.IsRecursiveField = isRecursive;
                                ...
                                {
                                    string expressionKey = this.GetExpressionKey("=" + subexp, dataSetName, true);
                                    List<DataField> fields = this.FieldInformations[expressionKey];
                                    ...
                                    foreach (var field in fields)
                                    {
                                        ...
                                        field.IsRecursiveField = isRecursive;
                                        if (fieldsCount == 0)
                                        {
                                    ...
                                    sumField.FunctionName = match.Groups["Function"].Value;
                                    sumField.Expression = expressionKey;
                                    resultExpression = resultExpression.Replace(filedExpression, "Fields!" + sumField.Name + ".Value");
                                    expressionFields.Add(sumField);
                                }
                            }
                            else if (parameters.Length == 1 && isChildItem)
                            {
                                string subexp = parameters[0].Trim();
                                Match subMatch = new Regex(@"^((?i)Fields!)\s*(?<Field>\w+)\s*((?i).Value)$").Match(subexp);
                                ...
                                {
                                    string subEvaluatedExpression = null;
                                    var sumField = this.UpdateExpressionFields(subexp, dataSetName, out subEvaluatedExpression, true).First();
                                    string fieldGuid = Guid.NewGuid().ToString().Split('-')[0];
                                    sumField.Name = "Field" + fieldGuid;
                                    sumField.FunctionName = match.Groups["Function"].Value;
                            ...
                            else if (!isChildItem || (isChildItem && parameters.Length > 1))
                            {
                                string dataSourceName = null;
                                ...
                                {
                                    string expressionKey = this.GetExpressionKey("=" + subexp, dataSourceName, true);

                                    List<DataField> fields = this.FieldInformations[expressionKey];

                                    foreach (var field in fields)
                                    {
                                        field.IsDataSetField = true;

                                        var fieldsCount = (from expField in expressionFields
                                                           where field.FunctionName.Equals(expField.FunctionName)
                                                               && field.Name.Equals(expField.Name)
                                                           select expField).Count();
                ...
                if (isChildItem)
                {
                    do
                    {
                        match = new Regex(@"((?i)Fields!)\s*(?<Field>\w+)\s*((?i).Value)").Match(expression);
                        ...
                        if (match.Success)
                        {
                            ...
                            sumField.FieldName = this.GetFieldName(field, dataSetName);
                            sumField.DataSetName = dataSetName;
                            sumField.DataTypeName = this.GetTypeName(sumField.FieldName, dataSetName);
            ...
            {
                throw new Exception("Expression  "+  expression + "is invalid");
            }
        }

        public List<ParameterField> UpdateParameterFields(string expression, out string resultExpression)
        {
            List<ParameterField> parameterFields = new List<ParameterField>();
            resultExpression = expression;
            Match match = null;

            do
            {
                match = ExpressionMatch.Parameter.Match(expression);
                string paramGuid = Guid.NewGuid().ToString().Split('-')[0];
                ParameterField param = new ParameterField();
                param.Name = "param" + paramGuid;
                param.ParamName = match.Groups["Parameter"].Value;
                param.Data = match.Groups["Data"].Value;
                param.ParamType = "String";
                resultExpression = resultExpression.Replace(match.Value, param.ParamName);
            } while (match.Success);
        ...
        public object GetParameterPlaceHolderValue(string parameter, string paramData)
        {
            ...
            if (parameters.Count() > 0)
            {
                ...
                if (param.Value != null && param.Value.Count > 0)
                {
                    ...
                    {
                        if (param.Label.Count == 0 && param.Value.Count != 0)
                        {
                            return (param.IsMultiValue) ? param.Value : param.Value.First();
                        }
        ...
        internal string GetFormattedText(object value, string format, string language)
        {
            if (value != null)
            {
                ...
                if (!string.IsNullOrEmpty(format) || !string.IsNullOrEmpty(language))
                {
                    ...
                    {
                       string formattedtext = numberData.ToString(format, info);

                       if (Regex.IsMatch(formattedtext, @"^[a-zA-Z]+$"))
                       {
                           return text;
                       }
        ...
        internal class ExpressionMatch
        {
            ...
            private const string FieldMatch = @"((?i)Fields!)\s*(?<Field>\w+)\s*((?i).Value)";
            private const string EnumMatch = @"(?<Enum>(CompareMethod|DueDate|vbStrConv|FirstDayOfWeek|DateFormat|DateInterval)).(?<Value>\w+)";
            private const string ConstMatch = @"(?<Name>(Now|Nothing|System.Globalization.CultureInfo.\w+))";
            ...
            internal static Regex ExecutionTimeMatch = new Regex(
               "Globals!ExecutionTime|Globals.ExecutionTime",
               RegexOptions.None
           );
        }


        internal void GenerateCodeClasses()
        {
            Assembly codeAssembly = null;

#if !SILVERLIGHT
            if (this.pageModel.Report != null && this.pageModel.Report.Code != null)
            {
...
#else                
                cp.ReferencedAssemblies.Add("syncfusion.reportcontrols.wpf.dll");
#endif
                ...
                string[] imports = { "Imports System\n", "Imports Microsoft.VisualBasic\n", "Imports System.Convert\n", "Imports System.Math\n", "Imports Syncfusion.RDL.DOM\n", "namespace Syncfusion.Reports.Controls.Data\n", "Public Class Code\n" };
                string[] endtags = { "End Class\n", "End Namespace\n" };
                List<string> functions = new List<string>();
                string function = "";
                foreach (string line in codes)
                {
                    if (Regex.Match(line, "(?i)(function)").Success && !Regex.Match(line, @"(?i)(\s*end\s+function\s*)").Success)
                    {
                ...
                List<string> CompiledFn = new List<string>();
                for (int i = 1; i <= functions.Count(); i++)
                {
                    function = string.Join("\n", imports);
                    function += string.Join("\n", functions.Take(i).ToArray());
                    function += "\n";
                    function += string.Join("\n", endtags);
                    CompilerResults preCompile = provider.CompileAssemblyFromSource(cp, function);
                    {
                        if (preCompile.Errors.HasErrors == false)
                        {
                            CompiledFn.Add(functions[i - 1]);
                        }
                        preCompile = null;
                    }
                }
...
#endif
            UpdateCodeClass(codeAssembly);

            if (this.pageModel.Report != null && this.pageModel.Report.CodeModules != null)
            {
                foreach (var codeModule in this.pageModel.Report.CodeModules)
                {
                    ...
                    //        codeAssembly = new System.Windows.AssemblyPart().Load(info.Stream);
                    //        break;
                    //    }
                    //}
#elif SILVERLIGHT
        ...
        public void UpdateCodeClass(Assembly assembly)
        {
            if (assembly != null)
            {
#if !WINRT
        ...
        internal string EvaluateCodeValue(string function, object[] parameters)
        {
            string[] classMethods = function.Split('.');

            if (this.pageModel.Report.Classes != null && this.pageModel.Report.Classes.Count > 0)
            {
                foreach (var name in this.pageModel.Report.Classes)
                {
                    if (classMethods.Contains(name.InstanceName))
                    {
                        classMethods[Array.IndexOf(classMethods, name.InstanceName)] = name.ClassName;
                    }
                }
            }
            ...
            for (int i = 1; i < classMethods.Length; i++)
            {
                ...
                else
                {
                    ...
                    if (objectType == null)
                    {
                        if (!string.Equals(classMethods[0], "code", StringComparison.InvariantCultureIgnoreCase))
                        {
                            name = classMethods[0];
                            i = 0;
                        }
        ...
        internal class ExpressionEval
        {
            ...
            public ExpressionEval(string expression, ExpressionEngine engine)
            {
                ...
                if (!isParsed)
                {
                    for (int x = 0; x < this.Expression.Length; x = NextToken(x)) ;
                }
            ...
            private int NextToken(int startIndex)
            {
                ...
                //Apply paraanchas for unary operators .
                if(this.Expression.StartsWith("NOT ",StringComparison.CurrentCultureIgnoreCase))
                {
                  var expr = this.Expression.Replace(" ",string.Empty);

                  if (expr.IndexOf('(')!= expMatchedObject.Index)
                  {
                      this.Expression = this.Expression.Insert(expMatchedObject.Index, "(");
                      this.Expression = this.Expression.Insert(this.Expression.Length, ")");
                  }
                }
                ...
                if (RegexObject == null || RegexObject.Index > startIndex)
                {
                    ...
                    if (expMatchedObject.Success && (RegexObject == null || expMatchedObject.Index < RegexObject.Index))
                    {
                        RegexObject = expMatchedObject;
                        DateTime value;
                        bool isDate = DateTime.TryParse(expMatchedObject.Groups["Date"].Value,out value);
                ...
                if (RegexObject == null || RegexObject.Index > startIndex)
                {
                    ...
                    {
                        RegexObject = expMatchedObject;
                        ParameterExpression expression = new ParameterExpression(this.expressionEngine);
                        expression.Name = expMatchedObject.Groups["Parameter"].Value;
                        expression.Mode = expMatchedObject.Groups["Data"].Value;
                        val = expression;
                    }
                }
                ...
                if (RegexObject == null || RegexObject.Index > startIndex)
                {
                    ...
                    {
                        RegexObject = expMatchedObject;
                        FieldExpression expression = new FieldExpression(this.expressionEngine);
                        expression.Name = RegexObject.Groups["Field"].Value;
                        val = expression;
                    }
                }
            ...
            {
                List<object> list = expressionlist.ToList();
                ...
                for (int x = 0; x < list.Count; x++)
                {
                    ...
                    else if (list[x] is FunctionEval)
                    {
                        list[x] = ((FunctionEval)list[x]).Evaluate();
                    }
                    else if (list[x] is FieldExpression)
                    {
                        list[x] = ((FieldExpression)list[x]).Evaluate();
                    }
                    else if (list[x] is ParameterExpression)
                    {
                        list[x] = ((ParameterExpression)list[x]).Evaluate();
                    }
                    else if (list[x] is ExpressionEval)
                    {
                        list[x] = ((ExpressionEval)list[x]).Evaluate();
                    }
                }

                //Do the evluation for Split with index
                int val;
                if (list.Count == 2 && int.TryParse(list[1].ToString(), out val))
                {
                    try
                    {
                        if (list[0] is IEnumerable && int.TryParse(list[1].ToString(), out val))
                            return (list[0] as IEnumerable).Cast<object>().ToList().ElementAt(val);
                        else if (list[0] == null)
                            return "#Error";
                ...
                //}
                //else if (list[0] is FieldExpression)
                //{
                //    retExpression = ((FieldExpression)list[0]).Evaluate();
                //}
                //else if (list[0] is ParameterExpression)
                //{
                //    retExpression = ((ParameterExpression)list[0]).Evaluate();
                //}
            ...
            private static object PerformBinaryOp(BinaryOperator binOperator, object var1, object var2)
            {
                ...
                else if (var1 is FieldExpression)
                {
                    var1 = ((FieldExpression)var1).Evaluate();
                }
                else if (var1 is ParameterExpression)
                {
                    var1 = ((ParameterExpression)var1).Evaluate();
                }
                ...
                else if (var2 is FieldExpression)
                {
                    var2 = ((FieldExpression)var2).Evaluate();
                }
                else if (var1 is ParameterExpression)
                {
                    var2 = ((ParameterExpression)var2).Evaluate();
                }
            ...
            private static object PerformUnaryOp(UnaryOperator unaryOperator, object var)
            {
                ...
                else if (var is ParameterExpression)
                {
                    var = ((ParameterExpression)var).Evaluate();
                }
                else if (var is FieldExpression)
                {
                    var = ((FieldExpression)var).Evaluate();
                }
            ...
            internal object Evaluate(object var)
            {
                ...
                switch (function.ToLower(CultureInfo.CurrentCulture))
                {
                    ...
                    case "length":
                        if (var is object[])
                        {
                            return (var as object[]).Length;
                        }
                        else
                        {
                            return (var as string).Length;
                        }
                }
#endif
        ...
        internal class FieldExpression
        {
            ...
            public FieldExpression(ExpressionEngine expressionEngine)
            {
                this.ExpressionEngine = expressionEngine;
            }
            ...
            internal object Evaluate()
            {
                ...
                return fieldValues.Keys.Contains(this.Name) ? fieldValues[this.Name] : null;
            }
        }
        ...
        internal class ParameterExpression
        {
            ...
            public ParameterExpression(ExpressionEngine expressionEngine)
            {
                this.ExpressionEngine = expressionEngine;
            }
            ...
            internal object Evaluate()
            {
                return this.ExpressionEngine.GetParameterPlaceHolderValue(this.Name, this.Mode);
            }
        }
        ...
        internal class FunctionEval
        {
            ...
            public void Parse()
            {
                ...
                if (!isParsed)
                {
                    ...
                    Match  m = ExpressionMatch.Function.Match(strNext);

                    if (m.Success && !strNext.ToLower().StartsWith("code."))
                    {
                    ...
                    {
                        m = ExpressionMatch.CodeMatchFunction.Match(strNext);
                        if (m.Success)
                        {
            ...
            public object Evaluate()
            {
                object retExp = ExecuteFunction(function, pars);
                return retExp;
            }
            ...
            private object ExecuteFunction(string name, object[] p)
            {
                ...
                if (isCodeModuleFunction)
                {
                    ...
                    {
                        return this.expressionEngine.EvaluateCodeValue(name, parameters);
                    }
                    else
                    {
                        return "#Error";
                ...
                switch (name.ToLower())
                {
                    ...
                    case "round":
                        if (parameters.Length!=0 && string.IsNullOrEmpty(parameters[0].ToString()))
                        {
                            parameters[0] = 0;
                        }
                            return (parameters.Length > 1) ?
                    ...
                    case "monthname":
                        if (parameters.Count()==2 &&parameters[1] != null && parameters[1].ToString().ToLower() == "true")
                        {
                            return (new DateTime(2000, Convert.ToInt32(parameters[0]), 1)).ToString("MMM");
                        }
                    ...
                    case "rownumber":
                        if (parameters != null && parameters.Count() > 0)
                        {
                            try
                            {
                                return this.expressionEngine.RowNumbers.Count != 0 ? (parameters[0] != null && !string.IsNullOrEmpty(parameters[0].ToString()) ?
                                       this.expressionEngine.RowNumbers[parameters[0].ToString()] : this.expressionEngine.RowNumbers.First().Value) : 0;
                            }
                            catch
                            {
                                return "#Error";
                   ...
                   case "level":
                        if (parameters != null && parameters.Count() > 0)
                        {
                            return this.expressionEngine.GroupLevels.Count != 0 ? this.expressionEngine.GroupLevels[parameters[0].ToString()] : 0;
                        }
                        else
                        {
                            return this.expressionEngine.GroupLevels.Count != 0 ? this.expressionEngine.GroupLevels["Level"]:0;
                        }
                    case "countrows":
                        return GetRowCount(parameters);
                    case "tolocaltime":
                        return ToLocalTime(parameters);
                    default:
                        return "#Error";
                }
            }

            private object GetRowCount(object[] parameters)
            {
                var DatasourceObjects = this.expressionEngine.pageModel.ProcessedData.DataSourceObjects;
                int count = 0;
                ...
                if (DatasourceObjects != null)
                {
                    if (parameters != null)
                    {
                        foreach (var dataset in DatasourceObjects)
                        {
                            if (dataset.Key == parameters[0].ToString())
                            {
                                foreach (var res in dataset.Value)
                                    count++;
                        ...
                        foreach (var reportItem in this.expressionEngine.pageModel.ProcessedData.Report.Body.ReportItems)
                        {
                            if (reportItem is Syncfusion.RDL.DOM.Tablix)
                            {
                                RDL.DOM.Tablix item = reportItem as Syncfusion.RDL.DOM.Tablix;

                                if (reportItem.Name == parameters[0].ToString())
                                {
                                    foreach (var datasetval in DatasourceObjects)
                                    {
                                        if (datasetval.Key == item.DataSetName)
                                        {
                                            foreach (var res in datasetval.Value)
                                                count++;
                                ...
                                else if (item.TablixRowHierarchy != null)
                                {
                                    foreach (var groupinfo in item.TablixRowHierarchy.TablixMembers)
                                    {
                                        if (groupinfo.Group != null && groupinfo.Group.Name == parameters[0].ToString())
                                        {
                                            foreach (var datasetval in DatasourceObjects)
                                            {
                                                if (datasetval.Key == item.DataSetName)
                                ...
                                else if (item.TablixColumnHierarchy != null)
                                {
                                    foreach (var groupinfo in item.TablixColumnHierarchy.TablixMembers)
                                    {
                                        if (groupinfo.Group != null && groupinfo.Group.Name == parameters[0].ToString())
                                        {
                                            foreach (var datasetval in DatasourceObjects)
                    ...
                    {
                        foreach (var res in DatasourceObjects.First().Value)
                            count++;
                        return count;
                    }
                }
            ...
            private static object FormatDateTime(object[] parameters)
            {
               int formatValue = -1;
                ...
                {
                     formatValue = int.Parse(parameters[1].ToString());
                }

                else if (parameters.Count() == 1)
                {
                    DateTime date = Convert.ToDateTime(parameters[0]);
                    object obj = date.ToString();
                    return date;
                }
                ...
                switch (formatValue)
                {
                    ...
                    case 3:
                        {
                            DateTime date = Convert.ToDateTime(parameters[0]);
                            object obj = date.ToString("h:mm:ss tt"); 
                            return obj;
                        }
                    case 4:
                        {
                            DateTime date = Convert.ToDateTime(parameters[0]);
                            object obj = date.ToString("HH:mm");
                            return obj;
                        }
                    default:
            ...
            private object ToLocalTime(object[] parameters)
            {
                if (parameters[0] != null)
                {
                    DateTime dt = Convert.ToDateTime(parameters[0]);
#if !SILVERLIGHT
                    object obj = System.TimeZone.CurrentTimeZone.ToLocalTime(dt);
#else
                    object obj = dt.ToLocalTime();
#endif
                    return obj;
                }
                return "#error";
            ...
            public static object Iif(params object[] parameters)
            {
                ...
                try
                {
                    if (parameters[0] == null || (parameters[0] != null && string.IsNullOrEmpty(parameters[0].ToString())))
                    {
                        return parameters[2];
                    }

                    else if (Convert.ToBoolean(parameters[0], CultureInfo.CurrentCulture))
                        return parameters[1];
                    return parameters[2];
                }
                catch
            ...
            public static object ParseExact(params object[] parameters)
            {
                try
                {
                    if (parameters[2] != null && !String.IsNullOrEmpty(parameters[2].ToString()))
                    {
                        return DateTime.ParseExact(parameters[0].ToString(), parameters[1].ToString(), parameters[2] as CultureInfo);
                    }
                }
            ...
            {
                if (parameters.Length == 1 || (parameters.Length == 2 && parameters[1].ToString().ToLower().Equals("standard")))
                {
                    return parameters[0].ToString();
                }
                ...
                if (DateTime.TryParse(parameters[0].ToString(),out isDate))
                {
                    DateTime dt = isDate;
                ...
                {
                    return string.Format(parameters[0].ToString(), parameters.Where((val, index) => index != 0).ToArray());
                }
            ...
            public static object FormatNumber(params object[] parameters)
            {
                if (!string.IsNullOrEmpty(parameters[0].ToString()))
                {
                    ...
                    try
                    {
                         value = Convert.ToDouble(parameters[0]);
                    }
                    catch
                    {
                        return "#Error";
