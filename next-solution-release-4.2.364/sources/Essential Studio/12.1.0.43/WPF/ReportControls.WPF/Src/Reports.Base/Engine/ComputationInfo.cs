...
namespace Syncfusion.RDL.Internal
{
    ...
    internal class DoubleMinSummary : SummaryBase
    {
        ...
        internal object min = double.MaxValue;

        public override void Combine(object other)
        {
            object d = double.MaxValue;
            if (other is double)
            {
                d = (double)other;
            }
            else if (other is int || other is Int16 || other is decimal)
            {
#if SILVERLIGHT
            ...
            else if (d is string)
            {
                ...
                min = string.Compare(d.ToString(), min.ToString()) > 0 ? min : d;
            }
            else if (d is DateTime)
            {
                ...
                {
                    DateTime d1 = (DateTime)d;
                    DateTime d2 = (DateTime)min;
                    min = DateTime.Compare(d1, d2) > 0 ? min : d;
                }
            }
        }
        ...
        public override void CombineSummary(SummaryBase other)
        {
            ...
            if (asum.min is double)
            {
                double d1 = (double)asum.min;
                double min1 = (double)min;
                if (min1 > d1)
                    min = d1;
            }
            else if (asum.min is string)
            {
                if (min is double)
                {
                    min = asum.min;
                }
                min = string.Compare(asum.min.ToString(), min.ToString()) > 0 ? min : min;
            }
            else if (asum.min is DateTime)
            {
                if (min is double)
                {
                    min = asum.min;
                ...
                {
                    DateTime d1 = (DateTime)asum.min;
                    DateTime d2 = (DateTime)min;
                    min = DateTime.Compare(d1, d2) > 0 ? min : asum.min;
                }
            }
        }
    ...
    internal class DoubleMaxSummary : SummaryBase
    {
        ...
        internal object max = double.MinValue;
        ...
        public override void Combine(object other)
        {
            ...
            else if (d is string)
            {
                ...
                max = string.Compare(d.ToString(), max.ToString()) > 0 ? d : max;
            }
            else if (d is DateTime)
            {
                if (max is double)
                ...
                {
                    DateTime d1 = (DateTime)d;
                    DateTime d2 = (DateTime)max;
                    max = DateTime.Compare(d1, d2) > 0 ? d : max;
                }
            }
        }
        ...
        public override void CombineSummary(SummaryBase other)
        {
            ...
            if (asum.max is double)
            {
                double d1 = (double)asum.max;
                double max1 = (double)max;
                if (max1 < d1)
                    max = d1;
            }
            else if (asum.max is string)
            {
                if (max is double)
                {
                    max = asum.max;
                }
                max = string.Compare(asum.max.ToString(), max.ToString()) > 0 ? asum.max : max;
            }
            else if (asum.max is DateTime)
            {
                if (max is double)
                {
                    max = asum.max;
                ...
                {
                    DateTime d1 = (DateTime)asum.max;
                    DateTime d2 = (DateTime)max;
                    max = DateTime.Compare(d1, d2) > 0 ? asum.max : max;
                }
            }
        }
    ...
    internal class DoubleStDevPSummary : SummaryBase
    {
        ...
        public override void CombineSummary(SummaryBase other)
        {
            DoubleStDevPSummary asum = (DoubleStDevPSummary)other;
            sumX2 += asum.sumX2;
            sumX += asum.sumX;
            n += asum.n;
        }
    ...
    internal class DoubleVariancePSummary : SummaryBase
    {
        ...
        public override void CombineSummary(SummaryBase other)
        {
            DoubleVariancePSummary asum = (DoubleVariancePSummary)other;
            sumX2 += asum.sumX2;
            sumX += asum.sumX;
            n += asum.n;
        }
    ...
    internal class IntTotalSummary : SummaryBase
    {
        ...
        public override void Combine(object other)
        {
            if (other is int || other is Int16)
            {
                total += (int)other;
            }
    ...
    internal class DecimalTotalSummary : SummaryBase
    {
        ...
        public override void Combine(object other)
        {
            ...
            else if (other is double || other is int || other is Int16)
            {
#if SILVERLIGHT
    ...
    internal class TextSummary : SummaryBase
    {
        ...
        public override void CombineSummary(SummaryBase other)
        {
            values.Add(((TextSummary)other).GetResult() as string);
        }
