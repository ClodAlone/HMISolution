...
namespace Syncfusion.Windows.Styles
{
      ...
      IXmlSerializable
    {
                ...
                if (sip.IsSerializable && this.HasValue(sip))
                {
                    ...
                    else
                    {
                        ...
                        if (sb == SerializeXmlBehavior.SerializeAsString)
                        {
                            ...
                            else if (sip.PropertyType == typeof(Pen))
                            {
                                var text = SerialzeObject(value);
                                writer.WriteString(text);
                            }
                            else
                                writer.WriteString(sip.FormatValue(value));
                        }
                        else if (sb == SerializeXmlBehavior.SerializeWithXmlSerializer && sip.PropertyType != typeof(Brush))
                        {
                            try
                            {
        ...
        public string SerialzeObject(object obj)
        {
            var pdc = TypeDescriptor.GetProperties(obj);
            string text = "";
            foreach (PropertyDescriptor pd in pdc)
            {
                var localValue = pd.GetValue(obj);
                if (localValue != null)
                {
                    text += "<" + pd.Name + ">";
                    text += localValue;
                        ...
                        else
                        {
                            ...
                            else
                            {
                                ...
                                if (sb == SerializeXmlBehavior.SerializeAsString)
                                {
                                    ...
                                    {
                                        value = DeserialzeObject(new Pen(), xmlValue);
                                    }
                                    else
                                ...
                                else if (sb == SerializeXmlBehavior.SerializeWithXmlSerializer)
                                {
                                    try
                                    {
                                        ...
                                        while (r.Name == "Type")
                                        {
                                            ...
                                            if (externalTypes[0] == null)
                                                r.Read();
                                        }

                                        // Create serializer based on <Type> tags that we just read in.
                                        if (r.AttributeCount > 0 && externalTypes != null && externalTypes[0] != null)
                                        {
                                        ...
                                        else
                                        {
                                            ...
                                            {
                                                if (sip.PropertyType == typeof(System.Object))
                                                {
                                                    string name = r.Name;
                                                    if(r.NodeType == XmlNodeType.Whitespace ||(r.HasAttributes && r.GetAttribute(0) == "ArrayOfString"))
                                                        r.Read();
                                                    List<string> value1 = new List<string>();

                                                    while (r.Name != name && r.NodeType != XmlNodeType.EndElement)
                                                    {
                                                        r.ReadStartElement();
                                                        while (r.NodeType == XmlNodeType.Text)
                                                        {
                                                            value1.Add(r.ReadContentAsString());
                                                            r.Read();
                                                        }
                                                        r.Read();
                                                        continue;
                                                    }
                                                    this.SetValue(sip, value1);
        ...
        public object DeserialzeObject(object obj, string text)
        {
            var pdc = TypeDescriptor.GetProperties(obj);
            ...
            foreach (PropertyDescriptor pd in pdc)
            {
                ...
                XmlNode node = doc.SelectNodes(nodeName)[0];
                if (node != null)
                {
                    if (pd.PropertyType.IsEnum)
                    {
                        var objValue = Enum.Parse(pd.PropertyType, node.InnerXml);
                        pd.SetValue(obj, objValue);
                    }
                    else if (pd.PropertyType == typeof(Brush))
                    {
                        var objValue = new BrushConverter().ConvertFromString(node.InnerXml);
                        pd.SetValue(obj, objValue);
                    }
                    else
                    {
                        if (pd.PropertyType.IsValueType && pd.PropertyType != typeof(System.Windows.Rect) && !pd.IsReadOnly)
                        {
                            var objValue = Convert.ChangeType(node.InnerXml, pd.PropertyType, null);
                            pd.SetValue(obj, objValue);
                        }
                    }
                }
