...
namespace Syncfusion.Windows.Controls.Scroll
{
    ...
    public class AutoScroller : IDisposable
    {
        ...
        Size insideScrollMargins = new Size(1, 1);
        ...
        public virtual Rect InsideScrollBounds
        {
            get
            {
                ...
                if (!r.IsEmpty)
                {
                    ...
                    r.Width -= InsideScrollMargins.Width * 2;
                    r.Y += InsideScrollMargins.Height;
                    r.Height -= InsideScrollMargins.Height * 2;
#else
        ...
        protected virtual void CheckAutoScroll(Point mousePoint)
        {
                ...
                )
            {
                ...
                if ((AutoScrolling & AutoScrollOrientation.Vertical) != 0)
                {
                    ...
                    else if (mousePoint.Y > insideScrollBounds.Bottom &&(VScrollBar.Value + VScrollBar.LargeChange <= VScrollBar.Maximum || VScrollBar.Value > VScrollBar.Minimum))
                    {
                        inMouseDragScroll = true;
                        isLineDown = true;
                        host.LineDown();
