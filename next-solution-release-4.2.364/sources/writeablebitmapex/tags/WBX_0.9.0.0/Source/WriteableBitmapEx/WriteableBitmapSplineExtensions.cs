...
namespace System.Windows.Media.Imaging
{
   ...
   public static partial class WriteableBitmapExtensions
   {
      ...
      {
         bmp.DrawBezier(x1, y1, cx1, cy1, cx2, cy2, x2, y2, (color.A << 24) | (color.R << 16) | (color.G << 8) | color.B);
      }
      ...
      {
         bmp.DrawBeziers(points, (color.A << 24) | (color.R << 16) | (color.G << 8) | color.B);
      }
      ...
      /// <summary>
      /// Draws a Cardinal spline (cubic) defined by four control points.
      /// </summary>
      /// <param name="bmp">The WriteableBitmap.</param>
      ...
      /// <param name="color">The color.</param>
      private static void DrawCurve(this WriteableBitmap bmp, int x1, int y1, int x2, int y2, int x3, int y3, int x4, int y4, float tension, int color)
      {
         // Determine distances between controls points (bounding rect) to find the optimal stepsize
      ...
      {
         bmp.DrawCurve(points, tension, (color.A << 24) | (color.R << 16) | (color.G << 8) | color.B);
      }
      ...
      {
         // First segment
         bmp.DrawCurve(points[0], points[1], points[0], points[1], points[2], points[3], points[4], points[5], tension, color);

         // Middle segments
         int i;
         for (i = 2; i < points.Length - 4; i += 2)
         {
            bmp.DrawCurve(points[i - 2], points[i - 1], points[i], points[i + 1], points[i + 2], points[i + 3], points[i + 4], points[i + 5], tension, color);
         }

         // Last segment
         bmp.DrawCurve(points[i - 2], points[i - 1], points[i], points[i + 1], points[i + 2], points[i + 3], points[i + 2], points[i + 3], tension, color);
      }
