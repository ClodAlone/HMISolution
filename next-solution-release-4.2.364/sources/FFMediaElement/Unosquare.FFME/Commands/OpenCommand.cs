    ...
    internal sealed class OpenCommand : MediaCommand
    {
        ...
        internal override void ExecuteInternal()
        {
            ...
            try
            {
                ...
                m.runner.UIInvoke(DispatcherPriority.DataBind, () => { m.ResetDependencyProperies(); });
                ...
                m.Container = new MediaContainer(mediaUrl, m.Logger, Source.IsFile ? "async" : null, m.forceTCP, m.BypassPTSCheck);
                ...
                { IsBackground = true, Name = nameof(m.PacketReadingTask), Priority = m.IsHighPriority ? ThreadPriority.Normal : ThreadPriority.Lowest };

                m.FrameDecodingTask = new Thread(m.RunFrameDecodingWorker)
                { IsBackground = true, Name = nameof(m.FrameDecodingTask), Priority = m.IsHighPriority ? ThreadPriority.AboveNormal : ThreadPriority.BelowNormal };

                m.BlockRenderingTask = new Thread(m.RunBlockRenderingWorker)
                { IsBackground = true, Name = nameof(m.BlockRenderingTask), Priority = m.IsHighPriority ? ThreadPriority.Normal : ThreadPriority.Lowest };
                ...
                m.IsOpening = false;
                m.HasChangedWhileOpening = false;
            ...
            {
                m.IsOpening = false;
                m.runner.UIInvoke(DispatcherPriority.DataBind, () => { m.NotifyPropertyChanges(); });
