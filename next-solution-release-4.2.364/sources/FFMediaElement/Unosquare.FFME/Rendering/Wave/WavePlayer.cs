namespace Unosquare.FFME.Rendering.Wave
{
    ...
    internal class WavePlayer : IDisposable
    {
        ...
        public int DeviceNumber
        {
            ...
            set
            {
                ...
                lock (WaveOutLock)
                {
                    ...
                    WaveInterop.NativeMethods.waveOutGetDevCaps((IntPtr)m_DeviceNumber, out caps, Marshal.SizeOf(typeof(WaveOutCapabilities)));
                    Capabilities = caps;
                }
            }
        }
        ...
        public void Init(IWaveProvider waveProvider)
        {
            ...
            {
                throw new Exception(nameof(WaveInterop.NativeMethods.waveOutOpen));
            }
        ...
        public void Play()
        {
            ...
            if (m_PlaybackState == PlaybackState.Stopped)
            {
                ...
                AudioPlaybackTask = new Thread(StartPlaybackThread)
                {
                    ...
                    Priority = Renderer.MediaElement.IsHighPriority ? ThreadPriority.AboveNormal : ThreadPriority.BelowNormal
                };
        ...
        public void Stop()
        {
            ...
            //Waiting for StartPlaybackThread to finish before disposing resources
            m_PlaybackState = PlaybackState.Stopped;
            CallbackEvent?.Set();
            AudioPlaybackTask?.Join();
            //StartPlaybackThread finished
            ...
            if (result != MmResult.NoError)
                Renderer?.MediaElement?.Logger?.Log(MediaLogMessageType.Error, $"{nameof(WavePlayer)} stopping (non blocking). Result: {result.ToString()}. Mothod: {nameof(WaveInterop.NativeMethods.waveOutReset)}");
        ...
        protected void Dispose(bool alsoManaged)
        {
            ...
            catch (Exception e)
            {
                ...
                Renderer?.MediaElement?.Logger?.Log(MediaLogMessageType.Error, $"{nameof(WavePlayer)} disposing. {e.Message}. Stack Trace:\r\n{e.StackTrace}");
            }
