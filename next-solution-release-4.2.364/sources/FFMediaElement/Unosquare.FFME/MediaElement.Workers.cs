...
#pragma warning restore SA1401 // Fields must be private
        ...
        internal void RunPacketReadingWorker()
        {
            ...
            try
            {
                ...
                auxs = Container.Components.MediaTypes.Where(c => c != main && (c == MediaType.Audio || c == MediaType.Video)).ToArray();
                all = auxs.Union(new[] { main }).ToArray();
            }
            catch (Exception)
            {
                Logger?.Log(MediaLogMessageType.Warning, "PacketReadingWorker: Media Container was disposed or not available.");
                return;
            }
            ...
            while (IsTaskCancellationPending == false && !ct.IsCancellationRequested)
            {
                // Enter a read cycle
                SeekingDone.WaitOne();
        ...
        internal void RunFrameDecodingWorker()
        {
            ...
            try
            {
                ...
                auxs = Container.Components.MediaTypes.Where(x => x != main).ToArray();

                // Holds all components
                all = Container.Components.MediaTypes.ToArray();
            }
            catch (Exception)
            {
                Logger?.Log(MediaLogMessageType.Warning, "FrameDecodingWorker: Media Container was disposed or not available.");
                return;
            }
            ...
            while (IsTaskCancellationPending == false && !ct.IsCancellationRequested)
            {
                ...
                if (comp.PacketBufferCount > 0)
                {
                    ...
                    if (isInRange == false || Container.bypassPTSCheck)
                    {
        ...
        internal void RunBlockRenderingWorker()
        {
            ...
            try
            {
                ...
                // Holds the auxiliary media types
                auxs = Container.Components.MediaTypes.Where(t => t != main).ToArray();
            ...
            {
                Logger?.Log(MediaLogMessageType.Warning, "BlockRenderingWorker: Media Container was disposed or not available.");
                return;
            }
