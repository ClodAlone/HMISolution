...
public class CommandService
{
    ...
    /// <summary>Raised when a NumericPad command fires with the variable path.</summary>
    public event Action<string>? RequestNumericPad;

    /// <summary>Raised when an AlphaNumericPad command fires with the variable path.</summary>
    public event Action<string>? RequestAlphaNumericPad;
    ...
    public async Task ExecuteAsync(SymbolCommand cmd)
    {
        switch (cmd.Action)
        {
            ...
            case "NumericPad":
                if (!string.IsNullOrEmpty(cmd.VariablePath))
                    RequestNumericPad?.Invoke(cmd.VariablePath);
                break;

            case "AlphaNumericPad":
                if (!string.IsNullOrEmpty(cmd.VariablePath))
                    RequestAlphaNumericPad?.Invoke(cmd.VariablePath);
                break;
        }
    }
