...
public sealed class WebPushSender : IDisposable
{
    ...
    public WebPushSender(WebPushNotificationChannel config, string projectDirectory)
    {
        ...
        var subsFile = config.SubscriptionsFile;
        if (string.IsNullOrWhiteSpace(subsFile)) subsFile = "push-subscriptions.json";
        _subscriptionsFilePath = Path.IsPathRooted(subsFile)
            ? subsFile
            : Path.Combine(projectDirectory, subsFile);
    }

    // ──────────── Subscription storage ────────────

    public List<PushSubscriptionInfo> LoadSubscriptions()
    {
        try
        {
        ...
        {
            Log.Warning(ex, "Failed to load push subscriptions from {Path}", _subscriptionsFilePath);
            return [];
        }
    }

    public void SaveSubscriptions(List<PushSubscriptionInfo> subs)
    {
        try
        {
        ...
        {
            Log.Error(ex, "Failed to save push subscriptions to {Path}", _subscriptionsFilePath);
        }
    }

    public string SubscriptionsFilePath => _subscriptionsFilePath;
    ...
    private async Task<bool> SendPushAsync(PushSubscriptionInfo sub, string payload, string urgency)
    {
        ...
        Log.Warning("Web Push failed (HTTP {Status}) for {Endpoint}: {Body}",
            (int)response.StatusCode, sub.Endpoint, body);
    ...
    private byte[] EncryptPayload(PushSubscriptionInfo sub, byte[] plaintext)
    {
        ...
        // Extract the uncompressed point (65 bytes) from the SubjectPublicKeyInfo
        ...
        var sharedSecret = ecdh.DeriveKeyFromPublicKey(userEcdh.PublicKey);
    ...
    {
        // uncompressed[0] == 0x04, followed by X (32 bytes) and Y (32 bytes)
    ...
    private static byte[] ConvertDerToRaw(byte[] der)
    {
        ...
        // Copy right-aligned (skip leading 0x00 padding if present)
