...
public class CameraStreamService : IDisposable
{
    ...
    // Recording control wrappers used by external callers (HTTP API / UI)
    public void StartRecording(string cameraId, string? reason = null)
    {
        if (_cameras.TryGetValue(cameraId, out var instance))
            instance.StartRecording(reason ?? "manual");
    }
    ...
    public (bool IsRecording, string CurrentFile)? GetRecordingStatus(string cameraId)
    {
        if (_cameras.TryGetValue(cameraId, out var instance))
            return instance.GetRecordingStatus();
        return null;
    }

    public List<string>? ListRecordings(string cameraId)
    {
        if (_cameras.TryGetValue(cameraId, out var instance))
            return instance.ListRecordings();
        return null;
    }
    ...
    private class CameraInstance : IDisposable
    {
        ...
        public void Start()
        {
            ...
            // If recording mode is 'always' start background rolling recordings
            if (_config.RecordingEnabled && string.Equals(_config.RecordingMode, "always", StringComparison.OrdinalIgnoreCase))
            {
                _ = Task.Run(() => RollingRecordLoop(_cts.Token));
            }
        }

        // Rolling recorder used for RecordingMode == "always"
        private async Task RollingRecordLoop(CancellationToken ct)
        {
            try
            {
                while (!ct.IsCancellationRequested)
                {
                    try
                    {
                        StartRecording("always");
                        var maxSec = Math.Max(1, _config.RecordingMaxDurationSeconds);
                    ...
                    {
                        _logger.LogWarning(ex, "RollingRecordLoop error for {Id}", _config.CameraId);
                ...
                try
                {
                    ...
                    if (_onDetection != null && !string.IsNullOrEmpty(_config.DetectionVariablePrefix))
                    {
                        if (detections.Count > 0)
                        {
                            ...
                            // Trigger recording when configured for on_detection
                            try
                            {
                                if (_config.RecordingEnabled && string.Equals(_config.RecordingMode, "on_detection", StringComparison.OrdinalIgnoreCase)
                                    && best.Confidence >= _config.RecordingMinConfidence)
                                {
                                    StartRecording(best.Label);
                                }
                            }
                            catch (Exception ex)
                            {
                                _logger.LogDebug(ex, "Recording trigger error for {Id}", _config.CameraId);
                            }
                        }
                        else
                        {
        ...
        // Recording helpers already implemented above: ResolveRecordingFolder / StartRecording / StopRecording / GetRecordingStatus / ListRecordings
        private string ResolveRecordingFolder()
        {
            var folder = string.IsNullOrEmpty(_config.RecordingFolder) ? "Recordings" : _config.RecordingFolder;
            try
            {
            ...
            catch { folder = Path.GetFullPath(folder); }
            if (!Directory.Exists(folder)) Directory.CreateDirectory(folder);
            return folder;
        }

        public void StartRecording(string reasonLabel)
        {
            if (!_config.RecordingEnabled) return;
            lock (_recordingLock)
            {
                try
                {
                    ...
                    var safeLabel = string.IsNullOrEmpty(reasonLabel) ? "record" : string.Concat(reasonLabel.Where(ch => !Path.GetInvalidFileNameChars().Contains(ch)));
                    var fname = Path.Combine(folder, $"{_config.CameraId}_{safeLabel}_{DateTime.UtcNow:yyyyMMddHHmmss}.mp4");
                    _currentRecordingFile = fname;
                    ...
                    if (!string.IsNullOrEmpty(_config.Username))
                    {
                        ...
                        {
                            var u = new Uri(url);
                            url = $"{u.Scheme}://{_config.Username}:{_config.Password}@{u.Host}{(u.IsDefaultPort ? "" : ":" + u.Port)}{u.PathAndQuery}";
                        }
                        catch { }
                    }

                    var maxSec = Math.Max(1, _config.RecordingMaxDurationSeconds);
                    var args = string.Equals(_config.Protocol, "rtsp", StringComparison.OrdinalIgnoreCase)
                        ? $"-rtsp_transport tcp -i \"{url}\" -c:v libx264 -preset veryfast -t {maxSec} -y \"{fname}\""
                        : $"-i \"{url}\" -c:v libx264 -preset veryfast -t {maxSec} -y \"{fname}\"";
                    ...
                    {
                        _logger.LogInformation("Started recording {File} for camera {Id}", fname, _config.CameraId);
                        _ = Task.Run(() =>
                        {
                            try
                            {
                                var s = _recordingProcess.StandardError.ReadToEnd();
                                if (!string.IsNullOrEmpty(s)) _logger.LogDebug("ffmpeg: {s}", s);
                            }
                            catch { }
                        });
                    }
                ...
                {
                    _logger.LogWarning(ex, "Failed to start recording for {Id}", _config.CameraId);
                }
            }
        }
        ...
        public void StopRecording()
        {
            lock (_recordingLock)
            {
                ...
                {
                    _logger.LogDebug(ex, "Error stopping recording for {Id}", _config.CameraId);
                }
                finally
                {
        ...
        public (bool IsRecording, string CurrentFile) GetRecordingStatus()
        {
            lock (_recordingLock)
            {
                var isRec = _recordingProcess != null && !_recordingProcess.HasExited;
                return (isRec, _currentRecordingFile ?? "");
            }
        }
        ...
        public List<string> ListRecordings()
        {
            ...
            {
                var folder = ResolveRecordingFolder();
                return Directory.Exists(folder) ? Directory.GetFiles(folder, "*.mp4").OrderByDescending(f => File.GetLastWriteTimeUtc(f)).ToList() : new List<string>();
            }
