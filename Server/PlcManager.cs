    ...
    internal static class StParser
    {
        public static StProgram Parse(string code)
        {
            ...
            while (Peek(tokens, pos).Type == TokenType.Var)
            {
                ...
                {
                    var decl = ParseVarDeclaration(tokens, ref pos);
                    program.Variables.Add(decl);
                }
        ...
        private static StVarDeclaration ParseVarDeclaration(List<Token> tokens, ref int pos)
        {
            var decl = new StVarDeclaration();
            var nameToken = Advance(tokens, ref pos);
            decl.Name = nameToken.Text;

            Expect(tokens, ref pos, TokenType.Colon);

            var typeToken = Advance(tokens, ref pos);
            decl.TypeName = typeToken.Text.ToUpperInvariant();

            if (Peek(tokens, pos).Type == TokenType.Assign)
            {
                pos++; // :=
                decl.InitialValue = ParseExpression(tokens, ref pos);
            }
